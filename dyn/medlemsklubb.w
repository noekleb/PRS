...
DEFINE VARIABLE MKlubbBeskrivelse AS CHARACTER FORMAT "X(30)" 
     LABEL "Beskrivelse" 
     VIEW-AS FILL-IN 
     SIZE 32 BY 1.

DEFINE VARIABLE MKlubbId AS INTEGER FORMAT ">>9" INITIAL 0 
     LABEL "Klubbid" 
     VIEW-AS FILL-IN 
     SIZE 7 BY 1.
...
DEFINE FRAME frmDetalj
     MKlubbId AT ROW 1.95 COL 19 COLON-ALIGNED HELP
          "Medlemsklubb"
     MKlubbBeskrivelse AT ROW 2.95 COL 19 COLON-ALIGNED HELP
          "MKlubbBeskrivelse"
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 2 ROW 3.62
...
ON END-ERROR OF C-Win /* Medlemsklubb register */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Medlemsklubb register */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* Medlemsklubb register */
DO:
...
DO:
    ...
    DO WITH FRAME frmDetalj:
        APPLY "Entry" TO MKlubbBeskrivelse.            .
    END.
...
OCXFile = SEARCH( "medlemsklubb.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "medlemsklubb.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY MKlubbId MKlubbBeskrivelse 
      WITH FRAME frmDetalj IN WINDOW C-Win.
  ENABLE MKlubbId MKlubbBeskrivelse 
      WITH FRAME frmDetalj IN WINDOW C-Win.
...
DO WITH FRAME frmListe:
  hBrowseListe = DYNAMIC-FUNCTION("NewBrowse",         /* Create a browse object */
                    ...
                    "Gavekort;butnr;IdentType;Dato|Reg.dato;+cRegTid|CHARACTER|x(5)|gavekort_regtid.p|Tid;Gyldigdato;Bruktdato;KasseNr;BongNr;Belop;!Tid," + 
                      "GaveKType;GKTMKlubbBeskrivelse",                     /* Buffers and fields: <buffer1>;<field1>;<field2>..,<buffer2>;.. No fields gives all*/
                    */
                    "Medlemsklubb;MKlubbId|Knd.grp.nr|>>>9;MKlubbBeskrivelse;EDato;BrukerId",
                    "WHERE true",
                    "sort|MKlubbId").                            /* Misc - for something I might need in next version.. */
  ...
  */
  ...
  /*DYNAMIC-FUNCTION("setAttribute",hBrowseListe,"basequery","WHERE Individ.MKlubbId = 1 ").*/
...
/*     DYNAMIC-FUNCTION("setAttribute",hFieldMap,"disabledFields","MKlubbId"). */
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
      ASSIGN
          MKlubbId:SENSITIVE = TRUE
          .
      APPLY "ENTRY" TO MKlubbId.
      RETURN NO-APPLY.
  END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
    ...
    IF cState = "NEW" THEN
    DO:
      IF int(MKlubbId:screen-value) = 0 THEN
      DO:
          MESSAGE "Angi en innbetalingstype større enn 0."
              VIEW-AS ALERT-BOX WARNING BUTTONS OK.
          RETURN NO-APPLY.
      END.
      IF int(MKlubbId:screen-value) = int(DYNAMIC-FUNCTION("getFieldValues","Medlemsklubb","WHERE MKlubbId = int(" + MKlubbId:SCREEN-VALUE + ")","MKlubbId")) THEN
      DO:
          MESSAGE "MKlubbId eksisterer fra før med type " + MKlubbId:SCREEN-VALUE + "."
