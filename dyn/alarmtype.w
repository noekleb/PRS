...
DEFINE VARIABLE AlarmTypeId AS INTEGER FORMAT ">9" INITIAL 0 
     LABEL "AlarmTypeId" 
     VIEW-AS FILL-IN 
     SIZE 7 BY 1.

DEFINE VARIABLE ATypeBeksr AS CHARACTER FORMAT "X(30)" 
     LABEL "ATypeBeksr" 
     VIEW-AS FILL-IN 
     SIZE 32 BY 1.
...
DEFINE FRAME frmDetalj
     AlarmTypeId AT ROW 1.95 COL 19 COLON-ALIGNED HELP
          "AlarmType"
     ATypeBeksr AT ROW 2.95 COL 19 COLON-ALIGNED HELP
          "ATypeBeksr"
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 2 ROW 3.62
...
ON END-ERROR OF C-Win /* AlarmType register */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* AlarmType register */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* AlarmType register */
DO:
...
OCXFile = SEARCH( "alarmtype.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "alarmtype.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY AlarmTypeId ATypeBeksr 
      WITH FRAME frmDetalj IN WINDOW C-Win.
  ENABLE AlarmTypeId ATypeBeksr 
      WITH FRAME frmDetalj IN WINDOW C-Win.
...
DO WITH FRAME frmListe:
  hBrowseListe = DYNAMIC-FUNCTION("NewBrowse",         /* Create a browse object */
                    ...
                    "Gavekort;butnr;IdentType;Dato|Reg.dato;+cRegTid|CHARACTER|x(5)|gavekort_regtid.p|Tid;Gyldigdato;Bruktdato;KasseNr;BongNr;Belop;!Tid," + 
                      "GaveKType;GKTATypeBeksr",                     /* Buffers and fields: <buffer1>;<field1>;<field2>..,<buffer2>;.. No fields gives all*/
                    */
                    "AlarmType;AlarmTypeId|Knd.grp.nr|>>>9;ATypeBeksr;EDato;BrukerId",
                    "WHERE true",
                    "sort|AlarmTypeId").                            /* Misc - for something I might need in next version.. */
  ...
  */
  ...
  /*DYNAMIC-FUNCTION("setAttribute",hBrowseListe,"basequery","WHERE Individ.AlarmTypeId = 1 ").*/
...
/*     DYNAMIC-FUNCTION("setAttribute",hFieldMap,"disabledFields","AlarmTypeId"). */
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
      ASSIGN
          AlarmTypeId:SENSITIVE = TRUE
          .
      APPLY "ENTRY" TO AlarmTypeId.
      RETURN NO-APPLY.
  END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
    ...
    IF cState = "NEW" THEN
    DO:
      IF int(AlarmTypeId:screen-value) = 0 THEN
      DO:
          MESSAGE "Angi en innbetalingstype større enn 0."
              VIEW-AS ALERT-BOX WARNING BUTTONS OK.
          RETURN NO-APPLY.
      END.
      IF int(AlarmTypeId:screen-value) = int(DYNAMIC-FUNCTION("getFieldValues","AlarmType","WHERE AlarmTypeId = int(" + AlarmTypeId:SCREEN-VALUE + ")","AlarmTypeId")) THEN
      DO:
          MESSAGE "AlarmTypeId eksisterer fra før med type " + AlarmTypeId:SCREEN-VALUE + "."
