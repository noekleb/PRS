...
DEFINE VARIABLE FILL-IN-20 AS CHARACTER FORMAT "X(256)":U 
     LABEL "Måste enablas vid översätting" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME DEFAULT-FRAME
     FILL-IN-20 AT ROW 1.24 COL 67 COLON-ALIGNED
     rectWinToolbar AT ROW 1.24 COL 197
...
ON END-ERROR OF C-Win /* Salgsbudsjett */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Salgsbudsjett */
DO:
...
/*     DYNAMIC-FUNCTION("setAttribute",SESSION,"tabFolderProg","JBoxJlwTabFolder.w"). */
  ...
  /* NB: !!!!! Denne koden skal egentlig kjøres lenger nede, men da vises inten bilder. Nå vises bare på første flik???? */
  IF SEARCH("controls.dll") NE ? THEN
    DYNAMIC-FUNCTION("setImageList" IN hTabFolder,"ico\3d_bar_chart.ico;ico\Accounting.ico").
  
  /* Bygger først første side og knytter opp browser. */
  DYNAMIC-FUNCTION("InitPages" IN hTabFolder,
                   "Budsjett|sbudhode_brw.w"
                   ,?).
  ...
  DYNAMIC-FUNCTION("addPage" IN hTabFolder,"Budsjett linjer","sbudlinje_brw.w","Accounting.ico").
...
ELSE
  DYNAMIC-FUNCTION("CreateParentLink",hCurrTabQuery,hBrowse,"SBudId,SBudId").
