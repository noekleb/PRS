...
DEFINE VARIABLE TerskelTildeling AS DECIMAL FORMAT "->,>>,>>9.99" INITIAL 0 
     LABEL "Terskel" 
     VIEW-AS FILL-IN 
     SIZE 20.2 BY 1 TOOLTIP "Terskelbeløp som det må være kjøpt for for å få tildelt rabattsjekk.".

DEFINE VARIABLE VerdiPaSjekk AS DECIMAL FORMAT "->>,>>9.99" INITIAL 0 
     LABEL "Verdi" 
     VIEW-AS FILL-IN 
     SIZE 20.2 BY 1 TOOLTIP "Verdi på rabattsjekk som tildeles.".
  ...
  ASSIGN
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        IF TRIM(RabSjekkTypeBeskrivelse:SCREEN-VALUE) = "" THEN DO:
            MESSAGE "Beskrivelse må legges inn."
                VIEW-AS ALERT-BOX INFO BUTTONS OK.
