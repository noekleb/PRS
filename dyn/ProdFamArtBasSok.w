...
DEF VAR hProdFamArtikkelNr     AS HANDLE NO-UNDO.
DEF VAR hArtNr                 AS HANDLE NO-UNDO.
DEF VAR hProdFamId             AS HANDLE NO-UNDO.
DEF VAR fProdFamId             AS DEC    NO-UNDO.
DEF VAR fKupongId              AS DEC    NO-UNDO.
...
sokLevKod sokStr btnStr rsButBeholdning tbLager sokPrisMin fiKjoptMnd ~
tbOPris tbTilbud rsButTidlKjop tbTidlKjop sokPrisMaks fiKjoptMndLabel 
...
sokVgBeskr sokLevKod sokStr rsButBeholdning tbLager sokPrisMin fiKjoptMnd ~
tbOPris tbTilbud rsButTidlKjop tbTidlKjop sokPrisMaks fiKjoptMndLabel 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD setKupongId C-Win 
FUNCTION setKupongId RETURNS LOGICAL
...
FUNCTION setMinMaksPris RETURNS LOGICAL
  (INPUT ipiMinMaksPris AS INT,
   INPUT ipiPrisBase    AS INT)  FORWARD.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD setProdFamId C-Win 
FUNCTION setProdFamId RETURNS LOGICAL
...
DEFINE VARIABLE sokPrisMaks AS INTEGER FORMAT "->,>>>,>>9":U INITIAL 0 
     LABEL "Maks pris" 
     VIEW-AS FILL-IN 
     SIZE 14 BY 1 NO-UNDO.

DEFINE VARIABLE sokPrisMin AS INTEGER FORMAT "->,>>>,>>9":U INITIAL 0 
     LABEL "Min. pris" 
     VIEW-AS FILL-IN 
     SIZE 14 BY 1 NO-UNDO.
...
DEFINE FRAME DEFAULT-FRAME
     ...
     sokPrisMin AT ROW 6.62 COL 41 COLON-ALIGNED
...
&Scoped-define SELF-NAME sokPrisMaks
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL sokPrisMaks C-Win
ON RETURN OF sokPrisMaks IN FRAME DEFAULT-FRAME /* Maks pris */
DO:
  IF SELF:MODIFIED THEN DO:
    RUN StartQuery.
    RETURN NO-APPLY.
...
&Scoped-define SELF-NAME sokPrisMin
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL sokPrisMin C-Win
ON RETURN OF sokPrisMin IN FRAME DEFAULT-FRAME /* Min. pris */
DO:
  IF SELF:MODIFIED THEN DO:
    RUN StartQuery.
    RETURN NO-APPLY.
...
DO:
  IF tbOpris:CHECKED THEN
    ASSIGN 
      sokPrisMin:SCREEN-VALUE  = "0"
      sokPrisMin:SENSITIVE     = FALSE
      sokPrisMaks:SCREEN-VALUE = "0"
      sokPrisMaks:SENSITIVE    = FALSE
    .
  ELSE
    ASSIGN 
      sokPrisMin:SENSITIVE  = TRUE
      sokPrisMaks:SENSITIVE = TRUE
    .
...
IF DYNAMIC-FUNCTION("getCurrentObject") = hBrwArtikkel THEN DO:
  IF hBrwStr:QUERY:NUM-RESULTS > 0 AND hBuffStr:AVAIL AND NOT bCloseOnSelect THEN DO:
  ...
  ELSE IF NOT bCloseOnSelect THEN DO:
    DYNAMIC-FUNCTION("DoMessage",0,0,"Det finnes ingen strekkode","","").
    RETURN.
  END.
  ELSE 
    bOk = DYNAMIC-FUNCTION("AddStr" IN hParent,
...
IF DYNAMIC-FUNCTION("getCurrentObject") = hBrwArtikkel THEN DO:
  ...
  IF hBuffer:AVAIL THEN DO:
    hDummy = DYNAMIC-FUNCTION("getTempTable","kampartbas_str_beh.p",
...
------------------------------------------------------------------------------*/
  DISPLAY sokBeskr sokAvdelingNr sokAvdelingNavn sokArtikkelNr sokStrKod sokHg 
          ...
          rsButBeholdning tbLager sokPrisMin fiKjoptMnd tbOPris tbTilbud 
          rsButTidlKjop tbTidlKjop sokPrisMaks fiKjoptMndLabel 
...
/*                            ELSE "")                                                                                        */
                        + (IF fProdFamId NE 0 THEN
                            ",FIRST ProduktFamMedlem WHERE ProduktFamMedlem.ProdFamId = DEC(" + STRING(fProdFamId) + ")" 
                            + " AND ProduktFamMedlem.ProdFamArtikkelNr = ArtBas.ArtikkelNr OUTER-JOIN"
                           ELSE "")
         cArtBasFields  = "ArtBas"
                        ...
                        + (IF fKOrdreId NE 0 THEN 
                           ...
                           ELSE "")
/*                         + (IF fKampId NE 0 THEN                                                             */
/*                             ",KampanjeTilbArtikkel;!KampId;!KampTilbId;!KampTilbArtId;!ProdFamId;!KupongId" */
/*                            ELSE "")                                                                         */
                        + (IF fProdFamId NE 0 THEN 
                            ",ProduktFamMedlem;!ProdFamId;!ProdFamArtikkelNr"
                           ELSE "")                      
...
/*   IF bCloseOnSelect THEN BrwArtikkel:WIDTH-PIXELS = BrwArtikkel:WIDTH-PIXELS + 200.  */
  ...
  DYNAMIC-FUNCTION("setAttribute",hBrwArtikkel,"calcfieldproc","artbassok2_brwcalc.p").
  
    hTilbField         = hBuffer:BUFFER-FIELD("TilbPris").
    hPaaOrdreField     = hBuffer:BUFFER-FIELD("KOrdre_Id") NO-ERROR.
    hProdFamArtikkelNr = hBuffer:BUFFER-FIELD("ProdFamArtikkelNr") NO-ERROR.
    hArtNr             = hBuffer:BUFFER-FIELD("ArtikkelNr") NO-ERROR.
    hProdFamId         = hBuffer:BUFFER-FIELD("ProdFamId") NO-ERROR.
...
/*                    ,"SeqNr").                                    */
  ...
  ASSIGN hBrwStr:GET-BROWSE-COLUMN(1):WIDTH-PIXELS = 103
...
/*   IF sokPrisMaks NE 0 THEN                                                                         */
/*     cCurrPrisJoin = ",FIRST ArtPris OF ArtBas NO-LOCK WHERE Pris LE " + sokPrisMaks:SCREEN-VALUE.  */
...
/*     DYNAMIC-FUNCTION("setAttribute",hBrwArtikkel,"queryjoin",cArtBasJoin + cCurrPrisJoin).                          */
  ...
  DYNAMIC-FUNCTION("setAttribute",hBrwArtikkel,"calcparamPris",cButikkNr + "¤" + sokPrisMin:SCREEN-VALUE + "¤" + sokPrisMaks:SCREEN-VALUE).
  DYNAMIC-FUNCTION("setAttribute",hBrwArtikkel,"calcparamInnPris",cButikkNr).
...
IF DYNAMIC-FUNCTION("getCurrentObject") = hBrwArtikkel THEN DO: 
  ...
  IF fProdFamId NE 0 THEN 
      IF VALID-HANDLE(hProdFamId) AND hProdFamArtikkelNr:BUFFER-VALUE = hArtNr:BUFFER-VALUE THEN
        hArtNrColumn:BGCOLOR = 10.
END.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION setKupongId C-Win 
FUNCTION setKupongId RETURNS LOGICAL
...
------------------------------------------------------------------------------*/
  DO WITH FRAME {&FRAME-NAME}:
    ASSIGN 
      tboPris:SENSITIVE        = ipiMinMaksPris = 0
      sokPrisMin:SCREEN-VALUE  = STRING(ipiPrisBase - ipiMinMaksPris)
      sokPrisMaks:SCREEN-VALUE = STRING(ipiPrisBase + ipiMinMaksPris)
      sokPrisMin:SENSITIVE     = ipiMinMaksPris = 0
      sokPrisMaks:SENSITIVE    = ipiMinMaksPris = 0
    .
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION setProdFamId C-Win 
FUNCTION setProdFamId RETURNS LOGICAL
