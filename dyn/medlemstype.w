...
DEFINE VARIABLE MedType AS INTEGER FORMAT ">>>9" INITIAL 0 
     LABEL "MedType" 
     VIEW-AS FILL-IN 
     SIZE 7 BY 1.
...
DEFINE FRAME frmDetalj
     MedType AT ROW 1.95 COL 19 COLON-ALIGNED HELP
          "Medlemstype"
...
ON END-ERROR OF C-Win /* Medlemstype register */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Medlemstype register */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* Medlemstype register */
DO:
...
OCXFile = SEARCH( "medlemstype.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "medlemstype.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY MedType Beskrivelse 
      WITH FRAME frmDetalj IN WINDOW C-Win.
  ENABLE MedType Beskrivelse 
      WITH FRAME frmDetalj IN WINDOW C-Win.
...
DO WITH FRAME frmListe:
  hBrowseListe = DYNAMIC-FUNCTION("NewBrowse",         /* Create a browse object */
                    ...
                    */
                    "Medlemstype;MedType|Knd.grp.nr|>>>9;Beskrivelse;EDato;BrukerId",
                    "WHERE true",
                    "sort|MedType").                            /* Misc - for something I might need in next version.. */
  ...
  */
  ...
  /*DYNAMIC-FUNCTION("setAttribute",hBrowseListe,"basequery","WHERE Individ.MedType = 1 ").*/
...
/*     DYNAMIC-FUNCTION("setAttribute",hFieldMap,"disabledFields","MedType"). */
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
      ASSIGN
          MedType:SENSITIVE = TRUE
          .
      APPLY "ENTRY" TO MedType.
      RETURN NO-APPLY.
  END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
    ...
    IF cState = "NEW" THEN
    DO:
      IF int(MedType:screen-value) = 0 THEN
      DO:
          MESSAGE "Angi en innbetalingstype større enn 0."
              VIEW-AS ALERT-BOX WARNING BUTTONS OK.
          RETURN NO-APPLY.
      END.
      IF int(MedType:screen-value) = int(DYNAMIC-FUNCTION("getFieldValues","Medlemstype","WHERE MedType = int(" + MedType:SCREEN-VALUE + ")","MedType")) THEN
      DO:
          MESSAGE "MedType eksisterer fra før med type " + MedType:SCREEN-VALUE + "."
