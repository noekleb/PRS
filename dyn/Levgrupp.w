...
DEFINE BUTTON B-Levgruppmedlem 
     LABEL "Välj leverantörer till grupp" 
     SIZE 29 BY 1.14.

DEFINE VARIABLE Gruppnamn AS CHARACTER FORMAT "x(25)" 
     LABEL "Gruppnamn" 
     VIEW-AS FILL-IN 
     SIZE 37 BY 1.

DEFINE VARIABLE Gruppnr AS INTEGER FORMAT "zz9" INITIAL 0 
     LABEL "Gruppnr" 
     VIEW-AS FILL-IN 
     SIZE 6.2 BY 1.
...
DEFINE FRAME frmDetalj
     Gruppnr AT ROW 1.48 COL 12 COLON-ALIGNED HELP
          "Brukergruppenummer."
     Gruppnamn AT ROW 2.48 COL 12 COLON-ALIGNED
     B-Levgruppmedlem AT ROW 4.14 COL 1.4
...
/* SETTINGS FOR FRAME frmDetalj
                                                                        */
/* SETTINGS FOR FILL-IN Gruppnr IN FRAME frmDetalj
   NO-ENABLE                                                            */
...
ON END-ERROR OF C-Win /* Leverantörsgrupper */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Leverantörsgrupper */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* Leverantörsgrupper */
DO:
...
&Scoped-define SELF-NAME B-Levgruppmedlem
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-Levgruppmedlem C-Win
ON CHOOSE OF B-Levgruppmedlem IN FRAME frmDetalj /* Välj leverantörer till grupp */
DO:
    /* Henter liste med koblinger */
    cLevbasRowIdList = DYNAMIC-FUNCTION("getRowIdList","Levgruppmedlem,Gruppnr","",
                                         "WHERE Gruppnr = " + Gruppnr:SCREEN-VALUE IN FRAME frmDetalj +
                                         ",FIRST Levbas OF levgruppmedlem NO-LOCK").
    ...
    bOk = DYNAMIC-FUNCTION("runProc","lagrelevbasgrupp.p",Gruppnr:SCREEN-VALUE + "|" + cLevbasRowIdList,?).
    IF bOk THEN
...
&Scoped-define SELF-NAME Gruppnr
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Gruppnr C-Win
ON TAB OF Gruppnr IN FRAME frmDetalj /* Gruppnr */
OR "RETURN" OF GruppNr DO:
  APPLY "ENTRY" TO Gruppnamn.
  RETURN NO-APPLY.
END.
...
END.
ELSE MESSAGE "Levgrupp.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  bOk = DYNAMIC-FUNCTION("runProc","slettlevbasgrupp.p",Gruppnr:SCREEN-VALUE IN FRAME frmDetalj,?).
  IF bOk <> TRUE THEN
      RETURN NO-APPLY.
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY Gruppnr Gruppnamn 
      WITH FRAME frmDetalj IN WINDOW C-Win.
  ENABLE RECT-62 rectBrowseButikktilgang Gruppnamn B-Levgruppmedlem 
      WITH FRAME frmDetalj IN WINDOW C-Win.
...
/*                     "Levgrupp;Gruppnr;Gruppnamn;!Knappeliste;!Navn", */
                    "Levgrupp;Gruppnr;Gruppnamn",
                    "WHERE true",
                    "sort|Gruppnr").                            /* Misc - for something I might need in next version.. */
...
DO WITH FRAME frmDetalj:
  ...
  */
  ...
  hBrowseButikktilgang = DYNAMIC-FUNCTION("NewBrowse",         /* Create a browse object */
                    ...
                    */
                    "Levgruppmedlem;levnr,Levbas;levnamn",
                    "WHERE false, first Levbas no-lock where levbas.levnr = Levgruppmedlem.levnr",
/*                     "WHERE Gruppnr = '" + Gruppnr:SCREEN-VALUE IN FRAME frmDetalj + "', first Butiker no-lock where Butiker.butik = ButikkTilgang.Butik", */
...
/*   DYNAMIC-FUNCTION("NewBrowseSearchField",RectBrowseSearchListe:HANDLE,hBrowseListe,1). */
  ...
  */
  ...
  DYNAMIC-FUNCTION("createParentLink",hBrowseButikktilgang,hBrowseListe,'Gruppnr').
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
    ...
    IF cState = "NEW" THEN
    DO:
      IF (Gruppnr:screen-value) = (DYNAMIC-FUNCTION("getFieldValues","LevGrupp","WHERE Gruppnr = '" + Gruppnr:SCREEN-VALUE + "'","Gruppnr")) THEN
      DO:
          MESSAGE "Grupp redan registrerad " + Gruppnr:SCREEN-VALUE + "."
