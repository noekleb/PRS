...
ON END-ERROR OF C-Win /* Varetelling */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Varetelling */
DO:
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  DYNAMIC-FUNCTION("InitPages" IN hTabFolder,
                   "Telleliste|tellehode_brw.w"
                   + "|Tellelinje|tellelinje_brw.w"
                   + "|Lokasjonsliste|tellelinje_brw2.w"
                   + "|Fillinjer|vpifilhode_brw.w"
                   ,hBrowse).
  
  DYNAMIC-FUNCTION("buildFolder" IN hTabFolder).
  
  hBrowse  = DYNAMIC-FUNCTION("getPageQuery" IN hTabFolder,1).
  hBrowse2 = DYNAMIC-FUNCTION("getPageQuery" IN hTabFolder,3).
  
  hPage = DYNAMIC-FUNCTION("getPageHandle" IN hTabFolder,2).
  DYNAMIC-FUNCTION("setQuery" IN hPage,hBrowse).

  hPage = DYNAMIC-FUNCTION("getPageHandle" IN hTabFolder,3).
  DYNAMIC-FUNCTION("setQuery" IN hPage,hBrowse).
  
  hPage = DYNAMIC-FUNCTION("getPageHandle" IN hTabFolder,4).
  DYNAMIC-FUNCTION("setQuery" IN hPage,hBrowse).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE sjekkDefaultTelleModul C-Win 
PROCEDURE sjekkDefaultTelleModul :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  /*
  IF NOT (CAN-FIND(FIRST EkstVPIFil WHERE
              EkstVPIFil.EkstVPILevNr = 893 AND
              EkstVPIFil.VPIFilNr = 1 AND
              EKSTVPIFil.VPIFilType = 8 AND
              EkstVPIFil.VPIFilNavn = "TELLING" AND
              EkstVPIFil.VPIFilAktiv = TRUE) OR 
          CAN-FIND(FIRST EkstVPIFil WHERE
             EkstVPIFil.EkstVPILevNr = 894 AND
             EkstVPIFil.VPIFilNr = 1 AND
             EKSTVPIFil.VPIFilType = 8 AND
             EkstVPIFil.VPIFilNavn = "VARETRAN" AND
             EkstVPIFil.VPIFilAktiv = TRUE)) THEN
  DO: /* - -----  */
    MESSAGE "Denne varetellingsmodulen er ikke satt som standard." SKIP
            "Skal den settes som standard?" SKIP(1)
            "Hvis ja, vil det medføre at automatisk innlesning av tellefiler aktiveres."
    ...
    ELSE DO TRANSACTION:
        ...
        FIND FIRST EkstVPIFil WHERE
              ...
              EKSTVPIFil.VPIFilType = 8 AND
              EkstVPIFil.VPIFilNavn = "TELLING" AND
              EkstVPIFil.VPIFilAktiv = FALSE NO-ERROR.
    ...
    MESSAGE "Denne tellemodulen er nå standard tellemodul." SKIP
            "Automatisk innlesning av tellefiler fra håndterminal er ekativert."
...
IF iiTab = 3 OR iiTab = 4 THEN
  DYNAMIC-FUNCTION("CreateParentLink",hCurrTabQuery,hBrowse,"koblettiltellenr;tellenr").
ELSE
  DYNAMIC-FUNCTION("CreateParentLink",hCurrTabQuery,hBrowse,"tellenr").
