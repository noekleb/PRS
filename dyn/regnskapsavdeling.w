...
&Scoped-Define ENABLED-OBJECTS BUTTON-SokRegnAvd rectBrowse rectToolBar ~
rectWinToolbar RAvdNr RAvdBeskrivelse OmradeAnsvarlig 
&Scoped-Define DISPLAYED-OBJECTS RAvdNr RAvdBeskrivelse OmradeAnsvarlig 
...
DEFINE VARIABLE OmradeAnsvarlig AS CHARACTER FORMAT "X(10)" 
     LABEL "Ansvarlig" 
     VIEW-AS FILL-IN 
     SIZE 15.6 BY 1.

DEFINE VARIABLE RAvdBeskrivelse AS CHARACTER FORMAT "X(256)":U 
     LABEL "Beskrivelse" 
     VIEW-AS FILL-IN 
     SIZE 35 BY 1 NO-UNDO.

DEFINE VARIABLE RAvdNr AS INTEGER FORMAT "->,>>>,>>9":U INITIAL 0 
     LABEL "Områdenr." 
     VIEW-AS FILL-IN 
...
DEFINE FRAME DEFAULT-FRAME
     BUTTON-SokRegnAvd AT ROW 4.86 COL 116.4
     RAvdNr AT ROW 2.67 COL 98.6 COLON-ALIGNED
     RAvdBeskrivelse AT ROW 3.76 COL 98.6 COLON-ALIGNED
     OmradeAnsvarlig AT ROW 4.81 COL 98.6 COLON-ALIGNED HELP
          "Brukers ID i systemet."
     rectBrowse AT ROW 2.43 COL 2
     rectToolBar AT ROW 1.14 COL 1.8
     rectWinToolbar AT ROW 1.1 COL 126
...
ON END-ERROR OF C-Win /* Vareområderegister */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Vareområderegister */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* Vareområderegister */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-SokRegnAvd C-Win
ON CHOOSE OF BUTTON-SokRegnAvd IN FRAME DEFAULT-FRAME /* ... */
or F10 of OmradeAnsvarlig
DO:
  DO WITH FRAME Default-Frame:
      DEF VAR cLookupValue AS CHAR NO-UNDO.

      cLookupValue = "BrukerId".

      RUN JBoxDLookup.w ("Bruker;BrukerId;Navn", "where true", INPUT-OUTPUT cLookupValue).

      IF cLookupValue NE "" THEN DO:
        OmradeAnsvarlig:SCREEN-VALUE = cLookupValue.
        DYNAMIC-FUNCTION("setToolbar",rectToolBar:HANDLE,"modified").
      END.
  END.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE CopyRecordxxx C-Win 
PROCEDURE CopyRecordxxx :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DO WITH FRAME {&FRAME-NAME}:
  RAvdNr:READ-ONLY = FALSE.
  APPLY 'ENTRY' TO RAvdNr.
END.
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  DISPLAY RAvdNr RAvdBeskrivelse OmradeAnsvarlig 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE BUTTON-SokRegnAvd rectBrowse rectToolBar rectWinToolbar RAvdNr 
         RAvdBeskrivelse OmradeAnsvarlig 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
DO WITH FRAME {&FRAME-NAME}:
  hBrowse = DYNAMIC-FUNCTION("NewBrowse",          /* Create a browse object */
                    ...
                    "RegnskapsAvdeling"
                    + ";RAvdNr;RAvdBeskrivelse;OmradeAnsvarlig|Områdeansvarlig;!RegistrertTid;!RegistrertDato;!RegistrertAv;!ETid;!EDato;!BrukerID"
                   ,"WHERE FALSE"
                    ,"sort|RAvdNr").             /* Initial sort column */
  ...
  hFieldMap = DYNAMIC-FUNCTION("NewFieldMap",      /* A fieldmap object holds extra info for display and input fields (fill-ins) 
                    ...
                    "RAvdNr,RAvdBeskrivelse,Omradeansvarlig",   /* Update columns in buffer */
...
IF DYNAMIC-FUNCTION("getToolbarState",hToolbar) = "new" THEN DO:
  DYNAMIC-FUNCTION("setAttribute",hFieldMap,"BufferExtraFields","RAvdNr").
  DYNAMIC-FUNCTION("setAttribute",hFieldMap,"BufferExtraValues",RAvdNr:SCREEN-VALUE IN FRAME {&FRAME-NAME}).
END.
ELSE DO:
  DYNAMIC-FUNCTION("setAttribute",hFieldMap,"BufferExtraFields","").
  DYNAMIC-FUNCTION("setAttribute",hFieldMap,"BufferExtraValues","").
END.
RAvdNr:READ-ONLY IN FRAME {&FRAME-NAME} = TRUE.
