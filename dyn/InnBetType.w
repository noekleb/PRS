...
DEFINE VARIABLE InnBetTId AS INTEGER FORMAT "9" INITIAL 0 
     LABEL "Innbetalingstype" 
     VIEW-AS COMBO-BOX INNER-LINES 25
     LIST-ITEM-PAIRS "Type 0",0,
                     "Type 1",1,
                     "Type 2",2,
                     "Type 3",3,
                     "Type 4",4,
...
DEFINE VARIABLE InnBBeskrivelse AS CHARACTER FORMAT "X(30)" 
     LABEL "Beskrivelse" 
     VIEW-AS FILL-IN 
     SIZE 46 BY 1.

DEFINE VARIABLE Kontonavn AS CHARACTER FORMAT "X(30)" 
     VIEW-AS FILL-IN 
     SIZE 31.2 BY 1.
...
DEFINE FRAME frmDetalj
     ...
     InnBetTId AT ROW 1.95 COL 19 COLON-ALIGNED HELP
          "Innbetalingstype"
     InnBBeskrivelse AT ROW 2.95 COL 19 COLON-ALIGNED HELP
          "Beskrivelse av innbetalingstype."
     KontoNr AT ROW 3.95 COL 19 COLON-ALIGNED HELP
          "Kontonummer i regnskap"
     Kontonavn AT ROW 3.95 COL 33.8 COLON-ALIGNED HELP
          "Kontonavn" NO-LABEL
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 2 ROW 3.62
...
/* SETTINGS FOR FRAME frmDetalj
                                                                        */
/* SETTINGS FOR FILL-IN Kontonavn IN FRAME frmDetalj
   NO-ENABLE                                                            */
...
ON END-ERROR OF C-Win /* Innbetalingstyper */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Innbetalingstyper */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* Innbetalingstyper */
DO:
...
DO:
  ...
  cLookupValue = "KontoNr".
  RUN JBoxDLookup.w ("Kontotabell;KontoNr;KontoNavn","where true",INPUT-OUTPUT cLookupValue).

  IF cLookupValue NE "" THEN 
  DO:
    KontoNr:SCREEN-VALUE = cLookupValue.
    APPLY "ANY-PRINTABLE":U TO KontoNr.
  END.
...
ON RETURN OF KontoNr IN FRAME frmDetalj /* Kontonummer */
OR "TAB" OF KontoNr
DO:
  IF SELF:MODIFIED THEN
  DO:
      KontoNavn:SCREEN-VALUE = DYNAMIC-FUNCTION("getFieldValues","Kontotabell",
                                              "WHERE KontoNr = '" + KontoNr:SCREEN-VALUE + "'","KontoNavn").  
  END.
END.
...
DO:
    ...
    DO WITH FRAME frmDetalj:
        APPLY "Entry" TO InnBBeskrivelse.            .
    END.
...
OCXFile = SEARCH( "InnBetType.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "InnBetType.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY InnBetTId InnBBeskrivelse KontoNr Kontonavn 
      WITH FRAME frmDetalj IN WINDOW C-Win.
  ENABLE btnSokBrGrp-2 InnBetTId InnBBeskrivelse KontoNr 
      WITH FRAME frmDetalj IN WINDOW C-Win.
...
cAdgang = (DYNAMIC-FUNCTION("getFieldValues","SysPara",
                            "WHERE SysHId = 16 and SysGr = 36 and ParaNr = 1","Parameter1")).  
...
DO WITH FRAME frmListe:
  hBrowseListe = DYNAMIC-FUNCTION("NewBrowse",         /* Create a browse object */
                    ...
                    */
                    "InnBetType;InnBetTId;InnBBeskrivelse;KontoNr,KontoTabell;KontoNavn",
                    "WHERE true, FIRST KontoTabell NO-LOCK where KontoTabell.KontoNr = InnBetType.KontoNr OUTER-JOIN",
                    "sort|InnBetTId").                            /* Misc - for something I might need in next version.. */
  ...
  */
  ...
  /*DYNAMIC-FUNCTION("setAttribute",hBrowseListe,"basequery","WHERE Individ.InnBetTId = 1 ").*/
...
DO WITH FRAME frmDetalj:
  ...
  hFieldMap = DYNAMIC-FUNCTION("NewFieldMap",      /* A fieldmap object holds extra info for display and input fields (fill-ins)
                    ...
                    "InnBetTId,InnBBeskrivelse,KontoNr"
                       ,       
                    "InnBetTId,InnBBeskrivelse,KontoNr"
                       ,       
                    "KontoNavn", /* Additional buffer and displ.fields - not updateable*/
                    "KontoNavn",  
...
/*     DYNAMIC-FUNCTION("setAttribute",hFieldMap,"disabledFields","InnBetTId"). */
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
    ...
    IF cState = "NEW" THEN
    DO:
      IF int(InnBetTId:screen-value) = 0 THEN
      DO:
          MESSAGE "Angi en innbetalingstype større enn 0."
              VIEW-AS ALERT-BOX WARNING BUTTONS OK.
          RETURN NO-APPLY.
      END.
      IF int(InnBetTId:screen-value) = int(DYNAMIC-FUNCTION("getFieldValues","InnBetType","WHERE InnBetTId = int(" + InnBetTId:SCREEN-VALUE + ")","InnBetTId")) THEN
      DO:
          MESSAGE "InnBetTId eksisterer fra før med type " + InnBetTId:SCREEN-VALUE + "."
              VIEW-AS ALERT-BOX WARNING BUTTONS OK  TITLE "Lagringskontroll".
          APPLY "ENTRY" TO InnBetTId.
          RETURN NO-APPLY.
      END.
    END.

    ASSIGN
        InnBetTId:SENSITIVE = FALSE
        .
