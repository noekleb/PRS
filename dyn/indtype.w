...
DEFINE VARIABLE IndividBeskr AS CHARACTER FORMAT "X(30)" 
     LABEL "Beskrivelse" 
     VIEW-AS FILL-IN 
     SIZE 32 BY 1.
...
DEFINE VARIABLE SerieNrReg AS INTEGER INITIAL 1 
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS 
          "Item 1", 1,
"Item 2", 2,
"Item 3", 3
     SIZE 32.2 BY 3 NO-UNDO.
...
DEFINE FRAME frmDetalj
     IndividType AT ROW 1.71 COL 14.2 COLON-ALIGNED HELP
          "Individtype"
     IndividBeskr AT ROW 2.71 COL 14.2 COLON-ALIGNED HELP
          "Kort beskrivelse av individtypen"
     SerieNrReg AT ROW 3.91 COL 16 HELP
          "1-Ingen spørsmål, 2-Tvang på Serienr, 3-Spørsmål om serienr" NO-LABEL
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 2 ROW 3.62
         SIZE 81 BY 12.38.
...
/* SETTINGS FOR FRAME frmDetalj
                                                                        */
/* SETTINGS FOR FILL-IN IndividType IN FRAME frmDetalj
   NO-ENABLE                                                            */
...
ON END-ERROR OF C-Win /* Individtyperegister */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Individtyperegister */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* Individtyperegister */
DO:
...
END.
ELSE MESSAGE "indtype.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY IndividType IndividBeskr SerieNrReg 
      WITH FRAME frmDetalj IN WINDOW C-Win.
  ENABLE IndividBeskr SerieNrReg 
      WITH FRAME frmDetalj IN WINDOW C-Win.
...
cAdgang = (DYNAMIC-FUNCTION("getFieldValues","SysPara",
                            "WHERE SysHId = 16 and SysGr = 33 and ParaNr = 1","Parameter1")).  
...
DO WITH FRAME frmListe:
  hBrowseListe = DYNAMIC-FUNCTION("NewBrowse",         /* Create a browse object */
                    ...
                    */
                    "IndType;IndividType;IndividBeskr;+cSerieNrReg|CHARACTER|x(22)|indtype_SerieNrReg.p|Handling;!SerieNrReg",
                    "WHERE true",
                    "sort|IndividType").                            /* Misc - for something I might need in next version.. */
...
DO WITH FRAME frmDetalj:
  ...
  hFieldMap = DYNAMIC-FUNCTION("NewFieldMap",      /* A fieldmap object holds extra info for display and input fields (fill-ins)
                    ...
                    "IndividType,IndividBeskr,SerieNrReg"
                       ,       
                    "IndividType,IndividBeskr,SerieNrReg"
                       ,       
...
/*   DYNAMIC-FUNCTION("setAttribute",hFieldMap,"customUpdateValProc","ignore"). */
  ...
  /* Felter som skal være disablet. Comma separert. */
    DYNAMIC-FUNCTION("setAttribute",hFieldMap,"disabledFields","IndividType"). 
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
    ...
    IF cState = "NEW" THEN
    DO:
      IF int(IndividType:screen-value) = int(DYNAMIC-FUNCTION("getFieldValues","IndType","WHERE IndividType = int(" + Individtype:SCREEN-VALUE + ")","IndividType")) THEN
      DO:
          MESSAGE "Individtype eksisterer fra før med type " + IndividType:SCREEN-VALUE + "."
              VIEW-AS ALERT-BOX WARNING BUTTONS OK  TITLE "Lagringskontroll".
          APPLY "ENTRY" TO IndividType.
          RETURN NO-APPLY.
      END.
    END.
