...
&Scoped-define INTERNAL-TABLES AlarmLog

/* Definitions for FRAME frmDetalj                                      */
&Scoped-define QUERY-STRING-frmDetalj FOR EACH AlarmLog SHARE-LOCK
&Scoped-define OPEN-QUERY-frmDetalj OPEN QUERY frmDetalj FOR EACH AlarmLog SHARE-LOCK.
&Scoped-define TABLES-IN-QUERY-frmDetalj AlarmLog
&Scoped-define FIRST-TABLE-IN-QUERY-frmDetalj AlarmLog
...
DEFINE VARIABLE AlarmDato AS DATE FORMAT "99/99/99" 
     LABEL "Alarm dato" 
     VIEW-AS FILL-IN 
     SIZE 13.2 BY 1.

DEFINE VARIABLE AlarmLogBeskrivelse AS CHARACTER FORMAT "X(30)" 
     LABEL "AlarmLogBeskrivelse" 
     VIEW-AS FILL-IN 
     SIZE 32 BY 1.

DEFINE VARIABLE AlarmLogId AS INTEGER FORMAT ">9" INITIAL 0 
     LABEL "AlarmLogId" 
     VIEW-AS FILL-IN 
     SIZE 7 BY 1.

DEFINE VARIABLE T-Ok AS LOGICAL INITIAL no 
     LABEL "Alarm OK" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME DEFAULT-FRAME
     rectToolBar AT ROW 1.1 COL 199
     rectUpdToolbar AT ROW 1.14 COL 1.4
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SCROLLABLE SIZE 206.6 BY 31.67.

DEFINE FRAME frmDetalj
     AlarmLogId AT ROW 1.95 COL 22.2 COLON-ALIGNED HELP
          "AlarmLog"
     AlarmLogBeskrivelse AT ROW 3 COL 22.2 COLON-ALIGNED HELP
          "AlarmLogBeskrivelse"
     T-Ok AT ROW 4.57 COL 24.4 WIDGET-ID 4
     AlarmDato AT ROW 5.76 COL 22.2 COLON-ALIGNED WIDGET-ID 6
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 2 ROW 3.62
         SIZE 205 BY 29.05.
...
/* SETTINGS FOR FRAME DEFAULT-FRAME
   FRAME-NAME                                                           */
ASSIGN 
       FRAME DEFAULT-FRAME:HEIGHT           = 31.67
       FRAME DEFAULT-FRAME:WIDTH            = 206.6.
...
ON END-ERROR OF C-Win /* AlarmLog register */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* AlarmLog register */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* AlarmLog register */
DO:
...
&Scoped-define SELF-NAME T-Ok
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL T-Ok C-Win
ON VALUE-CHANGED OF T-Ok IN FRAME frmDetalj /* Alarm OK */
DO:
  IF SELF:CHECKED THEN
      AlarmDato:SCREEN-VALUE IN FRAME frmDetalj = STRING(TODAY).
  ELSE 
      AlarmDato:SCREEN-VALUE IN FRAME frmDetalj = ?.
END.
...
DO:
    ...
    DO WITH FRAME frmDetalj:
        APPLY "Entry" TO AlarmLogBeskrivelse.            .
    END.
...
END.
ELSE MESSAGE "alarmlog.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  {&OPEN-QUERY-frmDetalj}
  GET FIRST frmDetalj.
  DISPLAY AlarmLogId AlarmLogBeskrivelse T-Ok AlarmDato 
      WITH FRAME frmDetalj IN WINDOW C-Win.
  ENABLE AlarmLogId AlarmLogBeskrivelse T-Ok AlarmDato 
      WITH FRAME frmDetalj IN WINDOW C-Win.
...
DO WITH FRAME frmListe:
  hBrowseListe = DYNAMIC-FUNCTION("NewBrowse",         /* Create a browse object */
                    ...
                    "Gavekort;butnr;IdentType;Dato|Reg.dato;+cRegTid|CHARACTER|x(5)|gavekort_regtid.p|Tid;Gyldigdato;Bruktdato;KasseNr;BongNr;Belop;!Tid," + 
                      "GaveKType;GKTAlarmLogBeskrivelse",                     /* Buffers and fields: <buffer1>;<field1>;<field2>..,<buffer2>;.. No fields gives all*/
                    */
                    "AlarmLog"
                    + ";AlarmLogId|Alarm lognr|>>>>>>>>>>>>9"
                    + ";AlarmLogBeskrivelse|Beskrivelse"
                    + ";AlarmDato|Dato"
                    + ";+ATid|character|x(6)|AlarmTid(ROWID)|Kl"
                    + ";EDato"
                    + ";+ETid|character|x(6)|ETid(ROWID)|Kl"
                    + ";BrukerId",
                    "WHERE true",
                    "sort|AlarmLogId").                            /* Misc - for something I might need in next version.. */
  ...
  */
  ...
  /*DYNAMIC-FUNCTION("setAttribute",hBrowseListe,"basequery","WHERE Individ.AlarmLogId = 1 ").*/
...
DO WITH FRAME frmDetalj:
  ...
  hFieldMap = DYNAMIC-FUNCTION("NewFieldMap",      /* A fieldmap object holds extra info for display and input fields (fill-ins)
                    ...
                    "AlarmLogId,AlarmLogBeskrivelse"
                       ,       
                    "AlarmLogId,AlarmLogBeskrivelse"
                       ,       
...
/*     DYNAMIC-FUNCTION("setAttribute",hFieldMap,"disabledFields","AlarmLogId"). */
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
      ASSIGN
          AlarmLogId:SENSITIVE = TRUE
          .
      APPLY "ENTRY" TO AlarmLogId.
      RETURN NO-APPLY.
  END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
    ...
    IF cState = "NEW" THEN
    DO:
      IF int(AlarmLogId:screen-value) = 0 THEN
      DO:
          MESSAGE "Angi en innbetalingstype større enn 0."
              VIEW-AS ALERT-BOX WARNING BUTTONS OK.
          RETURN NO-APPLY.
      END.
      IF int(AlarmLogId:screen-value) = int(DYNAMIC-FUNCTION("getFieldValues","AlarmLog","WHERE AlarmLogId = int(" + AlarmLogId:SCREEN-VALUE + ")","AlarmLogId")) THEN
      DO:
          MESSAGE "AlarmLogId eksisterer fra før med type " + AlarmLogId:SCREEN-VALUE + "."
