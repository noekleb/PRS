...
DEFINE VARIABLE DebKred AS INTEGER FORMAT "9":U INITIAL 0 
     LABEL "Debet/kredit" 
     VIEW-AS COMBO-BOX INNER-LINES 25
     LIST-ITEM-PAIRS "Ikke i bruk",0,
                     "Debet",1,
                     "Kredit",2
     DROP-DOWN-LIST
...
DEFINE FRAME DEFAULT-FRAME
     rectToolBar AT ROW 1.1 COL 134.4
     rectUpdToolbar AT ROW 1.14 COL 1.4
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SCROLLABLE SIZE 143 BY 21.91.

DEFINE FRAME frmDetalj
     B-KonvTbl AT ROW 1.24 COL 119.4
     KontoNr AT ROW 2.91 COL 19 COLON-ALIGNED HELP
          "Kontonummer i regnskap"
     Kontonavn AT ROW 3.91 COL 19 COLON-ALIGNED HELP
          "Kontonavn"
     DebKred AT ROW 4.95 COL 19 COLON-ALIGNED
     Notat AT ROW 6.24 COL 21 NO-LABEL
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 2 ROW 3.62
         SIZE 141 BY 19.05.
...
ASSIGN 
       ...
       FRAME DEFAULT-FRAME:WIDTH            = 143.

/* SETTINGS FOR FRAME frmDetalj
                                                                        */
ASSIGN 
       Notat:RETURN-INSERTED IN FRAME frmDetalj  = TRUE.
...
ON END-ERROR OF C-Win /* Kontoregister */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Kontoregister */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* Kontoregister */
DO:
...
DO:
  run d-bimpkonv.w (1015,
                    KontoNr:SCREEN-VALUE,
                    "Kobling av Kontotabeller"
                    ).
  return no-apply.
END.
...
OCXFile = SEARCH( "kontotabell.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "kontotabell.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME frmDetalj:
        IF CAN-FIND(FIRST UtBetType WHERE
                    UtBetType.KontoNr = int(KontoNr:screen-value)) THEN
        DO:
            MESSAGE "Kontonummer er i bruk på utbetalingstype."
                VIEW-AS ALERT-BOX WARNING BUTTONS OK  TITLE "Lagringskontroll".
            APPLY "ENTRY" TO KontoNr.
            RETURN NO-APPLY.
        END.
        IF CAN-FIND(FIRST InnBetType WHERE
                    InnBetType.KontoNr = int(KontoNr:screen-value)) THEN
        DO:
            MESSAGE "Kontonummer er i bruk på innbetalingstype."
                VIEW-AS ALERT-BOX WARNING BUTTONS OK  TITLE "Lagringskontroll".
            APPLY "ENTRY" TO KontoNr.
            RETURN NO-APPLY.
        END.
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY KontoNr Kontonavn DebKred Notat 
      WITH FRAME frmDetalj IN WINDOW C-Win.
  ENABLE B-KonvTbl KontoNr Kontonavn DebKred Notat 
      WITH FRAME frmDetalj IN WINDOW C-Win.
...
cAdgang = (DYNAMIC-FUNCTION("getFieldValues","SysPara",
                            "WHERE SysHId = 16 and SysGr = 38 and ParaNr = 2","Parameter1")).  
...
DO WITH FRAME frmListe:
  hBrowseListe = DYNAMIC-FUNCTION("NewBrowse",         /* Create a browse object */
                    ...
                    */
                    "Kontotabell" 
                  + ";KontoNr"
                  + ";KontoNavn|Kontonavn|x(60)"
                  + ";DebKred|DebKred"
                  + ";EDato"
                  + ";!Notat"
                  + ";!BrukerId",
                    "WHERE true",
                    "sort|KontoNr").                            /* Misc - for something I might need in next version.. */
  ...
  */
  ...
  /*DYNAMIC-FUNCTION("setAttribute",hBrowseListe,"basequery","WHERE Individ.KontoNr = 1 ").*/
...
DO WITH FRAME frmDetalj:
  ...
  hFieldMap = DYNAMIC-FUNCTION("NewFieldMap",      /* A fieldmap object holds extra info for display and input fields (fill-ins)
                    ...
                    "KontoNr,KontoNavn,DebKred,Notat"
                       ,       
                    "KontoNr,KontoNavn,DebKred,Notat"
                       ,       
...
/*     DYNAMIC-FUNCTION("setAttribute",hFieldMap,"disabledFields","KontoNr"). */
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
    ...
    IF cState = "NEW" THEN
    DO:
      IF int(KontoNr:screen-value) = 0 THEN
      DO:
          MESSAGE "Angi et kontonummer større enn 0."
              VIEW-AS ALERT-BOX WARNING BUTTONS OK.
          RETURN NO-APPLY.
      END.
      IF int(KontoNr:screen-value) = int(DYNAMIC-FUNCTION("getFieldValues","Kontotabell","WHERE KontoNr = int(" + KontoNr:SCREEN-VALUE + ")","KontoNr")) THEN
      DO:
          MESSAGE "Kontonummer eksisterer fra før med type " + KontoNr:SCREEN-VALUE + "."
              VIEW-AS ALERT-BOX WARNING BUTTONS OK  TITLE "Lagringskontroll".
          APPLY "ENTRY" TO KontoNr.
          RETURN NO-APPLY.
      END.
