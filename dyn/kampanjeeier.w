...
&Scoped-Define ENABLED-OBJECTS btnSokButikk rectBrowse rectToolBar ~
rectWinToolbar KampEierId KampEierNavn ButikkNr 
&Scoped-Define DISPLAYED-OBJECTS KampEierId KampEierNavn ButikkNr ButNamn 
...
DEFINE VARIABLE ButNamn AS CHARACTER FORMAT "x(40)" 
     VIEW-AS FILL-IN 
     SIZE 35.4 BY 1.

DEFINE VARIABLE KampEierId AS INTEGER FORMAT "->,>>>,>>9":U INITIAL 0 
     LABEL "Eier id" 
     VIEW-AS FILL-IN 
     SIZE 14 BY 1 NO-UNDO.

DEFINE VARIABLE KampEierNavn AS CHARACTER FORMAT "X(256)":U 
     LABEL "Navn" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME DEFAULT-FRAME
     btnSokButikk AT ROW 4.81 COL 71.2 WIDGET-ID 2 NO-TAB-STOP 
     KampEierId AT ROW 2.67 COL 59.2 COLON-ALIGNED
     KampEierNavn AT ROW 3.71 COL 59.2 COLON-ALIGNED
     ButikkNr AT ROW 4.81 COL 59.2 COLON-ALIGNED HELP
          "Hovedgruppe som varegruppen er koblet til" WIDGET-ID 4
     ButNamn AT ROW 4.81 COL 73.8 COLON-ALIGNED HELP
          "Kort beskrivelse av hovedgruppen" NO-LABEL WIDGET-ID 6
     rectBrowse AT ROW 2.43 COL 2
     rectToolBar AT ROW 1.14 COL 1.8
     rectWinToolbar AT ROW 1.14 COL 109.8
...
ON END-ERROR OF C-Win /* Kampanje eier register */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Kampanje eier register */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* Kampanje eier register */
DO:
...
ON CHOOSE OF btnSokButikk IN FRAME DEFAULT-FRAME /* ... */
OR F10 OF ButikkNr
DO:
...
ON RETURN OF ButikkNr IN FRAME DEFAULT-FRAME /* Butikknr */
OR "TAB" OF ButikkNr
DO:
  IF SELF:MODIFIED THEN
  DO:
      ButNamn:SCREEN-VALUE = DYNAMIC-FUNCTION("getFieldValues","Butiker",
                                              "WHERE Butik = '" + ButikkNr:SCREEN-VALUE + "'","ButNamn").  
  END.
END.
...
------------------------------------------------------------------------------*/
  ...
  RUN SUPER.
  KampEierId:SENSITIVE IN FRAME {&FRAME-NAME} =  DYNAMIC-FUNCTION('getToolbarState',hToolbar) = 'New'.
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  DISPLAY KampEierId KampEierNavn ButikkNr ButNamn 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE btnSokButikk rectBrowse rectToolBar rectWinToolbar KampEierId 
         KampEierNavn ButikkNr 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
DO WITH FRAME {&FRAME-NAME}:
  hBrowse = DYNAMIC-FUNCTION("NewBrowse",          /* Create a browse object */
                    ...
                    "KampanjeEier"
                    + ";KampEierId;KampEierNavn;ButikkNr;RegistrertTid;!RegistrertDato;!RegistrertAv;!ETid;!EDato;!BrukerID"
                   ,"WHERE true"
                    ,"sort|KampEierId").             /* Initial sort column */
  ...
  hFieldMap = DYNAMIC-FUNCTION("NewFieldMap",      /* A fieldmap object holds extra info for display and input fields (fill-ins) 
                    ...
                    "KampEierId,KampEierNavn,ButikkNr",   /* Update columns in buffer */
  ...
  DYNAMIC-FUNCTION("setAttribute",hFieldMap,"customUpdateValProc","=kampanjeeier_update.p"). 
