...
rectWinToolbar B-sokKonto-4 RECT-64 searchField fiButikkNr fiFraRegDato ~
fiTilRegDato fiBeskrivelse fiFraEDato fiTilEDato T-VisStd BUTTON-1 ButikkNr ~
Beskrivelse KontoNrMva KontoNrVarekost KontoNrVareSalg B-sokKonto-3 ~
B-sokKonto1 btnCalFraDato-11 btnCalFraDato-7 btnCalFraDato-12 ~
btnCalFraDato-8 
&Scoped-Define DISPLAYED-OBJECTS fiButikkNr fiFraRegDato fiTilRegDato ~
fiBeskrivelse fiFraEDato fiTilEDato T-VisStd ButikkNr MomsKod Beskrivelse ~
KontoNrMva KontoNrVarekost KontoNrVareSalg 
...
DEFINE BUTTON B-sokKonto-3 
     IMAGE-UP FILE "icon\e-sokpr":U NO-FOCUS
     LABEL "..." 
     SIZE 4.4 BY 1.

DEFINE BUTTON B-sokKonto-4 
     IMAGE-UP FILE "icon\e-sokpr":U NO-FOCUS
     LABEL "..." 
     SIZE 4.4 BY 1.

DEFINE BUTTON B-sokKonto1 
     IMAGE-UP FILE "icon\e-sokpr":U NO-FOCUS
     LABEL "..." 
     SIZE 4.4 BY 1.
...
DEFINE VARIABLE KontoNrMva AS INTEGER FORMAT ">>>9999" INITIAL 0 
     LABEL "Kontonr. mva" 
     VIEW-AS FILL-IN 
     SIZE 11.8 BY 1.

DEFINE VARIABLE KontoNrVarekost AS INTEGER FORMAT ">>>9999" INITIAL 0 
     LABEL "Kontonr. varekost" 
     VIEW-AS FILL-IN 
     SIZE 11.8 BY 1.

DEFINE VARIABLE KontoNrVareSalg AS INTEGER FORMAT ">>>9999" INITIAL 0 
     LABEL "Kontonr. varesalg" 
     VIEW-AS FILL-IN 
     SIZE 11.8 BY 1.

DEFINE VARIABLE MomsKod AS INTEGER FORMAT ">9" INITIAL 0 
     LABEL "Mva kode" 
     VIEW-AS FILL-IN 
     SIZE 16 BY 1.
...
DEFINE VARIABLE T-VisStd AS LOGICAL INITIAL no 
     LABEL "Vis bare standardoppsett" 
     VIEW-AS TOGGLE-BOX
     SIZE 45.6 BY .81 NO-UNDO.
...
DEFINE FRAME DEFAULT-FRAME
     ...
     B-sokKonto-4 AT ROW 12.76 COL 173.6
     ...
     fiTilEDato AT ROW 4.38 COL 164 COLON-ALIGNED HELP
          "Dato da sjekken ble utsted" NO-LABEL
     T-VisStd AT ROW 4.67 COL 24.4
     BUTTON-1 AT ROW 5.19 COL 189
     ButikkNr AT ROW 7.57 COL 159.8 COLON-ALIGNED HELP
          "Nr. på butikk som har utstedtsjekken"
     MomsKod AT ROW 8.62 COL 159.8 COLON-ALIGNED HELP
          "Unikt ID for eksport av data for butikk"
     Beskrivelse AT ROW 9.67 COL 159.6 COLON-ALIGNED HELP
          "Merknad"
     KontoNrMva AT ROW 10.71 COL 159.6 COLON-ALIGNED HELP
          "Kontonummer i regnskap"
     KontoNrVarekost AT ROW 11.71 COL 159.4 COLON-ALIGNED
     KontoNrVareSalg AT ROW 12.71 COL 159.4 COLON-ALIGNED
     B-sokKonto-3 AT ROW 11.76 COL 173.6
     B-sokKonto1 AT ROW 10.71 COL 173.6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Oppsett av MVA for SIE eksport"
         HEIGHT             = 30.19
         WIDTH              = 203.2
         MAX-HEIGHT         = 57.14
         MAX-WIDTH          = 320
...
/* SETTINGS FOR FRAME DEFAULT-FRAME
   FRAME-NAME                                                           */
/* SETTINGS FOR FILL-IN MomsKod IN FRAME DEFAULT-FRAME
   NO-ENABLE                                                            */
...
ON END-ERROR OF C-Win /* Oppsett av MVA for SIE eksport */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Oppsett av MVA for SIE eksport */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* Oppsett av MVA for SIE eksport */
DO:
...
&Scoped-define SELF-NAME B-sokKonto-3
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-sokKonto-3 C-Win
ON CHOOSE OF B-sokKonto-3 IN FRAME DEFAULT-FRAME /* ... */
OR F10 OF KontoNrVarekost
DO:
    cTekst = "KontoNr".                    
    RUN JBoxDLookup.w ("KontoTabell;KontoNr;KontoNavn;DebKred", "where true", INPUT-OUTPUT cTekst).

    IF RETURN-VALUE = "AVBRYT" THEN
        RETURN NO-APPLY.
    ELSE DO:
        ASSIGN
            KontoNrVareKost:SCREEN-VALUE = cTekst
            KontoNrVareKost:MODIFIED     = TRUE
            .
        APPLY "any-printable" TO KontoNrVareKost.
    END.
  APPLY "ENTRY" TO KontoNrVareKost.
...
&Scoped-define SELF-NAME B-sokKonto-4
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-sokKonto-4 C-Win
ON CHOOSE OF B-sokKonto-4 IN FRAME DEFAULT-FRAME /* ... */
OR F10 OF KontoNrVareSalg
DO:
    cTekst = "KontoNr".                    
    RUN JBoxDLookup.w ("KontoTabell;KontoNr;KontoNavn;DebKred", "where true", INPUT-OUTPUT cTekst).
    ...
    ELSE DO:
        ASSIGN
            KontoNrVareSalg:SCREEN-VALUE = cTekst
            KontoNrVareSalg:MODIFIED     = TRUE
            .
        APPLY "any-printable" TO KontoNrVareSalg.
    END.
  APPLY "ENTRY" TO KontoNrVareSalg.
...
&Scoped-define SELF-NAME B-sokKonto1
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-sokKonto1 C-Win
ON CHOOSE OF B-sokKonto1 IN FRAME DEFAULT-FRAME /* ... */
OR F10 OF KontoNrMva
DO:
    cTekst = "KontoNr".                    
    RUN JBoxDLookup.w ("KontoTabell;KontoNr;KontoNavn;DebKred", "where true", INPUT-OUTPUT cTekst).
    ...
    ELSE DO:
        ASSIGN
            KontoNrMva:SCREEN-VALUE = cTekst
            KontoNrMva:MODIFIED     = TRUE
            .
        APPLY "any-printable" TO KontoNrMva.
    END.
  APPLY "ENTRY" TO KontoNrMva.
...
ON VALUE-CHANGED OF T-VisStd IN FRAME DEFAULT-FRAME /* Vis bare standardoppsett */
DO:
  IF SELF:CHECKED THEN
      ASSIGN 
        fiButikkNr:SCREEN-VALUE = ''
        fiButikkNr:SENSITIVE    = FALSE
        B-sokButik-2:SENSITIVE  = FALSE
        .
  ELSE 
      ASSIGN 
        fiButikkNr:SENSITIVE    = TRUE 
        B-sokButik-2:SENSITIVE  = TRUE
        .
  RUN InvokeMethod (hBrowse,'OpenQuery').
END.
...
IF iReturn = 1 THEN
  bOk = DYNAMIC-FUNCTION("ProcessQuery",hBrowse,"siemoms_delete.p",'').
...
DO:
  ...
  END.
  bOk = DYNAMIC-FUNCTION("ProcessSelectedRows",hBrowse,"siemoms_delete.p",'').
END.
ELSE
  LEAVE.
IF NOT bOK THEN
...
------------------------------------------------------------------------------*/
  DISPLAY fiButikkNr fiFraRegDato fiTilRegDato fiBeskrivelse fiFraEDato 
          fiTilEDato T-VisStd ButikkNr MomsKod Beskrivelse KontoNrMva 
          KontoNrVarekost KontoNrVareSalg 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE B-sokButik-2 rectBrowse rectToolBar rectWinToolbar B-sokKonto-4 
         RECT-64 searchField fiButikkNr fiFraRegDato fiTilRegDato fiBeskrivelse 
         fiFraEDato fiTilEDato T-VisStd BUTTON-1 ButikkNr Beskrivelse 
         KontoNrMva KontoNrVarekost KontoNrVareSalg B-sokKonto-3 B-sokKonto1 
...
DO WITH FRAME {&FRAME-NAME}:
  hBrowse = DYNAMIC-FUNCTION("NewBrowse",          
                    ...
                    "SIEMoms"
                  + ";MomsKod|Mvakode"
                  + ";+MvaProc|dec|>>9.99|MomsProc(ROWID)|Mva%"
                  + ";Beskrivelse"
                  + ";ButikkNr|Butikknr"
                  + ";KontoNrVaresalg|Konto varesalg"
                  + ";KontoNrMva|Konto mva"
                  + ";KontoNrVarekost|Konto varekost"
                  + ";EDato|Endret"
  ...
  hFieldMap = DYNAMIC-FUNCTION("NewFieldMap",    
                    ...
                    "ButikkNr,MomsKod,Beskrivelse,KontoNrVareSalg,KontoNrMva,KontoNrVarekost",              
                    "",              
                    "","",                       
                    "").            
  ...
  DYNAMIC-FUNCTION('setAttribute',hBrowse,'calcfieldproc','siemoms_brwcalc.p').
  ...
  DYNAMIC-FUNCTION("NewToolBar",
                    ...
                    ",kopierMoms;Kopier mvaoppsett til butikk", 
...
DYNAMIC-FUNCTION("setAddMoveX",THIS-PROCEDURE:CURRENT-WINDOW,FRAME {&FRAME-NAME}:HANDLE,
                 "ButikkNr,MomsKod,Beskrivelse,KontoNrMva,KontoNrVaresalg,KontoNrVarekost").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE kopierMomsRecord C-Win 
PROCEDURE kopierMomsRecord :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DO WITH FRAME DEFAULT-FRAME:
    ...
    IF iReturn = 1 THEN
      bOk = DYNAMIC-FUNCTION("ProcessQuery",hBrowse,"siemoms_kopiermoms.p",cValues).
    ELSE IF iReturn = 2 THEN
    DO:
      DO ix = 1 TO hBrowse:NUM-SELECTED-ROWS:
        IF hBrowse:FETCH-SELECTED-ROW(ix) THEN
          cRowIdList = cRowIdList + hBrowse:QUERY:GET-BUFFER-HANDLE(1):BUFFER-FIELD('RowIdent1'):BUFFER-VALUE + ','.
      END.
      bOk = DYNAMIC-FUNCTION("ProcessSelectedRows",hBrowse,"siemoms_kopiermoms.p",cValues).
    END.
    ELSE
      LEAVE.
    IF NOT bOK THEN
      DYNAMIC-FUNCTION("DoMessage",0,0,DYNAMIC-FUNCTION("getTransactionMessage"),"Feil i sending av informasjon ",""). 
    ELSE ASSIGN
        fiButikkNr:SCREEN-VALUE = cValues
        T-VisStd:CHECKED        = TRUE.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN wbyggSIEMoms.w PERSISTENT SET hwgenData.
