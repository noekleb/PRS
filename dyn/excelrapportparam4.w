...
DEF VAR gsysHId     AS INT INIT 229 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getSysParamValue C-Win 
FUNCTION getSysParamValue RETURNS CHARACTER
  (INPUT iSysHid AS INT, 
   INPUT iSysGr AS INT, 
   INPUT cField AS CHAR  )  FORWARD.
...
ON END-ERROR OF C-Win /* Lager Analyse rapporter */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Lager Analyse rapporter */
DO:
...
/*   advfilePath:SCREEN-VALUE IN FRAME {&FRAME-NAME} = SESSION:TEMP-DIRECTORY. */
  ...
  ASSIGN 
    ...
    iY = INT((CURRENT-WINDOW:HEIGHT-PIXEL / 2 + CURRENT-WINDOW:Y))  
  .
  RUN excelrapportadhoc4.w PERSISTENT SET hAdHoc.
  ...
  DO:
    ...
    DYNAMIC-FUNCTION('setReportFile' IN hAdHoc,'excelrapport4.p').
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION getSysParamValue C-Win 
FUNCTION getSysParamValue RETURNS CHARACTER
  (INPUT iSysHid AS INT, 
   INPUT iSysGr AS INT, 
   INPUT cField AS CHAR  ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
  ...
  cFieldValue  = DYNAMIC-FUNCTION("getFieldValues","SysPara","WHERE SysHId = " + STRING(iSysHid) + " and SysGr = " + STRING(iSysgr) + 
                                 " and parameter1 = " + QUOTER(cField) ,"Parameter2").
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABL iSysHid AS INT INIT 229.
  DEFINE VARIABLE iSysGr AS INT INIT 1. 

  ASSIGN 
    pcEmailAddr   = getSysParamValue(iSysHid,iSysgr,"emailadresse").
    pcFilePath    = getSysParamValue(iSysHid,iSysgr,"avdFilePath").
    pcFileExtent  = getSysParamValue(iSysHid,iSysgr,"advFileExtent").
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ASSIGN 
      ...
      advRapportNavn:SCREEN-VALUE = IF advRapportNavn:SCREEN-VALUE = '' THEN 'Analyse Rapport - ' + STRING(TIME,"HH:MM:SS") + ':' + STRING(RANDOM(1,99)) ELSE advRapportNavn:SCREEN-VALUE
