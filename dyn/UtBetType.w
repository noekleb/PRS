...
DEFINE VARIABLE UtBetTId AS INTEGER FORMAT "9" INITIAL 0 
     LABEL "Utvetalingstype" 
...
DEFINE VARIABLE UtBBeskrivelse AS CHARACTER FORMAT "X(30)" 
     LABEL "Beskrivelse" 
     VIEW-AS FILL-IN 
     SIZE 46 BY 1.
...
DEFINE FRAME frmDetalj
     ...
     UtBetTId AT ROW 1.95 COL 19 COLON-ALIGNED HELP
          "Innbetalingstype"
     UtBBeskrivelse AT ROW 2.95 COL 19 COLON-ALIGNED HELP
...
ON END-ERROR OF C-Win /* Utbetalignstyper */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Utbetalignstyper */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* Utbetalignstyper */
DO:
...
OCXFile = SEARCH( "UtBetType.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "UtBetType.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY UtBetTId UtBBeskrivelse KontoNr Kontonavn 
      WITH FRAME frmDetalj IN WINDOW C-Win.
  ENABLE btnSokBrGrp-2 UtBetTId UtBBeskrivelse KontoNr 
      WITH FRAME frmDetalj IN WINDOW C-Win.
...
DO WITH FRAME frmListe:
  hBrowseListe = DYNAMIC-FUNCTION("NewBrowse",         /* Create a browse object */
                    ...
                    */
                    "UtBetType;UtBetTId;UtBBeskrivelse;KontoNr,KontoTabell;KontoNavn",
                    "WHERE true, FIRST KontoTabell NO-LOCK where KontoTabell.KontoNr = UtBetType.KontoNr OUTER-JOIN",
                    "sort|UtBetTId").                            /* Misc - for something I might need in next version.. */
  ...
  */
  ...
  /*DYNAMIC-FUNCTION("setAttribute",hBrowseListe,"basequery","WHERE Individ.UtBetTId = 1 ").*/
...
DO WITH FRAME frmDetalj:
  ...
  hFieldMap = DYNAMIC-FUNCTION("NewFieldMap",      /* A fieldmap object holds extra info for display and input fields (fill-ins)
                    ...
                    "UtBetTId,UtBBeskrivelse,KontoNr"
                       ,       
                    "UtBetTId,UtBBeskrivelse,KontoNr"
                       ,       
...
/*     DYNAMIC-FUNCTION("setAttribute",hFieldMap,"disabledFields","UtBetTId"). */
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
    ...
    IF cState = "NEW" THEN
    DO:
      IF int(UtBetTId:screen-value) = 0 THEN
      DO:
          MESSAGE "Angi en innbetalingstype større enn 0."
              VIEW-AS ALERT-BOX WARNING BUTTONS OK.
          RETURN NO-APPLY.
      END.
      IF int(UtBetTId:screen-value) = int(DYNAMIC-FUNCTION("getFieldValues","UtBetType","WHERE UtBetTId = int(" + UtBetTId:SCREEN-VALUE + ")","UtBetTId")) THEN
      DO:
          MESSAGE "UtBetTId eksisterer fra før med type " + UtBetTId:SCREEN-VALUE + "."
              VIEW-AS ALERT-BOX WARNING BUTTONS OK  TITLE "Lagringskontroll".
          APPLY "ENTRY" TO UtBetTId.
          RETURN NO-APPLY.
      END.
    END.

    ASSIGN
        UtBetTId:SENSITIVE = FALSE
        .
