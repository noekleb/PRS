...
DEFINE BUTTON btnLev 
     LABEL "..." 
     SIZE 4.4 BY 1 TOOLTIP "S�k hele leverand�r-register".
...
DEFINE FRAME DEFAULT-FRAME
     ...
     sokLevNr AT ROW 1.24 COL 162 COLON-ALIGNED HELP
          "Leverand�r - trykk F3 for � s�ke blant alle leverand�rer"
...
------------------------------------------------------------------------------*/
  ...
  bOk = FALSE.
  MESSAGE 'Er du sikker p� at du vil slette disse varelinjene fra pakkseddelen?'
...
DO  WITH FRAME DEFAULT-FRAME:
    ...
    DO:
        MESSAGE 'Import mot pakkseddel kan bare gj�res for h�ndterminaltype 6 (Symbol PPT8800),  7 (BxMobile Gml),10 (BxMobile) og 12 (CubComm).' SKIP
            'Valgt h�ndterminaltype er ' iHtType '.'
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
        bOk = FALSE.
        RETURN.
    END.
    ...
    IF cFileName = '' THEN
    DO:
        MESSAGE 'Filnavn ikke satt opp i h�ndterminalregisteret.'
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
        bOk = FALSE.
        RETURN.
    END.
    ...
    IF cKatalog = '' THEN
    DO:
        MESSAGE 'Importkatalog ikke satt opp i h�ndterminalregisteret.'
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
        bOk = FALSE.
        RETURN.
    END.
...
/* Hvis det ligger en Inv.Dat, skal denne f�rst konverteres til PPT880 formatet.            */
/* vpiartbas_importer_pricat_fil.p kaller opp xbxmobileinvinnles.p som gj�r konverteringen. */
...
SYSTEM-DIALOG GET-FILE cFileName 
              FILTERS "H�ndterminalfiler: " + cFilter cFilter 
...
IF bOk = FALSE THEN
    RETURN NO-APPLY.
ELSE 
    MESSAGE "Skal import av h�ndterminalfil starte?" SKIP
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  DYNAMIC-FUNCTION("NewMenuBand",hBrowse
                 ...
                 + ",SettLevAntTilAnt;Sett levert antall til antall p� pakkseddel for varer"
...
IF bOk THEN DO:
    ...
    /* Er det generert bestillinger, m� hele sp�rringen friskes opp. */
...
IF bOk AND bGenEAN THEN DO:
  MESSAGE "Det fins artikler i pakklisten som ikke har gyldige strekkoder," + CHR(10)
          + "Det m� registreres gyldige EAN koder p� disse f�rst."
      VIEW-AS ALERT-BOX INFO BUTTONS OK.
  RUN StrekKodeRecord.
  RUN InvokeMethod(hBrowse,"OpenQuery").
  bOk = hBuffer:FIND-FIRST("WHERE NOT GyldigStrekkode and pakke = false and AntLevert > 0") NO-ERROR.
...
cMsg = cMsg + "Skal oppdatering av varemottak startes (husk kontroll av antall og pris f�rst)?".

IF hBrowse:NUM-SELECTED-ROWS > 1 THEN
  cMsg = cMsg + CHR(10) + CHR(10) + "NB! Innleveranse skjer p� alle varer i s�kekriteriet - uavhengig av markering av rader".
...
DO:
  ...
  /* Strekkoder m� registreres f�r varemottaket oppdateres - frivillig. */
  ...
  /* Er pakkseddelen kommet fra sentrallageret, skal lageret trekkes ned p� sentrallageret n�r det gj�res varemottak i outlet. */ 
  ...
  DO:
    ...
    DO:
        DYNAMIC-FUNCTION("DoMessage",0,20,
                         "Pakkseddel har feil status. Kan ikke gj�re varemottak p� denne pakkseddel.","",""). 
        RETURN.
    END.
    
    /* TN 21/12-17 Utf�r priskontroll p� utpris. Gj�res ikke for Outlet butikkene. */
    ...
    /* Finner fra butikken hvis det er en overf�ring. Hvis ikke kommer varene fra lager 20. */    
    IF AVAILABLE PkSdlHode AND 
        NUM-ENTRIES(PkSdlHode.Merknad,CHR(13)) > 1 AND 
        ENTRY(2,PkSdlHode.Merknad,CHR(13)) BEGINS 'Overf�rt fra butikk ' AND
    ...
    DO:
        ...
        /* Er det overf�rt fra en annen butikk til outlet, skal det bare utstedes faktura. 'Fra' butikkens lager skal da ikke r�res her. Det er gjort tidligere. */
        IF (CAN-DO(cOutletLst,cButikkNr) OR CAN-DO(ceComLst,cButikkNr) ) AND CAN-DO('4,5,7',STRING(PkSdlHode.PkSdlOpphav)) THEN 
        DO: 
          ...
          DO:
              ...
              DO:
                  MESSAGE 'Det er innlest ' + STRING(piAnt) + ' linjer. Men filen skal inneholde ' + ENTRY(2,cLinje,';') + '.' + CHR(10) +
                          'Filens innhold samsvarer ikke med sjekksiffer. Filen kan v�re �delagt og b�r sjekkes.'
                      VIEW-AS ALERT-BOX INFO BUTTONS OK.
              END.
          END.
      END. /* CubComm */
...
IF hBuffer:AVAIL THEN DO:
  ...
  DO:
      bOk = FALSE.
      MESSAGE 'Strekkoden ligger p� en annen artikkel. Vil du sanere?' + CHR(10) + CHR(10) + 
              'Sanering rydder og flytter all informasjon (Salg,  lager m.m.) fra gammelt til nytt artikkelkort.' + CHR(10) + CHR(10) +
              'Hvis du ikke sanerer, m� antall levert settes til 0 p� pakkseddelen og varen leveres inn via direkte varemottak.'
      ...
      DO:
          ...
          /* Sjekker om ogs� st�rrelsen p� strekkoden m� flytte. */
  ...
  DO: 
      bOk = FALSE.
      /* Strekkoden finnes ikke p� artikkelen fra f�r. */
      IF TRIM(DYNAMIC-FUNCTION("getFieldList","Strekkode;ArtikkelNr","WHERE Kode = '" + STRING(hBuffer:BUFFER-FIELD("Kode"):BUFFER-VALUE) + "'")) = '' THEN 
      DO TRANSACTION:
          /* Strekkoden ligger p� samme artikkel, men p� en annen st�rrelse */
          MESSAGE 'Det ligger en ukjent strekkode p� pakkseddelen. Denne er n� opprettet p� artikkelen.'
      ...
      ELSE DO:
          /* Strekkoden ligger p� samme artikkel, men p� en annen st�rrelse */
          MESSAGE 'Strekkoden ligger p� en annen st�rrelse p� artikkelen. Vil du flytte strekkode til riktig st�rrelse?' + CHR(10) + CHR(10) + 
                  'N�r strekkoden flyttes til riktig st�rrelse flyttes ogs salg og lagerverdier med.' + CHR(10) + CHR(10) +
                  'Hvis du ikke flytter, m� antall levert settes til 0 p� pakkseddelen og varen leveres inn via direkte varemottak.'
              VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO TITLE 'Behandling av strekkode som ligger p� feil st�rrelse' UPDATE bOk.
          IF bOk THEN
          DO: 
              RUN bibl_flytt_lager_str.p (hBuffer:BUFFER-FIELD("Kode"):BUFFER-VALUE, INT(hBuffer:BUFFER-FIELD("StrKode"):BUFFER-VALUE)).
              DYNAMIC-FUNCTION("RefreshRowids",hBrowse,hBuffer:BUFFER-FIELD("RowIdent1"):BUFFER-VALUE).
  ...
  END.
  ELSE DO:
      MESSAGE 'Strekkoden p� varelinjen du har markert, er ikke feilkoblet. Kan ikke saneres.'
          VIEW-AS ALERT-BOX INFO BUTTONS OK.
  END.
END.
...
RUN JBoxBrowseMsgUpdateVal.w ("Sett levert antall til antall p� pakkseddel for varer",
                              hBrowse:NUM-SELECTED-ROWS,
                              DYNAMIC-FUNCTION("getAttribute",hBrowse,"Totalcount"),
                              "",
/*                               "LOGICAL|Ja/Nei|yes",  */
