...
DEFINE VARIABLE KatNr AS INTEGER FORMAT "z9" INITIAL 0 
     LABEL "Kategori" 
     VIEW-AS FILL-IN 
     SIZE 4.8 BY 1.
...
DEFINE FRAME frmDetalj
     ...
     KatNr AT ROW 3.1 COL 16 COLON-ALIGNED HELP
          "Kategorinummer"
     Beskrivelse AT ROW 4.1 COL 16 COLON-ALIGNED HELP
          "Kort beskrivelse av kategorien"
     Merknad AT ROW 5.1 COL 16 COLON-ALIGNED HELP
          "Merknad"
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 2 ROW 3.62
...
/* SETTINGS FOR FRAME frmDetalj
                                                                        */
/* SETTINGS FOR FILL-IN KatNr IN FRAME frmDetalj
   NO-ENABLE                                                            */
...
ON END-ERROR OF C-Win /* Kategoriregister */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Kategoriregister */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* Kategoriregister */
DO:
...
DO:
  run d-bimpkonv.w (1006,
                    KatNr:SCREEN-VALUE,
                    "Kobling av kategorier"
                    ).
  return no-apply.
END.
...
END.
ELSE MESSAGE "kategori.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY KatNr Beskrivelse Merknad 
      WITH FRAME frmDetalj IN WINDOW C-Win.
  ENABLE B-KonvTbl Beskrivelse Merknad 
      WITH FRAME frmDetalj IN WINDOW C-Win.
...
cAdgang = (DYNAMIC-FUNCTION("getFieldValues","SysPara",
                            "WHERE SysHId = 16 and SysGr = 6 and ParaNr = 2","Parameter1")).  
...
DO WITH FRAME frmListe:
  hBrowseListe = DYNAMIC-FUNCTION("NewBrowse",         /* Create a browse object */
                    ...
                    */
                    "Kategori;KatNr|KategoriNr|>9;Beskrivelse;EDato;BrukerId",
                    "WHERE true",
  ...
  */
  ...
  /*DYNAMIC-FUNCTION("setAttribute",hBrowseListe,"basequery","WHERE Individ.KatNr = 1 ").*/
...
/*     DYNAMIC-FUNCTION("setAttribute",hFieldMap,"disabledFields","KatNr"). */
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
      ASSIGN
          KatNr:SENSITIVE = TRUE
          .
      APPLY "ENTRY" TO KatNr.
      RETURN NO-APPLY.
  END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
    ...
    IF cState = "NEW" THEN
    DO:
      IF int(KatNr:screen-value) = 0 THEN
      DO:
          MESSAGE "Angi et kategorinummer større enn 0."
              VIEW-AS ALERT-BOX WARNING BUTTONS OK.
          RETURN NO-APPLY.
      END.
      IF int(KatNr:screen-value) = int(DYNAMIC-FUNCTION("getFieldValues","Kategori","WHERE KatNr = int(" + KatNr:SCREEN-VALUE + ")","KatNr")) THEN
      DO:
          MESSAGE "Kategori eksisterer fra før med type " + KatNr:SCREEN-VALUE + "."
