...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEF VAR iLev AS INT NO-UNDO.
  iLev = 10.
&ELSE
  DEF INPUT PARAMETER iLevNr AS INT NO-UNDO.
&ENDIF
...
DEFINE VARIABLE Merknad AS CHARACTER 
     ...
     SIZE 62 BY 3.52 NO-UNDO.

DEFINE VARIABLE E_MailKontakt AS CHARACTER FORMAT "X(40)" 
     LABEL "E-Mail kontakt" 
     VIEW-AS FILL-IN 
     SIZE 42 BY 1.

DEFINE VARIABLE KontNr AS INTEGER FORMAT ">>9" INITIAL 0 
     LABEL "Nr" 
     VIEW-AS FILL-IN 
     SIZE 6.2 BY 1.
...
DEFINE VARIABLE Mobiltelefon AS CHARACTER FORMAT "X(15)" 
     LABEL "Mobil" 
     VIEW-AS FILL-IN 
     SIZE 17 BY 1.
...
DEFINE VARIABLE Stilling AS CHARACTER FORMAT "X(30)" 
     LABEL "Stilling" 
     VIEW-AS FILL-IN 
     SIZE 32 BY 1.
...
DEFINE FRAME frmDetalj
     LevNr AT ROW 1.33 COL 16 COLON-ALIGNED HELP
          "Leverandørnummer."
     KontNr AT ROW 2.33 COL 16 COLON-ALIGNED HELP
          "Kontaktnr. hos leverandør"
     Navn AT ROW 3.33 COL 16 COLON-ALIGNED HELP
          "Kontaktpersonens navn"
     Telefon AT ROW 4.33 COL 16 COLON-ALIGNED HELP
          "Telefonnummer til kontakt"
     Telefaks AT ROW 5.33 COL 16 COLON-ALIGNED HELP
          "Telefaksnummer til kontakt"
     Mobiltelefon AT ROW 6.33 COL 16 COLON-ALIGNED HELP
          "Mobiltelefonnummer til kontakt"
     E_MailKontakt AT ROW 7.33 COL 16 COLON-ALIGNED HELP
          "E-Mail adresse til kontaktperson hos leverandør."
     Stilling AT ROW 8.33 COL 16 COLON-ALIGNED HELP
          "Stilling"
     Merknad AT ROW 9.48 COL 18 HELP
          "Merknad" NO-LABEL
...
/* SETTINGS FOR FRAME frmDetalj
                                                                        */
/* SETTINGS FOR FILL-IN LevNr IN FRAME frmDetalj
   NO-ENABLE                                                            */
...
ON END-ERROR OF C-Win /* Leverandørkontakt */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Leverandørkontakt */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* Leverandørkontakt */
DO:
...
------------------------------------------------------------------------------*/
  ...
  IF COM-SELF:SelectedItem:INDEX = 2 THEN
      APPLY "ENTRY" TO FRAME frmDetalj.
END PROCEDURE.
...
/*   DYNAMIC-FUNCTION("DoLockWindow",THIS-PROCEDURE:CURRENT-WINDOW). */
  RUN enable_UI.
  RUN InitWindow.
  hBrowseListe:SELECT-FOCUSED-ROW() NO-ERROR.
...
OCXFile = SEARCH( "levkontakt.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "levkontakt.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY LevNr KontNr Navn Telefon Telefaks Mobiltelefon E_MailKontakt Stilling 
          Merknad 
      WITH FRAME frmDetalj IN WINDOW C-Win.
  ENABLE RECT-4 KontNr Navn Telefon Telefaks Mobiltelefon E_MailKontakt 
         Stilling Merknad 
      WITH FRAME frmDetalj IN WINDOW C-Win.
...
DO WITH FRAME frmListe:
  hBrowseListe = DYNAMIC-FUNCTION("NewBrowse",         /* Create a browse object */
                    ...
                    */
                    "LevKontakt;LevNr;KontNr;Navn;Telefon;Telefaks;Mobiltelefon;E_MailKontakt|E-Mail;Stilling;Merknad",
                    "WHERE LevKontakt.LevNr = " + STRING(iLevNr),
  ...
  */
  ...
  /*DYNAMIC-FUNCTION("setAttribute",hBrowseListe,"basequery","WHERE Individ.KontNr = 1 ").*/
...
DO WITH FRAME frmDetalj:
  ...
  hFieldMap = DYNAMIC-FUNCTION("NewFieldMap",      /* A fieldmap object holds extra info for display and input fields (fill-ins)
                    ...
                    "LevNr,KontNr,Navn,Telefon,Mobiltelefon,Telefaks,E_MailKontakt,Stilling,Merknad"
                       ,       
                    "LevNr,KontNr,Navn,Telefon,Mobiltelefon,Telefaks,E_MailKontakt,Stilling,Merknad"
                       ,       
...
/*   DYNAMIC-FUNCTION("setAttribute",hFieldMap,"customUpdateValProc","ignore"). */
  ...
  DYNAMIC-FUNCTION("setAttribute",hFieldMap,"disabledFields","LevNr,KontNr").

    ASSIGN
     LevNr:SCREEN-VALUE = STRING(iLevNr).
...
/*   DYNAMIC-FUNCTION("DumpAllLinks",THIS-PROCEDURE:CURRENT-WINDOW,"c:\temp\links.txt").  */
  ...
  DYNAMIC-FUNCTION("setOrgWinSize", THIS-PROCEDURE:CURRENT-WINDOW,250,500,0,0).
...
RUN InvokeMethod(hBrowseListe,"OpenQuery").
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
      ASSIGN
          LevNr:SCREEN-VALUE = STRING(iLevNr)
          KontNr:SENSITIVE = TRUE
          .
      APPLY "ENTRY" TO KontNr.
  END.
