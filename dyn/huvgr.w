...
DEFINE VARIABLE HgBeskr AS CHARACTER FORMAT "X(30)" 
     LABEL "Beskrivelse" 
     VIEW-AS FILL-IN 
     SIZE 46 BY 1.
...
DEFINE FRAME frmDetalj
     ...
     btnSokBrGrp-2 AT ROW 4 COL 31.4 NO-TAB-STOP 
     Hg AT ROW 1.95 COL 19 COLON-ALIGNED HELP
          "Hovedgruppe"
     HgBeskr AT ROW 2.95 COL 19 COLON-ALIGNED HELP
          "Beskrivelse av innbetalingstype."
     AvdelingNr AT ROW 3.95 COL 19 COLON-ALIGNED HELP
          "Kontonummer i regnskap"
     AvdelingNavn AT ROW 3.95 COL 33.8 COLON-ALIGNED HELP
          "Kontonavn" NO-LABEL
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 2 ROW 3.62
...
/* SETTINGS FOR FRAME frmDetalj
                                                                        */
/* SETTINGS FOR FILL-IN AvdelingNavn IN FRAME frmDetalj
   NO-ENABLE                                                            */
...
ON END-ERROR OF C-Win /* Hovedgrupper */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Hovedgrupper */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* Hovedgrupper */
DO:
...
&Scoped-define SELF-NAME AvdelingNr
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL AvdelingNr C-Win
ON RETURN OF AvdelingNr IN FRAME frmDetalj /* Avdelingsnr */
OR "TAB" OF AvdelingNr
DO:
  IF SELF:MODIFIED THEN
  DO:
      AvdelingNavn:SCREEN-VALUE = DYNAMIC-FUNCTION("getFieldValues","Avdeling",
                                              "WHERE AvdelingNr = '" + AvdelingNr:SCREEN-VALUE + "'","AvdelingNavn").  
  END.
END.
...
DO:
  run d-bimpkonv.w (1502,
                    HG:SCREEN-VALUE,
                    "Kobling av hovedgrupper"
                    ).
  return no-apply.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btnSokBrGrp-2 C-Win
ON CHOOSE OF btnSokBrGrp-2 IN FRAME frmDetalj /* ... */
OR F10 OF AvdelingNr
DO:
...
END.
ELSE MESSAGE "huvgr.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY Hg HgBeskr AvdelingNr AvdelingNavn 
      WITH FRAME frmDetalj IN WINDOW C-Win.
  ENABLE B-KonvTbl btnSokBrGrp-2 Hg HgBeskr AvdelingNr 
      WITH FRAME frmDetalj IN WINDOW C-Win.
...
cAdgang = (DYNAMIC-FUNCTION("getFieldValues","SysPara",
                            "WHERE SysHId = 16 and SysGr = 5 and ParaNr = 2","Parameter1")).  
...
DO WITH FRAME frmListe:
  hBrowseListe = DYNAMIC-FUNCTION("NewBrowse",         /* Create a browse object */
                    ...
                    */
                    "HuvGr;Hg;HgBeskr;AvdelingNr;EDato;BrukerId,Avdeling;AvdelingNavn",
                    "WHERE true, FIRST Avdeling NO-LOCK where Avdeling.AvdelingNr = HuvGr.AvdelingNr OUTER-JOIN",
                    "sort|Hg").                            /* Misc - for something I might need in next version.. */
  ...
  /* Flytter kolonner */
  hBrowseListe:MOVE-COLUMN(3,1).
  hBrowseListe:MOVE-COLUMN(6,2).
...
/*   hBrowseListe:GET-BROWSE-COLUMN(2):LABEL = "Type". */
  ...
  */
  ...
  /*DYNAMIC-FUNCTION("setAttribute",hBrowseListe,"basequery","WHERE Individ.Hg = 1 ").*/
  /*DYNAMIC-FUNCTION("setAttribute",hBrowseListe,"sortmap","cRegTid;Tid").*/
/*   DYNAMIC-FUNCTION("setAttribute",hBrowseListe,"sortmap","cAvdelingNr;AvdelingNr"). */
/*   DYNAMIC-FUNCTION("setAttribute",hBrowseListe,"NoColumnSearch","cAvdelingNr").     */
...
DO WITH FRAME frmDetalj:
  ...
  hFieldMap = DYNAMIC-FUNCTION("NewFieldMap",      /* A fieldmap object holds extra info for display and input fields (fill-ins)
                    ...
                    "Hg,HgBeskr,AvdelingNr"
                       ,       
                    "Hg,HgBeskr,AvdelingNr"
                       ,       
                    "AvdelingNavn", /* Additional buffer and displ.fields - not updateable*/
                    "AvdelingNavn",  
...
/*     DYNAMIC-FUNCTION("setAttribute",hFieldMap,"disabledFields","Hg"). */
...
/*   DYNAMIC-FUNCTION("DumpAllLinks",THIS-PROCEDURE:CURRENT-WINDOW,"c:\temp\links.txt").  */
  ...
  DYNAMIC-FUNCTION("setNoResizeX", THIS-PROCEDURE:CURRENT-WINDOW, FRAME {&FRAME-NAME}:HANDLE, "RectBrowseSearchListe").
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
    ...
    DO:
      ...
      DO:
          MESSAGE "Angi en hovedgruppe større enn 0."
              VIEW-AS ALERT-BOX WARNING BUTTONS OK.
          RETURN NO-APPLY.
      END.
      IF int(Hg:screen-value) = int(DYNAMIC-FUNCTION("getFieldValues","HuvGr","WHERE Hg = int(" + Hg:SCREEN-VALUE + ")","Hg")) THEN
      DO:
          MESSAGE "Hg eksisterer fra før med nr. " + Hg:SCREEN-VALUE + "."
              VIEW-AS ALERT-BOX WARNING BUTTONS OK  TITLE "Lagringskontroll".
          APPLY "ENTRY" TO Hg.
          RETURN NO-APPLY.
      END.
    END.
    IF HgBeskr:SCREEN-VALUE = "" THEN
    DO:
        MESSAGE "Hovedgruppetekst må angis."
            VIEW-AS ALERT-BOX WARNING BUTTONS OK.
        APPLY "ENTRY" TO HgBeskr.
        RETURN NO-APPLY.
    END.
    ...
    DO:
        MESSAGE "Avdeling må angis."
            VIEW-AS ALERT-BOX WARNING BUTTONS OK.
        APPLY "ENTRY" TO AvdelingNr.
        RETURN NO-APPLY.
    END.
    IF DYNAMIC-FUNCTION("getFieldValues","Avdeling","WHERE AvdelingNr = '" + AvdelingNr:SCREEN-VALUE + "'","AvdelingNavn") = ? THEN
    DO:
        MESSAGE "Ugyldig avdeling."
            VIEW-AS ALERT-BOX WARNING BUTTONS OK.
        APPLY "ENTRY" TO AvdelingNr.
        RETURN NO-APPLY.
    END.
