...
DEFINE BUTTON btnTransfer  NO-FOCUS
     LABEL "Overfør utvalg til telleliste" 
     SIZE 36 BY 1.19.
...
ON CHOOSE OF btnTransfer IN FRAME DEFAULT-FRAME /* Overfør utvalg til telleliste */
DO:  
  RUN TransferUtvalg.
END.
...
DEF VAR iTellenr           AS INT   NO-UNDO.
  ...
  DO:
      MESSAGE "Du kan ikke hente inn lokasjonslister i en lokasjonsliste." SKIP
              "Marker en telleliste før du velger denne funksjonen."
          VIEW-AS ALERT-BOX INFO BUTTONS OK.
      RETURN.
  END.
...
/*   DYNAMIC-FUNCTION("setAttribute",hBrowse,"customDeleteValProc","ignore"). */
  ...
  hToolbar = DYNAMIC-FUNCTION("NewToolBar",
                    ...
                    + ",Utvalg,HentLokasjon;Hent lokasjon,OppdaterSum;Oppdater sum felt på valgt linje"
                    /*+ ",FriskOppBrows;Frisk opp vindu"*/
                    + ",HTsetup;Håndterminal oppsett"
                    + ",TilHT;Eksporter vareregister til håndterminal"
...
------------------------------------------------------------------------------*/
  ...
  MESSAGE "Ved kontrolltelling, kan tellelisten initieres for deler av varelageret. Dette gjøres via artikkelutvalget." SKIP
              "Ønsker du å gjøre dette nå, eller vil du gjøre dette senere? " SKIP
              "Bruk funksjonen 'Utvalg' for å gjøre det senere." SKIP
              "Bygg utvalg (Ja). Vent til senere med å bygge telleliste (Nei)."
...
------------------------------------------------------------------------------*/
  ...
  DO:
      ...
      MESSAGE "Denne funksjonen skal bare benyttes i forbindelse med korreksjon av en varetelling, " + 
              "og da bare i sammarbeide med support. Er du sikker på at du vil gjøre dette?" 
          VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO UPDATE bOk.
      IF bOk = FALSE THEN
          RETURN NO-APPLY.
  END.
...
------------------------------------------------------------------------------*/
    ...
    IF iTypeId = 0 THEN DO:
        MESSAGE "Ingen håndterminaltype ble valgt."
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
        RETURN.
    END.
    
    MESSAGE 'Starte utlegg av ALLE varer (med EAN koder) til håndterminal?' + CHR(10) 
    ...
    ELSE DO:
        ...
        MESSAGE "Utlegg til håndterminal klar."
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
    END.
...
/*     APPLY "choose" TO B-Linjer IN FRAME {&FRAME-NAME}. */
  ...
  ELSE IF hBuffer:AVAILABLE THEN 
    DYNAMIC-FUNCTION("DoMessage",0,0,"Telling &1 er oppdatert og kan ikke tilføres flere artikler","",STRING(hBuffer:BUFFER-FIELD('tellenr'):BUFFER-VALUE)).
  ELSE 
...
------------------------------------------------------------------------------*/
  ...
  IF hBuffer:BUFFER-FIELD('TelleType'):BUFFER-VALUE = "1" THEN 
      MESSAGE "Ved å velge Full varetelling (Ja), bygges komplett telleliste. " SKIP
              "Bygging av full telleliste går hurtig. Alternativt kan man bygge " SKIP
              "telleliste på deler av artikkelregisteret (Nei)." SKIP
              "Ved bygging av telleliste på deler av artikkelregisteret, benyttes artikkelutvalget." SKIP(1)
              "Bygg fullstendig telleliste (Ja), bruk artikkelutvalget til å bygge telleliste (Nei)."
          VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO-CANCEL UPDATE svar AS LOGICAL.
  IF svar = ? THEN
      RETURN NO-APPLY.
  ELSE IF svar = TRUE THEN DO:
