...
DEFINE VARIABLE TypeId AS INTEGER FORMAT ">>>9" INITIAL 0 
     LABEL "TypeId" 
     VIEW-AS FILL-IN 
     SIZE 10 BY 1.
...
DEFINE FRAME frmDetalj
     TypeId AT ROW 1.95 COL 19 COLON-ALIGNED HELP
          "KundeType"
...
ON END-ERROR OF C-Win /* KundeType register */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* KundeType register */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* KundeType register */
DO:
...
OCXFile = SEARCH( "kundetype.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "kundetype.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY TypeId Beskrivelse 
      WITH FRAME frmDetalj IN WINDOW C-Win.
  ENABLE TypeId Beskrivelse 
      WITH FRAME frmDetalj IN WINDOW C-Win.
...
DO WITH FRAME frmListe:
  hBrowseListe = DYNAMIC-FUNCTION("NewBrowse",         /* Create a browse object */
                    ...
                    */
                    "KundeType;TypeId|Knd.grp.nr|>>>9;Beskrivelse;EDato;BrukerId",
                    "WHERE true",
                    "sort|TypeId").                            /* Misc - for something I might need in next version.. */
  ...
  */
  ...
  /*DYNAMIC-FUNCTION("setAttribute",hBrowseListe,"basequery","WHERE Individ.TypeId = 1 ").*/
...
/*     DYNAMIC-FUNCTION("setAttribute",hFieldMap,"disabledFields","TypeId"). */
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
      ASSIGN
          TypeId:SENSITIVE = TRUE
          .
      APPLY "ENTRY" TO TypeId.
      RETURN NO-APPLY.
  END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
    ...
    IF cState = "NEW" THEN
    DO:
      IF int(TypeId:screen-value) = 0 THEN
      DO:
          MESSAGE "Angi en innbetalingstype større enn 0."
              VIEW-AS ALERT-BOX WARNING BUTTONS OK.
          RETURN NO-APPLY.
      END.
      IF int(TypeId:screen-value) = int(DYNAMIC-FUNCTION("getFieldValues","KundeType","WHERE TypeId = int(" + TypeId:SCREEN-VALUE + ")","TypeId")) THEN
      DO:
          MESSAGE "TypeId eksisterer fra før med type " + TypeId:SCREEN-VALUE + "."
