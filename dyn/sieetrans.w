...
rectWinToolbar RECT-64 searchField fiButikkNr fiFraSalgsDato fiTilSalgsDato ~
fiFraEkspDato fiTilEkspDato fiFraRegDato fiTilRegDato fiMerknad fiFraEDato ~
fiTilEDato BUTTON-1 btnCalFraDato-9 btnCalFraSalgsDato ButikkNr Merknad ~
btnCalFraDato-6 Notat btnCalFraDato-10 btnCalFraDato-11 btnCalFraDato-7 ~
btnCalFraDato-12 btnCalFraDato-8 B-sokButik 
&Scoped-Define DISPLAYED-OBJECTS fiButikkNr fiFraSalgsDato fiTilSalgsDato ~
fiFraEkspDato fiTilEkspDato fiFraRegDato fiTilRegDato fiMerknad fiFraEDato ~
fiTilEDato SIEEksportNr ButikkNr Merknad Notat 
...
DEFINE VARIABLE fiFraEkspDato AS DATE FORMAT "99/99/99" 
...
DEFINE VARIABLE fiTilEkspDato AS DATE FORMAT "99/99/99" 
     VIEW-AS FILL-IN 
     SIZE 16 BY 1.
...
DEFINE VARIABLE SIEEksportNr AS DECIMAL FORMAT ">>>>>>>>9" INITIAL 0 
     LABEL "Eksportnr" 
     VIEW-AS FILL-IN 
     SIZE 16 BY 1.
...
DEFINE FRAME DEFAULT-FRAME
     ...
     fiFraEkspDato AT ROW 2.43 COL 142 COLON-ALIGNED HELP
          "Siste gyldighetsdato på rabattsjekken"
     fiTilEkspDato AT ROW 2.43 COL 164 COLON-ALIGNED HELP
          "Siste gyldighetsdato på rabattsjekken" NO-LABEL
     ...
     SIEEksportNr AT ROW 8.62 COL 159.8 COLON-ALIGNED HELP
...
/* SETTINGS FOR FILL-IN SIEEksportNr IN FRAME DEFAULT-FRAME
   NO-ENABLE                                                            */
...
OR F10 OF fiTilEkspDato
DO:
  RUN Cal.w (fiTilEkspDato:HANDLE).
  RUN InvokeMethod (hBrowse,'OpenQuery').
...
OR F10 OF fiFraEkspDato
DO:
  RUN Cal.w (fiFraEkspDato:HANDLE).
  RUN InvokeMethod (hBrowse,'OpenQuery').
...
&Scoped-define SELF-NAME fiFraEkspDato
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fiFraEkspDato C-Win
ON LEAVE OF fiFraEkspDato IN FRAME DEFAULT-FRAME /* Fra/til eksport dato */
DO:
    RUN InvokeMethod (hBrowse,'OpenQuery').
...
&Scoped-define SELF-NAME fiTilEkspDato
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fiTilEkspDato C-Win
ON LEAVE OF fiTilEkspDato IN FRAME DEFAULT-FRAME
DO:
    RUN InvokeMethod (hBrowse,'OpenQuery').
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN wbyggSIEEksport.w PERSISTENT SET hwgenData.
...
------------------------------------------------------------------------------*/
  DO WITH FRAME {&FRAME-NAME}:
      ASSIGN
          ...
          fiFraEkspDato:SCREEN-VALUE   = ?
          fiTilEkspDato:SCREEN-VALUE   = ?
...
IF iReturn = 1 THEN
  bOk = DYNAMIC-FUNCTION("ProcessQuery",hBrowse,"sieeksport_delete.p",'').
...
DO:
  ...
  END.
  bOk = DYNAMIC-FUNCTION("ProcessSelectedRows",hBrowse,"sieeksport_delete.p",'').
END.
ELSE
  LEAVE.
IF NOT bOK THEN
...
IF iReturn = 1 THEN
  bOk = DYNAMIC-FUNCTION("ProcessQuery",hBrowse,"sieeksport_eksporter.p",'').
ELSE IF iReturn = 2 THEN
DO:
  DO ix = 1 TO hBrowse:NUM-SELECTED-ROWS:
    IF hBrowse:FETCH-SELECTED-ROW(ix) THEN
      cRowIdList = cRowIdList + hBrowse:QUERY:GET-BUFFER-HANDLE(1):BUFFER-FIELD('RowIdent1'):BUFFER-VALUE + ','.
  END.
  bOk = DYNAMIC-FUNCTION("ProcessSelectedRows",hBrowse,"sieeksport_eksporter.p",'').
END.
ELSE
  LEAVE.
IF NOT bOK THEN
...
------------------------------------------------------------------------------*/
  DISPLAY fiButikkNr fiFraSalgsDato fiTilSalgsDato fiFraEkspDato fiTilEkspDato 
          fiFraRegDato fiTilRegDato fiMerknad fiFraEDato fiTilEDato SIEEksportNr 
          ButikkNr Merknad Notat 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE B-sokButik-2 rectBrowse rectToolBar rectWinToolbar RECT-64 searchField 
         fiButikkNr fiFraSalgsDato fiTilSalgsDato fiFraEkspDato fiTilEkspDato 
         fiFraRegDato fiTilRegDato fiMerknad fiFraEDato fiTilEDato BUTTON-1 
         btnCalFraDato-9 btnCalFraSalgsDato ButikkNr Merknad btnCalFraDato-6 
         Notat btnCalFraDato-10 btnCalFraDato-11 btnCalFraDato-7 
         btnCalFraDato-12 btnCalFraDato-8 B-sokButik 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  hFieldMap = DYNAMIC-FUNCTION("NewFieldMap",    
                    ...
                    "SIEEksportNr,ButikkNr,Merknad,Notat",              
                    "",              
                    "","",                       
                    "").            
  ...
  DYNAMIC-FUNCTION('setAttribute',hBrowse,'calcfieldproc','sieeksport_brwcalc.p').
...
DYNAMIC-FUNCTION("setAddMoveX",THIS-PROCEDURE:CURRENT-WINDOW,FRAME {&FRAME-NAME}:HANDLE,
                 "SIEEksportNr,B-sokButik,ButikkNr,Merknad,Notat").
...
DEF VAR iTilETid AS INT NO-UNDO.
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    cWhere = buildFilter(cWhere,fiButikkNr:HANDLE,'TGStore_Id','EQ').
    cWhere = cWhere + buildFilter(cWhere,fiMerknad:HANDLE,'Merknad','BEGINS').
    cWhere = cWhere + buildFilter(cWhere,fiFraEkspDato:HANDLE,'EkspDato','GE').
    cWhere = cWhere + buildFilter(cWhere,fiTilEkspDato:HANDLE,'EkspDato','LE').
    cWhere = cWhere + buildFilter(cWhere,fiFraSalgsDato:HANDLE,'SalgsDato','GE').
    cWhere = cWhere + buildFilter(cWhere,fiTilSalgsDato:HANDLE,'SalgsDato','LE').
    ...
    ASSIGN
        ...
        fiFraEkspDato:MODIFIED   = FALSE
        fiTilEkspDato:MODIFIED   = FALSE
