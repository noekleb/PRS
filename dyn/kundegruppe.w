...
DEFINE VARIABLE GruppeId AS INTEGER FORMAT ">>>9" INITIAL 0 
     LABEL "GruppeId" 
     VIEW-AS FILL-IN 
     SIZE 10 BY 1.
...
DEFINE FRAME frmDetalj
     GruppeId AT ROW 1.95 COL 19 COLON-ALIGNED HELP
          "Kundegruppe"
...
ON END-ERROR OF C-Win /* Kundegruppe register */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Kundegruppe register */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* Kundegruppe register */
DO:
...
OCXFile = SEARCH( "kundegruppe.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "kundegruppe.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY GruppeId Beskrivelse 
      WITH FRAME frmDetalj IN WINDOW C-Win.
  ENABLE GruppeId Beskrivelse 
      WITH FRAME frmDetalj IN WINDOW C-Win.
...
DO WITH FRAME frmListe:
  hBrowseListe = DYNAMIC-FUNCTION("NewBrowse",         /* Create a browse object */
                    ...
                    */
                    "KundeGruppe;GruppeId|Knd.grp.nr|>>>9;Beskrivelse;EDato;BrukerId",
                    "WHERE true",
  ...
  */
  ...
  /*DYNAMIC-FUNCTION("setAttribute",hBrowseListe,"basequery","WHERE Individ.GruppeId = 1 ").*/
...
/*     DYNAMIC-FUNCTION("setAttribute",hFieldMap,"disabledFields","GruppeId"). */
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
      ASSIGN
          GruppeId:SENSITIVE = TRUE
          .
      APPLY "ENTRY" TO GruppeId.
      RETURN NO-APPLY.
  END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
    ...
    IF cState = "NEW" THEN
    DO:
      IF int(GruppeId:screen-value) = 0 THEN
      DO:
          MESSAGE "Angi en innbetalingstype større enn 0."
              VIEW-AS ALERT-BOX WARNING BUTTONS OK.
          RETURN NO-APPLY.
      END.
      IF int(GruppeId:screen-value) = int(DYNAMIC-FUNCTION("getFieldValues","Kundegruppe","WHERE GruppeId = int(" + GruppeId:SCREEN-VALUE + ")","GruppeId")) THEN
      DO:
          MESSAGE "GruppeId eksisterer fra før med type " + GruppeId:SCREEN-VALUE + "."
