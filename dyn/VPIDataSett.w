...
DEF VAR bVisVarebok     AS LOG    NO-UNDO.
...
&Scoped-Define ENABLED-OBJECTS rectWinToolbar rectFolder EkstVPILevNr 
&Scoped-Define DISPLAYED-OBJECTS EkstVPILevNr FilId Beskrivelse ImportDato 
...
DEFINE VARIABLE EkstVPILevNr AS INTEGER FORMAT "->>>>>>>9":U INITIAL 0 
     LABEL "VPI" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME DEFAULT-FRAME
     EkstVPILevNr AT ROW 1.19 COL 10.2 COLON-ALIGNED
     FilId AT ROW 1.19 COL 41.4 COLON-ALIGNED
     Beskrivelse AT ROW 1.19 COL 70.4 COLON-ALIGNED
     ImportDato AT ROW 1.19 COL 124.4 COLON-ALIGNED
     rectWinToolbar AT ROW 1.24 COL 182.6
     rectFolder AT ROW 2.29 COL 1
...
ON END-ERROR OF C-Win /* VPI mottakskontroll */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* VPI mottakskontroll */
DO:
...
------------------------------------------------------------------------------*/
  DISPLAY EkstVPILevNr FilId Beskrivelse ImportDato 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE rectWinToolbar rectFolder EkstVPILevNr 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  cTekst = DYNAMIC-FUNCTION("getFieldValues","SysPara","WHERE SysHId = 50 and SysGr =  25 and ParaNr = 2","Parameter1").
  ...
  IF bVisVareBok THEN
      DYNAMIC-FUNCTION("InitPages" IN hTabFolder,
                   "VPI leverandør|VPIDatasettBrw.w|Avstem mot artikkel register|VPIArtBas_artregBrw.w|Avstem mot varebok register|VPIArtBas_varebokBrw.w",hBrowse).
  ELSE 
      DYNAMIC-FUNCTION("InitPages" IN hTabFolder,
               "VPI leverandør|VPIDatasettBrw.w|Avstem mot artikkel register|VPIArtBas_artregBrw.w",hBrowse).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setEkstVPILevNr C-Win 
PROCEDURE setEkstVPILevNr :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  IF VALID-HANDLE(hCurrTabProc) THEN
  DO:
      RUN setEkstVPILev IN hCurrTabProc (piEkstVPILevNr) NO-ERROR.
  END.
