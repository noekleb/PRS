...
DEFINE VARIABLE MedGruppe AS INTEGER FORMAT ">>>9" INITIAL 0 
     LABEL "MedGruppe" 
     VIEW-AS FILL-IN 
     SIZE 7 BY 1.
...
DEFINE FRAME frmDetalj
     MedGruppe AT ROW 1.95 COL 19 COLON-ALIGNED HELP
          "MedlemsGruppe"
     Beskrivelse AT ROW 2.95 COL 19 COLON-ALIGNED HELP
          "Beskrivelse"
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 2 ROW 3.62
...
ON END-ERROR OF C-Win /* MedlemsGruppe register */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* MedlemsGruppe register */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* MedlemsGruppe register */
DO:
...
OCXFile = SEARCH( "medlemsgruppe.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "medlemsgruppe.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY MedGruppe Beskrivelse 
      WITH FRAME frmDetalj IN WINDOW C-Win.
  ENABLE MedGruppe Beskrivelse 
      WITH FRAME frmDetalj IN WINDOW C-Win.
...
DO WITH FRAME frmListe:
  hBrowseListe = DYNAMIC-FUNCTION("NewBrowse",         /* Create a browse object */
                    ...
                    */
                    "MedlemsGruppe;MedGruppe|Knd.grp.nr|>>>9;Beskrivelse;EDato;BrukerId",
                    "WHERE true",
                    "sort|MedGruppe").                            /* Misc - for something I might need in next version.. */
  ...
  */
  ...
  /*DYNAMIC-FUNCTION("setAttribute",hBrowseListe,"basequery","WHERE Individ.MedGruppe = 1 ").*/
...
/*     DYNAMIC-FUNCTION("setAttribute",hFieldMap,"disabledFields","MedGruppe"). */
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
      ASSIGN
          MedGruppe:SENSITIVE = TRUE
          .
      APPLY "ENTRY" TO MedGruppe.
      RETURN NO-APPLY.
  END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME frmDetalj:
    ...
    IF cState = "NEW" THEN
    DO:
      IF int(MedGruppe:screen-value) = 0 THEN
      DO:
          MESSAGE "Angi en innbetalingstype større enn 0."
              VIEW-AS ALERT-BOX WARNING BUTTONS OK.
          RETURN NO-APPLY.
      END.
      IF int(MedGruppe:screen-value) = int(DYNAMIC-FUNCTION("getFieldValues","MedlemsGruppe","WHERE MedGruppe = int(" + MedGruppe:SCREEN-VALUE + ")","MedGruppe")) THEN
      DO:
          MESSAGE "MedGruppe eksisterer fra før med type " + MedGruppe:SCREEN-VALUE + "."
