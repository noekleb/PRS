...
FIND OrderLine WHERE ROWID(OrderLine) = TO-ROWID(icRowId) EXCLUSIVE-LOCK NO-ERROR.
IF AVAIL OrderLine THEN DO:
  DELETE OrderLine NO-ERROR.
  ocReturn = ERROR-STATUS:GET-MESSAGE(1).
  DYNAMIC-FUNCTION("SuppressDynDelete" IN SOURCE-PROCEDURE).
END.
