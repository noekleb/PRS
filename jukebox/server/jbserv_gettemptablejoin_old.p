...
FOR EACH ttSpecialFields:
   ...
   LogThis(ttSpecialFields.cFillField, 
         ...
         + " CalcSortFld: " + STRING(ttSpecialFields.cCalcSortField NE "")
         + " Filter operator: " + ttSpecialFields.cFilterOperator
...
FOR EACH ttSpecialFields
    WHERE NOT ttSpecialFields.b2phAcc
      and (ttSpecialFields.bDistinct OR ttSpecialFields.cCalcSortField NE "" or ttSpecialFields.bGrandTotal 
           or ttSpecialFields.cFilterOperator ne "" OR ttSpecialFields.cFillField = "jbCountDistinct" OR bDistinctRows)
    ,first ttRowSpecialValues of ttSpecialFields
    :
...
IF cLogFile NE "" AND CAN-FIND(FIRST ttSpecialFields WHERE ttSpecialFields.bCalcByPreScan) THEN DO:
  LogThis("Calculations done during prescan:","",0).
  FOR EACH ttSpecialFields
      WHERE ttSpecialFields.bCalcByPreScan:
    LogThis(ttSpecialFields.cFillField, 
          ttSpecialFields.cCallProc + "(" 
        + ttSpecialFields.cCallProcParam + ")" 
        + " Distinct: " + STRING(ttSpecialFields.bDistinct)
        + " CalcSortField NE '': " + STRING(ttSpecialFields.cCalcSortField NE "")
        + " GrandTotal: " + STRING(ttSpecialFields.bGrandTotal)
        + " cFilterOperator: " + ttSpecialFields.cFilterOperator
            ,0). 
  END.
  LogThis("","",1).
END.
  ...
  ELSE IF icStatFields NE "" AND NOT bPreScanDidAllCalc THEN DO:
    IF icStatFields NE "rowcount" OR bCheckForSkipRow OR cAccumFields NE "" THEN DO:
      DO iy = 1 TO iBufferCount:
        ...
        ELSE 
          FOR EACH ttRowSpecialValues 
              WHERE ttRowSpecialValues.iCallBufferNum = iy
                AND NOT ttRowSpecialValues.bCalcByPreScan:
            ttRowSpecialValues.fRowValue = 0.
          END.
      END.
...
DEF VAR fTot AS DEC NO-UNDO.
  ...
  FOR EACH ttRowSpecialValues
      WHERE ttRowSpecialValues.fGrandTotal NE ?: 
    logThis(ttRowSpecialValues.cFillField,STRING(ttRowSpecialValues.fGrandTotal),0).
    fTot = fTot + ttRowSpecialValues.fGrandTotal.
  END.
