...
DEF VAR bUseWithJukebox     AS LOG    NO-UNDO INIT YES.
...
DEF TEMP-TABLE ttNeverResizeX
    ...
    INDEX idxNeverWindow hWindow hFrame
...
DEF TEMP-TABLE ttStatusFrame
    FIELD hWindow          AS HANDLE
    FIELD hStatusFrame     AS HANDLE
    FIELD hStatusRectangle AS HANDLE
    FIELD hStatusTopLine   AS HANDLE
    FIELD bHiddenOnInit    AS LOG
    FIELD iDeltaHeight     AS INT.
...
&IF DEFINED(EXCLUDE-DoResizeSetting) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE DoResizeSetting Procedure
PROCEDURE DoResizeSetting:
/*------------------------------------------------------------------------------
 Purpose:
 Notes:
------------------------------------------------------------------------------*/
DEF INPUT PARAM icSetting         AS CHAR NO-UNDO.
DEF INPUT PARAM ihFrameOrSplitbar AS HANDLE NO-UNDO.
DEF INPUT PARAM icWidgetList      AS CHAR  NO-UNDO.

DYNAMIC-FUNCTION(icSetting, ihFrameOrSplitbar:WINDOW, ihFrameOrSplitbar,"+," + icWidgetList).
...
&IF DEFINED(EXCLUDE-DoSplitBar) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE DoSplitBar Procedure
PROCEDURE DoSplitBar:
/*------------------------------------------------------------------------------
 Purpose:
 Notes:
------------------------------------------------------------------------------*/
DEF INPUT PARAM icSetting       AS CHAR   NO-UNDO.
DEF INPUT PARAM ihSplitBarBtn   AS HANDLE NO-UNDO.
DEF INPUT PARAM icWidgetList    AS CHAR   NO-UNDO.
DEF INPUT PARAM iiMinXorY       AS INT    NO-UNDO.
DEF INPUT PARAM iiMaxXorY       AS INT    NO-UNDO.

IF icSetting MATCHES "*limits" THEN
  DYNAMIC-FUNCTION(icSetting,ihSplitBarBtn,iiMinXorY,iiMaxXorY).
ELSE IF icSetting BEGINS "setFollow" THEN  
  DYNAMIC-FUNCTION(icSetting, ihSplitBarBtn:WINDOW, ihSplitbarBtn,"+," + icWidgetList).
ELSE  
  DYNAMIC-FUNCTION(icSetting, ihSplitBarBtn:WINDOW, ihSplitbarBtn,NO).
...
&IF DEFINED(EXCLUDE-DoResizeGroup) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE DoResizeGroup Procedure
PROCEDURE DoResizeGroup:
/*------------------------------------------------------------------------------
 Purpose:
 Notes:
------------------------------------------------------------------------------*/
...
DEF INPUT PARAM ifPercent         AS DEC    NO-UNDO.
DEF INPUT PARAM icWidgetList      AS CHAR   NO-UNDO.

DYNAMIC-FUNCTION(icSetting, ihFrame:WINDOW, ihFrame,ifPercent,"+," + icWidgetList).
...
&IF DEFINED(EXCLUDE-DoRedrawWindow) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD DoRedrawWindow Procedure
FUNCTION DoRedrawWindow RETURNS LOGICAL 
  (INPUT ihWindow AS HANDLE) FORWARD.
...
&IF DEFINED(EXCLUDE-getFrameFields) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getFrameFields Procedure 
FUNCTION getFrameFields RETURNS CHARACTER
  ( INPUT ihFrame           AS HANDLE,
    INPUT icNameMask        AS CHAR,
    INPUT icTypeMask        AS CHAR,
    INPUT icScreenValueMask AS CHAR,
    INPUT icTrimPrefix      AS CHAR )  FORWARD.
...
&IF DEFINED(EXCLUDE-getUseResizeLibWithJukebox) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getUseResizeLibWithJukebox Procedure 
FUNCTION getUseResizeLibWithJukebox RETURNS LOGICAL
  ( /* parameter-definitions */ )  FORWARD.
...
&IF DEFINED(EXCLUDE-setUseResizeLibWithJukebox) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD setUseResizeLibWithJukebox Procedure 
FUNCTION setUseResizeLibWithJukebox RETURNS LOGICAL
  ( INPUT ibUseWithJukebox AS LOG )  FORWARD.
...
&IF DEFINED(EXCLUDE-StaticBrowseToExcel) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD StaticBrowseToExcel Procedure 
FUNCTION StaticBrowseToExcel RETURNS LOGICAL
...
SUBSCRIBE TO "SetStatusBarWidth" ANYWHERE.
SUBSCRIBE TO "DoResizeSetting" ANYWHERE.
SUBSCRIBE TO "DoSplitBar" ANYWHERE.
SUBSCRIBE TO "DoResizeGroup" ANYWHERE.
...
IF AVAIL ttOrgWinSize AND ((AVAIL ttLastInitFunction AND AVAIL ttSettingsLoaded) OR bJBoxUserSetting) THEN DO:
  ...
  FIND FIRST ttStatusFrame 
       WHERE ttStatusFrame.hWindow = ihWindow
       NO-ERROR.
  ...
  IF bJBoxUserSetting THEN DO:
    IF VALID-HANDLE(ttOrgWinSize.hProcedure) AND NOT DYNAMIC-FUNCTION("setUserSetting",
                    ...
                    ,(IF bSaveSize THEN
                       ...
                       STRING(ihWindow:HEIGHT-PIXELS - ttOrgWinSize.iHeightPixels - (IF ihWindow:STATUS-AREA THEN 5 ELSE 0) - (IF AVAIL ttStatusFrame THEN ttStatusFrame.iDeltaHeight ELSE 0)) +
  ...
  ELSE DO:
    ...
    EXPORT DELIMITER ";" 
        ...
        (IF bSaveSize THEN ihWindow:HEIGHT-PIXELS - ttOrgWinSize.iHeightPixels - 
                            (IF ihWindow:STATUS-AREA THEN 5 ELSE 0) - (IF AVAIL ttStatusFrame THEN ttStatusFrame.iDeltaHeight ELSE 0) ELSE ttSettingsLoaded.iCurrDeltaY)
...
IF icDirection = "X" THEN DO:
  ...
  IF NOT AVAIL ttNeverMoveX THEN DO:
    ...
    ASSIGN ttNeverMoveX.hWidget = ihWidget
           ttNeverMoveX.hFrame  = ihWidget:FRAME
...
ELSE DO:
  ...
  IF NOT AVAIL ttNeverMoveY THEN DO:
    ...
    ASSIGN ttNeverMoveY.hWidget = ihWidget
           ttNeverMoveY.hFrame  = ihWidget:FRAME
...
IF icDirection = "X" THEN DO:
  ...
  IF NOT AVAIL ttNeverResizeX THEN DO:
    ...
    ASSIGN ttNeverResizeX.hWidget = ihWidget
           ttNeverResizeX.hFrame  = ihWidget:FRAME
...
ELSE DO:
  ...
  IF NOT AVAIL ttNeverResizeY THEN DO:
    ...
    ASSIGN ttNeverResizeY.hWidget = ihWidget
           ttNeverResizeY.hFrame  = ihWidget:FRAME
...
&IF DEFINED(EXCLUDE-SetStatusBarWidth) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE SetStatusBarWidth Procedure
...
DEF VAR hRect      AS HANDLE NO-UNDO.
DEF VAR cPixelList AS CHAR NO-UNDO.
DEF VAR ix         AS INT NO-UNDO.
DEF VAR iTotPixels AS INT NO-UNDO.
DEF VAR iTotal     AS INT NO-UNDO.
DEF VAR iFields    AS INT NO-UNDO.
DEF VAR iOrgWidth1 AS INT NO-UNDO.
DEF VAR iNewWidth1 AS INT NO-UNDO.

cPixelList = DYNAMIC-FUNCTION("getAttribute",ihWindow,"StatusBarPixelList").

FIND FIRST ttStatusFrame WHERE ttStatusFrame.hWindow = ihWindow NO-ERROR.

IF NUM-ENTRIES(cPixelList) LE 1 OR NOT AVAIL ttStatusFrame THEN RETURN.

iOrgWidth1 = INT(ENTRY(1,cPixelList)).

DO ix = 2 TO NUM-ENTRIES(cPixelList):
  iTotPixels = iTotPixels + INTEGER(ENTRY(ix,cPixelList)) + {&rightLeft} * 4.
END.
iNewWidth1 = hWindow:WIDTH-PIXELS - {&rightLeft} * 4 - iTotPixels + NUM-ENTRIES(cPixelList) * 1.4 - 1.
ENTRY(1,cPixelList) = STRING(iNewWidth1).

IF iNewWidth1 > iOrgWidth1 THEN
  ASSIGN /* ttStatusFrame.hStatusFrame:VIRTUAL-WIDTH-PIXELS = ihWindow:WIDTH-PIXELS */
         ttStatusFrame.hStatusFrame:WIDTH-PIXELS = ihWindow:WIDTH-PIXELS
         ttStatusFrame.hStatusRectangle:WIDTH-PIXELS = ihWindow:WIDTH-PIXELS
         ttStatusFrame.hStatusTopLine:WIDTH-PIXELS = ihWindow:WIDTH-PIXELS
         .

DYNAMIC-FUNCTION("setAttribute",ihWindow,"StatusBarPixelList",cPixelList).

DO ix = 1 TO NUM-ENTRIES (cPixelList):

  iFields = iFields + 1.
  hText = WIDGET-HANDLE(DYNAMIC-FUNCTION("getAttribute",ihWindow,"StatusBarTextFillIn" + STRING(iFields))) NO-ERROR.
  IF NOT VALID-HANDLE(hText) THEN LEAVE.
  
  ASSIGN hText:X            = iTotal + {&rightLeft} + 2 + 1
         hText:WIDTH-PIXELS = INTEGER(ENTRY(ix,cPixelList)).

  hRect = WIDGET-HANDLE(DYNAMIC-FUNCTION("getAttribute",ihWindow,"StatusBarRectangle" + STRING(iFields))) NO-ERROR.
  IF NOT VALID-HANDLE(hRect) THEN LEAVE.
 
  ASSIGN hRect:X            = iTotal + {&rightLeft} + 1
         hRect:WIDTH-PIXELS = hText:WIDTH-PIXELS  + ({&rightLeft} * 2).
  
  iTotal =  hText:X + hText:WIDTH-PIXELS /* + {&topBottom} + 1*/.
END.

IF iNewWidth1 < iOrgWidth1 THEN 
  ASSIGN ttStatusFrame.hStatusRectangle:WIDTH-PIXELS = ihWindow:WIDTH-PIXELS
         ttStatusFrame.hStatusTopLine:WIDTH-PIXELS = ihWindow:WIDTH-PIXELS
         ttStatusFrame.hStatusFrame:VIRTUAL-WIDTH-PIXELS = ihWindow:WIDTH-PIXELS         
         ttStatusFrame.hStatusFrame:WIDTH-PIXELS = ihWindow:WIDTH-PIXELS
         .
...
DEFINE VARIABLE iOrgHeight          AS INT         NO-UNDO.
...
DYNAMIC-FUNCTION("setAttribute",ihWindow,"StatusBarPixelList",icFieldPixelList).
...
iOrgHeight = ihWindow:HEIGHT-PIXELS.
...
CREATE ttStatusFrame.
ASSIGN ttStatusFrame.hWindow = ihWindow
       ttStatusFrame.hStatusFrame = wFrame2
       ttStatusFrame.iDeltaHeight = ihWindow:HEIGHT-PIXELS - iOrgHeight.
...
ASSIGN cNoResizeYList = cNoResizeYList + "," + wRect1:NAME
       ...
       iTotal = 0
       ttStatusFrame.hStatusRectangle = wRectFrame
       ttStatusFrame.hStatusTopLine = wRect1
       .
...
IF ihWindow:HEIGHT-PIXELS = iOrgHeight THEN 
  ASSIGN wFrame2:HIDDEN = YES
         ttStatusFrame.bHiddenOnInit = YES.
...
IF NOT bUseWithJukebox THEN RETURN FALSE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION DoRedrawWindow Procedure
FUNCTION DoRedrawWindow RETURNS LOGICAL 
  (INPUT ihWindow AS HANDLE ):
/*------------------------------------------------------------------------------
 Purpose:
 Notes:
------------------------------------------------------------------------------*/
IF VALID-HANDLE(ihWindow) AND ihWindow:HWND NE ? THEN DO:
  RUN SendMessageW(ihWindow:HWND, 11, 1, 0, OUTPUT iRetVal).
  RUN RedrawWindow(ihWindow:HWND, 0, 0, 4 + 1 + 128, OUTPUT iRetVal).
  PUBLISH "ViewHideAllControls" (ihWindow,YES).
  RETURN YES.
END.
RETURN NO.
...
DO WHILE hWidget <> ? :
  ...
  /* Retrieve widget absolute position in window (and set parent frame - if not the widget is a frame). */
  IF VALID-HANDLE(hWidget:PARENT) THEN DO:
    ASSIGN hWidget2   = hWidget:PARENT
           iAbsoluteX = hWidget:X
           iAbsoluteY = hWidget:Y.
    REPEAT WHILE VALID-HANDLE(hWidget2) AND hWidget2:TYPE NE "WINDOW":
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION getFrameFields Procedure 
FUNCTION getFrameFields RETURNS CHARACTER
  ( INPUT ihFrame           AS HANDLE,
    INPUT icNameMask        AS CHAR,
    INPUT icTypeMask        AS CHAR,
    INPUT icScreenValueMask AS CHAR,
    INPUT icTrimPrefix      AS CHAR ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
...
REPEAT WHILE VALID-HANDLE(hWdgt):
  IF icTypeMask = "" OR hWdgt:TYPE = icTypeMask THEN DO:
    IF (INDEX(icNameMask,"*") > 0 AND hWdgt:NAME MATCHES icNameMask)
       OR (icNameMask NE "" AND hWdgt:NAME BEGINS icNameMask)
       OR (icNameMask = "") THEN DO:
  
      IF (INDEX(icScreenValueMask,"*") > 0 AND hWdgt:SCREEN-VALUE MATCHES icScreenValueMask)
          OR (icScreenValueMask NE "" AND hWdgt:SCREEN-VALUE BEGINS icScreenValueMask)
          OR (icScreenValueMask = "") THEN 
  
        cReturn = cReturn + (IF cReturn NE "" THEN "," ELSE "") + LEFT-TRIM(hWdgt:NAME,icTrimPrefix).         
    ...
    IF hWindow = ihWindow THEN DO:
      ...
      ELSE DO:
        ASSIGN httBuffer:BUFFER-FIELD("hFrame"):BUFFER-VALUE = WIDGET-HANDLE(hCurrBuffer:BUFFER-FIELD("chFrame"):BUFFER-VALUE)
               ...
               NO-ERROR.
        IF cCurrBufferName MATCHES("*Group") THEN
          httBuffer:BUFFER-FIELD("cSetting"):BUFFER-VALUE = httBuffer:BUFFER-FIELD("cSetting"):BUFFER-VALUE
                                                          + " ("
                                                          + STRING(hCurrBuffer:BUFFER-FIELD("fPercent"):BUFFER-VALUE) + "%)".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION getUseResizeLibWithJukebox Procedure 
FUNCTION getUseResizeLibWithJukebox RETURNS LOGICAL
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  
...
REPEAT WHILE hWidget NE ?:
  ...
  ELSE IF CAN-DO(icTypes,hWidget:TYPE) AND hWidget:TYPE MATCHES "*MENU*" THEN  
    cWidgetList = cWidgetList + STRING(hWidget) + ",".
...
REPEAT WHILE hWidget NE ?:
  ...
  IF hWidget:NAME BEGINS "rtf" THEN
    MESSAGE hWidget:NAME
        VIEW-AS ALERT-BOX INFO BUTTONS OK.
...
DEF VAR iAbsX               AS INT  NO-UNDO.
DEF VAR iAbsY               AS INT  NO-UNDO.
DEF VAR hCurrFrame          AS HANDLE NO-UNDO.
...
IF hWidget:TYPE = "FRAME" THEN hCurrFrame = hWidget. 

REPEAT WHILE hWidget NE ?:
  IF NOT CAN-DO(cLassoExceptionList,STRING(hWidget)) THEN DO:     
    IF CAN-DO(icTypes,hWidget:TYPE) AND (IF VALID-HANDLE(hCurrFrame) THEN hWidget:FRAME = hCurrFrame OR hWidget:TYPE = "FRAME" ELSE TRUE) THEN DO:
      ASSIGN iAbsX = getAbsPosition(hWidget,"X")
             iAbsY = getAbsPosition(hWidget,"Y")
             .
      IF iAbsX GE iMinX
...
IF AVAIL ttSplitBarX THEN DO:
  ASSIGN iOrgSplitXpos           = ttSplitBarX.hSplitBar:X
         ttSplitBarX.hSplitBar:X = ttSplitBarX.iOrgSplitBarX NO-ERROR.
...
END.
FOR EACH ttStatusFrame
    WHERE ttStatusFrame.hWindow = hWindow:
  DELETE ttStatusFrame.
END.
...
DO ix = 1 TO NUM-ENTRIES(cWidgetHandleList):
  ...
  IF NOT CAN-FIND(FIRST ttFollowSplitBarX
                  WHERE ttFollowSplitBarX.chWindow       = STRING(hWindow)
                    AND ttFollowSplitBarX.hSplitBar      = hSplitBar
                    AND ttFollowSplitBarX.hWidget        = WIDGET-HANDLE(ENTRY(ix,cWidgetHandleList))) THEN DO:
    CREATE ttFollowSplitBarX.
...
ELSE IF hLockWinForResize = ? THEN DO:
  IF VALID-HANDLE(hLockWindow) AND hLockWindow:HWND NE ? THEN DO:
    ...
    PUBLISH "ViewHideAllControls" (hLockWindow,YES).
  END.
  IF hCurrWindow NE  hLockWindow AND VALID-HANDLE(hCurrWindow) AND hCurrWindow:HWND NE ? THEN DO:
    ...
    PUBLISH "ViewHideAllControls" (hCurrWindow,YES).
  END.
  IF hRepaintWindow NE hCurrWindow AND hRepaintWindow NE hLockWindow AND VALID-HANDLE(hRepaintWindow) AND hRepaintWindow:HWND NE ? THEN DO:
    ...
    PUBLISH "ViewHideAllControls" (hRepaintWindow,YES).
  END.
     
  hProc = SESSION:FIRST-PROCEDURE.
  REPEAT WHILE VALID-HANDLE(hProc):
    hProcWin = hProc:CURRENT-WINDOW.
    IF NOT hProc:FILE-NAME MATCHES "JBox*Timer.w" AND VALID-HANDLE(hProcWin) AND hProcWin:HWND NE ? 
       AND NOT CAN-DO(cRefrList,STRING(hProcWin))
      ...
      cRefrList = cRefrList + (IF cRefrList NE "" THEN "," ELSE "") + STRING(hProcWin).
    END.
...
SetScrollableFrames (hWindow:FIRST-CHILD).
...
FIND FIRST ttResizeTypes WHERE ttResizeTypes.chWindow = STRING(hWindow) NO-ERROR.
IF NOT AVAIL ttResizeTypes THEN DO:
  FIND FIRST ttResizeTypes WHERE ttResizeTypes.chWindow = "" NO-ERROR.
  IF NOT AVAIL ttResizeTypes THEN DO:
    CREATE ttResizeTypes.
    ASSIGN ttResizeTypes.chWindow = ""
           ttResizeTypes.cTypes   = "FRAME,BROWSE,RECTANGLE,EDITOR,SELECTION-LIST,CONTROL-FRAME".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION setUseResizeLibWithJukebox Procedure 
FUNCTION setUseResizeLibWithJukebox RETURNS LOGICAL
  ( INPUT ibUseWithJukebox AS LOG ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
bUseWithJukebox = ibUseWithJukebox.

IF NOT bUseWithJukebox THEN bJBoxUserSetting = ?.
...
IF NOT bDebug AND hWindow:HWND NE ? THEN
...
IF hWindow NE ? THEN DO:
  ...
  IF iDeltaY < 0 AND 
     AVAIL ttStatusFrame AND
     ((ttOrgWinSize.iMinY = 0 AND hWindow:HEIGHT-PIXELS < ttOrgWinSize.iHeightPixels - ttStatusFrame.hStatusFrame:HEIGHT-PIXELS) OR
      (ttOrgWinSize.iMinY NE 0 AND hWindow:HEIGHT-PIXELS < ttOrgWinSize.iMinY - ttStatusFrame.hStatusFrame:HEIGHT-PIXELS)) THEN 
...
/* For an embedded window, view the status frame if window height allows it: */
IF AVAIL ttStatusFrame AND ttStatusFrame.bHiddenOnInit AND iOrgDeltaY NE 0 THEN
  ASSIGN ttStatusFrame.hStatusFrame:HIDDEN = NO
         ttStatusFrame.hStatusFrame:Y = hWindow:HEIGHT-PIXELS - ttStatusFrame.hStatusFrame:HEIGHT-PIXELS
         ttStatusFrame.bHiddenOnInit = NO.
ELSE IF AVAIL ttStatusFrame THEN 
  RUN SetStatusBarWidth (hWindow).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION StaticBrowseToExcel Procedure 
FUNCTION StaticBrowseToExcel RETURNS LOGICAL
...
DEF VAR cExptList AS CHAR   NO-UNDO.
DEF VAR iBuffNr   AS INT    NO-UNDO.
DEF VAR cExptFld  AS CHAR   NO-UNDO.
DEF VAR cExptBuff AS CHAR   NO-UNDO EXTENT 20.
DEF VAR cFldBuff  AS CHAR   NO-UNDO EXTENT 20.
DEF VAR hCalcFlds AS HANDLE NO-UNDO EXTENT 20.
DEF VAR iCalcIx   AS INT    NO-UNDO.
DEF VAR iCount    AS INT    NO-UNDO.
DEF VAR rRowid1   AS ROWID  NO-UNDO.
DEF VAR rRowid2   AS ROWID  NO-UNDO.
DEF VAR rRowid3   AS ROWID  NO-UNDO.
DEF VAR iReposRow AS INT    NO-UNDO.
  
ihBrowse:QUERY:GET-FIRST().
IF NOT ihBrowse:QUERY:GET-BUFFER-HANDLE(1):AVAIL THEN RETURN NO.

iReposRow = ihBrowse:FOCUSED-ROW.
...
DO iy = 1 TO ihBrowse:QUERY:NUM-BUFFERS:
  cBuffers = cBuffers + (IF cBuffers NE "" THEN "," ELSE "") + ihBrowse:QUERY:GET-BUFFER-HANDLE(iy):NAME.
  CASE iy: 
    WHEN 1 THEN
      IF ihBrowse:QUERY:GET-BUFFER-HANDLE(iy):AVAIL THEN rRowid1 = ihBrowse:QUERY:GET-BUFFER-HANDLE(iy):ROWID.
    WHEN 2 THEN
      IF ihBrowse:QUERY:GET-BUFFER-HANDLE(iy):AVAIL THEN rRowid2 = ihBrowse:QUERY:GET-BUFFER-HANDLE(iy):ROWID.
    WHEN 3 THEN
      IF ihBrowse:QUERY:GET-BUFFER-HANDLE(iy):AVAIL THEN rRowid3 = ihBrowse:QUERY:GET-BUFFER-HANDLE(iy):ROWID.
  END.
END.
...
DO ix = 1 TO ihBrowse:NUM-COLUMNS:
  ...
  iy = LOOKUP(hColumn:BUFFER-FIELD:BUFFER-NAME,cBuffers) NO-ERROR.
  IF ERROR-STATUS:ERROR THEN 
    ASSIGN iy = 20
           iCalcIx = iCalcIx + 1
           hCalcFlds[iCalcIx] = hColumn.

  IF NOT CAN-DO(cFldList,hColumn:NAME) THEN DO:
    hTT:ADD-NEW-FIELD(hColumn:NAME,
                      hColumn:DATA-TYPE,0,
                      hColumn:FORMAT,"",
                      hColumn:LABEL).
    ASSIGN cFldList = cFldList + (IF cFldList NE "" THEN "," ELSE "") + hColumn:NAME
           cFldBuff[iy] = cFldBuff[iy] + (IF cFldBuff[iy] NE "" THEN "," ELSE "") + hColumn:NAME + "," + hColumn:NAME.
  END.
  ELSE DO:
    hTT:ADD-NEW-FIELD(hColumn:NAME + "_" + STRING(LOOKUP(hColumn:BUFFER-FIELD:BUFFER-NAME,cBuffers)),
                      hColumn:DATA-TYPE,0,
                      hColumn:FORMAT,"",
                      hColumn:LABEL).
    ASSIGN cExptList = cExptList + (IF cExptList NE "" THEN "," ELSE "") + hColumn:NAME + "_" + STRING(iy)
           cExptBuff[iy] = cExptBuff[iy] + (IF cExptBuff[iy] NE "" THEN "," ELSE "") + hColumn:NAME.
  END. 
END.
DO ix = 1 TO ihBrowse:QUERY:NUM-BUFFERS:
  DO iy = 1 TO ihBrowse:QUERY:GET-BUFFER-HANDLE(ix):NUM-FIELDS:
    IF NOT CAN-DO(cFldBuff[ix],ihBrowse:QUERY:GET-BUFFER-HANDLE(ix):BUFFER-FIELD(iy):NAME) THEN
      cExptBuff[ix] = cExptBuff[ix] + (IF cExptBuff[ix] NE "" THEN "," ELSE "") + ihBrowse:QUERY:GET-BUFFER-HANDLE(ix):BUFFER-FIELD(iy):NAME.
  END.
END.

hTT:TEMP-TABLE-PREPARE(ihBrowse:QUERY:GET-BUFFER-HANDLE(1):NAME).
hBuffer = hTT:DEFAULT-BUFFER-HANDLE.

ix = 0.
APPLY "home" TO ihBrowse.
ihBrowse:SELECT-ROW(1) NO-ERROR.

REPEAT WHILE ihBrowse:QUERY:GET-BUFFER-HANDLE(1):AVAIL:

  hBuffer:BUFFER-CREATE().
  DO iy = 1 TO ihBrowse:QUERY:NUM-BUFFERS:
    IF ihBrowse:QUERY:GET-BUFFER-HANDLE(iy):AVAIL THEN
      hBuffer:BUFFER-COPY(ihBrowse:QUERY:GET-BUFFER-HANDLE(iy),cExptBuff[iy]).
  END.
  DO iy = 1 TO NUM-ENTRIES(cExptList):
    ASSIGN cExptFld = SUBSTR(ENTRY(iy,cExptList),1,R-INDEX(ENTRY(iy,cExptList),"_") - 1)
           iBuffNr  = INT(SUBSTR(ENTRY(iy,cExptList),R-INDEX(ENTRY(iy,cExptList),"_") + 1))
           hBuffer:BUFFER-FIELD(ENTRY(iy,cExptList)):BUFFER-VALUE = ihBrowse:QUERY:GET-BUFFER-HANDLE(iBuffNr):BUFFER-FIELD(cExptFld):BUFFER-VALUE
           NO-ERROR.
  END.
  IF cFldBuff[20] NE "" THEN DO:
    DO iy = 1 TO ihBrowse:NUM-COLUMNS:
      hColumn = ihBrowse:GET-BROWSE-COLUMN(iy).
      IF CAN-DO(cFldBuff[20],hColumn:NAME) THEN
        DYNAMIC-FUNCTION("AssignStringValue",hBuffer:BUFFER-FIELD(hColumn:NAME),hColumn:SCREEN-VALUE).
    END.
  END.

  ix = ix + 1.
  IF ix > 80000 OR ix > iCount THEN LEAVE.
  IF NOT ihBrowse:SELECT-NEXT-ROW() THEN LEAVE.
END.

IF iReposRow NE 0 THEN DO:
  ihBrowse:SET-REPOSITIONED-ROW(iReposRow).
  CASE NUM-ENTRIES(cBuffers):
    WHEN 1 THEN ihBrowse:QUERY:REPOSITION-TO-ROWID(rRowid1) NO-ERROR.
    WHEN 2 THEN ihBrowse:QUERY:REPOSITION-TO-ROWID(rRowid1,rRowid2) NO-ERROR.
    OTHERWISE ihBrowse:QUERY:REPOSITION-TO-ROWID(rRowid1,rRowid2,rRowid3) NO-ERROR.
  END CASE.
  IF ERROR-STATUS:GET-MESSAGE(1) NE "" THEN DO:
    ihBrowse:QUERY:GET-FIRST().
    ihBrowse:SET-REPOSITIONED-ROW(1).
    ihBrowse:QUERY:REPOSITION-TO-ROW(1).
  END.
END.

ToExcelViaFile(hBuffer,0).
...
DEF VAR cExcelViewer    AS CHAR   NO-UNDO.
DEF VAR hExcelViewer    AS HANDLE NO-UNDO.
DEF VAR hHelpWidget     AS HANDLE NO-UNDO.
DEF VAR bReturn         AS LOG    NO-UNDO.
DEF VAR cValue          AS CHAR   NO-UNDO.
DEF VAR cNumSep         AS CHAR NO-UNDO.
DEF VAR cNumDec         AS CHAR NO-UNDO.

cNumSep = SESSION:NUMERIC-SEPARATOR.
cNumDec = SESSION:NUMERIC-DECIMAL.
    ...
    REPEAT WHILE NOT hQuery:QUERY-OFF-END:
      DO iColumn = 1 TO ihObject:NUM-COLUMNS:
        ...
        IF VALID-HANDLE(hColumn) AND hColumn:VISIBLE THEN DO:
          IF hField:DATA-TYPE = "DATE" AND hField:FORMAT = "99/99/99" THEN hField:FORMAT = "99/99/9999". 
          
          IF hField:DATA-TYPE = "decimal" THEN DO:
            cValue = STRING(hField:BUFFER-VALUE).
/*            IF INDEX(cValue,".") > INDEX(cValue,",") THEN cValue = REPLACE(cValue,",","").*/
/*            ELSE cValue = REPLACE(cValue,",",".").                                        */
            cOutput = cOutput + (IF cValue NE ? THEN cValue ELSE "0") + cDelimiter.
          END.
          ELSE IF hField:DATA-TYPE = "integer" THEN DO:
            cValue = REPLACE(STRING(hField:BUFFER-VALUE),",","").
            cOutput = cOutput + (IF cValue NE ? THEN cValue ELSE "0") + cDelimiter.
          END.
    ...
    IF cExcelFields NE "" THEN
      DO iColumn = 1 TO NUM-ENTRIES(cExcelFields):
        ...
        DO ix = 0 TO hField:EXTENT:
          ...
          IF hField:DATA-TYPE = "decimal" THEN DO:
            cValue = STRING(hField:BUFFER-VALUE[ix]).
/*            IF INDEX(cValue,".") > INDEX(cValue,",") THEN cValue = REPLACE(cValue,",","").*/
/*            ELSE cValue = REPLACE(cValue,",",".").                                        */
            cOutput = cOutput + (IF cValue NE ? THEN cValue ELSE "0") + cDelimiter.
          END.
          ELSE IF hField:DATA-TYPE = "integer" THEN DO:
            cValue = REPLACE(STRING(hField:BUFFER-VALUE[ix]),",","").
            cOutput = cOutput + (IF cValue NE ? THEN cValue ELSE "0") + cDelimiter.
          END.
/*          IF hField:DATA-TYPE = "integer" OR hField:DATA-TYPE = "decimal" THEN DO:                      */
/*            fDec = hField:BUFFER-VALUE[ix].                                                             */
/*            cOutput = cOutput + (IF fDec NE ? THEN REPLACE(STRING(fDec),",",".") ELSE "0") + cDelimiter.*/
/*          END.                                                                                          */
          ELSE IF hField:DATA-TYPE = "DATE" AND cDateFormat = "yyyy-mm-dd" THEN 
...
PUBLISH "ExcelFileComplete" (ihObject,cFileName,OUTPUT bReturn).
IF bReturn THEN RETURN FALSE.

cExcelViewer = DYNAMIC-FUNCTION("getAttribute",SESSION,"excelViewer").
IF cExcelViewer NE "" 
   AND (SEARCH(cExcelViewer) NE ? OR (R-INDEX(cExcelViewer,".") > 0 
                                      AND SEARCH(SUBSTR(cExcelViewer,1,R-INDEX(cExcelViewer,".")) + ".r") NE ?)
        ) THEN DO:
          
  PUBLISH "StartTabWindow" (cExcelViewer,
                            "Spreadsheet",
                            THIS-PROCEDURE, /* parent */
                            yes, /* (always) new instance */
                            yes,     /* keep the tab open after close of this window */
                            OUTPUT hExcelViewer).
  IF VALID-HANDLE(hExcelViewer) THEN DO:
    RUN ViewFile IN hExcelViewer (cFileName,cDelimiter).
    hHelpWidget = WIDGET-HANDLE(DYNAMIC-FUNCTION("getAttribute",hExcelViewer:CURRENT-WINDOW,"HelpTextWidget")) NO-ERROR.
    IF VALID-HANDLE(hHelpWidget) THEN
      hHelpWidget:SCREEN-VALUE = IF VALID-HANDLE(ihObject:WINDOW) THEN
                                   REPLACE(REPLACE(ihObject:WINDOW:TITLE,"[",""),"]","")
                                 ELSE IF ihObject:NAME NE ? THEN ihObject:NAME
                                 ELSE DYNAMIC-FUNCTION("getAppTitle").
    RETURN TRUE. 
  END.
END. 
...
IF ihObject:TYPE = "browse" THEN 
  chWorkSheet:NAME  =  IF ihObject:NAME NE ? THEN ihObject:NAME
                       ELSE IF VALID-HANDLE(ihObject:WINDOW) THEN
                         REPLACE(REPLACE(REPLACE(ihObject:WINDOW:TITLE,"[",""),"]","")," ","")
                       ELSE DYNAMIC-FUNCTION("getAppTitle") NO-ERROR.
ELSE IF DYNAMIC-FUNCTION("getAppTitle") NE "" THEN 
  chWorkSheet:NAME  =  DYNAMIC-FUNCTION("getAppTitle") NO-ERROR.
...
FINALLY:
  SESSION:SET-NUMERIC-FORMAT(cNumSep,cNumDec).
END FINALLY.  
