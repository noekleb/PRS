...
/* Custom exclude file */
DEF VAR hTargetProcedure  AS HANDLE NO-UNDO.
...
&IF DEFINED(EXCLUDE-setTargetProcedure) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD setTargetProcedure Procedure 
FUNCTION setTargetProcedure RETURNS LOGICAL
  ( INPUT ihTargetProcedure AS HANDLE )  FORWARD.
...
------------------------------------------------------------------------------*/
  ...
  END.
  ELSE 
     hNodeKey:BUFFER-VALUE = DYNAMIC-FUNCTION("getNextNodeKey" IN hTargetProcedure).
 ...
 END.
  ...
  DO:
    IF hImage:BUFFER-VALUE <> '':U THEN
        iImageIndex = DYNAMIC-FUNCTION("loadImage" IN hTargetProcedure,  hImage:BUFFER-VALUE).

      IF hSelectedImage:BUFFER-VALUE <> '':U THEN
        iSelectedImageIndex = DYNAMIC-FUNCTION("loadImage" IN hTargetProcedure, hSelectedImage:BUFFER-VALUE).
...
------------------------------------------------------------------------------*/
 ...
 RELEASE OBJECT chtreeView NO-ERROR.
  ...
  {get TreeDataTable hTreeDataTable hTargetProcedure}.
...
------------------------------------------------------------------------------*/
  ...
  DO WHILE hLocalBuffer:AVAILABLE:
    RUN addNode IN hTargetProcedure (hLocalBuffer).
    RUN populateTree IN hTargetProcedure (phTable, hParentKey:BUFFER-VALUE).
    hQry:GET-NEXT().
  END.
...
------------------------------------------------------------------------------*/
  ...
  {get ContainerSource hContainer hTargetProcedure} NO-ERROR.  
...
/*------------------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------------*/
  ...
  ASSIGN ghProp = WIDGET-HANDLE(ENTRY(1, hTargetProcedure:ADM-DATA, CHR(1)))
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION setTargetProcedure Procedure 
FUNCTION setTargetProcedure RETURNS LOGICAL
  ( INPUT ihTargetProcedure AS HANDLE ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
hTargetProcedure = ihTargetProcedure.
RETURN TRUE.
