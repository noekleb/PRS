...
oDynDataBrowse:EnableSaveQuery = YES.
oDynDataBrowse:TotalFields = "OrderTot".
oDynDataBrowse:BROWSE-OBJECT:userSettingContext = THIS-PROCEDURE:FILE-NAME.
oDynDataBrowse:BROWSE-OBJECT:calcFieldProc = "customer_browsecalc.p".
...
oDynDataBrowse:InitializeQuery(
    "Customer"
      ...
      + ";Custnum"
      + ";Name"
      + ";+OrderTot|DECIMAL|->><>>><>>9.99|CustOrderTotal(CustNum)|Order total"
    + ",SalesRep"
      + ";RepName@2"
    ,"WHERE false"
      + ",FIRST SalesRep NO-LOCK OF Customer"
     ,"Salesrep"  /* Initial sort column */
...
oDynDataBrowse:BROWSE-OBJECT:setExtraFilterField("Item","ItemName","Item bought",
                                                 "EACH OrderLine OF Item NO-LOCK,FIRST Order NO-LOCK OF OrderLine,FIRST Customer NO-LOCK OF Order").

/* Assist in describing the prescan query route for Item since it is not directly joined from Order: */
/*oDynDataBrowse:BROWSE-OBJECT:setPreScanQuery("Item","EACH OrderLine OF Item NO-LOCK,FIRST Order NO-LOCK OF OrderLine,FIRST Customer NO-LOCK OF Order").*/
