...
FUNCTION getBuffersAndFieldsBrwCustomer RETURNS CHARACTER():
  ...
  + ',Order'
     ...
     + ';OrderDate'
     + ';OrderStatus'
     + ';+OrderTot|DECIMAL||jb_total(EACH OrderLine OF Order ¤Price * Qty)|Order total'
  + ',buf1_Salesrep'
     + ';RepName'
  + ',Salesrep'
     + ';RepName'
...
FUNCTION getQueryJoinBrwCustomer RETURNS CHARACTER():
  RETURN 'EACH Order OF Customer NO-LOCK,EACH buf1_Salesrep  WHERE buf1_Salesrep.SalesRep = Order.SalesRep NO-LOCK,EACH Salesrep OF Customer NO-LOCK'.
END FUNCTION.
...
Customer.RepName4 Customer.Ordernum Customer.OrderDate Customer.OrderStatus ~
Customer.RepName Customer.OrderTot 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BrwCustomer C-Win _STRUCTURED
  QUERY BrwCustomer NO-LOCK DISPLAY
      ...
      Customer.OrderTot COLUMN-LABEL "Order total" FORMAT "->>,>>>,>>9.99":U
  ENABLE
...
DEFINE FRAME DEFAULT-FRAME
     ...
     TabCustomer AT ROW 13.86 COL 5 WIDGET-ID 2
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SCROLLABLE SIZE 139.6 BY 25.33 WIDGET-ID 100.
...
ASSIGN 
       FRAME DEFAULT-FRAME:HEIGHT           = 25.33
       FRAME DEFAULT-FRAME:WIDTH            = 139.6.
...
"Customer.OrderTot" "Order total" "->>,>>>,>>9.99" "DECIMAL" ? ? ? ? ? ? no "" no no "14.4" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  oTabCustomer = NEW JBoxMsTabs(TabCustomer:HANDLE,oBrwCustomer,"ico\acctres.ico").
  oTabCustomer:pageOneType = "child".

  oTabCustomer:setLinkFields("OrderNum").
  oTabCustomer:AddPage("Items","oo/OrdLine.w","acctres.ico").
END.
oBrwCustomer:OpenQuery().

/*oBrwCustomer:setNoResizeY().*/
oContainer:initResize(200,50).
