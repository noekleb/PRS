...
FUNCTION getBuffersAndFieldsBrwCustomer RETURNS CHARACTER():
  ...
  + ',Order'
     ...
     + ';Ordernum'
     + ';OrderDate'
     + ';+OrderCnt|INTEGER||jb_total(EACH OrderLine WHERE Orderline.OrderNum = Order.OrderNum¤COUNT¤)'
     .
END FUNCTION.
...
&Scoped-define FIELDS-IN-QUERY-BrwCustomer Customer.CreditLimit ~
Customer.CustNum Customer.Discount Customer.Name Customer.OrderCnt ~
Customer.OrderStatus Customer.Ordernum Customer.OrderDate 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BrwCustomer Customer.CreditLimit 
...
CreditLimit OrderDate CustNum Discount Name OrderStatus Ordernum 
&Scoped-Define DISPLAYED-OBJECTS CreditLimit OrderDate CustNum Discount ~
Name OrderStatus Ordernum 
...
DEFINE VARIABLE Discount AS INTEGER FORMAT ">>9%" INITIAL 0 
     LABEL "Discount" 
     VIEW-AS FILL-IN 
     SIZE 7.6 BY 1 TOOLTIP "Please enter a percentage from 0 to 100.".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BrwCustomer C-Win _STRUCTURED
  QUERY BrwCustomer NO-LOCK DISPLAY
      ...
      Customer.OrderCnt COLUMN-LABEL "Ordercount" FORMAT "->,>>>,>>9":U
  ...
  ENABLE
      Customer.CreditLimit HELP "Please enter a Credit Limit."
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 125 BY 8.57 FIT-LAST-COLUMN.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME DEFAULT-FRAME
     new_tbCustomer AT ROW 1.33 COL 2.2 WIDGET-ID 18
     edit_tbCustomer AT ROW 1.33 COL 6.8 WIDGET-ID 20
     copy_tbCustomer AT ROW 1.33 COL 11.4 WIDGET-ID 22
     undo_tbCustomer AT ROW 1.33 COL 16 WIDGET-ID 24
     save_tbCustomer AT ROW 1.33 COL 20.6 WIDGET-ID 26
     delete_tbCustomer AT ROW 1.33 COL 25.2 WIDGET-ID 28
     filter_tbCustomer AT ROW 1.33 COL 29.8 WIDGET-ID 30
     browseconfig_tbCustomer AT ROW 1.33 COL 34.4 WIDGET-ID 32
     excel_tbCustomer AT ROW 1.33 COL 39 WIDGET-ID 34
     BrwCustomer AT ROW 3.14 COL 2 WIDGET-ID 200
     CreditLimit AT ROW 13 COL 24 COLON-ALIGNED HELP
          "Please enter a Credit Limit."
     OrderDate AT ROW 13 COL 76 COLON-ALIGNED HELP
          "Please enter the date of order."
     CustNum AT ROW 14 COL 24 COLON-ALIGNED HELP
          "Please enter a customer number."
     Discount AT ROW 15 COL 24 COLON-ALIGNED HELP
          "Please enter a percentage from 0 to 100."
     Name AT ROW 16 COL 24 COLON-ALIGNED HELP
          "Please enter a name."
     OrderStatus AT ROW 17 COL 24 COLON-ALIGNED HELP
          "Please enter the Order Status."
     Ordernum AT ROW 18 COL 24 COLON-ALIGNED HELP
          "Please enter an order number."
     tbCustomer AT ROW 1.24 COL 2 WIDGET-ID 16
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SCROLLABLE SIZE 127.8 BY 19.43 WIDGET-ID 100.
...
ASSIGN 
       FRAME DEFAULT-FRAME:HEIGHT           = 19.43
...
"Customer.CreditLimit" "Credit Limit" "->,>>>,>>9" "decimal" ? ? ? ? ? ? yes "Please enter a Credit Limit." no no "10.2" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"Customer.OrderCnt" "Ordercount" "->,>>>,>>9" "INTEGER" ? ? ? ? ? ? no "" no no "10.6" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"Customer.OrderDate" "Ordered" "99/99/99" "date" ? ? ? ? ? ? no "Please enter the date of order." no no "16.2" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  DISPLAY CreditLimit OrderDate CustNum Discount Name OrderStatus Ordernum 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE tbCustomer new_tbCustomer edit_tbCustomer copy_tbCustomer 
         undo_tbCustomer save_tbCustomer delete_tbCustomer filter_tbCustomer 
         browseconfig_tbCustomer excel_tbCustomer BrwCustomer CreditLimit 
         OrderDate CustNum Discount Name OrderStatus Ordernum 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  oFmCustomer:updateFields = 'CreditLimit,CustNum,Discount,Name'.
  oFmCustomer:displayFields = 'OrderStatus,Ordernum,OrderDate'.
