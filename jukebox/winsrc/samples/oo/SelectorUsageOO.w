...
DO:
  JBoxServerAPI:Instance:Selector(
                      "Department"      
                      + ";DeptCode"
                      + ";DeptName"
                      ,"where true").
...
------------------------------------------------------------------------------*/
  ENABLE btnSelectSalesreps btnSelectDepartments 
...
IF VALID-HANDLE(JBoxServerAPI:Instance:SELECTOR-PANEL-HANDLE) THEN
  MESSAGE PROGRAM-NAME(1) SKIP
          "Liquidation date: " DYNAMIC-FUNCTION("getLiquidationDate" IN JBoxServerAPI:Instance:SELECTOR-PANEL-HANDLE)
          VIEW-AS ALERT-BOX.
END PROCEDURE.
...
IF ioSelector:NAME = "Department" THEN
  ioSelector:LoadPanel("DepartmentSelectPanel.w").
