...
DEFINE VARIABLE oRTFeditor  AS uc.JBoxRadRtfEdit NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    oRTFeditor = NEW uc.JBoxRadRtfEdit(THIS-PROCEDURE, rectTabFolder:HANDLE).
    oRTFeditor:RegisterWithJukeBox(YES).
/*    oRTFeditor:ReportFileName = 'reports/test2.trdx'.*/
/*    oRTFeditor:DataFileName   = 'reports/test2.crv'. */
          
                                                                                                                    
/*    oToolbar = NEW JBoxToolbar(tbToolbar:HANDLE,"File").*/
/*    oToolbar:designWidget = tbToolbar:handle.           */
/*    oToolbar:AddTool("Print" ,"Print",true).            */
/*    oRTFeditor:loadDocument().*/
 END.
...
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
  IF VALID-OBJECT(oRTFeditor) THEN 
      DELETE OBJECT oRTFeditor NO-ERROR. 
