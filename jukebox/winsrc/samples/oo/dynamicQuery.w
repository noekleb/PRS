...
DEF VAR oQuery      AS JBoxQuery NO-UNDO.
...
&Scoped-Define ENABLED-OBJECTS tbCustomer filter_tbCustomer ~
first_tbCustomer next_tbCustomer prev_tbCustomer last_tbCustomer CustNum ~
Name RepName Ordernum RepName4 
&Scoped-Define DISPLAYED-OBJECTS CustNum Name RepName Ordernum RepName4 
...
DEFINE VARIABLE RepName4 AS CHARACTER FORMAT "X(256)":U 
     LABEL "Rep for order" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME DEFAULT-FRAME
     filter_tbCustomer AT ROW 1.33 COL 4.2 WIDGET-ID 12
     first_tbCustomer AT ROW 1.33 COL 10.2 WIDGET-ID 14
     next_tbCustomer AT ROW 1.33 COL 16.4 WIDGET-ID 16
     prev_tbCustomer AT ROW 1.33 COL 22.4 WIDGET-ID 18
     last_tbCustomer AT ROW 1.33 COL 28.6 WIDGET-ID 20
     CustNum AT ROW 4 COL 13 COLON-ALIGNED
     Name AT ROW 5 COL 13 COLON-ALIGNED
     RepName AT ROW 6.05 COL 13 COLON-ALIGNED
     Ordernum AT ROW 7.91 COL 13 COLON-ALIGNED
     RepName4 AT ROW 9.1 COL 13 COLON-ALIGNED WIDGET-ID 22
     tbCustomer AT ROW 1.24 COL 4 WIDGET-ID 10
...
ASSIGN 
       tbCustomer:PRIVATE-DATA IN FRAME DEFAULT-FRAME     = 
                "filter;Filter,first;First,next;Next,prev;Prev,last;Lastmaxborder".
...
/* Query rebuild information for QUERY QUERY-2
     _Design-Parent    is FRAME DEFAULT-FRAME @ ( 3.14 , 53 )
*/  /* QUERY QUERY-2 */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
  DISPLAY CustNum Name RepName Ordernum RepName4 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE tbCustomer filter_tbCustomer first_tbCustomer next_tbCustomer 
         prev_tbCustomer last_tbCustomer CustNum Name RepName Ordernum RepName4 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  oQuery:queryJoin = ',EACH Order OF Customer NO-LOCK,EACH Salesrep OF Customer NO-LOCK,EACH buf1_Salesrep  WHERE buf1_Salesrep.SalesRep = Order.SalesRep NO-LOCK'.
  
  oQuery:altPrimaryBufferList = "Order,SalesRep,buf1_Salesrep".
  oQuery:setPreScanQuery("buf1_Salesrep","EACH Order WHERE Order.SalesREp = buf1_Salesrep.salesrep,FIRST Customer OF Order").
  
  oFmCustomer = NEW JBoxFieldMap(FRAME {&FRAME-NAME}:HANDLE).
  oFmCustomer:updateFields = 'CustNum,Name'.
  oFmCustomer:displayFields = 'Ordernum,RepName,RepName4'.
  oFmCustomer:primaryKeyFields = 'CustNum'.

  oFmCustomer:QUERY-OBJECT = oQuery.
  otbCustomer = NEW JBoxToolbar(tbCustomer:HANDLE).

  oQuery:TOOLBAR-OBJECT = otbCustomer.
