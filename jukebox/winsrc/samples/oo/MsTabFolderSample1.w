...
&Scoped-Define ENABLED-OBJECTS btnSplitBarY TabCustomer tbCustomer ~
new_tbCustomer edit_tbCustomer copy_tbCustomer undo_tbCustomer ~
delete_tbCustomer save_tbCustomer excel_tbCustomer BrwCustomer CustNum ~
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 142 BY 5.71 FIT-LAST-COLUMN.
...
DEFINE FRAME DEFAULT-FRAME
     ...
     new_tbCustomer AT ROW 1.29 COL 2.2 WIDGET-ID 10
     edit_tbCustomer AT ROW 1.29 COL 6.8 WIDGET-ID 12
     copy_tbCustomer AT ROW 1.29 COL 11.6 WIDGET-ID 14
     undo_tbCustomer AT ROW 1.29 COL 16.2 WIDGET-ID 16
     delete_tbCustomer AT ROW 1.29 COL 21 WIDGET-ID 18
     save_tbCustomer AT ROW 1.29 COL 25.6 WIDGET-ID 20
     excel_tbCustomer AT ROW 1.29 COL 30.4 WIDGET-ID 22
     ...
     tbCustomer AT ROW 1.19 COL 2 WIDGET-ID 8
...
ASSIGN 
       tbCustomer:PRIVATE-DATA IN FRAME DEFAULT-FRAME     = 
                "new;Ny,edit;Edit,copy;Kopier,undo;Angre,delete;Slett,save;Lagre,excel;Eksporter til E&xcelmaxborder".
...
"Customer.Address" "Address" "x(35)" "character" ? ? ? ? ? ? no "Please enter an address." no no "35" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > Customer.Comments
...
"Customer.Ordernum" "Order Num" "zzzzzzzzz9" "integer" ? ? ? ? ? ? no "Please enter an order number." no no "10.2" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > Customer.OrderStatus
...
"Customer.SalesRep" "Sales Rep" "x(4)" "character" ? ? ? ? ? ? no "Please enter the Sales Rep." no no "9.8" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > Customer.RepName
...
------------------------------------------------------------------------------*/
  ...
  ENABLE btnSplitBarY TabCustomer tbCustomer new_tbCustomer edit_tbCustomer 
         copy_tbCustomer undo_tbCustomer delete_tbCustomer save_tbCustomer 
         excel_tbCustomer BrwCustomer CustNum Ordernum OrderDate Name 
         OrderStatus Address SalesRep Comments RepName 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
/*  oBrwCustomer:enableOnDblClick = yes.       */
  ...
  oFmCustomer:updateFields = 'CustNum,Name,Address,Comments,OrderDate'.
  oFmCustomer:displayFields = 'Ordernum,OrderStatus,SalesRep,RepName'.
  ...
  oTabCustomer = NEW JBoxMsTabs(TabCustomer:HANDLE,oBrwCustomer,"bmp\bullet_ball_green.bmp;bmp\bullet_ball_red.bmp;gif\afinternet.gif").

  oTabCustomer:setLinkFields("CustNum").
  oTabCustomer:pageOneType = "child".
  
 
  oTabCustomer:AddPage("Orders","oo\MsTab2.w","bullet_ball_green.bmp").
 
  oTabCustomer:AddPage("Customer","oo\oCustView.w","bullet_ball_red.bmp").
  
/*
  oTabCustomer:AddPage("Customer3","Customer3.w","afinternet.gif").
  oTabCustomer:AddPage("Orders","oo/C-win6.w","afinternet.gif").
/*  MESSAGE 2                                                                  */
/*  VIEW-AS ALERT-BOX.                                                         */
  oTabCustomer:AddPage("Orderview","OrderViewDotNet.w","afinternet.gif",yes).
 */
...
oContainer:setSplitBarYlimits(btnSplitBarY:HANDLE,200,300).
