...
DEF TEMP-TABLE Customer
    ...
    FIELD CustOrderTotal AS CHARACTER
    FIELD FirstItem AS CHARACTER
...
FUNCTION getBuffersAndFieldsBrwCustomer RETURNS CHARACTER():
  RETURN
    'Customer'
     + ';CustNum'
     + ';Name'
     + ';+CustOrderTotal|CHARACTER||CustOrderTotal(CustNum)|CustOrderTotal'
  + ',Order'
     + ';Ordernum'
     + ';OrderStatus'
     + ';OrderDate'
     + ';+FirstItem|CHARACTER||FirstItem(ROWID)|FirstItem'
  + ',Salesrep'
     + ';RepName'
     + ';SalesRep'
  + ',buf1_Salesrep'
...
FUNCTION getQueryJoinBrwCustomer RETURNS CHARACTER():
  RETURN 'EACH Order OF Customer NO-LOCK,EACH Salesrep OF Customer NO-LOCK,EACH buf1_Salesrep  WHERE buf1_Salesrep.SalesRep = Order.SalesRep NO-LOCK'.
END FUNCTION.
FUNCTION getCalcFieldProcBrwCustomer RETURNS CHARACTER():
  RETURN 
     'customer_browsecalc.p' /* CustOrderTotal(CustNum) */
   + ',customer_browsecalc.p' /* FirstItem(ROWID) */
     .
END FUNCTION.
...
Customer.Ordernum Customer.OrderStatus Customer.OrderDate Customer.RepName ~
Customer.SalesRep Customer.RepName4 Customer.CustOrderTotal ~
Customer.FirstItem 
...
&Scoped-define QUERY-STRING-BrwCustomer FOR EACH Customer NO-LOCK, ~
    ...
    EACH buf1_Salesrep WHERE buf1_Salesrep.SalesRep = Order.SalesRep NO-LOCK INDEXED-REPOSITION
...
&Scoped-Define ENABLED-OBJECTS tbCustomer filter_tbCustomer BrwCustomer 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BrwCustomer C-Win _STRUCTURED
  QUERY BrwCustomer NO-LOCK DISPLAY
      ...
      Customer.CustOrderTotal COLUMN-LABEL "CustOrderTotal" FORMAT "X(8)":U
      Customer.FirstItem COLUMN-LABEL "FirstItem" FORMAT "X(8)":U
  ENABLE
      Customer.CustNum HELP "Please enter a customer number."
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 60 BY 8.57 FIT-LAST-COLUMN.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME DEFAULT-FRAME
     filter_tbCustomer AT ROW 1.33 COL 3.2 WIDGET-ID 4
...
"Customer.CustOrderTotal" "CustOrderTotal" "X(8)" "CHARACTER" ? ? ? ? ? ? no "" no no "14.2" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > "_<CALC>"
"Customer.FirstItem" "FirstItem" "X(8)" "CHARACTER" ? ? ? ? ? ? no "" no no "8" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ENABLE tbCustomer filter_tbCustomer BrwCustomer 
...
//  oBrwCustomer:getRecordCount = yes.
  oBrwCustomer:altPrimaryBufferList = "Order,Customer,Salesrep,buf1_Salesrep".
  oBrwCustomer:setPreScanQuery("Order", "EACH Customer OF Order,").
  oBrwCustomer:setPreScanQuery("SalesRep", "EACH Customer OF SalesRep,").
  oBrwCustomer:setPreScanQuery("buf1_SalesRep", "EACH Order OF buf1_SalesRep,").
  oBrwCustomer:setJBoxAttribute("viewqueryfilter","yes").
END.
