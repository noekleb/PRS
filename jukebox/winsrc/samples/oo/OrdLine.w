...
OrderLine.Itemnum OrderLine.OrderLineStatus OrderLine.Qty ~
OrderLine.ItemName OrderLine.Ordernum OrderLine.Allocated 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BrwOrderLine C-Win _STRUCTURED
  QUERY BrwOrderLine NO-LOCK DISPLAY
      ...
      OrderLine.Allocated FORMAT "->>>>9":U
  ENABLE
      OrderLine.Linenum
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 60 BY 9.29 FIT-LAST-COLUMN.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME DEFAULT-FRAME
     BrwOrderLine AT ROW 2.91 COL 3 WIDGET-ID 200
...
ASSIGN 
       ...
       OrderLine.Allocated:VISIBLE IN BROWSE BrwOrderLine = FALSE.
...
"OrderLine.Itemnum" "Item Num" "zzzzzzzzz9" "integer" ? ? ? ? ? ? no "Please enter an item number." no no "10.2" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > OrderLine.OrderLineStatus
"OrderLine.OrderLineStatus" "Order Line Status" "x(20)" "character" ? ? ? ? ? ? no "Please enter the order line status." no no "20" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > OrderLine.Qty
"OrderLine.Qty" "Qty" "->>>>9" "integer" ? ? ? ? ? ? no "Please enter the quantity." no no "6.6" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > OrderLine.ItemName
"OrderLine.ItemName" "Item Name" "x(25)" "character" ? ? ? ? ? ? no "Please enter an item name." no no "25" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > OrderLine.Ordernum
"OrderLine.Ordernum" "Order Num" "zzzzzzzzz9" "integer" ? ? ? ? ? ? no "Please enter and Order Number for this order line." no no "10.2" no no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[7]   > OrderLine.Allocated
"OrderLine.Allocated" "Allocated" "->>>>9" "integer" ? ? ? ? ? ? no "Please enter allocated" no no "8.8" no no no "U" "" "" "" "" "" "" 0 no 0 no no
