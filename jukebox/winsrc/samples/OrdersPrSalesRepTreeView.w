...
------------------------------------------------------------------------------*/
  ...
  ENABLE btnFromDate OrderFolder rectTreeView cmbRegion btnToDate dFrom dTo 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
hQuery:QUERY-PREPARE("FOR EACH " + hTempTable:DEFAULT-BUFFER-HANDLE:NAME + " BY iNodeIndex").
hQuery:QUERY-OPEN().
hQuery:GET-FIRST().
REPEAT WHILE NOT hQuery:QUERY-OFF-END:
  IF cExpandNode = "" THEN
    cExpandNode = STRING(hQuery:GET-BUFFER-HANDLE(1):BUFFER-FIELD("iNodeIndex"):BUFFER-VALUE).

  RUN CreateNodes(
             hBuffer:BUFFER-FIELD("iLevel"):BUFFER-VALUE,
             STRING(hBuffer:BUFFER-FIELD("iParentIdx"):BUFFER-VALUE),
             hBuffer:BUFFER-FIELD("cNodeLabel"):BUFFER-VALUE,
             TRUE,
             FALSE,
             STRING(hBuffer:BUFFER-FIELD("iNodeIndex"):BUFFER-VALUE)
             ).
  hQuery:GET-NEXT().
END.
...
IF bOk THEN
  cExpandNode = STRING(hBuffer:BUFFER-FIELD('iNodeIndex'):BUFFER-VALUE).
...
IF CAN-DO("CLICK,MYCLICK",icEvent) THEN DO:
  ...
  IF bOk THEN DO:
    bOK = hBuffer:FIND-FIRST("WHERE iNodeIndex = " + hTreeTableBuffer:BUFFER-FIELD('private_data'):BUFFER-VALUE) NO-ERROR.
    IF bOk THEN DO:
