...
DEF VAR hRtfEdit          AS HANDLE NO-UNDO.
DEF VAR hRtfEditFrame     AS HANDLE NO-UNDO.
...
&Scoped-Define ENABLED-OBJECTS rectToolbar rectEdit CustNum Name SalesRep ~
Address CreditLimit Address2 Discount PostalCode Terms City Balance State ~
btnState StateName Country Contact EmailAddress Phone Fax 
...
Contact EmailAddress Phone Fax fiCommentLabel 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getEditorFrameHandle C-Win 
FUNCTION getEditorFrameHandle RETURNS HANDLE
...
DEFINE FRAME frmCustView
     ...
     Name AT ROW 3.76 COL 16 COLON-ALIGNED HELP
          "Please enter a name."
     SalesRep AT ROW 3.76 COL 69.2 COLON-ALIGNED
     ...
     Contact AT ROW 11.81 COL 16 COLON-ALIGNED HELP
          "Please enter a contact."
     EmailAddress AT ROW 12.86 COL 16 COLON-ALIGNED HELP
          "Please enter an full Internet Email Address."
     Phone AT ROW 13.91 COL 16 COLON-ALIGNED HELP
          "Please enter a phone number"
     Fax AT ROW 13.91 COL 46 COLON-ALIGNED HELP
          "Please enter a fax number."
     fiCommentLabel AT ROW 15.29 COL 5.6 COLON-ALIGNED NO-LABEL
     rectToolbar AT ROW 1.24 COL 1.8
     rectEdit AT ROW 15.29 COL 17.8
...
&Scoped-define SELF-NAME Fax
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Fax C-Win
ON TAB OF Fax IN FRAME frmCustView /* Fax */
DO:
  RUN SetRtfFocus IN hRtfEdit.
  RETURN NO-APPLY.
END.
...
ON CLOSE OF THIS-PROCEDURE DO:
  IF VALID-HANDLE(hRtfEdit) THEN APPLY "close" TO hRtfEdit.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEF VAR cCustNum AS CHAR NO-UNDO.

IF hFieldMap:AVAILABLE THEN cCustNum = STRING(hFieldMap:BUFFER-FIELD("CustNum"):BUFFER-VALUE).

RUN SUPER.

DYNAMIC-FUNCTION("DeleteRtfTextDoc" IN hRtfEdit,"Customer",cCustNum).
...
DYNAMIC-FUNCTION("ViewRtfTextDoc" IN hRtfEdit,"Customer",
                 IF hFieldMap:AVAIL THEN STRING(hFieldMap:BUFFER-FIELD("CustNum"):BUFFER-VALUE) ELSE "",
                 IF hFieldMap:AVAIL THEN hFieldMap:BUFFER-FIELD("Comments"):BUFFER-VALUE ELSE "").
...
------------------------------------------------------------------------------*/
  ...
  ENABLE rectToolbar rectEdit CustNum Name SalesRep Address CreditLimit 
         Address2 Discount PostalCode Terms City Balance State btnState 
         StateName Country Contact EmailAddress Phone Fax 
      WITH FRAME frmCustView.
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  hFieldMap = DYNAMIC-FUNCTION("NewFieldMap",
                             ...
                             "Name,Address,Address2,City,Contact,Country,CreditLimit,Discount,EmailAddress,Fax,Phone,PostalCode,SalesRep,State,Terms","",
  ...
  DYNAMIC-FUNCTION("setAttribute",hFieldMap,"bufferExtraFields","Comments").

  hToolbar = DYNAMIC-FUNCTION("NewToolbar",
                            rectToolbar:HANDLE,
                            "",
                            "New,Undo,Save,Delete,print,rule,first,prev,next,last",
                            "maxborder").

  DYNAMIC-FUNCTION("CreateObjectLink",hQuery,hToolbar).
  DYNAMIC-FUNCTION("CreateObjectLink",hFieldMap,hToolbar).

  RUN JBoxJlwRtfEdit.w PERSIST SET hRtfEdit.
  RUN InitializeObject IN hRtfEdit (rectEdit:HANDLE).
  DYNAMIC-FUNCTION("setToolbarHandle" IN hRtfEdit,hToolbar).
  hRtfEditFrame = DYNAMIC-FUNCTION("getFrameHandle" IN hRtfEdit).
  DYNAMIC-FUNCTION("setTabFromWidget" IN hRtfEdit,Fax:HANDLE).
  DYNAMIC-FUNCTION("setHelpText" IN hRtfEdit,hFieldMap:BUFFER-FIELD("comments"):HELP).

  DYNAMIC-FUNCTION("setLassoExceptionList",STRING(hRtfEditFrame)).
  DYNAMIC-FUNCTION("setNoMoveY", THIS-PROCEDURE:CURRENT-WINDOW,hRtfEditFrame,hRtfEditframe:NAME).

  DYNAMIC-FUNCTION("setAnchor",THIS-PROCEDURE:CURRENT-WINDOW,rectToolbar:HANDLE,
...
------------------------------------------------------------------------------*/
RUN SUPER.
DYNAMIC-FUNCTION("ClearRtfEditor" IN hRtfEdit,NO).
END PROCEDURE.
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       Store content of editor temporary and commit the change if record save was successful
------------------------------------------------------------------------------*/
DEF VAR cPlainTextComment AS CHAR NO-UNDO.

cPlainTextComment = DYNAMIC-FUNCTION("PrepareSaveRtfDoc" IN hRtfEdit).
...
DYNAMIC-FUNCTION("CommitSaveRtfDoc" IN hRtfEdit,"Customer",
                 STRING(hFieldMap:BUFFER-FIELD("Custnum"):BUFFER-VALUE),
                 "",NO).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION getEditorFrameHandle C-Win 
FUNCTION getEditorFrameHandle RETURNS HANDLE
...
RETURN DYNAMIC-FUNCTION("getFrameHandle" IN hRtfEdit).
