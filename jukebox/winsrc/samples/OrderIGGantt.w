...
&IF DEFINED(AdvGuiWin) &THEN
  DEF VAR oIGGantt  AS JBoxIGGantt NO-UNDO.
&ENDIF
...
&Scoped-Define ENABLED-OBJECTS Order Toolbar IGGantt 
...
DEFINE FRAME DEFAULT-FRAME
     ...
     IGGantt AT ROW 10.67 COL 2 WIDGET-ID 8
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
/*oIGGantt:IGGanttView1:Tasks:ClearAndDispose().*/
...
&IF DEFINED(PureABLWin) = 0 &THEN
  oIGGantt:DisableTextChanged(YES).
  IF hFieldMap:AVAIL THEN DO:
    oIGGantt:textBox1:Text = hFieldMap:BUFFER-FIELD("Comments"):BUFFER-VALUE.
    /* Trick since DisplayRecord is executed twice because of the overlay field(s) */
    IF oIGGantt:textBox1:Text = "" THEN
      oIGGantt:DisableTextChanged(NO).
  /*   IF hFieldMap:BUFFER-FIELD("Balance"):BUFFER-VALUE > 100 THEN                              */
  /*     oIGGantt:BackgroundImage = CAST(resGlobal:GetObject(icBgImage), System.Drawing.Image).  */
  END.
  ELSE DO:
    oIGGantt:textBox1:Text = "".
  END.
&ENDIF
*/
RUN SUPER.
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  &IF DEFINED(AdvGuiWin) &THEN
    oIGGantt = NEW JBoxIGGantt(THIS-PROCEDURE,IGGantt:HANDLE).
    oIGGantt:iDeltaWidth = -2.
    oIGGantt:iDeltaHeight = -20.
    oIGGantt:RegisterWithJukeBox(YES).
    oIGGantt:CreateDisplayLink(hBrowse,"Ordernum,Name,OrderDate,ShipDate").
    oIGGantt:setColumns("Customer|Name;Duration|Duration;Order date|Start;Ship date|Finish").
/*    oIGGantt:IGGanttView1:Columns:Item[1]:Width = 88.*/
  &ENDIF
