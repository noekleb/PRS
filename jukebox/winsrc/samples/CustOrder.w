...
&Scoped-Define ENABLED-OBJECTS CustomerFolder CustomerNavBrowse ~
CustomerNavToolbar searchField 
...
DEFINE RECTANGLE CustomerNavBrowse
     EDGE-PIXELS 2 GRAPHIC-EDGE  NO-FILL   
     SIZE 38.6 BY 22.71.

DEFINE RECTANGLE CustomerNavToolbar
     EDGE-PIXELS 2 GRAPHIC-EDGE  NO-FILL   
     SIZE 15.4 BY .95.
...
DEFINE FRAME CustomerFrame
     CustomerFolder AT ROW 2.67 COL 42.6
     CustomerNavBrowse AT ROW 2.57 COL 1.2
     CustomerNavToolbar AT ROW 1.24 COL 22
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Customer and order maintenance"
         HEIGHT             = 24.38
         WIDTH              = 158.8
         MAX-HEIGHT         = 57.14
         MAX-WIDTH          = 320
...
/* REPARENT FRAME */
ASSIGN FRAME frameGradient:FRAME = FRAME CustomerFrame:HANDLE
       FRAME frSplitBarX:FRAME = FRAME CustomerFrame:HANDLE.
...
/* SETTINGS FOR FRAME frameGradient
                                                                        */
...
ON END-ERROR OF C-Win /* Customer and order maintenance */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Customer and order maintenance */
DO:
...
------------------------------------------------------------------------------*/
  ENABLE CustomerFolder CustomerNavBrowse CustomerNavToolbar searchField 
      WITH FRAME CustomerFrame IN WINDOW C-Win.
  {&OPEN-BROWSERS-IN-QUERY-CustomerFrame}
  VIEW FRAME frameGradient IN WINDOW C-Win.
  {&OPEN-BROWSERS-IN-QUERY-frameGradient}
  ENABLE btnSplitBarX 
...
/* If we want all records to the client to enable filtering on every column: */
...
/*------------------------------------------------------------------------------
  Purpose:     Good place to set attributes for the FlatView browse. 
  Parameters:  <none>
  Notes:       An even better place is to create an ExtraFlatViewRecord procedure
------------------------------------------------------------------------------*/
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  DYNAMIC-FUNCTION("setSortString",hBrowse,"Name").
...
/*                   + ",addTab,deleteTab" */
  ...
  DYNAMIC-FUNCTION("InitPages" IN hTabFolder,"Customer|"
/*                  + (IF SEARCH("controls.dll") NE ? THEN "CustViewWithRtfEdit.w" ELSE "CustView.w") */
                 + "CustView.w"
                 + "|Orders|OrderBrw.w"
                   ,hBrowse).
...
/* All screen updates can be but on hold: */
...
/* DYNAMIC-FUNCTION("InitDynFilter",hBrowse,"salesrep,orderdate","=,>","dos|01/06/08","").  */
...
IF SEARCH("controls.dll") NE ? THEN DO:
  RUN GradientFrameCreate(FRAME frameGradient:HWND,"",{&GRAD_FRAME-GRADIENT},15953229,6479746,
                           {&GRAD_FRAME_GRADIENT-BACKWARDDIAGONAL},OUTPUT iStatus).  
  
  
  DYNAMIC-FUNCTION("setNoResizeY", THIS-PROCEDURE:CURRENT-WINDOW, FRAME frameGradient:HANDLE,"frameGradient").
END.
  
FRAME frameGradient:MOVE-TO-BOTTOM().

DO WITH FRAME {&FRAME-NAME}:
  DYNAMIC-FUNCTION("setSplitBarX",THIS-PROCEDURE:CURRENT-WINDOW,btnSplitBarX:HANDLE IN FRAME frSplitBarX,NO).

  DYNAMIC-FUNCTION("setFollowSplitBarX",THIS-PROCEDURE:CURRENT-WINDOW,btnSplitBarX:HANDLE IN FRAME frSplitBarX, 
                    DYNAMIC-FUNCTION("getWidgetsByLasso",CustomerNavBrowse:HANDLE IN FRAME {&FRAME-NAME},0,"frame,control-frame,rectangle,browse") + "," +
                    DYNAMIC-FUNCTION("getWidgetsByLasso",CustomerFolder:HANDLE IN FRAME {&FRAME-NAME},0,"frame,browse,control-frame,rectangle,editor,button,toggle-box,combo-box,fill-in")
/*                   + (IF SEARCH("controls.dll") NE ? THEN "," + DYNAMIC-FUNCTION("getEditorFrameHandle" IN hCurrTabProc) ELSE "") */
                    ).
