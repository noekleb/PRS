...
&Scoped-Define ENABLED-OBJECTS rectTabFolder NavToolbar brwBrowse 
...
------------------------------------------------------------------------------*/
  ENABLE rectTabFolder NavToolbar brwBrowse 
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  DO ix = 1 TO 2:
    ASSIGN hCurrTabFrame   = DYNAMIC-FUNCTION("getPageFrame" IN hTabFolder,ix)
           hCurrTabQuery   = DYNAMIC-FUNCTION("getPageQuery" IN hTabFolder,ix)
           hPageBrwDesRect = DYNAMIC-FUNCTION("getDesignObject",hCurrTabQuery)
    ...
    DYNAMIC-FUNCTION("setNoResizeY", THIS-PROCEDURE:CURRENT-WINDOW,hCurrTabFrame,
                     (IF VALID-HANDLE(hCurrTabQuery) THEN hCurrTabQuery:NAME + "," ELSE "") 
                   + (IF VALID-HANDLE(hPageBrwDesRect) THEN hPageBrwDesRect:NAME + "," ELSE "") 
                   + hCurrTabFrame:NAME + "," + cWidgetList).
  END.

  DYNAMIC-FUNCTION("setNoResizeY", THIS-PROCEDURE:CURRENT-WINDOW, FRAME {&FRAME-NAME}:HANDLE,"rectTabFolder").
...
/* To avoid error message when running the template: */
