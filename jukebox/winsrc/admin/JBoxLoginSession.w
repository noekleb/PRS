...
ON END-ERROR OF C-Win /* Vis aktivitet */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Vis aktivitet */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-RESIZED OF C-Win /* Vis aktivitet */
DO:
...
DO WITH FRAME {&FRAME-NAME}:
  DYNAMIC-FUNCTION("setAttribute",rectBrowse:HANDLE,"getrecordcount","yes").
  hBrowse = DYNAMIC-FUNCTION("NewBrowse",          /* Create a browse object */
                    ...
                    "JBoxLoginSession"                   /* Buffers and fields: <buffer1>;<field1>;<field2>..,<buffer2>;.. No fields gives all*/
                    + ";dCreated|Created"
                    + ";cJBoxUserId|User id|x(40)"
                    + ";iJBoxCompanyId"
                    + ";cLanguage|Language"
                    + ";cDateFormat|Date format"
                    + ";cNumFormat|Num format"
                    + (IF DYNAMIC-FUNCTION("IsFieldNameInTable","JBoxLoginSession","iLoginTime") THEN
                         ";+LoginTime|CHARACTER|x(8)|jb_hhmmss(iLoginTime)|Login time"
                       ELSE "")
                    + (IF DYNAMIC-FUNCTION("IsFieldNameInTable","JBoxLoginSession","iLogoutTime") THEN
                         ";+LogoutTime|CHARACTER|x(8)|jb_hhmmss(iLogoutTime)|Logout time"
                       ELSE "")
                    + ";cContext|Context"
                  + ",JBoxUser"
                    + ";cUserName|User name@3"
                  + ",JBoxCompany"
                    + ";cCompanyName|Company name@5"
                    ,"WHERE true,FIRST JBoxUser OF JBoxLoginSession NO-LOCK OUTER-JOIN,FIRST JBoxCompany OF JBoxLoginSession NO-LOCK OUTER-JOIN"
                    ,"sort|dCreated desc").          /* Initial sort */

/*   hBrowse:MOVE-COLUMN(8,3). */
/*   hBrowse:MOVE-COLUMN(9,5). */

  DYNAMIC-FUNCTION("setAttribute",hBrowse,"filterlookupfields_cJBoxUserId","JBoxUser;cJBoxUserId;cUserName").
  DYNAMIC-FUNCTION("setAttribute",hBrowse,"filterlookupquery_cJBoxUserId","where true").
  DYNAMIC-FUNCTION("setAttribute",hBrowse,"filterlookupreturnfield_cJBoxUserId","cJBoxUserId").
