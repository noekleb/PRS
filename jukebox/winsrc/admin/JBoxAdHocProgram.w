...
DEF VAR cProgramType  AS CHAR NO-UNDO.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  cProgramType = DYNAMIC-FUNCTION("getParameter" IN SOURCE-PROCEDURE) NO-ERROR.

  &IF DEFINED(UIB_is_Running) NE 0 &THEN
    cProgramType = "kontroll av utgiftsdata".
...
DEF VAR bReport          AS LOG    NO-UNDO.
...
IF iReturn > 0 THEN DO:
  ...
  ASSIGN cProgName = REPLACE(hBuffer:BUFFER-FIELD("cCodeValue"):BUFFER-VALUE,"\","/")
         cReportFileType = (IF hBuffer::cMisc2 NE "" THEN hBuffer::cMisc2 ELSE " ")
         .

  IF NOT DYNAMIC-FUNCTION("getMyTempTable",hBuffer::cCodeValue,cProgName + ";" + cParam,hReportBuffer) THEN
    DYNAMIC-FUNCTION("DoMessage",0,0,DYNAMIC-FUNCTION("getTransactionMessage"),"","").
  ELSE DO:
    cFileTypes = DYNAMIC-FUNCTION("getTransactionMessage").
    IF MAX(1,NUM-ENTRIES(cFileTypes)) < NUM-ENTRIES(cReportFileType) THEN
      DO ix = NUM-ENTRIES(cFileTypes) TO NUM-ENTRIES(cReportFileType):
        cFileTypes = cFileTypes + (IF cFileTypes NE "" THEN "," ELSE "") + "txt".
      END.
    DO ix = 1 TO NUM-ENTRIES(cReportFileType):
      cFileName = SESSION:TEMP-DIR + ENTRY(NUM-ENTRIES(cProgName,"/"),cProgName,"/") + "_" + STRING(YEAR(TODAY)) + "_" + STRING(MONTH(TODAY),"99") + "_" + STRING(DAY(TODAY),"99") 
                + (IF cReportFileType NE " " THEN ENTRY(ix,cReportFileType) ELSE "") + "." + ENTRY(ix,cFileTypes).
      ...
      hQuery:QUERY-PREPARE("FOR EACH " + hReportBuffer:NAME + " WHERE "
                         + (IF cReportFileType NE " " THEN "cType = '" + ENTRY(ix,cReportFileType) + "'" ELSE "true")
                         + " BY iLineNum"
                           ).
      hQuery:QUERY-OPEN().
      hQuery:GET-FIRST().
...
DO WITH FRAME {&FRAME-NAME}:
  hBrowse = DYNAMIC-FUNCTION("NewBrowse"
          ...
          ,"JBoxGenCode"
           + ";cCodeValue|Program"
           + ";cDescription|" + (IF bScand THEN "Beskrivelse" ELSE "Description")
           + ";cMisc1|Parameter (misc1)"
           + ";cMisc2|" + (IF bScand THEN "logg-fil typer" ELSE "log-file types") + " (misc2)|x(80)"
           + ";!+LastParam|CHARACTER|x(40)||"
          ,"WHERE false"
          ,"").
  ...
  DYNAMIC-FUNCTION("setAttribute",hBrowse,"baseQuery","WHERE cCodeType = '" + cProgramType + "'").
/*   DYNAMIC-FUNCTION("setAttribute",hBrowse,"baseQuery","WHERE cCodeType = 'ad-hoc-programs'(+Company)"). */
