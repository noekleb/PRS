...
&Scoped-Define ENABLED-OBJECTS brwJBoxEventList 
...
DEFINE RECTANGLE brwJBoxEventList
...
DEFINE FRAME DEFAULT-FRAME
     brwJBoxEventList AT ROW 1.29 COL 2
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SCROLLABLE SIZE 35.2 BY 4.29.
...
/* SETTINGS FOR FRAME DEFAULT-FRAME
   FRAME-NAME                                                           */
ASSIGN 
       FRAME DEFAULT-FRAME:HEIGHT           = 4.29
       FRAME DEFAULT-FRAME:WIDTH            = 35.2.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
IF bManageEvents THEN DO:
  RUN JBoxDEventFollowUp.w (hBrowse:QUERY:GET-BUFFER-HANDLE(1):BUFFER-FIELD("iJBoxEventLogId"):BUFFER-VALUE,
                            "",
                            "closed",
                            "",
                            OUTPUT bOk).
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  hBrowse = DYNAMIC-FUNCTION("NewBrowse"
          ,brwJBoxEventList:HANDLE
          ,100
          ,""
          ,"JBoxEventLog"
           + ";dEventDate|" + (IF bScand THEN "Dato" ELSE "Date")
           + ";+EventTime|CHARACTER|x(5)|jb_hhmm(iEventTime)|" + (IF bScand THEN "Tid" ELSE "Time")
           + ";cEventText|" + (IF bScand THEN "Tekst" ELSE "Text") + "|x(256)"
           + (IF bManageEvents THEN 
                ";dResponseDate|" + (IF bScand THEN "Oppfølging" ELSE "Follow-up") 
              + ";cResponseText|" + (IF bScand THEN "Oppfølgingstekst" ELSE "Follow-up text")
              + ";dModified" + (IF NOT bScand THEN "|Modified" ELSE "")
              + ";cModifiedBy|" + (IF NOT bScand THEN "Mod.by" ELSE "Endr.av")
              ELSE 
                ";+EventMonth|INTEGER|999999|jb_month(dEventDate)|" + (IF bScand THEN "Måned" ELSE "Month")
              )
           + ";dCreated" + (IF NOT bScand THEN "|Created" ELSE "")
           + ";cCreatedBy|" + (IF NOT bScand THEN "Cre.by" ELSE "Oppr.av")
           + ";!cEventStatus|Status"
           + ";!cEventLogType|Type"
           + ";!iJBoxEventLogId"
         + ",JBoxEventLogType"
           + ";cEventLogTypeText|Type@5"
         + ",JBoxEventLogStatus"
           + ";cEventStatusText|Status@4"
          ,"WHERE false"
         + ",FIRST JBoxEventLogType OF JBoxEventLog NO-LOCK"
         + ",FIRST JBoxEventLogStatus OF JBoxEventLog NO-LOCK OUTER-JOIN"
          ,"").
  DYNAMIC-FUNCTION("setSortString",hBrowse,"dEventDate;desc,EventTime;desc").

  IF bManageEvents THEN
    hBrowse:TOOLTIP = IF DYNAMIC-FUNCTION("Scandinavian") THEN
                        "Dobbeltklikk for å behandle hendelse"
                      ELSE "Doubleclick to manage event".

  DYNAMIC-FUNCTION("CreateObjectLink",hBrowse,THIS-PROCEDURE).

  DYNAMIC-FUNCTION("NewMenuBand",hBrowse
          ,"MultiSortBrowse;" + (IF bScand THEN
                                  "Sorter på flere kolonner"
                                 ELSE "Sort on multiple columns")
         + ",Filter,Accum;" + (IF bScand THEN "Akkumuler" ELSE "Accumulate") 
         + ",Excel"
          ,"").
...
hColumn = DYNAMIC-FUNCTION("getBrowseColumn",ihBrowse,"cEventText").
...
DYNAMIC-FUNCTION("setNoMoveX", THIS-PROCEDURE:CURRENT-WINDOW, FRAME {&FRAME-NAME}:HANDLE,"brwJBoxEventList").
DYNAMIC-FUNCTION("setNoMoveY", THIS-PROCEDURE:CURRENT-WINDOW, FRAME {&FRAME-NAME}:HANDLE,"brwJBoxEventList").
