...
DEF VAR hDataTypeOverlay  AS HANDLE NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE FileImportToTable C-Win 
PROCEDURE FileImportToTable :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEF INPUT PARAM icDatabase  AS CHAR NO-UNDO.
DEF INPUT PARAM icDBtable   AS CHAR NO-UNDO.

IF VALID-HANDLE(hDBfieldOverlay) AND VALID-HANDLE(hBrowse) THEN DO:
  IF icDBtable NE ? AND icDBtable NE "" THEN
    hDBfieldOverlay:LIST-ITEM-PAIRS = "||" +
                                 DYNAMIC-FUNCTION("GetFieldList",
                                                  icDatabase
                                                + "_file," + icDatabase + "_field;_field-name;_field-name",
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  /* Create the browse: */
   hBrowse = DYNAMIC-FUNCTION("NewBrowse",
                           ...
                           "JBoxFileImportSpec"              /* Buffers and fields (and calculated fields) for browse */
                           + ";iSeq"
                           + ";+cAlfaSeq|CHARACTER|x(3)|jbserv_seqno_to_alpha.p(iSeq)|Alpha"
                           + ";cFieldName;cDataType;cDBfieldName"
                           , 
                           "WHERE false",  
...
/*   DYNAMIC-FUNCTION("setNoColumnSort",hBrowse,"iSeq;cFieldName,cDataType,cDBtableName,cDBfieldName").  */

  hDataTypeOverlay = DYNAMIC-FUNCTION("NewBrowseDropDown",
                    hBrowse,                      /* Handle to browse */
                    "cDataType",                  /* Browse column (display) */
                    "cDataType",                   /* Buffer column (to update - foreign key. Maps to value - under) */
                    "",                           /* DB buffers and fields for drop-down values: Label,value */
                    "",                           /* Query to get drop-down values */
                    "||CHARACTER|CHARACTER|DATE|DATE|DECIMAL|DECIMAL|INTEGER|INTEGER|LOGICAL|LOGICAL").   /* I've got the values. Don't go to the database */
  DYNAMIC-FUNCTION("CreateOverlayLink",hBrowse,hDataTypeOverlay,"cDataType"). /* Link the dropdown to the browse with column-name info */
...
/*   hDBtableOverlay = DYNAMIC-FUNCTION("NewBrowseDropDown",                        */
/*                     hBrowse,                                                     */
/*                     "cDBtableName",                                              */
/*                     "",                                                          */
...
/*                     "||").                                                       */
/*   DYNAMIC-FUNCTION("CreateOverlayLink",hBrowse,hDBtableOverlay,"cDBtableName").  */
  ...
  SUBSCRIBE TO "FileImportToTable" ANYWHERE.
END.
...
hSeqOverlay:SCREEN-VALUE = DYNAMIC-FUNCTION("getLocalQueryMaxValue",hBrowse,"iSeq",1).
APPLY "tab" TO hSeqOverlay.
...
DYNAMIC-FUNCTION("setAttribute",hBrowse,"enableOnDblClick","").
APPLY "value-changed" TO hBrowse.
DYNAMIC-FUNCTION("setAttribute",hBrowse,"enableOnDblClick","yes").
APPLY "entry" TO hFieldNameOverlay.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE xxExtraValChngBrowseDropDown C-Win 
PROCEDURE xxExtraValChngBrowseDropDown :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
IF ihCombo = hDBtableOverlay THEN DO:
  ...
  IF ihCombo:SCREEN-VALUE <> ? THEN
    hDBfieldOverlay:LIST-ITEM-PAIRS = "||" +
                                 DYNAMIC-FUNCTION("GetFieldList",
                                                  "_file,_field;_field-name;_field-name",
