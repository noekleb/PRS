...
Modified    : 
    ...
    26.jan14 brynjar@chemistry.no
                      Modified tool_choose_with_projects for JukeBox objects
                      JukeBox tools are swapped with standeard tools and posted
                      to the super procedure so they can be picked up by _drawobj.p                  
    10.sep 14 brynjar@chemistry.no
                      Changes moved forward to 11.4                        
                      Corrected bug in downloaded source:
                      wrappers/destdefi.i was referred as destdefi.i
----------------------------------------------------------------------------*/
/* ===================================================================== */
...
procedure tool_choose_with_projects:
  ...
  /* JukeBox, Brynjar: */
  DEF VAR cJBoxTool AS CHAR NO-UNDO.
  IF tool BEGINS "jb-" THEN DO:
    RUN JBoxLoadLib.p ("JukeBox\abTmpDrawObjSuper.p").
    DYNAMIC-FUNCTION("setToolChoose",tool). /* To be picked up by _drawobj.p */  
    tool = SUBSTRING(tool,4).
    IF CAN-DO("toolbar,dot-net-usercontrol,jlw-control,tabfolder,viewer,searchfield",tool) THEN tool = "rectangle".
    IF tool = "fieldmap" THEN tool = "db-fields".
    IF tool = "override" THEN tool = "text".
    IF tool = "popup" THEN tool = "selection-list". /* to pick the right cursor image */
  END.
  ELSE DYNAMIC-FUNCTION("setToolChoose","") NO-ERROR.
