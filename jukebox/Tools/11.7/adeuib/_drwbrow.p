...
Modified 28.feb.14 by Brynjar for JukeBox
                   Reshape the _BC table before writing out to file
----------------------------------------------------------------------------*/
/* ===================================================================== */
...
DO TRANSACTION ON STOP UNDO,LEAVE:
  ...
  /* Brynjar for JukeBox: reshape _BC before writing the code */
  bJukeBox = DYNAMIC-FUNCTION("getNewJukeboxObject") NO-ERROR.
  IF bJukeBox AND _Q._TblList NE "" THEN DO:
    _Q._OpenQury = NO.
    IF CAN-FIND(FIRST x_U WHERE x_U._NAME = "Brw" + ENTRY(NUM-ENTRIES(ENTRY(1,_Q._TblList),"."),ENTRY(1,_Q._TblList),".")
                AND x_U._WINDOW-HANDLE = _h_win AND x_U._TYPE = "browse") THEN
       MESSAGE "Error: Could not assign " "Brw" + ENTRY(NUM-ENTRIES(ENTRY(1,_Q._TblList),"."),ENTRY(1,_Q._TblList),".")
               " as name for browse widget (duplicate)." SKIP
           VIEW-AS ALERT-BOX INFO BUTTONS OK.
    ELSE
      _U._NAME = "Brw" + ENTRY(NUM-ENTRIES(ENTRY(1,_Q._TblList),"."),ENTRY(1,_Q._TblList),".").
    DYNAMIC-FUNCTION("initJukeBoxObject",_U._NAME,_U._WINDOW-HANDLE,"browse",NO).
    RUN genBrowseDisplay (_U._NAME).
    DYNAMIC-FUNCTION("getBrowseTable",_U._NAME,NO). /* reshape _BC before writing the code */
  END.
