...
Modified:
  ...
  11.01.14 - brynjar@chemistry.no
	           Added palette type (group) 2 with JukeBox objects
             (only if oeideisrunning)
---------------------------------------------------------------------------*/
...
{adecomm/oeideservice.i}   /* Added by Brynjar to be able for check for oeideisrunning  */
...
/* Brynjar: */
DEF BUFFER b_palette_item FOR _palette_item.
...
IF OEIDEIsRunning THEN
  FOR EACH _palette_item
      WHERE _palette_item._type = 0 OR _palette_item._type = 3:
/*     _palette_item._type = 4. */
    DELETE _palette_item.
  END.
/* DEF VAR hPaletteItem AS HANDLE NO-UNDO.      */
/* hPaletteItem = BUFFER b_palette_item:HANDLE. */
/* RUN toexcelviafile.p (hPaletteItem,0).       */
RETURN.
...
/*
def var h_palette as handle no-undo.
h_palette = buffer _palette_item:handle.
run toexcelviafile.p (h_palette,-1).
*/
  /* JukeBox, Brynjar: */
  
  IF OEIDEIsRunning THEN DO:
    FOR EACH b_palette_item
      	WHERE b_palette_item._order > 5
	      BY b_palette_item._order DESC:
      b_palette_item._order = b_palette_item._order + 5.
    END.

    FIND FIRST b_palette_item 
         WHERE b_palette_item._name = "Browse"
         NO-ERROR.
    IF AVAIL b_palette_item THEN DO:
      CREATE _palette_item.
      BUFFER-COPY b_palette_item TO _palette_item
      ASSIGN _palette_item._type   = 2
             _palette_item._label  = "JB Browse"
             _palette_item._label2 = "JB Browse"
             _palette_item._name   = "jb-browse"
             _palette_item._order  = 6	
             .
    END.
    FIND FIRST b_palette_item 
         WHERE b_palette_item._name = "Query"
         NO-ERROR.
    IF AVAIL b_palette_item THEN DO:
      CREATE _palette_item.
      BUFFER-COPY b_palette_item TO _palette_item
      ASSIGN _palette_item._type   = 2
             _palette_item._label  = "JB Query"
             _palette_item._label2 = "JB Query"
             _palette_item._name   = "jb-query"
             _palette_item._order  = 7	
             .
    END.
    FIND FIRST b_palette_item 
         WHERE b_palette_item._name = "Db-Fields"
         NO-ERROR.
    IF AVAIL b_palette_item THEN DO:
      CREATE _palette_item.
      BUFFER-COPY b_palette_item TO _palette_item
      ASSIGN _palette_item._type   = 2
             _palette_item._label  = "JB FieldMap"
             _palette_item._label2 = "JB FieldMap"
             _palette_item._name   = "jb-fieldmap"
             _palette_item._order  = 8	
    	       .
    END.
    FIND FIRST b_palette_item 
         WHERE b_palette_item._name = "SmartToolbar"
         NO-ERROR.
    IF AVAIL b_palette_item THEN DO:
      CREATE _palette_item.
      BUFFER-COPY b_palette_item TO _palette_item
      ASSIGN _palette_item._type   = 2
             _palette_item._label  = "JB Toolbar"
             _palette_item._label2 = "JB Toolbar"
             _palette_item._name   = "jb-toolbar"
             _palette_item._order  = 9	
    	       .
    END.
    FIND FIRST b_palette_item 
         WHERE b_palette_item._name = "DataView"
         NO-ERROR.
    IF AVAIL b_palette_item THEN DO:
      CREATE _palette_item.
      BUFFER-COPY b_palette_item TO _palette_item
      ASSIGN _palette_item._type   = 2
             _palette_item._label  = "JB Searchfield"
             _palette_item._label2 = "JB Searchfield"
             _palette_item._name   = "jb-searchfield"
             _palette_item._order  = 10	
    	       .
    END.
    FIND FIRST b_palette_item 
         WHERE b_palette_item._name = "SmartFolder"
         NO-ERROR.
    IF AVAIL b_palette_item THEN DO:
      CREATE _palette_item.
      BUFFER-COPY b_palette_item TO _palette_item
      ASSIGN _palette_item._type   = 2
             _palette_item._label  = "JB Tabs"
             _palette_item._label2 = "JB Tabs"
             _palette_item._name   = "jb-tabfolder"
             _palette_item._order  = 11	
    	       .
    END.
    FIND FIRST b_palette_item 
         WHERE b_palette_item._name = "Selection-List"
         NO-ERROR.
    IF AVAIL b_palette_item THEN DO:
      CREATE _palette_item.
      BUFFER-COPY b_palette_item TO _palette_item
      ASSIGN _palette_item._type   = 2
             _palette_item._label  = "JB Popup"
             _palette_item._label2 = "JB Popup"
             _palette_item._name   = "jb-popup"
             _palette_item._order  = 12	
    	       .
    END.
    FIND FIRST b_palette_item 
         WHERE b_palette_item._name = "Text"
         NO-ERROR.
    IF AVAIL b_palette_item THEN DO:
      CREATE _palette_item.
      BUFFER-COPY b_palette_item TO _palette_item
      ASSIGN _palette_item._type   = 2
             _palette_item._label  = "JB Override and cliplib"
             _palette_item._label2 = "JB Override and cliplib"
             _palette_item._name   = "jb-override"
             _palette_item._order  = 13	
    	       .
    END.
    FIND FIRST b_palette_item 
         WHERE b_palette_item._name = "SmartRouter"
         NO-ERROR.
    IF AVAIL b_palette_item THEN DO:
      CREATE _palette_item.
      BUFFER-COPY b_palette_item TO _palette_item
      ASSIGN _palette_item._type   = 2
             _palette_item._label  = ".Net usercontrol"
             _palette_item._label2 = ".Net usercontrol"
             _palette_item._name   = "jb-dot-net-usercontrol"
             _palette_item._order  = 14	
    	       .
    END.
    ...
    FIND FIRST b_palette_item 
         WHERE b_palette_item._name = "DB-Fields"
         NO-ERROR.
    IF AVAIL b_palette_item THEN b_palette_item._order = 5.
    */
  END.
