...
Modified:     1/98 SLK Added runtime code for SmartData's PRIVATE-DATA
          ...
          04.mar.14 by Brynjar for JukeBox
                    Checks if table name is ? (ie calc.field) when hiding a column
---------------------------------------------------------------------------- */
/* ************************************************************************* */
/*                                                                           */
/*                           RUN-TIME ATTRIBUTES                             */
            ...
            IF NOT _BC._VISIBLE OR _BC._AUTO-RESIZE OR _BC._COLUMN-READ-ONLY THEN DO:
              ...
              IF NOT _BC._VISIBLE THEN DO:
                ...
                ELSE 
                  PUT STREAM P_4GL UNFORMATTED SKIP
                    /* Brynjar for JukeBox: */
                    "       " (IF _BC._TABLE NE ? THEN _BC._TABLE + "." + _BC._NAME ELSE ENTRY(1,_BC._DISP-NAME,".") + "." + ENTRY(2,_BC._DISP-NAME,".")) + ":VISIBLE IN BROWSE " x_U._NAME " = FALSE".
              END.  /* if not visible */
