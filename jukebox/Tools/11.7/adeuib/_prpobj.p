...
Modified:
  ...
  Brynjar 14.01.13 Added button for JukeBox properties 
                   Also overrides freeform_setup for browse/query  
                   To see changes search for JukeBox             
----------------------------------------------------------------------------*/
...
DEF VAR hAbTmpDrawObjSuper   AS HANDLE NO-UNDO.
DEF VAR cJBoxQueryDef        AS CHAR NO-UNDO.
DEF VAR cJBoxUserControl     AS CHAR NO-UNDO.
DEF VAR cJBoxPropCode        AS CHAR NO-UNDO.
DEF VAR cJBoxUserControlType AS CHAR NO-UNDO.
DEF VAR cTbName              AS CHAR NO-UNDO.
DEF VAR cMenu                AS CHAR NO-UNDO.
DEF VAR cMenuProp              AS CHAR NO-UNDO.
...
DEF VAR fStartBtnRow         AS DEC  NO-UNDO.
...
/* JukeBox: */
DEFINE BUTTON    btn_jukebox      LABEL "&JukeBox...":L SIZE 15 BY 1.125.
...
/* JukeBox initialize widget properties based on JukeBox code: */
IF AVAIL _U THEN DO:
  _query-u-rec = RECID(_U).
  _h_win = _U._WINDOW-HANDLE.
  RUN JBoxDeleteLib.p ("JukeBox\abTmpDrawObjSuper.p",YES). /* restart */ 
  ...
  DYNAMIC-FUNCTION("setActiveWidget",_U._NAME).
  RUN FindTablesAndFieldsInUse. /* Read from JukeBox view definitions in definition section */
  cJBoxQueryDef = DYNAMIC-FUNCTION("getObjectQueryDef",_U._NAME).
  IF cJBoxQueryDef NE "" THEN
    bJukeBox = YES.
  bJukeBox = DYNAMIC-FUNCTION("getIsJukeBoxObject",_U._NAME,_U._WINDOW-HANDLE).

  IF NOT bJukeBox THEN
    bJukeBox =  INDEX(DYNAMIC-FUNCTION("getFieldsInUse"),_U._NAME) > 0.

  IF bJukeBox AND _U._TYPE = "browse" THEN
    DYNAMIC-FUNCTION("getBrowseTable",_U._NAME,NO).
...
IF NOT RETRY THEN DO:
  DEFINE FRAME prop_sht
         ...
         btn_jukebox  AT ROW 25    COL 53    /* JukeBox */
...
ON CHOOSE OF btn_jukebox IN FRAME prop_sht DO:

  IF AVAIL _U THEN DO:
    CASE _U._TYPE:
      WHEN "BROWSE"    THEN RUN JukeBox\JBoxBrowseProperties.w (RECID(_U),OUTPUT cJBoxUserControl,OUTPUT cJBoxUserControlType,OUTPUT cJBoxPropCode).
      WHEN "EDITOR"    THEN RUN JukeBox\JBoxDotNetProperties.w (RECID(_U),OUTPUT cJBoxUserControl,OUTPUT cJBoxUserControlType,OUTPUT cJBoxPropCode).
      WHEN "FILL-IN"   THEN RUN JukeBox\JBoxDotNetProperties.w (RECID(_U),OUTPUT cJBoxUserControl,OUTPUT cJBoxUserControlType,OUTPUT cJBoxPropCode).
      WHEN "RECTANGLE" THEN DO:
        ASSIGN cTbName = _U._NAME
               cMenu = _U._PRIVATE-DATA
               .
    ...
    END.
  END.
  ELSE MESSAGE "_U not avail"
      VIEW-AS ALERT-BOX INFO BUTTONS OK.
END.
/* JukeBox: 
ON CHOOSE OF btn_cancel 
DO:
  RETURN.
END.
*/
...
/* JukeBox */  
IF bJukeBox THEN DO:
  IF _U._TYPE = "browse" THEN 
    DYNAMIC-FUNCTION("getBrowseTable",_U._NAME,NO). /* re-generate _BC and prepare it for code-save, adeshar/gen4gl.p */ 
  ELSE IF _U._TYPE = "rectangle" AND fBtnHeight > 0 /* buttons have been modified */ THEN DO:
/*     cTbName = _U._NAME. */
    ...
    FOR EACH ttJBoxTool 
        WHERE ttJBoxTool.bDeleted
           OR ttJBoxTool.cViewAs = "menu-item"
       ,FIRST x_U WHERE RECID(x_U) = ttJBoxTool.U_recid
       ,FIRST x_L WHERE RECID(x_L) = x_U._lo-recid 
       BY ttJBoxTool.fCol DESC:

      RUN adeuib\_delet_u.p(ttJBoxTool.U_recid,YES).
      IF ttJBoxTool.bDeleted THEN
        DELETE ttJBoxTool.
    END.
    ix = 0.
    FOR EACH ttJBoxTool
        WHERE ttJBoxTool.bSelect
          ...
          AND ttJBoxTool.cViewAs NE "menu-item"
          AND NOT CAN-DO("menu,sub-menu",ttJBoxTool.cType)
        BY ttJBoxTool.iSeq
        :

      IF NOT ttJBoxTool.bExist THEN DO:
        _frmx = 1. /* _frmx + 23. */
        _frmy = 1. /* _frmx + 23. */
        _cur_row_mult = 1.
        _cur_col_mult = 1.
        _second_corner_x = _second_corner_x + 5.
        _second_corner_y = _second_corner_y + 5.
      ...
      END.
      ELSE 
        FIND _U WHERE RECID(_U) = ttJBoxTool.U_recid.
      ...
      ASSIGN _F._IMAGE-FILE = ttJBoxTool.cImage
             ...
             _L._COL = fStartBtnCol + fBtnWidth * ix + (IF ix > 0 THEN .1 ELSE 0) + fBtnColAdj
             _L._ROW = fStartBtnRow
    ...
    FIND FIRST _U 
         WHERE _U._WINDOW-HANDLE = _P._WINDOW-HANDLE
           ...
           AND NOT _U._DELETED
           AND _U._NAME = cTbName
           .
    RUN BuildJBoxToolbarDef (RECID(_U),httJBoxTool,cTbName,cMenu,cMenuProp).
  END.
  DYNAMIC-FUNCTION("setJBoxUserContrProperties",RECID(_U),cJBoxUserControl,cJBoxUserControlType,cJBoxPropCode).
END.
...
PROCEDURE do_field_edit.
  ...
  IF bJukeBox THEN
    xtbls = DYNAMIC-FUNCTION("getBrowseTable",_U._NAME,YES). /* Yes: generate _BC records or prepare them for mod */
  ELSE
  ...
  /* Now replace the display trigger based on edited _BC and clean-up _BC */
  IF bJukeBox THEN
    RUN genBrowseDisplay(_U._NAME).
...
procedure do_query_modify:
  DO ON QUIT, LEAVE:
    IF bJukeBox THEN
      DYNAMIC-FUNCTION("getBrowseTable",_U._NAME,YES). /* Yes: generate _BC and _Q records also */
    ...
    END. 
    /* Replace the display trigger based on edited _BC and clean-up _BC */
    IF bJukeBox THEN
      RUN genBrowseDisplay(_U._NAME).
...
/*     DEFINE VAR h_q AS HANDLE NO-UNDO.  */
/*     h_q = BUFFER _Q:HANDLE.            */
/*     RUN TOexcelviafile.p(h_q,-1).      */
  END.
end procedure.    
...
PROCEDURE adjust_frame.
  ASSIGN FRAME prop_sht:HIDDEN     = TRUE 
    ...
    &endif
         ...
         /* JukeBox: */
         btn_jukebox:SENSITIVE     = bJukeBox
...
PROCEDURE final_adjustments:
  DO WITH FRAME prop_sht:
    ...
    /* JukeBox: */
    btn_jukebox:ROW = btn_Cancel:ROW.
    ...
    ASSIGN cur-row                 = cur-row + (tog-rows * tog-spc) + {&IVM_OKBOX} -
           ...
           /* JukeBox: */
           btn_jukebox:ROW         = btn_jukebox:ROW - adjust
...
PROCEDURE freeform_setup.
  IF bJukeBox THEN DO:
    ASSIGN h_btn_mdfy:SENSITIVE = YES
           h_btn_flds:SENSITIVE = YES
           h_query:SCREEN-VALUE = "JukeBox query:" + CHR(10) + cJBoxQueryDef
           .
    RETURN.
  END.
