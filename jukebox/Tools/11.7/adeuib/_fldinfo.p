...
Modified:
  ...
  Modified for JukeBox 18.02.15 by brynjar@chemistry.no
                For calculated fields in freeform query defined _Field is not available
...
/* JukeBox: */
IF p_tbl_name MATCHES "buf*_*" THEN p_tbl_name = SUBSTR(p_tbl_name,6).
DEF VAR hbtt_field AS HANDLE NO-UNDO.
/* End JukeBox */
...
IF AVAIL _field THEN DO: /* JukeBox */
...
END.
ELSE DO:
  RUN getBttField (p_db_name,p_tbl_name,p_fld_name,OUTPUT hbtt_field).
  IF VALID-HANDLE(hbtt_field) THEN
    ASSIGN p_fld_label     = hbtt_field::_label
           p_fld_format    = hbtt_field::_format
           p_fld_type      = hbtt_field::_data-type
           p_fld_help      = hbtt_field::_help
           p_fld_extent    = hbtt_field::_extent
           p_fld_initial   = hbtt_field::_initial
           p_fld_description = hbtt_field::_desc
           p_fld_valexp    = hbtt_field::_valexp
           p_fld_valmsg    = hbtt_field::_valmsg
           p_fld_mandatory = hbtt_field::_mandatory
           p_fld_label_sa  = hbtt_field::_Label-SA 
           p_fld_format_sa = hbtt_field::_Format-SA 
           p_fld_help_sa   = hbtt_field::_Help-SA
           p_fld_valmsg_sa = hbtt_field::_Valmsg-SA
           .
  ELSE DO:
    MESSAGE "Field info for " p_fld_name " cannot be retrieved" SKIP
            "If it is a calculated field for a freeform query you must supply info for the calculation (see tips in text file)" SKIP
            "Assumes CHARACTER as datatype in the temp-table definition"
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
    p_fld_type = "CHARACTER".
  END.
END.
/* End JukeBox */
