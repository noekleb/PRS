...
DEF VAR cViewName            AS CHAR NO-UNDO.
...
/*           cTtFieldDef      = "/* Code for Definitions: */" + CHR(10)
           ...
           cViewName        = cPrimaryTable + "View"
           ENTRY(3,_4GLQury," ") = cViewName
           _4GLQury         = ENTRY(1,_4GLQury) + " INDEXED-REPOSITION."
           _Q._TblList      = cViewName
           .
  ...
  IF _U._TYPE = "BROWSE":U THEN DO:
    ...
    MESSAGE "_TRG._tCODE" _TRG._tCODE
        VIEW-AS ALERT-BOX INFO BUTTONS OK.
    /* JukeBox option selected: */
    IF ok-cancel = "JukeBox" THEN DO:
      cTtFieldDef  = "/* Code for Definitions: */" + CHR(10) + CHR(10)
                   + "/*** Start instance property definitions for JBoxBrowse object oBrw" + cViewName + " ***/" + CHR(10)
                   + "DEF VAR oBrw" + cViewName + " AS JBoxBrowse NO-UNDO." + CHR(10) + CHR(10) 
                   + "DEF TEMP-TABLE " + cViewName + CHR(10)
                   .
      ...
      DO ix = 1 TO NUM-ENTRIES(_TRG._tCODE,CHR(10)):
        ...
        IF NUM-ENTRIES(cDisplayFieldDef,".") > 1 THEN DO:
          ...
          ASSIGN ttTableFlds.cTable      = cTable
                 ...
                 ttTableFlds.cDisplayDef = SUBSTR(cDisplayFieldDef,INDEX(cDisplayFieldDef," "))
                 ttTableFlds.iSeq        = ix
                 .
      ...
      FIND LAST _U WHERE _U._NAME BEGINS "BROWSE-"
           NO-ERROR.
      IF AVAIL _U THEN 
        ASSIGN _U._NAME  = "brw" + cViewName + "_AssignedByJbox"
/*               cFileName = SESSION:TEMP-DIR + _U._PROC-HANDLE:FILE-NAME + "_" + cViewName + "_JukeBox_BrowseDef.txt" */
               .

      IF cFileName = "" OR cFileName = ? THEN
        cFileName = SESSION:TEMP-DIR + cViewName + "_JukeBox_BrowseDef.txt".
  ...
  ELSE IF ok-cancel = "JukeBox" THEN DO:
    cTtFieldDef  = "/* Code for Definitions: */" + CHR(10) + CHR(10)
                 + "/*** Start instance property definitions for JBoxQuery object oQry" + cViewName + " ***/" + CHR(10)
                 + "DEF VAR oQry" + cViewName + " AS JBoxQuery NO-UNDO." + CHR(10) + CHR(10) 
                 + "DEF TEMP-TABLE " + cViewName + CHR(10)
                 .
    ...
    /*
    FIND LAST _U WHERE _U._NAME BEGINS "QUERY-"
         NO-ERROR.
    IF AVAIL _U THEN 
      ASSIGN _U._NAME  = "qry" + cViewName 
                    /* + "_AssignedByJbox" - Should be added but cannot figure out why it isn't handled in adeshar/_bstname.p */
             .
    */
    IF cFileName = "" OR cFileName = ? THEN
      cFileName = SESSION:TEMP-DIR + cViewName + "_JukeBox_QueryDef.txt".
...
PROCEDURE BuildTtAndQueryDefintion:
    ...
    FOR EACH ttTableFlds BY ttTableFlds.iSeq:
      ASSIGN cDisplayFieldDef  = cViewName + "." + ttTableFlds.cTtFld + ttTableFlds.cTtExtent + ttTableFlds.cDisplayDef
    ...
    DO ix = 1 TO NUM-ENTRIES(cTableList):
      ...
      ELSE
        cTableFieldDef = cTableFieldDef + (IF cTableFieldDef NE "" THEN "," ELSE "'") + ENTRY(ix,cTableList).
/*      IF icType = "browse" THEN */
        FOR EACH ttTableFlds
            WHERE ttTableFlds.cTable = ENTRY(ix,cTableList)
            BY iSeq:
          cTableFieldDef = cTableFieldDef + (IF icType = "browse" THEN CHR(10) + "      + ';" ELSE ";") + ttTableFlds.cDbFld + ttTableFlds.cDbExtent + (IF icType = "browse" THEN "'" ELSE "").
        END.
    ...
    cTtFieldDef = cTtFieldDef 
                ...
                + "    ." + CHR(10) + CHR(10)
                + (IF icType = "query" THEN
                    "FUNCTION getTableHandleQry" + cViewName + " RETURNS HANDLE()." + CHR(10)
                  + "  RETURN BUFFER " + cViewName + ":HANDLE:TABLE-HANDLE." + CHR(10)
                  + "END FUNCTION." + CHR(10)
                   ELSE "")
                + "FUNCTION getBuffersAndFields"
                  + (IF icType = "browse" THEN "Brw" ELSE "Qry") 
                  + cViewName + " RETURNS CHARACTER()." + CHR(10)
                ...
                + "END FUNCTION." + CHR(10)
                + "/*** End instance property settings for JBox" + icType + " object " + (IF icType = "query" THEN "oQry" ELSE "oBrw") + cViewName + " ***/" + CHR(10)
                + "/* End Definitions */" + CHR(10) + CHR(10)
                + "/* Code for InitializeObject (in DO WITH FRAME.. block): */" + CHR(10)
                + (IF icType = "browse" THEN
                    "  oBrw" + cViewName + " = NEW JBoxBrowse(brw" + cViewName + ":HANDLE)." + CHR(10)
                   ELSE
                    "  oQry" + cViewName + " = NEW JBoxQuery('" + cViewName + "')." + CHR(10)
                   )
