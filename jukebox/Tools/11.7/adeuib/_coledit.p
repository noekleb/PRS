...
/*------------------------------------------------------------------------
  ...
  * NOTE that 2 and 3 are mutually exclusive
  ...
  Modified: 1/98 SLK Added morphing for the SmartData column editor
            ...
            12/20/13 Brynjar: Added procedure ExpandFrame  
            01/21/14 Brynjar: Added call on end-error trigger to prevent JukeBox browse rebuild     
            02/19/14 Brynjar: Replaced dialog for add/edit calculated field  ChooseCalcField/DoChooseCalcField and ChooseEdit/DoChooseEdit
                              tog_visible should always be sensitive.            
                              All changes by Brynjar can be found by search on 'Brynjar' or JukeBox
            10.sep.14 Brynjar: Moved changes forward to 11.4                  
                              NB: Bug in downloaded source: wrappers\destdefi.i 
                              was incorrectly refered as destdef.i
            21.sep.16 Brynjar in add-fields.ip added OR.. to the test for how to select table-name from added field:
                              IF imode = "3" OR NUM-ENTRIES(tmp-name,".") = 3
...
/* Brynjar: Is this a JukeBox object?: */
DEF VAR bJukeBox AS LOG NO-UNDO.
bJukeBox = DYNAMIC-FUNCTION("getIsJukeBoxObject",_U._NAME,_U._WINDOW-HANDLE) NO-ERROR.
IF NOT bJukeBox THEN
  bJukeBox = DYNAMIC-FUNCTION("getNewJukeBoxObject") NO-ERROR.
IF bJukeBox = ? THEN bJukeBox = NO.
...
DO:
    ...
    /* JukeBox, Brynjar */
    IF bJukeBox THEN
      DYNAMIC-FUNCTION("setCancelQueryRebuild",YES) NO-ERROR.
...
procedure DoChooseEdit:
 ...
 DEFINE VARIABLE i                 AS INTEGER          NO-UNDO. 
  ...
  /* JukeBox, custom edit of calc.fields: */
  ...
  IF bJukeBox THEN DO:
    ...
    ASSIGN cCalcCont   = ENTRY(1,_BC._DEF-VALEXP,"|")
           cCalcProc   = (IF INDEX(ENTRY(2,_BC._DEF-VALEXP,"|"),"(") > 1 THEN
                            SUBSTR(ENTRY(2,_BC._DEF-VALEXP,"|"),1,INDEX(ENTRY(2,_BC._DEF-VALEXP,"|"),"(") - 1)
                          ELSE ENTRY(2,_BC._DEF-VALEXP,"|"))
           cCalcParam  = (IF INDEX(ENTRY(2,_BC._DEF-VALEXP,"|"),"(") > 1 THEN
                            REPLACE(RIGHT-TRIM(SUBSTR(ENTRY(2,_BC._DEF-VALEXP,"|"),INDEX(ENTRY(2,_BC._DEF-VALEXP,"|"),"(") + 1),")"),"¤",CHR(10))
                          ELSE "ROWID")
          cDataType    = _BC._DATA-TYPE  
          cFormat      = _BC._FORMAT 
          cLabel       = _BC._LABEL
          .
   IF NUM-ENTRIES(_BC._DISP-NAME,".") = 2 THEN
     ASSIGN cSourceTable = ENTRY(1,_BC._DISP-NAME,".")
            cFieldName   = ENTRY(2,_BC._DISP-NAME,".").
   ELSE
     ASSIGN cSourceTable = pTbl
            cFieldName   = _BC._DISP-NAME.
             
  
    RUN jukebox\defineCalcField.w (Tbl-List,
                                   INPUT-OUTPUT cSourceTable,
  ...
  /* JukeBox, assign output from custom calc.proc */
  IF bJukeBox THEN
    ASSIGN _BC._DEF-VALEXP = cCalcCont + "|" + cCalcProc + cCalcParam
           _BC._DATA-TYPE  = cDataType
           _BC._FORMAT     = cFormat
           _BC._LABEL      = cLabel
           _BC._DISP-NAME  = cSourceTable + "." + cFieldName
           .
...
/*   END.                                                  */
  ...
  END.
  ELSE DO:
    /* JukeBox, custom procedure for calculated fields: */
    DEF VAR cSourceTable AS CHAR NO-UNDO.
    DEF VAR cCalcParam   AS CHAR NO-UNDO.
    DEF VAR cFieldName   AS CHAR NO-UNDO.
    DEF VAR cDataType    AS CHAR NO-UNDO.
...
PROCEDURE add-fields.ip:
  ...
  DEF VAR cDispNameList  AS CHAR NO-UNDO. /* JukeBox */
 ...
 IF empty-flg THEN
  ...
  FOR EACH _BC WHERE _BC._x-recid = _query-u-rec:
    ...
    cDispNameList = cDispNameList + (IF cDispNameList NE "" THEN "," ELSE "") + _BC._DISP-NAME. /* JukeBox */
  ...
  ASSIGN cNewFieldList = IF cDispNameList NE "" THEN "NoMove" ELSE "". 
        /* "". JukeBox: When existing fields, move up/down should be disabled in _mfldsel.p */
  IF srcSmartData THEN 
       RUN {&PROCEDURE-NAME} 
  ...
  DO i = 1 TO NUM-ENTRIES(cNewFieldList,CHR(10)):
    ...
    ASSIGN
           ...
           tmp-tbl  = IF srcSmartData  
                      ...
                      ELSE IF imode = "3" OR NUM-ENTRIES(tmp-name,".") = 3 THEN ENTRY(2,tmp-name,".") /* JukeBox: added OR.. */
  ...
  /* JukeBox: re-sequence and put new fields at the bottom: */
/*   MESSAGE cDispNameList                                    */
/*       VIEW-AS ALERT-BOX INFO BUTTONS OK-CANCEL UPDATE bOk. */
/*   IF bOk THEN RUN toexcelviafile.p (h_BC,0).               */
  FOR EACH _BC WHERE _BC._x-recid = _query-u-rec
      BY _BC._SEQUENCE DESC:
    _BC._SEQUENCE = _BC._SEQUENCE + 500.
  END.
  DO cur-seq = 1 TO NUM-ENTRIES(cDispNameList):
    FIND FIRST _BC WHERE _BC._x-recid = _query-u-rec AND _BC._DISP-NAME = ENTRY(cur-seq,cDispNameList)
         NO-ERROR.
    IF AVAIL _BC THEN
      _BC._SEQUENCE = cur-seq.
  END.
  FOR EACH _BC WHERE _BC._x-recid = _query-u-rec:
    IF NOT CAN-DO(cDispNameList,_BC._DISP-NAME) THEN
...
PROCEDURE ExpandFrame:
  ...
  ASSIGN FRAME bc-editor:WIDTH-PIXELS  = FRAME bc-editor:WIDTH-PIXELS + 100
         FRAME bc-editor:HEIGHT-PIXELS = FRAME bc-editor:HEIGHT-PIXELS + 250
         .
  hFg = FRAME bc-editor:FIRST-CHILD.
  REPEAT WHILE VALID-HANDLE(hFg):
    hWidget = hFg:FIRST-CHILD.
    REPEAT WHILE VALID-HANDLE(hWidget):
      IF hWidget:Y > 322 THEN hWidget:Y = hWidget:Y + 250.
      IF hWidget:X > 135 THEN hWidget:X = hWidget:X + 100.
      IF CAN-DO("selection-list,browse",hWidget:TYPE) THEN 
        ASSIGN hWidget:WIDTH-PIXELS = hWidget:WIDTH-PIXELS + 100
