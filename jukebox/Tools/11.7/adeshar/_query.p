...
/*----------------------------------------------------------------------------
  ...
  Modified:
    ...
    03/29/16 brynjar@chemistry.no 
                 - patching up query-string for JukeBox              
                 - added procedures AddAlias.ip and setSimpleSelectAttributes for JukeBox buffers
    05/20/16 brynjar
                 - fixed bug in IsJoinable.ip to avoid error message
...
ASSIGN cTblList = REPLACE({&TableRight}:LIST-ITEMS, 'TEMP-TABLES.', '').
  ...
  /* JukeBox: */
  cQuery = REPLACE(cQuery,"NO-LOCK INDEXED-REPOSITION NO-LOCK INDEXED-REPOSITION","NO-LOCK INDEXED-REPOSITION").
  cQuery = REPLACE(cQuery,"NO-LOCK, NO-LOCK,","NO-LOCK,").
  cQuery = REPLACE(cQuery,", NO-LOCK,"," NO-LOCK,").
  /* end JukeBox */
...
PROCEDURE IsJoinable.ip:
  ...
  /*  JukeBox / Brynjar: */
  IF cRight = "..." OR cLeft = "..." THEN RETURN.
...
PROCEDURE AddAlias.ip:
  /* JukeBox - add alias (buffer) for table */
  DEF VAR cReturn    AS CHAR NO-UNDO.
  DEF VAR ix         AS INT  NO-UNDO.
  DEF VAR iBuffNum   AS INT  NO-UNDO.
  DEF VAR thisEntry  AS CHAR NO-UNDO.
  DEF VAR thisTable  AS CHAR NO-UNDO.
  DEF VAR thisDb     AS CHAR NO-UNDO.
    
  IF eCurrentTable:SCREEN-VALUE IN FRAME dialog-1 NE "Temp-Tables":U THEN DO:
    RUN JBoxDSimpleSelect.w ("",?,OUTPUT cReturn).
    IF cReturn NE ? THEN DO:
      DO ix = 1 to {&CurLeft}:NUM-ITEMS:
        thisEntry = ENTRY(1, {&CurLeft}:ENTRY(ix), " ").
        IF NUM-ENTRIES(thisEntry,".":U) > 1 THEN
          ASSIGN thisDb    = ENTRY(1,thisEntry,".")
                 thisTable = ENTRY(2,thisEntry,".").
        ELSE ASSIGN thisDb    = eCurrentTable:SCREEN-VALUE
                    thisTable = thisEntry.
        IF thisTable = cReturn OR thisTable MATCHES "buf*_" + cReturn THEN
          iBuffNum = iBuffNum + 1.
      END.  /* DO i 1 to NUM-ITEMs on left */
      DO ix = 1 to {&CurRight}:NUM-ITEMS:
        thisEntry = ENTRY(1, {&CurRight}:ENTRY(ix), " ").
        IF NUM-ENTRIES(thisEntry,".":U) > 1 THEN
          ASSIGN thisDb    = ENTRY(1,thisEntry,".")
                 thisTable = ENTRY(2,thisEntry,".").
        ELSE ASSIGN thisDb    = eCurrentTable:SCREEN-VALUE
      ...
      cReturn = "buf" + STRING(iBuffNum) + "_" + cReturn.
      {&CurLeft}:ADD-LAST(cReturn).
      {&CurLeft}:SCROLL-TO-ITEM(cReturn).
    END.
  END.
END PROCEDURE.

PROCEDURE setSimpleSelectAttributes:    
  /* JukeBox - fill selection-list for with complete list of tables (for db) 
     callback procedure from JBoxDSimpleSelect.w */

  DEF INPUT PARAM ihSelList AS HANDLE NO-UNDO.
  DEF VAR db-recid AS RECID NO-UNDO.
  DEF VAR lParam   AS LOG   NO-UNDO.

  DO WITH FRAME dialog-1:
    ihSelList:DELIMITER = {&Sep1}.
    RUN adecomm/_getdbid.p (LDBNAME(eCurrentTable:SCREEN-VALUE),
                            OUTPUT db-recid).
    RUN adecomm/_tbllist.p (INPUT ihSelList,
                            INPUT NO,
                            INPUT db-recid,
                            INPUT "",
