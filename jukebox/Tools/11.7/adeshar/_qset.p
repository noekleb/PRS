...
Modified 21.may.16 by brynjar@chemistry.no
       - added code in setComboBoxQuery to support buffers (buf1_..)
----------------------------------------------------------------------------*/
...
PROCEDURE setComboBoxQuery:
  ...
  DEFINE VAR cTmp AS CHAR NO-UNDO. /* JukeBox */  
  ...
  /* Jukebox: */
  IF {&CurTable} MATCHES "buf*_*" THEN
    ASSIGN cTmp = {&CurTable}
           {&CurTable} = SUBSTR({&CurTable},6).
  /* End Jukebox */
  ...
  IF cTmp NE "" THEN {&CurTable} = cTmp. /* JukeBox */
