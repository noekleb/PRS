...
/*----------------------------------------------------------------------------
  ...
  Modified: 25.jan.14 by Brynjar for JukeBox
            If new JukeBox query or Browse apply freeform on GO of frame
            (the include is used in adeshar\_query.p) 
            remember to also compile adeuib\ide\_dialog_query.p 
            after compiling adeuib\_query.p
            (Since there are no changes to adeuib\_query.p you must specify SAVE INTO)
-----------------------------------------------------------------------------*/
&SCOPED-DEFINE SKP &IF "{&WINDOW-SYSTEM}" = "OSF/MOTIF" &THEN SKIP &ELSE &ENDIF


/* JukeBox, Brynjar (new trigger) 
ON CHOOSE OF bOk DO:
  IF DYNAMIC-FUNCTION("getNewJukeBoxObject") THEN DO:
