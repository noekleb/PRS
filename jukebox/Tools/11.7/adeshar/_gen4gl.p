...
Last Modified:
    ...
    Brynjar    26.feb.14 Mofified for jukebox:
                         database name has no place in browse definition (_FldNameList and _TblList) 
               10.sep.14 Changes for JukeBox added to 11.4 version of original file that 
                         probably was patced to be able to insert triggers in PDS AB          
---------------------------------------------------------------------------- */
...
RUN JBoxLoadLib.p ("JukeBox\abTmpDrawObjSuper.p").
RUN FindTablesAndFieldsInUse. /* Read from JukeBox view definitions in definition section */
DYNAMIC-FUNCTION("setFirstColumnEnabled") NO-ERROR. /* For any JukeBox browsers make sure that at least one column is enabled */
...
IF NOT CreatingSuper OR _P._TYPE = "SmartDataObject":U THEN DO:
  ...
  FOR EACH _U WHERE _U._WINDOW-HANDLE = _h_win 
    ...
    IF bJukeBox = ? OR NOT bJukeBox THEN
      ...
      IF i_count EQ 1 THEN 
        ...
        IF NOT AVAILABLE _TRG THEN DO:  /* If not a freeform query */
          ...
          FOR EACH _BC WHERE _BC._x-recid = RECID(_U):
            ...
            ASSIGN output-label    = IF _BC._LABEL NE _BC._DEF-LABEL OR bJukeBox
                                     THEN _BC._LABEL ELSE ?
                   output-colLabel = IF _BC._COL-LABEL NE _BC._DEF-COLLABEL OR bJukeBox
                                     THEN _BC._COL-LABEL ELSE ?
                   output-format   = IF _BC._FORMAT NE _BC._DEF-FORMAT OR bJukeBox
                                     THEN _BC._FORMAT ELSE ?
                   output-help     = IF _BC._HELP NE _BC._DEF-HELP OR bJukeBox
                                     THEN _BC._HELP ELSE ?
                   output-width    = IF _BC._WIDTH NE _BC._DEF-WIDTH OR bJukeBox
            ...
            IF bJukeBox THEN def-values = NO.
                                                                       
            /* Always put out the field name.  Only put out the Label and Format
               if they exist */
            PUT STREAM P_4GL UNFORMATTED 
              "     _FldNameList[" i (IF def-values THEN "]   = " ELSE "]   > ")
              IF _BC._DBNAME NE "_<CALC>" AND NOT bJukeBox THEN 
                _BC._DBNAME + ".":U + _BC._TABLE + ".":U + _BC._NAME
              ELSE IF _BC._DBNAME NE "_<CALC>" AND bJukeBox THEN 
                _BC._TABLE + ".":U + _BC._NAME /* no _dbname, no . - Brynjar for JukeBox */
              ELSE '"_<CALC>"':U SKIP.
            ...
            IF NOT def-values THEN DO:
              ...
              ELSE
                EXPORT STREAM P_4GL
                  (IF bJukeBox AND NUM-ENTRIES(_BC._DISP-NAME,".") > 2 THEN SUBSTR(_BC._DISP-NAME,INDEX(_BC._DISP-NAME,".") + 1) ELSE _BC._DISP-NAME) 
                  output-label output-format _BC._DATA-TYPE _BC._BGCOLOR
        ...
        ELSE DO:
          ...
          PUT STREAM P_4GL UNFORMATTED
            "     _Query            is " 
            (IF NOT _Q._OpenQury OR bJukeBox THEN "NOT ":U ELSE "":U)  "OPENED":U
            SKIP.
        END. /* Else DO */
