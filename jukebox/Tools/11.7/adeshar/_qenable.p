...
Date Created: 3/23/93
     ...
     Changed: 12/20/13 Brynjar:
              Added procedure ExpandFrame   
              03/30/16 Brynjar:
              Added popup-menu on table sel-list to fire procedure AddAlias.ip (in adeshar\_query.p)                  
----------------------------------------------------------------------------*/
...
DEF VAR hHaavardDialogFrame AS HANDLE NO-UNDO.
SUBSCRIBE TO "HaavardDialogFrame" ANYWHERE.
DEF VAR hTableMenu        AS HANDLE NO-UNDO.
DEF VAR hAddAliasMenuItem AS HANDLE NO-UNDO.
...
DO WITH FRAME dialog-1:
  CASE rsMain:SCREEN-VALUE:
    ...
    OTHERWISE DO:
      ...
      */
      RUN init_frame.
      RUN ExpandFrame. /* JukeBox, Brynjar */
    END.
  END CASE.
END.
  ...
  DO i = 1 to {&Max-function}:    /* Max-function is 5 - one for each of:
    ...
    CREATE SELECTION-LIST whLeft[i]
       ASSIGN WIDTH       = 26
	      ...
	      SCROLLBAR-H = TRUE
	      SENSITIVE   = TRUE
        SORT        = IF i = 1 THEN YES ELSE NO /* JukeBox */
    ...
    /* JukeBox */
    IF i = 1 THEN DO:
      CREATE MENU hTableMenu
             ASSIGN POPUP-ONLY = YES.
      whLeft[i]:POPUP-MENU = hTableMenu.
      CREATE MENU-ITEM hAddAliasMenuItem
             ASSIGN PARENT     = hTableMenu
                    LABEL      = "Add buffer (alias) for table"
                    NAME       = "addAlias"
                    TRIGGERS:
                      ON CHOOSE PERSISTENT RUN "AddAlias.ip".  
                    END TRIGGERS.
    END.
    /* End Jukebox */
...
PROCEDURE ExpandFrame:
  ...
  ASSIGN FRAME dialog-1:WIDTH-PIXELS  = FRAME dialog-1:WIDTH-PIXELS + 200
         FRAME dialog-1:HEIGHT-PIXELS = FRAME dialog-1:HEIGHT-PIXELS + 250
         .
  hFg = FRAME dialog-1:FIRST-CHILD.
  REPEAT WHILE VALID-HANDLE(hFg):
    hWidget = hFg:FIRST-CHILD.
    REPEAT WHILE VALID-HANDLE(hWidget):
      IF hWidget:NAME = "bHelp" THEN DO:
        hWidget:Y = hWidget:Y + 250.
        hWidget:X = FRAME dialog-1:WIDTH-PIXELS - hWidget:WIDTH-PIXELS - 10.
      ...
      IF CAN-DO("browse,editor",hWidget:TYPE) OR hWidget:NAME = "lBrowseLabel" THEN 
        ASSIGN hWidget:WIDTH-PIXELS = hWidget:WIDTH-PIXELS + 200.

      IF hWidget:Y < 20 THEN DO:
        IF hWidget:TYPE = "radio-set" THEN
          hWidget:WIDTH-PIXELS = hWidget:WIDTH-PIXELS - 50.
        ELSE IF hWidget:X < 30 THEN
          hWidget:WIDTH-PIXELS = FRAME dialog-1:WIDTH-PIXELS - 6 NO-ERROR.
        ELSE
          hWidget:X = FRAME dialog-1:WIDTH-PIXELS - 4.
      END.
...
PROCEDURE HaavardDialogFrame:
  DEF INPUT PARAM ihHaavardDialogFrame AS HANDLE NO-UNDO.

  hHaavardDialogFrame = ihHaavardDialogFrame.

  MESSAGE hHaavardDialogFrame
