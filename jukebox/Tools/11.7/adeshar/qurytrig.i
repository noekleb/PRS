...
/*----------------------------------------------------------------------------
  ...
  Modified: 17.09.14 by brynjar@chemistry.no for JukeBox
            A JukeBox query is always freeform (see ON GO OF Frame..)
            COMPILE adeshar\_query.p SAVE INTO C:\Progress\JukeBox\Tools\11.4.
            COMPILE adeuib\ide\_dialog_query.p SAVE.
            08.nov.17 by brynjar for JukeBox
                Captures new exit code from freeform ("exit") that is returned
                if the option to copy the query definition to clipboard
-----------------------------------------------------------------------------*/
...
ON GO OF FRAME DIALOG-1 DO:
  DEF VAR bJukeBoxObj AS LOG NO-UNDO.
  bJukeBoxObj = DYNAMIC-FUNCTION("getNewJukeBoxObject") NO-ERROR.
  IF NOT ERROR-STATUS:ERROR AND bJukeBoxObj 
     AND DYNAMIC-FUNCTION("getLastToolChoose") = "jb-query" THEN DO:
    DYNAMIC-FUNCTION("setToolChoose","").
    APPLY "choose" TO b_FreeFormQ.
    RETURN NO-APPLY.
  END.
