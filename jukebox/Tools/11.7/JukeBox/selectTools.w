...
cmbSubMenu btnFilter btnFilter1 btnFilter2 btnFilter3 btnFilter4 btnFilter5 ~
fiMenu fiMenu1 fiMenu2 fiMenu3 fiMenu4 fiMenu5 tbAddWinMenu tbAdd1 tbAdd2 ~
tbAdd3 tbAdd4 tbAdd5 brwTool btnSplitBarY edCode edTip fiToolbarName ~
fiBtnWidth btnSample cmbButtonTemplate cmbApplyToQry fiBtnHeight ~
cmbApplyToFm rsBorder btnOk btnCancel tbFlat tbCreateProcs fiCodeLabel ~
fiTipLabel 
&Scoped-Define DISPLAYED-OBJECTS cmbSubMenu fiMenu fiMenu1 fiMenu2 fiMenu3 ~
fiMenu4 fiMenu5 tbAddWinMenu tbAdd1 tbAdd2 tbAdd3 tbAdd4 tbAdd5 edCode ~
edTip fiToolbarName fiBtnWidth cmbButtonTemplate cmbApplyToQry fiBtnHeight ~
cmbApplyToFm rsBorder tbFlat tbCreateProcs fiCodeLabel fiTipLabel 
...
DEFINE SUB-MENU m_File 
       ...
       MENU-ITEM m_Load_toolbar_definition LABEL "Load toolbar definition from clipboard".
...
DEFINE BUTTON btnFilter 
     ...
     LABEL "Filter" 
     SIZE 4.4 BY 1.1 TOOLTIP "View only tools and sub-menus parented to this sub-menu".

DEFINE BUTTON btnFilter1 
     IMAGE-UP FILE "adeicon/filter.bmp":U
     LABEL "Filter" 
     SIZE 4.4 BY 1.1 TOOLTIP "View only tools and sub-menus parented to this sub-menu".

DEFINE BUTTON btnFilter2 
     IMAGE-UP FILE "adeicon/filter.bmp":U
     LABEL "Filter" 
     SIZE 4.4 BY 1.1 TOOLTIP "View only tools and sub-menus parented to this sub-menu".

DEFINE BUTTON btnFilter3 
     IMAGE-UP FILE "adeicon/filter.bmp":U
     LABEL "Filter" 
     SIZE 4.4 BY 1.1 TOOLTIP "View only tools and sub-menus parented to this sub-menu".

DEFINE BUTTON btnFilter4 
     IMAGE-UP FILE "adeicon/filter.bmp":U
     LABEL "Filter" 
     SIZE 4.4 BY 1.1 TOOLTIP "View only tools and sub-menus parented to this sub-menu".

DEFINE BUTTON btnFilter5 
     IMAGE-UP FILE "adeicon/filter.bmp":U
     LABEL "Filter" 
     SIZE 4.4 BY 1.1 TOOLTIP "View only tools and sub-menus parented to this sub-menu".
...
DEFINE BUTTON btnSample 
     LABEL "" 
     SIZE 9 BY 1.38 TOOLTIP "Press to change image file".
...
DEFINE VARIABLE cmbButtonTemplate AS CHARACTER FORMAT "X(256)":U 
     LABEL "Use size template" 
     VIEW-AS COMBO-BOX INNER-LINES 5
...
DEFINE VARIABLE fiBtnHeight AS DECIMAL FORMAT ">9.99":U INITIAL 1.1 
     LABEL "Height" 
     VIEW-AS FILL-IN 
     SIZE 9 BY 1 NO-UNDO.

DEFINE VARIABLE fiBtnWidth AS DECIMAL FORMAT ">9.99":U INITIAL 4.6 
     LABEL "Button width" 
     VIEW-AS FILL-IN 
     SIZE 9 BY 1 NO-UNDO.
...
DEFINE VARIABLE fiMenu AS CHARACTER FORMAT "X(256)":U 
     VIEW-AS FILL-IN 
     SIZE 11.8 BY 1 TOOLTIP "Label for menu or sub-menu" NO-UNDO.

DEFINE VARIABLE fiMenu1 AS CHARACTER FORMAT "X(256)":U INITIAL "<label>" 
     VIEW-AS FILL-IN 
     SIZE 11.8 BY 1 TOOLTIP "Label for sub-menu" NO-UNDO.

DEFINE VARIABLE fiMenu2 AS CHARACTER FORMAT "X(256)":U INITIAL "<label>" 
     VIEW-AS FILL-IN 
     SIZE 11.8 BY 1 TOOLTIP "Label for sub-menu" NO-UNDO.

DEFINE VARIABLE fiMenu3 AS CHARACTER FORMAT "X(256)":U INITIAL "<label>" 
     VIEW-AS FILL-IN 
     SIZE 11.8 BY 1 TOOLTIP "Label for sub-menu" NO-UNDO.

DEFINE VARIABLE fiMenu4 AS CHARACTER FORMAT "X(256)":U INITIAL "<label>" 
     VIEW-AS FILL-IN 
     SIZE 11.8 BY 1 TOOLTIP "Label for sub-menu" NO-UNDO.

DEFINE VARIABLE fiMenu5 AS CHARACTER FORMAT "X(256)":U INITIAL "<label>" 
     VIEW-AS FILL-IN 
     SIZE 11.8 BY 1 TOOLTIP "Label for sub-menu" NO-UNDO.
...
DEFINE VARIABLE rsBorder AS CHARACTER INITIAL "maxborder" 
     VIEW-AS RADIO-SET HORIZONTAL
     RADIO-BUTTONS 
          "Hide border rectangle (at run-time)", "hide",
"Border", "border",
"Max border", "maxborder"
     SIZE 72 BY 1.19 NO-UNDO.

DEFINE VARIABLE tbAdd1 AS LOGICAL INITIAL no 
     LABEL ">" 
     VIEW-AS TOGGLE-BOX
     SIZE 4.6 BY .81 TOOLTIP "Select to add next sub-menu for menu-bar" NO-UNDO.

DEFINE VARIABLE tbAdd2 AS LOGICAL INITIAL no 
     LABEL ">" 
     VIEW-AS TOGGLE-BOX
     SIZE 4.6 BY .81 TOOLTIP "Select to add next sub-menu for menu-bar" NO-UNDO.

DEFINE VARIABLE tbAdd3 AS LOGICAL INITIAL no 
     LABEL ">" 
     VIEW-AS TOGGLE-BOX
     SIZE 4.6 BY .81 TOOLTIP "Select to add next sub-menu for menu-bar" NO-UNDO.

DEFINE VARIABLE tbAdd4 AS LOGICAL INITIAL no 
     LABEL ">" 
     VIEW-AS TOGGLE-BOX
     SIZE 4.6 BY .81 TOOLTIP "Select to add next sub-menu for menu-bar" NO-UNDO.

DEFINE VARIABLE tbAdd5 AS LOGICAL INITIAL no 
     LABEL ">" 
     VIEW-AS TOGGLE-BOX
     SIZE 4.6 BY .81 TOOLTIP "Select to add next sub-menu for menu-bar" NO-UNDO.

DEFINE VARIABLE tbAddWinMenu AS LOGICAL INITIAL no 
...
DEFINE VARIABLE tbFlat AS LOGICAL INITIAL no 
     LABEL "Flat buttons (also no-focus)" 
     VIEW-AS TOGGLE-BOX
...
ENABLE cLabel cTooltip fWidth cViewAs cParentMenu
       bAlwaysEnable HELP "Check to enable button regardless of record availability"
       bToggle HELP "Check to make a toggle menu-item"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS MULTIPLE SIZE 155.2 BY 12.29 ROW-HEIGHT-CHARS .8 FIT-LAST-COLUMN.
...
DEFINE FRAME DEFAULT-FRAME
     ...
     btnFilter AT ROW 2.48 COL 36 WIDGET-ID 86
     btnFilter1 AT ROW 2.48 COL 58.4 WIDGET-ID 88
     btnFilter2 AT ROW 2.48 COL 80.6 WIDGET-ID 90
     btnFilter3 AT ROW 2.48 COL 102.6 WIDGET-ID 92
     btnFilter4 AT ROW 2.48 COL 124.8 WIDGET-ID 98
     btnFilter5 AT ROW 2.48 COL 147.4 WIDGET-ID 104
     fiMenu AT ROW 2.52 COL 22.2 COLON-ALIGNED NO-LABEL
     fiMenu1 AT ROW 2.52 COL 44.6 COLON-ALIGNED NO-LABEL
     fiMenu2 AT ROW 2.52 COL 66.8 COLON-ALIGNED NO-LABEL
     fiMenu3 AT ROW 2.52 COL 88.8 COLON-ALIGNED NO-LABEL
     fiMenu4 AT ROW 2.52 COL 111 COLON-ALIGNED NO-LABEL
     fiMenu5 AT ROW 2.52 COL 133.6 COLON-ALIGNED NO-LABEL
     tbAddWinMenu AT ROW 2.62 COL 2.4 WIDGET-ID 18
     tbAdd1 AT ROW 2.62 COL 42 WIDGET-ID 80
     tbAdd2 AT ROW 2.62 COL 64.2 WIDGET-ID 84
     tbAdd3 AT ROW 2.62 COL 86.2 WIDGET-ID 96
     tbAdd4 AT ROW 2.62 COL 108.4 WIDGET-ID 102
     tbAdd5 AT ROW 2.62 COL 131 WIDGET-ID 108
     brwTool AT ROW 3.71 COL 1.8 WIDGET-ID 200
     ...
     fiBtnWidth AT ROW 21.95 COL 61 COLON-ALIGNED
     btnSample AT ROW 22 COL 35.2 WIDGET-ID 48
     cmbButtonTemplate AT ROW 22.05 COL 18.4 COLON-ALIGNED WIDGET-ID 50
     cmbApplyToQry AT ROW 22.81 COL 114 COLON-ALIGNED WIDGET-ID 4
     fiBtnHeight AT ROW 23 COL 61 COLON-ALIGNED
     cmbApplyToFm AT ROW 23.91 COL 114 COLON-ALIGNED WIDGET-ID 10
     rsBorder AT ROW 24.24 COL 2.8 NO-LABEL WIDGET-ID 24
     btnOk AT ROW 25.52 COL 126.8 WIDGET-ID 6
     btnCancel AT ROW 25.52 COL 142 WIDGET-ID 8
     tbFlat AT ROW 25.71 COL 55 WIDGET-ID 22
...
/* SETTINGS FOR FRAME DEFAULT-FRAME
   FRAME-NAME                                                           */
/* BROWSE-TAB brwTool tbAdd5 DEFAULT-FRAME */
ASSIGN 
...
ASSIGN 
       fiMenu:HIDDEN IN FRAME DEFAULT-FRAME           = TRUE.

ASSIGN 
       fiMenu1:HIDDEN IN FRAME DEFAULT-FRAME           = TRUE.

ASSIGN 
       fiMenu2:HIDDEN IN FRAME DEFAULT-FRAME           = TRUE.

ASSIGN 
       fiMenu3:HIDDEN IN FRAME DEFAULT-FRAME           = TRUE.

ASSIGN 
       fiMenu4:HIDDEN IN FRAME DEFAULT-FRAME           = TRUE.

ASSIGN 
       fiMenu5:HIDDEN IN FRAME DEFAULT-FRAME           = TRUE.

ASSIGN 
       tbAdd1:HIDDEN IN FRAME DEFAULT-FRAME           = TRUE.

ASSIGN 
       tbAdd2:HIDDEN IN FRAME DEFAULT-FRAME           = TRUE.

ASSIGN 
       tbAdd3:HIDDEN IN FRAME DEFAULT-FRAME           = TRUE.

ASSIGN 
       tbAdd4:HIDDEN IN FRAME DEFAULT-FRAME           = TRUE.

ASSIGN 
       tbAdd5:HIDDEN IN FRAME DEFAULT-FRAME           = TRUE.
...
ON ROW-LEAVE OF brwTool IN FRAME DEFAULT-FRAME
DO:
/*   MESSAGE hViewAsCol:SCREEN-VALUE SKIP     */
/*           hParentMenuCol:SCREEN-VALUE SKIP */
/*           ttJBoxTool.cParentMenu           */
/*       VIEW-AS ALERT-BOX INFO BUTTONS OK.   */

  IF hViewAsCol:SCREEN-VALUE NE "menu-item" THEN
    ASSIGN ttJBoxTool.bToggle = NO
           .
  ELSE IF hViewAsCol:SCREEN-VALUE = "button" THEN DO:
    ASSIGN ttJBoxTool.cParentMenu = ""
           hParentMenuCol:SCREEN-VALUE = "" 
           .
  END.
END.
...
DO:
  IF AVAIL ttJboxTool THEN DO:
    ...
    btnSample:LOAD-IMAGE(SEARCH(ttJBoxTool.cImage)).
  END.
  ELSE
...
DO:
  ...
  IF LENGTH(cTool + fiToolbarName:SCREEN-VALUE) > 32 THEN DO:
    MESSAGE "Name of toobar + name of tool cannot exceed 32 characters"
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
    RETURN.
  END.

  IF cTool NE "" THEN DO:
    IF cAddToParentMenu = ? THEN cAddToParentMenu = "".
    ...
    ELSE DO:
      ...
      ELSE DO:
        ...
        ASSIGN ttJBoxTool.iSeq        = iSeq + 10
               ...
               ttJBoxTool.cViewAs     = IF cAddToParentMenu NE "" THEN "Button and Menu-item" ELSE "Button"
...
DO:
  ...
  IF fiMenu:SCREEN-VALUE = "" THEN DO:
    MESSAGE "No window menu (root menu node) specified"
    ...
    DO ix = 1 TO NUM-ENTRIES(cIdList,"|"):
      ...
      IF AVAIL ttJBoxTool THEN DO:
        ...
        ASSIGN ttJBoxTool.iSeq = iInsertAfter + ix * 10
               ...
               ttJBoxTool.cViewTool = ttJBoxTool.cTool.
      END.
    END.
    RUN OpenQuery.
  END.
...
DO:
  IF SELF:PRIVATE-DATA = ? THEN DO:
    ASSIGN cMenuParentFilter = fiMenu:SCREEN-VALUE
           SELF:PRIVATE-DATA = "checked"
           .
    SELF:LOAD-IMAGE("gif\filterc.gif").

    FOR EACH ttJBoxTool WHERE ttJBoxTool.hMenuFilter NE ?:
  ...
  ELSE DO:
    ASSIGN cMenuParentFilter = ""
           SELF:PRIVATE-DATA = ?
           .
    SELF:LOAD-IMAGE("gif\filter.gif").
  END.
...
&Scoped-define SELF-NAME btnFilter1
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btnFilter1 C-Win
ON CHOOSE OF btnFilter1 IN FRAME DEFAULT-FRAME /* Filter */
DO:
  setMenuFilter(SELF,fiMenu1:HANDLE).
END.
...
&Scoped-define SELF-NAME btnFilter2
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btnFilter2 C-Win
ON CHOOSE OF btnFilter2 IN FRAME DEFAULT-FRAME /* Filter */
DO:
  setMenuFilter(SELF,fiMenu2:HANDLE).
END.
...
&Scoped-define SELF-NAME btnFilter3
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btnFilter3 C-Win
ON CHOOSE OF btnFilter3 IN FRAME DEFAULT-FRAME /* Filter */
DO:
  setMenuFilter(SELF,fiMenu3:HANDLE).
END.
...
&Scoped-define SELF-NAME btnFilter4
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btnFilter4 C-Win
ON CHOOSE OF btnFilter4 IN FRAME DEFAULT-FRAME /* Filter */
DO:
  setMenuFilter(SELF,fiMenu4:HANDLE).
END.
...
&Scoped-define SELF-NAME btnFilter5
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btnFilter5 C-Win
ON CHOOSE OF btnFilter5 IN FRAME DEFAULT-FRAME /* Filter */
DO:
  setMenuFilter(SELF,fiMenu5:HANDLE).
END.
...
DO:
  IF NOT CAN-FIND(FIRST ttJBoxTool WHERE ttJBoxTool.bSelect) THEN DO:
    ...
    RETURN NO-APPLY.
  END.
  IF NOT fiMenu:HIDDEN AND fiMenu:SCREEN-VALUE = "" THEN DO:
    MESSAGE "Missing label for primary window menu"
        VIEW-AS ALERT-BOX INFO BUTTONS OK.  
    RETURN NO-APPLY.
  END.
  ...
  IF NOT fiToolbarName:READ-ONLY THEN DO:
    IF fiToolbarName:SCREEN-VALUE = "" OR fiToolbarName:SCREEN-VALUE = ? THEN DO:
      MESSAGE "Toolbar must have a name"
          VIEW-AS ALERT-BOX INFO BUTTONS OK.
      RETURN NO-APPLY.
    END.
  ...
  ASSIGN
         iocToolbarDef  = iocToolbarDef + CHR(1) 
                     + (IF NOT fiMenu:HIDDEN THEN fiMenu:SCREEN-VALUE ELSE "") + CHR(1)
                     + rsBorder:SCREEN-VALUE
         obFlat      = tbFlat:CHECKED
         ocQryObject = cmbApplyToQry:SCREEN-VALUE
         ocFmObject  = cmbApplyToFm:SCREEN-VALUE
         ofBtnWidth  = DECIMAL(fiBtnWidth:SCREEN-VALUE)
         ofBtnHeight = DECIMAL(fiBtnHeight:SCREEN-VALUE)
         obOk        = YES.
...
&Scoped-define SELF-NAME btnSample
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btnSample C-Win
ON CHOOSE OF btnSample IN FRAME DEFAULT-FRAME
DO:
  DEF VAR bOk AS LOG NO-UNDO.
...
/*   IF ttJBoxTool.bExist THEN DO:                            */
/*     MESSAGE "Cannot change image of existing button here"  */
/*             VIEW-AS ALERT-BOX ERROR.                       */
/*     RETURN NO-APPLY.                                       */
/*   END.                                                     */

/*   IF CAN-DO(cToolList,ttJBoxTool.cTool) THEN DO:                                                                */
/*     MESSAGE "The button has a standard image. Sure you want to change it?" SKIP                                 */
/*             "(To change the standard set the attribute BtnImg_" + ttJBoxTool.cTool + " for the SESSION handle)" */
...
/*   END.                                                                                                          */
  ...
  SYSTEM-DIALOG GET-FILE cFileName 
                ...
                MUST-EXIST
                UPDATE bOk.
  cFileName = ValidateImage(cFileName).
  IF cFileName NE "" THEN DO:
    ASSIGN ttJBoxTool.cImage = cFileName
           ttJBoxTool.fWidth = 0.
...
&Scoped-define SELF-NAME cmbButtonTemplate
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL cmbButtonTemplate C-Win
ON VALUE-CHANGED OF cmbButtonTemplate IN FRAME DEFAULT-FRAME /* Use size template */
DO:
  DEF VAR cSize AS CHAR NO-UNDO.
  cSize = SELF:SCREEN-VALUE.
  IF NUM-ENTRIES(cSize,"x") = 2 THEN DO:
    ASSIGN fiBtnWidth:SCREEN-VALUE  = STRING(INT(ENTRY(1,cSize,"x")) / 100)  
           fiBtnHeight:SCREEN-VALUE = STRING(INT(ENTRY(2,cSize,"x")) / 100)
           .
    APPLY "leave" TO fiBtnWidth.
    APPLY "leave" TO fiBtnHeight.
  END.
END.
...
DO:
  IF btnFilter:PRIVATE-DATA NE ? THEN DO:
    btnFilter:PRIVATE-DATA = ?.
    btnFilter:LOAD-IMAGE("gif\filter.gif").
  END.
  FOR EACH ttJBoxTool 
...
&Scoped-define SELF-NAME fiBtnHeight
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fiBtnHeight C-Win
ON LEAVE OF fiBtnHeight IN FRAME DEFAULT-FRAME /* Height */
DO:
  IF SELF:MODIFIED THEN DO:
    ASSIGN fiBtnHeight.  
    btnSample:HEIGHT-CHARS = fiBtnHeight NO-ERROR.
  END.
END.
...
&Scoped-define SELF-NAME fiBtnWidth
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fiBtnWidth C-Win
ON LEAVE OF fiBtnWidth IN FRAME DEFAULT-FRAME /* Button width */
DO:
  IF SELF:MODIFIED THEN DO:
    ASSIGN fiBtnWidth.
    btnSample:WIDTH-CHARS = fiBtnWidth NO-ERROR.
  END.
END.
...
&Scoped-define SELF-NAME fiMenu
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fiMenu C-Win
ON ENTRY OF fiMenu IN FRAME DEFAULT-FRAME
DO:
  cCurrRootMenu = SELF:SCREEN-VALUE.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fiMenu C-Win
ON LEAVE OF fiMenu IN FRAME DEFAULT-FRAME
DO:
  IF SELF:SCREEN-VALUE = "" THEN DO:
    FIND FIRST ttJBoxTool
         WHERE ttJBoxTool.cParentMenu NE ""
         NO-ERROR.
    IF AVAIL ttJBoxTool THEN DO:
      MESSAGE "Menu root node must be specified when sub-menus are defined"
              VIEW-AS ALERT-BOX ERROR.
      RETURN NO-APPLY.
    END.
    ELSE DO:
      tbAddWinMenu:CHECKED = NO.
      APPLY "VALUE-CHANGED" TO tbAddWinMenu.
    END.
  END.
  ELSE IF SELF:SCREEN-VALUE NE cCurrRootMenu THEN DO:
    FOR EACH ttJBoxTool
        WHERE ttJBoxTool.cParentMenu = cCurrRootMenu:
      ttJBoxTool.cParentMenu = SELF:SCREEN-VALUE.
    END.
    setMenuCombos("").   
    hBrowse:REFRESH().
  END.
...
&Scoped-define SELF-NAME fiMenu1
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fiMenu1 C-Win
ON LEAVE OF fiMenu1 IN FRAME DEFAULT-FRAME
DO:
  leaveFiMenu(SELF,tbAdd1:HANDLE,btnFilter1:HANDLE).
END.
...
&Scoped-define SELF-NAME fiMenu2
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fiMenu2 C-Win
ON LEAVE OF fiMenu2 IN FRAME DEFAULT-FRAME
DO:
  leaveFiMenu(SELF,tbAdd2:HANDLE,btnFilter2:HANDLE).
END.
...
&Scoped-define SELF-NAME fiMenu3
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fiMenu3 C-Win
ON LEAVE OF fiMenu3 IN FRAME DEFAULT-FRAME
DO:
  leaveFiMenu(SELF,tbAdd3:HANDLE,btnFilter3:HANDLE).
END.
...
&Scoped-define SELF-NAME fiMenu4
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fiMenu4 C-Win
ON LEAVE OF fiMenu4 IN FRAME DEFAULT-FRAME
DO:
  leaveFiMenu(SELF,tbAdd4:HANDLE,btnFilter4:HANDLE).
END.
...
&Scoped-define SELF-NAME fiMenu5
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fiMenu5 C-Win
ON LEAVE OF fiMenu5 IN FRAME DEFAULT-FRAME
DO:
  leaveFiMenu(SELF,tbAdd5:HANDLE,btnFilter5:HANDLE).
END.
...
&Scoped-define SELF-NAME m_Load_toolbar_definition
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL m_Load_toolbar_definition C-Win
ON CHOOSE OF MENU-ITEM m_Load_toolbar_definition /* Load toolbar definition from clipboard */
DO:
  iocToolbarDef = CLIPBOARD:VALUE.

  creToolDef(ENTRY(1,iocToolbarDef,CHR(1))).

  IF NUM-ENTRIES(iocToolbarDef,CHR(1)) > 1 THEN DO WITH FRAME {&FRAME-NAME}:
    fiMenu:SCREEN-VALUE = ENTRY(2,iocToolbarDef,CHR(1)).
    IF NUM-ENTRIES(iocToolbarDef,CHR(1)) > 2 THEN 
      rsBorder = ENTRY(3,iocToolbarDef,CHR(1)).

    addMenusFromToolDef().

    IF fiMenu:SCREEN-VALUE NE "" THEN
      tbAddWinMenu:CHECKED = YES.
...
&Scoped-define SELF-NAME tbAdd1
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tbAdd1 C-Win
ON VALUE-CHANGED OF tbAdd1 IN FRAME DEFAULT-FRAME /* > */
DO:
  IF NOT SELF:CHECKED THEN unCheckTbAdd(SELF,fiMenu1:HANDLE,btnFilter1:HANDLE).
  ELSE DO:
    fiMenu1:HIDDEN = NO.
    btnFilter1:HIDDEN = NO.
    tbAdd2:HIDDEN = NO.
    APPLY "entry" TO fiMenu1.
    RETURN NO-APPLY.
  END.
END.
...
&Scoped-define SELF-NAME tbAdd2
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tbAdd2 C-Win
ON VALUE-CHANGED OF tbAdd2 IN FRAME DEFAULT-FRAME /* > */
DO:
  IF NOT SELF:CHECKED THEN unCheckTbAdd(SELF,fiMenu2:HANDLE,btnFilter2:HANDLE).
  ELSE DO:
    fiMenu2:HIDDEN = NO.
    btnFilter2:HIDDEN = NO.
    tbAdd3:HIDDEN = NO. 
    APPLY "entry" TO fiMenu2.
    RETURN NO-APPLY.
  END.
END.
...
&Scoped-define SELF-NAME tbAdd3
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tbAdd3 C-Win
ON VALUE-CHANGED OF tbAdd3 IN FRAME DEFAULT-FRAME /* > */
DO:
  IF NOT SELF:CHECKED THEN unCheckTbAdd(SELF,fiMenu3:HANDLE,btnFilter3:HANDLE).
  ELSE DO:
    fiMenu3:HIDDEN = NO.
    btnFilter3:HIDDEN = NO.
    tbAdd4:HIDDEN = NO.
    APPLY "entry" TO fiMenu3.
    RETURN NO-APPLY.
  END.
END.
...
&Scoped-define SELF-NAME tbAdd4
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tbAdd4 C-Win
ON VALUE-CHANGED OF tbAdd4 IN FRAME DEFAULT-FRAME /* > */
DO:
  IF NOT SELF:CHECKED THEN unCheckTbAdd(SELF,fiMenu4:HANDLE,btnFilter4:HANDLE).
  ELSE DO:
    fiMenu4:HIDDEN = NO.
    btnFilter4:HIDDEN = NO.
    tbAdd5:HIDDEN = NO. 
    APPLY "entry" TO fiMenu4.
    RETURN NO-APPLY.
  END.
END.
...
&Scoped-define SELF-NAME tbAdd5
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tbAdd5 C-Win
ON VALUE-CHANGED OF tbAdd5 IN FRAME DEFAULT-FRAME /* > */
DO:
  IF NOT SELF:CHECKED THEN unCheckTbAdd(SELF,fiMenu5:HANDLE,btnFilter5:HANDLE).
  ELSE DO:
    fiMenu5:HIDDEN = NO.
    btnFilter5:HIDDEN = NO.
/*     tbAdd6:HIDDEN = NO. */
    APPLY "entry" TO fiMenu5.
    RETURN NO-APPLY.
  END.
END.
...
&Scoped-define SELF-NAME tbAddWinMenu
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tbAddWinMenu C-Win
ON VALUE-CHANGED OF tbAddWinMenu IN FRAME DEFAULT-FRAME /* Add window menu: */
DO:
  ...
  IF NOT SELF:CHECKED THEN DO:
    FIND FIRST ttJBoxTool
         WHERE (CAN-DO("menu,sub-menu",ttJBoxTool.cType) OR cParentMenu NE "")
           AND NOT ttJBoxTool.bDeleted
         NO-ERROR.
    IF AVAIL ttJBoxTool THEN 
      MESSAGE "Removing the menu root node will cause all sub-menus and parent menu references to be deleted" SKIP
              "Continue?"
              VIEW-AS ALERT-BOX WARNING BUTTONS OK-CANCEL UPDATE bOk.
    IF bOk THEN DO:
      FOR EACH ttJBoxTool: 
        IF CAN-DO("menu,sub-menu",ttJBoxTool.cType) THEN DO:
          ASSIGN ttJBoxTool.bDeleted = YES
                 ttJboxTool.bSelect = NO
                 ttJBoxTool.cParentMenu = "".
          IF ttJBoxTool.cType = "menu" AND VALID-HANDLE(ttJBoxTool.hMenuFillIn) THEN DO:
            ASSIGN ttJBoxTool.hMenuFillIn:PRIVATE-DATA = ""
                   ttJBoxTool.hMenuFillIn:SCREEN-VALUE = "<label>"
                   ttJBoxTool.hMenuFillIn:HIDDEN = YES
                   ttJBoxTool.hMenuToggle:CHECKED = NO
                   ttJBoxTool.hMenuToggle:HIDDEN = YES /* NOT ttJBoxTool.hMenuToggle:NAME MATCHES "*1" */
                   ttJBoxTool.hMenuFillIn = ?
                   .
            IF ttJBoxTool.hMenuFilter:PRIVATE-DATA = "checked" THEN RUN OpenQuery.

            ASSIGN ttJBoxTool.hMenuFilter:PRIVATE-DATA = ""
                   ttJBoxTool.hMenuFilter:HIDDEN = YES.
          END.
        END.
        ELSE
          ASSIGN ttJBoxTool.cViewAs = "Button and Menu-item"
                 ttJBoxTool.cParentMenu = ""
                 .
      END.
      ASSIGN tbAdd1:HIDDEN = YES
             tbAdd2:HIDDEN = YES
             tbAdd3:HIDDEN = YES
             tbAdd4:HIDDEN = YES
             tbAdd5:HIDDEN = YES
  ...
  ASSIGN fiMenu:HIDDEN = NOT SELF:CHECKED
         tbAdd1:HIDDEN = NOT SELF:CHECKED
         btnFilter:HIDDEN = NOT SELF:CHECKED
         btnAddSubMenu:HIDDEN = NOT SELF:CHECKED
         hViewAsCol:VISIBLE = SELF:CHECKED
         hParentMenuCol:VISIBLE = SELF:CHECKED
         hToggleCol:VISIBLE = SELF:CHECKED
         cMenuParentFilter = ""
         .

  IF SELF:CHECKED AND fiMenu:SCREEN-VALUE = "" THEN
    fiMenu:SCREEN-VALUE = IF DYNAMIC-FUNCTION("Scandinavian") THEN "Fil" ELSE "File".
  ...
  IF SELF:CHECKED THEN DO:
    ...
    FOR EACH ttJBoxTool WHERE ttJBoxTool.cType NE "menu": 
      IF ttJBoxTool.cViewAs = "" THEN
        ttJBoxTool.cViewAs = "Button and Menu-item".
      IF ttJBoxTool.cViewAs MATCHES "*menu*" AND ttJBoxTool.cParentMenu = "" THEN
        ttJBoxTool.cParentMenu = fiMenu:SCREEN-VALUE.
    END.    
  END.
...
ON 'value-changed' OF hViewAsCol DO:
  MESSAGE SELF:SCREEN-VALUE
      VIEW-AS ALERT-BOX INFO BUTTONS OK.
  IF SELF:SCREEN-VALUE NE "menu-item" THEN
    ASSIGN ttJBoxTool.bToggle = NO
           .
  ELSE IF SELF:SCREEN-VALUE = "button" THEN
    ASSIGN ttJBoxTool.cParentMenu = ""
           hParentMenuCol:SCREEN-VALUE = ?
           .
  RETURN.
...
------------------------------------------------------------------------------*/
  DISPLAY cmbSubMenu fiMenu fiMenu1 fiMenu2 fiMenu3 fiMenu4 fiMenu5 tbAddWinMenu 
          tbAdd1 tbAdd2 tbAdd3 tbAdd4 tbAdd5 edCode edTip fiToolbarName 
          fiBtnWidth cmbButtonTemplate cmbApplyToQry fiBtnHeight cmbApplyToFm 
          rsBorder tbFlat tbCreateProcs fiCodeLabel fiTipLabel 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE btnAddTool btnToolGroup btnAddCustomTool btnAddRule btnAddSubMenu 
         btnDelete btnMoveUp btnMoveDown btnSelectCode cmbSubMenu btnFilter 
         btnFilter1 btnFilter2 btnFilter3 btnFilter4 btnFilter5 fiMenu fiMenu1 
         fiMenu2 fiMenu3 fiMenu4 fiMenu5 tbAddWinMenu tbAdd1 tbAdd2 tbAdd3 
         tbAdd4 tbAdd5 brwTool btnSplitBarY edCode edTip fiToolbarName 
         fiBtnWidth btnSample cmbButtonTemplate cmbApplyToQry fiBtnHeight 
         cmbApplyToFm rsBorder btnOk btnCancel tbFlat tbCreateProcs fiCodeLabel 
         fiTipLabel 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  ASSIGN cBrowseList = "new,edit,copy,undo,save,delete,refresh,filter,browseConfig,excel"
         ...
         fiMenu4:HIDDEN = YES
         fiMenu5:HIDDEN = YES
         btnFilter1:HIDDEN = YES
         btnFilter2:HIDDEN = YES
         btnFilter3:HIDDEN = YES
         btnFilter4:HIDDEN = YES
         btnFilter5:HIDDEN = YES
  ...
  ELSE DO:
    ...
    DO ix = 1 TO MIN(NUM-ENTRIES(cFirstTblList),NUM-ENTRIES(cTbObjects)):
      IF NOT ENTRY(ix,cTbObjects) MATCHES "*" + ENTRY(ix,cFirstTblList) + "*" THEN DO:
        fiToolbarName:SCREEN-VALUE = "tb" + ENTRY(ix,cFirstTblList).
    ...
    IF fiToolbarName:SCREEN-VALUE = "" AND cFirstTblList NE "" THEN 
      ASSIGN fiToolbarName:SCREEN-VALUE = "tb" + ENTRY(1,cFirstTblList)
    ...
    ELSE IF ttToolDef.cType = "tool" THEN DO:
      ...
      IF AVAIL ttJBoxTool THEN
        ASSIGN ttJBoxTool.iSeq = ttToolDef.iSeq
               ttJBoxTool.cParentMenu = ttToolDef.cParentMenu
               ttJBoxTool.cViewAs     = ttToolDef.cViewAs
               ttJBoxTool.cViewTool   = ttToolDef.cTool
               ttJBoxTool.bAlwaysEnable = ttToolDef.bAlwaysEnable
               ttJBoxTool.bSelect = YES
               .
      ELSE DO:
...
DO ix = 1 TO NUM-ENTRIES(icToolList):
  cTool = TRIM(ENTRY(1,ENTRY(1,ENTRY(ix,icToolList),";"),"¤"),CHR(10)).
...
IF NOT fiMenu:HIDDEN IN FRAME {&FRAME-NAME} THEN
  DO WITH FRAME {&FRAME-NAME}:
    cMenuList = fiMenu:SCREEN-VALUE. 
    IF ibPairs THEN
      cMenuList = cMenuList + "|" + fiMenu:SCREEN-VALUE. 
    FOR EACH ttJBoxTool 
...
IF icParentMenu = ? THEN icParentMenu = "".
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  FOR EACH b_U 
    ...
    IF ofStartCol = 0 THEN 
      ASSIGN ofStartCol               = b_L._COL
             ofStartRow               = b_L._ROW
             fiBtnWidth:SCREEN-VALUE  = STRING(b_L._WIDTH)
             fiBtnHeight:SCREEN-VALUE = STRING(b_L._HEIGHT)
             .
    ASSIGN cBtnTool = SUBSTR(b_U._NAME,1,INDEX(b_U._NAME,iocToolbarName) - 2)
           ...
           .
    IF cmbButtonTemplate:LOOKUP(cBtnSize) > 0 THEN
      cmbButtonTemplate:SCREEN-VALUE = cBtnSize.
    FIND FIRST ttJBoxTool
  ...
  IF NUM-ENTRIES(icToolbarDef,CHR(1)) > 1 THEN DO:
    fiMenu:SCREEN-VALUE = ENTRY(2,icToolbarDef,CHR(1)).
    IF NUM-ENTRIES(icToolbarDef,CHR(1)) > 2 THEN 
      rsBorder:SCREEN-VALUE = ENTRY(3,icToolbarDef,CHR(1)) NO-ERROR.
  ...
  ASSIGN /* tbAddWinMenu:HIDDEN = YES */
/*          cmbApplyToFm:HIDDEN = YES  */
/*          cmbApplyToQry:HIDDEN = YES */
/*            rsBorder:HIDDEN = YES */
/*            edTip:HEIGHT-PIXELS = edTip:HEIGHT-PIXELS + 75 */
         .
  APPLY "leave" TO fiBtnWidth.
  APPLY "leave" TO fiBtnHeight.
END.
  ...
  IF AVAIL ttJBoxTool THEN DO:
    ...
    iocToolbarDef = iocToolbarDef + (IF iocToolbarDef NE "" THEN "," ELSE "")
               + (IF ttJBoxTool.cType = "sub-menu" THEN
                  ...
                  + (IF NOT fiMenu:HIDDEN IN FRAME {&FRAME-NAME} AND (ttJBoxTool.cParentMenu NE "" OR ttJBoxTool.cViewAs = "button") THEN "|" 
                      + (IF ttJBoxTool.cViewAs = "button" THEN "" ELSE ttJBoxTool.cParentMenu)
                     ELSE "")
...
DO WITH FRAME {&FRAME-NAME}:
  ASSIGN iocToolbarDef = ""
         iMasterSeq = 0.
  FOR EACH ttJBoxTool
      WHERE ttJBoxTool.cType NE "menu"
        AND ttJBoxTool.bSelect 
        AND ttJBoxTool.cParentMenu = ""
      BY ttJBoxTool.iSeq:
    ASSIGN /* iMasterSeq = ttJBoxTool.iSeq */
           iocToolbarDef = iocToolbarDef + (IF iocToolbarDef NE "" THEN "," ELSE "")
             + (IF ttJBoxTool.cType = "sub-menu" THEN
                 "sub-menu|" + ttJBoxTool.cParentMenu + ";" + ttJBoxTool.cTool
                ELSE
                ...
                + (IF ttJBoxTool.cViewAs = "menu-item" THEN "menu" ELSE "")
                + (IF ttJBoxTool.bAlwaysEnable THEN "enable" ELSE "")
          cRowidList = cRowidList + (IF cRowidList NE "" THEN "," ELSE "") + STRING(ROWID(ttJBoxTool))
      .
  END.
  DO ix = 1 TO NUM-ENTRIES(cRowidList):
    FIND ttJBoxTool WHERE ROWID(ttJBoxTool) = TO-ROWID(ENTRY(ix,cRowidList)) NO-ERROR.
    IF AVAIL ttJBoxTool THEN
      ASSIGN iMasterSeq = iMasterSeq + 10
             ttJBoxTool.iSeq = iMasterSeq.
  END.

  IF NOT fiMenu:HIDDEN AND fiMenu:SCREEN-VALUE NE "" THEN DO:
    reSeqSubMenu(0,fiMenu:SCREEN-VALUE,1).
    FOR EACH ttJBoxTool
        WHERE ttJBoxTool.cType = "menu"
          AND NOT ttJBoxTool.bDeleted
        BY ttJBoxTool.iSeq:

      iocToolbarDef = iocToolbarDef + (IF iocToolbarDef NE "" THEN "," ELSE "") + "|" + ttJBoxTool.cTool.

      reSeqSubMenu(ttJBoxTool.iSeq,ttJBoxTool.cTool,1).
    END.
  END.
/*   ELSE DO:                                 */
/*     reSeqSubMenu(0,"",1).                  */
/*     MESSAGE iocToolbarDef                  */
/*         VIEW-AS ALERT-BOX INFO BUTTONS OK. */
/*   END.                                     */
END.
...
DO WITH FRAME {&FRAME-NAME}:
  IF ihBtnFilter:PRIVATE-DATA = ? THEN DO:
    ...
    FOR EACH ttJBoxTool 
      ...
      END.
    END.
    IF btnFilter:PRIVATE-DATA = "checked" THEN DO:
      btnFilter:PRIVATE-DATA = ?.
      btnFilter:LOAD-IMAGE("gif\filter.gif").
    END.
  END.
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  IF AVAIL ttJBoxTool THEN DO:
    MESSAGE "Removing the menu node will cause it's child nodes to be" SKIP
            "swapped to the primary menu node: " fiMenu:SCREEN-VALUE 
            "Continue?"
