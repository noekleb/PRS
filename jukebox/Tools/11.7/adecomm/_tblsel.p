...
Updated: 01/06/99 adams Selected Databases selection-list item should default 
   ...
   Brynjar 29 dec 13:
           ...
           Added pre-selected tables from JukeBox view definitions
----------------------------------------------------------------------------*/
...
DEF VAR cTablesInUse AS CHAR NO-UNDO. /* <- Brynjar - tables defined in query / queries */
...
ON VALUE-CHANGED OF db-browse IN FRAME schemapk DO:
    ...
    /* Brynjar: */
    IF h_tbl_brws:HEIGHT-PIXELS < 270 THEN
      ASSIGN h_tbl_brws:FRAME:WIDTH-PIXELS = h_tbl_brws:FRAME:WIDTH-PIXELS + 150
             h_tbl_brws:FRAME:HEIGHT-PIXELS = h_tbl_brws:FRAME:HEIGHT-PIXELS + 250
             h_tbl_brws:WIDTH-PIXELS = h_tbl_brws:WIDTH-PIXELS + 150
             h_tbl_brws:HEIGHT-PIXELS = h_tbl_brws:HEIGHT-PIXELS + 250.
END.
...
IF NOT ERROR-STATUS:ERROR AND NOT DYNAMIC-FUNCTION("getIsNotJukeBoxObject") THEN DO:
/*   IF NOT bJukeBox THEN                                  */
/*     bJukeBox = DYNAMIC-FUNCTION("getIsJukeBoxObject").  */
  IF bJukeBox THEN
    cTablesInUse = DYNAMIC-FUNCTION("getTablesInUse").
END.  
IF cTablesInUse NE "" THEN
  RUN load_tables (INPUT cTablesInUse).
ELSE
  /* Open table browse */
...
PROCEDURE ExpandFrame:
  ...
  ASSIGN FRAME schemapk:WIDTH-PIXELS  = FRAME schemapk:WIDTH-PIXELS + 150
         FRAME schemapk:HEIGHT-PIXELS = FRAME schemapk:HEIGHT-PIXELS + 250
         h_tbl_brws:FRAME:WIDTH-PIXELS = h_tbl_brws:FRAME:WIDTH-PIXELS + 150
         h_tbl_brws:FRAME:HEIGHT-PIXELS = h_tbl_brws:FRAME:HEIGHT-PIXELS + 250
         h_tbl_brws:WIDTH-PIXELS = h_tbl_brws:WIDTH-PIXELS + 150
         h_tbl_brws:HEIGHT-PIXELS = h_tbl_brws:HEIGHT-PIXELS + 250
         .
  hFg = FRAME schemapk:FIRST-CHILD.
  REPEAT WHILE VALID-HANDLE(hFg):
    ...
    REPEAT WHILE VALID-HANDLE(hWidget):
      ...
      IF hWidget:X > 135 THEN hWidget:X = hWidget:X + 150.
