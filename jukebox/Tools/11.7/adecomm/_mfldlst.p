...
Modified: 03.30.16 by brynjar@chemistry.no to support JuleBox table buffers (buf1_customer..)                      
----------------------------------------------------------------------------*/
...
DEF VAR cTableNames AS CHAR EXTENT 50 NO-UNDO. /* JukeBox */
...
DO v_ListCnt = 1 TO MINIMUM(50,NUM-ENTRIES(p_TblList)):
  ...
  ELSE DO:
    ...
    cTableNames[v_ListCnt] = v_Tblname. /* JukeBox */
  END.
...
DO v_ListCnt = 1 to MINIMUM(50,NUM-ENTRIES(p_TblList)):
  ...
  /* JukeBox: */
  ELSE DO:
    IF cTableNames[v_ListCnt] MATCHES "buf*_*" THEN
      use-table = cTableNames[v_ListCnt].
    ELSE
      use-table = ?.
  END.
...
/*   MESSAGE "p_List" p_List SKIP               */
/*           "use-table" use-table SKIP         */
/*           "p_Alpha" p_Alpha SKIP             */
/*           "p_Items" p_Items SKIP             */
/*           "p_CallBack" p_CallBack SKIP       */
/*           "p_Stat" p_Stat                    */
