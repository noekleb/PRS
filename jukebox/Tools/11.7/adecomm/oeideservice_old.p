...
/*------------------------------------------------------------------------
    ...
    Notes       : Some ide information is stored in ide context in the session.
                  ...
                  runtime/_server.p. 
    Modified    : 12/20/13 by brynjar@chemistry.no
                  Expands dialogs for query table and field selection (in createDialogService)    
  ----------------------------------------------------------------------*/
...
------------------------------------------------------------------------------*/
    ...
    /* Brynjar: Must expand frames here rather than in the dialogs themselves: */
    IF CAN-DO("adeuib/ide/_dialog_mfldsel.p", PROGRAM-NAME(2)) THEN 
      ASSIGN pframe:WIDTH-PIXELS  = pframe:WIDTH-PIXELS + 300
             pframe:HEIGHT-PIXELS = pframe:HEIGHT-PIXELS + 250.
    ELSE IF CAN-DO("adeuib/ide/_dialog_query.p", PROGRAM-NAME(2)) THEN 
      ASSIGN pframe:WIDTH-PIXELS  = pframe:WIDTH-PIXELS + 200
             pframe:HEIGHT-PIXELS = pframe:HEIGHT-PIXELS + 250.
    ELSE IF PROGRAM-NAME(2) = "adeuib\ide\_dialog_coledit.p" THEN
      ASSIGN pframe:WIDTH-PIXELS  = pframe:WIDTH-PIXELS + 100
             pframe:HEIGHT-PIXELS = pframe:HEIGHT-PIXELS + 250.
    ELSE IF PROGRAM-NAME(2) = "adeuib\ide\_dialog_advisor.p" THEN
      ASSIGN pframe:WIDTH-PIXELS  = pframe:WIDTH-PIXELS + 300
             pframe:HEIGHT-PIXELS = pframe:HEIGHT-PIXELS + 50.
    ELSE IF PROGRAM-NAME(2) = "adeuib\ide\_dialog_tblsel.p" THEN
      ASSIGN pframe:WIDTH-PIXELS  = pframe:WIDTH-PIXELS + 150
             pframe:HEIGHT-PIXELS = pframe:HEIGHT-PIXELS + 250.
...
function AddTrigger return logical
     ...
     /* valid check - may be called when the window itself is deleted (save all)  
      ...
      @TODO -  Maybe sanitize should be moved to getDesignId or use an id instead of handle 
         */
/*       MESSAGE "Dette virker ikke i 11.6.1 - bruk JB Override" */
