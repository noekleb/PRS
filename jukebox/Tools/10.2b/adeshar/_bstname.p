...
END.
/* Brynjar 05.11.13: */
IF AVAIL _U AND _U._NAME MATCHES "*_AssignedByJbox" THEN DO:
  p_best = SUBSTR(p_test,1,R-INDEX(p_test,"_") - 1).
  FOR EACH _U
      WHERE _U._WINDOW-HANDLE eq p_h_win
        AND _U._NAME BEGINS p_best:
    iCnt = iCnt + 1.
  END.
  IF iCnt LE 1 THEN RETURN.
END.
/* End 05.11.13 */
