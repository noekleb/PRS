...
/**************************************************************************
    ...
    Modified by : jep on 10/20/99 - Delete user AppServer and Sockets.
                  ...
                  brynjar@chemistry.no on 24.nov.2013
                  - added argument "Developer_Studio_Is_Running" when code is run 
                    from Developer Studio. Purpose is to be able to set UIB_is_RUNNING
                    in the .w procedure
**************************************************************************/
...
REPEAT ON QUIT       , LEAVE
  ...
  */
  ...
  /* Modified by brynjar@chemistry.no nov 13: */
  IF v_RunPersist = FALSE THEN DO:
    IF OS-GETENV("ABL_LAUNCH_MODE") = "run" AND SUBSTR(p_RunFile,LENGTH(p_RunFile) - 1) = ".w" THEN 
      RUN VALUE( p_RunFile ) "Developer_Studio_Is_Running".
    ELSE
      RUN VALUE( p_RunFile ).
  END.
  /* Original version:
