...
DEFINE BUTTON btnOpenPSDN 
     LABEL "ABHack on communities.progress.com" 
     CONTEXT-HELP-ID 0
     SIZE 41 BY 1.19 TOOLTIP "http://communities.progress.com/pcom/docs/DOC-18868".
...
ON CHOOSE OF btnOpenPSDN IN FRAME fMain /* ABHack on communities.progress.com */
DO:
   DEFINE VARIABLE iDontCare AS INTEGER    NO-UNDO.
   DEFINE VARIABLE cFileName AS CHARACTER  NO-UNDO.
...
/*-----------------------------------------------------------------------------------------
 ...
 Code to illustrate the problem:
  ...
  Solution: store ERROR-STATUS:ERROR and restore it as it is before retruning from this procedure 
...
IF ghLastActiveWinNoEditor = ACTIVE-WINDOW THEN RETURN.

cActiveWindowTitle = ACTIVE-WINDOW:TITLE NO-ERROR.  /* 24-DEC-2008 sla: added to solve possible error 3140 with .NET forms */
...
FIND ttEdt WHERE ttEdt.hWin = ACTIVE-WINDOW NO-ERROR.
/*Sanity cleanup*/
...
IF NOT AVAILABLE ttEdt THEN DO:
    ...
    hFrame = ACTIVE-WINDOW:FIRST-CHILD NO-ERROR.
    ...
    IF VALID-HANDLE(hEditor) = NO
     ...
     OR hEditor:SOURCE-EDITOR = NO THEN DO:
        IF glAutoStopWhenNonEditorWin 
         AND ACTIVE-WINDOW <> {&WINDOW-NAME} /*THIS-PROCEDURE*/
         ...
         THEN DO:
            ...
            cMon:SCREEN-VALUE IN FRAME fMain = "active window is not editor, stop spying until a printable key event occurs"
             + "~nINFO: A-W:PRIVATE-DATA = " + QUOTER(ACTIVE-WINDOW:PRIVATE-DATA).
            RETURN. /*leave the timer disabled*/
        END.
                                     
        /*we never save this var if glAutoStopWhenNonEditorWin*/
        ghLastActiveWinNoEditor = ACTIVE-WINDOW.
        {&RETURN}
    END.
...
hListPopUpFrame = ACTIVE-WINDOW:FIRST-CHILD.
...
------------------------------------------------------------------------------*/
    ...
    hActiveWindowToRestore = ACTIVE-WINDOW.
    ...
    RUN GetInputWidgetInXY (ACTIVE-WINDOW:HANDLE, OUTPUT hWidget).
    ...
    IF NOT VALID-HANDLE(hWidget) THEN DO:
        cmon:SCREEN-VALUE = STRING(TIME, "hh:mm:ss") + " copyUISVBelowMouse couldn't find any valid widget below the mouse pointer"
         + " in window " + QUOTER(ACTIVE-WINDOW:TITLE).
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY glProposeOpenSelectedFile gcFancyInABWin glDisplaytextSelectedInfo 
          ...
          glForceWaitFor giQueryTooltipTimeSuccessTxt giQueryTooltipTimeFailTxt 
...
IF VALID-HANDLE(ghMainAppbuilderWindow) THEN
...
ELSE DO:
    ...
    IF   NOT cClassFileName MATCHES "*~~~.cls"
     THEN cClassFileName = cClassFileName + ".cls".
    FILE-INFO:FILE-NAME = cClassFileName.
    IF FILE-INFO:FULL-PATHNAME = ?
     THEN FOR EACH ttUsing WHERE ttUsing.hEditor = phEditor:
        FILE-INFO:FILE-NAME = REPLACE(ttUsing.cUsing, ".*", "") + "/" + cClassFileName.
...
/*------------------------------------------------------------------------------
  Purpose:  to build a (error) RETURN-VALUE String
    ...
    errors that occured earlier, which could be very confusing.
------------------------------------------------------------------------------*/
