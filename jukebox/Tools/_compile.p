...
define var v_wname       as char no-undo.  /* .w name */
...
DEFINE VARIABLE cOriName AS CHARACTER  NO-UNDO.
...
REPEAT:
  ...
  cOriName = v_Curfile.
  /******************/
  /* Skip Compilation if a .w exists */
  v_wname = "":U.
  IF INDEX(v_curfile,".htm":U) > 0
  THEN DO:
    IF R-INDEX(v_curfile, ".") > 0
    THEN
      v_wname = substr(v_curfile,1, R-INDEX(v_curfile, ".") - 1) + ".w".
    ELSE
      v_wname = v_curfile + ".w".

    ASSIGN v_wname = SEARCH(v_wname).
    IF v_wname <> ? THEN NEXT.

    /* Transform .html into progressCode. */
    DEFINE VARIABLE v_leeg AS CHARACTER  NO-UNDO.
    v_leeg = "":U.
    v_wname = "":U.

    FILE-INFO:FILE-NAME = v_curfile.
    RUN webutil/e4gl-gen.p
    (input        FILE-INFO:FULL-PATHNAME,
     input-output v_leeg,
     input-output v_wname).
    v_curfile = v_wname.
  END.
  ...
  */
  ...
  put unformatted "Compiling "  cOriName skip.
  t_log = v_EditorOut:INSERT-STRING("Compiling " + cOriName +
