...
DEFINE VARIABLE rsAlle AS INTEGER 
     ...
     RADIO-BUTTONS 
          "Alle", 1,
"Valgte poster", 2
     SIZE 21 BY 1.58 NO-UNDO.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME Dialog-Frame
     fi-cNumTotal AT ROW 4.42 COL 17.14 COLON-ALIGNED
     rsAlle AT ROW 4.54 COL 36.14 NO-LABEL
     fi-cNumSelected AT ROW 5.38 COL 17.14 COLON-ALIGNED
     Btn_OK AT ROW 6.12 COL 63.86
     Btn_Cancel AT ROW 6.12 COL 79.43
     fi-cMessage AT ROW 1.69 COL 2 NO-LABEL
     SPACE(1.39) SKIP(4.77)
...
DO:
  ASSIGN rsAlle
         ...
         IF VALID-HANDLE(hInputField) THEN 
           IF hInputField:DATA-TYPE = "LOGICAL" THEN 
             ocValue = hInputField:INPUT-VALUE.
           ELSE
             ocValue = hInputField:SCREEN-VALUE.
END.
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  ASSIGN iBlanks                      = IF LENGTH(icMessage) < 90 THEN 90 - LENGTH(icMessage) ELSE 2
         ...
         .
  IF iiNumSelected > 0 OR iiNumSelected < 0 OR iiNumTotal = ? THEN
    rsAlle:SCREEN-VALUE = "2".
  ELSE 
    rsAlle:SCREEN-VALUE = "1".

  IF iiNumSelected = 0 OR iiNumTotal = ? THEN 
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  ELSE 
    ASSIGN cScreenValue             = rsAlle:SCREEN-VALUE
           ...
           rsAlle:SCREEN-VALUE      = cScreenValue
           .
