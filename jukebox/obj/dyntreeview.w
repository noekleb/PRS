...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getTreeHandle sObject 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getTreeSuper sObject 
FUNCTION getTreeSuper RETURNS CHARACTER
  ( /* parameter-definitions */ )  FORWARD.

/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD InitTreeView sObject 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD setAutoSort sObject 
...
DEFINE VARIABLE iY      AS INTEGER    NO-UNDO. 
DEFINE VARIABLE chItem  AS COM-HANDLE NO-UNDO.
DEF VAR iTry            AS INT NO-UNDO.

ASSIGN iX = DYNAMIC-FUNCTION("PixelsToTwips":U, INT(pdx), 0)
       iY = DYNAMIC-FUNCTION("PixelsToTwips":U, INT(ABS(pdy)), 1)
       NO-ERROR.

IF OS-GETENV("sessionname") BEGINS "ICA" THEN iy = iy + 30.
chItem =  chhTreeView:TreeView:HitTest(ix, iy) NO-ERROR.
IF NOT VALID-HANDLE(chItem) THEN DO iTry = 1 TO -100 BY -1:
  chItem =  chhTreeView:TreeView:HitTest(ix + iTry, iy) NO-ERROR.
  IF VALID-HANDLE(chItem) THEN LEAVE.
END.
IF NOT VALID-HANDLE(chItem) THEN
  RETURN NO-APPLY.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE deleteTree sObject 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE emptyTree sObject 
PROCEDURE emptyTree :
/*------------------------------------------------------------------------------
  Purpose:     Super Override
  Parameters:  
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION getTreeHandle sObject 
FUNCTION getTreeHandle RETURNS HANDLE
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION getTreeSuper sObject 
FUNCTION getTreeSuper RETURNS CHARACTER
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  
...
------------------------------------------------------------------------------*/
  ...
  RETURN THIS-PROCEDURE:SUPER-PROCEDURES.   /* Function return value. */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION InitTreeView sObject 
FUNCTION InitTreeView RETURNS HANDLE
  ( INPUT ihRectTree   AS HANDLE,
...
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
DYNAMIC-FUNCTION('setTargetProcedure',THIS-PROCEDURE).
...
ASSIGN FRAME frTreeView:X = DYNAMIC-FUNCTION("getAbsPosition",ihRectTree,"X")
       FRAME frTreeView:Y = DYNAMIC-FUNCTION("getAbsPosition",ihRectTree,"Y")
       FRAME frTreeView:WIDTH-PIXELS = ihRectTree:WIDTH-PIXELS
       FRAME frTreeView:HEIGHT-PIXELS = ihRectTree:HEIGHT-PIXELS
       hTreeView:WIDTH-PIXELS = ihRectTree:WIDTH-PIXELS
       hTreeView:HEIGHT-PIXELS = ihRectTree:HEIGHT-PIXELS
       .
...
RETURN FRAME frTreeView:HANDLE.  
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION setAutoSort sObject 
FUNCTION setAutoSort RETURNS LOGICAL
