...
ON 'F5':U OF FRAME {&FRAME-NAME} ANYWHERE DO:
  chCtrlFrame:WebBrowser:REFRESH() NO-ERROR.
  RETURN.
END.
...
OCXFile = SEARCH( "JBoxSupIe.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "JBoxSupIe.wrx":U SKIP(1)
...
ASSIGN FRAME {&FRAME-NAME}:X             = ihRectangle:X 
       ...
       CtrlFrame:WIDTH-PIXELS            = ihRectangle:WIDTH-PIXELS - 2
       CtrlFrame:HEIGHT-PIXELS           = ihRectangle:HEIGHT-PIXELS
       NO-ERROR.
...
RETURN STRING(FRAME {&FRAME-NAME}:HANDLE) + "," + STRING(CtrlFrame:HANDLE).   /* Function return value. */
