...
DEF VAR iButtonWidth        AS INT    NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD setButtonTooltip C-Win 
FUNCTION setButtonTooltip RETURNS LOGICAL
  ( INPUT iiBtnIndex AS INTEGER,
    INPUT icTooltip  AS CHAR )  FORWARD.
...
ON A OF FRAME panel-frame
DO:
  DEF VAR iSelectedItem AS INTEGER NO-UNDO.

  DO WITH FRAME DEFAULT-FRAME:
    /*First get the index of the selected item that triggered this event.*/
    RUN PanelGetActiveItem(iPanel,OUTPUT iSelectedItem).

    FIND FIRST ttPanel
         WHERE ttPanel.iButtonId = iSelectedItem + 1
         NO-ERROR.
    IF AVAIL ttPanel THEN DO:
      IF CAN-FIND(FIRST ttObject WHERE ttObject.hObject = THIS-PROCEDURE) THEN DO:
        FIND FIRST ttObjectLink 
             WHERE ttObjectLink.hFromObject = THIS-PROCEDURE  
        ...
        IF NOT AVAIL ttObjectLink THEN 
          FIND FIRST ttObjectLink 
               ...
               NO-ERROR.
      END.
      IF CAN-FIND(FIRST ttObject WHERE ttObject.hObject = THIS-PROCEDURE) AND AVAIL ttObjectLink THEN 
        RUN InvokeMethod(THIS-PROCEDURE,ttPanel.cMethod).
      ELSE
        RUN VALUE(ttPanel.cMethod) IN hSourceProc NO-ERROR.  
    END.
    ELSE 
      MESSAGE PROGRAM-NAME(1) SKIP
              "Error in panel definition - button not found"
              VIEW-AS ALERT-BOX ERROR.

    /*Now, if you don't want the item to stay selected make a call
      to PanelSetActiveItem and pass in {&GEN_NOSELECT}.
      This makes the control act like a menu or NavBar.
      NOTE: Items are zero base indexed.  This means that the first item
            added had an index of 0, the second Item added has an index of 1,
            and so on.*/
...
DEF VAR iLabelWidth AS INT NO-UNDO.
DEF VAR iNumSpace   AS INT NO-UNDO.
...
IF iImgIdx = -1 AND cImageList NE "" THEN DO:
  iLabelWidth = FONT-TABLE:GET-TEXT-WIDTH-PIXELS(icLabel,hRectangle:FRAME:FONT).
  IF iLabelWidth < iButtonWidth - iSpaceWidth THEN 
    ASSIGN iNumSpace = (iButtonWidth - iLabelWidth) / iSpaceWidth / 2
           icLabel = FILL(" ",iNumSpace) + icLabel.
END.
...
------------------------------------------------------------------------------*/
  ...
  {&OPEN-BROWSERS-IN-QUERY-panel-frame}
END PROCEDURE.
...
ASSIGN cCtrlHotkeyActions = DYNAMIC-FUNCTION("getAttribute",SESSION,"CtrlHotkeyActions")
       ...
       iButtonWidth       = IF iiButtonWidth = 0 THEN ihRectangle:WIDTH-PIXELS - 25 ELSE iiButtonWidth
       hRectangle         = ihRectangle
       iSpaceWidth        = FONT-TABLE:GET-TEXT-WIDTH-PIXELS(" ",hRectangle:FRAME:FONT)
       .
...
IF icMenu NE "" THEN DO:
  hMenu = WIDGET-HANDLE(DYNAMIC-FUNCTION("getAttribute",ihRectangle:WINDOW,"MENUBAR")) NO-ERROR.
  IF NOT VALID-HANDLE(ihRectangle:WINDOW:MENUBAR) AND NOT VALID-HANDLE(hMenu) THEN DO:
    CREATE MENU hMenu.
    ihRectangle:WINDOW:MENUBAR = hMenu.
    DYNAMIC-FUNCTION("setAttribute",ihRectangle:WINDOW,"MENUBAR",STRING(hMenu)).
  END.
  ELSE DO:
...
DO ix = 1 TO NUM-ENTRIES(icActionList):
  ...
  ASSIGN bToggle      = NUM-ENTRIES(cActionDef,";") > 2 AND ENTRY(3,cActionDef,";") = "toggle"
         ...
         cLabel       = (IF INDEX(cLabel,"ctrl-") > 0 THEN 
                         ...
                         ELSE IF R-INDEX(cLabel,"&") = LENGTH(cLabel) THEN 
                          REPLACE(cLabel,"&","")
                         ELSE cLabel)
         cToolTip     = IF cToolTip = "" THEN cLabel ELSE cToolTip
  ...
  IF NOT bMenuOnly AND NOT bToggle THEN DO:
    ...
    RUN AddItem (cAction,cLabel,"",cToolTip,cImage,"",cMethod).
/*                 (IF NUM-ENTRIES(cImageList,";") GE ix THEN ENTRY(ix,cImageList,";") ELSE ""),"",cMethod). */
    ...
    IF bToggle THEN
      CREATE MENU-ITEM ttEvent.hWidget
            ASSIGN TOGGLE-BOX = TRUE
                   PARENT     = hPlaceHold
                   LABEL      = cLabel
                   NAME       = ihRectangle:NAME + "_mitm_" + cAction 
                   TRIGGERS:
...
IF icOtherProp MATCHES "*border*" THEN 
...
DO WITH FRAME panel-frame:
  ASSIGN FRAME panel-frame:WIDTH-PIXELS  = ihPanelRect:WIDTH-PIXELS
         ...
         FRAME panel-frame:X = DYNAMIC-FUNCTION("getAbsPosition",ihPanelRect,"X")
         FRAME panel-frame:Y = DYNAMIC-FUNCTION("getAbsPosition",ihPanelRect,"Y")
         .
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION setButtonTooltip C-Win 
FUNCTION setButtonTooltip RETURNS LOGICAL
  ( INPUT iiBtnIndex AS INTEGER,
    INPUT icTooltip  AS CHAR ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
...
RUN PanelSetItemToolTip(iPanel,iiBtnIndex - 1,icTooltip).
RETURN YES.
