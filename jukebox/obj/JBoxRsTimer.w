...
DEF VAR hTimer    AS COM-HANDLE NO-UNDO.
...
FUNCTION setInterval RETURNS LOGICAL
  ( INPUT iiNewInterval AS INT )  FORWARD.
...
/* CtrlFrame OCXINFO:CREATE-CONTROL from: {A5849E76-E55F-11D3-AF35-00400565C94E} type: RSTimer */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL CtrlFrame C-Win OCX.Timer
PROCEDURE CtrlFrame.RSTimer.Timer .
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  None required for OCX.
  Notes:       
------------------------------------------------------------------------------*/
IF bSuspend THEN RETURN NO-APPLY.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  hTimer = chCtrlFrame:RSTimer.
  hTimer:Interval = iiInterval. /* set the time interval in milliseconds */
  hTimer:Enabled = TRUE.        /* start the timer */
...
OCXFile = SEARCH( "JBoxRsTimer.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "JBoxRsTimer.wrx":U SKIP(1)
...
FUNCTION setInterval RETURNS LOGICAL
  ( INPUT iiNewInterval AS INT ) :
/*------------------------------------------------------------------------------
  Purpose:  Set the timer intervall (default 60)
    Notes:  
------------------------------------------------------------------------------*/

iiInterval = iiNewInterval.
