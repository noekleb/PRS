...
DEFINE FRAME frmEmail
     "JLW Email listener" VIEW-AS TEXT
          SIZE 18 BY .62 AT ROW 1.81 COL 5.8
...
/*   RUN InitializeObject.  */
  ...
  SUBSCRIBE TO "getEmailTable" ANYWHERE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE getEMail C-Win 
PROCEDURE getEMail :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
IF NOT bCheckIfStored THEN 
  ASSIGN ix = INT(DYNAMIC-FUNCTION("getFieldValues","JBoxSysParam",
                                       "WHERE iJBoxCompanyId = " + DYNAMIC-FUNCTION("getCompany")
                                      + " AND cSysParamName = 'LastEmailCount'",
                                       "iSysParamIntValue"))
         iLastMsgCount = (IF ix NE ? THEN ix ELSE 0)
         .
...
IF iMsgCount > iLastMsgCount THEN DO:
  ...
  DO ix = iLastMsgCount + 1 TO iMsgCount: 
    ...
    ASSIGN ttNewEmail.cSubject       = TRIM(cSubject)
           ttNewEmail.cFrom          = TRIM(cFrom)
           ttNewEmail.cTo            = TRIM(cTo)
           ttNewEmail.cCc            = TRIM(cCc)
           ttNewEmail.cBcc           = TRIM(cBcc)
           ttNewEmail.cAttachments   = TRIM(cAttachments)
           ttNewEmail.cBody          = ""
           ttNewEmail.iAttachCount   = iAttachCount
           ttNewEmail.dDateSent      = DATE(iDateSent)
           ttNewEmail.iTimeSent      = iTimeSent / 1000
           ttNewEmail.cTimeSent      = STRING(ttNewEmail.iTimeSent,"HH:MM")
           ttNewEmail.iMsgId         = ix
           ttNewEmail.iPriority      = iPriority
           ttNewEmail.bBodyRetrieved = NO
           .
  END.

  IF NOT bCheckIfStored THEN
    iLastMsgCount = iMsgCount.
  ...
  FOR EACH ttNewEmail:
    ...
    END.
    ELSE DO:
      IF INDEX(ttNewEmail.cFrom,"<") > 0 THEN
        ASSIGN ttNewEmail.cFromEmail = TRIM(SUBSTR(ttNewEmail.cFrom,INDEX(ttNewEmail.cFrom,"<") + 1,R-INDEX(ttNewEmail.cFrom,">") - INDEX(ttNewEmail.cFrom,"<") - 1))
               ttNewEmail.cFromName  = TRIM(SUBSTR(ttNewEmail.cFrom,1,INDEX(ttNewEmail.cFrom,"<") - 1))
               .
      ELSE 
        ASSIGN ttNewEmail.cFromEmail = TRIM(ttNewEmail.cFrom)
               ttNewEmail.cFromName  = TRIM(SUBSTR(ttNewEmail.cFrom,1,INDEX(ttNewEmail.cFrom,"@") - 1))
               .
      ttNewEmail.cFileName = ttNewEmail.cFromEmail 
...
/*                            + ".htm" */
  ...
  IF bCheckIfStored THEN DO: /* If messages should not be deleted from server */
    DYNAMIC-FUNCTION("setShowHourGlass",NO).
    IF DYNAMIC-FUNCTION("runProc","jbdoc_checkemail_stored.p",
                        STRING(iMsgCount),httNewEmail) THEN DO:
      DYNAMIC-FUNCTION("getRunProcReturnTable",httNewEmailBuffer).
  
      FOR EACH ttNewEmail 
          WHERE ttNewEmail.cEntityId NE ""
          :
        FIND FIRST ttEmail
             WHERE ttEmail.cFrom     = ttNewEmail.cFrom
               AND ttEmail.dDateSent = ttNewEmail.dDateSent
      ...
      END.
    END.
    DYNAMIC-FUNCTION("setShowHourGlass",YES).
  END.

  PUBLISH "NewEmailCount" (iCntNewMsg).
      
/*   RUN RetrieveEmails (iCntNewMsg).  */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE getEmailTable C-Win 
...
bCompanySuper = LOGICAL(DYNAMIC-FUNCTION("getFieldValues","JBoxCompanyUser",
                "WHERE cJBoxUserId = '" + DYNAMIC-FUNCTION("getASuserId") + "' AND iJBoxCompanyId = " + DYNAMIC-FUNCTION("getCompany"),"bSuperUserCompany")).

IF cMailServer = "" THEN DO:
  ASSIGN cUserPwd = DYNAMIC-FUNCTION("getFieldValues","FIRST JBoxSysParam",
                                         "WHERE bActive AND cSysParamName BEGINS 'CompanyArchiveEmail'"
                                       + " AND iJBoxCompanyId = " + DYNAMIC-FUNCTION("getCompany")
                                       + " AND iSysParamIntValue = 1" /* <- indicates default */
                                        ,"cSysParamCharValue,cSysParamText")
         cMailServer = DYNAMIC-FUNCTION("getFieldValues","JBoxSysParam",
                                         "WHERE bActive AND cSysParamName = 'CompanyArchiveMailServer' AND iJBoxCompanyId = " + DYNAMIC-FUNCTION("getCompany"),
                                         "cSysParamCharValue")
         .


  IF cMailServer = ? OR cUserPwd = ? THEN DO:
    APPLY "close" TO THIS-PROCEDURE.
    RETURN.  
  END. 
  ASSIGN cUserName = ENTRY(1,cUserPwd,"|")
         cPassword = ENTRY(2,cUserPwd,"|")
         .
END.
ELSE IF cPassword = "" THEN
  cPassword = DYNAMIC-FUNCTION("getFieldValues","FIRST JBoxSysParam",
                               "WHERE bActive AND cSysParamName BEGINS 'CompanyArchiveEmail'"
                             + " AND iJBoxCompanyId = " + DYNAMIC-FUNCTION("getCompany")
                             + " AND cSysParamCharValue = '" + cUserName + "'"
                               ,"cSysParamText").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE RetrieveEmails C-Win 
PROCEDURE RetrieveEmails :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       Ikke brukbar (foreløpig)
------------------------------------------------------------------------------*/
DEF INPUT PARAM iiMsgCnt AS INT NO-UNDO.
...
RUN toExcelViaFile.p (httNewEmailBuffer,0).
...
DO ix = 1 TO iiMsgCnt:
  ...
  IF GET-LONG(iSize,5) LE 31500 THEN DO:
    ...
    FIND FIRST ttNewEmail 
         WHERE ttNewEmail.cSubject       = TRIM(cSubject)
           AND ttNewEmail.cFrom          = TRIM(cFrom)
           AND ttNewEmail.cTo            = TRIM(cTo)
           AND ttNewEmail.cCc            = TRIM(cCc)
           AND ttNewEmail.cBcc           = TRIM(cBcc)
           AND ttNewEmail.cAttachments   = TRIM(cAttachments)
           AND ttNewEmail.dDateSent      = DATE(iDateSent)
         NO-ERROR.
    MESSAGE "AVAIL ttNewEmail" AVAIL ttNewEmail SKIP
    ...
    IF AVAIL ttNewEmail
      ...
      MESSAGE "AVAIL ttNewEmail 2" AVAIL ttNewEmail
          VIEW-AS ALERT-BOX INFO BUTTONS OK.
      IF AVAIL ttNewEmail THEN
        DeleteEmail(ttNewEmail.iMsgId).
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE SaveEmailMessage C-Win 
