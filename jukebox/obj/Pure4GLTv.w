...
the frame of the emulated vertical scrollbar.  Fixed by calling ShowScrollBar IN hpApi
...
/* Customization for JukeBox: */
DEF VAR hRectTree   AS HANDLE NO-UNDO.
...
FUNCTION InitTreeView RETURNS HANDLE
  ( INPUT ihRectTree   AS HANDLE,
    INPUT ihParentProc AS HANDLE )  FORWARD.
...
DEFINE FRAME fsv
     emptySquare AT ROW 3.14 COL 1
     Sbsv AT ROW 1.71 COL 1
     SbtnScrollDown AT ROW 2.57 COL 1
...
keep scrolling until mouse button is released or until the scroll button is reached*/
  ...
  DO WHILE TRUE:
      ...
      IF iLastEventY < ghbsv:Y THEN DO:
          ...
          blackClickDownRectangle:HEIGHT-PIXELS = gbsvY NO-ERROR.
          blackClickDownRectangle:MOVE-TO-TOP().
          blackClickDownRectangle:VISIBLE = YES.
          tvScroll(- gVisibleIterations + 1, YES).
          IF gbsvY > gFsvimgY THEN blackClickDownRectangle:HEIGHT-PIXELS = gbsvY NO-ERROR.
      END.
      ...
      ELSE DO:
          ...
          blackClickDownRectangle:HEIGHT-PIXELS = MAX(1,gFsvimgHP - gbsvY - gbsvHP) NO-ERROR. /* when the button is just few pixels above the very bottom, we could get a result smaller than 1  */
          blackClickDownRectangle:MOVE-TO-TOP().
          blackClickDownRectangle:VISIBLE = YES.
          tvScroll(gVisibleIterations - 1, YES).
          IF gbsvY + gbsvHP + gFsvimgY < gFsvimgHP THEN ASSIGN
           ...
           blackClickDownRectangle:Y = gbsvY + gbsvHP + gFsvimgY NO-ERROR.
      END.
...
/*------------------------------------------------------------------------------
  Purpose:    Close the treeview object 
  Parameters:  <none>
  Notes:      Added custimization for use with JukeBox 
------------------------------------------------------------------------------*/
DEF INPUT PARAM ihProcHandle AS HANDLE NO-UNDO.

IF ihProcHandle = hContainer THEN DO:
  RUN DestroyObject.
...
/*------------------------------------------------------------------------------
  Purpose:    Resize the treeview after move of splitbar 
  Parameters:  <none>
  Notes:      Added customization for JukeBox 
------------------------------------------------------------------------------*/
DEF INPUT PARAM ihWindow AS HANDLE NO-UNDO.

IF ihWindow = THIS-PROCEDURE:CURRENT-WINDOW THEN DO: 
  RUN ResizeObject (hRectTree:HEIGHT,hRectTree:WIDTH).
  FRAME ftv:MOVE-TO-TOP().
END.
...
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  Customization for JukeBox: All height pixels are reduced with 1 pixel
------------------------------------------------------------------------------*/
  ...
  CREATE IMAGE ghbsv IN WIDGET-POOL cVertSBWP ASSIGN
   ...
   HEIGHT-PIXELS = Sbsv:HEIGHT-PIXELS - 1
   VISIBLE = YES
   SENSITIVE = YES
   TRIGGERS:
  ...
  CREATE IMAGE ghbtnScrollUp  IN WIDGET-POOL cVertSBWP ASSIGN
   ...
   HEIGHT-PIXELS = SbtnScrollUp:HEIGHT-PIXELS - 1
   VISIBLE = YES
   SENSITIVE = YES
   TRIGGERS:
  ...
  CREATE IMAGE ghbtnScrollDown  IN WIDGET-POOL cVertSBWP ASSIGN
   ...
   HEIGHT-PIXELS = SbtnScrollDown:HEIGHT-PIXELS - 1
   VISIBLE = YES
   SENSITIVE = YES
     TRIGGERS:
...
FUNCTION InitTreeView RETURNS HANDLE
  ( INPUT ihRectTree   AS HANDLE,
    INPUT ihParentProc AS HANDLE ) :
/*------------------------------------------------------------------------------
  Purpose:  Init the treeview and set size
    Notes:  Added for JukeBox
            Also added various NO-ERROR to avoid error messages due to resize
            most of them in resizeVertScrollBar and MOUSE-SELECT-DOWN of imgsv (widget blackClickDownRectangle)
------------------------------------------------------------------------------*/
DEF VAR ix AS INT NO-UNDO.
DEF VAR hSuperProc AS HANDLE NO-UNDO.

ASSIGN hRectTree  = ihRectTree
       hContainer = SOURCE-PROCEDURE.

ASSIGN FRAME fMain:X = DYNAMIC-FUNCTION("getAbsPosition",ihRectTree,"X")
       FRAME fMain:Y = DYNAMIC-FUNCTION("getAbsPosition",ihRectTree,"Y")
       FRAME fMain:WIDTH-PIXELS  = ihRectTree:WIDTH-PIXELS
       FRAME fMain:HEIGHT-PIXELS = ihRectTree:HEIGHT-PIXELS
       FRAME fMain:VIRTUAL-WIDTH-PIXELS  = ihRectTree:WIDTH-PIXELS
       FRAME fMain:VIRTUAL-HEIGHT-PIXELS = ihRectTree:HEIGHT-PIXELS
       .
...
DO ix = 1 TO NUM-ENTRIES(THIS-PROCEDURE:SUPER-PROCEDURES):
  hSuperProc = WIDGET-HANDLE(ENTRY(ix,THIS-PROCEDURE:SUPER-PROCEDURES)).
  ihParentProc:ADD-SUPER-PROC(hSuperProc).
END.

RETURN FRAME fMain:HANDLE.  
...
ASSIGN
 ...
 ghbsv:HEIGHT-PIXELS = gbsvHP NO-ERROR. /* ...smaller scrollbar, the next call of renderVertScrollBar() will calculate the right size again */
...
ASSIGN
 ...
 FRAME fsv:VIRTUAL-HEIGHT-PIXELS = FRAME fsv:HEIGHT-PIXELS NO-ERROR.
...
IF FRAME fsv:HWND <> ?
 THEN RUN ShowScrollBar IN hpApi (FRAME fsv:HWND, {&SB_BOTH},  0  {&APIRtnParmINT}).
