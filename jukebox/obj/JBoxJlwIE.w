...
&Scoped-Define ENABLED-OBJECTS RectSupIE 
...
DO:
  DYNAMIC-FUNCTION("NavigateToURL" IN hPanel,cUrl).
END.
...
RUN InitializeObject IN hPanel (RectSupIE:HANDLE IN FRAME {&FRAME-NAME},icUrl).
IF icUrl NE "" THEN
  RUN MoveToTop IN hPanel.

SUBSCRIBE TO "InvalidateHandle" IN hPanel.

ASSIGN THIS-PROCEDURE:CURRENT-WINDOW:TITLE = icTitle
       cURL = icURL.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEF INPUT PARAM ihPanel AS HANDLE NO-UNDO.

IF ihPanel = hPanel THEN
...
DYNAMIC-FUNCTION("NavigateToURL" IN hPanel,icUrl).
