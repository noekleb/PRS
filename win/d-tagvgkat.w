...
DEFINE TEMP-TABLE tKategori NO-UNDO LIKE SkoTex.VgKat
       field Beskrivelse as char format "x(30)"
       index VgKat is unique primary
       VgKat
       index Beskrivelse
       Beskrivelse.
...
&scoped-define DB-BROWSER BROWSE-Kategori

&scoped-define br-tabell Kategori

/* Kolonne 1 som det kan sorteres på */
&scoped-define sok-field1  KatNr
...
&scoped-define index-col1  KatNr
...
&scoped-define Assign-IO-Liste string(tKategori.VgKat) + ";" + STRING(t{&br-tabell}.{&sok-field1})
...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME BROWSE-Kategori

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES Kategori tKategori

/* Definitions for BROWSE BROWSE-Kategori                               */
&Scoped-define FIELDS-IN-QUERY-BROWSE-Kategori Kategori.KatNr ~
Kategori.Beskrivelse 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-Kategori Kategori.KatNr 
&Scoped-define FIELD-PAIRS-IN-QUERY-BROWSE-Kategori~
 ~{&FP1}KatNr ~{&FP2}KatNr ~{&FP3}
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-Kategori Kategori
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-Kategori Kategori
&Scoped-define OPEN-QUERY-BROWSE-Kategori OPEN QUERY BROWSE-Kategori FOR EACH Kategori NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-Kategori Kategori
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-Kategori Kategori


/* Definitions for BROWSE BROWSE-Tag                                    */
&Scoped-define FIELDS-IN-QUERY-BROWSE-Tag tKategori.VgKat tKategori.KatNr ~
wBeskrivelse 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-Tag tKategori.KatNr 
&Scoped-define FIELD-PAIRS-IN-QUERY-BROWSE-Tag~
 ~{&FP1}KatNr ~{&FP2}KatNr ~{&FP3}
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-Tag tKategori
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-Tag tKategori
&Scoped-define OPEN-QUERY-BROWSE-Tag OPEN QUERY BROWSE-Tag FOR EACH tKategori NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-Tag tKategori
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-Tag tKategori


/* Definitions for DIALOG-BOX Dialog-Frame                              */
&Scoped-define OPEN-BROWSERS-IN-QUERY-Dialog-Frame ~
    ~{&OPEN-QUERY-BROWSE-Kategori}

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS RECT-4 RECT-5 BROWSE-Kategori BROWSE-Tag ~
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY BROWSE-Kategori FOR 
      Kategori
    FIELDS(Kategori.KatNr
      Kategori.Beskrivelse) SCROLLING.

DEFINE QUERY BROWSE-Tag FOR 
      tKategori
    FIELDS(tKategori.VgKat
      tKategori.KatNr) SCROLLING.
&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE BROWSE-Kategori
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Kategori Dialog-Frame _STRUCTURED
  QUERY BROWSE-Kategori NO-LOCK DISPLAY
      Kategori.KatNr
      Kategori.Beskrivelse
  ENABLE
      Kategori.KatNr
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Tag Dialog-Frame _STRUCTURED
  QUERY BROWSE-Tag NO-LOCK DISPLAY
      tKategori.VgKat FORMAT "z9"
      tKategori.KatNr
      wBeskrivelse COLUMN-LABEL "Beskrivelse"
  ENABLE
      tKategori.KatNr
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME Dialog-Frame
     BROWSE-Kategori AT ROW 2.91 COL 3
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "Velg kategorier for varegruppe"
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: tKategori T "?" NO-UNDO SkoTex VgKat
      ADDITIONAL-FIELDS:
          field Beskrivelse as char format "x(30)"
          index VgKat is unique primary
          VgKat
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   Custom                                                               */
/* BROWSE-TAB BROWSE-Kategori 1 Dialog-Frame */
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE BROWSE-Kategori
/* Query rebuild information for BROWSE BROWSE-Kategori
     _TblList          = "SkoTex.Kategori"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _TblOptList       = "USED"
     _FldNameList[1]   > SkoTex.Kategori.KatNr
"Kategori.KatNr" ? ? "integer" ? ? ? ? ? ? yes ?
     _FldNameList[2]   = SkoTex.Kategori.Beskrivelse
     _Query            is OPENED
*/  /* BROWSE BROWSE-Kategori */
&ANALYZE-RESUME

&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE BROWSE-Tag
/* Query rebuild information for BROWSE BROWSE-Tag
     _TblList          = "Temp-Tables.tKategori"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _TblOptList       = "USED"
     _FldNameList[1]   > Temp-Tables.tKategori.VgKat
"tKategori.VgKat" ? "z9" "integer" ? ? ? ? ? ? no ?
     _FldNameList[2]   > Temp-Tables.tKategori.KatNr
"tKategori.KatNr" ? ? "integer" ? ? ? ? ? ? yes ?
     _FldNameList[3]   > "_<CALC>"
"wBeskrivelse" "Beskrivelse" ? ? ? ? ? ? ? ? no ?
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Velg kategorier for varegruppe */
DO:
...
&Scoped-define BROWSE-NAME BROWSE-Kategori
&Scoped-define SELF-NAME BROWSE-Kategori
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Kategori Dialog-Frame
ON CURSOR-LEFT OF BROWSE-Kategori IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Kategori Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-Kategori IN FRAME Dialog-Frame
DO:
  IF {&ant-sok-field} = 1 THEN
    RETURN NO-APPLY.
  RUN SD-CURSOR ("RIGHT").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Kategori Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-Kategori IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Kategori Dialog-Frame
ON START-SEARCH OF BROWSE-Kategori IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Kategori Dialog-Frame
ON VALUE-CHANGED OF BROWSE-Kategori IN FRAME Dialog-Frame
DO:
    IF BROWSE BROWSE-Tag:NUM-SELECTED-ROWS > 0 THEN DO:
        IF BROWSE BROWSE-Tag:DESELECT-ROWS() THEN.
...
DO:
  find Kategori no-lock where
    Kategori.KatNr = tKategori.KatNr no-error.
  if available Kategori then
    assign
      wBeskrivelse = if available Kategori
                        then Kategori.Beskrivelse
                        else "".
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-4 RECT-5 BROWSE-Kategori BROWSE-Tag Btn_OK Btn_Cancel Btn_Help 
         BUTTON-Sok 
      WITH FRAME Dialog-Frame.
    ...
    DO wAntEntries = 1 TO NUM-ENTRIES(IO-Liste) WITH FRAME {&FRAME-NAME}:
        ...
        */
        FIND b{&br-tabell} where 
          b{&br-tabell}.{&sok-field1} = INT(ENTRY(2,ENTRY(wAntEntries,IO-Liste),';')) no-lock no-error.
        
        IF AVAIL b{&br-tabell} THEN DO:
            CREATE t{&br-tabell}.
            assign
              wAntValgt           = wAntValgt + 1            
              t{&br-tabell}.VgKat = int(entry(1,entry(wAntEntries,IO-Liste),';'))
              t{&br-tabell}.KatNr = int(entry(2,entry(wAntEntries,IO-Liste),';')).
            /*
                ...
                IF NOT AVAIL t{&br-tabell} THEN DO:
                   ...
                   do wLoop = 1 to 99:
                     find t{&br-tabell} no-lock where
                       t{&br-tabell}.VgKat = wLoop no-error.
                   ...
                   assign
                     wAntValgt           = wAntValgt + 1
                     t{&br-tabell}.VgKat = wLoop
                     t{&br-tabell}.KatNr = {&br-tabell}.KatNr.                        
                    ...
                    IF BROWSE BROWSE-Tag:FETCH-SELECTED-ROW (wAntSel) THEN
                      do:
                        if can-find(first ArtBas where
                                      ArtBas.Vg    = t{&br-tabell}.Vg and
                                      ArtBas.VgKat = t{&br-tabell}.VgKat) then
                          do:
                            message "Posten er i bruk. Kan ikke slettes"
                            ...
                            find bufVgKat exclusive-lock where
                              ...
                              bufVgKat.VgKat = t{&br-tabell}.VgKat no-error.
                            if available bufVgKat then
                              delete bufVgKat.
                            DELETE t{&br-tabell}.
                            assign
                              wAntValgt = wAntValgt - 1.
