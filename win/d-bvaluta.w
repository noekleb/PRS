...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEFINE VAR wValKod LIKE Valuta.ValKod NO-UNDO.
&ELSE
  DEFINE INPUT-OUTPUT PARAMETER wValKod LIKE Valuta.ValKod NO-UNDO.
&ENDIF
...
&scoped-define sok-field1  ValKod
...
&scoped-define index-col1  ValutaIn
...
&scoped-define sok-field2  ValLand
...
&scoped-define index-col2  ValLand
...
&scoped-define ip-variabel wValKod
&scoped-define return-ip   wValKod = Valuta.ValKod.
&scoped-define assign-retur-verdi retur-verdi = string(recid(Valuta)).
...
&scoped-define KanSlettes ~
  if can-find(first ArtBas where ArtBas.ValKod = Valuta.ValKod) then ~
    do: ~
      message "Det finns artiklar med denna valutakod" ~
      view-as alert-box title "Fel vid borttagning". ~
      return no-apply. ~
    end.

&scoped-define VedlikeholdsRutine d-vvaluta.w
...
&Scoped-define FIELDS-IN-QUERY-BROWSE-Valuta Valuta.ValKod Valuta.ValNavn ~
Valuta.ValLand Valuta.ValKurs Valuta.indeks Valuta.ValDatum 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-Valuta Valuta.ValKod ~
Valuta.ValLand 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-Valuta Valuta
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-Valuta Valuta
&Scoped-define QUERY-STRING-BROWSE-Valuta FOR EACH Valuta NO-LOCK ~
...
FILL-IN-SOK-INT BUTTON-Sok BROWSE-Valuta BUTTON-Ny BUTTON-Endre Btn_OK ~
Btn_Cancel Btn_Help 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Valuta Dialog-Frame _STRUCTURED
  QUERY BROWSE-Valuta NO-LOCK DISPLAY
      Valuta.ValKod COLUMN-LABEL "ValutaKode *" FORMAT "x(3)":U
      Valuta.ValNavn FORMAT "X(30)":U
      Valuta.ValLand COLUMN-LABEL "Land *" FORMAT "x(15)":U
      Valuta.ValKurs FORMAT "z9.999999":U
      Valuta.indeks FORMAT ">>9":U
      Valuta.ValDatum FORMAT "99/99/9999":U WIDTH 15.6
  ENABLE
      Valuta.ValKod
      Valuta.ValLand
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 94 BY 7.38 EXPANDABLE.
...
DEFINE FRAME Dialog-Frame
     ...
     BROWSE-Valuta AT ROW 2.91 COL 2
     BUTTON-Ny AT ROW 2.91 COL 96.8
     BUTTON-Endre AT ROW 4.24 COL 96.8
     BUTTON-Slett AT ROW 5.57 COL 96.8
     Btn_OK AT ROW 10.62 COL 2.2
     Btn_Cancel AT ROW 10.62 COL 14.6
     Btn_Help AT ROW 10.62 COL 96.8
     SPACE(0.79) SKIP(0.00)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste kalkylevalutaer"
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
                                                                        */
/* BROWSE-TAB BROWSE-Valuta BUTTON-Sok Dialog-Frame */
ASSIGN 
...
"Valuta.ValKod" "ValutaKode *" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   = SkoTex.Valuta.ValNavn
     _FldNameList[3]   > SkoTex.Valuta.ValLand
"Valuta.ValLand" "Land *" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[4]   = SkoTex.Valuta.ValKurs
     _FldNameList[5]   = SkoTex.Valuta.indeks
     _FldNameList[6]   > SkoTex.Valuta.ValDatum
"Valuta.ValDatum" ? "99/99/9999" "date" ? ? ? ? ? ? no ? no no "15.6" yes no no "U" "" ""
     _Query            is OPENED
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Søkeliste kalkylevalutaer */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Valuta Dialog-Frame
ON ANY-PRINTABLE OF BROWSE-Valuta IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Valuta Dialog-Frame
ON CURSOR-LEFT OF BROWSE-Valuta IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Valuta Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-Valuta IN FRAME Dialog-Frame
DO:
  IF {&ant-sok-field} = 1 THEN
    RETURN NO-APPLY.
  RUN SD-CURSOR ("RIGHT").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Valuta Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-Valuta IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Valuta Dialog-Frame
ON START-SEARCH OF BROWSE-Valuta IN FRAME Dialog-Frame
DO:
  RUN SD-START-SEARCH.
  RETURN NO-APPLY.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Valuta Dialog-Frame
ON VALUE-CHANGED OF BROWSE-Valuta IN FRAME Dialog-Frame
DO:
  ASSIGN BUTTON-Endre:SENSITIVE = Valuta.ValKod <> "".
END.
...
DO:
    /* !!!!!!!!  vi returnerar knapp är egentligen disablad    !!!!!!!!! */
    RETURN NO-APPLY.
...
{syspara.i 16 10 2 iTilgang INT}
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-DATE FILL-IN-SOK-CHAR FILL-IN-SOK-INT BUTTON-Sok 
         BROWSE-Valuta BUTTON-Ny BUTTON-Endre Btn_OK Btn_Cancel Btn_Help 
      WITH FRAME Dialog-Frame.
