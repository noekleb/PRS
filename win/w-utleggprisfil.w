...
DEFINE VARIABLE FI-Tittel AS CHARACTER FORMAT "X(256)":U INITIAL "    Eksport av priser til kassene" 
      VIEW-AS TEXT 
     SIZE 77 BY 1.38
     FGCOLOR 1 FONT 8 NO-UNDO.

DEFINE VARIABLE FI-TotAntall AS CHARACTER FORMAT "X(256)":U 
     LABEL "Antall å eksportere" 
     VIEW-AS FILL-IN 
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Eksport av priser til kassene"
         HEIGHT             = 16.19
         WIDTH              = 80
         MAX-HEIGHT         = 16.19
         MAX-WIDTH          = 80
...
ON END-ERROR OF C-Win /* Eksport av priser til kassene */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Eksport av priser til kassene */
DO:
...
ON CHOOSE OF B-Start IN FRAME DEFAULT-FRAME /* Start eksport */
DO:
  ...
  if B-Start:private-data = "OPPDAT" then
    do:
      ...
      run x-utleggprisfil.w persistent set wProgram-Handle (this-procedure:handle).      
    end.
...
DO:
  ...
  assign
    ...
    wDato         = today
    wTid          = time
    FI-TotAntall:screen-value in frame DEFAULT-FRAME = "Teller artikkler og aktive profiler...".
  ...
  for each PrisProfil no-lock 
    ...
    if can-do(wProfilListe,string(PrisProfil.ProfilNr)) then
      wAntProfiler = wAntProfiler + 1.  
    else 
      next PROFIL.
