...
&scope assign-retur-verdi ASSIGN retur-verdi = STRING(recid({&br-tabell}))
...
&scoped-define KanSlettes ~
  if Sprak.Lng = "DES" then ~
    do: ~
      message "Språk 'DES' kan ikke slettes!" ~
      view-as alert-box title "Fel vid borttagning". ~
      return no-apply. ~
    end. ~
  if can-find(first Lng where Lng.Lng = Sprak.Lng) then ~
    do: ~
      message "Det finnes språkkoblinger til dette språket. Kan ikke slettes!" ~
      view-as alert-box title "Fel vid borttagning". ~
      return no-apply. ~
    end. ~
  else if can-find(first Bruker where Bruker.Lng = Sprak.Lng) then ~
    do: ~
      message "Det finnes bruker koblet til dette språket. Kan ikke slettes!" ~
      view-as alert-box title "Fel vid borttagning". ~
      return no-apply. ~
    end. 

&scoped-define VedlikeholdsRutine d-vsprak.w
...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME BROWSE-Sprak

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES sprak

/* Definitions for BROWSE BROWSE-Sprak                                  */
&Scoped-define FIELDS-IN-QUERY-BROWSE-Sprak sprak.Lng sprak.Beskrivelse 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-Sprak sprak.Lng 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-Sprak sprak
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-Sprak sprak
&Scoped-define OPEN-QUERY-BROWSE-Sprak OPEN QUERY BROWSE-Sprak FOR EACH sprak NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-Sprak sprak
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-Sprak sprak
...
FILL-IN-SOK-DATE BUTTON-Sok BROWSE-Sprak Btn_OK Btn_Help 
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY BROWSE-Sprak FOR 
      sprak SCROLLING.
&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE BROWSE-Sprak
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Sprak Dialog-Frame _STRUCTURED
  QUERY BROWSE-Sprak NO-LOCK DISPLAY
      sprak.Lng FORMAT "X(3)":U
      sprak.Beskrivelse FORMAT "X(20)":U
  ENABLE
      sprak.Lng
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 50 BY 14.76 ROW-HEIGHT-CHARS .62 EXPANDABLE.
...
DEFINE FRAME Dialog-Frame
     ...
     BROWSE-Sprak AT ROW 2.91 COL 2
     Btn_OK AT ROW 17.91 COL 2
     Btn_Cancel AT ROW 17.91 COL 15.2
     Btn_Help AT ROW 17.91 COL 40
     BUTTON-Ny AT ROW 2.91 COL 53
     BUTTON-Endre AT ROW 4.24 COL 53
     BUTTON-Slett AT ROW 5.57 COL 53
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   Custom                                                               */
/* BROWSE-TAB BROWSE-Sprak BUTTON-Sok Dialog-Frame */
ASSIGN 
       FRAME Dialog-Frame:SCROLLABLE       = FALSE
       FRAME Dialog-Frame:HIDDEN           = TRUE.

ASSIGN 
       BROWSE-Sprak:MAX-DATA-GUESS IN FRAME Dialog-Frame         = 481.
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE BROWSE-Sprak
/* Query rebuild information for BROWSE BROWSE-Sprak
     _TblList          = "SkoTex.sprak"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _FldNameList[1]   > SkoTex.sprak.Lng
"sprak.Lng" ? ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   = SkoTex.sprak.Beskrivelse
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Språk */
DO:
...
&Scoped-define BROWSE-NAME BROWSE-Sprak
&Scoped-define SELF-NAME BROWSE-Sprak
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Sprak Dialog-Frame
ON ANY-PRINTABLE OF BROWSE-Sprak IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Sprak Dialog-Frame
ON CURSOR-LEFT OF BROWSE-Sprak IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Sprak Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-Sprak IN FRAME Dialog-Frame
DO:
  IF wAntSortCol > 1 THEN
      RUN SD-CURSOR ("RIGHT").
  RETURN NO-APPLY.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Sprak Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-Sprak IN FRAME Dialog-Frame
DO:
  IF wType = "V" THEN
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Sprak Dialog-Frame
ON HOME OF BROWSE-Sprak IN FRAME Dialog-Frame
DO:
  APPLY "ENTRY" TO SELF.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Sprak Dialog-Frame
ON START-SEARCH OF BROWSE-Sprak IN FRAME Dialog-Frame
DO:
