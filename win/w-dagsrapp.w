...
DEF VAR cAvdListe     AS CHAR NO-UNDO.
...
DEFINE BUTTON B-Jmf  NO-FOCUS
     LABEL "Sammenligningsrapport ..." 
     SIZE 31 BY 1.1 TOOLTIP "Sammenlign 2 perioder, Excel"
     FONT 6.
...
DEFINE FRAME DEFAULT-FRAME
     B-SpinFraUp AT ROW 1.1 COL 70
     B-Jmf AT ROW 1.19 COL 18
     B-SpinTilUp AT ROW 1.14 COL 98
     B-Butikker AT ROW 1.19 COL 108.6
     FI-FraDat AT ROW 1.24 COL 52 COLON-ALIGNED
     FI-TilDat AT ROW 1.24 COL 79.8 COLON-ALIGNED
     FI-Butiker AT ROW 1.24 COL 121.2 COLON-ALIGNED NO-LABEL
     B-SpinFraDown AT ROW 1.71 COL 70
     B-SpinTilDown AT ROW 1.76 COL 98
     ...
     BUTTON-SokDato AT ROW 1.24 COL 72.8
     BUTTON-SokDato-2 AT ROW 1.24 COL 100.6
...
ON CHOOSE OF B-Jmf IN FRAME DEFAULT-FRAME /* Sammenligningsrapport ... */
DO:
...
/*     END.                                             */
  END.
  RUN AckumJmf(FI-FraDat,FI-TilDat,dJmfStart).
END.
...
OCXFile = SEARCH( "w-dagsrapp.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-dagsrapp.wrx":U SKIP(1)
...
ELSE
  wMva% = 0.
    ...
    DO wCount = 1 TO NUM-ENTRIES(wvButiker):
        ...
        /* Akkumulering av statistikk */
        FOR EACH Avdeling NO-LOCK WHERE
            Avdeling.AvdelingNr > 0:
          DO dDato = FI-FraDat TO FI-TilDat:
            /* Henter juliansk dato */
            iDato = dDato - (DATE(01,01,YEAR(dDato)) - 1).

            FOR EACH StLinje NO-LOCK WHERE
              StLinje.Butik       = INT(ENTRY(wCount,wvButiker)) AND
              StLinje.StType      = 'AVDELING' AND
              StLinje.PerId       = 'DAG' AND 
              StLinje.AarPerLinNr = int(STRING(year(dDato),"9999") + STRING(iDato,"999")) AND
              StLinje.DataObjekt  = string(Avdeling.AvdelingNr,"9999"):

              CASE Avdeling.AvdelingNr:
                  WHEN 1 THEN ASSIGN 
                      wTot[1]  = wTot[1]  + StLinje.VerdiSolgt + StLinje.MvaVerdi
                      wTot[10] = wTot[10] + StLinje.VerdiSolgt - StLinje.VVareKost
                      chGrid:TextMatrix(wRow,10) = INT(chGrid:TextMatrix(wRow,10)) + (StLinje.VerdiSolgt - StLinje.VVareKost)
                      chGrid:TextMatrix(wRow,1) = INT(chGrid:TextMatrix(wRow,1)) + (StLinje.VerdiSolgt + StLinje.MvaVerdi).
                  WHEN 2 THEN assign
                      wTot[2]  = wTot[2]  + StLinje.VerdiSolgt  + StLinje.MvaVerdi
                      wTot[11] = wTot[11] + StLinje.VerdiSolgt - StLinje.VVareKost
                      chGrid:TextMatrix(wRow,11) = INT(chGrid:TextMatrix(wRow,11)) + (StLinje.VerdiSolgt - StLinje.VVareKost)
                      chGrid:TextMatrix(wRow,2) = INT(chGrid:TextMatrix(wRow,2)) + (StLinje.VerdiSolgt + StLinje.MvaVerdi).
                  WHEN 3 THEN assign
                      wTot[3]  = wTot[3]  + StLinje.VerdiSolgt  + StLinje.MvaVerdi
                      wTot[12] = wTot[12] + StLinje.VerdiSolgt - StLinje.VVareKost
                      chGrid:TextMatrix(wRow,12) = INT(chGrid:TextMatrix(wRow,12)) + (StLinje.VerdiSolgt - StLinje.VVareKost)
                      chGrid:TextMatrix(wRow,3) = INT(chGrid:TextMatrix(wRow,3)) + (StLinje.VerdiSolgt + StLinje.MvaVerdi).
                  WHEN 4 THEN ASSIGN
                      wTot[4]  = wTot[4]  + StLinje.VerdiSolgt + StLinje.MvaVerdi
                      wTot[13] = wTot[13] + StLinje.VerdiSolgt - StLinje.VVareKost
                      chGrid:TextMatrix(wRow,13) = INT(chGrid:TextMatrix(wRow,13)) + (StLinje.VerdiSolgt - StLinje.VVareKost)
                      chGrid:TextMatrix(wRow,4) = INT(chGrid:TextMatrix(wRow,4)) + (StLinje.VerdiSolgt + StLinje.MvaVerdi).
                  WHEN 5 THEN assign
                      wTot[5]  = wTot[5]  + StLinje.VerdiSolgt + StLinje.MvaVerdi
                      wTot[14] = wTot[14] + StLinje.VerdiSolgt - StLinje.VVareKost
                      chGrid:TextMatrix(wRow,14) = INT(chGrid:TextMatrix(wRow,14)) + (StLinje.VerdiSolgt - StLinje.VVareKost)
                      chGrid:TextMatrix(wRow,5) = INT(chGrid:TextMatrix(wRow,5)) + (StLinje.VerdiSolgt + StLinje.MvaVerdi).
                  WHEN 6 THEN assign
                      wTot[6]  = wTot[6]  + StLinje.VerdiSolgt  + StLinje.MvaVerdi
                      wTot[15] = wTot[15] + StLinje.VerdiSolgt - StLinje.VVareKost
                      chGrid:TextMatrix(wRow,15) = INT(chGrid:TextMatrix(wRow,15)) + (StLinje.VerdiSolgt - StLinje.VVareKost)
                      chGrid:TextMatrix(wRow,6) = INT(chGrid:TextMatrix(wRow,6)) + (StLinje.VerdiSolgt + StLinje.MvaVerdi).
                  WHEN 7 THEN assign
                      wTot[7]  = wTot[7]  + StLinje.VerdiSolgt + StLinje.MvaVerdi
                      wTot[16] = wTot[16] + StLinje.VerdiSolgt - StLinje.VVareKost
                      chGrid:TextMatrix(wRow,16) = INT(chGrid:TextMatrix(wRow,16)) + (StLinje.VerdiSolgt - StLinje.VVareKost)
                      chGrid:TextMatrix(wRow,7) = INT(chGrid:TextMatrix(wRow,7)) + (StLinje.VerdiSolgt + StLinje.MvaVerdi).
                  WHEN 8 THEN assign
                      wTot[8]  = wTot[8]  + StLinje.VerdiSolgt + StLinje.MvaVerdi
                      wTot[17] = wTot[17] + StLinje.VerdiSolgt - StLinje.VVareKost 
                      chGrid:TextMatrix(wRow,17) = INT(chGrid:TextMatrix(wRow,17)) + (StLinje.VerdiSolgt - StLinje.VVareKost)
                      chGrid:TextMatrix(wRow,8) = INT(chGrid:TextMatrix(wRow,8)) + (StLinje.VerdiSolgt + StLinje.MvaVerdi).
                  WHEN 9 THEN assign
                      wTot[9]  = wTot[9]  + StLinje.VerdiSolgt  + StLinje.MvaVerdi
                      wTot[18] = wTot[18] + StLinje.VerdiSolgt - StLinje.VVareKost 
                      chGrid:TextMatrix(wRow,18) = INT(chGrid:TextMatrix(wRow,18)) + (StLinje.VerdiSolgt - StLinje.VVareKost)
                      chGrid:TextMatrix(wRow,9) = INT(chGrid:TextMatrix(wRow,9)) + (StLinje.VerdiSolgt + StLinje.MvaVerdi).
              END CASE.
              ASSIGN 
                  chGrid:TextMatrix(wRow,28) = INT(chGrid:TextMatrix(wRow,28)) + StLinje.VerdiSolgt + StLinje.MvaVerdi
                  chGrid:TextMatrix(wRow,29) = INT(chGrid:TextMatrix(wRow,29)) + (StLinje.VerdiSolgt - StLinje.VVareKost)
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE InitAvdeling C-Win 
PROCEDURE InitAvdeling :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH Avdeling NO-LOCK WHERE
    ...
    ASSIGN
        cAvdListe = cAvdListe + 
                    (IF cAvdListe <> '' THEN '|' ELSE '') +
                    substring(Avdeling.AvdelingNavn,1,8).
...
/*            chGrid:TextMatrix(0,1) = entry(1,cAvdListe,'|') /* "Oms Avd 1" */ */
/*            chGrid:TextMatrix(0,2) = entry(2,cAvdListe,'|') /* "Oms Avd 2" */ */
/*            chGrid:TextMatrix(0,3) = entry(3,cAvdListe,'|') /* "Oms Avd 3" */ */
/*            chGrid:TextMatrix(0,4) = entry(4,cAvdListe,'|') /* "Oms Avd 4" */ */
/*            chGrid:TextMatrix(0,5) = entry(5,cAvdListe,'|') /* "Oms Avd 5" */ */
/*            chGrid:TextMatrix(0,6) = entry(6,cAvdListe,'|') /* "Oms Avd 6" */ */
/*            chGrid:TextMatrix(0,7) = entry(7,cAvdListe,'|') /* "Oms Avd 7" */ */
/*            chGrid:TextMatrix(0,8) = entry(8,cAvdListe,'|') /* "Oms Avd 8" */ */
/*            chGrid:TextMatrix(0,9) = entry(9,cAvdListe,'|') /* "Oms Avd 9" */ */
