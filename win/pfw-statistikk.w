...
&Scoped-Define ENABLED-OBJECTS RECT-57 RECT-58 ED-Logg B-Start BtnDone 
...
DEFINE BUTTON B-Start 
     LABEL "Start oppdatering av statistikk..." 
     SIZE 45 BY 1.14.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Integrasjon ProfitBase - Oppdatering av statistikk"
         HEIGHT             = 18.19
         WIDTH              = 94
         MAX-HEIGHT         = 18.19
         MAX-WIDTH          = 101.6
...
ON END-ERROR OF C-Win /* Integrasjon ProfitBase - Oppdatering av statistikk */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Integrasjon ProfitBase - Oppdatering av statistikk */
DO:
...
ON CHOOSE OF B-Start IN FRAME DEFAULT-FRAME /* Start oppdatering av statistikk... */
DO:
  ASSIGN
    bSvar = FALSE
    .
  MESSAGE "Skal oppdatering av statistikk for ProfitBase startes?"
  ...
  DO:
    {sww.i}
    RUN PBR ("*CLEAR*").
    RUN PBR ("Starter oppdatering...").
    RUN pfxoppdatstat.p.
    RUN PBR ("Oppdatering ferdig.").
    {swn.i}
  END.
END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-57 RECT-58 ED-Logg B-Start BtnDone 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ASSIGN
      ED-Logg = " " + STRING(iAnt) + " " + pcTekst + CHR(10) + 
                ED-Logg
      .
    IF iAnt MOD 150 = 0 THEN
        ASSIGN pcTekst = "*CLEAR*".
