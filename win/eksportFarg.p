...
DEFINE VARIABLE cExportFil      AS CHARACTER INIT "farge."                NO-UNDO.
...
IF cLanButiker <> "" THEN DO:
  DO iCount = 1 TO NUM-ENTRIES(cLanButiker):
      RUN ExportFarg IN THIS-PROCEDURE (INT(ENTRY(iCount,cLanButiker)),"txt"). /* parameter = den loopade butiken */
  END.
END.
  ...
  IF CAN-FIND(FIRST TT_ELogg WHERE TT_ELogg.TabellNavn = "Farg" AND
      ...
      FOR EACH Farg NO-LOCK:
          ...
          ASSIGN TT_Farge.fargenr    = Farg.Farg
                 TT_Farge.fargetekst = SUBSTR(Farg.FarBeskr,1,30) /* Farg.KFarge  */
                 TT_Farge.aksjon  = 1.
      END.
  END.
  ELSE DO:
      FOR EACH TT_ELogg WHERE 
          ...
          IF AVAIL Farg THEN DO:
              CREATE TT_Farge.
              ASSIGN TT_Farge.fargenr = Farg.Farg
                     TT_Farge.fargetekst = SUBSTR(Farg.FarBeskr,1,30)
                     TT_Farge.aksjon  = 1.
          END.
      END.
  END.
