...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEFINE VAR wFylkesNr LIKE Fylke.FylkesNr NO-UNDO.
&ELSE
  DEFINE INPUT-OUTPUT PARAMETER wFylkesNr LIKE Fylke.FylkesNr NO-UNDO.
&ENDIF
...
&scoped-define index-col1  FylkesNr
...
&scoped-define ip-variabel wFylkesNr
&scoped-define return-ip   wFylkesNr = if available Fylke ~
                                         then Fylke.FylkesNr ~
                                         else "".
&scoped-define assign-retur-verdi retur-verdi = if available Fylke ~
                                                  then Fylke.Beskrivelse ~
                                                  else "".
&scoped-define SletteMelding "Skal posten slettes?"
&scoped-define KanSlettes if can-find(first Post where ~
                                       Post.Fylke = Fylke.Fylke) then ~
                            do: ~
                              message "Dette fylke er i bruk. Kan ikke slettes!" ~
...
&Scoped-define QUERY-STRING-BROWSE-Fylke FOR EACH Fylke NO-LOCK ~
...
FILL-IN-SOK-CHAR BUTTON-Sok BROWSE-Fylke BUTTON-Ny BUTTON-Endre ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Fylke Dialog-Frame _STRUCTURED
  QUERY BROWSE-Fylke NO-LOCK DISPLAY
      Fylke.FylkesNr COLUMN-LABEL "FNr *" FORMAT "X(8)":U
      Fylke.Beskrivelse COLUMN-LABEL "Fylkesnavn *" FORMAT "X(30)":U
  ENABLE
      Fylke.FylkesNr
      Fylke.Beskrivelse
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 43 BY 7.71.
...
DEFINE FRAME Dialog-Frame
     ...
     BROWSE-Fylke AT ROW 2.91 COL 2
     BUTTON-Ny AT ROW 2.91 COL 45.6
     BUTTON-Endre AT ROW 4.24 COL 45.6
     BUTTON-Slett AT ROW 5.57 COL 45.6
     Btn_Help AT ROW 10.71 COL 45.6
     Btn_OK AT ROW 10.81 COL 2.2
     Btn_Cancel AT ROW 10.81 COL 14.6
     SPACE(34.00) SKIP(0.08)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste fylkesregister"
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
                                                                        */
/* BROWSE-TAB BROWSE-Fylke BUTTON-Sok Dialog-Frame */
ASSIGN 
...
"Fylke.FylkesNr" "FNr *" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > SkoTex.Fylke.Beskrivelse
"Fylke.Beskrivelse" "Fylkesnavn *" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-Fylke */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Søkeliste fylkesregister */
DO:
...
ON ANY-PRINTABLE OF BROWSE-Fylke IN FRAME Dialog-Frame
DO:
...
DO:
  ...
  run d-v{&br-tabell}.w (input-output wRecid,"Endre").
...
DO:
  assign 
    wRecid = ?.
  run d-v{&br-tabell}.w (input-output wRecid,"Ny").
  if return-value = "AVBRYT" then
    return no-apply.
...
{syspara.i 16 25 2 iTilgang INT}
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-INT FILL-IN-SOK-DATE FILL-IN-SOK-CHAR BUTTON-Sok 
         BROWSE-Fylke BUTTON-Ny BUTTON-Endre BUTTON-Slett Btn_Help Btn_OK 
         Btn_Cancel 
      WITH FRAME Dialog-Frame.
