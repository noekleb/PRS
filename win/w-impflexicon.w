...
DEFINE VARIABLE FI-Tittel AS CHARACTER FORMAT "X(256)":U INITIAL "    Import fra eXcellence PRO" 
      VIEW-AS TEXT 
     SIZE 77 BY 1.38
...
DEFINE FRAME DEFAULT-FRAME
     ...
     FI-Tittel AT ROW 1.71 COL 1 COLON-ALIGNED NO-LABEL
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Import av ordre fra eXcellence PRO innkjøpssystem"
         HEIGHT             = 16.19
         WIDTH              = 80
         MAX-HEIGHT         = 16.19
         MAX-WIDTH          = 80
...
ON END-ERROR OF C-Win /* Import av ordre fra eXcellence PRO innkjøpssystem */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Import av ordre fra eXcellence PRO innkjøpssystem */
DO:
...
DO:
  ...
  if B-Start:private-data = "OPPDAT" then
    do:
      ...
      run x-impflexicon.w persistent set wProgram-Handle (this-procedure:handle).      
    end.
...
DO:
  ...
  def var wDummy           as char no-undo.
  
  /* Filnavn */
  {syspara.i 50 10 2 wPluFil}
  if wPluFil = "" then
    assign
      wPluFil = "dataout.plu".

  /* Katalognavn */
  {syspara.i 50 10 1 wKatalog}
  if wKatalog = "" then
