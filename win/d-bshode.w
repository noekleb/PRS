...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEFINE VAR wSysHId LIKE SysHode.SysHId NO-UNDO.
&ELSE
...
&scoped-define br-browse   BROWSE-SysHode
...
&scoped-define Edit-Prog   d-vsyshode.w
&scoped-define ip-variabel wSysHId
&scoped-define return-ip   wSysHId = if available SysHode ~
                                       then SysHode.SysHId ~
                                       else 0.
&scoped-define assign-retur-verdi retur-verdi = if available SysHode ~
                                                  then string(recid(SysHode)) ~
                                                  else " ".
&scoped-define ant-sok-field   2
&scoped-define init-datatype   INT /* CHAR, INT, DATE */
&scoped-define Del-Can-Find first SysGruppe where SysGruppe.SysHId = SysHode.SysHId
&scoped-define Del-Message message "Det ligger parametergrupper under denne modulen. Kan ikke slettes!" ~
...
FILL-IN-SOK-DATE BUTTON-Sok BUTTON-Ny BROWSE-SysHode BUTTON-Endre ~
BUTTON-Slett BUTTON-Detaljer Btn_OK Btn_Help 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-SysHode Dialog-Frame _STRUCTURED
  QUERY BROWSE-SysHode NO-LOCK DISPLAY
      SysHode.SysHId COLUMN-LABEL "Modul *"
      SysHode.Beskrivelse COLUMN-LABEL "Beskrivelse *"
      SysHode.Hjelpetekst
  ENABLE
      SysHode.SysHId
      SysHode.Beskrivelse
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 42.2 BY 7.38.
...
DEFINE FRAME Dialog-Frame
     ...
     BUTTON-Ny AT ROW 2.81 COL 44.8
     BROWSE-SysHode AT ROW 2.91 COL 2
     BUTTON-Endre AT ROW 4.14 COL 44.8
     BUTTON-Slett AT ROW 5.48 COL 44.8
     BUTTON-Detaljer AT ROW 7.29 COL 44.8
     Btn_OK AT ROW 10.81 COL 2.2
     Btn_Help AT ROW 10.81 COL 44.8
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Systemparameter - Modul"
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
                                                                        */
/* BROWSE-TAB BROWSE-SysHode BUTTON-Ny Dialog-Frame */
ASSIGN 
       FRAME Dialog-Frame:SCROLLABLE       = FALSE
       FRAME Dialog-Frame:HIDDEN           = TRUE.

ASSIGN 
       BROWSE-SysHode:NUM-LOCKED-COLUMNS IN FRAME Dialog-Frame = 1.
...
"SysHode.SysHId" "Modul *" ? "integer" ? ? ? ? ? ? yes ? no no ?
     _FldNameList[2]   > SkoTex.SysHode.Beskrivelse
"SysHode.Beskrivelse" "Beskrivelse *" ? "character" ? ? ? ? ? ? yes ? no no ?
     _FldNameList[3]   = SkoTex.SysHode.Hjelpetekst
     _Query            is OPENED
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Systemparameter - Modul */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-SysHode Dialog-Frame
ON ANY-PRINTABLE OF BROWSE-SysHode IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-SysHode Dialog-Frame
ON CURSOR-LEFT OF BROWSE-SysHode IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-SysHode Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-SysHode IN FRAME Dialog-Frame
DO:
  IF {&ant-sok-field} = 1 THEN
    RETURN NO-APPLY.
  RUN SD-CURSOR ("RIGHT").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-SysHode Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-SysHode IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-SysHode Dialog-Frame
ON START-SEARCH OF BROWSE-SysHode IN FRAME Dialog-Frame
DO:
  RUN SD-START-SEARCH.
  RETURN NO-APPLY.
END.
...
DO:
  ...
  run d-bsgruppe.w (input-output wRecid).
  return no-apply.
END.
...
DO:
  ...
  assign
    wRecid = recid({&br-tabell}).
  run {&Edit-Prog} (input-output wRecid,"Endre").
...
DO:
  assign 
    wRecid = ?.
  run {&Edit-Prog} (input-output wRecid,"Ny").
  if return-value = "AVBRYT" then
    return no-apply.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-CHAR FILL-IN-SOK-INT FILL-IN-SOK-DATE BUTTON-Sok BUTTON-Ny 
         BROWSE-SysHode BUTTON-Endre BUTTON-Slett BUTTON-Detaljer Btn_OK 
         Btn_Help 
      WITH FRAME Dialog-Frame.
