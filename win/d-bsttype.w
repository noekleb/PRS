...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEFINE VAR wStTypeId LIKE StType.StTypeId NO-UNDO.
&ELSE
  DEFINE INPUT-OUTPUT PARAMETER wStTypeId LIKE StType.StTypeId NO-UNDO.
&ENDIF

/* Preprossessor direktiver ---                                         */
&scoped-define br-tabell   StType

/* Kolonne 1 som det kan sorteres på */
&scoped-define sok-field1  StTypeId
...
&scoped-define index-col1  StTypeId
...
&scoped-define ip-variabel wStTypeId
&scoped-define return-ip   wStTypeId = if available StType ~
                                          then StType.StTypeId ~
                                          else "".
&scoped-define assign-retur-verdi retur-verdi = if available StType ~
                                                  then string(recid(StType)) ~
                                                  else "".
...
&scoped-define KanSlettes ~
  if can-find(first StDef of StType) then ~
    do: ~
      message "Det finns statistiker som använder denna koden" ~
      view-as alert-box title "Fel vid borttagning". ~
      return no-apply. ~
    end.

&scoped-define VedlikeholdsRutine d-vsttype.w
...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME BROWSE-StType

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES StType

/* Definitions for BROWSE BROWSE-StType                                 */
&Scoped-define FIELDS-IN-QUERY-BROWSE-StType StType.StTypeId ~
StType.Beskrivelse 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-StType StType.StTypeId ~
StType.Beskrivelse 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-StType StType
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-StType StType
&Scoped-define OPEN-QUERY-BROWSE-StType OPEN QUERY BROWSE-StType FOR EACH StType NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-StType StType
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-StType StType


/* Definitions for DIALOG-BOX Dialog-Frame                              */
&Scoped-define OPEN-BROWSERS-IN-QUERY-Dialog-Frame ~
    ~{&OPEN-QUERY-BROWSE-StType}

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS FILL-IN-SOK-CHAR FILL-IN-SOK-INT ~
FILL-IN-SOK-DATE BUTTON-Sok BROWSE-StType BUTTON-Ny BUTTON-Endre ~
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY BROWSE-StType FOR 
      StType SCROLLING.
&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE BROWSE-StType
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-StType Dialog-Frame _STRUCTURED
  QUERY BROWSE-StType NO-LOCK DISPLAY
      StType.StTypeId WIDTH 19.2
      StType.Beskrivelse WIDTH 45
  ENABLE
      StType.StTypeId
      StType.Beskrivelse
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 70 BY 10.67 ROW-HEIGHT-CHARS .63.
...
DEFINE FRAME Dialog-Frame
     ...
     BROWSE-StType AT ROW 2.71 COL 2
     BUTTON-Ny AT ROW 2.91 COL 74.2
     BUTTON-Endre AT ROW 4.24 COL 74.2
     BUTTON-Slett AT ROW 5.57 COL 74.2
     Btn_OK AT ROW 13.86 COL 2
     Btn_Cancel AT ROW 13.86 COL 14.4
     Btn_Help AT ROW 13.86 COL 74
     SPACE(1.19) SKIP(0.00)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste statistikktyper"
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
                                                                        */
/* BROWSE-TAB BROWSE-StType BUTTON-Sok Dialog-Frame */
ASSIGN 
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE BROWSE-StType
/* Query rebuild information for BROWSE BROWSE-StType
     _TblList          = "SkoTex.StType"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _FldNameList[1]   > SkoTex.StType.StTypeId
"StType.StTypeId" ? ? "character" ? ? ? ? ? ? yes ? no no "19.2"
     _FldNameList[2]   > SkoTex.StType.Beskrivelse
"StType.Beskrivelse" ? ? "character" ? ? ? ? ? ? yes ? no no "45"
     _Query            is OPENED
*/  /* BROWSE BROWSE-StType */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Søkeliste statistikktyper */
DO:
...
&Scoped-define BROWSE-NAME BROWSE-StType
&Scoped-define SELF-NAME BROWSE-StType
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-StType Dialog-Frame
ON ANY-PRINTABLE OF BROWSE-StType IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-StType Dialog-Frame
ON CURSOR-LEFT OF BROWSE-StType IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-StType Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-StType IN FRAME Dialog-Frame
DO:
  IF {&ant-sok-field} = 1 THEN
    RETURN NO-APPLY.
  RUN SD-CURSOR ("RIGHT").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-StType Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-StType IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-StType Dialog-Frame
ON START-SEARCH OF BROWSE-StType IN FRAME Dialog-Frame
DO:
  RUN SD-START-SEARCH.
  RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-CHAR FILL-IN-SOK-INT FILL-IN-SOK-DATE BUTTON-Sok 
         BROWSE-StType BUTTON-Ny BUTTON-Endre BUTTON-Slett Btn_OK Btn_Cancel 
         Btn_Help 
      WITH FRAME Dialog-Frame.
