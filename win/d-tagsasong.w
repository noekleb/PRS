...
DEFINE TEMP-TABLE tSasong NO-UNDO LIKE SaSong.
...
&scoped-define DB-BROWSER BROWSE-Sasong
...
&scoped-define BUFFER-COPY-USING USING SasBeskr
...
&Scoped-define INTERNAL-TABLES SaSong tSasong
...
&Scoped-define FIELDS-IN-QUERY-BROWSE-Tag tSasong.Sasong tSasong.SasBeskr 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-Tag tSasong.Sasong 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-Tag tSasong
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-Tag tSasong
&Scoped-define OPEN-QUERY-BROWSE-Tag OPEN QUERY BROWSE-Tag FOR EACH tSasong NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-Tag tSasong
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-Tag tSasong
...
&Scoped-Define ENABLED-OBJECTS RECT-4 RECT-5 BROWSE-Sasong BROWSE-Tag ~
...
DEFINE QUERY BROWSE-Sasong FOR 
      SaSong
    FIELDS(SaSong.Sasong
      SaSong.SasBeskr) SCROLLING.

DEFINE QUERY BROWSE-Tag FOR 
      tSasong
    FIELDS(tSasong.Sasong
      tSasong.SasBeskr) SCROLLING.
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Sasong Dialog-Frame _STRUCTURED
  QUERY BROWSE-Sasong NO-LOCK DISPLAY
      SaSong.Sasong FORMAT "zzzzz9"
      SaSong.SasBeskr FORMAT "x(30)"
  ENABLE
      SaSong.Sasong
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Tag Dialog-Frame _STRUCTURED
  QUERY BROWSE-Tag NO-LOCK DISPLAY
      tSasong.Sasong FORMAT "zzzzz9"
      tSasong.SasBeskr FORMAT "x(30)"
  ENABLE
      tSasong.Sasong
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME Dialog-Frame
     BROWSE-Sasong AT ROW 2.91 COL 3
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: tSasong T "?" NO-UNDO skotex SaSong
   END-TABLES.
 */
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   Custom                                                               */
/* BROWSE-TAB BROWSE-Sasong 1 Dialog-Frame */
...
"Sasong" ? "zzzzz9" "integer" ? ? ? ? ? ? yes ? no no ?
     _FldNameList[2]   > skotex.SaSong.SasBeskr
"SasBeskr" ? "x(30)" "character" ? ? ? ? ? ? no ? no no ?
     _Query            is OPENED
...
/* Query rebuild information for BROWSE BROWSE-Tag
     _TblList          = "Temp-Tables.tSasong"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _TblOptList       = "USED"
     _FldNameList[1]   > Temp-Tables.tSasong.Sasong
"Sasong" ? "zzzzz9" "integer" ? ? ? ? ? ? yes ? no no ?
     _FldNameList[2]   > Temp-Tables.tSasong.SasBeskr
"SasBeskr" ? "x(30)" "character" ? ? ? ? ? ? no ? no no ?
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Velg sesonger */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Sasong Dialog-Frame
ON VALUE-CHANGED OF BROWSE-Sasong IN FRAME Dialog-Frame
DO:
    IF BROWSE BROWSE-Tag:NUM-SELECTED-ROWS > 0 THEN DO:
        IF BROWSE BROWSE-Tag:DESELECT-ROWS() THEN.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-4 RECT-5 BROWSE-Sasong BROWSE-Tag Btn_OK Btn_Cancel Btn_Help 
         BUTTON-Sok 
      WITH FRAME Dialog-Frame.
