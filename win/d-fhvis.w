...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN   
   ...
   DEF VAR wRappNavn AS CHAR NO-UNDO INIT "Såldeprøverapport".
...
&Scoped-Define ENABLED-OBJECTS Btn_Done EDITOR-fv Btn_Help-2 ~
BUTTON-FontZoom BUTTON-Kopier BUTTON-LagreSom BUTTON-SkrivUt BUTTON-Sok 
...
DEFINE FRAME Dialog-Frame
     Btn_Done AT ROW 1.24 COL 128
     EDITOR-fv AT ROW 2.43 COL 2 NO-LABEL
     Btn_Help-2 AT ROW 1.24 COL 123
     BUTTON-FontZoom AT ROW 1.24 COL 23
     BUTTON-Kopier AT ROW 1.24 COL 12
     BUTTON-LagreSom AT ROW 1.24 COL 7
     BUTTON-SkrivUt AT ROW 1.24 COL 2
     BUTTON-Sok AT ROW 1.24 COL 18
     SPACE(110.40) SKIP(18.71)
...
DO:
  ...
  SYSTEM-DIALOG GET-FILE wFile USE-FILENAME SAVE-AS ASK-OVERWRITE RETURN-TO-START-DIR INITIAL-DIR wDir Update wOk.
  if wOk THEN
  DO WITH FRAME {&FRAME-NAME}:
...
------------------------------------------------------------------------------*/
  ENABLE Btn_Done EDITOR-fv Btn_Help-2 BUTTON-FontZoom BUTTON-Kopier 
         BUTTON-LagreSom BUTTON-SkrivUt BUTTON-Sok 
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    IF NOT wFilnavn BEGINS "<" THEN DO:
       ...
       IF ERROR-STATUS:NUM-MESSAGES > 0 THEN DO:
          ...
          ELSE MESSAGE "Kan ikke lese utskriftsfil " wFilNavn + "." 
                  VIEW-AS ALERT-BOX ERROR TITLE "Feil".
          PROCESS EVENTS.
          RETURN "Feil".
...
------------------------------------------------------------------------------*/
   ...
   RUN d-skrivfil.w(wValg,wUtenhet,wSkrNr,wFilnavn,FRAME {&FRAME-NAME}:TITLE,"{&TomMelding}" + " (Jobbnr. " + STRING(wJobbNr) + ")").
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  DO WITH FRAME {&FRAME-NAME}:
     ...
     RUN d-soketter.w(EDITOR-fv:HANDLE,INPUT-OUTPUT wSokeTekst,OUTPUT wSokeFlagg).
  END.   
