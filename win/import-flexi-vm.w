...
DEF VAR lVmNye           AS   LOG                NO-UNDO.
DEF VAR lVmOverskriv     AS   LOG                NO-UNDO.
...
{syspara.i 1 2 1007 cEDB-System}
{syspar2.i 1 2 1007 cImpTabell}
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH ImportLinje EXCLUSIVE-LOCK WHERE
      ...
      IF AVAILABLE ImpKonv THEN
        ASSIGN
          piVm = int(ImpKonv.InterntId)
          .

      FIND Varemerke EXCLUSIVE-LOCK WHERE
          VareMerke.VmId = piVm NO-ERROR.
      
      /* Er det ny leverandør, sjekk om den skal opprettes ? */
      IF lVmNye = FALSE AND NOT AVAILABLE Varemerke THEN
          NEXT MAINLOOP.
      ...
      DO:
          CREATE Varemerke.
          ASSIGN
              /* '#KOD' */ Varemerke.VmId = piVm
              .
      END.
      /* Hvis det ikke er en ny varegrupper, skal data overskrives ? */
      IF lVmOverskriv = FALSE AND NOT NEW VareMerke THEN
          NEXT MAINLOOP.
      ASSIGN
        /* '#LEVKOD'   */
        /* '#BESK'     */  Varemerke.Beskrivelse = ImportLinje.Felt[3]
                           Varemerke.KortNavn    = ImportLinje.Felt[3]
        /* '#KOSTNAD'  */
        .
  END. /* MAINLOOP */
END PROCEDURE.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEF INPUT PARAMETER plVmNye       AS LOG NO-UNDO.
  DEF INPUT PARAMETER plVmOverskriv AS LOG NO-UNDO.
  
  ASSIGN
    lVmNye       = plVmNye      
    lVmOverskriv = plVmOverskriv
    .
END PROCEDURE.
