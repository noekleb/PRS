...
DEFINE SHARED TEMP-TABLE tKundeTrans NO-UNDO LIKE KundeTrans
...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEFINE VAR rRecid AS RECID NO-UNDO. 
&ELSE
  DEFINE INPUT-OUTPUT PARAMETER rRecid AS recid NO-UNDO.
&ENDIF

/* Preprossessor direktiver ---                                         */
&scoped-define br-tabell   tKundeTrans
...
&scoped-define index-col1 KundeTrans
...
&scoped-define ip-variabel rRecid
&scoped-define return-ip   rRecid = if available {&br-tabell}  ~
                                       then recid({&br-tabell}) ~
                                       else ?.

&scoped-define assign-retur-verdi 
...
DEF VAR cReferanse as char format "x(30)"  NO-UNDO.
DEF VAR cSaldo     as char format "x(15)"  NO-UNDO.
...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME BROWSE-tKundeTrans

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES tKundeTrans

/* Definitions for BROWSE BROWSE-tKundeTrans                            */
&Scoped-define FIELDS-IN-QUERY-BROWSE-tKundeTrans tKundeTrans.SeqNr ~
cReferanse cSaldo 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-tKundeTrans 
&Scoped-define QUERY-STRING-BROWSE-tKundeTrans FOR EACH tKundeTrans NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-tKundeTrans OPEN QUERY BROWSE-tKundeTrans FOR EACH tKundeTrans NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-tKundeTrans tKundeTrans
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-tKundeTrans tKundeTrans


/* Definitions for DIALOG-BOX Dialog-Frame                              */
&Scoped-define OPEN-BROWSERS-IN-QUERY-Dialog-Frame ~
    ~{&OPEN-QUERY-BROWSE-tKundeTrans}

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS FILL-IN-SOK-CHAR FILL-IN-SOK-INT ~
FILL-IN-SOK-DATE BUTTON-Sok BROWSE-tKundeTrans Btn_OK Btn_Cancel 
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY BROWSE-tKundeTrans FOR 
      tKundeTrans SCROLLING.
&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE BROWSE-tKundeTrans
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-tKundeTrans Dialog-Frame _STRUCTURED
  QUERY BROWSE-tKundeTrans NO-LOCK DISPLAY
      tKundeTrans.SeqNr COLUMN-LABEL "Nr" FORMAT ">>>9":U
      cReferanse COLUMN-LABEL "Referanse" WIDTH 30
      cSaldo COLUMN-LABEL "Saldo" WIDTH 15
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 56 BY 13.1 ROW-HEIGHT-CHARS .62.
...
DEFINE FRAME Dialog-Frame
     ...
     BROWSE-tKundeTrans AT ROW 2.91 COL 2
     Btn_OK AT ROW 16.24 COL 2
     Btn_Cancel AT ROW 16.24 COL 46
     SPACE(0.00) SKIP(0.03)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Ubetalte fordringer"
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: tKundeTrans T "SHARED" NO-UNDO SkoTex KundeTrans
      ADDITIONAL-FIELDS:
          field Referanse as char format "x(30)"
          field Saldo     as dec  format "->>>,>>>,>>9.99"
          field Type      as int
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
                                                                        */
/* BROWSE-TAB BROWSE-tKundeTrans BUTTON-Sok Dialog-Frame */
ASSIGN 
       FRAME Dialog-Frame:SCROLLABLE       = FALSE
       FRAME Dialog-Frame:HIDDEN           = TRUE.

ASSIGN 
       BROWSE-tKundeTrans:NUM-LOCKED-COLUMNS IN FRAME Dialog-Frame     = 2.
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE BROWSE-tKundeTrans
/* Query rebuild information for BROWSE BROWSE-tKundeTrans
     _TblList          = "Temp-Tables.tKundeTrans"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _FldNameList[1]   > Temp-Tables.tKundeTrans.SeqNr
"tKundeTrans.SeqNr" "Nr" ">>>9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > "_<CALC>"
"cReferanse" "Referanse" ? ? ? ? ? ? ? ? no ? no no "30" yes no no "U" "" ""
     _FldNameList[3]   > "_<CALC>"
"cSaldo" "Saldo" ? ? ? ? ? ? ? ? no ? no no "15" yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-tKundeTrans */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Ubetalte fordringer */
DO:
...
&Scoped-define BROWSE-NAME BROWSE-tKundeTrans
&Scoped-define SELF-NAME BROWSE-tKundeTrans
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-tKundeTrans Dialog-Frame
ON ANY-PRINTABLE OF BROWSE-tKundeTrans IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-tKundeTrans Dialog-Frame
ON CURSOR-LEFT OF BROWSE-tKundeTrans IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-tKundeTrans Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-tKundeTrans IN FRAME Dialog-Frame
DO:
  IF {&ant-sok-field} = 1 THEN
    RETURN NO-APPLY.
  RUN SD-CURSOR ("RIGHT").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-tKundeTrans Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-tKundeTrans IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-tKundeTrans Dialog-Frame
ON ROW-DISPLAY OF BROWSE-tKundeTrans IN FRAME Dialog-Frame
DO:
  ASSIGN
      iLoop = iLoop + 1.
  ASSIGN
      tKundeTrans.SeqNr:SCREEN-VALUE IN BROWSE BROWSE-tKundeTrans = STRING(iLoop)
      .
  IF AVAILABLE tKundeTrans THEN
  DO:
      ASSIGN
          cReferanse = tKundeTrans.Referanse
          cSaldo     = string(tKundeTrans.Saldo,"zzz,zzz,zz9.99-")
          .
  END.
  ELSE
      ASSIGN
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-tKundeTrans Dialog-Frame
ON START-SEARCH OF BROWSE-tKundeTrans IN FRAME Dialog-Frame
DO:
  RUN SD-START-SEARCH.
  RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-CHAR FILL-IN-SOK-INT FILL-IN-SOK-DATE BUTTON-Sok 
         BROWSE-tKundeTrans Btn_OK Btn_Cancel 
      WITH FRAME Dialog-Frame.
