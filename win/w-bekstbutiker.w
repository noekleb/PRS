...
{runlib.i}
DEFINE TEMP-TABLE tmpekstbutiker LIKE ekstbutiker
...
&Scoped-define BROWSE-NAME BROWSE-Ekstbutiker
&Scoped-define QUERY-NAME QUERY-Alle

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES ekstbutiker bekstbutiker tmpekstbutiker

/* Definitions for BROWSE BROWSE-Ekstbutiker                            */
&Scoped-define FIELDS-IN-QUERY-BROWSE-Ekstbutiker ekstbutiker.Butik ~
ekstbutiker.ButNamn ekstbutiker.KortNavn ekstbutiker.KundeNr wEmpty 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-Ekstbutiker ekstbutiker.Butik ~
ekstbutiker.ButNamn 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-Ekstbutiker ekstbutiker
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-Ekstbutiker ekstbutiker
&Scoped-define QUERY-STRING-BROWSE-Ekstbutiker FOR EACH ekstbutiker NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-Ekstbutiker OPEN QUERY BROWSE-Ekstbutiker FOR EACH ekstbutiker NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-Ekstbutiker ekstbutiker
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-Ekstbutiker ekstbutiker
...
&Scoped-define QUERY-STRING-QUERY-Alle FOR EACH bekstbutiker NO-LOCK BY Butik
&Scoped-define OPEN-QUERY-QUERY-Alle OPEN QUERY {&SELF-NAME} FOR EACH bekstbutiker NO-LOCK BY Butik.
&Scoped-define TABLES-IN-QUERY-QUERY-Alle bekstbutiker
&Scoped-define FIRST-TABLE-IN-QUERY-QUERY-Alle bekstbutiker


/* Definitions for QUERY QUERY-tmp                                      */
&Scoped-define SELF-NAME QUERY-tmp
&Scoped-define QUERY-STRING-QUERY-tmp FOR EACH tmpekstbutiker NO-LOCK BY tmpekstbutiker.Ordning
&Scoped-define OPEN-QUERY-QUERY-tmp OPEN QUERY {&SELF-NAME} FOR EACH tmpekstbutiker NO-LOCK BY tmpekstbutiker.Ordning.
&Scoped-define TABLES-IN-QUERY-QUERY-tmp tmpekstbutiker
&Scoped-define FIRST-TABLE-IN-QUERY-QUERY-tmp tmpekstbutiker


/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS RECT-50 RECT-51 RECT-52 FILL-IN-SOK-LOG ~
FILL-IN-SOK-DATE FILL-IN-SOK-CHAR FILL-IN-SOK-DECI FILL-IN-SOK-INTE ~
BUTTON-Ny BUTTON-Endre BUTTON-Slett B-Excel B-Rapp B-Oppdater Btn_Help ~
Btn_OK BUTTON-Sok BROWSE-Ekstbutiker 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-SOK-LOG FILL-IN-SOK-DATE ~
FILL-IN-SOK-CHAR FILL-IN-SOK-DECI FILL-IN-SOK-INTE 
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY BROWSE-Ekstbutiker FOR 
      ekstbutiker SCROLLING.

DEFINE QUERY QUERY-Alle FOR 
      bekstbutiker SCROLLING.

DEFINE QUERY QUERY-tmp FOR 
      tmpekstbutiker SCROLLING.
&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE BROWSE-Ekstbutiker
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Ekstbutiker C-Win _STRUCTURED
  QUERY BROWSE-Ekstbutiker NO-LOCK DISPLAY
      ekstbutiker.Butik FORMAT ">>>>>9":U
      ekstbutiker.ButNamn FORMAT "x(20)":U
      ekstbutiker.KortNavn FORMAT "X(8)":U
      ekstbutiker.KundeNr FORMAT ">>>>>>>>>>>>9":U
      wEmpty
  ENABLE
      ekstbutiker.Butik
      ekstbutiker.ButNamn
...
DEFINE FRAME DEFAULT-FRAME
     ...
     BROWSE-Ekstbutiker AT ROW 6.95 COL 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Søkeliste eksterne butikker"
         HEIGHT             = 24.24
         WIDTH              = 161.6
         MAX-HEIGHT         = 39.19
         MAX-WIDTH          = 230.4
...
/* SETTINGS FOR FRAME DEFAULT-FRAME
   FRAME-NAME                                                           */
/* BROWSE-TAB BROWSE-Ekstbutiker BUTTON-Sok DEFAULT-FRAME */
ASSIGN 
...
/* SETTINGS FOR BUTTON B-Print IN FRAME DEFAULT-FRAME
   NO-ENABLE                                                            */
ASSIGN 
       BROWSE-Ekstbutiker:NUM-LOCKED-COLUMNS IN FRAME DEFAULT-FRAME     = 2
       BROWSE-Ekstbutiker:MAX-DATA-GUESS IN FRAME DEFAULT-FRAME         = 481
       BROWSE-Ekstbutiker:COLUMN-RESIZABLE IN FRAME DEFAULT-FRAME       = TRUE.
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE BROWSE-Ekstbutiker
/* Query rebuild information for BROWSE BROWSE-Ekstbutiker
     _TblList          = "SkoTex.ekstbutiker"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _FldNameList[1]   > SkoTex.ekstbutiker.Butik
"Butik" ? ? "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > SkoTex.ekstbutiker.ButNamn
"ButNamn" ? ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   = SkoTex.ekstbutiker.KortNavn
     _FldNameList[4]   = SkoTex.ekstbutiker.KundeNr
     _FldNameList[5]   > "_<CALC>"
"wEmpty" ? ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is NOT OPENED
*/  /* BROWSE BROWSE-Ekstbutiker */
&ANALYZE-RESUME

&ANALYZE-SUSPEND _QUERY-BLOCK QUERY QUERY-Alle
/* Query rebuild information for QUERY QUERY-Alle
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH bekstbutiker NO-LOCK BY Butik.
     _END_FREEFORM
...
/* Query rebuild information for QUERY QUERY-tmp
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH tmpekstbutiker NO-LOCK BY tmpekstbutiker.Ordning.
     _END_FREEFORM
...
ON END-ERROR OF C-Win /* Søkeliste eksterne butikker */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Søkeliste eksterne butikker */
DO:
...
&Scoped-define BROWSE-NAME BROWSE-Ekstbutiker
&Scoped-define SELF-NAME BROWSE-Ekstbutiker
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Ekstbutiker C-Win
ON ANY-PRINTABLE OF BROWSE-Ekstbutiker IN FRAME DEFAULT-FRAME
DO:
  IF LASTKEY <> 32 THEN
    DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Ekstbutiker C-Win
ON CURSOR-LEFT OF BROWSE-Ekstbutiker IN FRAME DEFAULT-FRAME
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Ekstbutiker C-Win
ON CURSOR-RIGHT OF BROWSE-Ekstbutiker IN FRAME DEFAULT-FRAME
DO:
  IF wAntSortCol < 2 THEN
    RETURN NO-APPLY.
  RUN SD-CURSOR ("RIGHT").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Ekstbutiker C-Win
ON DEFAULT-ACTION OF BROWSE-Ekstbutiker IN FRAME DEFAULT-FRAME
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Ekstbutiker C-Win
ON END-MOVE OF BROWSE-Ekstbutiker IN FRAME DEFAULT-FRAME
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Ekstbutiker C-Win
ON HOME OF BROWSE-Ekstbutiker IN FRAME DEFAULT-FRAME
DO:
  APPLY "ENTRY" TO SELF.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Ekstbutiker C-Win
ON START-SEARCH OF BROWSE-Ekstbutiker IN FRAME DEFAULT-FRAME
DO:
...
DO:
  ...
  RUN w-vekstbutiker PERSISTENT SET wTmpChild (RECID({&br-tabell}),"ENDRE").
  CREATE tmpChild.
...
DO:
  DEF VAR wTmpChild AS HANDLE NO-UNDO.
  RUN w-vekstbutiker PERSISTENT SET wTmpChild (?,"NY").
  CREATE tmpChild.
  ASSIGN tmpChild.wChild = wTmpChild.
  RELEASE tmpChild.
  RETURN NO-APPLY.  
...
/*
  ...
  */
  ...
  RUN w-vekstbutiker (RECID({&br-tabell}),"SLETT").
...
{genlib.i
  &NoLibCall      = "Nei"
  &WindowName     = "Søkeliste vbutikker"
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-SOK-LOG FILL-IN-SOK-DATE FILL-IN-SOK-CHAR FILL-IN-SOK-DECI 
          FILL-IN-SOK-INTE 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE RECT-50 RECT-51 RECT-52 FILL-IN-SOK-LOG FILL-IN-SOK-DATE 
         FILL-IN-SOK-CHAR FILL-IN-SOK-DECI FILL-IN-SOK-INTE BUTTON-Ny 
         BUTTON-Endre BUTTON-Slett B-Excel B-Rapp B-Oppdater Btn_Help Btn_OK 
         BUTTON-Sok BROWSE-Ekstbutiker 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR hekstbutiker       AS HANDLE NO-UNDO.
 ...
 DO WITH FRAME {&FRAME-NAME}:
  ...
  IF RETURN-VALUE = "TMP" THEN DO:
     ...
     OPEN QUERY QUERY-tmp FOR EACH tmpekstbutiker NO-LOCK BY tmpekstbutiker.Ordning.
  END.
  ELSE DO:
      ASSIGN hQuery = QUERY QUERY-Alle:HANDLE
             hekstbutiker = {&BROWSE-NAME}:QUERY.
      hQuery:QUERY-PREPARE(REPLACE(hekstbutiker:PREPARE-STRING,"ekstbutiker","bekstbutiker")).
      hQuery:QUERY-OPEN().
  END.
  DO:
...
------------------------------------------------------------------------------*/
  DEF VAR wSvar AS LOG NO-UNDO.
  DEF VAR wLoop AS INT NO-UNDO.
    EMPTY TEMP-TABLE tmpekstbutiker. /* [ NO-ERROR ] */
    wSvar = FALSE.
    IF BROWSE BROWSE-ekstbutiker:NUM-SELECTED-ROWS > 1 THEN DO:  
      MESSAGE "Skal rapport startes for de merkede butikkene?"
              VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO TITLE "Bekreft utskrift"
              UPDATE wSvar.
      IF wSvar = TRUE THEN DO:
          DO wLoop = 1 TO BROWSE BROWSE-ekstbutiker:NUM-SELECTED-ROWS:
              IF BROWSE BROWSE-ekstbutiker:FETCH-SELECTED-ROW(wLoop) THEN DO:
                  CREATE tmpekstbutiker.
                  ASSIGN tmpekstbutiker.Butik   = ekstbutiker.Butik
                         tmpekstbutiker.Ordning = wLoop.
                  RELEASE tmpekstbutiker.
              END.
          END.
          RETURN "TMP".
      END.
