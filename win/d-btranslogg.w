...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEFINE VAR wBatchNr LIKE TransLogg.BatchNr NO-UNDO.
  def    var wTitle   as   char              no-undo.
&ELSE
  DEFINE INPUT-OUTPUT PARAMETER wBatchNr LIKE TransLogg.BatchNr NO-UNDO.
...
&scoped-define br-tabell   TransLogg
&scoped-define OptWhere ~
  b{&br-tabell}.BatchNr = wBatchNr and
...
&scoped-define sok-field2  ArtikkelNr
&scoped-define sok-field2-def  INT
&scoped-define sortby-col2 {&br-tabell}.{&sok-field2}
&scoped-define sortby-phrase2 {&br-tabell}.{&sok-field2} /* DESCENDING */
&scoped-define index-col2  ArtButStr
&scoped-define sok-phrase2 FIND FIRST b{&br-tabell} WHERE ~
                b{&br-tabell}.{&sok-field2} >= INPUT FILL-IN-SOK-{&sok-field2-def} and ~
                b{&br-tabell}.BatchNr = wBatchNr ~
                USE-INDEX {&index-col2} NO-LOCK NO-ERROR.
...
TransLogg.TransNr TransLogg.Dato TransLogg.TTId TransLogg.TBId ~
TransLogg.ArtikkelNr TransLogg.Vg TransLogg.LopNr TransLogg.Storl ~
TransLogg.TilStorl TransLogg.BongId TransLogg.BongLinjeNr TransLogg.BestNr ~
TransLogg.Antall TransLogg.Pris TransLogg.RabKr TransLogg.Mva ~
TransLogg.ForsNr TransLogg.KassaNr TransLogg.KundNr TransLogg.LevNr ~
TransLogg.Plukket TransLogg.Postert TransLogg.PostertDato 
...
FILL-IN-SOK-INT BUTTON-Sok BROWSE-TransLogg Btn_Help Btn_OK Btn_Cancel 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-TransLogg Dialog-Frame _STRUCTURED
  QUERY BROWSE-TransLogg NO-LOCK DISPLAY
      ...
      TransLogg.Vg COLUMN-LABEL "Vg" FORMAT "zzz9":U WIDTH 6.4
      TransLogg.LopNr COLUMN-LABEL "LøpNr" FORMAT "zzz9":U WIDTH 6
      TransLogg.Storl FORMAT "x(4)":U
      TransLogg.TilStorl COLUMN-LABEL "FraStr" FORMAT "X(4)":U
      ...
      TransLogg.Pris FORMAT "-zz,zzz,zz9.99":U
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 150 BY 19.29.
...
DEFINE FRAME Dialog-Frame
     ...
     BROWSE-TransLogg AT ROW 2.91 COL 2
     Btn_Help AT ROW 22.43 COL 114
     Btn_OK AT ROW 22.52 COL 2
     Btn_Cancel AT ROW 22.52 COL 14.4
     SPACE(125.79) SKIP(0.04)
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
                                                                        */
/* BROWSE-TAB BROWSE-TransLogg BUTTON-Sok Dialog-Frame */
ASSIGN 
...
"Butik" "ButNr *" ? "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   = skotex.TransLogg.TransNr
     _FldNameList[3]   = skotex.TransLogg.Dato
     _FldNameList[4]   > skotex.TransLogg.TTId
"TTId" "TTId" ? "integer" ? ? ? ? ? ? no ? no no "7.2" yes no no "U" "" ""
     _FldNameList[5]   > skotex.TransLogg.TBId
"TBId" "TBId" ? "integer" ? ? ? ? ? ? no ? no no "8.2" yes no no "U" "" ""
     _FldNameList[6]   > skotex.TransLogg.ArtikkelNr
"ArtikkelNr" "Artikkelnummer *" ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[7]   > skotex.TransLogg.Vg
"Vg" "Vg" ? "integer" ? ? ? ? ? ? no ? no no "6.4" yes no no "U" "" ""
     _FldNameList[8]   > skotex.TransLogg.LopNr
"LopNr" "LøpNr" ? "integer" ? ? ? ? ? ? no ? no no "6" yes no no "U" "" ""
     _FldNameList[9]   = skotex.TransLogg.Storl
     _FldNameList[10]   > skotex.TransLogg.TilStorl
"TilStorl" "FraStr" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[11]   = skotex.TransLogg.BongId
     _FldNameList[12]   = skotex.TransLogg.BongLinjeNr
     _FldNameList[13]   = skotex.TransLogg.BestNr
     _FldNameList[14]   = skotex.TransLogg.Antall
     _FldNameList[15]   = skotex.TransLogg.Pris
     _FldNameList[16]   = skotex.TransLogg.RabKr
     _FldNameList[17]   = skotex.TransLogg.Mva
     _FldNameList[18]   = skotex.TransLogg.ForsNr
     _FldNameList[19]   = skotex.TransLogg.KassaNr
     _FldNameList[20]   = skotex.TransLogg.KundNr
     _FldNameList[21]   = skotex.TransLogg.LevNr
     _FldNameList[22]   = skotex.TransLogg.Plukket
     _FldNameList[23]   = skotex.TransLogg.Postert
     _FldNameList[24]   = skotex.TransLogg.PostertDato
     _Query            is OPENED
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Søkeliste transaksjonslogg */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-TransLogg Dialog-Frame
ON ANY-PRINTABLE OF BROWSE-TransLogg IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-TransLogg Dialog-Frame
ON CURSOR-LEFT OF BROWSE-TransLogg IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-TransLogg Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-TransLogg IN FRAME Dialog-Frame
DO:
  IF {&ant-sok-field} = 1 THEN
    RETURN NO-APPLY.
  RUN SD-CURSOR ("RIGHT").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-TransLogg Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-TransLogg IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-TransLogg Dialog-Frame
ON START-SEARCH OF BROWSE-TransLogg IN FRAME Dialog-Frame
DO:
  RUN SD-START-SEARCH.
  RETURN NO-APPLY.
END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ON END-KEY UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK:
    ...
    assign
      frame Dialog-Frame:title = "Transaksjoner tilhørende batch " +
                                 string(wBatchNr) + " " + 
                                  wTitle.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-CHAR FILL-IN-SOK-DATE FILL-IN-SOK-INT BUTTON-Sok 
         BROWSE-TransLogg Btn_Help Btn_OK Btn_Cancel 
      WITH FRAME Dialog-Frame.
