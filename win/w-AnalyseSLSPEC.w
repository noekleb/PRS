...
B-Summera CB-Analyse FI-FraDato FI-TilDato BR-TTButiker BR-TTValgteButiker ~
BUTTON-LeggTil FI-dant FILL-IN-31 BROWSE-TT_RapLinjePB FI-Butiknr ~
...
&Scoped-Define DISPLAYED-OBJECTS CB-Analyse FI-FraDato FI-TilDato FI-dant ~
FILL-IN-31 FI-Butiknr S-Bearbetade FI-KasseNr FI-DatoLoop FI-HodeSum ~
...
DEFINE VARIABLE FI-dant AS DECIMAL FORMAT "->>>,>>>,>>9":U INITIAL 0 
     LABEL "Antal analyselogg" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE FILL-IN-31 AS CHARACTER FORMAT "X(256)":U INITIAL " SPECIALARE" 
     VIEW-AS FILL-IN 
     SIZE 60 BY 2.14
     FONT 8 NO-UNDO.
...
DEFINE FRAME DEFAULT-FRAME
     ...
     CB-Analyse AT ROW 2.67 COL 75 COLON-ALIGNED
     ...
     FI-dant AT ROW 11.48 COL 27 COLON-ALIGNED
     FILL-IN-31 AT ROW 12.91 COL 3 COLON-ALIGNED NO-LABEL
...
/* BROWSE-TAB BR-TTButiker FI-TilDato DEFAULT-FRAME */
/* BROWSE-TAB BR-TTValgteButiker BR-TTButiker DEFAULT-FRAME */
/* BROWSE-TAB BROWSE-TT_RapLinjePB FILL-IN-31 DEFAULT-FRAME */
...
DO:
  ...
  IF AVAIL TT_RapHodePB THEN DO:
      ...
      END.
      EXPORT DELIMITER ";" "SL" sumFsgSL " " dRSumKontSL.
  END.
  ...
  IF AVAIL TT_RapHodePB THEN DO:
      ...
      END.
      EXPORT DELIMITER ";" "SL" sumFsgSL " " dRSumKortSL.
  END.
  ...
  END.
  EXPORT DELIMITER ";" "Summa SL" dSumSL     " " dRSumSumSL.
  EXPORT DELIMITER ";" "Summa Övrigt" dSumOv " " dRSumSumOv.
  EXPORT DELIMITER ";" "Summa" dTotSum       " " dRSumTotSum.
  PUT " " SKIP.    
  FIND TT_RapHodePB WHERE TT_RapHodePB.b_id = 3 NO-ERROR.
  DO iCount = 1 TO 12:
...
/*  */
  ...
  IF AVAIL TT_RapHodePB THEN DO:
      ...
      END.
      EXPORT DELIMITER ";" "SL" Antall " " dRSumKontSL.
  END.
  FIND TT_RapHodePB WHERE TT_RapHodePB.b_id = 2 NO-ERROR.
  IF AVAIL TT_RapHodePB THEN DO:
      DO iCount = 1 TO 12:
  ...
  IF AVAIL TT_RapHodePB THEN DO:
      ...
      END.
      EXPORT DELIMITER ";" "SL" Antall " " dRSumKortSL.
  END.
  FIND TT_RapHodePB WHERE TT_RapHodePB.b_id = 4 NO-ERROR.
  IF AVAIL TT_RapHodePB THEN DO:
      DO iCount = 1 TO 12:
...
DO:
  ...
  DEFINE VARIABLE dAnt AS DECIMAL    NO-UNDO.
  DEFINE BUFFER LoopAnalyseLogg FOR AnalyseLogg.
...
/*               PROCESS EVENTS.                                        */
              FOR EACH AnalyseLogg NO-LOCK:
                  ASSIGN FI-dAnt = FI-dAnt + 1.
                  IF FI-dAnt MODULO 1000 = 0 THEN DO:
                      ASSIGN FI-dAnt:SCREEN-VALUE = STRING(FI-dant) NO-ERROR.
                      PROCESS EVENTS.
                  END.
/*               FOR EACH LoopAnalyseLogg WHERE                                  */
/*                        LoopAnalyseLogg.AnalyseId = Analyse.AnalyseId AND      */
/*                        LoopAnalyseLogg.Dato      = FI-DatoLoop       AND      */
/*                        LoopAnalyseLogg.ButikkNr  = Kasse.butik       AND      */
/*                        LoopAnalyseLogg.GruppeNr  = Kasse.GruppeNr    AND      */
/*                        LoopAnalyseLogg.KasseNr   = Kasse.KasseNr     NO-LOCK. */
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY CB-Analyse FI-FraDato FI-TilDato FI-dant FILL-IN-31 FI-Butiknr 
          S-Bearbetade FI-KasseNr FI-DatoLoop FI-HodeSum FI-LinjeSum FILL-IN-20 
          FILL-IN-19 FILL-IN-18 FILL-IN-21 FILL-IN-24 FILL-IN-22 FILL-IN-26 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE RECT-56 RECT-57 BROWSE-TT_RapHodePB B-Summera CB-Analyse FI-FraDato 
         FI-TilDato BR-TTButiker BR-TTValgteButiker BUTTON-LeggTil FI-dant 
         FILL-IN-31 BROWSE-TT_RapLinjePB FI-Butiknr S-Bearbetade FI-KasseNr 
