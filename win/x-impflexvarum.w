...
def var wFKortNavn as char no-undo.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE ImporterVaremerker Procedure 
PROCEDURE ImporterVaremerker :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
put stream LoggData unformatted 
  " " skip
  " -----------------------------------------------------------------------" skip
  " Import av varemerker fra eXcellenc PRO Innkjøpssystem startet         " skip
  ...
  repeat on error undo TRANSBLOKK, leave TRANSBLOKK:
    ...
    assign
      ...
      do:
        assign
          wAntBest     = wAntBest + 1
          wFKortNavn   = substring(wLinje,4,4)
          wFTekst      = substring(wLinje,8,25).

        /*
        /* Sjekker numeriske felt */
        if SjekkStreng(trim(wFKortNavn)) = false then
          do:
            OUTPUT STREAM LoggData to value(wLogFil) no-echo append.
            put stream LoggData unformatted 
              " Feil i materialkode: " wFKortNavn " Linje:" wTotAntall skip. 
            OUTPUT STREAM LoggData close.
            next IMPORT_AV_DATA. 
          end.          
        */

        /* Oppretter eller oppdaterer varemerke. */
        find first Varemerke exclusive-lock where
          VareMerke.KortNavn = wFKortNavn no-error.
        if not available VareMerke then
          do:
            find last bufVareMerke use-index VMId no-lock no-error.
            create VareMerke.
            assign
              VareMerke.VmId = if available bufVareMerke 
                                 then bufVaremerke.VmId + 1
                                 else 1.
          end. 
        assign
          VareMerke.KortNavn    = trim(wFKortNavn)
          VareMerke.Beskrivelse = trim(wFTekst).
        ...
        if valid-handle(wProgram-Handle) then
          run ProfilInfo in wProgram-Handle (input string(wFKortNavn) + " " +
                                        string(wFTekst)).
      end. /* DATA */    
...
put stream LoggData unformatted 
  " " skip
  " -----------------------------------------------------------------------" skip
  " Antall materialkoder: " string(wAntBest,"zzzzzzz9") skip
