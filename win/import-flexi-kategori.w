...
DEF VAR lKategoriNye       AS   LOG                NO-UNDO.
DEF VAR lKategoriOverskriv AS   LOG                NO-UNDO.
...
ON END-ERROR OF C-Win /* Import av kategorier */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Import av kategorier */
DO:
...
{syspara.i 1 2 1006 cEDB-System}
{syspar2.i 1 2 1006 cImpTabell}
...
------------------------------------------------------------------------------*/
  ...
  DEF VAR piKategorier AS INT NO-UNDO.
  ...
  FOR EACH ImportLinje EXCLUSIVE-LOCK WHERE
      ...
      ASSIGN
        piKategorier = int(ImportLinje.Felt[1])
        .
      ...
      IF AVAILABLE ImpKonv THEN
        ASSIGN
          piKategorier = int(ImpKonv.InterntId)
          .

      FIND Kategori EXCLUSIVE-LOCK WHERE
          Kategori.KatNr = piKategorier NO-ERROR.
      
      /* Er det ny leverandør, sjekk om den skal opprettes ? */
      IF lKategoriNye = FALSE AND NOT AVAILABLE Kategori THEN
          NEXT MAINLOOP.
      ...
      DO:
          CREATE Kategori.
          ASSIGN
              /* '#KOD' */ Kategori.KatNr = INT(ImportLinje.Felt[1])
              .
      END.

      /* Hvis det ikke er en ny varegrupper, skal data overskrives ? */
      IF lKategoriOverskriv = FALSE AND NOT NEW Kategori THEN
          NEXT MAINLOOP.

      ASSIGN
        /* '#BESK'        */  Kategori.Beskrivelse   = ImportLinje.Felt[2]
        .
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEF INPUT PARAMETER plKategoriNye       AS LOG NO-UNDO.
  DEF INPUT PARAMETER plKategoriOverskriv AS LOG NO-UNDO.
  
  ASSIGN
    lKategoriNye       = plKategoriNye      
    lKategoriOverskriv = plKategoriOverskriv
    .
END PROCEDURE.
