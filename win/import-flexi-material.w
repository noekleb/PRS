...
DEF VAR lMaterialNye         AS  LOG                NO-UNDO.
DEF VAR lMaterialOverskriv   AS  LOG                NO-UNDO.
...
ON END-ERROR OF C-Win /* Import av material */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Import av material */
DO:
...
{syspara.i 1 2 1005 cEDB-System}
{syspar2.i 1 2 1005 cImpTabell}
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH ImportLinje EXCLUSIVE-LOCK WHERE
      ...
      ASSIGN
        piMaterial = int(ImportLinje.Felt[1])
        .
      ...
      FIND Material EXCLUSIVE-LOCK WHERE
          Material.MatKod = piMaterial NO-ERROR.
      
      /* Er det ny leverandør, sjekk om den skal opprettes ? */
      IF lMaterialNye = FALSE AND NOT AVAILABLE Material THEN
          NEXT MAINLOOP.
      ...
      DO:
          CREATE Material.
          ASSIGN
              /* '#KOD' */ Material.MatKod = piMaterial
              .
      END.
      /* Hvis det ikke er en ny leverandør, skal data overskrives ? */
      IF lMaterialOverskriv = FALSE AND NOT NEW Material THEN
          NEXT MAINLOOP.
      ASSIGN
        /* '#BESK'     */  Material.MatBeskr = ImportLinje.Felt[2]
        .
  END. /* MAINLOOP */
END PROCEDURE.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEF INPUT PARAMETER plMaterialNye       AS LOG NO-UNDO.
  DEF INPUT PARAMETER plMaterialOverskriv AS LOG NO-UNDO.
  
  ASSIGN
    lMaterialNye       = plMaterialNye      
    lMaterialOverskriv = plMaterialOverskriv
    .
END PROCEDURE.
