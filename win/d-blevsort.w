...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEFINE VAR wSortId LIKE LevSort.SortId NO-UNDO.
  def    var wRecid  as   recid          no-undo.
  find LevBas no-lock where
    LevBas.LevNr = 6 no-error.
    if available LevBas then
      assign wRecid = recid(LevBas).
&ELSE
  DEFINE INPUT-OUTPUT PARAMETER wSortId LIKE LevSort.SortId NO-UNDO.
...
&scoped-define sok-field1  SortId
...
&scoped-define index-col1  LevSort
&scoped-define sok-phrase1 FIND FIRST b{&br-tabell} WHERE ~
                b{&br-tabell}.LevNr = LevBas.LevNr and ~
...
&scoped-define sok-field3  StrTypeId
...
&scoped-define index-col3  StrType
...
&scoped-define ip-variabel wSortId
&scoped-define return-ip   wSortId = if available LevSort  ~
                                       then LevSort.SortId ~
                                       else "".
&scoped-define assign-retur-verdi retur-verdi = if available LevSort ~
                                                  then string(recid(LevSort)) ~
                                                  else "".
&scoped-define SletteMelding "Skal leverandørsortimentet slettes? "
&scoped-define KanSlettes ~
  if can-find(first BestLinje where ~
    BestSort.SortId = LevSort.SortId) then ~
    do: ~
      message "Det finnes Bestillinger med denne størrelsestype!" skip ~
              "Posten kan ikke slettes." view-as alert-box title "Feil ved sletting". ~
      return no-apply. ~
    end.

&scoped-define ant-sok-field   3
&scoped-define init-datatype   CHAR /* CHAR, INT, DATE */
&scoped-define VedlikeholdsRutine d-vlevsort.w
...
def var wIntervall as char format "x(20)" no-undo.
def var wFordeling as char format "x(30)" no-undo.
...
&Scoped-define INTERNAL-TABLES LevSort LevBas

/* Definitions for BROWSE BROWSE-LevSort                                */
&Scoped-define FIELDS-IN-QUERY-BROWSE-LevSort LevSort.SortID ~
LevSort.Beskrivelse LevSort.StrTypeID LevSort.Fri wIntervall wFordeling 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-LevSort LevSort.SortID ~
LevSort.Beskrivelse LevSort.StrTypeID 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-LevSort LevSort
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-LevSort LevSort
...
&Scoped-define QUERY-STRING-Dialog-Frame FOR EACH LevBas SHARE-LOCK
...
FILL-IN-SOK-CHAR BUTTON-Sok B-KonvTbl BROWSE-LevSort BUTTON-Ny BUTTON-Endre ~
BUTTON-Slett Btn_OK Btn_Cancel Btn_Help 
&Scoped-Define DISPLAYED-FIELDS LevBas.levnr LevBas.levnamn 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD Fordeling Dialog-Frame 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD Intervall Dialog-Frame 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-LevSort Dialog-Frame _STRUCTURED
  QUERY BROWSE-LevSort NO-LOCK DISPLAY
      LevSort.SortID COLUMN-LABEL "SortimentID *" FORMAT "X(12)":U
      LevSort.Beskrivelse COLUMN-LABEL "Beskrivelse *" FORMAT "X(30)":U
      LevSort.StrTypeID COLUMN-LABEL "StrType *" FORMAT "zz9":U
      LevSort.Fri FORMAT "Fri/Fast":U
      wIntervall COLUMN-LABEL "Inndeling" FORMAT "x(15)":U
      wFordeling COLUMN-LABEL "Fordeling" FORMAT "x(50)":U
  ENABLE
      LevSort.SortID
      LevSort.Beskrivelse
      LevSort.StrTypeID
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 109.4 BY 13.81.
...
DEFINE FRAME Dialog-Frame
     ...
     B-KonvTbl AT ROW 1.48 COL 104.8
     LevBas.levnr AT ROW 1.57 COL 44.6 COLON-ALIGNED
          LABEL "Leverandør" FORMAT "zzzzz9"
          VIEW-AS FILL-IN 
          SIZE 10.4 BY 1
     LevBas.levnamn AT ROW 1.57 COL 56 COLON-ALIGNED NO-LABEL FORMAT "x(200)"
          VIEW-AS FILL-IN 
          SIZE 40 BY 1
     BROWSE-LevSort AT ROW 2.91 COL 2
     BUTTON-Ny AT ROW 2.91 COL 111.8
     BUTTON-Endre AT ROW 4.24 COL 111.8
     BUTTON-Slett AT ROW 5.57 COL 111.8
     Btn_OK AT ROW 16.91 COL 1.8
     Btn_Cancel AT ROW 16.91 COL 14.2
     Btn_Help AT ROW 16.91 COL 111.8
     SPACE(0.19) SKIP(0.00)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste leverandørsortiment"
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
                                                                        */
/* BROWSE-TAB BROWSE-LevSort levnamn Dialog-Frame */
ASSIGN 
       FRAME Dialog-Frame:SCROLLABLE       = FALSE
       FRAME Dialog-Frame:HIDDEN           = TRUE.

ASSIGN 
       BROWSE-LevSort:NUM-LOCKED-COLUMNS IN FRAME Dialog-Frame     = 2.
...
/* Query rebuild information for BROWSE BROWSE-LevSort
     ...
     _Where[1]         = "LevSort.LevNr = LevBas.LevNr"
     _FldNameList[1]   > SkoTex.LevSort.SortID
"LevSort.SortID" "SortimentID *" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > SkoTex.LevSort.Beskrivelse
"LevSort.Beskrivelse" "Beskrivelse *" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[3]   > SkoTex.LevSort.StrTypeID
"LevSort.StrTypeID" "StrType *" "zz9" "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[4]   = SkoTex.LevSort.Fri
     _FldNameList[5]   > "_<CALC>"
"wIntervall" "Inndeling" "x(15)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[6]   > "_<CALC>"
"wFordeling" "Fordeling" "x(50)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _Query            is OPENED
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Søkeliste leverandørsortiment */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-LevSort Dialog-Frame
ON ANY-PRINTABLE OF BROWSE-LevSort IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-LevSort Dialog-Frame
ON CURSOR-LEFT OF BROWSE-LevSort IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-LevSort Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-LevSort IN FRAME Dialog-Frame
DO:
  IF {&ant-sok-field} = 1 THEN
    RETURN NO-APPLY.
  RUN SD-CURSOR ("RIGHT").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-LevSort Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-LevSort IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-LevSort Dialog-Frame
ON ROW-DISPLAY OF BROWSE-LevSort IN FRAME Dialog-Frame
DO:
  assign
    wIntervall = Intervall()
    wFordeling = Fordeling().
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-LevSort Dialog-Frame
ON START-SEARCH OF BROWSE-LevSort IN FRAME Dialog-Frame
DO:
  RUN SD-START-SEARCH.
  RETURN NO-APPLY.
END.
...
DO:
  ...
  run {&VedlikeholdsRutine} (input-output wRecid,"Endre",LevBas.LevNr).
...
DO:
  assign 
    wRecid = ?.
  run {&VedlikeholdsRutine} (input-output wRecid,"Ny",LevBas.LevNr).
  if return-value = "AVBRYT" then
    return no-apply.
  ...
  if wOk = true then
    do transaction:
      ...
      if available b{&br-tabell} then
        do:
          for each LevSAnt of b{&br-tabell} exclusive-lock:
            delete LevSAnt.
          end.
          delete b{&br-tabell}.
...
/* Henter leverandøren */
find LevBas no-lock where
  recid(LevBas) = wRecid no-error.
if not available LevBas then
  do:
    message "Ukjent leverandør - kan ikke starte!"
    view-as alert-box title "Oppstartsfeil".
    return no-apply.
  end.
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-SOK-INT FILL-IN-SOK-DATE FILL-IN-SOK-CHAR 
      WITH FRAME Dialog-Frame.
  IF AVAILABLE LevBas THEN 
    DISPLAY LevBas.levnr LevBas.levnamn 
      WITH FRAME Dialog-Frame.
  ENABLE FILL-IN-SOK-INT FILL-IN-SOK-DATE FILL-IN-SOK-CHAR BUTTON-Sok B-KonvTbl 
         BROWSE-LevSort BUTTON-Ny BUTTON-Endre BUTTON-Slett Btn_OK Btn_Cancel 
         Btn_Help 
      WITH FRAME Dialog-Frame.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION Fordeling Dialog-Frame 
FUNCTION Fordeling RETURNS CHARACTER
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION Intervall Dialog-Frame 
FUNCTION Intervall RETURNS CHARACTER
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  
