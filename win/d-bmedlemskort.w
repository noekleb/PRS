...
&scope br-tabell   MedlemsKort
&scope Sorttype    BY,BY
/* Sorttype entries = antal sortkolonner, kan vara tomma. */
&scope BrowseIdx   MedlemsKort,Merknad
...
&scope QWhere ~
      'MedlemsKort.MedlemsNr = ' +  string(wMedlemsNr)
...
&IF LENGTH("{&ip-variabel}") > 1 &THEN
  ...
  &IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
    ...
    DEFINE VAR wMedlemsNr       LIKE kunde.KundeNr           NO-UNDO.
  &ELSE
    DEFINE INPUT PARAMETER {&ip-variabel} LIKE {&br-tabell}.{&ip-felt} NO-UNDO.
    DEFINE INPUT PARAMETER wMedlemsNr       LIKE Kunde.KundeNr           NO-UNDO.
  &ENDIF
...
DEFINE VAR cEAN              AS CHAR   NO-UNDO.
&scoped-define SletteMelding "Skall posten tas bort?"
&scoped-define KanSlettes 
&scoped-define VedlikeholdsRutine d-vmedlemskort.w
...
{etikettlogg.i &NEW=NEW}
    ...
    /* 
    def {&new} shared temp-table EtikettLogg
...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME BROWSE-MedlemsKort

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES MedlemsKort

/* Definitions for BROWSE BROWSE-MedlemsKort                            */
&Scoped-define FIELDS-IN-QUERY-BROWSE-MedlemsKort MedlemsKort.KortNr ~
MedlemsKort.AktivertDato MedlemsKort.UtgarDato MedlemsKort.Sperret ~
MedlemsKort.Innehaver MedlemsKort.KundeKortNr MedlemsKort.Merknad 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-MedlemsKort ~
MedlemsKort.KortNr MedlemsKort.Merknad 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-MedlemsKort MedlemsKort
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-MedlemsKort MedlemsKort
&Scoped-define QUERY-STRING-BROWSE-MedlemsKort FOR EACH MedlemsKort ~
      WHERE MedlemsKort.MedlemsNr = wMedlemsNr NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-MedlemsKort OPEN QUERY BROWSE-MedlemsKort FOR EACH MedlemsKort ~
      WHERE MedlemsKort.MedlemsNr = wMedlemsNr NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-MedlemsKort MedlemsKort
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-MedlemsKort MedlemsKort


/* Definitions for DIALOG-BOX Dialog-Frame                              */
&Scoped-define OPEN-BROWSERS-IN-QUERY-Dialog-Frame ~
    ~{&OPEN-QUERY-BROWSE-MedlemsKort}

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS FILL-IN-SOK-CHAR FILL-IN-SOK-INTE ~
FILL-IN-SOK-DATE BUTTON-Sok BROWSE-MedlemsKort Btn_OK Btn_Help 
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY BROWSE-MedlemsKort FOR 
      MedlemsKort SCROLLING.
&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE BROWSE-MedlemsKort
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-MedlemsKort Dialog-Frame _STRUCTURED
  QUERY BROWSE-MedlemsKort NO-LOCK DISPLAY
      MedlemsKort.KortNr FORMAT "X(22)":U
      MedlemsKort.AktivertDato FORMAT "99/99/99":U
      MedlemsKort.UtgarDato FORMAT "99/99/99":U
      MedlemsKort.Sperret FORMAT "*/":U
      MedlemsKort.Innehaver FORMAT "X(30)":U
      MedlemsKort.KundeKortNr FORMAT "X(22)":U
      MedlemsKort.Merknad FORMAT "X(40)":U WIDTH 32.8
  ENABLE
      MedlemsKort.KortNr
      MedlemsKort.Merknad
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 145 BY 14.76 ROW-HEIGHT-CHARS .62 EXPANDABLE.
...
DEFINE FRAME Dialog-Frame
     ...
     BROWSE-MedlemsKort AT ROW 2.91 COL 2
     Btn_OK AT ROW 17.91 COL 2
     Btn_Cancel AT ROW 17.91 COL 15.2
     Btn_Help AT ROW 17.91 COL 141
     BUTTON-Ny AT ROW 2.91 COL 148
     BUTTON-Endre AT ROW 4.24 COL 148
     BUTTON-Slett AT ROW 5.57 COL 148
     BUTTON-Etikett AT ROW 6.95 COL 148
     SPACE(1.59) SKIP(10.95)
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   Custom                                                               */
/* BROWSE-TAB BROWSE-MedlemsKort BUTTON-Sok Dialog-Frame */
ASSIGN 
       FRAME Dialog-Frame:SCROLLABLE       = FALSE
       FRAME Dialog-Frame:HIDDEN           = TRUE.

ASSIGN 
       BROWSE-MedlemsKort:NUM-LOCKED-COLUMNS IN FRAME Dialog-Frame     = 4
       BROWSE-MedlemsKort:MAX-DATA-GUESS IN FRAME Dialog-Frame         = 481.
...
/* SETTINGS FOR BUTTON BUTTON-Etikett IN FRAME Dialog-Frame
   NO-ENABLE                                                            */
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE BROWSE-MedlemsKort
/* Query rebuild information for BROWSE BROWSE-MedlemsKort
     _TblList          = "skotex.MedlemsKort"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _Where[1]         = "MedlemsKort.MedlemsNr = wMedlemsNr"
     _FldNameList[1]   > skotex.MedlemsKort.KortNr
"MedlemsKort.KortNr" ? ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   = skotex.MedlemsKort.AktivertDato
     _FldNameList[3]   = skotex.MedlemsKort.UtgarDato
     _FldNameList[4]   > skotex.MedlemsKort.Sperret
"MedlemsKort.Sperret" ? "*~~/" "logical" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[5]   = skotex.MedlemsKort.Innehaver
     _FldNameList[6]   = skotex.MedlemsKort.KundeKortNr
     _FldNameList[7]   > skotex.MedlemsKort.Merknad
"MedlemsKort.Merknad" ? ? "character" ? ? ? ? ? ? yes ? no no "32.8" yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-MedlemsKort */
&ANALYZE-RESUME
...
&Scoped-define BROWSE-NAME BROWSE-MedlemsKort
&Scoped-define SELF-NAME BROWSE-MedlemsKort
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-MedlemsKort Dialog-Frame
ON ANY-PRINTABLE OF BROWSE-MedlemsKort IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-MedlemsKort Dialog-Frame
ON CURSOR-LEFT OF BROWSE-MedlemsKort IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-MedlemsKort Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-MedlemsKort IN FRAME Dialog-Frame
DO:
  IF wAntSortCol > 1 THEN
      RUN SD-CURSOR ("RIGHT").
  RETURN NO-APPLY.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-MedlemsKort Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-MedlemsKort IN FRAME Dialog-Frame
DO:
  IF wType = "V" THEN
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-MedlemsKort Dialog-Frame
ON HOME OF BROWSE-MedlemsKort IN FRAME Dialog-Frame
DO:
  APPLY "ENTRY" TO SELF.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-MedlemsKort Dialog-Frame
ON START-SEARCH OF BROWSE-MedlemsKort IN FRAME Dialog-Frame
DO:
...
DO:
  ...
  run {&VedlikeholdsRutine} (input-output wRecid,"Endre",wMedlemsNr).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-Etikett Dialog-Frame
ON CHOOSE OF BUTTON-Etikett IN FRAME Dialog-Frame /* Etikett */
DO:
  ...
  IF ERROR-STATUS:ERROR THEN DO:
      MESSAGE "Kortnummer ikke numeriskt."
          VIEW-AS ALERT-BOX INFO BUTTONS OK.
      RETURN NO-APPLY.
  END.
/*   IF LENGTH(STRING(dKortNummer)) > 7 THEN DO:                   */
/*       MESSAGE "Kortnummer for stort for etikett; max 7 siffer." */
/*           VIEW-AS ALERT-BOX INFO BUTTONS OK.                    */
/*       RETURN NO-APPLY.                                          */
/*   END.                                                          */
/*   ELSE                                                          */
  IF MedlemsKort.Sperret THEN DO:
      MESSAGE "Kortet er sperret."
          VIEW-AS ALERT-BOX INFO BUTTONS OK.
      RETURN NO-APPLY.
  END.
  ELSE IF MedlemsKort.UtgarDato <> ? AND MedlemsKort.UtgarDato < TODAY THEN DO:
      MESSAGE "Kortet er utgått på dato."
          VIEW-AS ALERT-BOX INFO BUTTONS OK.
      RETURN NO-APPLY.
  END.
  FIND Medlem OF MedlemsKort NO-LOCK NO-ERROR.
  IF LENGTH(STRING(Medlem.ButikkNr)) > 3 THEN DO:
      MESSAGE "Medlemkort koblet till butikk med butikknr > 999."
          VIEW-AS ALERT-BOX INFO BUTTONS OK.
      RETURN NO-APPLY.
  END.
  ELSE IF Medlem.Opphort <> ? AND Medlem.Opphort < TODAY THEN DO:
      MESSAGE "Medlemsskap har utgått."
          VIEW-AS ALERT-BOX INFO BUTTONS OK.
      RETURN NO-APPLY.
  END.
  ASSIGN cEAN = DYNAMIC-FUNCTION('fixChkEAN':U IN h_dproclib,
           cEANprefix + STRING(dKortNummer,"9999999999")).
/*   RUN Skrivetikett (Medlem.ButikkNr,cEan,MedlemsKort.Innehaver). */
...
DO:
  assign 
    wRecid = ?.
  run {&VedlikeholdsRutine} (input-output wRecid,"Ny",wMedlemsNr).
  if return-value = "AVBRYT" then
    return no-apply.
...
DO:
  ...
  /* Kan ikke slettes hvis det ligger medlemstransaksjoner på kortet. */
  IF AVAILABLE bMedlemsKort THEN
  DO:
      IF CAN-FIND(FIRST MedTrans WHERE 
                  MedTrans.KortNr = bMedlemsKort.KortNr) THEN
      DO:
          MESSAGE "Det er salgstransaksjoner registrert på dette medlemskortet." SKIP
                  "Kortet kan ikke slettes."
              VIEW-AS ALERT-BOX INFO BUTTONS OK.
          RETURN NO-APPLY.
      END.
  END.
  ...
  if wOk = true then
    do transaction:
      ...
      if available b{&br-tabell} then
        do:
          FIND Medlem OF b{&br-tabell} NO-ERROR.
          IF Medlem.KundeNr <> 0 THEN
              ASSIGN Medlem.EDato = TODAY NO-ERROR.
          IF AVAIL Medlem THEN
              RELEASE Medlem.

          IF b{&br-tabell}.InterntKKortId > 0 THEN
          DO:
              FIND FIRST KundeKort EXCLUSIVE-LOCK WHERE
                  KundeKort.InterntKKortId = b{&br-tabell}.InterntKKortId NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-CHAR FILL-IN-SOK-INTE FILL-IN-SOK-DATE BUTTON-Sok 
         BROWSE-MedlemsKort Btn_OK Btn_Help 
      WITH FRAME Dialog-Frame.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Skrivetikett Dialog-Frame 
PROCEDURE Skrivetikett :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  DEFINE INPUT  PARAMETER cInneHaver AS CHARACTER  NO-UNDO.
...
/*          cColOren   = "240,275,310,330,340".       */
  ...
  /* används för att kunna veta startetikett: Ant = startetikett */
      ...
      IF NOT wOK THEN
          RETURN NO-APPLY.
/*       ASSIGN cWinPrinterName = SESSION:PRINTER-NAME. */
...
/*       OUTPUT TO PRINTER VALUE(cWinPrinterName) CONVERT TARGET "IBM850". */
  ...
  PUT CONTROL cFormat.
      ...
      IF AVAIL Butiker THEN
          ASSIGN iAntFill = IF LENGTH(TRIM(Butiker.butnamn)) < 25 THEN (24 - LENGTH(TRIM(Butiker.butnamn))) / 2 ELSE 0
                 cButnamn = FILL(" ",iAntFill)  + Butiker.butnamn
                 iAntFill = IF LENGTH(TRIM(cInneHaver)) < 25 THEN (24 - LENGTH(TRIM(cInneHaver))) / 2 ELSE 0
...
/*                       "121100002200020" ArtBas.Bongtekst CHR(13) */
                      ...
                      "1F2208000900065" cEAN CHR(13)
                      "191100400350105 Medlem" CHR(13)
  ...
  DO WITH FRAME {&FRAME-NAME}:
    IF wType = "V" THEN DO: /* Enabla vedlikeholdsknappar */
        ASSIGN BUTTON-Ny:HIDDEN       = FALSE
               ...
               BUTTON-Etikett:HIDDEN    = FALSE
               BUTTON-Etikett:SENSITIVE = TRUE
               Btn_OK:LABEL           = "&Lukk".
    END.
