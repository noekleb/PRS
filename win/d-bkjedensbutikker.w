...
&scoped-define br-tabell   TT_KjedensButikker
&scoped-define OptWhere 

/* Kolonne 1 som det kan sorteres på */
&scoped-define sok-field1  ButikkNr
...
&scoped-define index-col1  KjedensButikker
...
&scoped-define sok-field2  ButikkNavn
...
&scoped-define index-col2  ButikkNavn
...
&scoped-define ip-variabel rRowId
/* &scoped-define return-ip   rRowId = if available {&br-tabell}  ~  */
/*                                        then ROWID({&br-tabell}) ~ */
/*                                        else  ?.                   */
...
&Scoped-define INTERNAL-TABLES TT_KjedensButikker

/* Definitions for BROWSE BROWSE-Butiker                                */
&Scoped-define FIELDS-IN-QUERY-BROWSE-Butiker TT_KjedensButikker.ButikkNr ~
TT_KjedensButikker.ButikkNavn 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-Butiker ~
TT_KjedensButikker.ButikkNr TT_KjedensButikker.ButikkNavn 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-Butiker TT_KjedensButikker
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-Butiker TT_KjedensButikker
&Scoped-define QUERY-STRING-BROWSE-Butiker FOR EACH TT_KjedensButikker NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-Butiker OPEN QUERY BROWSE-Butiker FOR EACH TT_KjedensButikker NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-Butiker TT_KjedensButikker
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-Butiker TT_KjedensButikker
...
FILL-IN-SOK-CHAR BUTTON-Sok BROWSE-Butiker Btn_Help Btn_OK Btn_Cancel 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Butiker Dialog-Frame _STRUCTURED
  QUERY BROWSE-Butiker NO-LOCK DISPLAY
      TT_KjedensButikker.ButikkNr FORMAT ">>>>>9":U
      TT_KjedensButikker.ButikkNavn FORMAT "X(30)":U
  ENABLE
      TT_KjedensButikker.ButikkNr
      TT_KjedensButikker.ButikkNavn
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME Dialog-Frame
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "Søkeliste kjedens butikkregister"
...
ASSIGN 
       TT_KjedensButikker.ButikkNr:COLUMN-READ-ONLY IN BROWSE BROWSE-Butiker = TRUE
       TT_KjedensButikker.ButikkNavn:COLUMN-READ-ONLY IN BROWSE BROWSE-Butiker = TRUE.
...
/* Query rebuild information for BROWSE BROWSE-Butiker
     _TblList          = "Temp-Tables.TT_KjedensButikker"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _FldNameList[1]   > Temp-Tables.TT_KjedensButikker.ButikkNr
"TT_KjedensButikker.ButikkNr" ? ? "integer" ? ? ? ? ? ? yes ? no no ? yes no yes "U" "" ""
     _FldNameList[2]   > Temp-Tables.TT_KjedensButikker.ButikkNavn
"TT_KjedensButikker.ButikkNavn" ? ? "character" ? ? ? ? ? ? yes ? no no ? yes no yes "U" "" ""
     _Query            is OPENED
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Søkeliste kjedens butikkregister */
DO:
...
DO:
  IF AVAIL {&br-tabell} THEN DO:
      FIND KjedensButikker WHERE KjedensButikker.Kjede    = {&br-tabell}.Kjede AND
                                 KjedensButikker.RegionNr = {&br-tabell}.RegionNr AND
                                 KjedensButikker.DistriktNr = {&br-tabell}.DistriktNr AND
                                 KjedensButikker.ButikkNr   = {&br-tabell}.ButikkNr NO-LOCK NO-ERROR.
      IF AVAIL KjedensButikker THEN
          ASSIGN {&ip-variabel} = ROWID(KjedensButikker).
  END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ON END-KEY UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK:
    ...
    IF BROWSE {&BROWSE-NAME}:FOCUSED-ROW = ? THEN DO:
        IF wAnt > 0 THEN
            MESSAGE "Det er registrert " + STRING(wAnt) + " butikker i " + 
                    "kjedens butikkregister. De er ALLE koblet til en butikk." SKIP
                    "Skal det opprettes ny butikk, må det først legges opp en ny 'kjedens butikk'."
                VIEW-AS ALERT-BOX INFO BUTTONS OK.
        ELSE
            MESSAGE "Det må registreres en 'kjedens butikk' før ny butikk kan opprettes."
                VIEW-AS ALERT-BOX INFO BUTTONS OK.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE SkapaTT Dialog-Frame 
PROCEDURE SkapaTT :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    FOR EACH KjedensButikker NO-LOCK WHERE NOT CAN-FIND(Butiker WHERE Butiker.Butik = KjedensButikker.ButikkNr):
        BUFFER-COPY KjedensButikker TO TT_KjedensButikker.
        RELEASE TT_KjedensButikker.
    END.
    FOR EACH KjedensButikker NO-LOCK WHERE CAN-FIND(Butiker WHERE Butiker.Butik = KjedensButikker.ButikkNr):
        wAnt = wAnt + 1.
    END.
END PROCEDURE.
