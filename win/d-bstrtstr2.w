...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  def var wStrTypeId like STrTStr.StrTypeId no-undo.
  assign
    wStrTypeId = 1.
&ELSE
  DEFINE INPUT-output PARAMETER wStrTypeId like STrTStr.StrTypeId   NO-UNDO.  
&ENDIF
DEFINE VAR wSoStorl LIKE StrTStr.SoStorl NO-UNDO.
...
&scoped-define sok-field1  SoStorl
...
&scoped-define index-col1 SoStorl
&scoped-define sok-phrase1 FIND FIRST b{&br-tabell} WHERE ~
               {&OptWhere} ~
                trim(b{&br-tabell}.{&sok-field1}) >= ~
                INPUT FILL-IN-SOK-{&sok-field1-def} USE-INDEX {&index-col1} NO-LOCK NO-ERROR.

&scoped-define ip-variabel wSoStorl
&scoped-define return-ip   wSoStorl = if available {&br-tabell}  ~
...
/*
&scoped-define init-phrase  FIND first b{&br-tabell} WHERE ~
                           {&OptWhere} ~
                           trim(b{&br-tabell}.{&sok-field1}) = ~
...
&Scoped-define FIELDS-IN-QUERY-BROWSE-StrTStr StrTStr.SoStorl 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-StrTStr 
...
FILL-IN-SOK-CHAR BUTTON-Sok BROWSE-StrTStr BUTTON-Endre BUTTON-Slett Btn_OK ~
Btn_Cancel Btn_Help 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-StrTStr Dialog-Frame _STRUCTURED
  QUERY BROWSE-StrTStr NO-LOCK DISPLAY
      StrTStr.SoStorl WIDTH 9.2
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 14 BY 21.67 ROW-HEIGHT-CHARS .62.
...
DEFINE FRAME Dialog-Frame
     ...
     BROWSE-StrTStr AT ROW 2.91 COL 2
     BUTTON-Endre AT ROW 2.91 COL 29
     BUTTON-Slett AT ROW 4.33 COL 29
     Btn_OK AT ROW 24.81 COL 1
     Btn_Cancel AT ROW 24.81 COL 14
     Btn_Help AT ROW 24.81 COL 29
     SPACE(0.59) SKIP(0.18)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste størrelser"
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
                                                                        */
/* BROWSE-TAB BROWSE-StrTStr BUTTON-Sok Dialog-Frame */
ASSIGN 
...
"StrTStr.SoStorl" ? ? "character" ? ? ? ? ? ? no ? no no "9.2"
     _Query            is OPENED
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Søkeliste størrelser */
DO:
...
ON ANY-PRINTABLE OF BROWSE-StrTStr IN FRAME Dialog-Frame
DO:
...
DO:
  ...
  run d-vstrtstr2.w (input-output wRecid,"Endre").
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-DATE FILL-IN-SOK-INT FILL-IN-SOK-CHAR BUTTON-Sok 
         BROWSE-StrTStr BUTTON-Endre BUTTON-Slett Btn_OK Btn_Cancel Btn_Help 
      WITH FRAME Dialog-Frame.
