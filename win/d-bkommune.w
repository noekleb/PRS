...
/*------------------------------------------------------------------------
  Forfatter:    Tom Nøkleby
  Beskrivelse:  Søkeliste kommuneregister
  Parametere:   wKommNr - Kommunenummer søk skal starte på.
  Endringer:
...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEFINE VAR wKommNr LIKE Kommune.KommNr NO-UNDO.
&ELSE
  DEFINE INPUT-OUTPUT PARAMETER wKommNr LIKE Kommune.KommNr NO-UNDO.
&ENDIF
...
&scoped-define ip-variabel wKommNr
&scoped-define return-ip   wKommNr = if available Kommune ~
                                       then Kommune.KommNr ~
                                       else "".
&scoped-define assign-retur-verdi retur-verdi = if available Kommune ~
                                       then string(recid(Kommune)) ~
                                       else "".
...
&Scoped-define INTERNAL-TABLES Kommune
...
&Scoped-define QUERY-STRING-BROWSE-Kommune FOR EACH Kommune NO-LOCK ~
...
&Scoped-Define ENABLED-OBJECTS FILL-IN-SOK-DATE FILL-IN-SOK-CHAR ~
FILL-IN-SOK-INT BUTTON-Sok BROWSE-Kommune BUTTON-Ny BUTTON-Endre ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Kommune Dialog-Frame _STRUCTURED
  QUERY BROWSE-Kommune NO-LOCK DISPLAY
      Kommune.KommNr COLUMN-LABEL "KommuneNr *" FORMAT "X(8)":U
      Kommune.Beskrivelse COLUMN-LABEL "Kommunenavn *" FORMAT "X(30)":U
      Kommune.FylkesNr FORMAT "X(8)":U
      Kommune.Merknad FORMAT "X(50)":U
  ENABLE
      Kommune.KommNr
      Kommune.Beskrivelse
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 57 BY 9.81.
...
DEFINE FRAME Dialog-Frame
     ...
     BROWSE-Kommune AT ROW 2.91 COL 2
     BUTTON-Ny AT ROW 2.91 COL 60.2
     BUTTON-Endre AT ROW 4.24 COL 60.2
     BUTTON-Slett AT ROW 5.57 COL 60.2
     Btn_Help AT ROW 12.81 COL 60.2
     Btn_OK AT ROW 12.91 COL 2.2
     Btn_Cancel AT ROW 12.91 COL 14.6
     SPACE(48.60) SKIP(0.12)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste kommune"
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
                                                                        */
/* BROWSE-TAB BROWSE-Kommune BUTTON-Sok Dialog-Frame */
ASSIGN 
       FRAME Dialog-Frame:SCROLLABLE       = FALSE
       FRAME Dialog-Frame:HIDDEN           = TRUE.

ASSIGN 
       BROWSE-Kommune:NUM-LOCKED-COLUMNS IN FRAME Dialog-Frame     = 1.
...
"Kommune.KommNr" "KommuneNr *" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > SkoTex.Kommune.Beskrivelse
"Kommune.Beskrivelse" "Kommunenavn *" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[3]   = SkoTex.Kommune.FylkesNr
     _FldNameList[4]   = SkoTex.Kommune.Merknad
     _Query            is OPENED
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Søkeliste kommune */
DO:
...
ON ANY-PRINTABLE OF BROWSE-Kommune IN FRAME Dialog-Frame
DO:
...
{syspara.i 16 26 2 iTilgang INT}
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-SOK-DATE FILL-IN-SOK-CHAR FILL-IN-SOK-INT 
      WITH FRAME Dialog-Frame.
  ENABLE FILL-IN-SOK-DATE FILL-IN-SOK-CHAR FILL-IN-SOK-INT BUTTON-Sok 
         BROWSE-Kommune BUTTON-Ny BUTTON-Endre BUTTON-Slett Btn_Help Btn_OK 
         Btn_Cancel 
      WITH FRAME Dialog-Frame.
