...
&scope QWhere ~
      'KundeKort.KundeNr = ' +  string(wKundeNr)
...
DEFINE VAR cMedlemskort      AS CHAR   NO-UNDO.
...
KundeKort.AktivertDato KundeKort.UtgarDato KundeKort.Sperret ~
KundeKort.Innehaver cMedlemskort KundeKort.Merknad 
...
&Scoped-define QUERY-STRING-BROWSE-Kundekort FOR EACH KundeKort ~
      WHERE KundeKort.KundeNr = wKundeNr NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-Kundekort OPEN QUERY BROWSE-Kundekort FOR EACH KundeKort ~
      WHERE KundeKort.KundeNr = wKundeNr NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Kundekort Dialog-Frame _STRUCTURED
  QUERY BROWSE-Kundekort NO-LOCK DISPLAY
      ...
      KundeKort.Innehaver FORMAT "X(30)":U
      cMedlemskort COLUMN-LABEL "Tilknyttet medlem" FORMAT "x(20)":U
      KundeKort.Merknad FORMAT "X(40)":U WIDTH 33.8
  ENABLE
      KundeKort.KortNr
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 144 BY 14.76 ROW-HEIGHT-CHARS .62 EXPANDABLE.
...
DEFINE FRAME Dialog-Frame
     ...
     Btn_Help AT ROW 17.91 COL 147
     BUTTON-Ny AT ROW 2.91 COL 147
     BUTTON-Endre AT ROW 4.24 COL 147
     BUTTON-Slett AT ROW 5.57 COL 147
     SPACE(0.99) SKIP(12.33)
...
/* Query rebuild information for BROWSE BROWSE-Kundekort
     ...
     _Where[1]         = "KundeKort.KundeNr = wKundeNr"
     _FldNameList[1]   > skotex.KundeKort.KortNr
"KundeKort.KortNr" ? ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   = skotex.KundeKort.AktivertDato
     _FldNameList[3]   = skotex.KundeKort.UtgarDato
     _FldNameList[4]   > skotex.KundeKort.Sperret
"KundeKort.Sperret" ? "*~~/" "logical" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[5]   = skotex.KundeKort.Innehaver
     _FldNameList[6]   > "_<CALC>"
"cMedlemskort" "Tilknyttet medlem" "x(20)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[7]   > skotex.KundeKort.Merknad
"KundeKort.Merknad" ? ? "character" ? ? ? ? ? ? no ? no no "33.8" yes no no "U" "" ""
     _Query            is OPENED
...
DO:
  ...
  IF AVAILABLE MedlemsKort 
      THEN cMedlemskort = MedlemsKort.KortNr + "/" + string(Medlemskort.MedlemsNr).
      ELSE cMedlemskort = "".
END.
...
DO:
  FIND LAST KundeKort NO-LOCK WHERE
      KundeKort.KundeNr = wKundeNr NO-ERROR.

  RUN createKundekort.p (INPUT wKundeNr, (IF AVAILABLE KundeKort
                                              THEN KundeKort.KortNr
                                              ELSE '') , INPUT iGyldighet, OUTPUT wRecid).
...
DO:
  ...
  DO:
      IF CAN-FIND(FIRST MedTrans WHERE 
                  MedTrans.KortNr = b{&br-tabell}.KortNr) THEN
      DO:
          MESSAGE "Det er salgstransaksjoner registrert på dette kundekortet." SKIP
  ...
  view-as alert-box buttons yes-no-cancel title "Bekreft" 
  update wOk.
  if wOk = true then
    do transaction:
      {&KanSlettes}
      ...
      if available b{&br-tabell} then
        do:
          ...
          IF can-find(FIRST MedlemsKort where
                      MedlemsKort.InterntKKortId = b{&br-tabell}.InterntKKortId) THEN
          DO:
              FIND FIRST MedlemsKort EXCLUSIVE-LOCK WHERE
                  MedlemsKort.InterntKKortId = b{&br-tabell}.InterntKKortId NO-ERROR.
