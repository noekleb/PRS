...
&scope br-tabell   ButikkTeam
&scope Sorttype    USE-INDEX
/* Sorttype entries = antal sortkolonner, kan vara tomma. */
&scope BrowseIdx   ButikkTeam
...
&scope InitIdx     ButikkTeam
&scope ip-felt     TeamTypeId
/* Om du önskar input parameter. Ger en startup-record */ 
&scope ip-variabel w{&ip-felt}

&scope assign-retur-verdi ASSIGN retur-verdi = STRING(ROWID({&br-tabell}))

&scope QWhere   'ButikkTeam.BrGrpNr = ' + STRING(wBrGrpNr) + ~
                ' AND ButikkTeam.TeamTypeId = ' + STRING({&ip-variabel})
...
&IF LENGTH("{&ip-variabel}") > 1 &THEN
  ...
  &IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
    ...
    DEFINE VAR {&ip-variabel} LIKE {&br-tabell}.{&ip-felt} INIT 1 NO-UNDO.
  &ELSE
...
/* 
  if can-find(first xxx where xxx.TeamTypeId = {&br-tabell}.TeamTypeId) then ~
    do: ~
      message "Det finnes xxxx med denne id. Kan ikke slettes!" ~
      view-as alert-box title "Fel vid borttagning". ~
      return no-apply. ~
    end.
    */
&scoped-define VedlikeholdsRutine d-vbutikkteam.w
...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME BROWSE-ButikkTeam

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES ButikkTeam

/* Definitions for BROWSE BROWSE-ButikkTeam                             */
&Scoped-define FIELDS-IN-QUERY-BROWSE-ButikkTeam ButikkTeam.TeamNr ~
ButikkTeam.Beskrivelse 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-ButikkTeam ButikkTeam.TeamNr 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-ButikkTeam ButikkTeam
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-ButikkTeam ButikkTeam
&Scoped-define QUERY-STRING-BROWSE-ButikkTeam FOR EACH ButikkTeam NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-ButikkTeam OPEN QUERY BROWSE-ButikkTeam FOR EACH ButikkTeam NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-ButikkTeam ButikkTeam
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-ButikkTeam ButikkTeam


/* Definitions for DIALOG-BOX Dialog-Frame                              */

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS CB-TeamType FILL-IN-SOK-CHAR ~
FILL-IN-SOK-INTE FILL-IN-SOK-DATE BUTTON-Sok BROWSE-ButikkTeam Btn_OK ~
Btn_Help 
&Scoped-Define DISPLAYED-OBJECTS CB-TeamType FILL-IN-SOK-CHAR ~
FILL-IN-SOK-INTE FILL-IN-SOK-DATE 
...
DEFINE VARIABLE CB-TeamType AS CHARACTER FORMAT "X(256)":U 
     LABEL "Teamtype" 
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY BROWSE-ButikkTeam FOR 
      ButikkTeam SCROLLING.
&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE BROWSE-ButikkTeam
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-ButikkTeam Dialog-Frame _STRUCTURED
  QUERY BROWSE-ButikkTeam NO-LOCK DISPLAY
      ButikkTeam.TeamNr FORMAT "zzz9":U
      ButikkTeam.Beskrivelse FORMAT "X(40)":U
  ENABLE
      ButikkTeam.TeamNr
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 77 BY 14.76 ROW-HEIGHT-CHARS .62 EXPANDABLE.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME Dialog-Frame
     CB-TeamType AT ROW 1.48 COL 46 COLON-ALIGNED
     ...
     BROWSE-ButikkTeam AT ROW 2.91 COL 2
     Btn_OK AT ROW 17.91 COL 2
     Btn_Cancel AT ROW 17.91 COL 15.2
     Btn_Help AT ROW 17.91 COL 67
     BUTTON-Ny AT ROW 2.91 COL 80.6
     BUTTON-Endre AT ROW 4.24 COL 80.6
     BUTTON-Slett AT ROW 5.57 COL 80.6
     SPACE(1.39) SKIP(12.30)
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   Custom                                                               */
/* BROWSE-TAB BROWSE-ButikkTeam BUTTON-Sok Dialog-Frame */
ASSIGN 
       FRAME Dialog-Frame:SCROLLABLE       = FALSE
       FRAME Dialog-Frame:HIDDEN           = TRUE.

ASSIGN 
       BROWSE-ButikkTeam:MAX-DATA-GUESS IN FRAME Dialog-Frame         = 481.
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE BROWSE-ButikkTeam
/* Query rebuild information for BROWSE BROWSE-ButikkTeam
     _TblList          = "SkoTex.ButikkTeam"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _FldNameList[1]   > SkoTex.ButikkTeam.TeamNr
"ButikkTeam.TeamNr" ? ? "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   = SkoTex.ButikkTeam.Beskrivelse
     _Query            is NOT OPENED
*/  /* BROWSE BROWSE-ButikkTeam */
&ANALYZE-RESUME
...
&Scoped-define BROWSE-NAME BROWSE-ButikkTeam
&Scoped-define SELF-NAME BROWSE-ButikkTeam
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-ButikkTeam Dialog-Frame
ON ANY-PRINTABLE OF BROWSE-ButikkTeam IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-ButikkTeam Dialog-Frame
ON CURSOR-LEFT OF BROWSE-ButikkTeam IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-ButikkTeam Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-ButikkTeam IN FRAME Dialog-Frame
DO:
  IF wAntSortCol > 1 THEN
      RUN SD-CURSOR ("RIGHT").
  RETURN NO-APPLY.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-ButikkTeam Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-ButikkTeam IN FRAME Dialog-Frame
DO:
  IF wType = "V" THEN
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-ButikkTeam Dialog-Frame
ON HOME OF BROWSE-ButikkTeam IN FRAME Dialog-Frame
DO:
  APPLY "ENTRY" TO SELF.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-ButikkTeam Dialog-Frame
ON START-SEARCH OF BROWSE-ButikkTeam IN FRAME Dialog-Frame
DO:
...
DO:
  IF NOT BROWSE {&BROWSE-NAME}:FOCUSED-ROW = ? THEN DO:
  {&return-ip}
  .
...
DO:
  ...
  run {&VedlikeholdsRutine} (BrukerGrp.BrGrpNr,TeamType.TeamTypeId,input-output wRecid,"Endre").
...
DO:
  assign 
    wRecid = ?.
  run {&VedlikeholdsRutine} (BrukerGrp.BrGrpNr,TeamType.TeamTypeId,input-output wRecid,"Ny").
  if return-value = "AVBRYT" then
    return no-apply.
  ...
  if wOk = true then
    do transaction:
      ...
      if available b{&br-tabell} then
        do:
          for each ButikkKobling OF b{&br-tabell}:
...
&Scoped-define SELF-NAME CB-TeamType
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL CB-TeamType Dialog-Frame
ON VALUE-CHANGED OF CB-TeamType IN FRAME Dialog-Frame /* Teamtype */
DO:
  FIND TeamType NO-LOCK WHERE TeamType.Beskrivelse = CB-TeamType:SCREEN-VALUE.
  ASSIGN {&ip-variabel} = TeamType.TeamTypeId.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ON END-KEY UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK:
    FIND BrukerGrp WHERE BrukerGrp.BrGrpNr = wBrGrpNr NO-LOCK.
    FRAME {&FRAME-NAME}:TITLE = FRAME {&FRAME-NAME}:TITLE + " Brukergruppe: " +
                    BrukerGrp.Beskrivelse.
    IF NOT CAN-FIND(TeamType WHERE TeamType.TeamTypeId = 1 OR TeamType.TeamTypeId = 2) THEN
        RUN TypeKontroll. /* Skapar TeamType om ikke finnes */
    RUN InitCB.
    IF wType = "V" THEN DO:
        IF {&ip-variabel} <> ? THEN
            FIND TeamType WHERE TeamType.TeamTypeId = {&ip-variabel} NO-LOCK NO-ERROR.
        IF NOT AVAIL TeamType THEN
            FIND FIRST TeamType NO-LOCK.
        {&ip-variabel} = TeamType.TeamTypeId.
    END.
    ELSE
      FIND TeamType WHERE TeamType.TeamTypeId = {&ip-variabel} NO-LOCK.
    RUN InitVars. /* inkl open-query */
    IF RETURN-VALUE = "FEIL" THEN
       RETURN.
    RUN SX-INIT-BUTTONS.
    RUN enable_UI.
    ASSIGN CB-TeamType:SENSITIVE = wType = "V"
           CB-TeamType:SCREEN-VALUE = TeamType.Beskrivelse.
    run lockwindowupdate(frame {&FRAME-NAME}:hwnd).
    RUN SD-QUERY-OPEN.
    IF RETURN-VALUE = "FEIL" THEN
        LEAVE MAIN-BLOCK.
...
------------------------------------------------------------------------------*/
  DISPLAY CB-TeamType FILL-IN-SOK-CHAR FILL-IN-SOK-INTE FILL-IN-SOK-DATE 
      WITH FRAME Dialog-Frame.
  ENABLE CB-TeamType FILL-IN-SOK-CHAR FILL-IN-SOK-INTE FILL-IN-SOK-DATE 
         BUTTON-Sok BROWSE-ButikkTeam Btn_OK Btn_Help 
      WITH FRAME Dialog-Frame.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH TeamType NO-LOCK:
      ASSIGN wListItems = IF wListItems = "" THEN TeamType.Beskrivelse ELSE
                             wListItems + "," + TeamType.Beskrivelse.
  END.
  ASSIGN CB-TeamType:LIST-ITEMS IN FRAME {&FRAME-NAME} = wListItems.
END PROCEDURE.
    ...
    REPEAT WHILE VALID-HANDLE(wW):
        IF LOOKUP(wW:NAME,wSearchCols) > 0 THEN
            ASSIGN wW:PRIVATE-DATA      = PREP-PRIVATE-DATA(wW:HANDLE,wCount)
                   ...
                   wW:LABEL = Trim(wW:LABEL + STRING(LOOKUP("*",wW:LABEL," ") = 0," */"))
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE TypeKontroll Dialog-Frame 
PROCEDURE TypeKontroll :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    FIND TeamType WHERE TeamType.TeamTypeId = 1 NO-ERROR.
    IF NOT AVAIL TeamType THEN DO:
        CREATE TeamType.
        ASSIGN TeamType.TeamTypeId  = 1
               TeamType.Beskrivelse = "Order"
               TeamType.Notat       = "Obligatorisk".
    END.
    FIND TeamType WHERE TeamType.TeamTypeId = 2 NO-ERROR.
    IF NOT AVAIL TeamType THEN DO:
        CREATE TeamType.
        ASSIGN TeamType.TeamTypeId  = 2
               TeamType.Beskrivelse = "Rapport"
               TeamType.Notat       = "Obligatorisk".
    END.
