...
FOR EACH ListeLinje OF Lister NO-LOCK TRANSACTION:
  ...
  FIND levPost NO-LOCK where
    levPost.PostNr = Kunde.LevPostNr NO-ERROR.
...
PROCEDURE OpprettJobbLinje:
  ...
  assign
    ...
    JobbLinje.Char3      = Kunde.Adresse1
    JobbLinje.DivX[ 1]   = STRING(Kunde.KundeNr,">>>>>>>>>>>>>")
    JobbLinje.DivX[ 2]   = Kunde.Navn
    JobbLinje.DivX[ 5]   = Kunde.KontTelefon
    JobbLinje.DivX[ 6]   = Kunde.KontTelefaks
    JobbLinje.DivX[ 7]   = Kunde.KontMobil
    JobbLinje.DivX[ 8]   = Kunde.Adresse1
    JobbLinje.DivX[ 9]   = Kunde.KontNavn
    JobbLinje.DivX[10]   = Kunde.Land
    JobbLinje.DivX[11]   = Kunde.KontE-Post
    JobbLinje.DivX[14]   = Kunde.Adresse2
    JobbLinje.DivX[15]   = Kunde.PostNR
    JobbLinje.DivX[16]   = STRING(Kunde.MaksKredit)
    JobbLinje.DivX[17]   = Kunde.Telefon
    JobbLinje.DivX[18]   = Kunde.LevAdresse1
    JobbLinje.DivX[19]   = Kunde.LevLand
    JobbLinje.DivX[20]   = Kunde.LevPostNr
    JobbLinje.DivX[21]   = Kunde.Telefaks
    JobbLinje.DivX[22]   = Kunde.Mobil
    JobbLinje.DivX[24]   = STRING(Kunde.TypeId)
    JobbLinje.DivX[25]   = STRING(Kunde.GruppeId)
    JobbLinje.DivX[26]   = if AVAILABLE Post THEN Post.Beskrivelse ELSE ""
    JobbLinje.DivX[27]   = if AVAILABLE levPost THEN levPost.Beskrivelse ELSE ""
    ...
    JobbLinje.DivX[41] = if wSortering = 1 THEN JobbLinje.Char2
                         ELSE if wSortering = 2 THEN Kunde.Navn
                         ELSE if wSortering = 3 THEN Kunde.PostNr
                         ELSE ""
    JobbLinje.DivX[42] = if wSortering = 1 THEN ""
                         ELSE if wSortering = 2 THEN ""
                         ELSE if wSortering = 3 THEN Kunde.Navn
                         ELSE ""
