...
&IF "{&OPSYS}":U="WIN32":U &THEN
   &GLOB ProExtra ".\ocx\ProExtra.DLL"

def var hProExtraDLL as integer no-undo.
hProExtraDLL = ?.

/* why LoadLibrary and FreeLibrary?
   because it allows us to find the dll if we are not sure 
   about the location  */

RUN LoadLibraryA (/* "c:\u\Delphi\ProExtra\" + */ {&ProExtra}, output hProExtraDLL).
...
PROCEDURE BrowseForFolder EXTERNAL {&ProExtra} :
  ...
  define input  parameter lpInital   as CHAR.
