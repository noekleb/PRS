...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEFINE var wDataInn as char NO-UNDO.
  wDatainn = "blank.bmp".
&ELSE
  DEFINE input parameter wDataInn as char NO-UNDO.
&ENDIF
...
/* CtrlFrame OCXINFO:CREATE-CONTROL from: {9A93B740-C96B-11D0-8883-444553540000} type: Picbuf */
      CtrlFrame:MOVE-BEFORE(FI-Billedinfo:HANDLE IN FRAME Dialog-Frame).
...
OCXFile = SEARCH( "d-visbil2.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "d-visbil2.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  if search(wFilNavn) = ? then
    do:
      message "Finner ikke bilde:" wFilNavn view-as alert-box ERROR
        title "Feil".    
      return no-apply.
    end.

  /* Nullstiller memory mellom hver lasting av bilde. */
  chCtrlFrame:Picbuf:clear(2).
  ...
  assign
    FI-Billedinfo:screen-value in frame Dialog-Frame = wFilNavn.
END PROCEDURE.
