...
DEFINE TEMP-TABLE tOrdre NO-UNDO LIKE Ordre.
...
&scoped-define DB-BROWSER BROWSE-Ordre

&scoped-define br-tabell Ordre

/* Kolonne 1 som det kan sorteres på */
&scoped-define sok-field1 OrdreNr
...
&scoped-define index-col1 OrdreNr
...
&scoped-define BUFFER-COPY-USING USING OrdreNr LevNr

/* Denna måste finnas før riktig sortering vid uppstart */
&SCOPED-DEFINE SORTBY-PHRASE BY {&br-tabell}.{&sok-field1} DESCENDING
...
&Scoped-define FIELDS-IN-QUERY-BROWSE-Tag tOrdre.OrdreNr tOrdre.LevNr 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-Tag tOrdre.OrdreNr 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-Tag tOrdre
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-Tag tOrdre
&Scoped-define OPEN-QUERY-BROWSE-Tag OPEN QUERY BROWSE-Tag FOR EACH tOrdre NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-Tag tOrdre
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-Tag tOrdre
...
&Scoped-Define ENABLED-OBJECTS BROWSE-Ordre BROWSE-Tag Btn_OK Btn_Cancel ~
...
DEFINE QUERY BROWSE-Ordre FOR 
      Ordre
    FIELDS(Ordre.OrdreNr
      Ordre.EkstId
      Ordre.OrdreStatus
      Ordre.LevNr
      Ordre.SendtDato
      Ordre.Merknad) SCROLLING.

DEFINE QUERY BROWSE-Tag FOR 
      tOrdre
    FIELDS(tOrdre.OrdreNr
      tOrdre.LevNr) SCROLLING.
&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE BROWSE-Ordre
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Ordre Dialog-Frame _STRUCTURED
  QUERY BROWSE-Ordre NO-LOCK DISPLAY
      Ordre.OrdreNr FORMAT "zzzzzz9":U
      Ordre.EkstId COLUMN-LABEL "Ekstern ref" FORMAT "X(15)":U
      Ordre.OrdreStatus FORMAT ">9":U
      Ordre.LevNr FORMAT "zzzzz9":U
      Ordre.SendtDato FORMAT "99/99/9999":U
      Ordre.Merknad FORMAT "X(40)":U WIDTH 11.8
  ENABLE
      Ordre.OrdreNr
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH SEPARATORS MULTIPLE SIZE 73 BY 13.95 ROW-HEIGHT-CHARS .62 EXPANDABLE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Tag Dialog-Frame _STRUCTURED
  QUERY BROWSE-Tag NO-LOCK DISPLAY
      tOrdre.OrdreNr FORMAT "zzzzzz9":U
      tOrdre.LevNr FORMAT "zzzzz9":U WIDTH 21.4
  ENABLE
      tOrdre.OrdreNr
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH SEPARATORS MULTIPLE SIZE 34 BY 13.95 ROW-HEIGHT-CHARS .62 EXPANDABLE.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME Dialog-Frame
     BROWSE-Ordre AT ROW 2.91 COL 3
     BUTTON-LeggTil AT ROW 6.71 COL 78.6
     BUTTON-TrekkFra AT ROW 7.91 COL 78.6
     BUTTON-AlleFra AT ROW 9.1 COL 78.6
     BROWSE-Tag AT ROW 2.91 COL 96
     Btn_OK AT ROW 17.38 COL 2
     Btn_Cancel AT ROW 17.38 COL 15
     Btn_Help AT ROW 17.38 COL 119
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: tOrdre T "?" NO-UNDO skotex Ordre
   END-TABLES.
 */
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   Custom                                                               */
/* BROWSE-TAB BROWSE-Ordre 1 Dialog-Frame */
...
"OrdreNr" ? ? "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > skotex.Ordre.EkstId
"EkstId" "Ekstern ref" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
"Merknad" ? ? "character" ? ? ? ? ? ? no ? no no "11.8" yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-Ordre */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE BROWSE-Tag
     _TblList          = "Temp-Tables.tOrdre"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _TblOptList       = "USED"
     _FldNameList[1]   > Temp-Tables.tOrdre.OrdreNr
"OrdreNr" ? ? "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > Temp-Tables.tOrdre.LevNr
"LevNr" ? ? "integer" ? ? ? ? ? ? no ? no no "21.4" yes no no "U" "" ""
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Velg ordre */
DO:
...
ON CURSOR-LEFT OF BROWSE-Ordre IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Ordre Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-Ordre IN FRAME Dialog-Frame
DO:
  IF {&ant-sok-field} = 1 THEN
    RETURN NO-APPLY.
  RUN SD-CURSOR ("RIGHT").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Ordre Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-Ordre IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Ordre Dialog-Frame
ON START-SEARCH OF BROWSE-Ordre IN FRAME Dialog-Frame
DO:
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BROWSE-Ordre BROWSE-Tag Btn_OK Btn_Cancel Btn_Help BUTTON-Sok RECT-4 
         RECT-5 
      WITH FRAME Dialog-Frame.
