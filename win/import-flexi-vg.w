...
DEF VAR lVgNye         AS LOG NO-UNDO.
DEF VAR lVgOverskriv   AS LOG NO-UNDO.
...
ON END-ERROR OF C-Win /* Import av varegrupper */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Import av varegrupper */
DO:
...
{syspara.i 1 2 1003 cEDB-System}
{syspar2.i 1 2 1003 cImpTabell}
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH ImportLinje EXCLUSIVE-LOCK WHERE
      ...
      ASSIGN
        piVg = int(ImportLinje.Felt[1])
        .

      /* Utfører konvertering av varegruppen. */
      ...
      /* Er det ny varegruppe, sjekk om den skal opprettes ? */
      IF lVgNye = FALSE AND NOT AVAILABLE VarGr THEN
          NEXT MAINLOOP.
      
      /* Sjekker om hovedgruppen skal konverteres. */
      FIND FIRST ImpKonv NO-LOCK WHERE 
          ImpKonv.EDB-System = cEDB-System AND 
          ImpKonv.Tabell     = cImpHg AND 
      ...
      DO:
          ...
          ASSIGN
              /* '#KOD'  */ VarGr.Vg   = piVg
              /* '#MOMS' */ VarGr.Moms = INT(ImportLinje.Felt[4]) 
              .
      END.

      /* Hvis det ikke er en ny varegrupper, skal data overskrives ? */
      IF lVgOverskriv = FALSE AND NOT NEW VarGr THEN
          NEXT MAINLOOP.

      ASSIGN
        /* '#HGRUPP'      */  VarGr.Hg        = piHg
        /* '#BESK'        */  VarGr.VgBeskr   = ImportLinje.Felt[3]
        /* '#MOMS'        */  /* VarGr.Moms      = INT(ImportLinje.Felt[4]) */
        /* '#MARGINAL'    */  VarGr.Kost_Proc = dec(ImportLinje.Felt[5])
        /* '#STARTSÄSONG' */
        /* '#SLUTSÄSONG'  */
        /* '#GRPARTGRP'   */
        .
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEF INPUT PARAMETER plVgNye       AS LOG NO-UNDO.
  DEF INPUT PARAMETER plVgOverskriv AS LOG NO-UNDO.
  
  ASSIGN
    lVgNye       = plVgNye      
    lVgOverskriv = plVgOverskriv
    .
END PROCEDURE.
