...
def var wFMatKod   as char no-undo.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE ImporterMaterialkoder Procedure 
PROCEDURE ImporterMaterialkoder :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
put stream LoggData unformatted 
  " " skip
  " -----------------------------------------------------------------------" skip
  " Import av materialkoder fra eXcellenc PRO Innkjøpssystem startet         " skip
  ...
  repeat on error undo TRANSBLOKK, leave TRANSBLOKK:
    ...
    assign
      ...
      do:
        assign
          wAntBest     = wAntBest + 1
          wFMatKod     = trim(substring(wLinje,4,3))
          wFTekst      = trim(substring(wLinje,7,25)).

        /* Sjekker numeriske felt */
        if SjekkStreng(trim(wFMatKod)) = false then
          do:
            OUTPUT STREAM LoggData to value(wLogFil) no-echo append.
            put stream LoggData unformatted 
              " Feil i materialkode: " wFMatKod " Linje:" wTotAntall skip. 
            OUTPUT STREAM LoggData close.
            next IMPORT_AV_DATA. 
          end.          

        /* Sjekker lengden på kodefeltet */
        if length(trim(wFMatKod)) > 2 then
          do:
            OUTPUT STREAM LoggData to value(wLogFil) no-echo append.
            put stream LoggData unformatted 
              " Materialkode større enn 99: " wFMatKod " Linje:" wTotAntall skip. 
            OUTPUT STREAM LoggData close.
            next IMPORT_AV_DATA. 
          end.          
        ...
        find Material exclusive-lock where
          Material.MatKod = int(wFMatKod) no-error.
        if not available Material then
          do:
            create Material.
            assign
              Material.MatKod = int(wFMatKod).
          end.
        assign
          Material.MatBeskr = wFTekst.
        ...
        if valid-handle(wProgram-Handle) then
          run ProfilInfo in wProgram-Handle (input string(wFMatKod) + " " +
                                        string(wFTekst)).
      end. /* DATA */    
