...
/* &scope QWhere      'TransLogg.ArtikkelNr = ' + STRING(wArtikkelNr) + ' AND TransLogg.TTId = 1' */
...
TransLogg.OvButik wTransType TransLogg.Dato TransLogg.Storl ~
TransLogg.Antall TransLogg.Pris TransLogg.RabKr TransLogg.Mva ~
TransLogg.Postert TransLogg.PostertDato TransLogg.TilStorl TransLogg.KundNr ~
TransLogg.MedlemsNr TransLogg.KortType TransLogg.KortNr TransLogg.ForsNr ~
TransLogg.Plukket TransLogg.TransNr TransLogg.KassaNr TransLogg.BongId ~
TransLogg.BongLinjeNr TransLogg.BestNr TransLogg.LevNr 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-TransLogg C-Win _STRUCTURED
  QUERY BROWSE-TransLogg NO-LOCK DISPLAY
      ...
      TransLogg.Storl COLUMN-LABEL "Str" FORMAT "x(10)":U
...
DEFINE FRAME DEFAULT-FRAME
     ...
     BUTTON-Motposter AT ROW 1.48 COL 127
     Btn_Help AT ROW 1.48 COL 147
     Btn_OK AT ROW 1.48 COL 152
     T-VisAlle AT ROW 1.62 COL 114
...
/* SETTINGS FOR BUTTON BUTTON-Motposter IN FRAME DEFAULT-FRAME
   NO-ENABLE                                                            */
ASSIGN 
       BUTTON-Motposter:HIDDEN IN FRAME DEFAULT-FRAME           = TRUE.
...
/* SETTINGS FOR FILL-IN FILL-IN-SOK-INTE IN FRAME DEFAULT-FRAME
   ALIGN-L                                                              */
/* SETTINGS FOR TOGGLE-BOX T-VisAlle IN FRAME DEFAULT-FRAME
   NO-ENABLE                                                            */
...
"TransLogg.Butik" ? "zzzzz9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > SkoTex.TransLogg.OvButik
"TransLogg.OvButik" "TilBut" "zzzzz9" "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > "_<CALC>"
"wTransType" "TransType" "x(15)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > SkoTex.TransLogg.Dato
"TransLogg.Dato" "Dato" ? "date" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > SkoTex.TransLogg.Storl
"TransLogg.Storl" "Str" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   = SkoTex.TransLogg.Antall
     _FldNameList[7]   = SkoTex.TransLogg.Pris
     _FldNameList[8]   = SkoTex.TransLogg.RabKr
     _FldNameList[9]   = SkoTex.TransLogg.Mva
     _FldNameList[10]   > SkoTex.TransLogg.Postert
"TransLogg.Postert" "Post" ? "logical" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[11]   > SkoTex.TransLogg.PostertDato
"TransLogg.PostertDato" "PosttDato" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[12]   = SkoTex.TransLogg.TilStorl
     _FldNameList[13]   > SkoTex.TransLogg.KundNr
"TransLogg.KundNr" ? ? "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[14]   = SkoTex.TransLogg.MedlemsNr
     _FldNameList[15]   = SkoTex.TransLogg.KortType
     _FldNameList[16]   = SkoTex.TransLogg.KortNr
     _FldNameList[17]   > SkoTex.TransLogg.ForsNr
"TransLogg.ForsNr" ? ? "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[18]   = SkoTex.TransLogg.Plukket
     _FldNameList[19]   > SkoTex.TransLogg.TransNr
"TransLogg.TransNr" "TransNr" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[20]   > SkoTex.TransLogg.KassaNr
"TransLogg.KassaNr" "Kasse" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[21]   = SkoTex.TransLogg.BongId
     _FldNameList[22]   = SkoTex.TransLogg.BongLinjeNr
     _FldNameList[23]   = SkoTex.TransLogg.BestNr
     _FldNameList[24]   > SkoTex.TransLogg.LevNr
"TransLogg.LevNr" "LevNr" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
RETURN.
  ...
  create tmpChild.  
  run w-vtranslogg persistent set tmpChild.wChild (input-output wBatchNr).
END.
...
------------------------------------------------------------------------------*/
  ...
  cTTId = "1,3,6,10".
  DO ii = 1 TO NUM-ENTRIES(cTTId):
      FIND TransType WHERE TransType.TTID = INT(ENTRY(ii,cTTId)) NO-LOCK.
      IF AVAIL TransType THEN
        assign cListItem = cListItem + (IF cListItem <> "" THEN "," ELSE "") + string(TransType.TTId,"zz9") + ": " + TransType.Beskrivelse.
  END.
/*   FIND TransType WHERE TransType.TTID = 1 NO-LOCK.                                  */
/*     assign cListItem = string(TransType.TTId,"zz9") + ": " + TransType.Beskrivelse. */
    assign
      CB-TTId:list-items   in frame DEFAULT-FRAME = cListItem
      CB-TTId:screen-value in frame DEFAULT-FRAME = ENTRY(1,cListItem).
END PROCEDURE.
  ...
  assign
    ...
    wWhere = wWhere + " " +
             (if (CB-TTId <> wAlle and CB-TTId <> "")
               then (if wWhere = "" then "" else "and ") + ("TransLogg.TTId = " + entry(1,CB-TTId:SCREEN-VALUE,':'))
               else "")
    wWhere = wWhere + " " +
             (if T-VisAlle = false
