...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEFINE VAR wOrdreNr LIKE Ordre.OrdreNr NO-UNDO.
  define var wLevNr   like LevBas.LevNr  no-undo.
&ELSE
  DEFINE INPUT-OUTPUT PARAMETER wOrdreNr LIKE Ordre.OrdreNr NO-UNDO.
  define input        parameter wLevNr   like LevBas.LevNr  no-undo.
&ENDIF
...
&scoped-define sok-field2  LevNr
...
&scoped-define index-col2  LevOrdNr
...
&scoped-define sok-field3  SendtDato
...
&scoped-define index-col3  SendtDato
...
&scoped-define ip-variabel wOrdreNr
&scoped-define return-ip   wOrdreNr = if available Ordre ~
                                        then Ordre.OrdreNr ~
                                        else 0.
&scoped-define assign-retur-verdi retur-verdi = if available Ordre ~
                                                  then string(recid(Ordre)) ~
                                                  else "".
...
Ordre.OrdreStatus Ordre.LevNr Ordre.SendtDato 
...
&Scoped-define QUERY-STRING-BROWSE-Ordre FOR EACH Ordre ~
      WHERE Ordre.LevNr = wLevNr and ~
Ordre.OrdreStatus = 1 ~
 AND Ordre.LapTop = wLaptop NO-LOCK ~
...
FILL-IN-SOK-CHAR BUTTON-Sok BROWSE-Ordre Btn_OK Btn_Cancel B-Ny Btn_Help 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Ordre Dialog-Frame _STRUCTURED
  QUERY BROWSE-Ordre NO-LOCK DISPLAY
      Ordre.OrdreNr COLUMN-LABEL "OrdreNr *" FORMAT ">>>>>>>9":U
            WIDTH 10.2
      Ordre.Merknad FORMAT "X(40)":U
      Ordre.OrdreStatus FORMAT ">9":U
      Ordre.LevNr COLUMN-LABEL "Leverandør *" FORMAT "zzzzz9":U
            WIDTH 13
      Ordre.SendtDato COLUMN-LABEL "Sendt dato *" FORMAT "99/99/9999":U
            WIDTH 12
  ENABLE
      Ordre.OrdreNr
      Ordre.LevNr
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 90 BY 14.76 ROW-HEIGHT-CHARS .62.
...
DEFINE FRAME Dialog-Frame
     ...
     Btn_OK AT ROW 18.14 COL 3
     Btn_Cancel AT ROW 18.14 COL 16
     B-Ny AT ROW 18.14 COL 31.6
     Btn_Help AT ROW 18.14 COL 78
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   FRAME-NAME                                                           */
/* BROWSE-TAB BROWSE-Ordre BUTTON-Sok Dialog-Frame */
ASSIGN 
...
/* Query rebuild information for BROWSE BROWSE-Ordre
     ...
     _Where[1]         = "Ordre.LevNr = wLevNr and
Ordre.OrdreStatus = 1
 AND Ordre.LapTop = wLaptop"
     _FldNameList[1]   > SkoTex.Ordre.OrdreNr
"Ordre.OrdreNr" "OrdreNr *" ? "integer" ? ? ? ? ? ? yes ? no no "10.2" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = SkoTex.Ordre.Merknad
     _FldNameList[3]   = SkoTex.Ordre.OrdreStatus
     _FldNameList[4]   > SkoTex.Ordre.LevNr
"Ordre.LevNr" "Leverandør *" ? "integer" ? ? ? ? ? ? yes ? no no "13" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > SkoTex.Ordre.SendtDato
"Ordre.SendtDato" "Sendt dato *" ? "date" ? ? ? ? ? ? yes ? no no "12" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-Ordre */
&ANALYZE-RESUME
...
DO:
    RUN d-nyordre.w (INPUT-OUTPUT wOrdreNr,INPUT wLevNr).
    IF wOrdreNr <> ? THEN DO:
        FIND Ordre WHERE Ordre.OrdreNr = wOrdreNr NO-LOCK NO-ERROR.
        IF AVAIL Ordre THEN
...
ON ANY-PRINTABLE OF BROWSE-Ordre IN FRAME Dialog-Frame
DO:
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-DATE FILL-IN-SOK-INT FILL-IN-SOK-CHAR BUTTON-Sok 
         BROWSE-Ordre Btn_OK Btn_Cancel B-Ny Btn_Help 
      WITH FRAME Dialog-Frame.
