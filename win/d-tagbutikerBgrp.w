...
DEFINE TEMP-TABLE tButiker NO-UNDO LIKE Butiker
       field TagFlagg as log initial true.
...
&scoped-define DB-BROWSER BROWSE-Butiker
...
&scoped-define index-col1  ButikIn
...
/*   
&scoped-define BUFFER-COPY-USING USING ButNamn TagFlagg
*/
...
DEFINE VAR cButikerBgrp      AS CHAR        NO-UNDO.
{runlib.i}
...
&Scoped-define INTERNAL-TABLES Butiker tButiker
...
&Scoped-define QUERY-STRING-BROWSE-Butiker FOR EACH Butiker ~
      WHERE CAN-DO(cButikerBgrp,STRING(Butiker.butik)) NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-Butiker OPEN QUERY BROWSE-Butiker FOR EACH Butiker ~
      WHERE CAN-DO(cButikerBgrp,STRING(Butiker.butik)) NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
...
&Scoped-define FIELDS-IN-QUERY-BROWSE-Tag tButiker.Butik tButiker.ButNamn 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-Tag tButiker.Butik 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-Tag tButiker
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-Tag tButiker
&Scoped-define QUERY-STRING-BROWSE-Tag FOR EACH tButiker NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-Tag OPEN QUERY BROWSE-Tag FOR EACH tButiker NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-Tag tButiker
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-Tag tButiker
...
&Scoped-Define ENABLED-OBJECTS BROWSE-Butiker BROWSE-Tag Btn_OK Btn_Cancel ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Butiker Dialog-Frame _STRUCTURED
  QUERY BROWSE-Butiker NO-LOCK DISPLAY
      ...
      Butiker.ButNamn FORMAT "x(30)":U WIDTH 30.8
  ENABLE
      Butiker.Butik
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH SEPARATORS MULTIPLE SIZE 44 BY 12.19.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Tag Dialog-Frame _STRUCTURED
  QUERY BROWSE-Tag NO-LOCK DISPLAY
      tButiker.Butik FORMAT ">>>>>9":U
      tButiker.ButNamn FORMAT "x(30)":U
  ENABLE
      tButiker.Butik
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH SEPARATORS MULTIPLE SIZE 41 BY 12.19 ROW-HEIGHT-CHARS .62.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME Dialog-Frame
     BROWSE-Butiker AT ROW 2.91 COL 3
     BUTTON-LeggTil AT ROW 6.71 COL 49
     BUTTON-TrekkFra AT ROW 7.91 COL 49
     BUTTON-AlleFra AT ROW 9.1 COL 49
     BROWSE-Tag AT ROW 2.91 COL 66
     Btn_OK AT ROW 15.52 COL 2
     Btn_Cancel AT ROW 15.52 COL 15
     Btn_Help AT ROW 15.52 COL 96
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: tButiker T "?" NO-UNDO skotex Butiker
      ADDITIONAL-FIELDS:
          field TagFlagg as log initial true
      END-FIELDS.
   END-TABLES.
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   Custom                                                               */
/* BROWSE-TAB BROWSE-Butiker 1 Dialog-Frame */
...
/* Query rebuild information for BROWSE BROWSE-Butiker
     ...
     _Where[1]         = "CAN-DO(cButikerBgrp,STRING(Butiker.butik))"
     _FldNameList[1]   > skotex.Butiker.Butik
"Butiker.Butik" ? ? "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > skotex.Butiker.ButNamn
"Butiker.ButNamn" ? "x(30)" "character" ? ? ? ? ? ? no ? no no "30.8" yes no no "U" "" ""
     _Query            is OPENED
...
/* Query rebuild information for BROWSE BROWSE-Tag
     _TblList          = "Temp-Tables.tButiker"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _FldNameList[1]   > Temp-Tables.tButiker.Butik
"tButiker.Butik" ? ? "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > Temp-Tables.tButiker.ButNamn
"tButiker.ButNamn" ? "x(30)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Butiker Dialog-Frame
ON VALUE-CHANGED OF BROWSE-Butiker IN FRAME Dialog-Frame
DO:
    IF BROWSE BROWSE-Tag:NUM-SELECTED-ROWS > 0 THEN DO:
        IF BROWSE BROWSE-Tag:DESELECT-ROWS() THEN.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ON END-KEY UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK:
    ...
    IF CAN-FIND(FIRST t{&br-tabell}) THEN DO:
        ...
        BUTTON-AlleFra:SENSITIVE IN FRAME {&FRAME-NAME} = YES.
    END.
    IF BROWSE {&DB-BROWSER}:FOCUSED-ROW <> ? THEN DO:
        APPLY "ENTRY" TO BROWSE {&DB-BROWSER}.
        BROWSE {&DB-BROWSER}:SELECT-FOCUSED-ROW().
        APPLY "VALUE-CHANGED" TO BROWSE {&DB-BROWSER}.
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE ButikerBgrp Dialog-Frame 
PROCEDURE ButikerBgrp :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    FOR EACH butikktilgang OF BrukerGrp NO-LOCK.
        ASSIGN cButikerBgrp = cButikerBgrp + (IF cButikerBgrp <> "" THEN "," ELSE "") +
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BROWSE-Butiker BROWSE-Tag Btn_OK Btn_Cancel Btn_Help BUTTON-Sok RECT-4 
         RECT-5 
      WITH FRAME Dialog-Frame.
