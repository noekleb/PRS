...
&Scoped-Define ENABLED-OBJECTS B-AlleBut FI-Aar FI-Mnd FI-Aar2 B-SokButikk ~
FI-Mnd2 Btn_OK Btn_Cancel Btn_Help RECT-49 
&Scoped-Define DISPLAYED-OBJECTS FI-Aar FI-Mnd FI-Aar2 FI-Mnd2 FI-Butikk ~
FILL-IN-3 FI-FraTxt FI-TilTxt 
...
DEFINE VARIABLE FI-Mnd AS INTEGER FORMAT "99" INITIAL 0 
     VIEW-AS FILL-IN 
     SIZE 8 BY 1.

DEFINE VARIABLE FI-Mnd2 AS INTEGER FORMAT "99" INITIAL 0 
     VIEW-AS FILL-IN 
     SIZE 8 BY 1.
...
DEFINE FRAME Dialog-Frame
     ...
     FI-Mnd AT ROW 3.57 COL 22 COLON-ALIGNED NO-LABEL
     FI-Aar2 AT ROW 3.57 COL 38 COLON-ALIGNED NO-LABEL
     B-SokButikk AT ROW 4.81 COL 33 NO-TAB-STOP 
     FI-Mnd2 AT ROW 3.57 COL 50 COLON-ALIGNED NO-LABEL
...
DO:
  ...
  else if (input FI-Aar = input FI-Aar2) then
    do:
      if (input FI-Mnd > input FI-Mnd2) then
        do:
          message "Ugyldig ukesangivelse!" 
            view-as alert-box message title "Melding".
          return no-apply.
        end.  
    end.

  if  input FI-Mnd  = 0 or
      input FI-Mnd2 = 0 or
      input FI-Mnd  > 53 or 
      input FI-Mnd2 > 53 then
    do:
      message "Ugyldig ukesangivelse!" 
        view-as alert-box message title "Melding".
      return no-apply.
  ...
  assign
    wKriterier = string(input FI-Aar) + "," + 
                 string(input FI-Aar2) + "," +
                 string(input FI-Mnd) + "," +
                 string(input FI-Mnd2) + "," +
                 string(wButik).    
  assign
    wRetStatus = "OK".
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  IF wKriterier <> "" THEN
    DO:
      ASSIGN
        ...
        FI-Mnd    = int(ENTRY(3,wKriterier))
        FI-Mnd2   = int(ENTRY(4,wKriterier))
  ...
  ELSE 
    display
      ...
      year(today)      @ FI-Aar2
      1                @ FI-Mnd
      ((today + 1) - date(1,1,year(today))) / 7  @ FI-Mnd2
  with frame Dialog-Frame.
...
------------------------------------------------------------------------------*/
  DISPLAY FI-Aar FI-Mnd FI-Aar2 FI-Mnd2 FI-Butikk FILL-IN-3 FI-FraTxt FI-TilTxt 
      WITH FRAME Dialog-Frame.
  ENABLE B-AlleBut FI-Aar FI-Mnd FI-Aar2 B-SokButikk FI-Mnd2 Btn_OK Btn_Cancel 
         Btn_Help RECT-49 
      WITH FRAME Dialog-Frame.
