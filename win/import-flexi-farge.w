...
DEF VAR lFargNye         AS   LOG                NO-UNDO.
DEF VAR lFargOverskriv   AS   LOG                NO-UNDO.
...
ON END-ERROR OF C-Win /* Import av farge */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Import av farge */
DO:
...
{syspara.i 1 2 1004 cEDB-System}
{syspar2.i 1 2 1004 cImpTabell}
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH ImportLinje EXCLUSIVE-LOCK WHERE
      ...
      FIND FIRST ImpKonv NO-LOCK WHERE 
          ...
          ImpKonv.EksterntId = ImportLinje.Felt[2] NO-ERROR.
      IF AVAILABLE ImpKonv THEN
        ASSIGN
          piFarg = int(ImpKonv.InterntId)
          .

      FIND Farg EXCLUSIVE-LOCK WHERE
          Farg.Farg = piFarg NO-ERROR.
      
      /* Er det ny leverandør, sjekk om den skal opprettes ? */
      IF lFargNye = FALSE AND NOT AVAILABLE Farg THEN
          NEXT MAINLOOP.
      ...
      DO:
          CREATE Farg.
          ASSIGN
              /* '#KOD' */ Farg.Farg = piFarg
              .
      END.
      
      /* Hvis det ikke er en ny varegrupper, skal data overskrives ? */
      IF lFargOverskriv = FALSE AND NOT NEW Farg THEN
          NEXT MAINLOOP.

      ASSIGN
        /* '#GRFÄRG'   */  
        /* '#BESK'     */ Farg.FarBeskr = ImportLinje.Felt[3]
        /* '#EGENFÄRG' */
        .
  END. /* MAINLOOP */
END PROCEDURE.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEF INPUT PARAMETER plFargNye       AS LOG NO-UNDO.
  DEF INPUT PARAMETER plFargOverskriv AS LOG NO-UNDO.
  
  ASSIGN
    lFargNye       = plFargNye      
    lFargOverskriv = plFargOverskriv
    .
END PROCEDURE.
