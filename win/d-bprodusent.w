...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEFINE VAR wProdNr LIKE Produsent.ProdNr NO-UNDO.
&ELSE
  DEFINE INPUT-OUTPUT PARAMETER wProdNr LIKE Produsent.ProdNr NO-UNDO.
&ENDIF
...
&scoped-define sok-field1  ProdNr
...
&scoped-define index-col1  ProdNr
...
&scoped-define ip-variabel wProdNr
&scoped-define return-ip   wProdNr = if available Produsent  ~
                                       then Produsent.ProdNr ~
                                       else 0.
&scoped-define assign-retur-verdi retur-verdi = if available Produsent ~
                                                  then string(recid(Produsent)) ~
                                                  else ?.
&scoped-define SletteMelding "Skal produsenten slettes?"
&scoped-define KanSlettes ~
  if can-find(first ArtBas where ArtBas.ProdNr = Produsent.ProdNr) then ~
    do: ~
      message "Det finnes artikkler merket med dette produsentnummer!" skip ~
...
&scoped-define VedlikeholdsRutine d-vprodusent.w
...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME BROWSE-Produsent

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES Produsent

/* Definitions for BROWSE BROWSE-Produsent                              */
&Scoped-define FIELDS-IN-QUERY-BROWSE-Produsent Produsent.ProdNr ~
Produsent.Beskrivelse 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-Produsent Produsent.ProdNr ~
Produsent.Beskrivelse 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-Produsent Produsent
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-Produsent Produsent
&Scoped-define QUERY-STRING-BROWSE-Produsent FOR EACH Produsent NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-Produsent OPEN QUERY BROWSE-Produsent FOR EACH Produsent NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-Produsent Produsent
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-Produsent Produsent


/* Definitions for DIALOG-BOX Dialog-Frame                              */
&Scoped-define OPEN-BROWSERS-IN-QUERY-Dialog-Frame ~
    ~{&OPEN-QUERY-BROWSE-Produsent}

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS FILL-IN-SOK-INT FILL-IN-SOK-DATE ~
FILL-IN-SOK-CHAR BUTTON-Sok B-KonvTbl BROWSE-Produsent BUTTON-Ny ~
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY BROWSE-Produsent FOR 
      Produsent SCROLLING.
&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE BROWSE-Produsent
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Produsent Dialog-Frame _STRUCTURED
  QUERY BROWSE-Produsent NO-LOCK DISPLAY
      Produsent.ProdNr COLUMN-LABEL "Produsent *" FORMAT "zzzzz9":U
      Produsent.Beskrivelse COLUMN-LABEL "Beskrivelse *" FORMAT "x(50)":U
  ENABLE
      Produsent.ProdNr
      Produsent.Beskrivelse
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 66 BY 10.29.
...
DEFINE FRAME Dialog-Frame
     ...
     B-KonvTbl AT ROW 1.48 COL 46
     BROWSE-Produsent AT ROW 2.91 COL 2
     BUTTON-Ny AT ROW 2.91 COL 68.6
     BUTTON-Endre AT ROW 4.24 COL 68.6
     BUTTON-Slett AT ROW 5.57 COL 68.6
     Btn_Help AT ROW 13.29 COL 68.6
     Btn_OK AT ROW 13.38 COL 2.2
     Btn_Cancel AT ROW 13.38 COL 14.6
     SPACE(57.00) SKIP(0.00)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste produsent"
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
                                                                        */
/* BROWSE-TAB BROWSE-Produsent B-KonvTbl Dialog-Frame */
ASSIGN 
       FRAME Dialog-Frame:SCROLLABLE       = FALSE
       FRAME Dialog-Frame:HIDDEN           = TRUE.

ASSIGN 
       BROWSE-Produsent:NUM-LOCKED-COLUMNS IN FRAME Dialog-Frame     = 1.
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE BROWSE-Produsent
/* Query rebuild information for BROWSE BROWSE-Produsent
     _TblList          = "SkoTex.Produsent"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _FldNameList[1]   > SkoTex.Produsent.ProdNr
"Produsent.ProdNr" "Produsent *" ? "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > SkoTex.Produsent.Beskrivelse
"Produsent.Beskrivelse" "Beskrivelse *" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-Produsent */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Søkeliste produsent */
DO:
...
DO:
  if not available Produsent then
    return.
  run d-bimpkonv.w (1510,
                    string(Produsent.ProdNr),
                    "Kobling av produsenter"
                    ).
  return no-apply.
END.
...
&Scoped-define BROWSE-NAME BROWSE-Produsent
&Scoped-define SELF-NAME BROWSE-Produsent
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Produsent Dialog-Frame
ON ANY-PRINTABLE OF BROWSE-Produsent IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Produsent Dialog-Frame
ON CURSOR-LEFT OF BROWSE-Produsent IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Produsent Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-Produsent IN FRAME Dialog-Frame
DO:
  IF {&ant-sok-field} = 1 THEN
    RETURN NO-APPLY.
  RUN SD-CURSOR ("RIGHT").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Produsent Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-Produsent IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Produsent Dialog-Frame
ON START-SEARCH OF BROWSE-Produsent IN FRAME Dialog-Frame
DO:
  RUN SD-START-SEARCH.
  RETURN NO-APPLY.
END.
...
{syspara.i 16 11 2 iTilgang INT}
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-INT FILL-IN-SOK-DATE FILL-IN-SOK-CHAR BUTTON-Sok B-KonvTbl 
         BROWSE-Produsent BUTTON-Ny BUTTON-Endre BUTTON-Slett Btn_Help Btn_OK 
         Btn_Cancel 
      WITH FRAME Dialog-Frame.
