...
/*------------------------------------------------------------------------
  Forfatter:    Tom Nøkleby T.N.Consult 12/10-98
  Beskrivelse:  Søkeliste StrType
  Parametere:   wStrTypeId
  Endringer:
...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEFINE VAR wStrTypeId LIKE StrType.StrTypeID NO-UNDO.
&ELSE
  DEFINE INPUT-OUTPUT PARAMETER wStrTypeId LIKE StrType.StrTypeID NO-UNDO.
&ENDIF
...
&scoped-define sok-field1  StrTypeID
...
&scoped-define index-col1  StrType
...
&scoped-define sok-field3  Beskrivelse
...
&scoped-define index-col3  Beskrivelse
...
&scoped-define ip-variabel wStrTypeID
&scoped-define return-ip   wStrTypeID = if available StrType  ~
                                       then StrType.StrTypeID ~
                                       else 0.
&scoped-define assign-retur-verdi retur-verdi = if available StrType ~
                                                  then string(recid(StrType)) ~
                                                  else ?.
&scoped-define SletteMelding "Skal størrelsestype slettes?"
&scoped-define KanSlettes ~
  if can-find(first LevSort where LevSort.StrTypeID = StrType.StrTypeId) or ~
     can-find(first ArtBas where ArtBas.StrTypeId   = StrType.StrTypeId) or ~
     can-find(first BestHode where BestHode.StrTypeId = StrType.StrTypeId) then ~
    do: ~
      message "Det finnes artikkler, bestillinger eller leverandørsortiment med denne størrelsestypen!" skip ~
...
&scoped-define VedlikeholdsRutine d-vstrtype.w
...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME BROWSE-StrType

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES StrType

/* Definitions for BROWSE BROWSE-StrType                                */
&Scoped-define FIELDS-IN-QUERY-BROWSE-StrType StrType.StrTypeID ~
StrType.KortNavn StrType.Beskrivelse StrType.Intervall 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-StrType StrType.StrTypeID ~
StrType.KortNavn StrType.Beskrivelse 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-StrType StrType
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-StrType StrType
&Scoped-define QUERY-STRING-BROWSE-StrType FOR EACH StrType NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-StrType OPEN QUERY BROWSE-StrType FOR EACH StrType NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-StrType StrType
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-StrType StrType
...
FILL-IN-SOK-DATE BUTTON-Sok B-KonvTbl BROWSE-StrType BUTTON-Ny BUTTON-Endre ~
BUTTON-Slett BUTTON-Storrelser B-Utskrift Btn_OK Btn_Help 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD stTyperOK Dialog-Frame 
FUNCTION stTyperOK RETURNS LOGICAL
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY BROWSE-StrType FOR 
      StrType SCROLLING.
&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE BROWSE-StrType
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-StrType Dialog-Frame _STRUCTURED
  QUERY BROWSE-StrType NO-LOCK DISPLAY
      StrType.StrTypeID COLUMN-LABEL "StrType *" FORMAT ">>>>>>9":U
      StrType.KortNavn COLUMN-LABEL "Kortnavn *" FORMAT "X(8)":U
      StrType.Beskrivelse COLUMN-LABEL "Beskrivelse *" FORMAT "X(60)":U
      StrType.Intervall FORMAT "x(12)":U WIDTH 71.2
  ENABLE
      StrType.StrTypeID
      StrType.KortNavn
      StrType.Beskrivelse
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 127 BY 12.86 ROW-HEIGHT-CHARS .62 FIT-LAST-COLUMN.
...
DEFINE FRAME Dialog-Frame
     ...
     B-KonvTbl AT ROW 1.48 COL 106.8
     BROWSE-StrType AT ROW 2.91 COL 2
     BUTTON-Ny AT ROW 2.91 COL 129.8
     BUTTON-Endre AT ROW 4.24 COL 129.8
     BUTTON-Slett AT ROW 5.57 COL 129.8
     BUTTON-Storrelser AT ROW 7.95 COL 129.8
     B-Utskrift AT ROW 12.19 COL 129.8
     Btn_OK AT ROW 15.86 COL 2.2
     Btn_Help AT ROW 15.86 COL 129.8
     SPACE(0.59) SKIP(0.00)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste størrelsestyper"
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   FRAME-NAME                                                           */
/* BROWSE-TAB BROWSE-StrType B-KonvTbl Dialog-Frame */
ASSIGN 
       FRAME Dialog-Frame:SCROLLABLE       = FALSE
       FRAME Dialog-Frame:HIDDEN           = TRUE.

ASSIGN 
       BROWSE-StrType:NUM-LOCKED-COLUMNS IN FRAME Dialog-Frame     = 1.
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE BROWSE-StrType
/* Query rebuild information for BROWSE BROWSE-StrType
     _TblList          = "SkoTex.StrType"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _FldNameList[1]   > SkoTex.StrType.StrTypeID
"StrType.StrTypeID" "StrType *" ">>>>>>9" "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > SkoTex.StrType.KortNavn
"StrType.KortNavn" "Kortnavn *" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > SkoTex.StrType.Beskrivelse
"StrType.Beskrivelse" "Beskrivelse *" "X(60)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > SkoTex.StrType.Intervall
"StrType.Intervall" ? ? "character" ? ? ? ? ? ? no ? no no "71.2" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is NOT OPENED
*/  /* BROWSE BROWSE-StrType */
&ANALYZE-RESUME
...
ON GO OF FRAME Dialog-Frame /* Søkeliste størrelsestyper */
DO:
    IF NOT stTyperOK() THEN
      RETURN NO-APPLY.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Dialog-Frame Dialog-Frame
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Søkeliste størrelsestyper */
DO:
  IF NOT stTyperOK() THEN
      RETURN NO-APPLY.
...
DO:
  if not available StrType then
    return.
  run d-bimpkonv.w (1508,
                    string(StrType.StrTypeId),
                    "Kobling av størrelsestyper"
                    ).
  return no-apply.
END.
...
&Scoped-define BROWSE-NAME BROWSE-StrType
&Scoped-define SELF-NAME BROWSE-StrType
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-StrType Dialog-Frame
ON ANY-PRINTABLE OF BROWSE-StrType IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-StrType Dialog-Frame
ON CURSOR-LEFT OF BROWSE-StrType IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-StrType Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-StrType IN FRAME Dialog-Frame
DO:
  IF {&ant-sok-field} = 1 THEN
    RETURN NO-APPLY.
  RUN SD-CURSOR ("RIGHT").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-StrType Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-StrType IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-StrType Dialog-Frame
ON START-SEARCH OF BROWSE-StrType IN FRAME Dialog-Frame
DO:
  RUN SD-START-SEARCH.
  RETURN NO-APPLY.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-StrType Dialog-Frame
ON VALUE-CHANGED OF BROWSE-StrType IN FRAME Dialog-Frame
DO:
  DO WITH FRAME {&FRAME-NAME}:
    ASSIGN B-KonvTbl:SENSITIVE         =
                          NOT CAN-DO(cStrTypeLst,STRING(StrType.StrTypeID))
           /*
           B-Utskrift:SENSITIVE        = B-KonvTbl:SENSITIVE
           BUTTON-Endre:SENSITIVE      = B-KonvTbl:SENSITIVE 
           BUTTON-Slett:SENSITIVE      = B-KonvTbl:SENSITIVE
           BUTTON-Storrelser:SENSITIVE = B-KonvTbl:SENSITIVE
           */
           .
...
DO:
  ...
  DO:
      MESSAGE "Størrelsestype 1 kan ikke slettes."
  ...
  if wOk = true then
    do transaction:
      ...
      if available b{&br-tabell} then
        do:
          for each StrTStr of b{&br-tabell} exclusive-lock:
            delete StrTStr.
          end.
          delete b{&br-tabell}.
...
ON CHOOSE OF BUTTON-Storrelser IN FRAME Dialog-Frame /* Størrelser... */
DO:
  ...
  DO:
      ...
      IF StrType.StrTypeId = 1 THEN
      DO:
          MESSAGE "Størrelsestype 1 kan ikke endres."
              VIEW-AS ALERT-BOX INFO BUTTONS OK.
          RETURN NO-APPLY.
      END.
      */
      run d-vstrtstr.w (input StrType.StrTypeID).
  END.
...
{syspara.i 5 4 2 cStrTypeLst} /* inte godkænd stuorleksdef. */
{syspara.i 16 13 2 iTilgang INT}

/* Oppretter størrelsestype 1 hvis den ikke finnes fra før.   */
/* Denne størrelsestypen skal ikke ha noen størrelser på seg. */
SJEKK-STORRELSER:
DO FOR bbStrType TRANSACTION:
    IF NOT CAN-FIND(bbStrType WHERE
                    bbStrType.StrTypeId = 1) THEN
    DO:
        CREATE bbStrType.
        ASSIGN
            bbStrType.StrTypeId   = 1
            bbStrType.Beskrivelse = "Ingen størrelser"
            .
        RELEASE bbStrType.
    END.
    /* Tar bort eventuelle størrelser på størrelsestype 1. */
    FOR EACH StrTStr EXCLUSIVE-LOCK WHERE
        StrTStr.StrTypeId = 1:
        DELETE StrTStr.
    END.
END. /* SJEKK-STORRELSER */
IF CAN-FIND(FIRST bbStrType WHERE bbStrType.StrTypeID > 1 AND
               (bbStrType.Fordeling = "" OR bbStrType.AlfaFordeling = "")) THEN DO:
    FOR EACH bbStrType WHERE bbStrType.StrTypeID > 1 AND 
                            (bbStrType.Fordeling = "" OR 
                             bbStrType.AlfaFordeling = "") NO-LOCK.
        {sww.i}
        RUN settStrTypeFelt.p (bbStrType.StrTypeID).
        {swn.i}
    END.
END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-INT FILL-IN-SOK-CHAR FILL-IN-SOK-DATE BUTTON-Sok B-KonvTbl 
         BROWSE-StrType BUTTON-Ny BUTTON-Endre BUTTON-Slett BUTTON-Storrelser 
         B-Utskrift Btn_OK Btn_Help 
      WITH FRAME Dialog-Frame.
...
------------------------------------------------------------------------------*/
  ...
  if valid-handle(wLibHandle) then
    run GetTempFileName in wLibHandle ("StrType", cExcEkstent, output cFileName). 
  ...
  FOR EACH StrType no-lock:
    ...
    EXPORT STREAM sExportFile DELIMITER ";"
      /* A */ StrType.StrTypeId     
      /* B */ StrType.KortNavn
      /* C */ StrType.Beskrivelse
      /* D */ StrType.Intervall
      /* E */ StrType.AlfaFordeling
      .
  ...
  chWorkSheets:Range("A1:E2"):Font:Bold = TRUE.
  chWorkSheets:Range("A1:E2"):Font:Italic = TRUE.
  ...
  */
  ...
  chWorkSheets:Columns("A:E"):AutoFit().
  ...
  chWorksheets:PageSetup:PrintArea      = "A:D".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION stTyperOK Dialog-Frame 
FUNCTION stTyperOK RETURNS LOGICAL
...
------------------------------------------------------------------------------*/
  ...
  IF CAN-FIND(FIRST StrType WHERE StrType.StrTypeId > 1 AND
              StrType.Intervall = "") THEN 
  DO:
      FOR EACH StrType NO-LOCK WHERE StrType.StrTypeId > 1 AND
          NOT CAN-FIND(FIRST StrTStr OF StrType):
            ASSIGN 
                 cString = cString + 
                           (IF cString = "" 
                                         THEN "" 
                           ...
                           STRING(StrType.StrTypeID).
      END.
      MESSAGE "Det finnes størrelsestype(r) som mangler størrelser." SKIP
              "Størrelsetype > 1" SKIP
              "Slett/registrer størrelser." SKIP
          "(" cString ")"
          VIEW-AS ALERT-BOX INFO BUTTONS OK.
      RETURN FALSE.
  END.
