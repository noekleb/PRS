...
DEFINE VARIABLE cExportFil      AS CHARACTER INIT "tekster."                NO-UNDO.
...
IF cLanButiker <> "" THEN DO:
  DO iCount = 1 TO NUM-ENTRIES(cLanButiker):
      RUN ExportTekster IN THIS-PROCEDURE (INT(ENTRY(iCount,cLanButiker)),"txt"). /* parameter = den loopade butiken */
  END.
END.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH TT_Tekster:
        EXPORT iButik
               TT_Tekster.teksttype
               TT_Tekster.tekstnr  
               TT_Tekster.transtype
               TT_Tekster.tekst    
               TT_Tekster.aksjon   
               .
