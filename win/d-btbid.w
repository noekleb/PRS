...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEFINE VAR wTBId LIKE TransBeskr.TBId NO-UNDO.
  DEFINE VAR wTTId LIKE TransBeskr.TTId NO-UNDO.
&ELSE
  DEFINE INPUT-OUTPUT PARAMETER wTBId LIKE TransBeskr.TBId NO-UNDO.
  DEFINE INPUT        PARAMETER wTTId LIKE TransBeskr.TBId NO-UNDO.
&ENDIF
...
&scoped-define index-col1  TransBeskr
...
&scoped-define ip-variabel wTBId
&scoped-define return-ip   wTBId = if available {&br-tabell}  ~
...
&Scoped-define OPEN-QUERY-BROWSE-TransBeskr OPEN QUERY BROWSE-TransBeskr FOR EACH TransBeskr ~
      WHERE TransBeskr.TTId = wTTId NO-LOCK INDEXED-REPOSITION.
...
FILL-IN-SOK-CHAR BUTTON-Sok BROWSE-TransBeskr Btn_OK Btn_Cancel Btn_Help 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-TransBeskr Dialog-Frame _STRUCTURED
  QUERY BROWSE-TransBeskr NO-LOCK DISPLAY
      TransBeskr.TBId COLUMN-LABEL "TBId" WIDTH 7.2
      TransBeskr.Beskrivelse WIDTH 36
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME Dialog-Frame
     ...
     BROWSE-TransBeskr AT ROW 2.91 COL 2
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
                                                                        */
/* BROWSE-TAB BROWSE-TransBeskr BUTTON-Sok Dialog-Frame */
ASSIGN 
...
/* Query rebuild information for BROWSE BROWSE-TransBeskr
     ...
     _Where[1]         = "TransBeskr.TTId = wTTId"
     _FldNameList[1]   > skotex.TransBeskr.TBId
"TBId" "TBId" ? "integer" ? ? ? ? ? ? no ? no no "7.2"
     _FldNameList[2]   > skotex.TransBeskr.Beskrivelse
"Beskrivelse" ? ? "character" ? ? ? ? ? ? no ? no no "36"
     _Query            is OPENED
...
&Scoped-define BROWSE-NAME BROWSE-TransBeskr
&Scoped-define SELF-NAME BROWSE-TransBeskr
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-TransBeskr Dialog-Frame
ON ANY-PRINTABLE OF BROWSE-TransBeskr IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-TransBeskr Dialog-Frame
ON CURSOR-LEFT OF BROWSE-TransBeskr IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-TransBeskr Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-TransBeskr IN FRAME Dialog-Frame
DO:
  IF {&ant-sok-field} = 1 THEN
    RETURN NO-APPLY.
  RUN SD-CURSOR ("RIGHT").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-TransBeskr Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-TransBeskr IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-TransBeskr Dialog-Frame
ON START-SEARCH OF BROWSE-TransBeskr IN FRAME Dialog-Frame
DO:
  RUN SD-START-SEARCH.
  RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-DATE FILL-IN-SOK-INT FILL-IN-SOK-CHAR BUTTON-Sok 
         BROWSE-TransBeskr Btn_OK Btn_Cancel Btn_Help 
      WITH FRAME Dialog-Frame.
