...
&ELSE
  DEFINE INPUT PARAMETER wPerId LIKE Periode.PerId NO-UNDO.
&ENDIF

/* Lokale variabler */
def var wPerLinNr like PerLin.PerLinNr no-undo.

/* Preprossessor direktiver ---                                         */
&scoped-define br-tabell   PerLin

/* Kolonne 1 som det kan sorteres på */
&scoped-define sok-field1  PerLinNr
...
&scoped-define index-col1  PeriodeLinje
&scoped-define sok-phrase1 FIND FIRST b{&br-tabell} WHERE ~
                b{&br-tabell}.PerId = wPerId and ~
                b{&br-tabell}.{&sok-field1} >= ~
                INPUT FILL-IN-SOK-{&sok-field1-def} USE-INDEX {&index-col1} NO-LOCK NO-ERROR.

&scoped-define ip-variabel wPerLinNr
&scoped-define return-ip   wPerLinNr = if available PerLin ~
                                          then PerLin.PerLinNr ~
                                          else 0.
&scoped-define assign-retur-verdi retur-verdi = if available PerLin ~
                                                  then string(recid(PerLin)) ~
                                                  else "".
...
&scoped-define VedlikeholdsRutine d-vperlin.w
...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME BROWSE-PerLin

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES PerLin

/* Definitions for BROWSE BROWSE-PerLin                                 */
&Scoped-define FIELDS-IN-QUERY-BROWSE-PerLin PerLin.PerLinNr PerLin.FraDato ~
PerLin.TilDato 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-PerLin 
&Scoped-define OPEN-QUERY-BROWSE-PerLin OPEN QUERY BROWSE-PerLin FOR EACH PerLin ~
      WHERE PerLin.PerId = wPerId NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-PerLin PerLin
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-PerLin PerLin


/* Definitions for DIALOG-BOX Dialog-Frame                              */
&Scoped-define OPEN-BROWSERS-IN-QUERY-Dialog-Frame ~
    ~{&OPEN-QUERY-BROWSE-PerLin}

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS FILL-IN-SOK-CHAR FILL-IN-SOK-INT ~
FILL-IN-SOK-DATE BUTTON-Sok BROWSE-PerLin BUTTON-Ny BUTTON-Endre ~
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY BROWSE-PerLin FOR 
      PerLin SCROLLING.
&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE BROWSE-PerLin
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-PerLin Dialog-Frame _STRUCTURED
  QUERY BROWSE-PerLin NO-LOCK DISPLAY
      PerLin.PerLinNr
      PerLin.FraDato WIDTH 14
      PerLin.TilDato WIDTH 12
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 41 BY 10.67.
...
DEFINE FRAME Dialog-Frame
     ...
     BROWSE-PerLin AT ROW 2.71 COL 2
     BUTTON-Ny AT ROW 2.91 COL 44
     BUTTON-Endre AT ROW 4.24 COL 44
     BUTTON-Slett AT ROW 5.57 COL 44
     Btn_OK AT ROW 13.62 COL 2
     Btn_Cancel AT ROW 13.62 COL 14.4
     Btn_Help AT ROW 13.62 COL 44
     SPACE(0.19) SKIP(0.09)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste periodelinjer"
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
                                                                        */
/* BROWSE-TAB BROWSE-PerLin BUTTON-Sok Dialog-Frame */
ASSIGN 
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE BROWSE-PerLin
/* Query rebuild information for BROWSE BROWSE-PerLin
     _TblList          = "SkoTex.PerLin"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _Where[1]         = "PerLin.PerId = wPerId"
     _FldNameList[1]   = SkoTex.PerLin.PerLinNr
     _FldNameList[2]   > SkoTex.PerLin.FraDato
"FraDato" ? ? "date" ? ? ? ? ? ? no ? no no "14"
     _FldNameList[3]   > SkoTex.PerLin.TilDato
"TilDato" ? ? "date" ? ? ? ? ? ? no ? no no "12"
     _Query            is OPENED
*/  /* BROWSE BROWSE-PerLin */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Søkeliste periodelinjer */
DO:
...
&Scoped-define BROWSE-NAME BROWSE-PerLin
&Scoped-define SELF-NAME BROWSE-PerLin
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-PerLin Dialog-Frame
ON ANY-PRINTABLE OF BROWSE-PerLin IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-PerLin Dialog-Frame
ON CURSOR-LEFT OF BROWSE-PerLin IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-PerLin Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-PerLin IN FRAME Dialog-Frame
DO:
  IF {&ant-sok-field} = 1 THEN
    RETURN NO-APPLY.
  RUN SD-CURSOR ("RIGHT").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-PerLin Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-PerLin IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-PerLin Dialog-Frame
ON START-SEARCH OF BROWSE-PerLin IN FRAME Dialog-Frame
DO:
  RUN SD-START-SEARCH.
  RETURN NO-APPLY.
END.
...
DO:
  ...
  run {&VedlikeholdsRutine} (input-output wRecid,"Endre", wPerId).
...
DO:
  assign 
    wRecid = ?.
  run {&VedlikeholdsRutine} (input-output wRecid,"Ny", wPerId).
  if return-value = "AVBRYT" then
    return no-apply.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ON END-KEY UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK:
    ...
    assign
      frame Dialog-Frame:title = frame Dialog-Frame:title + " [" + wPerId + "]".
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-CHAR FILL-IN-SOK-INT FILL-IN-SOK-DATE BUTTON-Sok 
         BROWSE-PerLin BUTTON-Ny BUTTON-Endre BUTTON-Slett Btn_OK Btn_Cancel 
         Btn_Help 
      WITH FRAME Dialog-Frame.
