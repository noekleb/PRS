...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEFINE VAR wTypeId LIKE HT-Type.TypeId INIT 31 NO-UNDO.
&ELSE
  DEFINE INPUT-OUTPUT PARAMETER wTypeId LIKE HT-Type.TypeId INIT 31 NO-UNDO.
&ENDIF
...
&scoped-define sok-field1  TypeId
...
&scoped-define index-col1  HT-Type
...
&scoped-define sok-field2  Betegnelse
...
&scoped-define index-col2  Betegnelse
...
&scoped-define ip-variabel wTypeId
&scoped-define return-ip   wTypeId = if available {&br-tabell}  ~
                                       then {&br-tabell}.{&sok-field1} ~
                                       else  {&sok-field1-def}("").
&scoped-define assign-retur-verdi retur-verdi = if available {&br-tabell} ~
                                                  then string(recid({&br-tabell})) ~
                                                  else "".
&scoped-define SletteMelding "Skal hånsterminaltypen tas bort? "
&scoped-define KanSlettes ~
  if can-find(first HT-FilHode where ~
    HT-FilHode.TypeId = HT-Type.TypeId) then ~
    do: ~
      message "Denne håndterminaltypen er i bruk. Kan ikke tas bort." ~
...
DEFINE VARIABLE lUppslag AS LOGICAL     NO-UNDO.
...
&Scoped-define FRAME-NAME Main
&Scoped-define BROWSE-NAME BROWSE-HT-Type

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES HT-Type

/* Definitions for BROWSE BROWSE-HT-Type                                */
&Scoped-define FIELDS-IN-QUERY-BROWSE-HT-Type HT-Type.TypeId ~
HT-Type.Betegnelse HT-Type.HTAktiv HT-Type.ImportKatalog 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-HT-Type HT-Type.TypeId ~
HT-Type.Betegnelse 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-HT-Type HT-Type
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-HT-Type HT-Type
&Scoped-define QUERY-STRING-BROWSE-HT-Type FOR EACH HT-Type NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-HT-Type OPEN QUERY BROWSE-HT-Type FOR EACH HT-Type NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-HT-Type HT-Type
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-HT-Type HT-Type


/* Definitions for DIALOG-BOX Main                                      */

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS FILL-IN-SOK-DATE FILL-IN-SOK-INT ~
FILL-IN-SOK-CHAR BUTTON-Sok BROWSE-HT-Type BUTTON-Ny BUTTON-Endre ~
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY BROWSE-HT-Type FOR 
      HT-Type SCROLLING.
&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE BROWSE-HT-Type
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-HT-Type Main _STRUCTURED
  QUERY BROWSE-HT-Type NO-LOCK DISPLAY
      HT-Type.TypeId COLUMN-LABEL "Type *" FORMAT "zz9":U
      HT-Type.Betegnelse COLUMN-LABEL "Betegnelse *" FORMAT "X(40)":U
            WIDTH 40.8
      HT-Type.HTAktiv FORMAT "yes/no":U
      HT-Type.ImportKatalog FORMAT "X(40)":U WIDTH 40.8
  ENABLE
      HT-Type.TypeId
      HT-Type.Betegnelse
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 95 BY 14.29.
...
DEFINE FRAME Main
     ...
     BROWSE-HT-Type AT ROW 2.91 COL 2
     BUTTON-Ny AT ROW 2.91 COL 99
     BUTTON-Endre AT ROW 4.24 COL 99
     BUTTON-Slett AT ROW 5.57 COL 99
     Btn_Help AT ROW 17.43 COL 98.8
     Btn_OK AT ROW 17.52 COL 2
     Btn_Cancel AT ROW 17.52 COL 14.4
     SPACE(88.39) SKIP(0.18)
...
/* SETTINGS FOR DIALOG-BOX Main
   FRAME-NAME                                                           */
/* BROWSE-TAB BROWSE-HT-Type BUTTON-Sok Main */
ASSIGN 
       FRAME Main:SCROLLABLE       = FALSE
       FRAME Main:HIDDEN           = TRUE.

/* SETTINGS FOR FILL-IN FILL-IN-SOK-CHAR IN FRAME Main
   ALIGN-L                                                              */
/* SETTINGS FOR FILL-IN FILL-IN-SOK-DATE IN FRAME Main
   ALIGN-L                                                              */
/* SETTINGS FOR FILL-IN FILL-IN-SOK-INT IN FRAME Main
   ALIGN-L                                                              */
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE BROWSE-HT-Type
/* Query rebuild information for BROWSE BROWSE-HT-Type
     _TblList          = "skotex.HT-Type"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _FldNameList[1]   > skotex.HT-Type.TypeId
"HT-Type.TypeId" "Type *" ? "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > skotex.HT-Type.Betegnelse
"HT-Type.Betegnelse" "Betegnelse *" "X(40)" "character" ? ? ? ? ? ? yes ? no no "40.8" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   = skotex.HT-Type.HTAktiv
     _FldNameList[4]   > skotex.HT-Type.ImportKatalog
"HT-Type.ImportKatalog" ? "X(40)" "character" ? ? ? ? ? ? no ? no no "40.8" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is NOT OPENED
*/  /* BROWSE BROWSE-HT-Type */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Main /* Vedlikehold håndterminaltyper */
DO:
...
&Scoped-define BROWSE-NAME BROWSE-HT-Type
&Scoped-define SELF-NAME BROWSE-HT-Type
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-HT-Type Main
ON ANY-PRINTABLE OF BROWSE-HT-Type IN FRAME Main
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-HT-Type Main
ON CURSOR-LEFT OF BROWSE-HT-Type IN FRAME Main
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-HT-Type Main
ON CURSOR-RIGHT OF BROWSE-HT-Type IN FRAME Main
DO:
  IF {&ant-sok-field} = 1 THEN
    RETURN NO-APPLY.
  RUN SD-CURSOR ("RIGHT").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-HT-Type Main
ON DEFAULT-ACTION OF BROWSE-HT-Type IN FRAME Main
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-HT-Type Main
ON START-SEARCH OF BROWSE-HT-Type IN FRAME Main
DO:
  RUN SD-START-SEARCH.
  RETURN NO-APPLY.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Btn_Help Main
ON CHOOSE OF Btn_Help IN FRAME Main /* Help */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Btn_OK Main
ON CHOOSE OF Btn_OK IN FRAME Main /* OK */
DO:
  {&return-ip}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-Endre Main
ON CHOOSE OF BUTTON-Endre IN FRAME Main /* Endre... */
DO:
  assign
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-Ny Main
ON CHOOSE OF BUTTON-Ny IN FRAME Main /* Ny... */
DO:
  assign 
    wRecid = ?.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-Slett Main
ON CHOOSE OF BUTTON-Slett IN FRAME Main /* Slett */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-Sok Main
ON CHOOSE OF BUTTON-Sok IN FRAME Main /* Søk */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-SOK-CHAR Main
ON RETURN OF FILL-IN-SOK-CHAR IN FRAME Main
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-SOK-DATE Main
ON LEAVE OF FILL-IN-SOK-DATE IN FRAME Main
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-SOK-DATE Main
ON RETURN OF FILL-IN-SOK-DATE IN FRAME Main
DO:
    DEFINE VAR wDate AS DATE.
    wDate = DATE(FILL-IN-SOK-DATE:SCREEN-VALUE) NO-ERROR.
    IF ERROR-STATUS:ERROR THEN DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-SOK-INT Main
ON RETURN OF FILL-IN-SOK-INT IN FRAME Main
DO:
  APPLY "CHOOSE" TO BUTTON-Sok.
  RETURN NO-APPLY.
END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ON END-KEY UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK:
    ...
    IF VALID-HANDLE (hSourceProc) AND CAN-DO(hSourceProc:INTERNAL-ENTRIES,"HTuppslag") THEN
       RUN HTuppslag IN hSourceProc (OUTPUT lUppslag) NO-ERROR.
    RUN LabelColor.
    RUN Move-Fill-To-Top.
/*    MESSAGE "lUpp-2" lUppslag
        VIEW-AS ALERT-BOX INFO BUTTONS OK.*/
    {lng.i} RUN enable_UI.
    IF lUppslag = TRUE THEN
    DO:
      BUTTON-Ny:SENSITIVE = FALSE.
      BUTTON-Slett:SENSITIVE = FALSE.
      hQry = BROWSE-HT-Type:QUERY.
      hQry:QUERY-PREPARE("FOR EACH HT-Type NO-LOCK WHERE HT-Type.HTAktiv = TRUE").
      hQry:QUERY-OPEN().
    END.
    ELSE
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE BytSortering Main 
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-DATE FILL-IN-SOK-INT FILL-IN-SOK-CHAR BUTTON-Sok 
         BROWSE-HT-Type BUTTON-Ny BUTTON-Endre BUTTON-Slett Btn_Help Btn_OK 
         Btn_Cancel 
      WITH FRAME Main.
  VIEW FRAME Main.
  {&OPEN-BROWSERS-IN-QUERY-Main}
END PROCEDURE.

/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE HTuppslag Main 
PROCEDURE HTuppslag :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Init-Read-Only Main 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE LabelColor Main 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Move-Fill-To-Top Main 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE SD-ANY-PRINTABLE Main 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE SD-CURSOR Main 
PROCEDURE SD-CURSOR :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE SD-Reposition Main 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE SD-START-SEARCH Main 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE SortNyCol Main 
PROCEDURE SortNyCol :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
