...
&scoped-define KanSlettes 
 /*  ~
  if can-find(first ButikkTeam where ButikkTeam.TeamTypeId = {&br-tabell}.TeamTypeId) then ~
    do: ~
      message "Det finnes butikkteam med denne id. Kan ikke slettes!" ~
      view-as alert-box title "Fel vid borttagning". ~
      return no-apply. ~
    end.
*/
&scoped-define VedlikeholdsRutine d-vteamtype.w
...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME BROWSE-TeamType

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES TeamType

/* Definitions for BROWSE BROWSE-TeamType                               */
&Scoped-define FIELDS-IN-QUERY-BROWSE-TeamType TeamType.TeamTypeId ~
TeamType.Beskrivelse TeamType.Notat 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-TeamType TeamType.TeamTypeId ~
TeamType.Beskrivelse 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-TeamType TeamType
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-TeamType TeamType
&Scoped-define OPEN-QUERY-BROWSE-TeamType OPEN QUERY BROWSE-TeamType FOR EACH TeamType NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-TeamType TeamType
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-TeamType TeamType
...
FILL-IN-SOK-DATE BUTTON-Sok BROWSE-TeamType Btn_OK Btn_Help 
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY BROWSE-TeamType FOR 
      TeamType SCROLLING.
&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE BROWSE-TeamType
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-TeamType Dialog-Frame _STRUCTURED
  QUERY BROWSE-TeamType NO-LOCK DISPLAY
      TeamType.TeamTypeId FORMAT "z9":U
      TeamType.Beskrivelse FORMAT "X(40)":U
      TeamType.Notat FORMAT "X(40)":U
  ENABLE
      TeamType.TeamTypeId
      TeamType.Beskrivelse
/* _UIB-CODE-BLOCK-END */
...
DEFINE FRAME Dialog-Frame
     ...
     BROWSE-TeamType AT ROW 2.91 COL 2
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   Custom                                                               */
/* BROWSE-TAB BROWSE-TeamType BUTTON-Sok Dialog-Frame */
ASSIGN 
       FRAME Dialog-Frame:SCROLLABLE       = FALSE
       FRAME Dialog-Frame:HIDDEN           = TRUE.

ASSIGN 
       BROWSE-TeamType:MAX-DATA-GUESS IN FRAME Dialog-Frame         = 481.
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE BROWSE-TeamType
/* Query rebuild information for BROWSE BROWSE-TeamType
     _TblList          = "SkoTex.TeamType"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _FldNameList[1]   > SkoTex.TeamType.TeamTypeId
"TeamType.TeamTypeId" ? ? "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > SkoTex.TeamType.Beskrivelse
"TeamType.Beskrivelse" ? ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[3]   = SkoTex.TeamType.Notat
     _Query            is NOT OPENED
*/  /* BROWSE BROWSE-TeamType */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Teamtyper */
DO:
...
&Scoped-define BROWSE-NAME BROWSE-TeamType
&Scoped-define SELF-NAME BROWSE-TeamType
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-TeamType Dialog-Frame
ON ANY-PRINTABLE OF BROWSE-TeamType IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-TeamType Dialog-Frame
ON CURSOR-LEFT OF BROWSE-TeamType IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-TeamType Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-TeamType IN FRAME Dialog-Frame
DO:
  IF wAntSortCol > 1 THEN
      RUN SD-CURSOR ("RIGHT").
  RETURN NO-APPLY.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-TeamType Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-TeamType IN FRAME Dialog-Frame
DO:
  IF wType = "V" THEN
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-TeamType Dialog-Frame
ON HOME OF BROWSE-TeamType IN FRAME Dialog-Frame
DO:
  APPLY "ENTRY" TO SELF.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-TeamType Dialog-Frame
ON START-SEARCH OF BROWSE-TeamType IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-TeamType Dialog-Frame
ON VALUE-CHANGED OF BROWSE-TeamType IN FRAME Dialog-Frame
DO:
  ASSIGN BUTTON-Slett:SENSITIVE IN FRAME {&FRAME-NAME} =
       TeamType.TeamTypeId > 2.
END.
  ...
  if wOk = true then
    do transaction:
      ...
      if available b{&br-tabell} then
        do:
          FIND _User WHERE _User._Userid = b{&br-tabell}.Brukerid NO-ERROR.
          IF AVAIL _User THEN
              DELETE _User.
          delete b{&br-tabell}.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ON END-KEY UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK:
    ...
    APPLY "VALUE-CHANGED" TO BROWSE BROWSE-TeamType. 
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-CHAR FILL-IN-SOK-INTE FILL-IN-SOK-DATE BUTTON-Sok 
         BROWSE-TeamType Btn_OK Btn_Help 
      WITH FRAME Dialog-Frame.
