...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  IF wUtskrMaate = "VS90" THEN DO:
     FIND Skriver WHERE Skriver.SkrNr = wSkrNr NO-LOCK NO-ERROR.
     IF NOT AVAIL Skriver THEN DO:
        MESSAGE "Finner ikke skriver nr. " wSkrNr SKIP(1)
                "Kontakt systemansvarlig."
            VIEW-AS ALERT-BOX ERROR TITLE "Feil".
        LEAVE MAIN-BLOCK.
     END.      
     ASSIGN wSkriverNavn = Skriver.SkrNavn.     
...
/*      OS-COPY value(wFilnavn) value(Skriver.SkrUtenhet). */
      ...
      DO WITH FRAME {&FRAME-NAME}:
         ASSIGN      
            ...
            FILL-IN-Progress:SCREEN-VALUE    = IF OS-ERROR = 0 THEN "Utskriften er sendt til " + Skriver.SkrUtenhet ELSE OsErrorTekst(OS-ERROR)
