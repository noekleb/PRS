...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEFINE VAR wProvKod LIKE Prov.ProvKod NO-UNDO.
&ELSE
  DEFINE INPUT-OUTPUT PARAMETER wProvKod LIKE Prov.ProvKod NO-UNDO.
&ENDIF

/* Preprossessor direktiver ---                                         */
&scoped-define br-tabell   Prov

/* Kolonne 1 som det kan sorteres på */
&scoped-define sok-field1  ProvKod
...
&scoped-define index-col1  ProvIn
...
&scoped-define sok-field2  ProvBeskr
...
&scoped-define index-col2  ProvBeskr
...
&scoped-define ip-variabel wProvKod
&scoped-define return-ip   wProvKod = Prov.ProvKod.
&scoped-define assign-retur-verdi retur-verdi = string(recid(Prov)).
...
&scoped-define KanSlettes ~
  if can-find(first ArtBas where ArtBas.ProvKod = Prov.ProvKod) then ~
    do: ~
      message "Det finns artiklar med denna provisionskoden" ~
      view-as alert-box title "Fel vid borttagning". ~
      return no-apply. ~
    end.

&scoped-define VedlikeholdsRutine d-vprov.w
...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME BROWSE-Prov

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES Prov

/* Definitions for BROWSE BROWSE-Prov                                   */
&Scoped-define FIELDS-IN-QUERY-BROWSE-Prov Prov.ProvKod Prov.ProvBeskr ~
Prov.ProvProc 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-Prov Prov.ProvKod ~
Prov.ProvBeskr 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-Prov Prov
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-Prov Prov
&Scoped-define QUERY-STRING-BROWSE-Prov FOR EACH Prov NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-Prov OPEN QUERY BROWSE-Prov FOR EACH Prov NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-Prov Prov
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-Prov Prov


/* Definitions for DIALOG-BOX Dialog-Frame                              */
&Scoped-define OPEN-BROWSERS-IN-QUERY-Dialog-Frame ~
    ~{&OPEN-QUERY-BROWSE-Prov}

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS FILL-IN-SOK-CHAR FILL-IN-SOK-INT ~
FILL-IN-SOK-DATE BUTTON-Sok BROWSE-Prov BUTTON-Ny BUTTON-Endre BUTTON-Slett ~
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY BROWSE-Prov FOR 
      Prov SCROLLING.
&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE BROWSE-Prov
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Prov Dialog-Frame _STRUCTURED
  QUERY BROWSE-Prov NO-LOCK DISPLAY
      Prov.ProvKod COLUMN-LABEL "PK *" FORMAT "z9":U
      Prov.ProvBeskr COLUMN-LABEL "Beskrivelse *" FORMAT "X(30)":U
      Prov.ProvProc FORMAT "z9.99":U
  ENABLE
      Prov.ProvKod
      Prov.ProvBeskr
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 49 BY 7.38.
...
DEFINE FRAME Dialog-Frame
     ...
     BROWSE-Prov AT ROW 2.91 COL 2
     BUTTON-Ny AT ROW 3 COL 51.8
     BUTTON-Endre AT ROW 4.33 COL 51.8
     BUTTON-Slett AT ROW 5.67 COL 51.8
     Btn_Help AT ROW 10.71 COL 51.8
     Btn_OK AT ROW 10.81 COL 2.2
     Btn_Cancel AT ROW 10.81 COL 14.6
     SPACE(40.39) SKIP(0.08)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste provisjonskoder"
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
                                                                        */
/* BROWSE-TAB BROWSE-Prov BUTTON-Sok Dialog-Frame */
ASSIGN 
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE BROWSE-Prov
/* Query rebuild information for BROWSE BROWSE-Prov
     _TblList          = "SkoTex.Prov"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _FldNameList[1]   > SkoTex.Prov.ProvKod
"Prov.ProvKod" "PK *" ? "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > SkoTex.Prov.ProvBeskr
"Prov.ProvBeskr" "Beskrivelse *" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[3]   = SkoTex.Prov.ProvProc
     _Query            is OPENED
*/  /* BROWSE BROWSE-Prov */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Søkeliste provisjonskoder */
DO:
...
&Scoped-define BROWSE-NAME BROWSE-Prov
&Scoped-define SELF-NAME BROWSE-Prov
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Prov Dialog-Frame
ON ANY-PRINTABLE OF BROWSE-Prov IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Prov Dialog-Frame
ON CURSOR-LEFT OF BROWSE-Prov IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Prov Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-Prov IN FRAME Dialog-Frame
DO:
  IF {&ant-sok-field} = 1 THEN
    RETURN NO-APPLY.
  RUN SD-CURSOR ("RIGHT").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Prov Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-Prov IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Prov Dialog-Frame
ON START-SEARCH OF BROWSE-Prov IN FRAME Dialog-Frame
DO:
  RUN SD-START-SEARCH.
  RETURN NO-APPLY.
END.
...
{syspara.i 16 17 2 iTilgang INT}
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-CHAR FILL-IN-SOK-INT FILL-IN-SOK-DATE BUTTON-Sok 
         BROWSE-Prov BUTTON-Ny BUTTON-Endre BUTTON-Slett Btn_Help Btn_OK 
         Btn_Cancel 
      WITH FRAME Dialog-Frame.
