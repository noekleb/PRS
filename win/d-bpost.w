...
&scoped-define return-ip   wPostNr = if available Post  ~
                                       then Post.PostNr ~
                                       else "".
&scoped-define assign-retur-verdi retur-verdi = if available Post ~
                                                  then string(recid(Post)) ~
                                                  else "".
&scoped-define SletteMelding "Postnummer er i bruk. Kan ikke slettes!"
&scoped-define KanSlettes 
&scoped-define ant-sok-field   2
&scoped-define init-datatype   CHAR /* CHAR, INT, DATE */
&scoped-define VedlikeholdsRutine d-vpost.w
...
Post.FylkesNr Post.KommNr Post.Merknad 
...
&Scoped-define QUERY-STRING-BROWSE-Post FOR EACH Post NO-LOCK ~
...
FILL-IN-SOK-CHAR BUTTON-Sok BROWSE-Post BUTTON-Ny BUTTON-Endre BUTTON-Slett ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Post Dialog-Frame _STRUCTURED
  QUERY BROWSE-Post NO-LOCK DISPLAY
      Post.PostNr COLUMN-LABEL "PostNr *" FORMAT "X(10)":U
      Post.Beskrivelse COLUMN-LABEL "Poststed *" FORMAT "X(30)":U
      Post.FylkesNr FORMAT "X(8)":U
      Post.KommNr FORMAT "X(8)":U
      Post.Merknad FORMAT "X(50)":U
  ENABLE
      Post.PostNr
      Post.Beskrivelse
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 66.4 BY 10.57.
...
DEFINE FRAME Dialog-Frame
     ...
     BUTTON-Ny AT ROW 2.91 COL 69.2
     BUTTON-Endre AT ROW 4.24 COL 69.2
     BUTTON-Slett AT ROW 5.57 COL 69.2
     Btn_Help AT ROW 13.57 COL 69.2
     Btn_OK AT ROW 13.67 COL 2.2
     Btn_Cancel AT ROW 13.67 COL 14.6
     SPACE(57.60) SKIP(0.00)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste postregister"
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
                                                                        */
/* BROWSE-TAB BROWSE-Post BUTTON-Sok Dialog-Frame */
ASSIGN 
       FRAME Dialog-Frame:SCROLLABLE       = FALSE
       FRAME Dialog-Frame:HIDDEN           = TRUE.

ASSIGN 
       BROWSE-Post:NUM-LOCKED-COLUMNS IN FRAME Dialog-Frame     = 1.
...
"Post.PostNr" "PostNr *" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > SkoTex.Post.Beskrivelse
"Post.Beskrivelse" "Poststed *" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[3]   = SkoTex.Post.FylkesNr
     _FldNameList[4]   = SkoTex.Post.KommNr
     _FldNameList[5]   = SkoTex.Post.Merknad
     _Query            is OPENED
*/  /* BROWSE BROWSE-Post */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Søkeliste postregister */
DO:
...
ON ANY-PRINTABLE OF BROWSE-Post IN FRAME Dialog-Frame
DO:
...
{syspara.i 16 27 2 iTilgang INT}
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-DATE FILL-IN-SOK-INT FILL-IN-SOK-CHAR BUTTON-Sok 
         BROWSE-Post BUTTON-Ny BUTTON-Endre BUTTON-Slett Btn_Help Btn_OK 
         Btn_Cancel 
      WITH FRAME Dialog-Frame.
