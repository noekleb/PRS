...
&Scoped-Define ENABLED-OBJECTS FI-BildNr1 FI-BildNr2 Btn_OK Btn_Cancel ~
Btn_Help RECT-44 RECT-45 
...
DEFINE VARIABLE FI-BildNr2 AS INTEGER FORMAT "zzzzzzz9":U INITIAL 99999999 
     VIEW-AS FILL-IN 
...
DEFINE FRAME Dialog-Frame
     ...
     "Renser bilderegisteret for bilder som ikke er i bruk." VIEW-AS TEXT
          SIZE 58 BY .62 AT ROW 14.38 COL 5
     SPACE(1.79) SKIP(0.75)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Rens bilderegister"
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Rens bilderegister */
DO:
...
OCXFile = SEARCH( "d-rensbildereg.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "d-rensbildereg.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FI-BildNr1 FI-BildNr2 Btn_OK Btn_Cancel Btn_Help RECT-44 RECT-45 
      WITH FRAME Dialog-Frame.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE RensBildeReg Dialog-Frame 
PROCEDURE RensBildeReg :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  message "Skal rensing starte?" skip(1)
          "Alle bilderegister poster innenfor utvalget som ikke er " skip
          "knyttet til en artikkel, slettes. Bildefilen som er knyttet" skip
          "til posten, slettes også."
         view-as alert-box BUTTONS YES-NO title "Start av konvertering"
         update wOk.
  if wOk <> true then
    return no-apply.
  
  /* Leser aktuelle bildeposter. */
  BILDEREGISTER:
  for each BildeRegister exclusive-lock where
    ...
    if wStop then
      do:
        message "Skal renserutinen stoppes? " view-as alert-box buttons YES-NO 
                 set wStop.
        if wStop = true then
          leave BILDEREGISTER.
      end.

    /* Vi tar kun poster som ikke er knyttet til en artikkel under behandling. */
    if can-find(first ArtBas where ArtBas.BildNr = BildeRegister.BildNr) then 
      next BILDEREGISTER.
    ...
    if valid-handle(wLibHandle) then
     run HentBildePeker in wLibHandle (input BildeRegister.BildNr, input 3, wBildePeker, output wBildePeker).
