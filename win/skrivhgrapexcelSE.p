...
/* ***************************  Main Block  *************************** */
RUN SkapaTT.
IF CAN-FIND(FIRST TT_dags_rap) THEN
    RUN ExportToExcel.
ELSE
    MESSAGE "Ingen data"
  ...
  FOR EACH TT_dags_rap BREAK BY butikk:
          IF FIRST-OF(TT_dags_rap.butikk) THEN DO:
              ...
              EXPORT STREAM Eksport  DELIMITER ";" ENTRY(1,wKollonneLbl)
                                                   ...
                                                   "%"
                                                   ENTRY(10,wKollonneLbl)
                                                   "%"
                                                   ENTRY(11,wKollonneLbl).
          END.
          ASSIGN iButikRad = iButikRad + 1.
          EXPORT STREAM Eksport DELIMITER ";" 
              IF cAkkumulering = "1" THEN 
                   ...
                   ROUND(hg7_oms,0)
                   ROUND(hg7_oms * 100 / RadTotal,0)
                   ROUND(hg8_oms,0)
                   ROUND(hg8_oms * 100 / RadTotal,0)
                   ROUND(hg9_oms,0)
                   ROUND(hg9_oms * 100 / RadTotal,0)
                   ROUND(RadTotal,0).
  END.
  {swn.i}
...
*/  
  ...
  chWorkSheets:Range("B:T"):NumberFormat = "# ##0".
  /*
   ...
   */
  STATUS DEFAULT "Setter overskrift...".
  chWorkSheets:Range("A1:T1"):Merge().
  chWorkSheets:Range("A2:T2"):Merge().
  chWorkSheets:Range("A3:T3"):Merge().
  DO iCount1 = 1 TO NUM-ENTRIES(cButikRader):
      chWorkSheets:Range("A" + ENTRY(iCount1,cButikRader) + ":T" + ENTRY(iCount1,cButikRader)):Merge().
      chWorkSheets:Range("A" + ENTRY(iCount1,cButikRader) + ":D" + ENTRY(iCount1,cButikRader)):Font:Bold = TRUE.
      chWorkSheets:Range("A" + STRING(INT(ENTRY(iCount1,cButikRader)) + 1) + ":T" + STRING(INT(ENTRY(iCount1,cButikRader)) + 1)):Font:Bold = TRUE.
  END.
  /*
...
*/  
  ...
  chWorkSheets:Columns("A:T"):AutoFit().
/*
  ...
  chWorksheets:PageSetup:PrintArea      = "A:T".
/*
  ...
  */
   DO iCount1 = 1 TO NUM-ENTRIES(cButikRader) - 1:
      ...
      DO iCount2 = 1 TO NUM-ENTRIES("B,D,F,H,J,L,N,P,R,T"):
          ASSIGN cPar1 = ENTRY(iCount2,"B,D,F,H,J,L,N,P,R,T")
   ...
   DO iCount2 = 1 TO NUM-ENTRIES("B,D,F,H,J,L,N,P,R,T"):
          ...
          chExcelApplication:ActiveCell:FormulaR1C1 = SUBSTITUTE("=SUM(R[-&1]C:R[-1]C)",STRING(INT(cPar3) - INT(cPar2))).
   END.
   chWorkSheets:Range("A1:T1"):Select.
   chWorkSheets:PageSetup:ORIENTATION = 2.
