...
DEFINE TEMP-TABLE tMedlemsKort NO-UNDO LIKE MedlemsKort
...
&IF LENGTH("{&ip-variabel}") > 1 &THEN
  ...
  &IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
    ...
    DEFINE VAR lKundeNr       LIKE Kunde.KundeNr           NO-UNDO.
  &ELSE
    DEFINE INPUT PARAMETER {&ip-variabel} LIKE {&br-tabell}.{&ip-felt} NO-UNDO.
    DEFINE INPUT PARAMETER lKundeNr       LIKE Kunde.KundeNr           NO-UNDO.
  &ENDIF
...
DEFINE VAR cNavn             AS CHAR        NO-UNDO.
...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME BROWSE-tMedlemsKort

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES tMedlemsKort

/* Definitions for BROWSE BROWSE-tMedlemsKort                           */
&Scoped-define FIELDS-IN-QUERY-BROWSE-tMedlemsKort tMedlemsKort.MedlemsNr ~
cNavn tMedlemsKort.KortNr tMedlemsKort.AktivertDato tMedlemsKort.UtgarDato ~
tMedlemsKort.Sperret tMedlemsKort.Innehaver tMedlemsKort.Merknad 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-tMedlemsKort ~
tMedlemsKort.MedlemsNr tMedlemsKort.KortNr 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-tMedlemsKort tMedlemsKort
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-tMedlemsKort tMedlemsKort
&Scoped-define OPEN-QUERY-BROWSE-tMedlemsKort OPEN QUERY BROWSE-tMedlemsKort FOR EACH tMedlemsKort NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-tMedlemsKort tMedlemsKort
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-tMedlemsKort tMedlemsKort


/* Definitions for DIALOG-BOX Dialog-Frame                              */
&Scoped-define OPEN-BROWSERS-IN-QUERY-Dialog-Frame ~
    ~{&OPEN-QUERY-BROWSE-tMedlemsKort}

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS FILL-IN-SOK-CHAR FILL-IN-SOK-INTE ~
FILL-IN-SOK-DATE BUTTON-Sok BROWSE-tMedlemsKort Btn_OK Btn_Help 
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY BROWSE-tMedlemsKort FOR 
      tMedlemsKort SCROLLING.
&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE BROWSE-tMedlemsKort
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-tMedlemsKort Dialog-Frame _STRUCTURED
  QUERY BROWSE-tMedlemsKort NO-LOCK DISPLAY
      tMedlemsKort.MedlemsNr FORMAT ">>>>>>>>>>>>9":U
      cNavn COLUMN-LABEL "Navn" WIDTH 30
      tMedlemsKort.KortNr FORMAT "X(22)":U
      tMedlemsKort.AktivertDato FORMAT "99/99/99":U
      tMedlemsKort.UtgarDato FORMAT "99/99/99":U
      tMedlemsKort.Sperret FORMAT "*/":U
      tMedlemsKort.Innehaver FORMAT "X(30)":U
      tMedlemsKort.Merknad FORMAT "X(40)":U
  ENABLE
      tMedlemsKort.MedlemsNr
      tMedlemsKort.KortNr
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 129 BY 14.76 ROW-HEIGHT-CHARS .62 EXPANDABLE.
...
DEFINE FRAME Dialog-Frame
     ...
     BROWSE-tMedlemsKort AT ROW 2.91 COL 2
     Btn_OK AT ROW 17.91 COL 2
     Btn_Cancel AT ROW 17.91 COL 15.2
     Btn_Help AT ROW 17.91 COL 119
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: tMedlemsKort T "?" NO-UNDO SkoTex MedlemsKort
      ADDITIONAL-FIELDS:
          field tRecid as recid
      END-FIELDS.
   END-TABLES.
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   Custom                                                               */
/* BROWSE-TAB BROWSE-tMedlemsKort BUTTON-Sok Dialog-Frame */
ASSIGN 
       FRAME Dialog-Frame:SCROLLABLE       = FALSE
       FRAME Dialog-Frame:HIDDEN           = TRUE.

ASSIGN 
       BROWSE-tMedlemsKort:NUM-LOCKED-COLUMNS IN FRAME Dialog-Frame     = 4
       BROWSE-tMedlemsKort:MAX-DATA-GUESS IN FRAME Dialog-Frame         = 481.
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE BROWSE-tMedlemsKort
/* Query rebuild information for BROWSE BROWSE-tMedlemsKort
     _TblList          = "Temp-Tables.tMedlemsKort"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _FldNameList[1]   > Temp-Tables.tMedlemsKort.MedlemsNr
"tMedlemsKort.MedlemsNr" ? ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > "_<CALC>"
"cNavn" "Navn" ? ? ? ? ? ? ? ? no ? no no "30" yes no no "U" "" ""
     _FldNameList[3]   > Temp-Tables.tMedlemsKort.KortNr
"tMedlemsKort.KortNr" ? ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[4]   = Temp-Tables.tMedlemsKort.AktivertDato
     _FldNameList[5]   = Temp-Tables.tMedlemsKort.UtgarDato
     _FldNameList[6]   > Temp-Tables.tMedlemsKort.Sperret
"tMedlemsKort.Sperret" ? "*~~/" "logical" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[7]   = Temp-Tables.tMedlemsKort.Innehaver
     _FldNameList[8]   = Temp-Tables.tMedlemsKort.Merknad
     _Query            is OPENED
*/  /* BROWSE BROWSE-tMedlemsKort */
&ANALYZE-RESUME
...
&Scoped-define BROWSE-NAME BROWSE-tMedlemsKort
&Scoped-define SELF-NAME BROWSE-tMedlemsKort
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-tMedlemsKort Dialog-Frame
ON ANY-PRINTABLE OF BROWSE-tMedlemsKort IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-tMedlemsKort Dialog-Frame
ON CURSOR-LEFT OF BROWSE-tMedlemsKort IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-tMedlemsKort Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-tMedlemsKort IN FRAME Dialog-Frame
DO:
  IF wAntSortCol > 1 THEN
      RUN SD-CURSOR ("RIGHT").
  RETURN NO-APPLY.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-tMedlemsKort Dialog-Frame
ON HOME OF BROWSE-tMedlemsKort IN FRAME Dialog-Frame
DO:
  APPLY "ENTRY" TO SELF.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-tMedlemsKort Dialog-Frame
ON ROW-DISPLAY OF BROWSE-tMedlemsKort IN FRAME Dialog-Frame
DO:
  IF AVAILABLE tMedlemsKort THEN
  DO:
      FIND Medlem OF tMedlemsKort NO-ERROR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-tMedlemsKort Dialog-Frame
ON START-SEARCH OF BROWSE-tMedlemsKort IN FRAME Dialog-Frame
DO:
...
FOR EACH Medlem NO-LOCK WHERE
  ...
  FOR EACH MedlemsKort OF Medlem:
    create tMedlemsKort.
    BUFFER-COPY MedlemsKort TO tMedlemsKort
          ASSIGN
             tMedlemsKort.tRecid = RECID(MedlemsKort).
  END.
END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-CHAR FILL-IN-SOK-INTE FILL-IN-SOK-DATE BUTTON-Sok 
         BROWSE-tMedlemsKort Btn_OK Btn_Help 
      WITH FRAME Dialog-Frame.
