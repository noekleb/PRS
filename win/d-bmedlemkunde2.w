...
&scope br-tabell   tMedlem
&scope Sorttype    BY,BY,BY,BY
/* Sorttype entries = antal sortkolonner, kan vara tomma. */
&scope BrowseIdx   Medlem,ForNavn,EtterNavn,Kunde
...
&Scoped-define INTERNAL-TABLES tMedlem

/* Definitions for BROWSE BROWSE-Medlem                                 */
&Scoped-define FIELDS-IN-QUERY-BROWSE-Medlem tMedlem.MedlemsNr ~
tMedlem.ForNavn tMedlem.EtterNavn tMedlem.KundeNr cNavn 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-Medlem tMedlem.MedlemsNr ~
tMedlem.ForNavn tMedlem.EtterNavn tMedlem.KundeNr 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-Medlem tMedlem
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-Medlem tMedlem
&Scoped-define OPEN-QUERY-BROWSE-Medlem OPEN QUERY BROWSE-Medlem FOR EACH tMedlem NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-Medlem tMedlem
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-Medlem tMedlem
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Medlem Dialog-Frame _STRUCTURED
  QUERY BROWSE-Medlem NO-LOCK DISPLAY
      ...
      tMedlem.ForNavn FORMAT "X(40)":U WIDTH 26.8
      tMedlem.EtterNavn FORMAT "X(40)":U WIDTH 33.2
      tMedlem.KundeNr FORMAT ">>>>>>>>>>>>9":U WIDTH 20.2
      cNavn COLUMN-LABEL "Navn" WIDTH 25.6
  ENABLE
      tMedlem.MedlemsNr
      tMedlem.ForNavn
      tMedlem.EtterNavn
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 129 BY 18.81 ROW-HEIGHT-CHARS .63 EXPANDABLE.
...
DEFINE FRAME Dialog-Frame
     ...
     Btn_Cancel AT ROW 21.95 COL 15
     Btn_Help AT ROW 21.95 COL 119
...
"tMedlem.MedlemsNr" ? ? "decimal" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > Temp-Tables.tMedlem.ForNavn
"tMedlem.ForNavn" ? ? "character" ? ? ? ? ? ? yes ? no no "26.8" yes no no "U" "" ""
     _FldNameList[3]   > Temp-Tables.tMedlem.EtterNavn
"tMedlem.EtterNavn" ? ? "character" ? ? ? ? ? ? yes ? no no "33.2" yes no no "U" "" ""
     _FldNameList[4]   > Temp-Tables.tMedlem.KundeNr
"tMedlem.KundeNr" ? ? "decimal" ? ? ? ? ? ? yes ? no no "20.2" yes no no "U" "" ""
     _FldNameList[5]   > "_<CALC>"
"cNavn" "Navn" ? ? ? ? ? ? ? ? no ? no no "25.6" yes no no "U" "" ""
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Medlem Dialog-Frame
ON ROW-DISPLAY OF BROWSE-Medlem IN FRAME Dialog-Frame
DO:
  IF AVAILABLE tMedlem THEN
  DO:
      FIND Kunde OF tMedlem NO-LOCK NO-ERROR.
...
FOR EACH Medlem NO-LOCK WHERE
    ...
    CREATE tMedlem.
    BUFFER-COPY Medlem TO tMedlem
        ASSIGN
          tMedlem.tRecid = RECID(Medlem).
END.
