...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEFINE VAR wVMId LIKE VareMerke.VMId NO-UNDO.
&ELSE
  DEFINE INPUT-OUTPUT PARAMETER wVMId LIKE VareMerke.VMId NO-UNDO.
&ENDIF
...
&scoped-define ip-variabel wVMId
&scoped-define return-ip   wVMId = if available VareMerke  ~
                                       then VareMerke.VMId ~
                                       else 0.
&scoped-define assign-retur-verdi retur-verdi = if available VareMerke ~
                                                  then string(recid(VareMerke)) ~
                                                  else "".
&scoped-define SletteMelding "Skal varemerke slettes?"
&scoped-define KanSlettes ~
  if can-find(first ArtBas where ArtBas.VMId = VareMerke.VMId) then ~
    do: ~
      message "Det finnes artikkler med dette varemerke!" skip ~
              "Posten kan ikke slettes." view-as alert-box title "Melding". ~
      return no-apply. ~
    end.
    
&scoped-define ant-sok-field   3
&scoped-define init-datatype   INT /* CHAR, INT, DATE */
&scoped-define VedlikeholdsRutine d-vvaremerke.w
...
Varemerke.KortNavn Varemerke.Beskrivelse 
...
&Scoped-define QUERY-STRING-BROWSE-VareMerke FOR EACH Varemerke NO-LOCK ~
...
FILL-IN-SOK-CHAR BUTTON-Sok B-KonvTbl BROWSE-VareMerke BUTTON-Ny ~
BUTTON-Endre BUTTON-Slett B-Utskrift Btn_OK Btn_Cancel Btn_Help 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-VareMerke Dialog-Frame _STRUCTURED
  QUERY BROWSE-VareMerke NO-LOCK DISPLAY
      Varemerke.VMId COLUMN-LABEL "VmId *" FORMAT "zzzz9":U
      Varemerke.KortNavn COLUMN-LABEL "Varemerke *" FORMAT "X(20)":U
      Varemerke.Beskrivelse COLUMN-LABEL "Beskrivelse *" FORMAT "X(30)":U
  ENABLE
      Varemerke.VMId
      Varemerke.KortNavn
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 62.2 BY 11.52.
...
DEFINE FRAME Dialog-Frame
     ...
     B-KonvTbl AT ROW 1.48 COL 42
     BROWSE-VareMerke AT ROW 2.91 COL 2
     BUTTON-Ny AT ROW 3 COL 64.8
     BUTTON-Endre AT ROW 4.33 COL 64.8
     BUTTON-Slett AT ROW 5.67 COL 64.8
     B-Utskrift AT ROW 8.38 COL 65
     Btn_OK AT ROW 14.62 COL 1.8
     Btn_Cancel AT ROW 14.62 COL 14.6
     Btn_Help AT ROW 14.62 COL 64.8
     SPACE(0.59) SKIP(0.04)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste varemerke"
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
                                                                        */
/* BROWSE-TAB BROWSE-VareMerke B-KonvTbl Dialog-Frame */
ASSIGN 
       FRAME Dialog-Frame:SCROLLABLE       = FALSE
       FRAME Dialog-Frame:HIDDEN           = TRUE.

ASSIGN 
       BROWSE-VareMerke:NUM-LOCKED-COLUMNS IN FRAME Dialog-Frame     = 1.
...
"VMId" "VmId *" "zzzz9" "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > SkoTex.Varemerke.KortNavn
"KortNavn" "Varemerke *" "X(20)" "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > SkoTex.Varemerke.Beskrivelse
"Beskrivelse" "Beskrivelse *" ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Søkeliste varemerke */
DO:
...
DO:
  if not available Varemerke then
    return.
  run d-bimpkonv.w (1507,
                    string(Varemerke.VmId),
                    "Kobling av varemerke"
                    ).
  return no-apply.
END.
...
ON ANY-PRINTABLE OF BROWSE-VareMerke IN FRAME Dialog-Frame
DO:
...
{syspara.i 16 12 2 iTilgang INT}
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-INT FILL-IN-SOK-DATE FILL-IN-SOK-CHAR BUTTON-Sok B-KonvTbl 
         BROWSE-VareMerke BUTTON-Ny BUTTON-Endre BUTTON-Slett B-Utskrift Btn_OK 
         Btn_Cancel Btn_Help 
      WITH FRAME Dialog-Frame.
...
------------------------------------------------------------------------------*/
  ...
  if valid-handle(wLibHandle) then
    run GetTempFileName in wLibHandle ("Varemerke", cExcEkstent, output cFileName). 
  ...
  FOR EACH Varemerke no-lock:
    ...
    EXPORT STREAM sExportFile DELIMITER ";"
      /* A */ Varemerke.VmId     
      /* B */ Varemerke.KortNavn
      /* C */ Varemerke.Beskrivelse
      .
  ...
  chWorkSheets:Range("A1:C3"):Font:Bold = TRUE.
  chWorkSheets:Range("A1:C3"):Font:Italic = TRUE.
