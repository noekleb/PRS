...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEFINE VAR wListeNr LIKE Lister.ListeNr NO-UNDO.
  def    var wListeType as char no-undo.
&ELSE
  DEFINE INPUT-OUTPUT PARAMETER wListeNr LIKE Lister.ListeNr NO-UNDO.
  DEFINE INPUT        PARAMETER wListeType as char no-undo.
&ENDIF

/* Preprossessor direktiver ---                                         */
&scoped-define br-tabell   Lister
&scoped-define OptWhere ~
  Lister.ListeType = wListeType and

/* Kolonne 1 som det kan sorteres på */
&scoped-define sok-field1  ListeNr
...
&scoped-define index-col1  Lister
...
&scoped-define ip-variabel wListeNr
&scoped-define return-ip   wListeNr = if available {&br-tabell}  ~
...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME BROWSE-Lister

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES Lister

/* Definitions for BROWSE BROWSE-Lister                                 */
&Scoped-define FIELDS-IN-QUERY-BROWSE-Lister Lister.ListeNr Lister.Eier ~
Lister.Beskrivelse Lister.Merknad 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-Lister Lister.ListeNr ~
Lister.Eier 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-Lister Lister
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-Lister Lister
&Scoped-define QUERY-STRING-BROWSE-Lister FOR EACH Lister ~
      WHERE Lister.ListeType = wListeType and ~
Lister.ListeNr > 0 NO-LOCK ~
    BY Lister.ListeNr INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-Lister OPEN QUERY BROWSE-Lister FOR EACH Lister ~
      WHERE Lister.ListeType = wListeType and ~
Lister.ListeNr > 0 NO-LOCK ~
    BY Lister.ListeNr INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-Lister Lister
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-Lister Lister


/* Definitions for DIALOG-BOX Dialog-Frame                              */
&Scoped-define OPEN-BROWSERS-IN-QUERY-Dialog-Frame ~
    ~{&OPEN-QUERY-BROWSE-Lister}

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS FILL-IN-SOK-INT FILL-IN-SOK-CHAR ~
FILL-IN-SOK-DATE BUTTON-Sok BROWSE-Lister Btn_OK Btn_Cancel Btn_Help 
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY BROWSE-Lister FOR 
      Lister SCROLLING.
&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE BROWSE-Lister
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Lister Dialog-Frame _STRUCTURED
  QUERY BROWSE-Lister NO-LOCK DISPLAY
      Lister.ListeNr FORMAT "zzzzzzzz9":U
      Lister.Eier FORMAT "X(15)":U
      Lister.Beskrivelse COLUMN-LABEL "Beskrivelse" FORMAT "X(30)":U
      Lister.Merknad FORMAT "X(50)":U WIDTH 53
  ENABLE
      Lister.ListeNr
      Lister.Eier
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 115 BY 13.81.
...
DEFINE FRAME Dialog-Frame
     ...
     BROWSE-Lister AT ROW 2.91 COL 2
     Btn_OK AT ROW 16.95 COL 2
     Btn_Cancel AT ROW 16.95 COL 15
     Btn_Help AT ROW 16.95 COL 99
     SPACE(8.79) SKIP(0.14)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste Koleksjonslister"
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   FRAME-NAME                                                           */
/* BROWSE-TAB BROWSE-Lister BUTTON-Sok Dialog-Frame */
ASSIGN 
       FRAME Dialog-Frame:SCROLLABLE       = FALSE
       FRAME Dialog-Frame:HIDDEN           = TRUE.

ASSIGN 
       BROWSE-Lister:NUM-LOCKED-COLUMNS IN FRAME Dialog-Frame     = 1.
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE BROWSE-Lister
/* Query rebuild information for BROWSE BROWSE-Lister
     _TblList          = "skotex.Lister"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _OrdList          = "skotex.Lister.ListeNr|yes"
     _Where[1]         = "Lister.ListeType = wListeType and
Lister.ListeNr > 0"
     _FldNameList[1]   > skotex.Lister.ListeNr
"Lister.ListeNr" ? ? "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > skotex.Lister.Eier
"Lister.Eier" ? ? "character" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > skotex.Lister.Beskrivelse
"Lister.Beskrivelse" "Beskrivelse" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > skotex.Lister.Merknad
"Lister.Merknad" ? ? "character" ? ? ? ? ? ? no ? no no "53" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-Lister */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Søkeliste Koleksjonslister */
DO:
...
&Scoped-define BROWSE-NAME BROWSE-Lister
&Scoped-define SELF-NAME BROWSE-Lister
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Lister Dialog-Frame
ON ANY-PRINTABLE OF BROWSE-Lister IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Lister Dialog-Frame
ON CURSOR-LEFT OF BROWSE-Lister IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Lister Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-Lister IN FRAME Dialog-Frame
DO:
  IF {&ant-sok-field} = 1 THEN
    RETURN NO-APPLY.
  RUN SD-CURSOR ("RIGHT").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Lister Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-Lister IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Lister Dialog-Frame
ON START-SEARCH OF BROWSE-Lister IN FRAME Dialog-Frame
DO:
  RUN SD-START-SEARCH.
  RETURN NO-APPLY.
END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ON END-KEY UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK:
    ...
    {&init-phrase}
/*         FIND bLister WHERE bLister.ListeType = wListeType and                                  */
/*                                  bLister.ListeNr = wListeNr USE-INDEX Lister NO-LOCK NO-ERROR. */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-INT FILL-IN-SOK-CHAR FILL-IN-SOK-DATE BUTTON-Sok 
         BROWSE-Lister Btn_OK Btn_Cancel Btn_Help 
      WITH FRAME Dialog-Frame.
