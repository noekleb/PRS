...
&Scoped-Define ENABLED-OBJECTS RECT-44 RECT-45 FI-BildNr1 FI-BildNr2 Btn_OK ~
Btn_Cancel Btn_Help 
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Bildekonvertering */
DO:
...
OCXFile = SEARCH( "d-konvbmpjpg.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "d-konvbmpjpg.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-44 RECT-45 FI-BildNr1 FI-BildNr2 Btn_OK Btn_Cancel Btn_Help 
      WITH FRAME Dialog-Frame.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE KonvBilde Dialog-Frame 
PROCEDURE KonvBilde :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
------------------------------------------------------------------------------*/
  ...
  message "Skal konvertering starte?"
  ...
  for each BildeRegister exclusive-lock where
    ...
    if wStop then
      do:
        message "Skal konverteringsrutinen stoppes? " view-as alert-box buttons YES-NO 
                 set wStop.
        if wStop = true then
          leave BILDEREGISTER.
      end.

    /* Vi tar kun BMP under behandling. */
    if substring(BildeRegister.FilNavn,length(BildeRegister.FilNavn) - 2) = "BMP" then.
    else next BILDEREGISTER.
    ...
    assign
      BildeRegister.FilNavn = substring(BildeRegister.FilNavn,1,length(BildeRegister.FilNavn) - 3) + "jpg"
      wNyttFilNavn = substring(wBildePeker,1,length(wBildePeker) - 3) + "jpg"
      chIMAGE-Sko:Picbuf:filename  = search(wBildePeker)
      chIMAGE-Sko:Picbuf:AutoScale = True.        
    wOk = chIMAGE-Sko:Picbuf:load.    

    /* Tildeler filnavn */
    chIMAGE-Sko:Picbuf:FileName = wNyttFilNavn.
    ...
    /* Hvis ny fil er opprettet, kaster vi den gamle */
    if search(wNyttFilNavn) <> ? then
      do:
