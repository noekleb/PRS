...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  DEFINE VAR wSasong LIKE Sasong.Sasong NO-UNDO.
&ELSE
  DEFINE INPUT-OUTPUT PARAMETER wSasong LIKE Sasong.Sasong NO-UNDO.
&ENDIF
...
&scoped-define sok-field1  Sasong
...
&scoped-define index-col1  SasongIn
...
&scoped-define sok-field2  SasBeskr
...
&scoped-define index-col2  SasBeskr
...
/*
/* Kolonne 2 som det kan sorteras på, fjern om du bara vill ha 1 */
&scoped-define sok-field3  StartDato
...
&scoped-define index-col3  Sasong
&scoped-define sok-phrase3 FIND FIRST b{&br-tabell} WHERE b{&br-tabell}.{&sok-field2} >= ~
...
&scoped-define ip-variabel wSasong
&scoped-define return-ip   wSasong = if available SaSong then Sasong.Sasong else 0.
&scoped-define assign-retur-verdi retur-verdi = if available SaSong then ~
                                                  string(recid(Sasong)) else ?.
...
&scoped-define KanSlettes ~
  if can-find(first ArtBas where ArtBas.SaSong = SaSong.SaSong) then ~
    do: ~
      message "Det finns artiklar med denna sesongkoden" ~
      view-as alert-box title "Fel vid borttagning". ~
      return no-apply. ~
    end.

&scoped-define VedlikeholdsRutine d-vsasong.w
...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME BROWSE-Sasong

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES SaSong

/* Definitions for BROWSE BROWSE-Sasong                                 */
&Scoped-define FIELDS-IN-QUERY-BROWSE-Sasong SaSong.Sasong SaSong.SasBeskr ~
SaSong.StartDato SaSong.SluttDato 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-Sasong SaSong.Sasong ~
SaSong.SasBeskr 
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-Sasong SaSong
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-Sasong SaSong
&Scoped-define QUERY-STRING-BROWSE-Sasong FOR EACH SaSong NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-Sasong OPEN QUERY BROWSE-Sasong FOR EACH SaSong NO-LOCK ~
    ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-BROWSE-Sasong SaSong
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-Sasong SaSong


/* Definitions for DIALOG-BOX Dialog-Frame                              */
&Scoped-define OPEN-BROWSERS-IN-QUERY-Dialog-Frame ~
    ~{&OPEN-QUERY-BROWSE-Sasong}

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS FILL-IN-SOK-CHAR FILL-IN-SOK-INT ~
FILL-IN-SOK-DATE BUTTON-Sok B-KonvTbl BROWSE-Sasong BUTTON-Ny BUTTON-Endre ~
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY BROWSE-Sasong FOR 
      SaSong SCROLLING.
&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE BROWSE-Sasong
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-Sasong Dialog-Frame _STRUCTURED
  QUERY BROWSE-Sasong NO-LOCK DISPLAY
      SaSong.Sasong COLUMN-LABEL "Sesong *" FORMAT "zzzzz9":U
      SaSong.SasBeskr COLUMN-LABEL "Beskrivelse *" FORMAT "x(20)":U
            WIDTH 36.8
      SaSong.StartDato FORMAT "99/99/9999":U
      SaSong.SluttDato FORMAT "99/99/9999":U WIDTH 13
  ENABLE
      SaSong.Sasong
      SaSong.SasBeskr
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 77 BY 14.05 FIT-LAST-COLUMN.
...
DEFINE FRAME Dialog-Frame
     ...
     BROWSE-Sasong AT ROW 2.91 COL 2
     BUTTON-Ny AT ROW 2.91 COL 79.8
     BUTTON-Endre AT ROW 4.24 COL 79.8
     BUTTON-Slett AT ROW 5.57 COL 79.8
     B-Utskrift AT ROW 8.38 COL 80.2
     Btn_Help AT ROW 10.76 COL 79.8
     Btn_OK AT ROW 17.19 COL 1
     Btn_Cancel AT ROW 17.19 COL 13.4
     SPACE(70.19) SKIP(0.22)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste sesongkoder"
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   FRAME-NAME                                                           */
/* BROWSE-TAB BROWSE-Sasong B-KonvTbl Dialog-Frame */
ASSIGN 
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE BROWSE-Sasong
/* Query rebuild information for BROWSE BROWSE-Sasong
     _TblList          = "SkoTex.SaSong"
     _Options          = "NO-LOCK INDEXED-REPOSITION SORTBY-PHRASE"
     _FldNameList[1]   > SkoTex.SaSong.Sasong
"Sasong" "Sesong *" ? "integer" ? ? ? ? ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > SkoTex.SaSong.SasBeskr
"SasBeskr" "Beskrivelse *" ? "character" ? ? ? ? ? ? yes ? no no "36.8" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   = SkoTex.SaSong.StartDato
     _FldNameList[4]   > SkoTex.SaSong.SluttDato
"SluttDato" ? ? "date" ? ? ? ? ? ? no ? no no "13" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-Sasong */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Søkeliste sesongkoder */
DO:
...
&Scoped-define BROWSE-NAME BROWSE-Sasong
&Scoped-define SELF-NAME BROWSE-Sasong
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Sasong Dialog-Frame
ON ANY-PRINTABLE OF BROWSE-Sasong IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Sasong Dialog-Frame
ON CURSOR-LEFT OF BROWSE-Sasong IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Sasong Dialog-Frame
ON CURSOR-RIGHT OF BROWSE-Sasong IN FRAME Dialog-Frame
DO:
  IF {&ant-sok-field} = 1 THEN
    RETURN NO-APPLY.
  RUN SD-CURSOR ("RIGHT").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Sasong Dialog-Frame
ON DEFAULT-ACTION OF BROWSE-Sasong IN FRAME Dialog-Frame
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BROWSE-Sasong Dialog-Frame
ON START-SEARCH OF BROWSE-Sasong IN FRAME Dialog-Frame
DO:
  RUN SD-START-SEARCH.
  RETURN NO-APPLY.
END.
...
{syspara.i 16 14 2 iTilgang INT}
...
------------------------------------------------------------------------------*/
  ...
  ENABLE FILL-IN-SOK-CHAR FILL-IN-SOK-INT FILL-IN-SOK-DATE BUTTON-Sok B-KonvTbl 
         BROWSE-Sasong BUTTON-Ny BUTTON-Endre BUTTON-Slett B-Utskrift Btn_Help 
         Btn_OK Btn_Cancel 
      WITH FRAME Dialog-Frame.
...
------------------------------------------------------------------------------*/
  ...
  if valid-handle(wLibHandle) then
    run GetTempFileName in wLibHandle ("Sasong", cExcEkstent, output cFileName). 
