...
IF NOT AVAILABLE ArtBas THEN
  ...
  END.
  /* Skomodus */
  FIND SysPara WHERE SysPara.SysHId = 1 AND
                     SysPara.SysGr  = 1 AND
                     SysPara.ParaNr = 54 NO-LOCK NO-ERROR.
...
IF NUM-ENTRIES(wStrListe,";") <> 99 THEN DO:
    ...
    IF NUM-ENTRIES(wStrListe2,";") = NUM-ENTRIES(wStrListe,";") THEN
        wStrListe = wStrListe2.
END.

{swn.i}
    IF PROPATH MATCHES "*ken1*" THEN DO:
        OUTPUT TO "C:\tmp\strliste.txt" APPEND.
        PUT UNFORMATTED wStrListe SKIP wStrListe2 SKIP cBrukteStr SKIP.
        OUTPUT CLOSE.
    END.
END PROCEDURE.
...
&IF DEFINED(EXCLUDE-ByggLagerSko) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE ByggLagerSko Procedure 
PROCEDURE ByggLagerSko :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
IF ArtBas.StrTypeId > 1 THEN DO:
  /* Størrelser fra størrelsestypen */
    FOR EACH StrTStr NO-LOCK WHERE StrTStr.StrTypeId = ArtBas.StrTypeId BY StrTStr.SeqNr:
        FIND FIRST StrKonv WHERE strkonv.storl = StrTstr.soStorl NO-LOCK NO-ERROR.
        IF AVAIL StrKonv THEN DO:
            CREATE ttSTrKonv.
            BUFFER-COPY StrKonv EXCEPT strKonv.seqnr TO ttStrKonv
            ASSIGN ttStrKonv.seqNr = strTstr.seqNr
                   ttStrKonv.euStorl = strTstr.euStorl.
            iMax = MAX(iMax,strTstr.seqNr).
        END.
    END.

  /* Størrelser fra strekkodene */
    FOR EACH Strekkode OF ArtBas NO-LOCK:
        IF NOT CAN-FIND(FIRST ttStrKonv WHERE ttStrKonv.StrKode = Strekkode.StrKode) THEN DO:
            FIND StrKonv OF Strekkode NO-LOCK NO-ERROR.
            ...
            CREATE ttSTrKonv.
            BUFFER-COPY StrKonv EXCEPT strKonv.seqnr TO ttStrKonv
                ASSIGN ttStrKonv.seqNr = iMax.
        END.
    END.
    /* Størrelser fra størrelsestypen */
    FOR EACH ArtLag NO-LOCK WHERE ArtLag.ArtikkelNr = ArtBas.ArtikkelNr BREAK BY ArtLag.ArtikkelNr BY ArtLag.Storl:
        IF LAST-OF(ArtLag.Storl) THEN BLOKKEN: DO:
            FIND FIRST StrKonv NO-LOCK WHERE StrKonv.Storl = ArtLag.Storl NO-ERROR.
            IF NOT AVAILABLE StrKonv THEN 
                LEAVE BLOKKEN.
            IF NOT CAN-FIND(FIRST ttStrKonv WHERE ttStrKonv.StrKode = StrKonv.StrKode) THEN DO:
                iMax = iMax + 1.
                CREATE ttStrKonv.
                BUFFER-COPY StrKonv EXCEPT strKonv.seqnr TO ttStrKonv
                    ASSIGN ttStrKonv.seqNr = iMax.
    ...
    END.
  /* Sorterte størrelser */
  FOR EACH ttStrKonv BY ttStrKonv.SeqNr:
      ASSIGN wStrListe   = wStrListe   + (IF wStrListe  = "" THEN "" ELSE ";") + ttStrKonv.Storl
             wStrListe2  = wStrListe2  + (IF wStrListe2 = "" THEN "" ELSE ";") + ttStrKonv.Storl + (IF TRIM(ttStrKonv.EUStorl) <> "" AND
                                                                                                  TRIM(ttStrKonv.EUStorl) <> TRIM(ttStrKonv.Storl)
                                                                                               THEN  "/" + TRIM(ttStrKonv.EUStorl) ELSE "")
             cBrukteStr = cBrukteStr + (IF wStrListe = "" THEN "" ELSE ",").
  END.
END.
...
DO:
    ...
    IF NOT AVAIL strtype THEN
        LEAVE.
    FOR EACH strekkode OF artbas NO-LOCK.
        FIND ttstrkonv OF strekkode NO-LOCK NO-ERROR.
        IF ERROR-STATUS:ERROR THEN
            NEXT.
        iLookup = LOOKUP(ttStrkonv.Storl,wStrListe,";").
        IF iLookup > 0 AND iLookup < 99 THEN
            ENTRY(iLookup,cBrukteStr) = "J".
    END.
END.
