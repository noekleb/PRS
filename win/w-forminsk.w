...
DEFINE INPUT  PARAMETER ipBildNr LIKE Bilderegister.bildnr NO-UNDO.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ON END-KEY UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK:
    ...
    IF NOT AVAIL Bilderegister THEN DO:
        MESSAGE "Bilderegisterpost savnes!!"
...
OCXFile = SEARCH( "w-forminsk.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-forminsk.wrx":U SKIP(1)
...
FOR EACH Bildedata OF Bilderegister WHERE Bildedata.Teller >= 200:
    DELETE Bildedata.
END.
RUN visbilde (ipBildnr,3).

ASSIGN iWidth = 120
       iHeight  = iWidth * (chIMAGE-Sko:Picbuf:XResolution / chIMAGE-Sko:Picbuf:YResolution).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE KonverterOrg C-Win 
PROCEDURE KonverterOrg :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE KonverterStor C-Win 
PROCEDURE KonverterStor :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
RUN visbilde (ipBildnr,3).
ASSIGN iHeight = 400
       iWidth  = iHeight * (chIMAGE-Sko:Picbuf:YResolution / chIMAGE-Sko:Picbuf:XResolution).
/*        iWidth  = IF iHeight > 200 THEN 150 ELSE iHeight. */
 /* chSko2:AutoSize = TRUE. */
/* MESSAGE                                    */
/* "iWidth" iWidth  SKIP                      */
/* "Xres" chIMAGE-Sko:Picbuf:XResolution SKIP */
/* "iHeight" iHeight SKIP                     */
/* "Yres" chIMAGE-Sko:Picbuf:YResolution      */
...
/*             'saved as                                                    */
    ASSIGN cFilNavn = SUBSTR(chIMAGE-Sko:PicBuf:FILENAME,1,R-INDEX(chIMAGE-Sko:Picbuf:FILENAME,"\")) +
             "mini" +
             ENTRY(NUM-ENTRIES(chIMAGE-Sko:Picbuf:FILENAME,"\"),chIMAGE-Sko:Picbuf:FILENAME,"\").
    ASSIGN chIMAGE-Mini:filename  = cFilNavn NO-ERROR.
/*             'Save the new image                                          */
           chIMAGE-Mini:Store().
/*     chIMAGE-Sko:Picbuf:CLEAR(0). */
/*     chIMAGE-Mini:CLEAR(0).       */
