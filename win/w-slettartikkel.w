...
&Scoped-Define ENABLED-OBJECTS T-Salg B-slettArtikkler T-Best T-Lager T-PLU ~
FI-FraLevDato B-Rediger B-SokListe BUTTON-SokDato3 FI-Navn FI-Merknad ~
...
T-AktivTilbud B-Varemerker B-Sesong B-VarGr B-Farg B-LevBas B-Material ~
B-Exit B-Ny Btn_Help BUTTON-SokDato-2 BUTTON-SokDato FI-TilLevDato ~
BUTTON-SokLevDato1 BUTTON-SokLevDato2 RECT-2 RECT-5 RECT-6 RECT-7 RECT-8 
&Scoped-Define DISPLAYED-OBJECTS T-Salg T-Best T-Lager T-PLU FI-FraLevDato ~
FI-ListeNr FI-Navn FI-Merknad FI-FraLopNr FI-TilLopNr FI-Opprettet ~
FI-FraKateg FI-TilKateg FI-Endret FI-FraDato FI-TilDato FI-Eier FI-LevKod ~
FI-FraArtikkelNr FI-TilArtikkelNr FI-FraBildNr FI-TilBildNr FI-FraFraTilbud ~
FI-TilFraTilbud FI-FraTilTilbud FI-TilTilTilbud RS-Aktiv T-Annonse ~
T-AktivTilbud FI-Varemerker T-Butikk FI-Butiker FI-Sesong FI-VAreGr FI-Farg ~
FI-LevBas FI-Material FI-Info FILL-IN-Tekst FI-TilLevDato 
...
DEFINE BUTTON B-slettArtikkler 
     LABEL "Slett artikler" 
     SIZE 28 BY 1.14
     FGCOLOR 12 FONT 6.
...
DEFINE VARIABLE FI-TilBildNr AS INTEGER FORMAT "zzzzz9":U INITIAL 999999 
     VIEW-AS FILL-IN 
     SIZE 20 BY 1
     BGCOLOR 13  NO-UNDO.
...
DEFINE VARIABLE T-Best AS LOGICAL INITIAL yes 
     LABEL "Har IKKE bestillinger" 
     VIEW-AS TOGGLE-BOX
     SIZE 30 BY .81
     BGCOLOR 13  NO-UNDO.
...
DEFINE VARIABLE T-Lager AS LOGICAL INITIAL yes 
     LABEL "Har IKKE lager" 
     VIEW-AS TOGGLE-BOX
     SIZE 30 BY .81
     BGCOLOR 13  NO-UNDO.

DEFINE VARIABLE T-PLU AS LOGICAL INITIAL yes 
     LABEL "IKKE ta med PLUér" 
     VIEW-AS TOGGLE-BOX
     SIZE 30 BY .81
     BGCOLOR 13  NO-UNDO.

DEFINE VARIABLE T-Salg AS LOGICAL INITIAL yes 
     LABEL "Har IKKE salg" 
     VIEW-AS TOGGLE-BOX
     SIZE 30 BY .81
     BGCOLOR 13  NO-UNDO.
...
DEFINE FRAME DEFAULT-FRAME
     T-Salg AT ROW 15.29 COL 104
     B-slettArtikkler AT ROW 1.24 COL 64
     T-Best AT ROW 12.43 COL 104
     T-Lager AT ROW 13.38 COL 104
     T-PLU AT ROW 14.33 COL 104
     ...
     FI-Info AT ROW 24.1 COL 2 NO-LABEL
     B-Bygg AT ROW 1.29 COL 7
     B-Exit AT ROW 1.29 COL 131.4
     B-Forrige AT ROW 1.29 COL 26.6
     B-Neste AT ROW 1.29 COL 21.6
...
/* SETTINGS FOR FILL-IN FILL-IN-Tekst IN FRAME DEFAULT-FRAME
   NO-ENABLE ALIGN-L                                                    */
/* SETTINGS FOR TOGGLE-BOX T-Butikk IN FRAME DEFAULT-FRAME
   NO-ENABLE                                                            */
...
ON END-ERROR OF C-Win /* Sletting av artikkler */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Sletting av artikkler */
DO:
...
&Scoped-define SELF-NAME B-slettArtikkler
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-slettArtikkler C-Win
ON CHOOSE OF B-slettArtikkler IN FRAME DEFAULT-FRAME /* Slett artikler */
DO:
  ASSIGN
      lValg = FALSE
      .
  MESSAGE "Skal sletting av artikkler som er logget i listen startes?"
      VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO UPDATE lValg.
  IF lValg = TRUE THEN
  SLETTER:
  DO:
...
OCXFile = SEARCH( "w-slettartikkel.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-slettartikkel.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  RUN control_load.
  DISPLAY T-Salg T-Best T-Lager T-PLU FI-FraLevDato FI-ListeNr FI-Navn 
          ...
          FILL-IN-Tekst FI-TilLevDato 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE T-Salg B-slettArtikkler T-Best T-Lager T-PLU FI-FraLevDato B-Rediger 
         ...
         T-AktivTilbud B-Varemerker B-Sesong B-VarGr B-Farg B-LevBas B-Material 
         B-Exit B-Ny Btn_Help BUTTON-SokDato-2 BUTTON-SokDato FI-TilLevDato 
         BUTTON-SokLevDato1 BUTTON-SokLevDato2 RECT-2 RECT-5 RECT-6 RECT-7 
         RECT-8 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
*/
  ...
  do TRANSACTION:     
    ...
    assign
      ...
      bLister.Kriterier[20] = (if T-Best 
                               then "TRUE"
                               else "FALSE") + CHR(1) + 
                              (if T-Lager 
                               then "TRUE"
  ...
  if wModus = "NY" then
    do:
      run byggslettartikkler.p persistent set wChild
                         (wListerRecid,
                          FI-FraDato,
                          FI-TilDato,
                          FI-FraLevDato,
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE SlettArtBas C-Win 
...
------------------------------------------------------------------------------*/
  ...
  DEF BUFFER bListeLinje FOR ListeLinje.
  ...
  for each ListeLinje no-lock where
    ...
    STATUS DEFAULT "Antall slettet: " + string(wAntall) + " Artikkel: " +  STRING(ListeLinje.DataObjekt) + " Type: " + ListeLinje.ListeType + ".".
    RUN slettartbasbatch.w (DEC(ENTRY(1,ListeLinje.DataObjekt))).
    DO TRANSACTION:
        FIND bListeLinje EXCLUSIVE-LOCK WHERE
            RECID(bListeLinje) = RECID(ListeLinje).
        DELETE bListeLinje.
    END.
  end. /* SLETTER */

  STATUS DEFAULT "Slettet " + STRING(wAntall) + " artikkler.".

  MESSAGE "Slettet " + STRING(wAntall) + " artikkler."
...
------------------------------------------------------------------------------*/
  ...
  assign
    ...
    T-Best         = if entry(1,Lister.Kriterier[20],CHR(1)) = "TRUE"
                       then true
                       else false    
    T-Lager        = if entry(2,Lister.Kriterier[20],CHR(1)) = "TRUE"
                       then true
                       else false    
    T-PLU          = if entry(3,Lister.Kriterier[20],CHR(1)) = "TRUE"
                       then true
                       else false    
    T-Salg         = if entry(4,Lister.Kriterier[20],CHR(1)) = "TRUE"
                       then true
                       else false    
