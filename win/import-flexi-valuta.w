...
DEF VAR lValutaNye       AS   LOG                NO-UNDO.
DEF VAR lValutaOverskriv AS   LOG                NO-UNDO.
...
ON END-ERROR OF C-Win /* Import av varemerke */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Import av varemerke */
DO:
...
{syspara.i 1 2 1010 cEDB-System}
{syspar2.i 1 2 1010 cImpTabell}
...
------------------------------------------------------------------------------*/
  ...
  DEF VAR pcValuta AS char NO-UNDO.
  ...
  FOR EACH ImportLinje EXCLUSIVE-LOCK WHERE
      ...
      IF AVAILABLE ImpKonv THEN
        ASSIGN
          pcValuta = (ImpKonv.InterntId)
          .

      FIND Valuta EXCLUSIVE-LOCK WHERE
          Valuta.ValKod = pcValuta NO-ERROR.
      
      /* Er det ny valuta, sjekk om den skal opprettes ? */
      IF lValutaNye = FALSE AND NOT AVAILABLE Valuta THEN
          NEXT MAINLOOP.
      ...
      DO:
          CREATE Valuta.
          ASSIGN
              /* '#KOD' */ Valuta.ValKod = pcValuta
              .
      END.

      /* Hvis det ikke er en ny post, skal data overskrives ? */
      IF lValutaOverskriv = FALSE AND NOT NEW Valuta THEN
          NEXT MAINLOOP.
      
      ASSIGN
        /* '#SÄSONG'   */
        /* '#BESK'     */  Valuta.ValLand  = ImportLinje.Felt[3]
        /* '#AKTKURS'  */
        /* '#KURSISEK' */  Valuta.ValKurs  = dec(importLinje.Felt[5]) 
        /* '#ÄNDRING'  */  Valuta.ValDatum = IF NUM-ENTRIES(importLinje.Felt[6],"-") = 3
                                             THEN DATE(
                                                       int(ENTRY(2,importLinje.Felt[6],"-")),
                                                       int(ENTRY(3,importLinje.Felt[6],"-")),
                                                       int(ENTRY(1,importLinje.Felt[6],"-"))
                                                      )
                                             ELSE ?
        .
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEF INPUT PARAMETER plValutaNye       AS LOG NO-UNDO.
  DEF INPUT PARAMETER plValutaOverskriv AS LOG NO-UNDO.
  
  ASSIGN
    lValutaNye       = plValutaNye      
    lValutaOverskriv = plValutaOverskriv
    .
END PROCEDURE.
