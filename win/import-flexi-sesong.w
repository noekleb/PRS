...
DEF VAR lSesongNye         AS   LOG                NO-UNDO.
DEF VAR lSesongOverskriv   AS   LOG                NO-UNDO.
...
ON END-ERROR OF C-Win /* Import av sesonger */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Import av sesonger */
DO:
...
{syspara.i 1 2 1001 cEDB-System}
{syspar2.i 1 2 1001 cImpTabell}
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH ImportLinje EXCLUSIVE-LOCK WHERE
      ...
      IF lSesongNye = FALSE AND NOT AVAILABLE Sasong THEN
          NEXT MAINLOOP.
      ...
      DO:
          CREATE Sasong.
          ASSIGN
              /* '#KOD' */ Sasong.Sasong = piSaSong
              .
      END.

      /* Hvis det ikke er en ny varegrupper, skal data overskrives ? */
      IF lSesongOverskriv = FALSE AND NOT NEW Sasong THEN
          NEXT MAINLOOP.

      ASSIGN
        /* '#BESK'    */  Sasong.SasBeskr = ImportLinje.Felt[2]
        /* '#FRÅNDAT' */
        /* '#TILLDAT' */
        .
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEF INPUT PARAMETER plSesongNye       AS LOG NO-UNDO.
  DEF INPUT PARAMETER plSesongOverskriv AS LOG NO-UNDO.
  
  ASSIGN
    lSesongNye       = plSesongNye      
    lSesongOverskriv = plSesongOverskriv
    .
END PROCEDURE.
