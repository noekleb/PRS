...
DEFINE FRAME DEFAULT-FRAME
     ...
     "" VIEW-AS TEXT
          ...
          BGCOLOR 10 FONT 6
     "WHEAT med hämtning av str" VIEW-AS TEXT
...
/*                ant        = int(ENTRY(12,cString,";")) */
               levpris    = deci(REPLACE(ENTRY(13,cString,";")," ",""))
               veilpris    = deci(REPLACE(ENTRY(17,cString,";")," ",""))
                vg         = int(ENTRY(23,cString,";"))
               ntoforh    = deci(REPLACE(ENTRY(25,cString,";")," ",""))
...
/*            FI-AntallE:SCREEN-VALUE = STRING(iAnt).                */
    ...
    END.
    FOR EACH TT_Pricat:
        FIND strtype WHERE strtype.strtypeid = TT_Pricat.strtype NO-LOCK NO-ERROR.
            IF NOT AVAIL strtype THEN DO:
                MESSAGE "FInner ikker varegruppe " TT_Pricat.strtype
                    VIEW-AS ALERT-BOX INFO BUTTONS OK.
                NEXT.
            END.
        IF TT_Pricat.ean <> "" AND TT_Pricat.strtype <> 2 THEN DO:
            FIND strkonv WHERE TRIM(strKonv.storl) = TT_Pricat.storl NO-LOCK NO-ERROR.
            IF NOT AVAIL strkonv THEN DO:
                MESSAGE "finner ikke strkonv" TT_Pricat.storl
                    VIEW-AS ALERT-BOX INFO BUTTONS OK.
                NEXT.
            END.
            FIND strtstr WHERE StrTStr.StrTypeID = TT_Pricat.strtype AND 
...
DO:
    ...
    FOR EACH TT_Pricat BREAK BY levkod BY fargekode BY strtype.
        IF FIRST-OF(strtype) THEN DO:
...
/*             ArtBas.ArtikkelNr     = TT_Pricat.modell */
     ...
     TT_Pricat.ArtikkelNr = ArtBas.Artikkelnr.
      RELEASE ArtBas.
      IF TRIM(TT_Pricat.Ean) = "" THEN
          RUN genStrekKode.p (TT_Pricat.ArtikkelNr,1,"").
      ELSE IF TT_Pricat.strtype <> 2 THEN
...
------------------------------------------------------------------------------*/
    ...
    CREATE Strekkode.
    ASSIGN StrekKode.Kode       = STRING(DECI(TT_Pricat.Ean),"9999999999999")
