...
FOR EACH BongHode WHERE
    ...
    */    
    ...
    FOR EACH BongLinje NO-LOCK WHERE
       ...
       /*
        DISPLAY 
            ...
            BongLinje.Antall
            BongLinje.Linjesum
            TransLogg.KundNr WHEN AVAILABLE TransLogg
            TransLogg.KortNr WHEN AVAILABLE TransLogg
            .
       */
       KUNDETRANSER:
       DO:
          ...
          IF CAN-FIND(KundeTrans WHERE
                      KundeTrans.KundeNr   = BongHode.KundeNr AND
                      KundeTrans.Butik     = BongLinje.ButikkNr    AND
                      KundeTrans.TransNr   = BongLinje.TransNr  AND
                      KundeTrans.SeqNr     = BongLinje.SeqNr) THEN
            LEAVE KUNDETRANSER.
          ...
          DO TRANSACTION:
            CREATE KundeTrans.
            ASSIGN
                KundeTrans.KundeNr      = BongHode.KundeNr
                KundeTrans.Butik        = BongLinje.ButikkNr
                KundeTrans.TransNr      = BongLinje.TransNr
                KundeTrans.SeqNr        = BongLinje.SeqNr
                .
                
            ASSIGN
              KundeTrans.KassaNr        = BongLinje.KasseNr
              KundeTrans.Dato           = BongLinje.TransDato
              KundeTrans.Tid            = BongLinje.TransTid
              KundeTrans.BongId         = BongLinje.BongNr
              KundeTrans.BongLinjeNr    = BongLinje.LinjeNr
      
              KundeTrans.BatchNr        = TransLogg.BatchNr
              KundeTrans.TTId           = BongLinje.TTId
              KundeTrans.TBId           = BongLinje.TBId
              KundeTrans.ArtikkelNr     = dec(BongLinje.ArtikkelNr)
              KundeTrans.LevNr          = (IF AVAILABLE ArtBas
                                         THEN ArtBas.LevNr
                                         ELSE 0)
              KundeTrans.Vg             = BongLinje.VareGr
              KundeTrans.LopNr          = BongLinje.LopeNr
              KundeTrans.Storl          = BongLinje.Storrelse
              KundeTrans.Antall         = BongLinje.Antall
              KundeTrans.Pris           = BongLinje.LinjeSum
              KundeTrans.RabKr          = BongLinje.LinjeRab
              KundeTrans.SubTotalRab    = BongLinje.SubTotalRab
              KundeTrans.Mva            = BongLinje.MvaKr
              KundeTrans.VVarekost      = TransLogg.VVarekost
              KundeTrans.SelgerNr       = BongHode.SelgerNr
              KundeTrans.MeldemsNr      = BongHode.MedlemsNr
              KundeTrans.BongTekst      = BongLinje.BongTekst
              KundeTrans.SattVVareKost  = (IF BongLinje.VVareKost <> 0
                                           THEN TRUE 
                                           ELSE FALSE)
              KundeTrans.KortNr         = IF BongHode.KortType = 3 /* Medlemskort */
                                          THEN BongHode.MedlemsKort
                                        ELSE IF BongHode.KortType = 2 
                                          THEN BongHode.KundeKort
                                        ELSE ""
              KundeTrans.ForsNr         = BongHode.KassererNr
              KundeTrans.MotPostert     = FALSE
              KundeTrans.RefNr          = BongLinje.RefNr
              KundeTrans.RefTekst       = BongLinje.RefTekst
              .
      
            POSTERING-SALDO:
            DO WHILE TRUE:
              FIND KundeSaldo EXCLUSIVE-LOCK WHERE
                   KundeSaldo.KundeNr   = BongHode.KundeNr AND
                   KundeSaldo.Butik     = BongLinje.ButikkNr
                  NO-ERROR NO-WAIT.
              /* Posten holdes av en annen, vi forsøker igjen. */
              IF LOCKED KundeSaldo THEN
                  NEXT POSTERING-SALDO.
              /* Oppretter ny post. */
              IF NOT AVAILABLE KundeSaldo THEN
              DO:
                  CREATE KundeSaldo.
                  ASSIGN
                      KundeSaldo.KundeNr   = BongHode.KundeNr
                      KundeSaldo.Butik     = BongLinje.ButikkNr
                      .
              END.
              ...
              IF KundeSaldo.ForsteDato = ? THEN
                ASSIGN
                  KundeSaldo.ForsteDato = BongLinje.TransDato
                  KundeSaldo.ForsteTid  = BongLinje.TransTid
                  .
              /* Posterer siste gangs kjøp. */
              ASSIGN
                  KundeSaldo.DatoSiste  = BongLinje.TransDato
                  KundeSaldo.SisteTid   = BongLinje.TransTid
                  .
              /* Oppdaterer saldo. */
              ASSIGN
                  KundeSaldo.Saldo      = KundeSaldo.Saldo + KundeTrans.Pris 
                  KundeSaldo.TotaltKjop = KundeSaldo.TotaltKjop + KundeTrans.Pris
                  .
    ...
    RUN beregnkundesaldo(Kunde.kundeNr, BongHode.ButikkNr).
