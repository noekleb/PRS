...
assign
  wSkipListe = "2,807,808,809,810".

/* Setter batchNr */
run batchlogg.p (program-name(1),
                 "Fiks innleveranser - På butikk 2",
                 output wBatchNr).
...
for each translogg no-lock where
  translogg.ttid = 5 and /* Direkte innleveranser - Varekjøp */
  ...
  if wOldButik <> TransLogg.butik then
    DO:
      find last bufTransLogg where
        bufTransLogg.Butik = 2 /*TransLogg.butik*/
