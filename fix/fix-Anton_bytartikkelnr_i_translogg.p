...
ASSIGN
    ...
    cUtFil  = 'C:\Appdir\Anton_Artikkler_sport1_Ulik_renset_sportsnett_err.csv'
    .
...
repeat:
  ...
  /* Disse artiklene skal også få byttet sitt artikkelnr i Sportsnett databasen */       
  if not can-find(ArtBas where ArtBas.ArtikkelNr = ix1ArtikkelNr) then
  do:
    output to value("Anton_Artikkler_sport1_Ulik_renset_sportsnett.csv") append.
    put unformatted 
      cLinje skip.
    output close.
  end.    
  ...
  do transaction:
    run byttArtikkelNrTranslogg (ix1ArtikkelNr, ix2ArtikkelNr).
  end. 
  
  /* Logger fremdrift til fil. */
  FREMDRIFT:
  do:
    output to value("Anton_artikkelnr_konv_fremdrift_sportsnett.txt") append.
    put unformatted 
      "Antall: " iAntall " "
      ix1ArtikkelNr " "
      ix2ArtikkelNr " "
...
&IF DEFINED(EXCLUDE-byttArtikkelNrTranslogg) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE byttArtikkelNrTranslogg Procedure 
PROCEDURE byttArtikkelNrTranslogg :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
for each TransLogg where
  TransLogg.ArtikkelNr = iOldArtikkelNr:
