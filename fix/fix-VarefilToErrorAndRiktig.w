...
&Scoped-Define ENABLED-OBJECTS RECT-62 FI-Filnavn B-LesInn 
...
DEFINE FRAME DEFAULT-FRAME
     ...
     "" VIEW-AS TEXT
          ...
          BGCOLOR 10 FONT 6
     "Delning av varefil från kassa till en riktig fil + felaktig för rättning" VIEW-AS TEXT
...
DO:
    ...
    DEFINE VARIABLE cFelrad AS CHARACTER  NO-UNDO.
/*     INPUT FROM VALUE(FI-Filnavn:SCREEN-VALUE) CONVERT SOURCE "IBM850". */
    INPUT FROM VALUE(FI-Filnavn:SCREEN-VALUE).
    REPEAT:
        iAnt = iAnt + 1.
        CREATE TT_Vare.
        IMPORT TT_Vare NO-ERROR.
        IF ERROR-STATUS:ERROR THEN DO:
            cFelrad = cFelrad + (IF cFelrad <> "" THEN "," ELSE "") + STRING(iAnt).
        END.
        DELETE TT_Vare.
    END.
    INPUT CLOSE.
    iAnt = 0.
    INPUT FROM VALUE(FI-Filnavn:SCREEN-VALUE).
    OUTPUT TO VALUE(FI-Filnavn:SCREEN-VALUE + ".OK").
    REPEAT:
        iAnt = iAnt + 1.
        IMPORT UNFORMATTED cStr NO-ERROR.
        IF NOT CAN-DO(cFelrad,STRING(iAnt)) THEN
    ...
    iAnt = 0.
    INPUT FROM VALUE(FI-Filnavn:SCREEN-VALUE).
    OUTPUT TO VALUE(FI-Filnavn:SCREEN-VALUE + ".ERROR").
    REPEAT:
        iAnt = iAnt + 1.
        IMPORT UNFORMATTED cStr NO-ERROR.
        IF CAN-DO(cFelrad,STRING(iAnt)) THEN
            PUT UNFORMATTED cStr SKIP.
    END.
    INPUT CLOSE.
    OUTPUT CLOSE.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-62 FI-Filnavn B-LesInn 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
