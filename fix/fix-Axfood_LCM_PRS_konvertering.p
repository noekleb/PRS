...
/*                    artbas.negvare      = vare.ean >= 550 AND vare.ean <= 559 */
        ...
        END.
        ASSIGN 
               artpris.ValPris[1]      = IF AVAILABLE Pris THEN pris.engrosn ELSE artpris.ValPris[1]
...
/*  END.                                 */
  FOR EACH Butiker WHERE Butiker.butik = ibutikkNr:
    ...
    ASSIGN
      ...
      Butiker.ProfilNr     = (If available PrisProfil then PrisProfil.ProfilNr else 1)
      .
  END.
  ...
  DO TRANSACTION:
    ...
    FIND LAST SkoTex.JboxUserSetting NO-LOCK NO-ERROR.
    IF AVAIL SkoTex.JboxUserSetting THEN CURRENT-VALUE(seqJboxUserSettingId,skotex) = SkoTex.JBoxUserSetting.iJboxUserSettingId.        
  END.
