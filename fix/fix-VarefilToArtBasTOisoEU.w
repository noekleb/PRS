...
&Scoped-Define ENABLED-OBJECTS RECT-62 FI-Filnavn B-LesInn B-Skapa-2 
...
DEFINE BUTTON B-Skapa-2 
     LABEL "Skapa plufile saknas" 
     SIZE 28 BY 1.14.
...
DEFINE FRAME DEFAULT-FRAME
     ...
     B-Skapa AT ROW 9.57 COL 23
     B-Skapa-2 AT ROW 12.43 COL 23
     " Konvertering av kassans varefil till SE-artikklar" VIEW-AS TEXT
          SIZE 73 BY 1.67 AT ROW 1.43 COL 5
          BGCOLOR 10 FONT 6
     " Innläsning av fil som konverterar till ISO + Europeiskt num" VIEW-AS TEXT
...
DO:
    ...
    DEFINE VARIABLE cNumF AS CHARACTER  NO-UNDO.
    cNumF = SESSION:NUMERIC-FORMAT.
    SESSION:NUMERIC-FORMAT = "American".
    INPUT FROM VALUE(FI-Filnavn:SCREEN-VALUE). /* CONVERT SOURCE "IBM850" */.
    REPEAT:
        CREATE TT_Vare.
        IMPORT TT_Vare.
        iAnt = iAnt + 1.
...
/*         END.                                                                                                                */
    ...
    IF TT_Vare.Modell = 0 THEN
        DELETE TT_Vare.
    INPUT CLOSE.
    SESSION:NUMERIC-FORMAT = cNumF.
    FI-AntallA:SCREEN-VALUE = STRING(iAnt).
END.
...
DO:
    FOR EACH TT_Vare BREAK BY modell.
        IF FIRST-OF(modell) THEN DO:
            FIND VarGr WHERE VarGr.Vg = TT_Vare.Hg NO-LOCK NO-ERROR.
...
&Scoped-define SELF-NAME B-Skapa-2
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-Skapa-2 C-Win
ON CHOOSE OF B-Skapa-2 IN FRAME DEFAULT-FRAME /* Skapa plufile saknas */
DO:
    OUTPUT TO VALUE(FI-Filnavn:SCREEN-VALUE + ".SAKNAS").
    FOR EACH TT_Vare WHERE modell > 9999 BREAK BY modell.
/*         IF FIRST-OF(modell) THEN DO: */
            FIND VarGr WHERE VarGr.Vg = TT_Vare.Hg NO-LOCK NO-ERROR.
            IF NOT AVAIL VarGr THEN DO:
                MESSAGE TT_Vare.Hg
...
/*                 RUN SkapaArtPris. */
            END.
            ELSE DO:
                IF NOT CAN-FIND(strekkode WHERE strekkode.kode = STRING(TT_Vare.Ean,"9999999999999")) THEN DO:
                    EXPORT TT_Vare.
                END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-62 FI-Filnavn B-LesInn B-Skapa-2 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
/*      FIND LAST bArtbas WHERE bArtbas.Vg = TT_Vare.hgr USE-INDEX vglopnr NO-LOCK NO-ERROR. */
     ...
     ASSIGN
            ...
            ArtBas.Beskr          = REPLACE(TT_Vare.varetekst,'@','"')
            ArtBas.BongTekst      = REPLACE(TT_Vare.bong,'@','"')
            ...
            ArtBas.LopNr          = TT_Vare.LopeNr /* IF AVAIL bArtBas THEN bArtbas.lopnr + 1 ELSE 1 */
            ArtBas.OLLager        = TRUE
            ArtBas.OPris          = FALSE
