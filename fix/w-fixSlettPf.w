...
DEFINE VARIABLE FILL-IN-23 AS CHARACTER FORMAT "X(256)":U INITIAL "Datasett med 1 i pfflagg - töm pfdata" 
     VIEW-AS FILL-IN 
     SIZE 79 BY 1.67
     FONT 8 NO-UNDO.

DEFINE VARIABLE FILL-IN-24 AS CHARACTER FORMAT "X(256)":U INITIAL "Alla Datasett" 
     VIEW-AS FILL-IN 
     SIZE 61 BY 1.43
     FONT 8 NO-UNDO.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH pfTenderRevenue WHERE pfTenderRevenue.Store_No = iButik AND pftenderrevenue.DATE = dDato: 
        DELETE pfTenderRevenue. 
    END.
    
    FOR EACH pfStatisticsRevenue WHERE pfStatisticsRevenue.Store_No = iButik AND pfStatisticsrevenue.DATE = dDato:  
        DELETE pfStatisticsRevenue. 
    END.
    
    FOR EACH pfDaySales_HourExt WHERE pfDaysales_Hourext.Store_No = iButik AND pfDaysales_Hourext.DATE = dDato: 
        DELETE pfDaySales_HourExt. 
    END.
    
    FOR EACH pfDaySales WHERE pfDaysales.Store_No = iButik AND pfDaysales.DATE = dDato:
        DELETE pfDaySales. 
    END.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH datasett NO-LOCK WHERE pfflagg = 1:
        ...
        PROCESS EVENTS.
        FIND FIRST bongHode OF datasett NO-LOCK NO-ERROR.
        IF AVAIL bonghode THEN
            RUN Behandla (bonghode.butik,BongHode.Dato).
    END.
