...
DEFINE VARIABLE FILL-IN-23 AS CHARACTER FORMAT "X(256)":U INITIAL "Nu kör vi så här" 
     VIEW-AS FILL-IN 
     SIZE 36 BY 1.67
     FONT 8 NO-UNDO.
        ...
        FOR EACH bonglinje WHERE BongLinje.b_id = BongHode.b_id NO-LOCK:
            IF Bonglinje.strekkode = "31001" OR Bonglinje.strekkode = "31002" THEN DO:
                ...
                IF Bonglinje.linjesum > 0 AND BongLinje.Antall <> 0 AND BongLinje.Antall <> ? THEN DO:
                    ASSIGN dPrStyck = bonglinje.linjeSum / ABS(bonglinje.Antall)
                           dVVK     = dPrStyck * (IF Bonglinje.strekkode = "31001" THEN .95 ELSE .92).
