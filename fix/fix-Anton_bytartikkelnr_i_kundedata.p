...
ASSIGN
    ...
    cUtFil  = 'C:\Appdir\Anton_Artikkler_sport1_Ulik_renset_kundedata_err.csv'
    .
...
repeat:
  ...
  do transaction:
    run byttArtikkelNrKundedata (ix1ArtikkelNr, ix2ArtikkelNr).
  end. 
  
  /* Logger fremdrift til fil. */
  FREMDRIFT:
  do:
    output to value("Anton_artikkelnr_konv_fremdrift_kundedata.txt") append.
    put unformatted 
      "Antall: " iAntall " "
      ix1ArtikkelNr " "
      ix2ArtikkelNr " "
...
&IF DEFINED(EXCLUDE-byttArtikkelNrKundedata) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE byttArtikkelNrKundedata Procedure 
PROCEDURE byttArtikkelNrKundedata :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
for each KundeTrans where
  KundeTrans.ArtikkelNr = (iOldArtikkelNr):
  
  assign
    KundeTrans.ArtikkelNr = (iNewArtikkelNr)
    KundeTrans.Vg         = ArtBas.Vg
    KundeTrans.LopNr      = ArtBas.LopNr
    .
end.

for each MedTrans where
  MedTrans.ArtikkelNr = (iOldArtikkelNr):
  
  assign
    MedTrans.ArtikkelNr = (iNewArtikkelNr)
    MedTrans.Vg         = ArtBas.Vg
    MedTrans.LopNr      = ArtBas.LopNr
    .
end.

for each KOrdreLinje where KOrdreLinje.VareNr = string(iOldArtikkelNr):
  KOrdreLinje.VareNr = string(iNewArtikkelNr).
end.

for each FakturaLinje where FakturaLinje.VareNr = string(iOldArtikkelNr):
  FakturaLinje.VareNr = string(iNewArtikkelNr).
end.
