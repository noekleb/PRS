...
assign
  cFilNavn = 'c:\appdir\EANPrisListe550' + 
...
for each BongHode no-lock where
  ...
  if not can-find(first EANListe where
                  EANListe.Strekkode = cKode) then 
  do:
    create EANListe.
    assign
      EANListe.Strekkode  = cKode
    ...
    EANListe.Strekkode = cKode no-error.
    
  if available EANListe then 
  assign
    EANListe.Pris     = (BongLinje.LinjeSum + BongLinje.SubtotalRab + BongLinje.LinjeRab) / ABS(BongLinje.Antall)
