...
DEFINE VARIABLE FILL-IN-23 AS CHARACTER FORMAT "X(256)":U INITIAL "PF-flag på vissa datasett sätts till 1" 
     VIEW-AS FILL-IN 
     SIZE 76 BY 1.67
     FONT 8 NO-UNDO.

DEFINE VARIABLE FILL-IN-24 AS CHARACTER FORMAT "X(256)":U INITIAL "Alla butiker 2004 - 2005 MD" 
     VIEW-AS FILL-IN 
     SIZE 61 BY 1.43
     FONT 8 NO-UNDO.
...
DEFINE FRAME DEFAULT-FRAME
     FILL-IN-23 AT ROW 1.71 COL 1 COLON-ALIGNED NO-LABEL
...
/*             NEXT.                    */
        ...
        FOR EACH bonglinje WHERE BongLinje.b_id = BongHode.b_id AND BongLinje.ttid < 13 NO-LOCK:
            IF CAN-DO("1020,1021,1022,30211,30507,30801,31001,31002,31201,31302,31304",bonglinje.strekkode) THEN DO:
                IF bonglinje.strekkode = "1020" AND bonglinje.vvarekost = 493 THEN lFunnet = TRUE.
                ELSE IF bonglinje.strekkode = "1021" AND bonglinje.vvarekost = 294 THEN lFunnet = TRUE.
                ELSE IF bonglinje.strekkode = "1022" AND bonglinje.vvarekost = 140 THEN lFunnet = TRUE.
                ELSE IF bonglinje.strekkode = "30211" AND bonglinje.bongtekst BEGINS "DELI" THEN lFunnet = TRUE.
                ELSE IF bonglinje.strekkode = "30507" AND Bonglinje.vvarekost = 6.5 THEN lFunnet = TRUE.
                ELSE IF bonglinje.strekkode = "30801" AND Bonglinje.vvarekost = 1.5 THEN lFunnet = TRUE.
                ELSE IF bonglinje.strekkode = "31201" AND Bonglinje.vvarekost = 4.6 THEN lFunnet = TRUE.
                ELSE IF bonglinje.strekkode = "31302" AND Bonglinje.vvarekost = 36 THEN lFunnet = TRUE.
                ELSE IF bonglinje.strekkode = "31304" AND Bonglinje.vvarekost = 53 THEN lFunnet = TRUE.
                ELSE IF bonglinje.strekkode = "31001" THEN lFunnet = TRUE.
                ELSE IF bonglinje.strekkode = "31002" THEN lFunnet = TRUE.
                IF lFunnet = TRUE THEN
                    LEAVE BHODE.
            END.
        END.
    END.
    IF lFunnet = TRUE THEN DO:
        FIND datasett WHERE datasett.datasettid = bonghode.datasettid NO-LOCK NO-ERROR.
/*         IF AVAIL datasett AND datasett.pfflagg <> 1 THEN DO: */
/*             FIND CURRENT datasett EXCLUSIVE.                 */
/*             datasett.pfflagg = 1.                            */
/*         END.                                                 */
        IF NOT AVAIL datasett THEN DO:
...
------------------------------------------------------------------------------*/
    ...
    DO iCount = 1 TO NUM-ENTRIES(cButiker):
        DO dDato = DATE(1,1,2004) TO DATE(02,28,2005):
            FI-Beh:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ENTRY(iCount,cButiker) + " - " + STRING(dDato).
            PROCESS EVENTS.
            RUN Behandla (INT(ENTRY(iCount,cButiker)),dDato,OUTPUT dDataSettid).
            IF dDatasettid = 91 THEN DO:
                MESSAGE PROGRAM-NAME(1) SKIP
                ENTRY(iCount,cButiker)
                    VIEW-AS ALERT-BOX INFO BUTTONS OK.
                RETURN.
            END.
/*             IF dDataSettId <> 0 THEN DO:                                                                     */
