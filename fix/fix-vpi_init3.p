...
PAUSE 0.
TEST:
FOR EACH ArtBas WHERE ArtBas.StrTypeId = 0:
...
PAUSE 0.
FOR EACH VPIArtBas WHERE VPIArtBas.StrTypeId = 0:
...
DO:
  RUN vpiartbas_send_til_butikk.p (STRING(tmpVPIArtBas.EkstVPILEvNr) + ';|YES|NO|YES',ihBuffer,'',OUTPUT ocReturn, OUTPUT obOk).
END.
