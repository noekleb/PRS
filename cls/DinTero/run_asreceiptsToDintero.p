...
/*------------------------------------------------------------------------
    File        : run_asreceiptsToDintero.p
    Purpose     : Overfører loggede bonger til dintero.
...
ASSIGN
  ...
  bTest       = TRUE 
  cLogg       = 'receiptsToDintero' + REPLACE(STRING(TODAY),'/','')
...
DO:
  ...
  cReturn-Value = ''.
  RUN cls\Dintero\receiptsToDintero.p ON SERVER hServer.
