...
rCustomerDintero = NEW cls.DinTero.CustomerDintero().
...
CASE cEndPointCall:
  ...
  WHEN 'getGetSessionDetails' THEN RUN getGetSessionDetails.
  WHEN 'CustomerCollection' THEN RUN CustomerCollection.
  OTHERWISE 
  DO:
    obOk = FALSE.
...
PROCEDURE createSession:
  /*------------------------------------------------------------------------------
   Purpose:
   Notes: JSon reponse meldingen sendes tilbake i ocReturn entry 5.
  ------------------------------------------------------------------------------*/
  ...
  DO:
    rStandardFunksjoner:SkrivTilLogg(cLogg,
      '    Status: ' + STRING(iStatusCode) + ' ' + cStatusReason
      ).
    rStandardFunksjoner:SkrivTilLogg(cLogg,
      '    Entity: ' + STRING(rClientDinTero:cToken_Entity)
      ).
...
PROCEDURE getKOrdreSession:
  ...
  /* Henter datasett. */
  RUN cls\dintero\getdsOrdre.p(DEC(piKOrdre_Id), OUTPUT pbOk, OUTPUT DATASET dsOrder ).
  FIND FIRST ttOrder NO-ERROR.
  IF AVAILABLE ttOrder THEN 
    DATASET dsOrder:WRITE-JSON('file','konv\dsOrder' + STRING(ETIME) + '.json').
  ELSE DO:
    RETURN 'Fant ikke ordre ' + STRING(piKOrdre_Id) + '.'.
  END.
  ...
  FIND FIRST ttUrl NO-ERROR.
  IF AVAILABLE ttUrl THEN
  DO: 
    IF ttUrl.return_url <> '' THEN 
      poUrl:Add("return_url",ttUrl.return_url).
    IF ttUrl.callback_url <> '' THEN 
      poUrl:Add("callback_url",ttUrl.callback_url).
    IF ttUrl.merchant_terms_url <> '' THEN 
      poUrl:Add("merchant_terms_url",ttUrl.merchant_terms_url).
/*    poUrl:READ(TEMP-TABLE ttUrl:HANDLE).                 */
/*    CAST(poUrl, JSONObject):WriteFile('konv\pourl.json').*/
  END.
  ...
  DO:
    ...
    poOrder:ADD("vat_amount", ttOrder.vat_amount).
    poOrder:ADD("currency", ttOrder.currency).
    poOrder:ADD("merchant_reference", ttOrder.merchant_reference).
    poOrder:ADD("merchant_reference_2", ttOrder.merchant_reference_2).
    poOrder:ADD("partial_payment", ttOrder.partial_payment).
...
/*  poJson:Add("profile_id","T11112542.4XopPVLQtJfeUWbDHUPP8U"). /* Vipps */*/
  poJson:Add("profile_id","T11112542.4XopX7PCwvdZzWxu5anRmd"). /* Kort */  
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
    cParametre                   = ENTRY(4,icParam,'|')
    rClientDinTero:cFullPath     = 'https://api.dintero.com/checkouts/v1/sessions/' + ENTRY(2,cParametre,'=')
    rClientDinTero:cAccess_Token = ENTRY(2,icParam,'|')  
    rClientDinTero:cToken_Type   = ENTRY(3,icParam,'|')
    ocReturn                     = '||||'
    .
  ...
  rClientDinTero:getGetSessionDetails( INPUT  cParametre ,
    OUTPUT iStatusCode,
    OUTPUT cStatusReason,
    OUTPUT cJSonString,
    OUTPUT cJSonFileName,
