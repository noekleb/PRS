...
/*------------------------------------------------------------------------
    File        : run_assjekkDataMottak.p
    Purpose     : Kjører sjekk av datamottak via AppSErver.
...
DEFINE VARIABLE iOldPauseIntervall AS INTEGER NO-UNDO.
...
ASSIGN
    ...
    bTest       = TRUE 
    cLogg       = 'run_assjekkDataMottak' + REPLACE(STRING(TODAY),'/','')
...
DO:
  rStandardFunksjoner:SkrivTilLogg(cLogg,
      '  Inifil ikke opprettet (' + cConfigFile + ').' 
      ).    
  rStandardFunksjoner:SkrivTilLogg(cLogg,
      'Avslutter.' 
      ).    
...
DO:
  rStandardFunksjoner:SkrivTilLogg(cLogg,
      'Kjører sjekkDataMottak' 
      ).

  RUN cls\DataMottak\sjekkDataMottak.p.
