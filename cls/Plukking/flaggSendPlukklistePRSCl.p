...
/*------------------------------------------------------------------------
    File        : flaggSendPlukklistePRSCl.p
    Purpose     : 
...
rStandardFunksjoner:SkrivTilLogg(cLogg,
    '  flaggSendPlukklistePRSCl - Oppkoblet mot Appserver: ' + STRING(bConnected) + '.' 
    ).    

IF bConnected THEN 
DO: 
    RUN cls\Plukking\asflaggSendPlukklistePRS.p ON SERVER hServer (cLogg, INPUT-OUTPUT DATASET dsBxPickinglist ) NO-ERROR.
    ...
    DO:
        ...
        END.
        rStandardFunksjoner:SkrivTilLogg(cLogg,
            'flaggSendPlukklistePRSCl - Run: ' + cTekst 
            ).    
    END.
END.
ELSE obOk = FALSE.
