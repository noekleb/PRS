...
/*------------------------------------------------------------------------
    File        : run_asHentEDIFiler.p
    Purpose     : Kjører henting av EDI kommisjonsfiler via AppServer.
    ...
    Notes       : Ved å kjøre via AppServer, spares antall oppkoblede 
                  klienter på server.
                  TN 21/1-21 Satt opp for kjøring med brukerid Batch.
                    -U batch -P batch 
  ----------------------------------------------------------------------*/
...
ASSIGN
    ...
    bTest        = TRUE 
    cLogg        = 'run_asHentEDIFiler' + REPLACE(STRING(TODAY),'/','')
...
DO:
  ...
  /* Leser inn bongene. */
  RUN cls\Kommisjon\HentEDIFiler.p ON SERVER hServer.
