...
/*------------------------------------------------------------------------
    File        : abc_excelrapport2_run.p 
    Purpose     : 

    Syntax      :

    Description : Starter abc varegruppe rapport 1
...
DO iCount = 1 TO NUM-ENTRIES(SESSION:PARAMETER):
    ...
    IF ENTRY(iCount,SESSION:PARAMETER) BEGINS "OFFSET" AND 
        NUM-ENTRIES(ENTRY(iCount,SESSION:PARAMETER),"=") = 2 THEN 
    DO:
        ASSIGN iOffset = INT(ENTRY(2,ENTRY(iCount,SESSION:PARAMETER),"=")).
    END.
END.
/*
DEBUGGER:INITIATE().
...
ELSE RUN excelrapport2.p (INPUT '225|' + cRappNr + '|' + STRING(iOffSet),INPUT ?,INPUT ?, OUTPUT ocMelding,OUTPUT obOk) NO-ERROR.
