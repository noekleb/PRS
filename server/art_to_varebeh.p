/* Overføring av artikler til varehåndteringsbok 
   Parametere:   VarebehNr i parametersteng og temp-tabell med feltene Artikkelnr og Vg eller 
              eller
                 Liste over rowid's med artikler i parameterstreng:
                   <VarebehNr>,<"ROWID">,<Rowid1,Rowid2..>
              eller
                   Liste over artikkelnr i parameterstreng:
                   <VarebehNr>,<"ARTNR">,<Artnr1,Artnr2..>
...
DEF VAR iVarebehNr  AS DEC NO-UNDO.
...
iVarebehNr = DEC(ENTRY(1,icParam)).
...
FIND VarebehHode 
     WHERE VarebehHode.VarebehNr = iVarebehNr
     NO-LOCK NO-ERROR.
IF NOT AVAIL VarebehHode THEN DO:
  ocReturn = "Ugyldig VarebehNr: " + STRING(iVarebehNr).
  RETURN.
END.
...
REPEAT WHILE NOT hQuery:QUERY-OFF-END:
  ...
  IF AVAIL ArtBas 
     ...
     AND NOT ArtBas.Pant
     THEN 
      run newVarebehlinje.p (ArtBas.ArtikkelNr,iVarebehNr,OUTPUT rRowIdLinje) NO-ERROR.
