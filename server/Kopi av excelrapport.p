...
/*                                + ', first varemerke of artbas' */
  ...
  RUN fetchTTartbas.
   ...
   ASSIGN 
    bSortDesc         = LOGICAL(getParam('Filter','useSortDesc'))
    bSummary          = LOGICAL(getParam('Filter','useBreakBySummary'))
    bVisDelSummering  = LOGICAL(getParam('Filter','useDelSummering'))
    ...
    bSplittRapport   = LOGICAL(getParam('Filter','advSplittRapport'))
    bSendMail        = LOGICAL(getParam('Filter','advSendMail'))
    cTypeId          = getParam('Filter','stlinje.stTypeId')
    cPerId           = convText(getParam('Filter','stlinje.PerId'))
    cAarPerLinNr     = getParam('Filter','stlinje.aarPerLinNr')
    iFraAarPerLinNr  = INT(ENTRY(1,cAarPerLinNr)) 
...
/* expression.Open(FileName, UpdateLinks, ReadOnly, Format, Password, WriteResPassword, IgnoreReadOnlyRecommended, Origin, Delimiter, Editable, Notify, Converter, AddToMru, Local, CorruptLoad) */
    ...
    DO:
      IF iButikk = 99999998 THEN /*Move delsummer fliken et steg tilbake*/
    ...
    DO:
      ...
      FOR EACH ttBreakBy NO-LOCK WHERE ttBreakBy.iButikk = iButikk:
        chExcelApplication:ActiveSheet:Rows(ttBreakBy.iRowNum):FONT:BOLD = TRUE.
      END.
      
      /*For summary*/
      FIND LAST ttBreakBy WHERE ttBreakBy.iButikk = iButikk NO-LOCK NO-ERROR.
...
/*       chExcelApplication:ActiveSheet:VPageBreaks(1):Location = chExcelApplication:ActiveSheet:Range(cPrintAreaCol). */
    ...
    DO:
      ...
      chExcelApplication:ActiveSheet:Range('G:G'):NumberFormat = 'dd/mm/åååå;@'.
...
/*  cFileName = cFilePath + cReportName + '_' + REPLACE(STRING(TODAY,'99-99-99'),'-','') + '-' + REPLACE(STRING(TIME,'HH:MM:SS'),':','') + cFileExtent.  */
  cFileName = cFilePath + '_' + REPLACE(STRING(dFraDato,'99-99-9999'),'-','') + '-' + REPLACE(STRING(dTilDato,'99-99-9999'),'-','') + '_' + REPLACE(STRING(TODAY,'99-99-9999'),'-','') + REPLACE(STRING(TIME,'HH:MM'),':','') + cFileExtent.
...
/*  cFileName = cFilePath + cReportName + '_' + REPLACE(STRING(TODAY,'99-99-99'),'-','') + '_' + STRING(iSplittButikkNr) + '_' + REPLACE(STRING(TIME,'HH:MM:SS'),':','') + cFileExtent.  */
  cFileName = cFilePath + '_' + STRING(iSplittButikkNr) + '_' + REPLACE(STRING(dFraDato,'99-99-9999'),'-','') + '-' + REPLACE(STRING(dTilDato,'99-99-9999'),'-','') + '_' + REPLACE(STRING(TODAY,'99-99-9999'),'-','') + REPLACE(STRING(TIME,'HH:MM'),':','') + cFileExtent.
...
/*------------------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------------*/
  ...
  DO:
    ...
    CREATE BUFFER bhBreakPoint FOR TABLE qh:GET-BUFFER-HANDLE(1) NO-ERROR.
    IF ERROR-STATUS:ERROR THEN MESSAGE 'Her' qh:GET-BUFFER-HANDLE(1):BUFFER-FIELD('butik'):BUFFER-VALUE '**' STRING(rPrevRowId) '--' STRING(qh:GET-BUFFER-HANDLE(1):ROWID) VIEW-AS ALERT-BOX . 
...
/*       ttBreakBy.iButikk    = bhBreakPoint:BUFFER-FIELD('butik'):BUFFER-VALUE */
    NO-ERROR.
    IF ERROR-STATUS:ERROR THEN MESSAGE iButikk 'N/A' VIEW-AS ALERT-BOX.
...
/*       ttBreakBy.iButikk    = bhBreakPoint:BUFFER-FIELD('butik'):BUFFER-VALUE */
    ...
    IF ERROR-STATUS:ERROR THEN 
    do:
    MESSAGE program-name(3) skip iButikk 'Hg' skip 'Found find-by-row:' string(rPrevRowid)
      'Butikk:' iButikk skip iButikkNr bhBreakPoint:buffer-field('butik'):buffer-value skip
        VIEW-AS ALERT-BOX.
    end.
    IF bShowExtraInfo THEN
      PUT STREAM stA UNFORMATTED FILL({&Delim},1) ttBreakBy.iButikk.
...
/*       ttBreakBy.iButikk    = bhBreakPoint:BUFFER-FIELD('butik'):BUFFER-VALUE */
    ...
    NO-ERROR.
    IF ERROR-STATUS:ERROR THEN MESSAGE iButikkNr 'vg' 
    if bhBreakPoint:avail then bhBreakPoint:buffer-field('artikkelnr'):buffer-value else 'n/a'
    VIEW-AS ALERT-BOX.
...
/*       ttBreakBy.iButikk    = bhBreakPoint:BUFFER-FIELD('butik'):BUFFER-VALUE */
      ttBreakBy.cFieldName = 'Vg'
    NO-ERROR.
    IF ERROR-STATUS:ERROR THEN MESSAGE iButikk 'vg' VIEW-AS ALERT-BOX.
