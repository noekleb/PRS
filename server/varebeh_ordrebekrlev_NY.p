...
/*       (IF AVAIL Strekkode AND Strekkode.kode NE "" AND NOT Strekkode.kode BEGINS "02" THEN Strekkode.kode ELSE "") */
  ...
  ELSE DO ix = 1 TO NUM-ENTRIES(cStrList):
    ihBuffer:BUFFER-CREATE(). ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE =
       STRING(Butiker.Butik)         + "~t" + 
       Butiker.ButNamn       + "~t" +
      (IF VareBehLinje.LevKod NE ? THEN VareBehLinje.LevKod ELSE "") + "~t" +
      (IF VareBehLinje.Beskr NE ? THEN (IF VareBehLinje.Beskr BEGINS "+" THEN " " ELSE "") + VareBehLinje.Beskr ELSE "") + "~t" +
...
ihBuffer:BUFFER-CREATE(). 
ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "TOTALT" + FILL("~t",16) 
                + STRING(fGrandTot) + CHR(10) + CHR(10).


ihBuffer:BUFFER-CREATE(). ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "SUM pr uke:" + CHR(10).
FOR EACH ttTotUke BY ttTotUke.iUkeNr:
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "~t" + STRING(ttTotUke.iUkeNr) + FILL("~t",5) + STRING(ttTotUke.fTotUke) + CHR(10).
END.
ihBuffer:BUFFER-CREATE(). 
...
FOR EACH ttTotBut BY ttTotBut.cButNavn:
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "~t" + STRING(ttTotBut.cButNavn) +  FILL("~t",5) + STRING(ttTotBut.fTotBut) + CHR(10).
END.
ihBuffer:BUFFER-CREATE(). 
ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "TOTALT" + FILL("~t",6) 
                + STRING(fGrandTot) + CHR(10) + CHR(10).

ihBuffer:BUFFER-CREATE(). ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "SUM pr merknad:" + CHR(10).
FOR EACH ttTotMerk BY ttTotMerk.cMerknad:
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "~t" + STRING(ttTotMerk.cMerknad) +  FILL("~t",5) + STRING(ttTotMerk.fTotMerk) + CHR(10).
END.
ihBuffer:BUFFER-CREATE(). 
ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "TOTALT" + FILL("~t",6) 
                + STRING(fGrandTot) + CHR(10) + CHR(10).
