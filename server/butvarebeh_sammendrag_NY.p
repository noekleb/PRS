...
ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = 
   "Ordrerapport, vareh.bok: " + STRING(BestHode.VarebehNr) + CHR(10).

IF cButikkListe NE "*" THEN DO:    
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = 
     "Butikk(er): " + "~t" + cButikkListe             
                  + CHR(10).
END.
...
ihBuffer:BUFFER-CREATE(). 
ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = 
   CHR(10) + CHR(10) + "SUM pr avd:" + CHR(10).
FOR EACH ttTotAvd BY ttTotAvd.cAvdNavn:
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = 
     "~t" + STRING(ttTotAvd.cAvdNavn) +  "~t" + STRING(ttTotAvd.fTotAvd) +
          (IF bTotalSam THEN "~t~t" + STRING(ttTotAvd.fTotAvdKj) ELSE "") + CHR(10).
END.
ihBuffer:BUFFER-CREATE(). 
ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = 
   "TOTALT" + FILL("~t",2) 
            + STRING(fGrandTot) + (IF bTotalSam THEN "~t~t" + STRING(fGrandTotKj) ELSE "") + CHR(10) + CHR(10).

ihBuffer:BUFFER-CREATE(). 
ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = 
     "SUM pr hg" + (IF NOT bTotalSam THEN "/vg:" ELSE ":") + CHR(10).
FOR EACH ttTotHg BY ttTotHg.cHgNavn:
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = 
     "~t" + STRING(ttTotHg.cHgNavn) +  "~t" + STRING(ttTotHg.fTotHg) + CHR(10).
  IF NOT bTotalSam THEN DO:
    ...
    FOR EACH ttTotVg 
      ...
      ihBuffer:BUFFER-CREATE(). 
      ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = 
         (IF bFirstVg THEN "~t" ELSE "~t~t~t") + STRING(ttTotVg.cVgNavn) +  "~t" + STRING(ttTotVg.fTotVg) + CHR(10).
      bFirstVg = NO.
    END.
  END.
  ELSE PUT "~t~t" + STRING(ttTotHg.fTotHgKj) + CHR(10).
END.
...
FOR EACH ttTotUke BY ttTotUke.iUkeNr:
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = 
     "~t" + STRING(ttTotUke.iUkeNr) + "~t" + STRING(ttTotUke.fTotUke) +
          (IF bTotalSam THEN "~t~t" + STRING(ttTotUke.fTotUkeKj) ELSE "") + CHR(10).
...
FOR EACH ttTotLev BY ttTotLev.cLevNavn:
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = 
    "~t" + STRING(ttTotLev.cLevNavn) +  "~t" + STRING(ttTotLev.fTotLev) +
    (IF bTotalSam THEN "~t~t" + STRING(ttTotLev.fTotLevKj) ELSE "") + CHR(10).
END.
...
IF bTotalSam THEN DO:
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = 
 "SUM pr butikk:" + CHR(10).
  FOR EACH ttTotBut BY ttTotBut.cButNavn:
    ihBuffer:BUFFER-CREATE(). 
    ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = 
     "~t" + STRING(ttTotBut.cButNavn) +  "~t" + STRING(ttTotBut.fTotBut) +
        "~t~t" + STRING(ttTotBut.fTotButKj) + CHR(10).

  END.
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = 
    "TOTALT" + FILL("~t",2)
             + STRING(fGrandTot) + "~t~t" + STRING(fGrandTotKj) + CHR(10) + CHR(10).

  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = 
    "SUM kjedemargin: ~t~t" + STRING(fGrandTot - fGrandTotKj).
END.
