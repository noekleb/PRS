/* Endring av priser for kampanje 
   Parametere:   Kampanjeid i parametersteng og temp-tabell med feltene Artikkelnr og Vg eller 
              eller
                 Liste over rowid's med artikler i parameterstreng:
                   <KampanjeId>,<ROWID>,<AvslagType>,<Endring>,<Rowid1,Rowid2..>
              eller
                   Liste over artikkelnr i parameterstreng:
                   <KampanjeId>,<"ARTNR">,<AvslagType>,<Endring>,<Artnr1,Artnr2..>
...
DEF VAR iAvslagType AS INT NO-UNDO.
DEF VAR fEndring    AS DEC NO-UNDO.
...
ASSIGN iKampanjeId = INT(ENTRY(1,icParam))
       iAvslagType = INT(ENTRY(3,icParam))
       fEndring    = DEC(ENTRY(4,icParam))
       .
...
ELSE IF KampanjeHode.Aktivert THEN DO:
  ocReturn = "Kampanje: " + STRING(iKampanjeId) + " er aktivert. Priser kan ikke endres".
  RETURN.
END.
  ...
  REPEAT WHILE NOT hQuery:QUERY-OFF-END:
    ...
    IF AVAIL ArtBas AND ArtBas.LopNr NE ? THEN DO:
      ...
      ELSE DO:
        ...
        IF AVAIL KampanjeLinje THEN DO:
          IF iAvslagType = 1 THEN DO: /* Prosent */
            ASSIGN dDeci = ROUND(ArtPris.Pris[1] * (1 - ((fEndring * -1) / 100)),1).
            IF dDeci > 50 THEN
