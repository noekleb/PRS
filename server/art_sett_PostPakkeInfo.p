...
IF entry(1,cParaListe,'|') = "?" THEN
    RETURN.
IF NUM-ENTRIES(cParaListe,'|') < 4 THEN 
    RETURN.
...
DO:
  ...
  REPEAT WHILE NOT hQuery:QUERY-OFF-END TRANSACTION:
    ...
    IF AVAIL ArtBas THEN
      ASSIGN 
      ArtBas.PostLengde = INTEGER(entry(1,cParaListe,'|'))
      ArtBas.PostBredde = INTEGER(entry(2,cParaListe,'|'))
      ArtBas.PostHoyde  = INTEGER(entry(3,cParaListe,'|'))
      ArtBas.PostVekt   = DECIMAL(entry(4,cParaListe,'|'))
      .
