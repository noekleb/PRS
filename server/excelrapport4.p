...
DEF VAR iGeneralSysHid     AS INT INIT 229 NO-UNDO.
...
&IF DEFINED(EXCLUDE-getFormattedLevNr) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getFormattedLevNr Procedure 
FUNCTION getFormattedLevNr RETURNS CHARACTER
  ( INPUT ipiLevNr AS INT )  FORWARD.
...
&IF DEFINED(EXCLUDE-getLastDayInMonth) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getLastDayInMonth Procedure 
FUNCTION getLastDayInMonth RETURNS DATE
...
&IF DEFINED(EXCLUDE-GetAvdelingLagerVerdi) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE GetAvdelingLagerVerdi Procedure 
PROCEDURE GetAvdelingLagerVerdi :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    DEFINE INPUT PARAMETER ipiAvdelingNr AS INT NO-UNDO. 
    DEFINE OUTPUT PARAMETER opdLagerVerdi AS DEC NO-UNDO. 
    DEFINE VARIABLE dTmp AS DEC EXTENT 2 NO-UNDO. 
    
    DEFINE VARIABLE cAvdeling AS CHAR NO-UNDO. 
    DEFINE VARIABLE iYearMonth AS INT NO-UNDO. 
    cAvdeling  = getFormattedAvdeling(ipiAvdelingNr).
    iYearMonth = 0. 

    
    DEFINE BUFFER Avdeling FOR Avdeling. 
    DEFINE BUFFER stLager FOR stLager. 

    FIND FIRST  Avdeling WHERE 
                Avdeling.avdelingnr = ipiAvdelingnr NO-LOCK NO-ERROR.

    IF AVAIL Avdeling  THEN
    DO:          
        FIND stLager NO-LOCK WHERE 
             stLager.butik       = ipiButikkNr   AND
             stLager.sttypeid    = "AVDELING"  AND
             stLager.aarperlinnr = iYearMonth  AND
             stLager.dataobjekt  = cAvdeling NO-ERROR. 

        IF AVAIL stLager THEN
          opdLagerVerdi = stLager.vvarekost. 
    END.
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN 
      ...
      iToDate   = getDateCode(getLastDayInMonth(ipdDate)). 
      ...
      DO iCnt = iFromDate TO iToDate : 
        ...
        /* ------------------------------------------------------------------------------*/ 
         ...
         FOR EACH  stlinje NO-LOCK WHERE 
          ...
          /* akkum alle avdelinger */
          FIND FIRST SaleByDay  WHERE SaleByDay.dato = dDate NO-ERROR.
          ...
          ASSIGN 
              ...
              SaleByDay.MvaVerdi     = SaleByDay.mvaverdi + (stlinje.mvaverdi)
              ...
              SaleByDay.rabattant    = salebyday.rabattant + stlinje.antrabatt
              SaleByDay.rabattkr     = salebyday.rabattkr + stlinje.verdirabatt.
              SaleByDay.rabatt%      = (SaleByDay.rabattkr * 100) / salebyday.bruttosalg.
...
&IF DEFINED(EXCLUDE-getStatByMd) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE getStatByMd Procedure 
PROCEDURE getStatByMd :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    DEFINE OUTPUT PARAMETER opiAvdeling AS INT NO-UNDO.
    DEFINE OUTPUT PARAMETER opcAvdelingNavn AS CHAR NO-UNDO. 
    DEFINE OUTPUT PARAMETER opiRapportManed AS INT NO-UNDO. 
    DEFINE OUTPUT PARAMETER opiRapportAr AS INT NO-UNDO. 
    DEFINE OUTPUT PARAMETER opiAntallSolgt AS INT NO-UNDO. 
    DEFINE OUTPUT PARAMETER opdBruttoSalg AS DEC NO-UNDO. 
    DEFINE OUTPUT PARAMETER opdNettoSalg AS DEC NO-UNDO.
    DEFINE OUTPUT PARAMETER opdDBkr AS DEC NO-UNDO. 
    DEFINE OUTPUT PARAMETER opdDB% AS DEC NO-UNDO. 
    ...
    FOR EACH  stlinje NO-LOCK WHERE 
          ...
          ASSIGN 
            opiAvdeling     = INT(stlinje.dataobjekt)
            opcAvdelingNavn = IF AVAIL avdeling THEN avdeling.avdelingNavn ELSE ""
            opiRapportManed = INT(SUBSTRING(STRING(stlinje.aarperlinnr),5))
            opiRapportAr    = INT(SUBSTRING(STRING(stlinje.aarperlinnr),1,4))
            opiAntallSolgt  = stlinje.antsolgt
            opdBruttoSalg   = stlinje.mvaverdi + stlinje.verdiSolgt
            opdNettoSalg    = stlinje.verdisolgt
            opdDbKr         = stlinje.verdisolgt - stlinje.vvarekost
            opdDb%          = (opdDbKr * 100 )  / opdNettoSalg. 
...
&IF DEFINED(EXCLUDE-getStatByMdLeveran) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE getStatByMdLeveran Procedure 
PROCEDURE getStatByMdLeveran :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE iYearMonth AS INT NO-UNDO. 
    DEFINE VARIABLE cLevNr AS CHAR NO-UNDO. 
    ...
    FOR EACH  stlinje NO-LOCK WHERE 
              ...
              stlinje.perid       = "MANED"     AND 
              stlinje.aarperlinnr = iYearMonth  :
             
          FIND levbas WHERE 
               levbas.levnr = INT(stlinje.dataobjekt) NO-LOCK NO-ERROR. 
          

          FIND FIRST Rullerende12md_LeverandorAnalyse WHERE 
                     Rullerende12md_LeverandorAnalyse.leverandorid = levbas.levnr NO-ERROR. 
          IF NOT AVAIL Rullerende12md_LeverandorAnalyse THEN
              CREATE Rullerende12md_LeverandorAnalyse.

          ASSIGN 
            Rullerende12md_LeverandorAnalyse.leverandorid = INT(stlinje.dataobjekt)
            Rullerende12md_LeverandorAnalyse.leverandorbeskrivelse = IF AVAIL levbas THEN levbas.levnamn ELSE ""
           
            Rullerende12md_LeverandorAnalyse.antallsalg = 
              Rullerende12md_LeverandorAnalyse.antallsalg +
              stlinje.antsolgt
            Rullerende12md_LeverandorAnalyse.bruttosalg12md =
              Rullerende12md_LeverandorAnalyse.bruttosalg12md + 
              (stlinje.mvaverdi + stlinje.verdiSolgt)
            
            Rullerende12md_LeverandorAnalyse.lagerverdi = 
              Rullerende12md_LeverandorAnalyse.lagerverdi + 
              (stlinje.mvaverdi + stlinje.verdiSolgt)

            Rullerende12md_LeverandorAnalyse.dbkr12md = 
              Rullerende12md_LeverandorAnalyse.dbkr12md + 
              ( stlinje.verdisolgt - stlinje.vvarekost).
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN 
    ...
    cReportName    = IF cReportName = '' THEN 'Analyse Rapport' ELSE cReportName
  .

  /* hvis kjører batch - */
  IF gdFraDato = ? THEN gdFradato = TODAY. 
  gdReportDate   = gdFradato. 
 ...
 /*  showParam().    */
  ...
  addSend(cEmailAddr,'Analyse Rapport','',cReportFileList).
...
/*         .                                                                                              */
 ...
 /* tthParam:WRITE-XML('file','deleteme-now.xml').  */ 
...
------------------------------------------------------------------------------*/
    ...
    EMPTY TEMP-TABLE Akkummd_analyse NO-ERROR. 
    EMPTY TEMP-TABLE Rullerende12md_analyse NO-ERROR. 
    EMPTY TEMP-TABLE Rullerende12md_leverandoranalyse NO-ERROR. 
    EMPTY TEMP-TABLE Md_Analyse NO-ERROR. 


    DEFINE VARIABLE iMonth AS INT NO-UNDO. 
    DEFINE VARIABLE iYear AS INT NO-UNDO. 
    DEFINE VARIABLE iAvdeling AS INT EXTENT 5 NO-UNDO.
    DEFINE VARIABLE cAvdelingNavn AS CHAR EXTENT 5 NO-UNDO. 
    DEFINE VARIABLE iRapportManed AS INT EXTENT 5 NO-UNDO. 
    DEFINE VARIABLE iRapportAr AS INT EXTENT 5 NO-UNDO. 
    DEFINE VARIABLE iAntallSolgt AS INT EXTENT 5 NO-UNDO. 
    DEFINE VARIABLE dBruttoSalg AS DEC EXTENT 5 NO-UNDO. 
    DEFINE VARIABLE dNettoSalg AS DEC EXTENT 5 NO-UNDO.
    DEFINE VARIABLE dDBkr AS DEC EXTENT 5 NO-UNDO. 
    DEFINE VARIABLE dDB% AS DEC EXTENT 5 NO-UNDO. 
    DEFINE VARIABLE dLagerVerdi AS DEC NO-UNDO. 

    DO iCnt = 1 TO NUM-ENTRIES(ipcAvdelingList,"|"):
       dLastYear = DATE(MONTH(gdReportDate),1,YEAR(gdReportDate) - 1).
       
       RUN getStatByMd(  gdReportDate,
                         iButikkNr,
                         ENTRY(icnt,ipcAvdelingList,"|"),
                         OUTPUT iAvdeling[1],OUTPUT cAvdelingNavn[1],OUTPUT iRapportManed[1],OUTPUT iRapportAr[1],
                         OUTPUT iAntallSolgt[1],OUTPUT dBruttoSalg[1],OUTPUT dNettoSalg[1],OUTPUT ddbkr[1],
                         OUTPUT ddb%[1]).                                                              
       
       RUN getStatByMd(  dLastYear,
                         iButikkNr,
                         ENTRY(icnt,ipcAvdelingList,"|"),
                         OUTPUT iAvdeling[2],OUTPUT cAvdelingNavn[2],OUTPUT iRapportManed[2],OUTPUT iRapportAr[2],
                         OUTPUT iAntallSolgt[2],OUTPUT dBruttoSalg[2],OUTPUT dNettoSalg[2],OUTPUT ddbkr[2],
                         OUTPUT ddb%[2]).

      
       CREATE Md_Analyse. 
       ASSIGN
           md_analyse.AvdelingNavn = cAvdelingNavn[1] 
           md_analyse.Rapportmaned = iRapportManed[1]
           md_analyse.rapportar = irapportAr[1]
           md_analyse.nettosalg = dNettoSalg[1]
           md_analyse.nettosalgprev = dNettoSalg[2]
           md_analyse.db%  = dDb%[1]
           md_analyse.db%prev = dDB%[2].
       
       
       RUN getSalesThisMonth(gdReportDate,iButikkNr,ENTRY(icnt,ipcAvdelingList,"|")). 
       
       /* code for setting % basert på totaler, not nice :) */
       FOR EACH SaleByDay NO-LOCK: 
          dBruttoSalg[1] = dBruttoSalg[1] + saleByDay.BruttoSalg. 
       END.
       FOR EACH SaleByDay: 
           saleByDay.BruttoSalg% = (SaleByDay.bruttoSalg * 100) /(dBruttoSalg[1]).
       END.

      /* tab 2 salgsutviking pr kategori fra Januar - til aktive måned */
      /* leser data fra forrige år fra Jan og inneværende år */
        
       DO iMonth = 1 TO MONTH(gdReportDate):

           RUN getStatByMd( DATE(imonth,1,YEAR(gdReportDate)),
                            iButikkNr,
                            ENTRY(icnt,ipcAvdelingList,"|"),
                            OUTPUT iAvdeling[3],OUTPUT cAvdelingNavn[3],OUTPUT iRapportManed[3],OUTPUT iRapportAr[3],
                            OUTPUT iAntallSolgt[3],OUTPUT dBruttoSalg[3],OUTPUT dNettoSalg[3],OUTPUT ddbkr[3],
                            OUTPUT ddb%[3]).

           RUN getStatByMd( DATE(imonth,1,YEAR(gdReportDate) - 1),
                            iButikkNr,
                            ENTRY(icnt,ipcAvdelingList,"|"),
                            OUTPUT iAvdeling[4],OUTPUT cAvdelingNavn[4],OUTPUT iRapportManed[4],OUTPUT iRapportAr[4],
                            OUTPUT iAntallSolgt[4],OUTPUT dBruttoSalg[4],OUTPUT dNettoSalg[4],OUTPUT ddbkr[4],
                            OUTPUT ddb%[4]).

           IF iAvdeling[3] NE ? AND cAvdelingNavn[3] NE "" AND cAvdelingNavn[3] NE ? THEN
           DO:
             FIND FIRST AkkumMd_Analyse WHERE 
                        AkkumMd_Analyse.avdelingnr = iAvdeling[3] NO-ERROR.
             IF NOT AVAIL AkkumMd_Analyse THEN CREATE AkkumMd_Analyse. 
  
             ASSIGN
                 Akkummd_analyse.Avdelingnr = iAvdeling[3]
                 Akkummd_analyse.AvdelingNavn = cAvdelingNavn[3] 
                 Akkummd_analyse.Rapportmaned = iRapportManed[3]
                 Akkummd_analyse.rapportar = irapportAr[3]
                 Akkummd_analyse.nettosalg = Akkummd_analyse.nettosalg + dNettoSalg[3]
                 Akkummd_analyse.nettosalgprev = Akkummd_analyse.nettosalgprev + dNettoSalg[4]
                 Akkummd_analyse.dbkr = akkummd_analyse.dbkr + ddbkr[3]
                 Akkummd_analyse.db%  = Akkummd_analyse.dbkr * 100 / Akkummd_analyse.nettosalg
                 Akkummd_analyse.dbkrPrev = Akkummd_analyse.dbkrPrev + ddbKr[4]
                 Akkummd_analyse.db%prev = Akkummd_analyse.dbkrPrev * 100 / Akkummd_analyse.nettosalgPrev.
           END. 
       END.

       
       iAvdeling[1] = INT(ENTRY(icnt,ipcAvdelingList,"|")).
       RUN getAvdelingLagerVerdi(iButikkNr,iAvdeling[1], OUTPUT dLagerVerdi).
    
       /* Rullerende 12 md, tab 3 */
       DO  iCnt2 = 0 TO 11 : 
           iMonth = ((iCnt2 + MONTH(gdReportDate)) MOD 12 + 1).
           iYear  = YEAR(gdReportDate) + (IF (iCnt2 + MONTH(gdReportDate) + 1) GT 12 THEN 0 ELSE -1) .
    
           RUN getStatByMd( DATE(iMonth,1,iYear),
                           iButikkNr,
                           ENTRY(icnt,ipcAvdelingList,"|"),
                           OUTPUT iAvdeling[3],OUTPUT cAvdelingNavn[3],OUTPUT iRapportManed[3],OUTPUT iRapportAr[3],
                           OUTPUT iAntallSolgt[3],OUTPUT dBruttoSalg[3],OUTPUT dNettoSalg[3],OUTPUT ddbkr[3],
           ...
           IF iAvdeling[3] NE ? AND cAvdelingNavn[3] NE "" AND cAvdelingNavn[3] NE ? THEN
           DO:
             FIND FIRST   Rullerende12md_Analyse WHERE 
                          Rullerende12md_Analyse.avdelingnr = iAvdeling[3] NO-ERROR.
             IF NOT AVAIL Rullerende12md_Analyse THEN CREATE Rullerende12md_Analyse. 
  
             ASSIGN
                 Rullerende12md_Analyse.Avdelingnr   = iAvdeling[3]
                 Rullerende12md_Analyse.AvdelingNavn = cAvdelingNavn[3] 
                 Rullerende12md_Analyse.Rapportmaned = iRapportManed[3]
                 Rullerende12md_Analyse.rapportar    = irapportAr[3]
                 Rullerende12md_Analyse.nettosalg    = Rullerende12md_Analyse.nettosalg  + dNettoSalg[3]
                 Rullerende12md_Analyse.bruttosalg   = Rullerende12md_Analyse.bruttosalg + dbruttoSalg[3]
                 Rullerende12md_Analyse.dbkr         = Rullerende12md_Analyse.dbkr         + ddbkr[3]
                 Rullerende12md_Analyse.lagerverdi   =  dLagerVerdi
                 Rullerende12md_Analyse.lagerverdi2  =  dLagerVerdi
                 Rullerende12md_Analyse.omlopshastighetsmal = 2.
           END. 
       END.
       
    END.

    /* Levrandør Analyse tab sheet 4 */
    DO  iCnt2 = 0 TO 11 : 
         iMonth = ((iCnt2 + MONTH(gdReportDate)) MOD 12 + 1).
         iYear  = YEAR(gdReportDate) + (IF (iCnt2 + MONTH(gdReportDate) + 1) GT 12 THEN 0 ELSE -1) .
         RUN getStatByMdLeveran( DATE(iMonth,1,iYear),iButikkNr).
    END. 

    /* find lagerverdi for Lev */ 
    FOR EACH Rullerende12md_leverandoranalyse : 
         dLagerVerdi = 0. 
         FOR EACH ArtBas WHERE 
                  ArtBas.levnr  = Rullerende12md_leverandoranalyse.LeverandorId AND 
                  Artbas.lager NO-LOCK,
               EACH Lager OF ArtBas WHERE Lager.Butik = iButikkNr NO-LOCK :

             dLagerVerdi = dLagerVerdi + (lager.vvarekost * lager.lagant). 
         END. 
         Rullerende12md_leverandoranalyse.LagerVerdi = dLagerVerdi. 
    END. 
    ...
    opcReportFile = cFilePath + 'BID' + 
                    TRIM(STRING(butiker.butik,"9999"))  + 'Analyse' + 
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE cXLSTemplate AS CHAR INIT "server\excelrapport4Manalyse.template.xls" NO-UNDO.
  ...
  CREATE WIDGET-POOL "excel".
   ...
   RUN excelrapport4MAnalyse.p 
      (cXLSTemplate,
       ...
       ipdDate, 
       TABLE Md_Analyse,
       TABLE AkkumMd_Analyse,
       TABLE Rullerende12md_Analyse,
       TABLE Rullerende12md_LeverandorAnalyse,
       TABLE SaleByDay). 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION getFormattedLevNr Procedure 
FUNCTION getFormattedLevNr RETURNS CHARACTER
  ( INPUT ipiLevNr AS INT ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/

  RETURN STRING(ipiLevnr,"999999").   /* Function return value. */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION getLastDayInMonth Procedure 
FUNCTION getLastDayInMonth RETURNS DATE
  ( INPUT d AS DATE ) :
/*------------------------------------------------------------------------------
  Purpose:  
...
d2 = DATE(MONTH(d),28,YEAR(d)) + 5 .
d  = DATE(MONTH(d2),1,YEAR(d2)) - 1.
