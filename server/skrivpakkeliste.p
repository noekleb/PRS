...
/*------------------------------------------------------------------------
    File        : skrivpakkeliste.p
    Purpose     : Skriver ut en oversikt over det som ligger på ubehandlede
                  nettbutikk ordre.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH KOrdreHode NO-LOCK WHERE 
    KOrdreHode.ButikkNr = piButNr AND 
    ...
    IF KOrdreLinje.Varetekst = 'FRAKT' THEN 
      NEXT.  
    ASSIGN 
      pldummy = DEC(KOrdreLinje.VareNr) 
      NO-ERROR.
    ...
    ASSIGN
      ...
      ttPlukk.ArtikkelNr = DEC(KOrdreLinje.VareNr)
      ...
      ttPlukk.Storl      = KOrdreLinje.Storl
      ttPlukk.StrKode    = KOrdreLinje.StrKode
      ttPlukk.Antall     = KOrdreLinje.Antall
...
------------------------------------------------------------------------------*/
  ...
  cDato = "Dato: " + STRING(TODAY).
 
  ASSIGN 
    pcFilNavn =  cKatalog + "Pakkeliste" + "_" + REPLACE(STRING(TODAY),'/','') + '_' + REPLACE(STRING(TIME),':','') + ".pdf".
...
rSendEMail:parSUBJECT  = 'Pakkeliste for butikk: ' + STRING(Butiker.butik) + ' ' + Butiker.ButNamn + ' (Dato/Tid: ' + STRING(NOW,"99/99/9999 HH:MM:SS") + ').'.
rSendEMail:parMESSAGE  = "Pakkeliste: " + icFil + '.' + CHR(10) + 
                         "Listen inneholder varer som ligger på ubehandlede ordre.".
