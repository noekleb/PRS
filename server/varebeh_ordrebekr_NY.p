...
IF bVisButnr THEN DO:
  ...
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "Butikk:~t" + cButNr + "~t" + Butiker.ButNamn + CHR(10) + CHR(10).
END.

ihBuffer:BUFFER-CREATE(). 
ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = 
                (IF cLevNr NE "" THEN
                  "Butikknr"               + "~t" +
                  "Butikknavn"             + "~t" 
                 ELSE "") +
...
REPEAT WHILE NOT hQuery:QUERY-OFF-END:
  ...
  ELSE IF ENTRY(4,icParam) NE "" THEN DO: /* Leverandørsortert */
    IF VarebehLinje.levnr NE iPrevLevnr AND iPrevLevnr NE 0 THEN DO:
      ihBuffer:BUFFER-CREATE(). 
      ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "SUM Levnr" + "~t" + 
                      STRING(iPrevLevnr) + "~t" +  
                      cPrevLevNavn + FILL("~t",IF bEAN THEN 20 ELSE 19) 
                      + STRING(fTotLev) + CHR(10).
      ASSIGN fTotLev = 0
             iCount  = iCount + 1.
    END.
  END.
  ...
  IF NOT (bInndeling AND bDumpReport) THEN DO:
    ihBuffer:BUFFER-CREATE(). 
    ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE =
      (IF cLevNr NE "" THEN
  ...
  IF bInndeling THEN DO:
    IF NOT bDumpReport THEN DO:
      ihBuffer:BUFFER-CREATE(). 
      ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = (IF cLevNr NE "" THEN "~t~t" ELSE "") + "~t" +
                      "Størrelser:"            + "~t" +   
...
IF cButNr NE "" AND ENTRY(4,icParam) = "" THEN DO:
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "SUM H.gr" + "~t" + 
                  STRING(iPrevHg) + "~t" +  
                  cPrevHgBeskr + FILL("~t",IF bEAN THEN 20 ELSE 19) 
                  + STRING(fTotHg) + CHR(10).
...
ELSE IF ENTRY(4,icParam) NE "" THEN DO:
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "SUM Lev" + "~t" + 
                  STRING(iPrevLevnr) + "~t" +  
                  cPrevLevNavn + FILL("~t",IF bEAN THEN 20 ELSE 19) 
                  + STRING(fTotLev) + CHR(10) + CHR(10).
END.
ELSE IF cLevNr NE "" THEN DO:
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = FILL(CHR(10),3).     
END.

IF cButNr NE "" OR cLevNr NE "" THEN DO:    
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "TOTALT" + FILL("~t",IF cButNr NE "" THEN (IF bEAN THEN 22 ELSE 21) ELSE (IF bEAN THEN 24 ELSE 23)) 
                  + STRING(fGrandTot) + FILL(CHR(10),3).
END.
ELSE DO:
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = FILL(CHR(10),5). 
END. 

ihBuffer:BUFFER-CREATE(). 
ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "SUM pr uke:" + CHR(10).
FOR EACH ttTotUke BY ttTotUke.iUkeNr:
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = STRING(ttTotUke.iUkeNr) + FILL("~t",IF cButNr NE "" THEN 6 ELSE 8) + STRING(ttTotUke.fTotUke) + CHR(10).
END.
ihBuffer:BUFFER-CREATE(). 
ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "TOTALT" + FILL("~t",IF cButNr NE "" THEN 6 ELSE 8) 
                + STRING(fGrandTot) + FILL(CHR(10),3).

IF cLevNr = "" OR ENTRY(4,icParam) NE "" THEN DO:
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "SUM pr lev:" + CHR(10).
  FOR EACH ttTotLev BY ttTotLev.cLevNavn:
    ihBuffer:BUFFER-CREATE(). 
    ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "~t~t~t~t" + STRING(ttTotLev.cLevNavn) +  "~t~t" + STRING(ttTotLev.fTotLev) + CHR(10).
  END.
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "TOTALT" + FILL("~t",6) 
                  + STRING(fGrandTot) + FILL(CHR(10),3).
END.
IF cLevNr NE "" OR cButNr = "" THEN DO:
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "SUM pr butikk:" + CHR(10).
  FOR EACH ttTotBut BY ttTotBut.cButNavn:
    ihBuffer:BUFFER-CREATE(). 
    ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "~t" + STRING(ttTotBut.cButNavn) +  "~t~t~t~t~t~t~t" + STRING(ttTotBut.fTotBut) + CHR(10).
  END.
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "TOTALT" + FILL("~t",8) 
                  + STRING(fGrandTot) + FILL(CHR(10),3).
END.
IF ENTRY(4,icParam) NE "" THEN DO:
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "SUM pr avd:" + CHR(10).
  FOR EACH ttTotAvd BY ttTotAvd.cAvdNavn:
    ihBuffer:BUFFER-CREATE(). 
    ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "~t~t" + STRING(ttTotAvd.cAvdNavn) +  "~t~t~t~t" + STRING(ttTotAvd.fTotAvd) + CHR(10).
  END.
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "TOTALT" + FILL("~t",6) 
                  + STRING(fGrandTot) + FILL(CHR(10),3).
  ...
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "SUM pr hg:" + CHR(10).
  FOR EACH ttTotHg BY ttTotHg.cHgNavn:
    ihBuffer:BUFFER-CREATE(). 
    ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "~t~t" + STRING(ttTotHg.cHgNavn) +  "~t~t~t~t" + STRING(ttTotHg.fTotHg) + CHR(10).
  END.
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "TOTALT" + FILL("~t",6) 
                  + STRING(fGrandTot) + FILL(CHR(10),3).
END.
IF bKjedePris THEN DO:
  ihBuffer:BUFFER-CREATE(). 
  ihBuffer:BUFFER-FIELD("cRpLinje"):BUFFER-VALUE = "TOT.Kjede innpr" + FILL("~t",6) + STRING(fGrandTotKjede) + FILL(CHR(10),2) +
                  "TOT.Kjede margin" + FILL("~t",6) + STRING(fGrandTot - fGrandTotKjede)
      .
END.
