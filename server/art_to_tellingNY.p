...
REPEAT WHILE NOT hQuery:QUERY-OFF-END:
  FIND ArtBas
       ...
       NO-LOCK NO-ERROR.
  IF AVAIL ArtBas /*AND ArtBas.LopNr NE ? */ AND Artbas.Lager = TRUE AND NOT ArtBas.pakke AND NOT ArtBas.OPris THEN DO:
    ...
    DO ix = 1 TO NUM-ENTRIES(TelleHode.ButikkListe) TRANSACTION: /* vi har bara 1 butik */
        FIND Butiker WHERE Butiker.Butik = INT(ENTRY(ix,TelleHode.ButikkListe)) NO-LOCK NO-ERROR.
...
/*       FOR EACH StrekKode OF ArtBas WHERE StrekKode.KodeType = 1 AND StrekKode.StrKode  > 0:         */
...
/*               wNyFLagg              = TRUE. */
        ...
        IF wNedskriv THEN DO:
            FIND ArtPris NO-LOCK WHERE ArtPris.ArtikkelNr = ArtBas.ArtikkelNr AND
                                     ArtPris.ProfilNr   = clButiker.ProfilNr NO-ERROR.
            IF TelleLinje.NedSkrevet = 0 AND AVAILABLE ArtPris THEN DO:
                ASSIGN TelleLinje.VVareKost  = ArtPris.VareKost[IF ArtPris.Tilbud THEN 2 ELSE 1]
                       TelleLinje.NedSkrevet = ArtPris.VareKost[IF ArtPris.Tilbud THEN 2 ELSE 1].
          END.
        END.
