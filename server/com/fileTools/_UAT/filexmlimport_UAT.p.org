...
DO ON ERROR UNDO, LEAVE ON QUIT UNDO, LEAVE ON STOP UNDO, LEAVE:
    ...
    fileListDir  = NEW fileListDir(lok).

    fileXmlImport = NEW filexmlImport(,input-output hBuffer,?,output lok).


 /*       
        "C:\_cvs.test\_FtpClient_UAT\_Data\sendes","spool.*.xml",
        
        INPUT-OUTPUT hBuffer,?,OUTPUT lok).
  */
    IF NOT lok THEN message filexmlImport:getErrorMessage() VIEW-AS ALERT-BOX. 
END.
