...
ASSIGN
    cPersonNr    = IF 2014 - INT('19' + SUBSTRING(cPersonNr,1,2)) > 100
                     THEN "20" + cPersonNr
                   ELSE "19" + cPersonNr.
...
IF NOT CAN-DO(cLengdeLst,STRING(LENGTH(cOrgPersonNr))) THEN 
    DO:
        ...
        ASSIGN
            bOk      = FALSE
            cMelding = '** Feil lengde på medlemsid. Tillatte lengder er ' + cLengdeLst + '.'
            cStatus  = '0'
            .
        RETURN.    
    END.
...
DO: 
    ...
    DO:
        ASSIGN 
            bOK      = FALSE 
            cStatus  = "0"
            cMelding = "Ukjent lokalt medlemsid. " + cOrgPersonNr + ".".
        RETURN.
    END.
END. /* SJEKKPersonNr */
...
RUN bibl_loggDbFri.p ('asMedlemLogg', 'asmedlem.p: Stepp 6: ' 
    + 'MedemsId: '  + cOrgPersonNr + ' '
    + 'bOk: ' + STRING(bOk) + ' ' 
    + 'Status ' + cStatus + ' ' 
    + 'Melding ' + cMelding
...
/*     iAlder    = 16 */
  ...
  DO:
      ...
      bOk      = FALSE
      cStatus  = "0"
      cMelding = "Feil dato i personnummer " + cPersonNr + ".". 
      RETURN.
  END.
  
  IF (dFDato > dGDato) OR dFDato = ? THEN 
  DO:
      ASSIGN bOK      = TRUE 
             cStatus  = "0"
             cMelding = "Nye medlemmer må være " + STRING(iAlder) + " år eller mer.". 
  END.
  ELSE bOk = FALSE.
...
/*------------------------------------------------------------------------------
    ...
    FIND FIRST Medlem NO-LOCK WHERE 
        Medlem.MedlemsNr = DEC(cOrgPersonNr) NO-ERROR.
    ...
    ELSE 
        ...
        ASSIGN
            bOk      = FALSE
            cStatus  = '0'
            cMelding = 'Medlemsid ikke registrert lokalt.'
            .
...
DO:
    ...
    ELSE DO:
        ASSIGN bOK      = FALSE
               cStatus  = "0"
               cMelding = "Fikk kontakt med SPAR, men fant ikke personinformasjonen. " + CHR(10) + cErrormessage + CHR(10) + 
...
ELSE DO:
    ...
    ASSIGN bOK          = FALSE
           cStatus      = "0"
           cMelding     = "Ikke kontakt med SPAR. " + cOrgPersonNr + "."
           /*
