...
&Scoped-Define ENABLED-OBJECTS rsAlle fiVg B-SokVg Btn_OK Btn_Cancel ~
fi-cMessage fi-cNumTotal fi-cNumSelected 
&Scoped-Define DISPLAYED-OBJECTS rsAlle fiVg fi-cMessage fi-cNumTotal ~
fi-cNumSelected 
...
DEFINE VARIABLE fiVg AS INTEGER FORMAT "zzzzz9" INITIAL 0 
     LABEL "Varegruppe" 
     VIEW-AS FILL-IN 
     SIZE 12.6 BY 1.
...
DEFINE FRAME Dialog-Frame
     ...
     fiVg AT ROW 3.86 COL 67 COLON-ALIGNED HELP
          "'varegruppenummer"
     B-SokVg AT ROW 3.86 COL 82
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Endre varegruppe */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-SokVg Dialog-Frame
ON CHOOSE OF B-SokVg IN FRAME Dialog-Frame /* ... */
OR "MOUSE-SELECT-DBLCLICK":U OF B-SokVg
OR "F10" OF B-SokVg
DO:
  ...
  RUN JBoxDLookup.w ("VarGr;Vg;VgBeskr;Hg", "where Vg > 0", INPUT-OUTPUT cLookupValue).

  IF cLookupValue NE "" THEN DO: 
    fiVg:SCREEN-VALUE = cLookupValue.
    APPLY "return" TO fiVg.
  END.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY rsAlle fiVg fi-cMessage fi-cNumTotal fi-cNumSelected 
      WITH FRAME Dialog-Frame.
  ENABLE rsAlle fiVg B-SokVg Btn_OK Btn_Cancel fi-cMessage fi-cNumTotal 
         fi-cNumSelected 
      WITH FRAME Dialog-Frame.
