...
DEF VAR cErrorfildir AS CHAR NO-UNDO. 

DEF VAR iNumEntries  AS INTE INIT 13 NO-UNDO.
...
DEFINE TEMP-TABLE TT_Input NO-UNDO
    FIELD Avtkund      AS CHAR
    FIELD Artnr        AS CHAR
    FIELD Levart       AS CHAR
    FIELD Benamn       AS CHAR
    FIELD Ean          AS CHAR
    FIELD Avtpris      AS CHAR
    FIELD Utpris       AS CHAR
    FIELD Nyavtpr      AS CHAR
    FIELD Datum        AS CHAR
    FIELD Lev          AS CHAR
    FIELD Frp          AS CHAR
    FIELD Inh          AS CHAR
    FIELD Artstat      AS CHAR
...
ASSIGN
    ...
    cErrorfildir = VPIFilHode.Katalog
    .
...
&IF DEFINED(EXCLUDE-ExportFeil) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE ExportFeil Procedure 
PROCEDURE ExportFeil :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    cFil = cErrorfildir + "\" + "ERROR_" + VPIFilHode.FilNavn + SUBSTR(STRING(YEAR(TODAY)),3) + STRING(MONTH(TODAY),"99") + STRING(DAY(TODAY),"99") + STRING(TIME,"99999") + ".xls".
    OUTPUT TO VALUE(cFil).
    FOR EACH TT_Input WHERE TT_Input.feil = TRUE:
        PUT UNFORMATTED FeilTxt CHR(9)
                        Avtkund CHR(9)
                        Artnr   CHR(9)
                        Levart  CHR(9)
                        Benamn  CHR(9)
...
/* 
DEFINE TEMP-TABLE TT_Input NO-UNDO
    FIELD Avtkund Anbnr        AS CHAR
    FIELD Artnr   Vgp          AS CHAR
    FIELD Levart  Artnr        AS CHAR
    FIELD Benamn  Chk          AS CHAR
    FIELD Ean     Lev          AS CHAR
    FIELD Avtpris Levart       AS CHAR
    FIELD Utpris  Benamning    AS CHAR
    FIELD Nyavtpr Frp          AS CHAR
    FIELD Datum   Ean          AS CHAR
    FIELD Lev     Varuklasskod AS CHAR
    FIELD Frp     AAFpris      AS CHAR
    FIELD Inh     Utpris       AS CHAR
    FIELD Artstat Dummy1       AS CHAR
    FIELD Inpris       AS CHAR
    FIELD Artikkelnr   AS CHAR
    FIELD Beskr        AS CHAR
    FIELD Moms%        AS CHAR
...
/*         VIEW-AS ALERT-BOX INFO BUTTONS OK. */
    ...
    FOR EACH TT_Input WHERE TT_Input.Feil = FALSE:
        
        ASSIGN cRigalstr = cRigalEntries
               ENTRY(3,cRigalstr)   = TT_Input.Artikkelnr 
/*                ENTRY(4,cRigalstr)   = TT_Input.Levart */
               ENTRY(6,cRigalstr)   = "400"
               ENTRY(7,cRigalstr)   = TT_Input.Artnr
               ...
               ENTRY(18,cRigalstr)  = TT_Input.Vg
               ...
               ENTRY(79,cRigalstr)  = "1"
               ENTRY(92,cRigalstr)  = "0"
...
/*         RUN flyttprofiler.p ("cFlyttDir","cFlyttKommando",cUtfil) NO-ERROR. */
    ...
    IF CAN-FIND(FIRST TT_Input WHERE TT_Input.feil = TRUE) THEN
        RUN ExportFeil.
/*                                                                                                                                                                                                                                                                                                                                */
...
&IF DEFINED(EXCLUDE-FinnFeil) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE FinnFeil Procedure 
PROCEDURE FinnFeil :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
 ...
 */
 FOR EACH TT_Input:
     dTest = DECI(TT_Input.Artnr) NO-ERROR.
     IF ERROR-STATUS:ERROR OR dTest = 0 THEN DO:
         ASSIGN TT_Input.Feil    = TRUE
                TT_Input.FeilTxt = "Artikelnr: " + TT_Input.ArtNr.
         NEXT.
     END.
     dTest = DECI(TT_Input.Ean) NO-ERROR.
     IF ERROR-STATUS:ERROR OR dTest = 0 OR LENGTH(TT_Input.Ean) > 13 THEN DO:
         ASSIGN TT_Input.Feil    = TRUE
                TT_Input.FeilTxt = "Ean: " + TT_Input.Ean.
         NEXT.
     END.
     dTest = DECI(TT_Input.Avtpris) NO-ERROR.
     IF ERROR-STATUS:ERROR OR dTest = 0 THEN DO:
         ASSIGN TT_Input.Feil    = TRUE
                TT_Input.FeilTxt = "Avtalspris: " + TT_Input.Avtpris.
         NEXT.
     END.
     dTest = DECI(TT_Input.Utpris) NO-ERROR.
     IF ERROR-STATUS:ERROR OR dTest = 0 THEN DO:
         ASSIGN TT_Input.Feil    = TRUE
                TT_Input.FeilTxt = "Utpris: " + TT_Input.Utpris.
         NEXT.
     END.
     dTest = DECI(TT_Input.Frp) NO-ERROR.
     IF ERROR-STATUS:ERROR OR dTest = 0 THEN DO:
         ASSIGN TT_Input.Feil    = TRUE
                TT_Input.FeilTxt = "Frp: " + TT_Input.Frp.
         NEXT.
     END.
     FIND Artbas WHERE artbas.levkod = TT_Input.artnr NO-LOCK NO-ERROR.
     IF NOT AVAIL Artbas THEN DO:
         ASSIGN TT_Input.Feil    = TRUE
                TT_Input.FeilTxt = "Artikelnr: " + "Saknas".
         NEXT.
     END.
     FIND Strekkode WHERE strekkode.kode = TT_Input.Ean NO-LOCK NO-ERROR.
     IF AVAIL Strekkode AND Strekkode.Artikkelnr <> Artbas.Artikkelnr THEN DO:
         ASSIGN TT_Input.Feil    = TRUE
                TT_Input.FeilTxt = "Ean: " + "Annan artikel".
         NEXT.
     END.
     FIND FIRST Artpris OF Artbas NO-LOCK NO-ERROR.
     IF NOT AVAIL Artpris THEN DO:
         ASSIGN TT_Input.Feil    = TRUE
                TT_Input.FeilTxt = "Artpris: " + "Saknas i PRS".
         NEXT.
     END.
     ASSIGN TT_Input.Artikkelnr = STRING(Artbas.artikkelnr)
            TT_Input.Beskr      = Artbas.beskr
            TT_Input.Moms%      = STRING(Artpris.Mva%[1])
            TT_Input.Vg         = STRING(Artbas.vg)
            TT_Input.Levnr      = STRING(Artbas.levnr)
            TT_Input.Inpris     = REPLACE(STRING(ROUND(DECI(TT_Input.Inpris) / DECI(TT_Input.Frp),2)),",",".").
 END.
