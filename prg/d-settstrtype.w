...
&Scoped-Define ENABLED-OBJECTS rsAlle fiStrTypeID B-SokStrTypeID Btn_OK ~
Btn_Cancel fi-cMessage fi-cNumTotal fi-cNumSelected 
&Scoped-Define DISPLAYED-OBJECTS rsAlle fiStrTypeID fi-cMessage ~
...
DEFINE VARIABLE fiStrTypeID AS INTEGER FORMAT "zzzzz9" INITIAL ? 
...
DEFINE FRAME Dialog-Frame
     ...
     fiStrTypeID AT ROW 3.86 COL 67 COLON-ALIGNED HELP
          "Størrelsestype"
     B-SokStrTypeID AT ROW 3.86 COL 82
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Endre størrelsestype */
DO:
...
&Scoped-define SELF-NAME B-SokStrTypeID
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-SokStrTypeID Dialog-Frame
ON CHOOSE OF B-SokStrTypeID IN FRAME Dialog-Frame /* ... */
OR "MOUSE-SELECT-DBLCLICK":U OF B-SokStrTypeId
OR "F10" OF B-SokStrtypeId
DO:
  DEF VAR cLookupValue AS CHAR NO-UNDO.

  cLookupValue = "StrTypeId".

  RUN JBoxDLookup.w ("StrType;StrTypeId;Beskrivelse;AlfaFordeling;Intervall", "where StrTypeId > 1", INPUT-OUTPUT cLookupValue).

  IF cLookupValue NE "" THEN DO: 
    fiStrTypeId:SCREEN-VALUE = cLookupValue.
    APPLY "return" TO fiStrTypeId.
  END.
END.
...
DO:
  ASSIGN fiStrTypeId rsAlle.
  ASSIGN cValues = (IF fiStrTypeId <> ?
                      THEN STRING(fiStrTypeId)
                      ELSE "?")
         oiReturn = rsAlle
         .
END.
...
------------------------------------------------------------------------------*/
  DISPLAY rsAlle fiStrTypeID fi-cMessage fi-cNumTotal fi-cNumSelected 
      WITH FRAME Dialog-Frame.
  ENABLE rsAlle fiStrTypeID B-SokStrTypeID Btn_OK Btn_Cancel fi-cMessage 
         fi-cNumTotal fi-cNumSelected 
      WITH FRAME Dialog-Frame.
