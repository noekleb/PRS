...
/*------------------------------------------------------------------------
    File        : hitrate.p
    Purpose     : Eksport av hitrate fra butikk til hk.
...
DO: 
  RUN SettFraTilDato.
  RUN bibl_logg.p ('eksporter_hitrate', 'eksporthitrate.p: AUTO WinCheduler Periode: ' + string(dinpFraDato) + ' - ' + string(dinpTilDato) + ' ' + string(TIME,"HH:MM:SS")).
END.
ELSE DO:
  ASSIGN 
  ...
  RUN bibl_logg.p ('eksporter_hitrate', 'eksporthitrate.p: MANUELL Periode: ' + string(dinpFraDato) + ' - ' + string(dinpTilDato) + ' ' + string(TIME,"HH:MM:SS")).
END.
...
ASSIGN
    ...
    cKopi       = cKatalog + "\Imas"
    cFilNavn    = cKatalog + '\Imas\' + 'POSH' + STRING(YEAR(TODAY),"9999") + STRING(MONTH(TODAY),"99") + STRING(DAY(TODAY),"99") + REPLACE(STRING(TIME,"HH:MM"),':','') + '.'.
    ctmpFilNavn = cKatalog + '\Imas\' + 'tmpPOSH' + STRING(YEAR(TODAY),"9999") + STRING(MONTH(TODAY),"99") + STRING(DAY(TODAY),"99") + REPLACE(STRING(TIME,"HH:MM"),':','') + '.'.
