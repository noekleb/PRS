...
DEFINE TEMP-TABLE TT_StLinje NO-UNDO LIKE StLinje
       ...
       FIELD Jamforenhet as CHAR
       FIELD ProdNr as INTE
       FIELD Prodbeskr AS CHAR
       FIELD Grunnsortiment AS CHAR.
...
DEFINE VARIABLE corgLabel  AS CHARACTER  NO-UNDO. /* för ean-knapp om den har översatts */
...
ASSIGN cFieldDefs = 
        ...
        /* 44 */ "VMId;VareMerke;;1," +
        /* 44 */ "VMBeskr;VMBeskr;;," +
        /* 45 */ "Jamforenhet;jmfenhet;;1," +
        /* 45 */ "ProdNr;Prodnr;;1," +
        /* 45 */ "Prodbeskr;Prodbeskr;;," +
        /* 46 */ "Grunnsortiment;Grunnsortiment;;"
        .
...
CB-OPris B-EANsok TG-Grunnsortiment TG-AvFilter B-Refresh B-Avdeling ~
...
Tg-VisPerBut CB-OPris FI-Streckkod TG-Grunnsortiment TG-AvFilter FILL-IN-1 
...
DEFINE VARIABLE FI-Streckkod AS CHARACTER FORMAT "X(13)":U 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE TG-Grunnsortiment AS LOGICAL INITIAL no 
...
DEFINE FRAME fMain
     ...
     CB-OPris AT ROW 5.24 COL 34.8 COLON-ALIGNED
     FI-Streckkod AT ROW 5.24 COL 49.8 COLON-ALIGNED NO-LABEL
     B-EANsok AT ROW 5.24 COL 73 NO-TAB-STOP 
     TG-Grunnsortiment AT ROW 5.38 COL 90
     ...
     FILL-IN-1 AT ROW 1.24 COL 114.2 COLON-ALIGNED NO-LABEL
...
/* SETTINGS FOR FILL-IN FI-Sesong IN FRAME fMain
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN FI-Streckkod IN FRAME fMain
   NO-ENABLE                                                            */
...
&Scoped-define SELF-NAME B-EANsok
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-EANsok fFrameWin
ON CHOOSE OF B-EANsok IN FRAME fMain /* EAN */
DO:
    IF B-EANsok:LABEL <> "AVBRYT" THEN DO:
        cOrgLabel = B-EANsok:LABEL.
        B-EANsok:LABEL = "AVBRYT".
/*         FI-Streckkod:SENSITIVE = TRUE. */
        ...
        APPLY "ENTRY" TO FI-Streckkod.
        RUN EnaDisFilter ("1").
        APPLY "ENTRY" TO FI-Streckkod.
    END.
    ELSE DO:
        B-EANsok:LABEL = cOrgLabel.
/*         FI-Streckkod:SENSITIVE = TRUE. */
        EMPTY TEMP-TABLE TT_Art.
        RUN EnaDisFilter ("2").
        FI-Streckkod:SCREEN-VALUE = "".
        APPLY "ENTRY" TO B-Aktiver.
    END.
  RETURN NO-APPLY.
END.
...
&Scoped-define SELF-NAME FI-Streckkod
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FI-Streckkod fFrameWin
ON ANY-PRINTABLE OF FI-Streckkod IN FRAME fMain
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FI-Streckkod fFrameWin
ON RETURN OF FI-Streckkod IN FRAME fMain
DO:
  FIND Strekkode WHERE strekkode.kode = (IF LENGTH(SELF:SCREEN-VALUE) = 12 THEN "0" ELSE "") + SELF:SCREEN-VALUE NO-LOCK NO-ERROR.
  IF AVAIL strekkode THEN DO:
      FIND TT_Art WHERE TT_Art.artikkelnr = strekkode.artikkelnr NO-ERROR.
      IF NOT AVAIL TT_Art THEN DO:
          CREATE TT_Art.
          ASSIGN TT_Art.artikkelnr = strekkode.artikkelnr.
          RELEASE TT_Art.
      END.
      IF CAN-FIND(FIRST TT_art) THEN
          RUN StartSokArtDyn(?,FALSE,"HENTINTERNT" + CHR(1) + "0").
  END.
/*   SELF:SCREEN-VALUE = "". */
  APPLY "ENTRY" TO FI-Streckkod.
END.
...
&Scoped-define SELF-NAME RS-Level
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL RS-Level fFrameWin
ON VALUE-CHANGED OF RS-Level IN FRAME fMain
DO:
  IF SELF:SCREEN-VALUE <> "1" THEN DO:
      IF FI-Streckkod:SENSITIVE THEN DO:
          APPLY "CHOOSE" TO B-EANsok.
      END.
      TG-Grunnsortiment:CHECKED = FALSE.
  END.
  B-EANsok:SENSITIVE = SELF:SCREEN-VALUE = "1".
  TG-Grunnsortiment:SENSITIVE = SELF:SCREEN-VALUE = "1".
END.
...
&Scoped-define SELF-NAME TG-Grunnsortiment
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL TG-Grunnsortiment fFrameWin
ON VALUE-CHANGED OF TG-Grunnsortiment IN FRAME fMain /* Grunnsortiment */
DO:
  IF TG-AvFilter:CHECKED THEN
      SELF:CHECKED = FALSE.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY Tg-VisPeriode FI-Avdeling FI-Sesong FI-HuvGr FI-Farg RS-Level FI-VarGr 
          FI-VMId FI-LevNr FI-Material Tg-VisPerBut CB-OPris FI-Streckkod 
          TG-Grunnsortiment TG-AvFilter FILL-IN-1 
      WITH FRAME fMain.
  ENABLE Tg-VisPeriode B-AvdelingBlank B-VMId B-SesongBlank B-Aktiver B-HgBlank 
         ...
         B-MaterialBlank Tg-VisPerBut CB-OPris B-EANsok TG-Grunnsortiment 
         TG-AvFilter B-Refresh B-Avdeling B-LevNr B-HuvGr B-VarGr B-Material 
         B-Farg B-Sesong FILL-IN-1 
      WITH FRAME fMain.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE EnaDisFilter fFrameWin 
PROCEDURE EnaDisFilter :
/*------------------------------------------------------------------------------
  Purpose:SENSITIVE = cType = "2"     
  Parameters:SENSITIVE = cType = "2"  <none>
  Notes:SENSITIVE = cType = "2"       
------------------------------------------------------------------------------*/
   DEFINE INPUT  PARAMETER cType AS CHARACTER   NO-UNDO.
   DO WITH FRAME {&FRAME-NAME}:
       ASSIGN B-Avdeling:SENSITIVE      = cType = "2"
              B-Farg:SENSITIVE          = cType = "2"
              B-HuvGr:SENSITIVE         = cType = "2"
              B-LevNr:SENSITIVE         = cType = "2"
              B-Material:SENSITIVE      = cType = "2"
              B-Sesong:SENSITIVE        = cType = "2"
              B-VarGr:SENSITIVE         = cType = "2"
              B-VMId:SENSITIVE          = cType = "2"
              B-AvdelingBlank:SENSITIVE = cType = "2"
              B-FargBlank:SENSITIVE     = cType = "2"
              B-HgBlank:SENSITIVE       = cType = "2"
              B-LevNrBlank:SENSITIVE    = cType = "2"
              B-MaterialBlank:SENSITIVE = cType = "2"
              B-SesongBlank:SENSITIVE   = cType = "2"
              B-VgBlank:SENSITIVE       = cType = "2"
              B-VMIdBlank:SENSITIVE     = cType = "2"
              FI-Streckkod:SENSITIVE    = cType = "1"
              B-Aktiver:SENSITIVE       = cType = "2"
              B-Refresh:SENSITIVE       = cType = "2"
              CB-OPris:SENSITIVE        = cType = "2"
              TG-AvFilter:SENSITIVE     = cType = "2".
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN cField#List = getSumFelter(cFeltListe).
         ...
         PUT UNFORMATTED "cField#List" cField#List SKIP
                         "cFeltListe " cFeltListe  SKIP
                         "cFelter    " cFelter     SKIP.
         OUTPUT CLOSE.
         
  DO iCount = 1 TO NUM-ENTRIES(cField#List):
/*       IF NOT  THEN */
/*       MESSAGE ENTRY(INT(ENTRY(iCount,cField#List)),cRightCols) SKIP */
/*               cField#List SKIP                                      */
/*           cRightCols                                                */
/*           VIEW-AS ALERT-BOX INFO BUTTONS OK.                        */
      IF INT(ENTRY(iCount,cField#List)) = 0 THEN
          NEXT.
              ...
              FOR EACH StLinje WHERE Stlinje.Butik       = iButik    AND 
                  ...
                  DO:
                      ...
                      IF NOT AVAIL TT_Stlinje THEN DO:
                          ...
                          ELSE DO:
                              ASSIGN TT_StLinje.VgLopNr = STRING(ArtBas.Vg) + "/" + REPLACE(STRING(ArtBas.LopNr,"999999"),"0","  ")
...
------------------------------------------------------------------------------*/
  ...
  CASE ENTRY(1,cKriterier):
    ...
    WHEN "DAG" THEN DO:
      ASSIGN 
        cFraAar      = STRING(YEAR(DATE(ENTRY(2,cKriterier)))).
        cTilAar      = STRING(YEAR(DATE(ENTRY(3,cKriterier)))).
        cFraPerLinNr = STRING(DATE(ENTRY(2,cKriterier)) - DATE(12,31,YEAR(DATE(ENTRY(2,cKriterier))) - 1)).
        cTilPerLinNr = STRING(DATE(ENTRY(3,cKriterier)) - DATE(12,31,YEAR(DATE(ENTRY(3,cKriterier))) - 1)).
    END.
    ...
    DO iCount = 1 TO NUM-ENTRIES(cButiker):
      FOR EACH StLinje WHERE Stlinje.Butik    = INT(ENTRY(iCount,cButiker)) AND
          ...
          IF TRIM(CB-Opris:SCREEN-VALUE) = "" THEN DO:
              ...
              IF TG-Grunnsortiment:CHECKED = TRUE THEN DO:
                  IF NOT AVAIL Artbas OR ArtBas.Grunnsortiment = FALSE THEN
                      NEXT.
              END.
          END.
          ELSE DO:
            ...
            IF TG-Grunnsortiment:CHECKED = TRUE AND ArtBas.Grunnsortiment = FALSE THEN
                NEXT.
          END.
...
/*                 NEXT.                */
        ...
        DO:
          ...
          END.
          FIND TT_StLinje WHERE 
              TT_StLinje.Butik      = TT_StLinjeTMP.Butik      AND /* allt på en butik = 0 */
              TT_StLinje.StTypeId   = TT_StLinjeTMP.StTypeId   AND
              TT_StLinje.PerId      = TT_StLinjeTMP.PerId      AND
              TT_StLinje.AarPerLinNr = TT_StLinjeTMP.AarPerLinNr AND
              TT_StLinje.DataObjekt = TT_StLinjeTMP.DataObjekt USE-INDEX AarPerLinNr NO-ERROR.
          IF NOT AVAIL TT_Stlinje THEN DO:
            IF INPUT Tg-VisPeriode = TRUE THEN
              ASSIGN
                dDato = IF TT_StLinjeTMP.PerId = "DAG" THEN DATE(1,1,TT_StLinjeTMP.Aar) + TT_StLinjeTMP.PerLinNr - 1 ELSE ?
            ...
            ELSE DO:
             ...
             ASSIGN 
               ...
               TT_StLinje.VMId = ArtBas.VMId
               TT_StLinje.VMBeskr = IF AVAIL Varemerke THEN Varemerke.Beskrivelse ELSE  "Ukjent"
               TT_StLinje.Jamforenhet = Artbas.Jamforenhet
               TT_StLinje.ProdNr      = ArtBas.ProdNr
               TT_StLinje.Prodbeskr   = IF AVAIL Produsent THEN Produsent.Beskrivelse ELSE ""
               TT_StLinje.Grunnsortiment = IF RS-Level:SCREEN-VALUE = "1" AND AVAIL ArtBas AND Artbas.grunnsortiment = TRUE THEN "J" ELSE "".
            END.
            IF TG-VisPerBut:CHECKED THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE lLocalqh AS LOGICAL     NO-UNDO.
  EMPTY TEMP-TABLE TT_StLinje.
  IF qh = ? THEN DO:
      IF CAN-FIND(FIRST TT_Art) THEN DO:
          CREATE QUERY qh.
          hBuffer = TEMP-TABLE TT_Art:DEFAULT-BUFFER-HANDLE.
          qh:SET-BUFFERS(hBuffer).
          qh:QUERY-PREPARE("FOR EACH TT_Art").
          lLocalqh = TRUE.
      END.
      ELSE RETURN.
  END.
                      ...
                      IF NOT AVAIL TT_Stlinje THEN DO:
                          ...
                          IF NOT AVAIL ArtBas THEN
                              ASSIGN TT_StLinje.VgLopNr = "Ukjent"
                                     ...
                                     TT_StLinje.VMBeskr = "Ukjent".
                          ELSE DO:
                              ASSIGN TT_StLinje.VgLopNr = STRING(ArtBas.Vg) + "/ " + STRING(ArtBas.LopNr,"999999")
                                     TT_StLinje.LevKod = ArtBas.LevKod
                                     TT_StLinje.Beskrivelse = ArtBas.Beskr
                                     TT_StLinje.LevNr = ArtBas.LevNr 
                                     TT_StLinje.Levnamn = IF AVAIL LevBas THEN LevBas.LevNamn ELSE "Ukjent"
...
/*   MESSAGE "GS" cFeltNumListe SKIP        */
/*           "GS" cFeltnavnListe SKIP       */
/*           "GS" cFelter                   */
/*       VIEW-AS ALERT-BOX INFO BUTTONS OK. */
  DO iCount = 1 TO NUM-ENTRIES(cFeltnavnListe):
/*       IF LOOKUP(ENTRY(iCount,cFeltnavnListe),cFelter) = 0 THEN */
/*           NEXT.                                                */
/*       IF LOOKUP(ENTRY(iCount,cFeltnavnListe),cFelter) = 0 THEN DO: */
/*           MESSAGE "B" ENTRY(iCount,cFeltnavnListe)                 */
/*               VIEW-AS ALERT-BOX INFO BUTTONS OK.                   */
/*           NEXT.                                                    */
/*       END.                                                         */
