...
/*------------------------------------------------------------------------
    File        : xPRSMaterialInnles.p
    Purpose     : Innlesning av data til materialkoderegisteret

    Syntax      : xPRSMaterialInnles.p (lFilId, h_Parent, output iantLinjer).
...
DEFINE BUFFER bMaterial FOR Material.
DEFINE TEMP-TABLE ttMaterial LIKE Material
  INDEX MaterialDefNr MatKod.
...
DEFINE VARIABLE b2Ok AS LOG NO-UNDO.
  ...
  RUN TellOppLinjer.
  RUN bibl_logg.p ('PRSMaterialImport', 'xPRSMaterialInnles.p: Leser inn fil: ' + VPIFilHode.FilNavn + ' Antall linjer: ' + STRING(iTotAntLinjer) + ' ' + string(TIME,"HH:MM:SS")).
  ...
  REPEAT:
    ...
    RUN AssignInt(1,OUTPUT ttMaterial.MatKod, OUTPUT bOk).
    IF bOk = FALSE THEN b2Ok = FALSE.

    /*
    RUN AssignDec(17,OUTPUT ttMaterial.Rab1%, OUTPUT bOk).
    IF bOk = FALSE THEN b2Ok = FALSE.
    ...
    */
    ...
    ASSIGN 
      ttMaterial.MatBeskr = ENTRY(2,pcLinje,';')
    NO-ERROR.                
...
PROCEDURE PosterData:
  ...
  FOR EACH ttMaterial TRANSACTION:
    FIND Material EXCLUSIVE-LOCK WHERE 
      Material.MatKod = ttMaterial.MatKod NO-ERROR.
    IF NOT AVAILABLE Material THEN 
    DO:
      CREATE Material.
      ASSIGN
        Material.MatKod = ttMaterial.MatKod.
    END.

    IF TRIM(ENTRY(2,pcLinje,';')) <> '' THEN Material.MatBeskr =  ttMaterial.MatBeskr.          
