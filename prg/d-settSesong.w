...
&Scoped-Define ENABLED-OBJECTS rsAlle fiSesongNr B-SokSesongNr Btn_OK ~
Btn_Cancel fi-cMessage fi-cNumTotal fi-cNumSelected 
&Scoped-Define DISPLAYED-OBJECTS rsAlle fiSesongNr fi-cMessage fi-cNumTotal ~
fi-cNumSelected 
...
DEFINE VARIABLE fiSesongNr AS INTEGER FORMAT ">>>>>9" INITIAL ? 
     LABEL "Sesong" 
     VIEW-AS FILL-IN 
     SIZE 12.6 BY 1.
...
DEFINE FRAME Dialog-Frame
     ...
     fiSesongNr AT ROW 3.86 COL 67 COLON-ALIGNED HELP
          "Produsent"
     B-SokSesongNr AT ROW 3.86 COL 82
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Endre sesong */
DO:
...
&Scoped-define SELF-NAME B-SokSesongNr
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-SokSesongNr Dialog-Frame
ON CHOOSE OF B-SokSesongNr IN FRAME Dialog-Frame /* ... */
OR "MOUSE-SELECT-DBLCLICK":U OF B-SokSesongNr
OR "F10" OF B-SokSesongNr
DO:
  DEF VAR cLookupValue AS CHAR NO-UNDO.

  cLookupValue = "Sasong".

  RUN JBoxDLookup.w ("Sasong;SasBeskr;Sasong", "where true", INPUT-OUTPUT cLookupValue).
  IF cLookupValue NE "" THEN DO: 
    fiSesongNr:SCREEN-VALUE = cLookupValue.
    APPLY "return" TO fiSesongNr.
  END.
END.
...
DO:
  ASSIGN fiSesongNr rsAlle.
  ASSIGN cValues = (IF fiSesongNr <> ?
                      THEN STRING(fiSesongNr)
                      ELSE "?")
         oiReturn = rsAlle
         .
END.
...
------------------------------------------------------------------------------*/
  DISPLAY rsAlle fiSesongNr fi-cMessage fi-cNumTotal fi-cNumSelected 
      WITH FRAME Dialog-Frame.
  ENABLE rsAlle fiSesongNr B-SokSesongNr Btn_OK Btn_Cancel fi-cMessage 
