...
IF SEARCH(ipFileName) = ? THEN
    RETURN "ERROR".
ASSIGN cTmpFile = "C:\tmp\tmpLinje.txt".
INPUT FROM VALUE(ipFileName).

REPEAT:
    IMPORT UNFORMATTED cFilRad.
/*     MESSAGE PROGRAM-NAME(1) SKIP           */
/*     "- " cAktTabell SKIP cFilRad           */
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH TT_ArtBas:
        ...
        IF NOT AVAIL ArtBas THEN DO:
            CREATE ArtBas.
            BUFFER-COPY TT_ArtBas EXCEPT Lopnr TO ArtBas NO-ERROR.
...
/*         EXPORT TT_StrKonv.          */
    ...
    END.
    FOR EACH TT_StrType:
        FIND StrType OF TT_StrType NO-LOCK NO-ERROR.
