...
/*        cLabels = "Kasserer,Beskrivelse,Periode,Solgt,Verdi solgt,Solgt%,Mva verdi,DbKr,Db%,Rabatter,Rabatt kr,VVarekost,Kunderekl,Kunderekl kr,Levrekl,Levrekl kr,Svinn,Svinn kr,Gjenkjøp,Gjenkjøp kr,Tilbud,Tilbud kr,Brekkasje,Brekkasje kr"                       */
...
&Scoped-Define ENABLED-OBJECTS Tg-VisPeriode B-Avdeling B-ForsaljBlank ~
RS-SumLevel B-Aktiver B-AvdelingBlank B-HgBlank B-VgBlank Tg-VisPerBut ~
B-Forsalj B-HuvGr B-VarGr 
&Scoped-Define DISPLAYED-OBJECTS Tg-VisPeriode FI-Forsalj RS-SumLevel ~
...
DEFINE BUTTON B-Forsalj  NO-FOCUS
     LABEL "..." 
     SIZE 4.4 BY 1.

DEFINE BUTTON B-ForsaljBlank 
     LABEL "Blank" 
     SIZE 8 BY 1.
...
DEFINE VARIABLE FI-Forsalj AS CHARACTER FORMAT "X(10)":U 
     LABEL "Kasserer" 
     VIEW-AS FILL-IN 
     SIZE 14 BY 1 NO-UNDO.
...
DEFINE FRAME fMain
     ...
     FI-Forsalj AT ROW 1.19 COL 34.8 COLON-ALIGNED
     B-ForsaljBlank AT ROW 1.19 COL 56.8
     ...
     B-Forsalj AT ROW 1.19 COL 51.8 NO-TAB-STOP 
...
/* SETTINGS FOR FILL-IN FI-Avdeling IN FRAME fMain
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN FI-Forsalj IN FRAME fMain
   NO-ENABLE                                                            */
...
DO:
  ...
  IF NOT DYNAMIC-FUNCTION('getKriterier':U IN h_fstperiode,
     OUTPUT cKriterier /* CHARACTER */) THEN
      RETURN.
  IF FI-Forsalj <> "*" THEN
      ASSIGN pcFeltListe = "ForsNr"
             pcVerdier   = FI-Forsalj.
  /* Om vi har avd,hg,vg filter och vi inte avgrænsat på Lev lægger vi in lev før enklare hantering i SDO */
  IF FI-Avdeling <> "*" OR FI-HuvGr <> "*" OR FI-VarGr <> "*" THEN
    ASSIGN pcFeltListe = pcFeltListe + (IF pcFeltListe <> "" THEN "," ELSE "ForsNr,") + IF FI-Avdeling <> "*" THEN "AvdelingNr" ELSE IF FI-HuvGr <> "*" THEN "Hg" ELSE "Vg"
  ...
  cTilleggsFelter = "Butik,Butnamn,Forsnr,FoNamn,AvdelingNr,AvdelingNavn,Hg,HgBeskr,Vg,VgBeskr".
  ...
  IF RS-SumLevel:SCREEN-VALUE <> "" THEN DO:
    ASSIGN iEntry          = 4
           ...
           ENTRY(1,cFieldDefs) = "Forsnr;Kasserenr;;1,FoNamn;Kasserer;;".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-Forsalj fFrameWin
ON CHOOSE OF B-Forsalj IN FRAME fMain /* ... */
DO:
    DEFINE VARIABLE cRowIdList AS CHARACTER  NO-UNDO.
    DEFINE VARIABLE cIdList    AS CHARACTER  NO-UNDO.
    DEFINE VARIABLE bOK               AS LOGICAL    NO-UNDO.
    ...
    IF NUM-ENTRIES(SELF:PRIVATE-DATA,CHR(1)) = 2 THEN
        ASSIGN cRowIdList = ENTRY(1,SELF:PRIVATE-DATA,CHR(1))
               cIdList    = ENTRY(2,SELF:PRIVATE-DATA,CHR(1)).
    RUN JBoxDSelector.w (THIS-PROCEDURE,0,
                        "Forsalj;ForsNr;FoNamn;ForsaljAktiv",
                        "WHERE TRUE",
                        INPUT-OUTPUT cRowIdList,
                        "ForsNr",
                        INPUT-OUTPUT cIdList,
    ...
    IF bOK THEN DO:
        assign
          FI-Forsalj:SCREEN-VALUE = if cIdList = ""
                            then cAlle
                          else "( " + STRING(NUM-ENTRIES(cIdList,"|")) + " )"
          FI-Forsalj     = if cIdList = ""
                            then "*"
                            else REPLACE(cIdList,"|",",")
          FI-Forsalj:TOOLTIP = IF FI-Forsalj = "*" THEN "" ELSE FI-Forsalj.
        IF FI-Forsalj <> "*" THEN
            ASSIGN SELF:PRIVATE-DATA = cRowIdList + CHR(1) + cIdList
                   FI-Forsalj:BGCOLOR = 11.
        ELSE
            ASSIGN SELF:PRIVATE-DATA = ""
                   FI-Forsalj:BGCOLOR = ?.
     END.
...
&Scoped-define SELF-NAME B-ForsaljBlank
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-ForsaljBlank fFrameWin
ON CHOOSE OF B-ForsaljBlank IN FRAME fMain /* Blank */
DO:
    IF FI-Forsalj:SCREEN-VALUE <> cAlle THEN DO:
        ASSIGN FI-Forsalj:SCREEN-VALUE = cAlle
               FI-Forsalj              = "*"
               FI-Forsalj:TOOLTIP      = ""
               FI-Forsalj:BGCOLOR      = ?
               B-Forsalj:PRIVATE-DATA  = "".
    END.
    RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY Tg-VisPeriode FI-Forsalj RS-SumLevel FI-Avdeling FI-HuvGr FI-VarGr 
          Tg-VisPerBut 
      WITH FRAME fMain.
  ENABLE Tg-VisPeriode B-Avdeling B-ForsaljBlank RS-SumLevel B-Aktiver 
         B-AvdelingBlank B-HgBlank B-VgBlank Tg-VisPerBut B-Forsalj B-HuvGr 
         B-VarGr 
      WITH FRAME fMain.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ASSIGN cStTypeId = "SELGERSTAT"
             cFilename = SESSION:TEMP-DIRECTORY + "gridstlinje.txt"
             FI-Forsalj  = "*"
             FI-Forsalj:SCREEN-VALUE = cAlle
             FI-Avdeling  = "*"
...
------------------------------------------------------------------------------*/
  ...
  CASE ENTRY(1,ipKriterier):
      ...
      END.
  END CASE.
  ASSIGN cStTypeId = IF RS-SumLevel:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "" THEN "SELGERSTAT" ELSE 
                     IF RS-SumLevel:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "Artikkel" THEN "KASS-ART" ELSE "KASS-VG"
         cStTypeId = IF cStTypeId = "SELGERSTAT" AND (FI-Avdeling <> "*" OR FI-HuvGr <> "*" OR FI-VarGr <> "*") THEN "KASS-VG" ELSE cStTypeId
         cQryString = 
