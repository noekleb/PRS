...
------------------------------------------------------------------------------*/
  ...
  REPEAT:
    ...
    IMPORT STREAM InnFil 
        tmpPDA NO-ERROR.
    /* TN 9/1-09 Blanke linjer gir error, men skal ikke rapporteres.    
    ...
    DO:
        ...
        ASSIGN
          ...
          tt_Error.Tekst   = "** Feil på linjeimport " + STRING(iAntLinjer).
    ...
    */
    ...
    ASSIGN
        cTekst = trim(string(tmpPDA.EAN)).
    IF LENGTH(cTekst) > 6 AND length(cTekst) < 13 THEN
    ...
    DO TRANSACTION:
       ...
       ASSIGN
           VPIFilLinje.StorTekst  = trim(string(tmpPDA.Butnr)) + ";" +      
                                    ...
                                    cUkjentEAN
          NO-ERROR.
       IF ERROR-STATUS:ERROR OR VPIFilLinje.StorTekst = ? THEN
       DO:
           /* TN 9/1-09 Feil her kommer på blanke linjer.
           ASSIGN
             piAntFeil = piAntFeil + 1.
           CREATE tt_Error.
           ASSIGN
             tt_Error.LinjeNr = piAntFeil
             tt_Error.Tekst   = "** Feil på linje (Ved assign Stortekst på fillinje) " + STRING(iAntLinjer).
           */
           IF AVAILABLE tmpPDA THEN DELETE tmpPDA.
           if available VPIFilLinje then Delete VPIFilLinje.
           NEXT LESERLINJER.
       END.
