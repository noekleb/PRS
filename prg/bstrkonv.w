...
&Scoped-define FIELDS-IN-QUERY-br_table StrKode Storl SeqNr fBrukt Merknad 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table StrKode Storl SeqNr Merknad 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getBrowseLabels bTableWin 
FUNCTION getBrowseLabels RETURNS CHARACTER
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table bTableWin _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      StrKode FORMAT "z999":U
      Storl FORMAT "X(10)":U WIDTH 18
      SeqNr FORMAT "->>>>>>9":U
      fBrukt FORMAT "J/N":U
      Merknad FORMAT "X(60)":U WIDTH 70.6
  ENABLE
      StrKode HELP "?"
      Storl HELP "?"
      SeqNr
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN NO-AUTO-VALIDATE NO-ROW-MARKERS SEPARATORS SIZE 104 BY 14.76 FIT-LAST-COLUMN.
...
ASSIGN 
       rowObject.StrKode:COLUMN-READ-ONLY IN BROWSE br_table = TRUE
       rowObject.Storl:COLUMN-READ-ONLY IN BROWSE br_table = TRUE
       rowObject.SeqNr:COLUMN-READ-ONLY IN BROWSE br_table = TRUE
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[1]   > _<SDO>.rowObject.StrKode
"StrKode" ? ? "integer" ? ? ? ? ? ? yes "?" no no ? yes no yes "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > _<SDO>.rowObject.Storl
"Storl" ? ? "character" ? ? ? ? ? ? yes "?" no no "18" yes no yes "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > _<SDO>.rowObject.SeqNr
"SeqNr" ? ? "integer" ? ? ? ? ? ? yes ? no no ? yes no yes "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > _<SDO>.rowObject.fBrukt
"fBrukt" ? ? "logical" ? ? ? ? ? ? no "?" no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > _<SDO>.rowObject.Merknad
"Merknad" ? ? "character" ? ? ? ? ? ? yes "?" no no "70.6" yes no yes "U" "" "" "" "" "" "" 0 no 0 no no
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION getBrowseLabels bTableWin 
FUNCTION getBrowseLabels RETURNS CHARACTER
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN hColumn = BROWSE {&BROWSE-NAME}:FIRST-COLUMN.
  REPEAT WHILE VALID-HANDLE(hColumn):
      ASSIGN cLabels = cLabels + (IF cLabels = "" THEN "" ELSE ",") + hColumn:LABEL.
      ASSIGN hColumn = hColumn:NEXT-COLUMN.
  END.
  RETURN cLabels.   /* Function return value. */
