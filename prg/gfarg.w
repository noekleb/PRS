...
DEFINE VARIABLE h_bfarg AS HANDLE NO-UNDO.
DEFINE VARIABLE h_dfarg AS HANDLE NO-UNDO.
...
DEFINE FRAME gDialog
     ...
     Btn_Cancel AT ROW 13.86 COL 40
     SPACE(0.39) SKIP(0.13)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste fargeregister"
...
ON GO OF FRAME gDialog /* Søkeliste fargeregister */
DO:
     ASSIGN    
          cReturn-Value = "OK"    
          .
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL gDialog gDialog
ON WINDOW-CLOSE OF FRAME gDialog /* Søkeliste fargeregister */
DO:  
...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             INPUT  'sdo/dfarg.wDB-AWARE':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'AppServiceASInfoASUsePrompt?CacheDuration0CheckCurrentChangedyesDestroyStatelessnoDisconnectAppServernoServerOperatingModeNONEShareDatanoUpdateFromSourcenoForeignFieldsObjectNamedfargOpenOnInityesPromptColumns(NONE)PromptOnDeleteyesRowsToBatch200RebuildOnReposnoToggleDataTargetsyes':U ,
             OUTPUT h_dfarg ).
       RUN repositionObject IN h_dfarg ( 13.14 , 18.00 ) NO-ERROR.
       ...
       RUN constructObject (
             ...
             OUTPUT h_bfarg ).
       RUN repositionObject IN h_bfarg ( 2.91 , 1.00 ) NO-ERROR.
       RUN resizeObject IN h_bfarg ( 10.71 , 54.00 ) NO-ERROR.
       ...
       /* Links to SmartDataBrowser h_bfarg. */
       RUN addLink ( h_dfarg , 'Data':U , h_bfarg ).
       RUN addLink ( h_bfarg , 'Update':U , h_dfarg ).

       /* Links to SmartObject h_sortsok. */
       RUN addLink ( h_bfarg , 'Sortera':U , h_sortsok ).

       /* Adjust the tab order of the smart objects. */
       RUN adjustTabOrder ( h_bfarg ,
...
------------------------------------------------------------------------------*/
  ...
  IF cReturn-Value <> "AVBRYT" THEN  
   ASSIGN
       cColValues = DYNAMIC-FUNCTION('colValues':U IN h_dfarg,
                      INPUT "Farg,FarBeskr" /* CHARACTER */).  
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  PUBLISH "Sortera" FROM h_bfarg.

  DYNAMIC-FUNCTION('findRowWhere':U IN h_dfarg,
     INPUT cFelt    + (IF cFelt = "" THEN "" ELSE ",") + "Farg",
     INPUT cVerdier + (IF cVerdier = "" THEN "" ELSE CHR(1)) + cStart,
     INPUT "EQ,EQ" /* CHARACTER */).
  /* Signalerer til de andre objektene. */
  RUN dataAvailable IN h_dfarg
