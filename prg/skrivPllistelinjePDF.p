...
DEFINE TEMP-TABLE TT_LevSum NO-UNDO
    ...
    FIELD lsum    AS DECI
    FIELD iantrader AS INTE
    INDEX levnr IS PRIMARY UNIQUE levnr.
DEFINE TEMP-TABLE tt_rader NO-UNDO
    FIELD radnr AS INTE
    FIELD levnr AS INTE
    FIELD tmpdata AS CHAR EXTENT 13
    INDEX radnr IS PRIMARY UNIQUE levnr radnr.
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE iLagant LIKE ArtLag.LagAnt  NO-UNDO.
    IF cType = "O" THEN
...
/*             cTmpData[13] = TRIM(hBuffer:BUFFER-FIELD("Levnamn"):BUFFER-VALUE()).                      */
      ...
      CREATE tt_rader.
      ASSIGN tt_rader.radnr = iRadnr
             tt_rader.levnr = LevBas.Levnr
             tt_rader.tmpdata[ 1] = cTmpData[ 1]
             tt_rader.tmpdata[ 2] = cTmpData[ 2]
             tt_rader.tmpdata[ 3] = cTmpData[ 3]
             tt_rader.tmpdata[ 4] = cTmpData[ 4]
             tt_rader.tmpdata[ 5] = cTmpData[ 5]
             tt_rader.tmpdata[ 6] = cTmpData[ 6]
             tt_rader.tmpdata[ 7] = cTmpData[ 7]
             tt_rader.tmpdata[ 8] = cTmpData[ 8]
             tt_rader.tmpdata[ 9] = cTmpData[ 9]
             tt_rader.tmpdata[10] = cTmpData[10]
             tt_rader.tmpdata[11] = cTmpData[11]
             tt_rader.tmpdata[12] = cTmpData[12]
             tt_rader.tmpdata[13] = cTmpData[13]
          .
      ...
      ASSIGN TT_LevSum.levant    = TT_LevSum.levant + iAntall
             ...
             TT_LevSum.iantrader = TT_LevSum.iantrader + 1.
...
/*   RUN pdf_new_page IN h_PDFinc ("Spdf"). */
  ...
  IF CAN-FIND(FIRST TT_LevSum) THEN DO:
      ...
      FOR EACH TT_LevSum BREAK BY tt_levsum.levnr:
          ...
          i2 = i2 + 1.
          RUN pdf_skip    IN h_PDFinc ("Spdf").
          IF i2 MOD 47 = 0 AND NOT LAST (tt_levsum.levnr) THEN DO:
              RUN pdf_new_page IN h_PDFinc ("Spdf").
          END.
      END.
      ...
      FOR EACH TT_LevSum BREAK BY tt_levsum.levnr:
          ...
          RUN pdf_skip    IN h_PDFinc ("Spdf").
          RUN pdf_set_font IN h_PDFinc ("Spdf", "Helvetica",7).
          ASSIGN iRapprad = iRapprad + 1.
          FOR EACH tt_rader WHERE tt_rader.levnr = tt_levsum.levnr:
              DO ii = 1 TO iAntFelt:
                  IF iCols[ii] <> 0 THEN
                      RUN pdf_text_at IN h_PDFinc ("Spdf",TRIM(tt_rader.TmpData[ii]),iCols[ii]).
                  ELSE
                      RUN pdf_text_to IN h_PDFinc ("Spdf",TRIM(tt_rader.TmpData[ii]),iToRight[ii]).
              END.
              RUN pdf_skip    IN h_PDFinc ("Spdf").
              ASSIGN iRapprad = iRapprad + 1.
          END.
          ...
          RUN pdf_text_to IN h_PDFinc ("Spdf",STRING(tt_levsum.Levant),iToRight[10]).
          RUN pdf_text_to IN h_PDFinc ("Spdf",STRING(tt_levsum.lsum,">>>>>>9.99"),iToRight[11]).
          RUN pdf_set_font IN h_PDFinc ("Spdf", "Helvetica",7).
          IF NOT LAST (tt_levsum.levnr) THEN DO:
              RUN pdf_skip    IN h_PDFinc ("Spdf").
              ASSIGN iRapprad = iRapprad + 1.
          END.
      END.
...
&IF DEFINED(EXCLUDE-SkrivPDForg) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE SkrivPDForg Procedure 
PROCEDURE SkrivPDForg :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE ii AS INTEGER     NO-UNDO.
  DEFINE VARIABLE I2 AS INTEGER     NO-UNDO.
  DEFINE VARIABLE iLevSumAntall AS INTEGER FORMAT '->>,>>>,>>9.99'    NO-UNDO.
  DEFINE VARIABLE dLevSumTOT    AS DECIMAL FORMAT '->>,>>>,>>9.99'    NO-UNDO.
