...
------------------------------------------------------------------------------*/
  ...
  OUTPUT TO VALUE(cFilnavn).
    ...
    FOR EACH tt_Error BY tt_Error.linjenr:
...
------------------------------------------------------------------------------*/
  ...
  REPEAT:
    ...
    ASSIGN /* Dato og tid fra fil: '2012-11-4 12:28:00.000' */
        ...
        .
    /* Produkter fra Ryns gamle strekkoder */
/*     IF pcEAN = '' AND cProduktNr > '' THEN */
    IF cProduktNr > '' THEN
    RYNS:
    DO:
        IF NUM-ENTRIES(cProduktNr,'/') = 2 THEN
        DO:
            cVGlopnr = ENTRY(1,cProduktnr,"/").
            ASSIGN
                iLopnr   = INT(SUBSTR(cVGlopnr,LENGTH(cVGlopnr) - 2))
                iVg      = INT(SUBSTR(cVGlopnr,1,LENGTH(cVGlopnr) - 3))
/*                 iVg      = INT(SUBSTRING(STRING(INT(ENTRY(1,cProduktNr,'/')),"99999"),1,2)) */
/*                 iLopNr   = INT(SUBSTRING(STRING(INT(ENTRY(1,cProduktNr,'/')),"99999"),3,3)) */
                pcStr    = TRIM(ENTRY(2,cProduktNr,'/'))
                NO-ERROR.
             IF ERROR-STATUS:ERROR THEN 
             DO:
                 RUN bibl_logg.p ('ImportAvVaretelling', 'xbxwuinnles.p - Vg/LopNr tolkning feiler på linje: ' + STRING(iAntLinjer) +  ' Record: ' + pcLinje).
                 NEXT LESERLINJER.
             END.
             RUN FixStorl IN h_dproclib (INPUT-OUTPUT pcStr). /* Størrelse i standard SkoTex */
        ...
        DO:
            ASSIGN       
                iVg      = INT(SUBSTRING(cProduktNr,1,3)) 
                iLopNr   = INT(SUBSTRING(cProduktNr,4,4)) 
                pcStr    = LEFT-TRIM(SUBSTRING(cProduktNr,9,3),"0") 
                pcStr    = pcStr + (IF INT(SUBSTRING(cProduktNr,12,1)) > 0 
                                      THEN '.' + SUBSTRING(cProduktNr,12,1) 
                                      ELSE ''
                                    )
                .
            RUN FixStorl IN h_dproclib (INPUT-OUTPUT pcStr). /* Størrelse i standard SkoTex */
    ...
    /* Sjekker butikk - tidigare entry 10 */
    ASSIGN iButikkNr = INT(ENTRY(5,pcLinje,";")) NO-ERROR.
    ...
    DO:
        ...
        ASSIGN
          tt_Error.LinjeNr = piAntFeil
          tt_Error.Tekst   = "** Feil på linje " + STRING(iAntLinjer) + " ** Ukjent butikkNr: " + ENTRY(5,pcLinje,";") + ' ' + pcLinje.
    ...
    IF ERROR-STATUS:ERROR THEN
    DO:
        RUN bibl_logg.p ('ImportAvVaretelling', 'xbxwuinnles.p - Feil ved assign tmpPDA: ' + ' Record: ' + pcLinje).
        ASSIGN
          piAntFeil = piAntFeil + 1.
        CREATE tt_Error.
        ASSIGN
    ...
    END.
/*     ELSE                                                                                                              */
/*         RUN bibl_logg.p ('ImportAvVaretelling', 'xbxwuinnles.p - Vellykket assign tmpPDA: ' + ' Record: ' + pcLinje). */
...
/*             piAntFeil = piAntFeil + 1. */
          ...
          ASSIGN
            ...
            tt_Error.Tekst   = "** Feil på linje " + STRING(tmpPDA.linjenr) + " ** Okänd EAN " + STRING(tmpPDA.EAN) + " Antal: " + STRING(tmpPDA.Antall).
      ...
      TELLELINJEN:
      DO TRANSACTION:
        IF LENGTH(cEANNr) = 13 AND cEANNr BEGINS "20" AND STRING(tmpPDA.EAN) BEGINS "23" THEN DO:
            /* Vi förutsätter  */
            dTmp = DECI(SUBSTR(STRING(tmpPDA.EAN),9,4)) / 1000 NO-ERROR.
            IF ERROR-STATUS:ERROR THEN
                dTmp = 0.
            tmpPDA.Antall = dTmp.
        END.
        ...
        IF NOT AVAILABLE ArtBas THEN LEAVE TELLELINJEN.
        ...
        FIND FIRST TelleLinje EXCLUSIVE-LOCK WHERE 
             ...
             TelleLinje.Storl      = (IF AVAILABLE StrKonv THEN StrKonv.Storl ELSE '') NO-ERROR.
        ...
        DO:
            ...
            ASSIGN
              ...
              TelleLinje.Storl      = IF AVAILABLE StrKonv THEN StrKonv.Storl ELSE ''
