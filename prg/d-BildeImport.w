...
DEF OUTPUT PARAM cKatalog      AS CHAR NO-UNDO.
...
&Scoped-Define ENABLED-OBJECTS B-FilSok rsAlle FI-FilKatalog Btn_OK ~
Btn_Cancel fi-cMessage fi-cNumTotal fi-cNumSelected 
&Scoped-Define DISPLAYED-OBJECTS rsAlle FI-FilKatalog fi-cMessage ~
...
DEFINE BUTTON B-FilSok  NO-FOCUS
     LABEL "..." 
     SIZE 4.6 BY 1.1 TOOLTIP "Hent fil som skal importeres.".
...
DEFINE VARIABLE FI-FilKatalog AS CHARACTER FORMAT "X(256)":U 
     LABEL "Filkatalog" 
     VIEW-AS FILL-IN NATIVE 
     SIZE 72.6 BY 1 NO-UNDO.
...
DEFINE FRAME Dialog-Frame
     B-FilSok AT ROW 5.52 COL 90 NO-TAB-STOP 
     rsAlle AT ROW 3.48 COL 35 NO-LABEL
     FI-FilKatalog AT ROW 5.52 COL 15.4 COLON-ALIGNED
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Bildeimport */
DO:
...
&Scoped-define SELF-NAME B-FilSok
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-FilSok Dialog-Frame
ON CHOOSE OF B-FilSok IN FRAME Dialog-Frame /* ... */
DO:
  RUN HentKatalog.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY rsAlle FI-FilKatalog fi-cMessage fi-cNumTotal fi-cNumSelected 
      WITH FRAME Dialog-Frame.
  ENABLE B-FilSok rsAlle FI-FilKatalog Btn_OK Btn_Cancel fi-cMessage 
         fi-cNumTotal fi-cNumSelected 
      WITH FRAME Dialog-Frame.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE HentKatalog Dialog-Frame 
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEF VAR canceled AS LOGICAL NO-UNDO.

  LOOPEN:
  DO WITH FRAME {&FRAME-NAME}:
    RUN BrowseForFolder.p ("Velg katalog det skal importeres bilder fra",
                       OUTPUT cKatalog, 
                       OUTPUT canceled).
    IF canceled = FALSE THEN
        FI-FilKatalog:SCREEN-VALUE = cKatalog.
