...
DEFINE INPUT  PARAMETER iNyPlukkbutikkNr AS INTEGER   NO-UNDO.

DEFINE VARIABLE rKOLinjeRowId AS ROWID       NO-UNDO.
...
FOR EACH SysPara NO-LOCK WHERE
  ...
  SysPara.SysGr        = 10:
    cFrakt = cFrakt + (IF cFrakt <> "" THEN "," ELSE "") + TRIM(SysPara.Parameter1).
...
DO piLoop = 1 TO NUM-ENTRIES(cKOrdre_Id_Lst):
  RUN ByggtmpLager (DECIMAL(ENTRY(piLoop,cKOrdre_Id_Lst))).
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH KOrdrelinje WHERE KOrdrelinje.KOrdre_id = dKOrdre_id NO-LOCK:
        IF KOrdreLinje.plockstatus <> 0 THEN
            NEXT.
        /* syspara "posten" */
/*         IF cFrakt <> '' AND KOrdreLinje.VareNr = cFrakt THEN */
        IF cFrakt <> '' AND CAN-DO(cFrakt,KOrdreLinje.VareNr) THEN 
            NEXT.
        ASSIGN lDec = DECIMAL(KOrdreLinje.VareNr) NO-ERROR.
        IF ERROR-STATUS:ERROR THEN
            NEXT.
        IF CAN-FIND(FIRST tmpArtlag WHERE tmpartlag.artikkelnr = DECI(KOrdreLinje.VareNr) AND 
                                            TRIM(tmpArtLag.storl) = TRIM(KOrdreLinje.Storl)) THEN
            NEXT.
...
&IF DEFINED(EXCLUDE-bytPlockbutik) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE bytPlockbutik Procedure 
PROCEDURE bytPlockbutik :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    rKOLinjeRowId = TO-ROWID(cRowId).
    FIND KOrdreLinje WHERE ROWID(KOrdreLinje) = rKOLinjeRowId NO-ERROR.
    IF AVAIL KOrdreLinje THEN DO:
        ASSIGN KOrdreLinje.PlukkButikk   = iNyPlukkbutikkNr
               KOrdreLinje.plockstatus   = 1
               KOrdreLinje.plockdatetime = ?.
        FIND CURRENT KOrdreLinje NO-LOCK.
        RUN opprett_OverforingsordreJF.p ("",TRUE,cRowId).
    END.
END PROCEDURE.
...
/*------------------------------------------------------------------------------
  ...
  SETT_PLUKKBUTIKK:
  FOR EACH KOrdreLinje OF KOrdreHode EXCLUSIVE-LOCK BREAK BY KOrdreLinje.KOrdre_Id BY KOrdreLinje.KOrdreLinjeNr:
      IF KOrdreLinje.plockstatus <> 0 THEN
          NEXT.
      IF KOrdreLinje.Antall <> 1 THEN
          NEXT.
...
/*       END.                                          */
    /* Betaling m.m. skal ikke behandles for plukkbutikk */
      ...
      FIND ArtBas NO-LOCK WHERE ArtBas.ArtikkelNr = DECIMAL(KOrdreLinje.VareNr) NO-ERROR.
      IF NOT AVAIL ArtBas THEN
          NEXT.
    
     /* Legger opp lageret i tmpFile. */  
     /* redan gjort för hela ordern. se i Main */
     
     /* Nuller og behandler linjen pånytt */
     ASSIGN KOrdreLinje.Plukkbutikk = 0.
     ...
     IF ArtBas.Lager = FALSE AND KordreLinje.Plukkbutikk = 0 THEN 
         ASSIGN KordreLinje.Plukkbutikk = (IF cNettButLst <> '' THEN int(ENTRY(1,cNettButLst)) ELSE int(ENTRY(1,cLagerListe)))
                KordreLinje.plockstatus = 1.
     ELSE
         ASSIGN KOrdreLinje.PlukkButikk = getPlockbutik(DECI(KordreLinje.Varenr),TRIM(KordreLinje.storl)).     
                KordreLinje.plockstatus = IF KordreLinje.PlukkButikk > 0 THEN 1 ELSE 0.
...
&IF DEFINED(EXCLUDE-sett_plukk_butikk_delvis_org) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE sett_plukk_butikk_delvis_org Procedure 
PROCEDURE sett_plukk_butikk_delvis_org :
/*------------------------------------------------------------------------------
                        Purpose:                                                                                                                                          
                        Notes:                                                                                                                                            
        ------------------------------------------------------------------------------*/
  ...
  FOR EACH KOrdreLinje OF KOrdreHode EXCLUSIVE-LOCK BREAK BY KOrdreLinje.KOrdre_Id BY KOrdreLinje.KOrdreLinjeNr:
    ...
    IF NOT CAN-FIND(ArtBas WHERE ArtBas.ArtikkelNr = DECIMAL(KOrdreLinje.VareNr)) THEN
        NEXT.
