...
/*        cLabels = "Hovedgruppe,Beskrivelse,Periode,Solgt,Verdi solgt,Solgt%,Mva verdi,DbKr,Db%,Rabatter,Rabatt kr,VVarekost,Kunderekl,Kunderekl kr,Levrekl,Levrekl kr,Svinn,Svinn kr,Gjenkjøp,Gjenkjøp kr,Tilbud,Tilbud kr,Brekkasje,Brekkasje kr"                    */
...
B-Aktiver B-HgBlank B-VgStat Tg-VisPerBut B-HuvGr 
...
DO:
  ...
  IF NOT DYNAMIC-FUNCTION('getKriterier':U IN h_fstperiode,
     OUTPUT cKriterier /* CHARACTER */) THEN
      RETURN.
  IF FI-Avdeling <> "*" OR FI-HuvGr <> "*" THEN
      ASSIGN pcFeltListe = IF FI-Avdeling <> "*" THEN "AvdelingNr" ELSE "Hg"
             pcVerdier   = IF FI-Avdeling <> "*" THEN FI-Avdeling  ELSE FI-HuvGr.
...
&Scoped-define SELF-NAME B-VgStat
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-VgStat fFrameWin
ON CHOOSE OF B-VgStat IN FRAME fMain /* >> Varegr. */
DO:
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      APPLY "CHOOSE" TO B-AvdelingBlank.
      APPLY "CHOOSE" TO B-HgBlank.
      ASSIGN FI-Avdeling:SCREEN-VALUE = "( 1 )"
             FI-Avdeling     = cVerdi
             FI-Avdeling:TOOLTIP = cVerdi
             FI-Avdeling:BGCOLOR  = 11
             B-Avdeling:PRIVATE-DATA = cRowId + CHR(1) + cVerdi.
...
------------------------------------------------------------------------------*/
  DISPLAY Tg-VisPeriode FI-Avdeling FI-HuvGr Tg-VisPerBut 
      WITH FRAME fMain.
  ENABLE Tg-VisPeriode B-Avdeling B-AvdelingBlank B-Aktiver B-HgBlank B-VgStat 
         Tg-VisPerBut B-HuvGr 
      WITH FRAME fMain.
...
------------------------------------------------------------------------------*/
  ...
  IF cDataObjVerdi = "" THEN
    RETURN.
  FIND HuvGr WHERE HuvGr.Hg = INT(cDataObjVerdi) NO-LOCK NO-ERROR.
  IF NOT AVAIL HuvGr THEN 
      RETURN.
  RUN AutoAktiver IN h_Window (cStTypeId,cDataObjVerdi,STRING(ROWID(HuvGr))).
END PROCEDURE.
