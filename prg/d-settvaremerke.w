...
&Scoped-Define ENABLED-OBJECTS rsAlle fiVMId1 B-SokVmId1 Btn_OK Btn_Cancel ~
fi-cMessage fi-cNumTotal fi-cNumSelected 
&Scoped-Define DISPLAYED-OBJECTS rsAlle fiVMId1 fi-cMessage fi-cNumTotal ~
fi-cNumSelected 
...
DEFINE VARIABLE fiVMId1 AS INTEGER FORMAT "zzz9" INITIAL ? 
     LABEL "Varemerke" 
     VIEW-AS FILL-IN 
     SIZE 7.6 BY 1.
...
DEFINE FRAME Dialog-Frame
     ...
     fiVMId1 AT ROW 3.86 COL 72 COLON-ALIGNED HELP
          "Varemerke (~"Brand~")."
     B-SokVmId1 AT ROW 3.86 COL 82
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Endre varemerke */
DO:
...
&Scoped-define SELF-NAME B-SokVmId1
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-SokVmId1 Dialog-Frame
ON CHOOSE OF B-SokVmId1 IN FRAME Dialog-Frame /* ... */
OR "MOUSE-SELECT-DBLCLICK":U OF B-SokVmId1
OR "F10" OF B-SokVmId1
DO:
  DEF VAR cLookupValue AS CHAR NO-UNDO.

  cLookupValue = "VmId".

  RUN JBoxDLookup.w ("Varemerke;VmId;Beskrivelse", "where true", INPUT-OUTPUT cLookupValue).

  IF cLookupValue NE "" THEN DO: 
    fiVmId1:SCREEN-VALUE = cLookupValue.
    APPLY "return" TO fiVmId1.
  END.
END.
...
DO:
  ASSIGN fiVmId1 rsAlle.
  ASSIGN cValues = (IF fiVmId1 <> ?
                      THEN STRING(fiVmId1)
                      ELSE "?")
         oiReturn = rsAlle
         .
...
------------------------------------------------------------------------------*/
  DISPLAY rsAlle fiVMId1 fi-cMessage fi-cNumTotal fi-cNumSelected 
      WITH FRAME Dialog-Frame.
  ENABLE rsAlle fiVMId1 B-SokVmId1 Btn_OK Btn_Cancel fi-cMessage fi-cNumTotal 
         fi-cNumSelected 
      WITH FRAME Dialog-Frame.
