...
&IF DEFINED(EXCLUDE-UtPakkPro) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE UtPakkPro Procedure 
PROCEDURE UtPakkPro :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
ASSIGN
    piTid  = TIME
    cTekst = "Starter utpakking av Produsenter.".
PUBLISH "VPIFilLogg" (cTekst + chr(1) + "5").
...
FOR EACH VPIFilLinje OF VPIFilHode TRANSACTION:
    ...
    DO:
        ...
        DO:
            ASSIGN
                cTekst = "Linje " + STRING(iTotAntLinjer,"zzzzzz9") + ": Konvertert produsent " + TT_Leverant.Lev_Tillv_ID + " til " + ImpKonv.InterntId + "."
        ...
        ELSE 
        DO:
            FIND LAST Produsent NO-LOCK NO-ERROR.
            ASSIGN 
                piLev = IF AVAIL Produsent THEN Produsent.ProdNr + 1 ELSE 1
                .
            CREATE 
                Produsent.
            ASSIGN 
                Produsent.ProdNr = piLev
                .
            CREATE ImpKonv.
            ASSIGN 
            ...
            ASSIGN
              ...
              cTekst   =  "Linje " + STRING(iTotAntLinjer,"zzzzzz9") + ": * Ny produsent opprettet " + string(piLev) + ".".
            PUBLISH "VPIFilLogg" (cTekst + chr(1) + "30").
            CREATE tt_Error.
            ASSIGN
              tt_Error.LinjeNr = piAntFeil
              tt_Error.Tekst   = "Linje " + STRING(iTotAntLinjer,"zzzzzz9") + ": * Ny produsent opprettet " + string(piLev) + "."
              .
        END.
    END. /* KONVERTER */
    ...
    DO:
        FIND Produsent EXCLUSIVE-LOCK WHERE
            Produsent.ProdNr = piLev NO-ERROR.
        IF AVAIL Produsent THEN 
        DO: /* onödigt !!?? */
            IF Produsent.Beskrivelse <> TT_Leverant.Namn   OR Produsent.Adresse1 <> TT_Leverant.Postadress  OR
               Produsent.PostNr      <> TT_Leverant.Postnr OR Produsent.Land     <> TT_Leverant.Landskod    OR
               Produsent.telefon     <> TT_Leverant.TelNr  OR Produsent.Notat    <> TT_Leverant.Lev_Tillv_ID  THEN
                ASSIGN Produsent.Beskrivelse = TT_Leverant.Namn        
                       Produsent.Adresse1    = TT_Leverant.Postadress  
                       Produsent.PostNr      = TT_Leverant.Postnr      
                       Produsent.Land        = TT_Leverant.Landskod    
                       Produsent.telefon     = TT_Leverant.TelNr
                       Produsent.Notat       = TT_Leverant.Lev_Tillv_ID.
        END.
        IF AVAIL Produsent AND NOT CAN-FIND(Post WHERE Post.Postnr = Produsent.PostNr) THEN
            RUN SkapaPostNr(Produsent.PostNr,TT_Leverant.Ort).
    END. /* REGISTEROPPDAT */
