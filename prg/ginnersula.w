...
DEFINE VARIABLE h_binnersula AS HANDLE NO-UNDO.
DEFINE VARIABLE h_dinnersula AS HANDLE NO-UNDO.
...
DEFINE FRAME gDialog
     ...
     Btn_Cancel AT ROW 14.57 COL 42
     SPACE(0.19) SKIP(0.00)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste innersåle"
...
ON GO OF FRAME gDialog /* Søkeliste innersåle */
DO:
        ASSIGN    
          cReturn-Value = "OK"    
          .
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL gDialog gDialog
ON WINDOW-CLOSE OF FRAME gDialog /* Søkeliste innersåle */
DO:  
...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             INPUT  'sdo/dinnersula.wDB-AWARE':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'AppServiceASUsePromptASInfoForeignFieldsRowsToBatch200CheckCurrentChangedyesRebuildOnReposnoServerOperatingModeNONEDestroyStatelessnoDisconnectAppServernoObjectNamedinnersulaUpdateFromSourceno':U ,
             OUTPUT h_dinnersula ).
       RUN repositionObject IN h_dinnersula ( 13.86 , 24.00 ) NO-ERROR.
       /* Size in AB:  ( 1.62 , 10.80 ) */

       RUN constructObject (
             INPUT  'prg/binnersula.w':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'ScrollRemotenoDataSourceNamesUpdateTargetNamesHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_binnersula ).
       RUN repositionObject IN h_binnersula ( 2.91 , 1.00 ) NO-ERROR.
       RUN resizeObject IN h_binnersula ( 11.43 , 56.00 ) NO-ERROR.
       ...
       /* Links to SmartDataBrowser h_binnersula. */
       RUN addLink ( h_dinnersula , 'Data':U , h_binnersula ).
       RUN addLink ( h_binnersula , 'Update':U , h_dinnersula ).

       /* Links to SmartObject h_sortsok. */
       RUN addLink ( h_binnersula , 'Sortera':U , h_sortsok ).

       /* Adjust the tab order of the smart objects. */
       RUN adjustTabOrder ( h_sortsok ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjustTabOrder ( h_binnersula ,
             h_sortsok , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  IF cReturn-Value <> "AVBRYT" THEN  
   ASSIGN
       cColValues = DYNAMIC-FUNCTION('colValues':U IN h_dinnersula,
                      INPUT "Inner-Id,InnerBeskr" /* CHARACTER */).  
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  PUBLISH "Sortera" FROM h_binnersula.

  DYNAMIC-FUNCTION('findRowWhere':U IN h_dinnersula,
     INPUT cFelt    + (IF cFelt = "" THEN "" ELSE ",") + "Inner-Id",
     INPUT cVerdier + (IF cVerdier = "" THEN "" ELSE CHR(1)) + cStart,
     INPUT "EQ,EQ" /* CHARACTER */).
  /* Signalerer til de andre objektene. */
  RUN dataAvailable IN h_dinnersula
