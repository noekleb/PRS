...
DEFINE TEMP-TABLE TT_ArtTopp
    FIELD TT_ArtTopp01 AS CHARACTER  /* Artikelnummer          */
    FIELD TT_ArtTopp02 AS INTEGER    /* Inlevererat + Överfört */
    FIELD TT_ArtTopp03 AS INTEGER    /* Sålt Antal             */
    FIELD TT_ArtTopp04 AS INTEGER    /* UtfallsProcent         */
    FIELD TT_ArtTopp05 AS DECIMAL    /* Försäljning ink moms   */
    FIELD TT_ArtTopp06 AS DECIMAL    /* Försäljning exkl moms  */
    FIELD TT_ArtTopp07 AS DECIMAL    /* Bruttovinst            */
    FIELD TT_ArtTopp08 AS INTEGER    /* Marginal               */
    FIELD TT_ArtTopp09 AS CHARACTER  /* Artikelbenämning       */
    FIELD TT_ArtTopp10 AS CHARACTER  /* Leverantörsnamn        */
    FIELD TT_ArtTopp11 AS CHARACTER  /* ??????????             */
    FIELD TT_ArtTopp12 AS CHARACTER  /* Färg                   */
    FIELD TT_ArtTopp13 AS CHARACTER  /* Materialnamn           */
    FIELD TT_ArtTopp14 AS CHARACTER  /* Bildfil                */
    FIELD TT_ArtTopp15 AS CHARACTER  /* Vg / Löpnr             */
    FIELD TT_ArtTopp16 AS CHARACTER. /* Levfärg                */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getBildeFil fFrameWin 
...
/* Resultat OCXINFO:CREATE-CONTROL from: {C0A63B86-4B21-11d3-BD95-D426EF2C7949} type: VSFlexGrid */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE PDFArtikelTopp fFrameWin 
PROCEDURE PDFArtikelTopp :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
   ...
   DEFINE       VARIABLE  dCP3           AS DECIMAL EXTENT 90   NO-UNDO.
   ...
   DEFINE       VARIABLE  iTotTTArt      AS INTEGER             NO-UNDO.
   DEFINE       VARIABLE  iAntRad        AS INTEGER             NO-UNDO.
   DEFINE       VARIABLE  cWrk           AS CHARACTER           NO-UNDO.
   DEFINE       VARIABLE  dWrk           AS DECIMAL             NO-UNDO.
   DEFINE       VARIABLE  iLm            AS INTEGER             NO-UNDO.

   FOR EACH TT_ArtTopp EXCLUSIVE-LOCK:
       DELETE TT_ArtTopp.
   END.
   FOR EACH TT_image EXCLUSIVE-LOCK:
       DELETE TT_image.
   END.
   ASSIGN lLast = FALSE.
   IF cExtraInfo <> "" THEN
...
/*            VIEW-AS ALERT-BOX INFO BUTTONS OK.     */
   ...
   DO iCount = 1 TO NUM-ENTRIES(cPageCols,";"):
       ...
       DO iCount2 = 1 TO NUM-ENTRIES(ENTRY(iCount,cFormatStr,";")) - 1:
         ...
         ASSIGN dCP3[iMaxCol] = DEC(ENTRY(iCount2,ENTRY(iCount,cFormatStr,";"))).
   ...
   DO iCount = 1 TO chResultat:Rows - 1:
       IF chResultat:RowHidden(iCount) THEN
           NEXT.
       ASSIGN iAntRad = iAntRad + 1.
   END.
   ASSIGN iAntRad = iAntRad - 1.
/*       MESSAGE "iAntRad " iAntRad
           VIEW-AS ALERT-BOX INFO BUTTONS OK.*/
   ASSIGN iLastRow = 0.
   ASSIGN iSidNr = 1
          iRadNr = 1.
   ...
   DO iCount = 1 TO chResultat:Rows - 1:
       IF chResultat:RowHidden(iCount) THEN
           NEXT.
       IF iTotTTArt < iAntRad THEN
       DO:
         CREATE TT_ArtTopp.
         ASSIGN iTotTTArt = iTotTTArt + 1.
       END.
       ELSE
           ASSIGN iTotTTArt = iTotTTArt + 1.
...
DEFINE TEMP-TABLE TT_ArtTopp
    ...
    FIELD TT_ArtTopp09 AS CHARACTER  /* Artikelbenämning       */
    FIELD TT_ArtTopp10 AS CHARACTER  /* Leverantörsnamn        */
    FIELD TT_ArtTopp11 AS CHARACTER  /* LevArtNr??             */
    FIELD TT_ArtTopp12 AS CHARACTER  /* Färg                   */
    FIELD TT_ArtTopp13 AS CHARACTER  /* Materialnamn           */
    FIELD TT_ArtTopp14 AS CHARACTER. /* Bildfil                */
           ...
           */
           ...
           DO iCount2 = INT(ENTRY(1,ENTRY(iCountPageCols,cPageCols,";"))) TO INT(ENTRY(2,ENTRY(iCountPageCols,cPageCols,";"))):
               ...
               IF iTotTTArt < iAntRad OR iTotTTArt = iAntRad THEN
               DO:
                 IF iCount2 = 1 THEN
                 DO:
                     ASSIGN TT_ArtTopp.TT_ArtTopp01 = chResultat:Cell(19,iCount,iCount2,iCount,iCount2).
                     IF iTotTTArt = iAntRad THEN
                 END.
                 IF iCount2 = 2 THEN
                     ASSIGN TT_ArtTopp.TT_ArtTopp09 = chResultat:Cell(19,iCount,iCount2,iCount,iCount2).
                 IF iCount2 = 3 THEN
                     ASSIGN TT_ArtTopp.TT_ArtTopp11 = chResultat:Cell(19,iCount,iCount2,iCount,iCount2).
                 IF iCount2 = 4 THEN
                     ASSIGN TT_ArtTopp.TT_ArtTopp03 = INTEGER(chResultat:Cell(19,iCount,iCount2,iCount,iCount2)).
                 IF iCount2 = 5 THEN
                 DO:
                     ASSIGN wrk = TRIM(chResultat:Cell(19,iCount,iCount2,iCount,iCount2)).
                     ASSIGN cWrk = "".
                     DO iCount4 = 1 TO LENGTH(wrk):
                        IF SUBSTRING(wrk,iCount4,1) = CHR(160) THEN
                            NEXT.
                        ASSIGN cWrk = cWrk + SUBSTRING(wrk,iCount4,1).
                     END.
                     ASSIGN TT_ArtTopp.TT_ArtTopp05 = DECIMAL(cWrk).  
                 END.
                 IF iCount2 = 6 THEN
                 DO:
                     ASSIGN wrk = TRIM(chResultat:Cell(19,iCount,iCount2,iCount,iCount2)).
                     ...
                     DO iCount4 = 1 TO LENGTH(wrk):
                        ...
                        ASSIGN cWrk = cWrk + SUBSTRING(wrk,iCount4,1).
                     END.
                     ASSIGN TT_ArtTopp.TT_ArtTopp06 = DECIMAL(cWrk).
                 END.
                 IF iCount2 = 14 THEN
                 DO:
                     ASSIGN wrk = TRIM(chResultat:Cell(19,iCount,iCount2,iCount,iCount2)).
                     ...
                     DO iCount4 = 1 TO LENGTH(wrk):
                        ...
                        ASSIGN cWrk = cWrk + SUBSTRING(wrk,iCount4,1).
                     END.
                     ASSIGN TT_ArtTopp.TT_ArtTopp07 = TT_ArtTopp.TT_ArtTopp06 - DECIMAL(cWrk).
                     ASSIGN TT_ArtTopp.TT_ArtTopp08 = IF TT_ArtTopp.TT_ArtTopp06 = 0 THEN 0 ELSE (TT_ArtTopp.TT_ArtTopp07 * 100) / TT_ArtTopp.TT_ArtTopp06.
                 END.
                 IF iCount2 = 15 THEN
                     ASSIGN TT_ArtTopp.TT_ArtTopp02 = INTEGER(chResultat:Cell(19,iCount,iCount2,iCount,iCount2)).
                 IF iCount2 = 21 THEN
                 DO:
                     ASSIGN TT_ArtTopp.TT_ArtTopp02 = TT_ArtTopp.TT_ArtTopp02 + INTEGER(chResultat:Cell(19,iCount,iCount2,iCount,iCount2)).
                     ASSIGN TT_ArtTopp.TT_ArtTopp04 = IF TT_ArtTopp.TT_ArtTopp02 = 0 THEN 0 ELSE (TT_ArtTopp.TT_ArtTopp03 * 100) / TT_ArtTopp.TT_ArtTopp02.
                 END.
                 IF iCount2 = 31 THEN
                     ASSIGN cWrk = chResultat:Cell(19,iCount,iCount2,iCount,iCount2).
                     RUN PDFFormat (INPUT-OUTPUT cWrk).
                     ASSIGN TT_ArtTopp.TT_ArtTopp15 = cWrk.
                 IF iCount2 = 37 THEN
                     ASSIGN TT_ArtTopp.TT_ArtTopp10 = chResultat:Cell(19,iCount,iCount2,iCount,iCount2).
                 IF iCount2 = 41 THEN
                     ASSIGN TT_ArtTopp.TT_ArtTopp12 = chResultat:Cell(19,iCount,iCount2,iCount,iCount2).
                 IF iCount2 = 42 THEN
                     ASSIGN TT_ArtTopp.TT_ArtTopp16 = chResultat:Cell(19,iCount,iCount2,iCount,iCount2).
                 IF iCount2 = 44 THEN
                     ASSIGN TT_ArtTopp.TT_ArtTopp13 = chResultat:Cell(19,iCount,iCount2,iCount,iCount2).
               END.

/*               MESSAGE iCount2 SKIP cRowStr
                   VIEW-AS ALERT-BOX INFO BUTTONS OK.*/
           END.
...
/*                IF iRadNr = 36 AND iCount < chResultat:Rows - 1 THEN */
   ...
   MESSAGE "Antal rader är " iAntRad
       ". Hur många rader " SET iAntRad2 LIKE iAntRad.

   FOR EACH TT_ArtTopp NO-LOCK:
        FIND ArtBas WHERE ArtBas.ArtikkelNr = DECIMAL(TT_ArtTopp.TT_ArtTopp01).
        IF AVAILABLE ArtBas THEN
          ASSIGN TT_ArtTopp.TT_ArtTopp14 = getBildeFil(ArtBas.BildNr).
   END.
/*   OUTPUT CLOSE.*/
   ASSIGN pcRappFil = SESSION:TEMP-DIRECTORY + "rapp.pdf".
          iLeftMargin = 30.
   RUN pdf_new ("Spdf",pcRappfil).
   pdf_PageFooter ("Spdf",THIS-PROCEDURE:HANDLE,"PDFPageFooter2").
...
/*   MESSAGE "iPageWidth " iPageWidth
   ...
   DO iCount3 = 1 TO iMaxCol:
     ASSIGN dColPos[iCount3] = dCP3[iCount3] * 7.26.
   END.
   DO iCount3 = 1 TO iMaxCol:
...
/*    OUTPUT TO "CLIPBOARD". */
   ...
   ASSIGN dY = pdf_PageHeight ("Spdf") - 50.
   ...
   FOR EACH TT_ArtTopp NO-LOCK:
       ...
       END.
       RUN pdf_set_font IN h_PDFinc ("Spdf", "Helvetica",8).
       RUN pdf_line IN h_PDFinc  ("Spdf", pdf_LeftMargin ("Spdf"), dY, 565, dY, 0.5).

       RUN PDFBild (dY,iLm).
      
       dy = dy - 13.
       RUN pdf_set_font IN h_PDFinc ("Spdf", "Helvetica-Bold",12).
       RUN pdf_text_xy_dec ("Spdf","Artnr Vg/Löpnr",dCP3[1],dY).
       RUN pdf_text_xy_dec ("Spdf","Inlev",dCP3[2],dY).
       RUN pdf_text_xy_dec ("Spdf","Sålt",dCP3[3],dY).
       RUN pdf_text_xy_dec ("Spdf","Utf.gr",dCP3[4],dY).
       RUN pdf_text_xy_dec ("Spdf","Förs inkl",dCP3[5],dY).
       RUN pdf_text_xy_dec ("Spdf","Bruttovinst",dCP3[6],dY).
       RUN pdf_text_xy_dec ("Spdf","Marg",dCP3[7],dY).
       dy = dy - 26.
       RUN pdf_set_font IN h_PDFinc ("Spdf", "Helvetica-Bold",10).
       ASSIGN cWrk = TT_ArtTopp.TT_ArtTopp01 + " " + TT_ArtTopp.TT_ArtTopp15.
       RUN pdf_text_xy_dec ("Spdf",cWrk,dCP3[1],dY).
       ASSIGN dWrk = DECIMAL(TT_ArtTopp.TT_ArtTopp02).
       RUN pdf_text_xy_dec ("Spdf",TRIM(STRING(dWrk,"->>,>>9")),dCP3[8] - bredd(TRIM(STRING(dWrk,"->>,>>9"))),dY).
       ASSIGN dWrk = DECIMAL(TT_ArtTopp.TT_ArtTopp03).
       RUN pdf_text_xy_dec ("Spdf",TRIM(STRING(dWrk,"->>,>>9")),dCP3[9] - bredd(TRIM(STRING(dWrk,"->>,>>9"))),dY).
       ASSIGN dWrk = DECIMAL(TT_ArtTopp.TT_ArtTopp04).
       RUN pdf_text_xy_dec ("Spdf",TRIM(STRING(dWrk,"->>,>>9")),dCP3[10] - bredd(TRIM(STRING(dWrk,"->>,>>9"))),dY).
       RUN pdf_text_xy_dec ("Spdf","%",dCP3[10],dY).
       ASSIGN dWrk = DECIMAL(TT_ArtTopp.TT_ArtTopp05).
       RUN pdf_text_xy_dec ("Spdf",TRIM(STRING(dWrk,"->>,>>9.99")),dCP3[11] - bredd(TRIM(STRING(dWrk,"->>,>>9.99"))),dY).
       ASSIGN dWrk = DECIMAL(TT_ArtTopp.TT_ArtTopp07).
       RUN pdf_text_xy_dec ("Spdf",TRIM(STRING(dWrk,"->>,>>9.99")),dCP3[12] - bredd(TRIM(STRING(dWrk,"->>,>>9.99"))),dY).
       ASSIGN dWrk = DECIMAL(TT_ArtTopp.TT_ArtTopp08).
       RUN pdf_text_xy_dec ("Spdf",TRIM(STRING(dWrk,"->>,>>9")),dCP3[13] - bredd(TRIM(STRING(dWrk,"->>,>>9"))),dY).
       RUN pdf_text_xy_dec ("Spdf","%",dCP3[13],dY).
       dy = dy - 13.
       RUN pdf_text_xy_dec ("Spdf",TT_ArtTopp.TT_ArtTopp09,dCP3[1],dY).
       dy = dy - 13.
       RUN pdf_text_xy_dec ("Spdf",TT_ArtTopp.TT_ArtTopp10,dCP3[1],dY).
       dy = dy - 13.
       RUN pdf_text_xy_dec ("Spdf",TT_ArtTopp.TT_ArtTopp11,dCP3[1],dY).
       ASSIGN cWrk = TT_ArtTopp.TT_ArtTopp12 + "/" + TT_ArtTopp.TT_ArtTopp13.
       IF TT_ArtTopp.TT_ArtTopp16 <> "" THEN
         ASSIGN cWrk = cWrk + "/" + TT_ArtTopp.TT_ArtTopp16.

       RUN pdf_text_xy_dec ("Spdf",cWrk,dCP3[3],dY).

       ASSIGN iLastRow2 = iLastRow2 + 1.
       IF iLastRow2 >= iAntRad2 THEN
          LEAVE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE PDFBild fFrameWin 
PROCEDURE PDFBild :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT PARAMETER dY2    AS DECIMAL           NO-UNDO.
DEFINE INPUT PARAMETER iLm    AS INTEGER           NO-UNDO.
DEFINE VARIABLE dY3           AS DECIMAL     NO-UNDO.
...
ASSIGN TT_ArtTopp.TT_ArtTopp14 = REPLACE(TT_ArtTopp.TT_ArtTopp14,".bmp",".jpg").
IF TT_ArtTopp.TT_ArtTopp14 <> "" AND SEARCH(TT_ArtTopp.TT_ArtTopp14) <> ? THEN
DO:
  ASSIGN cBildNamn = "mini" + STRING(TT_ArtTopp.TT_ArtTopp01).
  IF NOT CAN-FIND(FIRST TT_image WHERE TT_image.image_name = cBildNamn NO-LOCK) THEN
  DO:
     RUN pdf_load_image ("Spdf",cBildNamn,TT_ArtTopp.TT_ArtTopp14).
     CREATE TT_image.
  ...
  IF iImageWidth >= iImageHeight THEN
    ASSIGN iImageHeight = 80 * (iImageHeight / iImageWidth)
           iImageWidth = 80.
  ELSE
    ASSIGN iImageWidth = 60 *  (iImageWidth / iImageHeight)
           iImageHeight = 60.
    ASSIGN dY2 = dY2 - 20
           dY3 = pdf_PageHeight ("Spdf") - dY2 + 30.
  RUN pdf_place_image ("Spdf",cBildNamn,iLm,dY3,iImageWidth,iImageHeight).
END.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE PDFFormat fFrameWin 
PROCEDURE PDFFormat :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT-OUTPUT PARAMETER cWrk2 AS CHARACTER NO-UNDO.
DEFINE VARIABLE               cWrk3 AS CHARACTER   NO-UNDO.
DEFINE VARIABLE ix  AS INTEGER     NO-UNDO.
DEFINE VARIABLE lTabort AS LOGICAL     NO-UNDO.

ASSIGN lTabort = FALSE
       cWrk3  = "".
DO ix = 1 TO LENGTH(cWrk2):
   IF SUBSTRING(cWrk2,ix,1) = " " AND lTabort = TRUE THEN
       NEXT.
   IF SUBSTRING(cWrk2,ix,1) = "0" AND lTabort = TRUE THEN
       NEXT.
   ASSIGN cWrk3 = cWrk3 + SUBSTRING(cWrk2,ix,1).
   IF SUBSTRING(cWrk2,ix,1) = "/" THEN
       ASSIGN lTabort = TRUE.
   ELSE IF lTabort = TRUE AND SUBSTRING(cWrk2,ix,1) <> "0" THEN
       ASSIGN lTabort = FALSE.
END.
ASSIGN cWrk2 = cWrk3.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE PDFPageFooter2 fFrameWin 
PROCEDURE PDFPageFooter2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE PDFPageHeader2 fFrameWin 
PROCEDURE PDFPageHeader2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
/*  RUN pdf_set_font IN h_PDFinc ("Spdf", "Helvetica-Bold",20).*/
  ...
  RUN pdf_rect IN h_PDFinc ("Spdf", iLm, iPageHeight - 30, iPageWidth - 40, 20, 0.1). /* Left,Bottum,Width,Hight,Tickness*/
  RUN pdf_stroke_fill IN h_PDFinc ("Spdf",1.0,1.0,1.0).
  RUN pdf_set_font IN h_PDFinc ("Spdf", "Helvetica-Bold",15).
  ASSIGN cWrk = "Artiklar topplista " + cExtraInfo.
  RUN pdf_text_xy_dec ("Spdf", cWrk, iLm + 10, iPageHeight - 26).
...
------------------------------------------------------------------------------*/
    ...
    END.
    ELSE IF cReportType = "XPRINT" THEN 
    DO:
      ASSIGN iFirstCol = 1.
...
/* ArtikelToppLista  */
    ELSE IF cReportType = "ARTIKELTOPP" THEN 
    DO:
      ASSIGN iFirstCol = 1.
      DO icount = 1 TO chResultat:Cols - 1:
/*           IF chResultat:ColHidden(icount) = TRUE THEN 
...
/*      MESSAGE "cUrvalsInfo " cUrvalsInfo SKIP
              "cProcent " cProcent SKIP
              "cPageCols " cPageCols SKIP
              "cRightCols " cRightCols
          VIEW-AS ALERT-BOX INFO BUTTONS OK.*/
      RUN PDFArtikelTopp (cUrvalsInfo,cProcent,cPageCols,cRightCols).
    END.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION getBildeFil fFrameWin 
FUNCTION getBildeFil RETURNS CHARACTER
