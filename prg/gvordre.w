...
DEFINE INPUT  PARAMETER iOrdreNr AS CHARACTER  NO-UNDO.
...
ON WINDOW-CLOSE OF FRAME gDialog /* Ordre */
DO:  
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             ...
             INPUT  'AppServiceASUsePromptASInfoForeignFieldsRowsToBatch200CheckCurrentChangedyesRebuildOnReposnoServerOperatingModeNONEDestroyStatelessnoDisconnectAppServernoObjectNamedordreUpdateFromSourceno':U ,
             OUTPUT h_dordre ).
       RUN repositionObject IN h_dordre ( 1.00 , 125.00 ) NO-ERROR.
       ...
       RUN constructObject (
             ...
             OUTPUT h_vordre ).
       RUN repositionObject IN h_vordre ( 1.71 , 2.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  IF VALID-HANDLE(h_dordre) THEN
     ...
     INPUT "Ordrenr = " + string(iOrdreNr)/* CHARACTER */).
  RUN SUPER.
