...
------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER cTmpData AS CHARACTER NO-UNDO.
      ...
      IF TelleHode.TTId <> 8 AND AVAILABLE ArtBas THEN DO:
      ...
      ASSIGN ENTRY(1,cTmpData,CHR(1)) = (IF AVAIL ArtBas THEN STRING(ArtBas.ArtikkelNr) ELSE "0")
             ...
             ENTRY(9,cTmpData,CHR(1)) = IF TelleHode.TTId = 8 
                 THEN STRING(TelleLinje.Nedskrevet,"->>,>>,>>9.99")
                 ELSE IF NOT AVAILABLE ArtPris THEN STRING(0,"->>,>>>,>>9.99")
