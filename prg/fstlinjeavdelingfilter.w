...
/*        cLabels = "Avdel,Beskrivelse,Periode,Solgt,Verdi solgt,Solgt%,Mva verdi,DbKr,Db%,Rabatter,Rabatt kr,VVarekost,Kunderekl,Kunderekl kr,Levrekl,Levrekl kr,Svinn,Svinn kr,Gjenkjøp,Gjenkjøp kr,Tilbud,Tilbud kr,Brekkasje,Brekkasje kr"                          */
...
ASSIGN cFieldDefs = 
        ...
        /* 26 */ "VerdiTilbSolgt;Tilbud kr;;1," +
        /* 27 */ "BrekkAnt;Brekkasje;;1," +
        /* 28 */ "BrekkVerdi;Brekkasje kr;;1".


/*
ASSIGN cFelter = ,Diverse,DiverseAnt,Diverseverdi,Hg,IntAnt         ,IntVerdi                ,JustAnt       ,JustVerdi    ,KjopAnt        ,KjopVerdi  ,LagerAnt,LagerVerdi,NedAnt           ,NedVerdi        ,OmlHast,OvAnt          ,OvVerdi                 ,PrimoAnt,Primoverdi,TilbMvaVerdi  ,TilbVVarekost       ,TotalPost,Utsolgt%,VisBut
       cLabels = ,Diverse,DiverseAnt,Diverseverdi,Hg,Internt forbruk,Verdi av internt forbruk,Justert antall,Justert verdi,Innkjopt antall,Verdi kjøpt,LagerAnt,LagerVerdi,Nedskrevet antall,Verdi nedskrevet,OmlHast,Overført antall,Verdi av overførte varer,PrimoAnt,Primoverdi,Tilb Mva verdi,Varekost tilbudssalg,TotalPost,Utsolgt%,VisBut
       */
/*
ASSIGN cFelter = Aar,AntRabatt    ,AntSolgt    ,AntTilbSolgt          ,Beskrivelse,BrekkAnt ,BrekkVerdi       ,BrukerID,Butik       ,DataObjekt,Db%,DbKr,Diverse,DiverseAnt,Diverseverdi,EDato ,ETid      ,GjenkjopAnt       ,GjenkjopVerdi            ,Hg,IntAnt         ,IntVerdi                ,JustAnt       ,JustVerdi    ,KjopAnt        ,KjopVerdi  ,LagerAnt,LagerVerdi,MvaVerdi ,NedAnt           ,NedVerdi        ,OmlHast,OvAnt          ,OvVerdi                 ,PerId    ,PerLinNr    ,PerLinTxt,PrimoAnt,Primoverdi,RegistrertAv ,RegistrertDato ,RegistrertTid         ,ReklAnt           ,ReklLAnt       ,ReklLVerdi                       ,ReklVerdi               ,StTypeId      ,SvinnAnt    ,SvinnVerdi ,TilbMvaVerdi  ,TilbVVarekost       ,TotalPost,Utsolgt%,VerdiRabatt ,VerdiSolgt ,VerdiTilbSolgt       ,VisBut,VVarekost      ,
       cLabels = År ,Antall rabatt,Antall solgt,Antall solgt på tilbud,Beskrivelse,Brekkasje,Verdi av brekasje,Bruker  ,Butikknummer,Dataobjekt,Db%,DbKr,Diverse,DiverseAnt,Diverseverdi,Endret,Endret tid,Gjenkjøp fra kunde,Verdi av gjenkjøpte varer,Hg,Internt forbruk,Verdi av internt forbruk,Justert antall,Justert verdi,Innkjopt antall,Verdi kjøpt,LagerAnt,LagerVerdi,Mva verdi,Nedskrevet antall,Verdi nedskrevet,OmlHast,Overført antall,Verdi av overførte varer,PeriodeId,PeriodeLinje,PerLinTxt,PrimoAnt,Primoverdi,Registrert av,Registrert dato,Registreringstidspunkt,Kundereklamasjoner,Rekl.lev.antall,Verdi av leveerandørreklamasjoner,Verdi kundereklamasjoner,Statistikktype,Antall svinn,Svinn verdi,Tilb Mva verdi,Varekost tilbudssalg,TotalPost,Utsolgt%,Verdi rabatt,Verdi solgt,Verdi solgt på tilbud,VisBut,Vektet varekost,
 */
...
&Scoped-Define ENABLED-OBJECTS B-Avdeling Tg-VisPeriode B-AvdelingBlank ~
B-Aktiver B-HgStat Tg-VisPerBut 
&Scoped-Define DISPLAYED-OBJECTS Tg-VisPeriode FI-Avdeling Tg-VisPerBut 
...
DO:
  ...
  RUN LoadGrid IN h_frapportgrid (cFileName,IF Tg-VisPerBut:CHECKED THEN 4 ELSE 3).  /* 3 = antall frozen cols  */
  ...
  IF cVisFelterNr <> "" THEN
      RUN VisKun IN h_frapportgrid (cVisFelterNr,"SKJUL").
  qh:QUERY-CLOSE().
  TTH:EMPTY-TEMP-TABLE().
/*   IF ENTRY(1,ENTRY(2,cKriterier,CHR(1))) = "DAG" AND TG-VisPeriode:CHECKED = TRUE THEN */
/*       RUN SetDateFormat IN h_frapportgrid (INT(getSumFelter("PerLinTxt")),"yymmdd").   */
...
&Scoped-define SELF-NAME B-HgStat
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-HgStat fFrameWin
ON CHOOSE OF B-HgStat IN FRAME fMain /* >> Hovedgr. */
DO:
...
------------------------------------------------------------------------------*/
  DISPLAY Tg-VisPeriode FI-Avdeling Tg-VisPerBut 
      WITH FRAME fMain.
  ENABLE B-Avdeling Tg-VisPeriode B-AvdelingBlank B-Aktiver B-HgStat 
         Tg-VisPerBut 
      WITH FRAME fMain.
...
/*   RUN  GetBrukerLng IN h_Window (OUTPUT cSprak) NO-ERROR. */
...
------------------------------------------------------------------------------*/
  ...
  RUN FeltVerdier IN h_frapportgrid (OUTPUT cDataObjVerdi,cGetVerdier,"SAME").
  IF cDataObjVerdi = "" THEN
    RETURN.
  FIND Avdeling WHERE Avdeling.AvdelingNr = INT(cDataObjVerdi) NO-LOCK NO-ERROR.
  IF NOT AVAIL Avdeling THEN 
      RETURN.
  RUN AutoAktiver IN h_Window (cStTypeId,cDataObjVerdi,STRING(ROWID(Avdeling))).
END PROCEDURE.
