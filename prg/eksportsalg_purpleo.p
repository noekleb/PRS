...
DO:
  ...
  RUN bibl_logg.p ('eksporter_salg_purpleo', 'eksportsalg.p: MANUELL Periode: ' + string(dinpFraDato) + ' - ' + string(dinpTilDato) + ' ' + string(TIME,"HH:MM:SS")).
END.
...
RUN bibl_logg.p ('eksporter_salg_purpleo', 'eksportsalg.p Butikk: Fil: ' + string(cFilNavn) + 'txt').
    ...
    FOR EACH Strekkode NO-LOCK WHERE
      ...
      FOR EACH tmpSalgPrStr WHERE
        ...
        PUT STREAM Ut UNFORMATTED
          ...
          tmpSalgPrStr.LevNr ';'
          ArtBas.Vg ';'
          ROUND((tmpSalgPrStr.UtVerdiKr - tmpSalgPrStr.MvaKr),2) ';'
          ArtBas.Beskr  
          SKIP.
        /*DELETE tmpSalgPrStr.*/
      END.
