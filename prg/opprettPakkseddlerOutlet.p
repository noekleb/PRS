...
/* Setter leverand�rnnumer som skal benyttes. */
...
/* Butikk 50 er opprettet for at nettbutikken skal kunne overf�re varer som ikke skal vises lenger i nettbutikk dit. */
/* Det skjer kun over�fringer til lager 50 fra butikk 16. */
...
/* TN 25/7-16 Det skal ikke gis rabatter ved overf�ringer mellom butikker uansett... 
/* Det skal ikke gis rabatt n�r det overf�res til andre butikker enn overskuddslageret. */
IF iOutlet <> iOverskuddslager THEN
    ASSIGN  
        lRab%     = 0
        lPrisRab% = 0.
        
/* Ved overf�ring fra en outlet, skal det ikke gis rabatt */
IF CAN-DO(cOutletLst,STRING(TT_OvBuffer.ButikkNrFra)) THEN 
    ASSIGN  
        lRab%     = 0
        lPrisRab% = 0.
        
/* Overf�res det til nettbutikkens overskuddslager, skal det ikke gis rabatt. */
...
/* Rydder opp f�r programmet avsluttes. */
    ...
    DO:
        ...
        ASSIGN PkSdlHode.PkSdlStatus    = 10
               ...
               PkSdlHode.Merknad        = "Via fakturamodul. " + CHR(13) + 
                                          'Overf�rt fra butikk ' + STRING(TT_OvBuffer.ButikkNrFra) + '.' + 
    ...
    FOR EACH tt_OvBuffer NO-LOCK WHERE
        ...
        DO:
            ...
            BUFFER-COPY ArtBas   
              ...
              TO        PkSdlPris.
        END.
        /* Er mottagende butikk en outlet, skal rabatter regnes inn. Men hvis ovef�ringen kommer fra */
        /* en annen outlet, skal det h�ndteres som en vanlig overf�ring mellom butikker.             */
