...
DEFINE VARIABLE h_bgavektype AS HANDLE NO-UNDO.
DEFINE VARIABLE h_dgavektype AS HANDLE NO-UNDO.
DEFINE VARIABLE h_dyntoolbar AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_sortsok AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vgavektype AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW wWin ASSIGN
         HIDDEN             = YES
         TITLE              = "Vedlikehold gavekorttype register"
         HEIGHT             = 16.43
         WIDTH              = 80
         MAX-HEIGHT         = 28.81
         MAX-WIDTH          = 146.2
...
ON CTRL-F1 OF wWin /* Vedlikehold gavekorttype register */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON END-ERROR OF wWin /* Vedlikehold gavekorttype register */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-CLOSE OF wWin /* Vedlikehold gavekorttype register */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             INPUT  'sdo/dgavektype.wDB-AWARE':U ,
             INPUT  FRAME fMain:HANDLE ,
             INPUT  'AppServiceASUsePromptASInfoForeignFieldsRowsToBatch200CheckCurrentChangedyesRebuildOnReposnoServerOperatingModeNONEDestroyStatelessnoDisconnectAppServernoObjectNamedgavektypeUpdateFromSourcenoToggleDataTargetsyesOpenOnInityesPromptOnDeleteyesPromptColumns(NONE)':U ,
             OUTPUT h_dgavektype ).
       RUN repositionObject IN h_dgavektype ( 1.00 , 62.00 ) NO-ERROR.
       ...
       /* Links to SmartDataObject h_dgavektype. */
       RUN addLink ( h_dyntoolbar , 'Navigation':U , h_dgavektype ).
    ...
    WHEN 1 THEN DO:
       RUN constructObject (
             INPUT  'prg/bgavektype.w':U ,
             INPUT  FRAME fMain:HANDLE ,
             INPUT  'ScrollRemotenoNumDown0CalcWidthnoMaxWidth80FetchOnReposToEndyesDataSourceNamesUpdateTargetNamesLogicalObjectNameHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_bgavektype ).
       RUN repositionObject IN h_bgavektype ( 5.76 , 3.00 ) NO-ERROR.
       RUN resizeObject IN h_bgavektype ( 11.43 , 76.00 ) NO-ERROR.
       ...
       /* Links to SmartDataBrowser h_bgavektype. */
       RUN addLink ( h_dgavektype , 'Data':U , h_bgavektype ).

       /* Links to SmartObject h_sortsok. */
       RUN addLink ( h_bgavektype , 'Sortera':U , h_sortsok ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN constructObject (
             INPUT  'prg/vgavektype.w':U ,
             INPUT  FRAME fMain:HANDLE ,
             ...
             OUTPUT h_vgavektype ).
       RUN repositionObject IN h_vgavektype ( 3.86 , 2.00 ) NO-ERROR.
       /* Size in AB:  ( 3.81 , 65.00 ) */

       /* Links to SmartDataViewer h_vgavektype. */
       RUN addLink ( h_dgavektype , 'Data':U , h_vgavektype ).
       RUN addLink ( h_vgavektype , 'Update':U , h_dgavektype ).
       RUN addLink ( h_dyntoolbar , 'TableIO':U , h_vgavektype ).
...
------------------------------------------------------------------------------*/
  ...
  {syspara.i 16 9 32 iTilgang INT}
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  PUBLISH "Sortera" FROM h_bgavektype.
...
------------------------------------------------------------------------------*/
  ...
  IF valid-handle(h_vgavektype) THEN
  DO:
      ASSIGN
          pbDataModified = 
              DYNAMIC-FUNCTION('getDataModified':U IN h_vgavektype)
          .
