...
&Scoped-Define ENABLED-OBJECTS rsAlle fiProdNr B-SokProdNr Btn_OK ~
Btn_Cancel fi-cMessage fi-cNumTotal fi-cNumSelected 
&Scoped-Define DISPLAYED-OBJECTS rsAlle fiProdNr fi-cMessage fi-cNumTotal ~
fi-cNumSelected 
...
DEFINE VARIABLE fiProdNr AS INTEGER FORMAT "zzzzz9" INITIAL ? 
     LABEL "Produsent" 
     VIEW-AS FILL-IN 
     SIZE 12.6 BY 1.
...
DEFINE FRAME Dialog-Frame
     ...
     fiProdNr AT ROW 3.86 COL 67 COLON-ALIGNED HELP
          "Produsent"
     B-SokProdNr AT ROW 3.86 COL 82
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Endre produsent */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-SokProdNr Dialog-Frame
ON CHOOSE OF B-SokProdNr IN FRAME Dialog-Frame /* ... */
OR "MOUSE-SELECT-DBLCLICK":U OF B-SokProdNr
OR "F10" OF B-SokProdNr
DO:
  DEF VAR cLookupValue AS CHAR NO-UNDO.

  cLookupValue = "ProdNr".

  RUN JBoxDLookup.w ("Produsent;ProdNr;Beskrivelse", "where true", INPUT-OUTPUT cLookupValue).

  IF cLookupValue NE "" THEN DO: 
    fiProdNr:SCREEN-VALUE = cLookupValue.
    APPLY "return" TO fiProdNr.
  END.
END.
...
DO:
  ASSIGN fiProdNr rsAlle.
  ASSIGN cValues = (IF fiPRodNr <> ?
                      THEN STRING(fiProdNr)
                      ELSE "?")
         oiReturn = rsAlle
         .
END.
...
------------------------------------------------------------------------------*/
  DISPLAY rsAlle fiProdNr fi-cMessage fi-cNumTotal fi-cNumSelected 
      WITH FRAME Dialog-Frame.
  ENABLE rsAlle fiProdNr B-SokProdNr Btn_OK Btn_Cancel fi-cMessage fi-cNumTotal 
         fi-cNumSelected 
      WITH FRAME Dialog-Frame.
