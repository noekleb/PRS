...
&Scoped-define FIELDS-IN-QUERY-br_table ForsNr FoNamn ButikkNr 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table ForsNr FoNamn 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table bTableWin _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ForsNr FORMAT ">>>>>>>9":U WIDTH 9
      FoNamn FORMAT "x(30)":U WIDTH 30.8
      ButikkNr FORMAT ">>>>>9":U
  ENABLE
      ForsNr
      FoNamn
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN NO-AUTO-VALIDATE NO-ROW-MARKERS SEPARATORS SIZE 42 BY 13.57 EXPANDABLE.
...
ASSIGN 
       rowObject.ForsNr:COLUMN-READ-ONLY IN BROWSE br_table = TRUE
       rowObject.FoNamn:COLUMN-READ-ONLY IN BROWSE br_table = TRUE.
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[1]   > _<SDO>.rowObject.ForsNr
"ForsNr" ? ? "integer" ? ? ? ? ? ? yes ? no no "9" yes no yes "U" "" ""
     _FldNameList[2]   > _<SDO>.rowObject.FoNamn
"FoNamn" ? ? "character" ? ? ? ? ? ? yes ? no no "30.8" yes no yes "U" "" ""
     _FldNameList[3]   = _<SDO>.rowObject.ButikkNr
