...
&Scoped-define FIELDS-IN-QUERY-br_table ValNavn CapsValKod ValKurs ValDatum ~
ValNr 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table ValNavn ValKurs ValDatum ~
ValNr 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table bTableWin _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ValNavn FORMAT "X(30)":U
      CapsValKod FORMAT "x(3)":U
      ValKurs FORMAT "z9.999999":U WIDTH 11.4
      ValDatum FORMAT "99/99/99":U WIDTH 11.6
      ValNr FORMAT ">9":U WIDTH 8.6
  ENABLE
      ValNavn HELP "?"
      ValKurs
      ValDatum
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN NO-AUTO-VALIDATE NO-ROW-MARKERS SEPARATORS SIZE 76 BY 6.67 FIT-LAST-COLUMN.
...
ASSIGN 
       rowObject.ValNavn:COLUMN-READ-ONLY IN BROWSE br_table = TRUE
       rowObject.ValKurs:COLUMN-READ-ONLY IN BROWSE br_table = TRUE
       rowObject.ValDatum:COLUMN-READ-ONLY IN BROWSE br_table = TRUE
       rowObject.ValNr:COLUMN-READ-ONLY IN BROWSE br_table = TRUE.
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[1]   > _<SDO>.rowObject.ValNavn
"ValNavn" ? ? "character" ? ? ? ? ? ? yes "?" no no ? yes no yes "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = _<SDO>.rowObject.CapsValKod
     _FldNameList[3]   > _<SDO>.rowObject.ValKurs
"ValKurs" ? ? "decimal" ? ? ? ? ? ? yes ? no no "11.4" yes no yes "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > _<SDO>.rowObject.ValDatum
"ValDatum" ? ? "date" ? ? ? ? ? ? yes ? no no "11.6" yes no yes "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > _<SDO>.rowObject.ValNr
"ValNr" ? ? "integer" ? ? ? ? ? ? yes "?" no no "8.6" yes no yes "U" "" "" "" "" "" "" 0 no 0 no no
