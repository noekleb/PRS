...
&Scoped-Define ENABLED-OBJECTS rsAlle fiRAvdNr B-SokRAvdNr rsNyTabort ~
Btn_OK Btn_Cancel fi-cMessage fi-cNumTotal fi-cNumSelected 
&Scoped-Define DISPLAYED-OBJECTS rsAlle fiRAvdNr rsNyTabort fi-cMessage ~
...
DEFINE VARIABLE fiRAvdNr AS INTEGER FORMAT "zz9" INITIAL ? 
     LABEL "Vareområde" 
     VIEW-AS FILL-IN 
     SIZE 12.6 BY 1.
...
DEFINE VARIABLE rsNyTabort AS INTEGER 
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS 
          "Lägg till", 1,
"Ta bort", 2
...
DEFINE FRAME Dialog-Frame
     ...
     fiRAvdNr AT ROW 3.86 COL 67 COLON-ALIGNED HELP
          "Størrelsestype"
     B-SokRAvdNr AT ROW 3.86 COL 82
     rsNyTabort AT ROW 5.76 COL 32 NO-LABEL
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Endre vareområde */
DO:
...
&Scoped-define SELF-NAME B-SokRAvdNr
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-SokRAvdNr Dialog-Frame
ON CHOOSE OF B-SokRAvdNr IN FRAME Dialog-Frame /* ... */
OR "MOUSE-SELECT-DBLCLICK":U OF B-SokRAvdNr
OR "F10" OF B-SokRAvdNr
DO:
  DEF VAR cLookupValue AS CHAR NO-UNDO.

  cLookupValue = "RAvdNr".

  RUN JBoxDLookup.w ("Regnskapsavdeling;RAvdNr;RAvdBeskrivelse|Beskrivelse", "where true", INPUT-OUTPUT cLookupValue).

  IF cLookupValue NE "" THEN DO: 
    fiRAvdNr:SCREEN-VALUE = cLookupValue.
    APPLY "return" TO fiRAvdNr.
  END.
END.
...
DO:
  ASSIGN fiRAvdNr rsAlle rsNyTabort.
  ASSIGN cValues = (IF fiRAvdNr <> ?
                      THEN STRING(fiRAvdNr) + "|" + STRING(rsNyTabort) 
                      ELSE "?")
         oiReturn = rsAlle
         .
END.
...
------------------------------------------------------------------------------*/
  DISPLAY rsAlle fiRAvdNr rsNyTabort fi-cMessage fi-cNumTotal fi-cNumSelected 
      WITH FRAME Dialog-Frame.
  ENABLE rsAlle fiRAvdNr B-SokRAvdNr rsNyTabort Btn_OK Btn_Cancel fi-cMessage 
         fi-cNumTotal fi-cNumSelected 
      WITH FRAME Dialog-Frame.
