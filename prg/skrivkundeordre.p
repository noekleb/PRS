...
/*       RUN pdf_text_xy_dec ("Spdf",cAdresse2,iLeftMarg + 200,dY). */
    ...
    DO:
/*       ASSIGN cWrk = "Kund har godk�nt att f� 250kr i rabatt pga av m�rk fl�ck p� h�gerskon. Detta �r en test av l�nga texter. Jag vet inte hur m�nga rader man f�r plats med. Kanske bara ytterligare en rad.".*/
...
/*    RUN pdf_text_xy_dec ("Spdf",TRIM(hTTHodeBuff:BUFFER-FIELD("LevLand"):BUFFER-VALUE),iLeftMarg + 75,dY).*/
    ...
    RUN pdf_text_xy_dec ("Spdf",":",iLeftMarg + 70,dY).
    RUN pdf_text_xy_dec ("Spdf",cDate,iLeftMarg + 75,dY).
/*    RUN pdf_text_xy_dec ("Spdf","V�r ref",iLeftMarg + 5,dY).
    RUN pdf_text_xy_dec ("Spdf",":",iLeftMarg + 70,dY).
...
/*            cFaktNr = IF cFaktNr = ? THEN "" ELSE cFaktNr                            */
    ...
    /* Kundeadress */
      ...
      "<R+1><C8><P10>" TRIM(hTTHodeBuff:BUFFER-FIELD("FaktLand"):BUFFER-VALUE)
    /* Referenser */
      "<R17><C6><P7>V�r ref<C12>: "     TRIM(hTTHodeBuff:BUFFER-FIELD("VaarRef"):BUFFER-VALUE)   SKIP
