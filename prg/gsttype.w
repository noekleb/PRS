...
DEFINE VARIABLE h_bsttype AS HANDLE NO-UNDO.
DEFINE VARIABLE h_dsttype AS HANDLE NO-UNDO.
...
DEFINE FRAME gDialog
     Btn_OK AT ROW 15.52 COL 1
     Btn_Cancel AT ROW 15.52 COL 33
     SPACE(0.19) SKIP(0.04)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste statistikktype"
...
ON GO OF FRAME gDialog /* Søkeliste statistikktype */
DO:
      ASSIGN    
          cReturn-Value = "OK"    
          .
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL gDialog gDialog
ON WINDOW-CLOSE OF FRAME gDialog /* Søkeliste statistikktype */
DO:  
...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             INPUT  'sdo/dsttype.wDB-AWARE':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'AppServiceASUsePromptASInfoForeignFieldsRowsToBatch200CheckCurrentChangedyesRebuildOnReposnoServerOperatingModeNONEDestroyStatelessnoDisconnectAppServernoObjectNamedsttypeUpdateFromSourceno':U ,
             OUTPUT h_dsttype ).
       RUN repositionObject IN h_dsttype ( 14.81 , 22.00 ) NO-ERROR.
       /* Size in AB:  ( 1.86 , 10.80 ) */

       RUN constructObject (
             INPUT  'prg/bsttype.w':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'ScrollRemotenoDataSourceNamesUpdateTargetNamesHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_bsttype ).
       RUN repositionObject IN h_bsttype ( 2.91 , 1.00 ) NO-ERROR.
       RUN resizeObject IN h_bsttype ( 12.38 , 47.00 ) NO-ERROR.
       ...
       /* Links to SmartDataBrowser h_bsttype. */
       RUN addLink ( h_dsttype , 'Data':U , h_bsttype ).
       RUN addLink ( h_bsttype , 'Update':U , h_dsttype ).

       /* Links to SmartObject h_sortsok. */
       RUN addLink ( h_bsttype , 'Sortera':U , h_sortsok ).

       /* Adjust the tab order of the smart objects. */
       RUN adjustTabOrder ( h_sortsok ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjustTabOrder ( h_bsttype ,
             h_sortsok , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
 ...
 IF cReturn-Value <> "AVBRYT" THEN  
  ASSIGN
      cColValues = DYNAMIC-FUNCTION('colValues':U IN h_dsttype,
                     INPUT "StTypeId,Beskrivelse" /* CHARACTER */).  
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  PUBLISH "Sortera" FROM h_bsttype.

  DYNAMIC-FUNCTION('findRowWhere':U IN h_dsttype,
     INPUT cFelt    + (IF cFelt = "" THEN "" ELSE ",") + "StTypeId",
     INPUT cVerdier + (IF cVerdier = "" THEN "" ELSE CHR(1)) + cStart,
     INPUT "EQ,EQ" /* CHARACTER */).
  /* Signalerer til de andre objektene. */
  RUN dataAvailable IN h_dsttype
