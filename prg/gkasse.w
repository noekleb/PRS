...
DEFINE FRAME gDialog
     ...
     Btn_Cancel AT ROW 18.38 COL 59
     SPACE(0.00) SKIP(0.18)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste kassaregister"
...
ON GO OF FRAME gDialog /* Søkeliste kassaregister */
DO:
      ASSIGN
      cReturn-Value = "OK"
      .
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL gDialog gDialog
ON WINDOW-CLOSE OF FRAME gDialog /* Søkeliste kassaregister */
DO:  
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             ...
             INPUT  'AppServiceASUsePromptASInfoForeignFieldsRowsToBatch200CheckCurrentChangedyesRebuildOnReposnoServerOperatingModeNONEDestroyStatelessnoDisconnectAppServernoObjectNamedkasseUpdateFromSourceno':U ,
             OUTPUT h_dkasse ).
       RUN repositionObject IN h_dkasse ( 17.67 , 28.00 ) NO-ERROR.
       ...
       RUN constructObject (
             ...
             OUTPUT h_bkasse ).
       RUN repositionObject IN h_bkasse ( 2.91 , 1.00 ) NO-ERROR.
       RUN resizeObject IN h_bkasse ( 15.24 , 73.00 ) NO-ERROR.
       ...
       RUN addLink ( h_bkasse , 'Update':U , h_dkasse ).

       /* Links to SmartObject h_sortsok. */
       RUN addLink ( h_bkasse , 'Sortera':U , h_sortsok ).

       /* Adjust the tab order of the smart objects. */
       RUN adjustTabOrder ( h_bkasse ,
...
------------------------------------------------------------------------------*/
  ...
  IF cReturn-Value <> "AVBRYT" THEN  
    ASSIGN
      cColValues = DYNAMIC-FUNCTION('colValues':U IN h_dkasse,
                     INPUT "KasseNr,Navn" /* CHARACTER */).  
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  PUBLISH "Sortera" FROM h_bkasse.
  
  DYNAMIC-FUNCTION('findRowWhere':U IN h_dkasse,
     INPUT cFelt     + (IF cFelt = "" THEN "" ELSE ",") + "KasseNr",
     INPUT cVerdier  + (IF cVerdier = "" THEN "" ELSE CHR(1)) + cStart,
     INPUT cOperator + (IF cOperator = "" THEN "" ELSE ",") + ">=").
  /* Signalerer til de andre objektene. */
  RUN dataAvailable IN h_dkasse
