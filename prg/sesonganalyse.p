...
------------------------------------------------------------------------------*/
  DEFINE INPUT  PARAMETER cArtModell AS CHARACTER   NO-UNDO.
  ...
  /* cArtModell: 1=Artiklar, 2=Modeller */
...
/*                           ENTRY(NUM-ENTRIES(TT_Artiklar.Verdier),TT_Artiklar.Verdier) = cTmpEntry. */
                      ...
                      IF iLookup > 0 AND cArtModell = "1" THEN DO:
                          /* BUT */
                          ...
                          ENTRY(NUM-ENTRIES(TT_Artiklar.Verdier),TT_Artiklar.Verdier) = cTmpEntry.
                      END.
                      ELSE IF iLookup > 0 AND cArtModell = "2" THEN DO:
                          /* BUT */
                          cTmpEntry = ENTRY(iLookup,TT_Artiklar.Verdier).
                          ENTRY(iSasong,cTmpEntry,";") = STRING(INT(cTmpEntry) + INT(cTotAntStr)).
                          ENTRY(iLookup,TT_Artiklar.Verdier) = cTmpEntry.
                          /* TOT */
                          cTmpEntry = ENTRY(NUM-ENTRIES(TT_Artiklar.Verdier),TT_Artiklar.Verdier).
                          ENTRY(iSasong,cTmpEntry,";") = STRING(INT(cTmpEntry) + INT(cTotAntStr)).
...
&IF DEFINED(EXCLUDE-Bestanalys1Org) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Bestanalys1Org Procedure 
PROCEDURE Bestanalys1Org :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes: 
...
&IF DEFINED(EXCLUDE-Salgsanalys1Org) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Salgsanalys1Org Procedure 
PROCEDURE Salgsanalys1Org :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes: 
