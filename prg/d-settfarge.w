...
&Scoped-Define ENABLED-OBJECTS rsAlle fiFargNr B-SokFargNr Btn_OK ~
Btn_Cancel fi-cMessage fi-cNumTotal fi-cNumSelected 
&Scoped-Define DISPLAYED-OBJECTS rsAlle fiFargNr fi-cMessage fi-cNumTotal ~
fi-cNumSelected 
...
DEFINE VARIABLE fiFargNr AS INTEGER FORMAT "zz9" INITIAL ? 
     LABEL "Farge" 
     VIEW-AS FILL-IN 
     SIZE 12.6 BY 1.
...
DEFINE FRAME Dialog-Frame
     ...
     fiFargNr AT ROW 3.86 COL 67 COLON-ALIGNED HELP
          "Produsent"
     B-SokFargNr AT ROW 3.86 COL 82
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Endre farge */
DO:
...
&Scoped-define SELF-NAME B-SokFargNr
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-SokFargNr Dialog-Frame
ON CHOOSE OF B-SokFargNr IN FRAME Dialog-Frame /* ... */
OR "MOUSE-SELECT-DBLCLICK":U OF B-SokFargNr
OR "F10" OF B-SokFargNr
DO:
  DEF VAR cLookupValue AS CHAR NO-UNDO.

  cLookupValue = "Farg,FarBeskr".

  RUN JBoxDLookup.w ("Farg;FarBeskr;Farg", "where true", INPUT-OUTPUT cLookupValue).

  IF cLookupValue NE "" THEN DO: 
    fiFargNr:SCREEN-VALUE = cLookupValue.
    APPLY "return" TO fiFargNr.
  END.
END.
...
DO:
  ASSIGN fiFargNr rsAlle.
  ASSIGN cValues = (IF fiFargNr <> ?
                      THEN STRING(fiFargNr)
                      ELSE "?")
         oiReturn = rsAlle
         .
END.
...
------------------------------------------------------------------------------*/
  DISPLAY rsAlle fiFargNr fi-cMessage fi-cNumTotal fi-cNumSelected 
      WITH FRAME Dialog-Frame.
  ENABLE rsAlle fiFargNr B-SokFargNr Btn_OK Btn_Cancel fi-cMessage fi-cNumTotal 
         fi-cNumSelected 
      WITH FRAME Dialog-Frame.
