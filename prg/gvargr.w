...
&IF DEFINED(UIB_IS_RUNNING) <> 0 &THEN          
  ...
  ASSIGN
      ...
      .
&ELSE
  DEF INPUT-OUTPUT PARAMETER iVg AS INT NO-UNDO.
&ENDIF

/* Local Variable Definitions ---                                       */
DEF VAR cVgBeskr AS CHAR INITIAL "AVBRYT" NO-UNDO.
...
DEFINE VARIABLE h_bvargr AS HANDLE NO-UNDO.
...
DEFINE FRAME gDialog
     ...
     Btn_Cancel AT ROW 23.62 COL 113
...
DO:
  ASSIGN
      iVg      = int(entry(2,DYNAMIC-FUNCTION('colValues':U IN h_dvargr,
                    INPUT "Vg" /* CHARACTER */),CHR(1)))
      cVgBeskr = entry(2,DYNAMIC-FUNCTION('colValues':U IN h_dvargr,
                    INPUT "VgBeskr" /* CHARACTER */),CHR(1))
      .
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             ...
             OUTPUT h_dvargr ).
       RUN repositionObject IN h_dvargr ( 7.67 , 94.00 ) NO-ERROR.
       /* Size in AB:  ( 1.86 , 10.80 ) */

       RUN constructObject (
             INPUT  'prg/bvargr.w':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'ScrollRemotenoNumDown0CalcWidthnoMaxWidth80FetchOnReposToEndyesDataSourceNamesUpdateTargetNamesLogicalObjectNameHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_bvargr ).
       RUN repositionObject IN h_bvargr ( 2.91 , 1.00 ) NO-ERROR.
       RUN resizeObject IN h_bvargr ( 20.48 , 127.00 ) NO-ERROR.
       ...
       /* Links to SmartDataBrowser h_bvargr. */
       RUN addLink ( h_dvargr , 'Data':U , h_bvargr ).
       RUN addLink ( h_bvargr , 'Update':U , h_dvargr ).

       /* Links to SmartObject h_sortsok. */
       RUN addLink ( h_bvargr , 'Sortera':U , h_sortsok ).

       /* Adjust the tab order of the smart objects. */
       RUN adjustTabOrder ( h_sortsok ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjustTabOrder ( h_bvargr ,
             h_sortsok , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  SUBSCRIBE "PostValgt" IN h_bvargr.
  PUBLISH "Sortera" FROM h_bvargr.
