...
DEFINE VARIABLE h_bstrtstr AS HANDLE NO-UNDO.
DEFINE VARIABLE h_dstrtstr AS HANDLE NO-UNDO.
...
DEFINE FRAME gDialog
     Btn_OK AT ROW 13.86 COL 1
     Btn_Cancel AT ROW 13.86 COL 18
...
ON GO OF FRAME gDialog /* Søkeliste størrelser */
DO:
      ASSIGN
      cReturn-Value = "OK"
      .
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL gDialog gDialog
ON WINDOW-CLOSE OF FRAME gDialog /* Søkeliste størrelser */
DO:  
...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             INPUT  'sdo/dstrtstr.wDB-AWARE':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'AppServiceASUsePromptASInfoForeignFieldsRowsToBatch200CheckCurrentChangedyesRebuildOnReposnoServerOperatingModeNONEDestroyStatelessnoDisconnectAppServernoObjectNamedstrtstrUpdateFromSourceno':U ,
             OUTPUT h_dstrtstr ).
       RUN repositionObject IN h_dstrtstr ( 7.67 , 17.00 ) NO-ERROR.
       /* Size in AB:  ( 1.91 , 11.00 ) */

       RUN constructObject (
             INPUT  'prg/bstrtstr.w':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'ScrollRemotenoDataSourceNamesUpdateTargetNamesHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_bstrtstr ).
       RUN repositionObject IN h_bstrtstr ( 2.91 , 1.00 ) NO-ERROR.
       RUN resizeObject IN h_bstrtstr ( 10.71 , 14.00 ) NO-ERROR.
       ...
       /* Links to SmartDataBrowser h_bstrtstr. */
       RUN addLink ( h_dstrtstr , 'Data':U , h_bstrtstr ).
       RUN addLink ( h_bstrtstr , 'Update':U , h_dstrtstr ).

       /* Links to SmartObject h_sortsok. */
       RUN addLink ( h_bstrtstr , 'Sortera':U , h_sortsok ).

       /* Adjust the tab order of the smart objects. */
       RUN adjustTabOrder ( h_bstrtstr ,
...
------------------------------------------------------------------------------*/
  ...
  IF cReturn-Value <> "AVBRYT" THEN  
    ASSIGN
      cColValues = DYNAMIC-FUNCTION('colValues':U IN h_dstrtstr,
                     INPUT "SoStorl" /* CHARACTER */).  
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  PUBLISH "Sortera" FROM h_bstrtstr.
  
  DYNAMIC-FUNCTION('findRowWhere':U IN h_dstrtstr,
     INPUT cFelt    + (IF cFelt = "" THEN "" ELSE ",") + "Sostorl",
     INPUT cVerdier + (IF cVerdier = "" THEN "" ELSE CHR(1)) + cStart,
     INPUT cOperator + (IF cOperator = "" THEN "" ELSE "," + ">=") /* CHARACTER */).
  /* Signalerer til de andre objektene. */
  RUN dataAvailable IN h_dstrtstr
