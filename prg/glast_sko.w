...
DEFINE VARIABLE h_blast_sko AS HANDLE NO-UNDO.
DEFINE VARIABLE h_dlast_sko AS HANDLE NO-UNDO.
...
DEFINE FRAME gDialog
     ...
     Btn_Cancel AT ROW 13.86 COL 36
...
ON GO OF FRAME gDialog /* Søkeliste læstkoder */
DO:
       ASSIGN    
          cReturn-Value = "OK"    
          .
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL gDialog gDialog
ON WINDOW-CLOSE OF FRAME gDialog /* Søkeliste læstkoder */
DO:  
...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             INPUT  'sdo/dlast_sko.wDB-AWARE':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'AppServiceASUsePromptASInfoForeignFieldsRowsToBatch200CheckCurrentChangedyesRebuildOnReposnoServerOperatingModeNONEDestroyStatelessnoDisconnectAppServernoObjectNamedlast_skoUpdateFromSourceno':U ,
             OUTPUT h_dlast_sko ).
       RUN repositionObject IN h_dlast_sko ( 13.62 , 22.00 ) NO-ERROR.
       /* Size in AB:  ( 1.57 , 10.80 ) */

       RUN constructObject (
             INPUT  'prg/blast_sko.w':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'ScrollRemotenoDataSourceNamesUpdateTargetNamesHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_blast_sko ).
       RUN repositionObject IN h_blast_sko ( 2.91 , 1.00 ) NO-ERROR.
       RUN resizeObject IN h_blast_sko ( 10.71 , 50.00 ) NO-ERROR.
       ...
       /* Links to SmartDataBrowser h_blast_sko. */
       RUN addLink ( h_dlast_sko , 'Data':U , h_blast_sko ).
       RUN addLink ( h_blast_sko , 'Update':U , h_dlast_sko ).

       /* Links to SmartObject h_sortsok. */
       RUN addLink ( h_blast_sko , 'Sortera':U , h_sortsok ).

       /* Adjust the tab order of the smart objects. */
       RUN adjustTabOrder ( h_blast_sko ,
...
------------------------------------------------------------------------------*/
  ...
  IF cReturn-Value <> "AVBRYT" THEN  
   ASSIGN
       cColValues = DYNAMIC-FUNCTION('colValues':U IN h_dlast_sko,
                      INPUT "Last-Id,LastBeskr" /* CHARACTER */).  
  RUN SUPER.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  PUBLISH "Sortera" FROM h_blast_sko.

  DYNAMIC-FUNCTION('findRowWhere':U IN h_dlast_sko,
     INPUT cFelt    + (IF cFelt = "" THEN "" ELSE ",") + "Last-Id",
     INPUT cVerdier + (IF cVerdier = "" THEN "" ELSE CHR(1)) + cStart,
     INPUT "EQ,EQ" /* CHARACTER */).
  /* Signalerer til de andre objektene. */
  RUN dataAvailable IN h_dlast_sko
