...
DEFINE VARIABLE iColLbl      AS INTEGER EXTENT 7         NO-UNDO. /* s�tts i SkrivRapportPDF */
...
IF iUtskrTyp = 0 OR iUtskrTyp = 1 THEN
  RUN SkrivRapportPDF. /* svensk layout Brukes ogs� hos Gant*/
...
/*   RUN pdf_Wrap_Text ("Spdf","Vid betalning efter f�rfallodatum debiteras dr�jsm�lsr�nta med  2% per m�nad. �garf�rbeh�ll: levererade varor f�rblir s�ljarens egendom till full betalning skett.",15,125,"left",OUTPUT iY). */
...
/*          iCol[3] = 340 */
  ...
  ASSIGN cLabel[1] = "Netto"
         cLabel[2] = "Exkl moms"
/*          cLabel[3] = "Moms %"  Moms% meningsl�s vid flera momssatser  */
         cLabel[4] = "Moms kr"
         cLabel[5] = "ATT BETALA".
  ELSE 
  ASSIGN cLabel[1] = "Netto"
...
IF CAN-DO("SVE,SE",TRIM(cSprak)) THEN
  ASSIGN cLabel[1] = "Artnr"
         ...
         cLabel[3] = "Lev ant"
         cLabel[4] = "Enh"
         cLabel[5] = "�-pris ex moms"
         cLabel[6] = "Rabatt"
         cLabel[7] = "Sum ink moms"
         cLabel[8] = "�-pris ink moms".
ELSE 
  ASSIGN cLabel[1] = "Artnr"
...
/*            cFaktNr = IF cFaktNr = ? THEN "" ELSE cFaktNr                            */
    ...
    /* Referenser */
      "<AT=70,><C6><P7>V�r ref<C12>: "     TRIM(hTTHodeBuff:BUFFER-FIELD("VaarRef"):BUFFER-VALUE) 
...
/*            hTTHodeBuff:BUFFER-FIELD("Faktura_id"):BUFFER-VALUE */
  ...
  DO:
    ...
    ASSIGN cRub1[1] = "Faktnr"
           ...
           cRub1[6] = "Er referens"
           cRub1[7] = "V�r referens"
           cRub1[8] = "Ert ordernr"
           cRub1[9] = "Betalningsvillkor"
           cRub1[10] = "Leveransvillkor"
           cRub1[11] = "F�rfallodatum"
           cRub1[12] = "Leveranss�tt"
           cRub1[13] = "Dr�jsm�lsr�nta".
  END.
  ELSE DO:
    ...
    ASSIGN cRub1[1] = "Faktnr"
           ...
           cRub1[9] = "Betalingsvilk�r"
           cRub1[10] = "Leveringvilk�r"
           cRub1[11] = "Forfallsdato"
           cRub1[12] = "Leveringsm�e"
...
/*     RUN pdf_text_xy_dec ("Spdf","Ert ordernr",iLeftCol,iPageHeight - 218). */
    ...
    IF CAN-DO("SVE,SE",TRIM(cSprak)) THEN
      ASSIGN cStr = REPLACE(hTTHodeBuff:BUFFER-FIELD("LevFormBeskrivelse"):BUFFER-VALUE,"Butikksalg","Butiksf�rs�ljning").
    ELSE
...
--*/
   ...
   /* anv�nds i SkrivColLabels och h�r */
   ASSIGN iLeftCol   = 50
          iColLbl[1] = iLeftCol
          iColLbl[2] = 130
          iColLbl[3] = 317
   ...
   REPEAT WHILE NOT qH:QUERY-OFF-END:
       ...
       CASE iBilagstype:
         ...
         WHEN 10 THEN ASSIGN cFakturaNr   = ""
                             cFakturaType = "Betalningsp�minnelse". 
         OTHERWISE ASSIGN cFakturaNr   = ""
                          cFakturaType = "". 
       END CASE.
       /* ref til pakkseddel n�r ikke referansetekster vises i layout. */
       ...
       DO:
         ...
         RUN RitaRamar. /* pt kvar att g�ra */
         RUN SkrivHeaderPDF(iSidnr,cFakturaNr,cFakturaType,FALSE).
         RUN pdf_set_VerticalSpace IN h_PDFinc ("Spdf",13).
         /* H�r skriver vi ut notat. F�rst s�tter vi Y */
         RUN pdf_set_TextY("Spdf",120).
...
/*           cTxt = TRIM(STRING(DEC(hTTLinjeBuff:BUFFER-FIELD("nettolinjesum":BUFFER-VALUE + dec(hTTLinjeBuff:BUFFER-FIELD("Mvakr"):BUFFER-VALUE,"->>,>>9.99")).*/
         ...
         dY = dY + 4 + 12.
         IF dYorg < dY THEN DO:
           /* beklagar, men vi m�ste sidbryta */
           RUN pdf_new_page ("Spdf").
           RUN RitaRamar. /* pt kvar att g�ra */
           RUN SkrivHeaderPDF(iSidnr,cFakturaNr,cFakturaType,FALSE).
           RUN pdf_set_VerticalSpace IN h_PDFinc ("Spdf",13).
...
/*            END.                                                                                           */
   ...
   /* Kallende rutine skal bare ha filen, utskrift skal ikke gj�res. */
   IF bBareFil THEN DO:
       FILE-INFO:FILENAME = cFilNavn.
       PUBLISH 'fakturaFilNavn' (FILE-INFO:FULL-PATHNAME). 
     END.
   /* Faktura skal skrives direkte ut p� skjerm. */
...
/*            RUN pdf_Wrap_Text ("Spdf","Vid betalning efter f�rfallodatum debiteras dr�jsm�lsr�nta med  2% per m�nad. �garf�rbeh�ll: levererade varor f�rblir s�ljarens egendom till full betalning skett.",15,125,"left",OUTPUT iY). */
           ...
           IF cVaruText = "" THEN
             RUN pdf_text_xy_dec ("Spdf","Varor enligt f�ljesedlar.",iColLbl[2],dY).
           ELSE
