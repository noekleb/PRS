...
&Scoped-Define ENABLED-OBJECTS rsAlle fiVaretype B-SokVaretype Btn_OK ~
Btn_Cancel fi-cMessage fi-cNumTotal fi-cNumSelected 
&Scoped-Define DISPLAYED-OBJECTS rsAlle fiVaretype fiLeveringstid ~
...
DEFINE VARIABLE fiLeveringstid AS INTEGER FORMAT ">9":U INITIAL 0 
     LABEL "Leveringstid" 
     VIEW-AS FILL-IN 
     SIZE 12.6 BY 1 NO-UNDO.

DEFINE VARIABLE fiVaretype AS INTEGER FORMAT "z9" INITIAL ? 
     LABEL "Varetype" 
     VIEW-AS FILL-IN 
     SIZE 12.6 BY 1.
...
DEFINE FRAME Dialog-Frame
     ...
     fiVaretype AT ROW 3.86 COL 67 COLON-ALIGNED HELP
          "Produsent"
     B-SokVaretype AT ROW 3.86 COL 82
     fiLeveringstid AT ROW 4.86 COL 67 COLON-ALIGNED 
     Btn_OK AT ROW 6.95 COL 64
     Btn_Cancel AT ROW 6.95 COL 80
...
/* SETTINGS FOR FILL-IN fiLeveringstid IN FRAME Dialog-Frame
   NO-ENABLE                                                            */
...
&Scoped-define SELF-NAME B-SokVaretype
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-SokVaretype Dialog-Frame
ON CHOOSE OF B-SokVaretype IN FRAME Dialog-Frame /* ... */
OR "MOUSE-SELECT-DBLCLICK":U OF B-SokVaretype
OR "F10" OF B-SokVaretype
DO:
  DEF VAR cLookupValue AS CHAR NO-UNDO.

  cLookupValue = "ParaNr".

  RUN JBoxDLookup.w ("Syspara;ParaNr;Beskrivelse", "where SysHId = '2' and SysGr = '11'", INPUT-OUTPUT cLookupValue).

  IF cLookupValue NE "" THEN DO: 
    fiVaretype:SCREEN-VALUE = cLookupValue.
    
    IF NOT CAN-DO('2,3',trim(cLookupValue)) THEN
      ASSIGN
        fiLeveringstid:SCREEN-VALUE = '0'
        fiLeveringstid:SENSITIVE = FALSE 
        .
    ELSE
      fiLeveringstid:SENSITIVE = TRUE. 
    
    APPLY "return" TO fiVareType.
  END.
END.
...
DO:
  ASSIGN fiVaretype rsAlle.
  ASSIGN cValues = (IF fiVaretype <> ?
                      THEN STRING(fiVaretype)
                      ELSE "?") + '|' +
                   fiLeveringstid:SCREEN-VALUE
         oiReturn = rsAlle
         .
END.
...
------------------------------------------------------------------------------*/
  DISPLAY rsAlle fiVaretype fiLeveringstid fi-cMessage fi-cNumTotal 
          fi-cNumSelected 
      WITH FRAME Dialog-Frame.
  ENABLE rsAlle fiVaretype B-SokVaretype Btn_OK Btn_Cancel fi-cMessage 
