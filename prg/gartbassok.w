...
&Scoped-Define ENABLED-OBJECTS Btn_OK Btn_Cancel Btn_Ny Btn_Help BtnVPISok 
&Scoped-Define DISPLAYED-OBJECTS FI-InfoTekst 
...
DEFINE VARIABLE FI-InfoTekst AS CHARACTER FORMAT "X(256)":U INITIAL "Bruk TAB ut av felt for å aktivere søk. RETURN for å velge artikkel." 
     VIEW-AS FILL-IN 
     SIZE 92 BY 1
...
DEFINE FRAME gDialog
     FI-InfoTekst AT ROW 12 COL 28.2 COLON-ALIGNED NO-LABEL
     Btn_OK AT ROW 26.24 COL 1
     Btn_Cancel AT ROW 26.24 COL 17
     Btn_Ny AT ROW 26.24 COL 62
     Btn_Help AT ROW 26.24 COL 140
     BtnVPISok AT ROW 11.95 COL 124
     SPACE(0.39) SKIP(14.29)
...
/* SETTINGS FOR FILL-IN FI-InfoTekst IN FRAME gDialog
   NO-ENABLE                                                            */
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             ...
             OUTPUT h_dartbassok ).
       RUN repositionObject IN h_dartbassok ( 10.52 , 128.00 ) NO-ERROR.
       ...
       RUN constructObject (
             ...
             OUTPUT h_fvisbilde ).
       RUN repositionObject IN h_fvisbilde ( 1.24 , 127.00 ) NO-ERROR.
       ...
       RUN constructObject (
             ...
             OUTPUT h_bartbassok ).
       RUN repositionObject IN h_bartbassok ( 13.38 , 1.00 ) NO-ERROR.
       RUN resizeObject IN h_bartbassok ( 12.38 , 154.00 ) NO-ERROR.

       RUN constructObject (
             INPUT  'prg/fartsokfilter.w':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'LogicalObjectNamePhysicalObjectNameDynamicObjectnoRunAttributeHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_fartsokfilter ).
       RUN repositionObject IN h_fartsokfilter ( 1.00 , 1.00 ) NO-ERROR.
       /* Size in AB:  ( 10.43 , 125.60 ) */
       ...
       RUN constructObject (
             ...
             OUTPUT h_sortsok ).
       RUN repositionObject IN h_sortsok ( 11.48 , 1.00 ) NO-ERROR.
       /* Size in AB:  ( 1.76 , 27.60 ) */

       /* Links to SmartDataObject h_dartbassok. */
       RUN addLink ( h_fartsokfilter , 'SokSdo':U , h_dartbassok ).

       /* Links to SmartFrame h_fvisbilde. */
       RUN addLink ( h_dartbassok , 'Data':U , h_fvisbilde ).
...
------------------------------------------------------------------------------*/
  DISPLAY FI-InfoTekst 
      WITH FRAME gDialog.
  ENABLE Btn_OK Btn_Cancel Btn_Ny Btn_Help BtnVPISok 
      WITH FRAME gDialog.
  VIEW FRAME gDialog.
...
------------------------------------------------------------------------------*/
 DEF VAR piLoop AS INT NO-UNDO.
  ...
  RUN SettFilterVal IN h_fartsokfilter (INPUT cFilter)/* NO-ERROR*/.
  /*
  DO WITH FRAME Dialog-Frame.
      FI-InfoTekst = "Bruk TAB ut av felt for å aktivere søk.".
      DISPLAY FI-InfoTekst.
      MESSAGE FI-InfoTekst
