...
IF iTilAar - iFraAar <= 2 THEN DO:
    DEFINE VARIABLE d1Budget     AS DATE        NO-UNDO.
    DEFINE VARIABLE d0Budget     AS DATE        NO-UNDO.
    DEFINE VARIABLE iBudgetAar      AS INTEGER     NO-UNDO.
    DEFINE VARIABLE iBudAarperlinnr AS INTEGER     NO-UNDO.
    DEFINE VARIABLE d1Stlinje       AS DATE        NO-UNDO.
    DEFINE VARIABLE d0Stlinje       AS DATE        NO-UNDO.
    DEFINE VARIABLE dFg0Stlinje       AS DATE        NO-UNDO.
    DEFINE VARIABLE iWeekday        AS INTEGER     NO-UNDO.
    DEFINE VARIABLE dTmp            AS DATE        NO-UNDO.
    DEFINE VARIABLE iStLinjeAar     AS INTEGER     NO-UNDO.
    DEFINE VARIABLE iStPerlinnr     AS INTEGER     NO-UNDO.
    DEFINE VARIABLE iStAarPerlinnr  AS INTEGER     NO-UNDO.
    iStLinjeAar = iFraAar.
    iBudgetAar = iTilAar.
    
    d1Budget = DATE(1,1,iBudgetAar).
    d0Budget = d1Budget - 1.

    d1Stlinje   = DATE(1,1,iStLinjeAar).
    d0Stlinje  = d1Stlinje - 1.
    iWeekday = WEEKDAY(d1Budget).
    
    DO dTmp = d1Stlinje TO d1Stlinje + 6:
        IF WEEKDAY(dTmp) = iWeekday THEN DO:
            d1StLinje = d1StLinje + DAY(dTmp) - DAY(d1Budget).
        END.
    END.
    d1StLinje = d1StLinje - 1. /* vi startar med att räkna upp 1 dag i loopen */
    DO d1Budget = DATE(1,1,iBudgetAar) TO DATE(12,31,iBudgetAar):
        d1StLinje = d1StLinje + 1.
        iStPerlinnr  = d1Stlinje - d0Stlinje.
        iStAarPerlinnr = iStLinjeAar * 1000 + iStPerlinnr.
        IF YEAR(d1StLinje) > iStLinjeAar THEN DO:
            /* här skall vi sätta perioden till året innan stlinjeår och budgetens man/dag */
            d1Stlinje   = DATE(1,1,iStLinjeAar).
            iStPerlinnr  = d1Stlinje - d0Stlinje.
            iStAarPerlinnr = iStLinjeAar * 1000 + iStPerlinnr.
/*             dFg0Stlinje = DATE(12,31,iStLinjeAar - 2).                  */
/*             dTmp = DATE(MONTH(d1Budget),DAY(d1Budget),iStLinjeAar - 1). */
/*             iStPerlinnr = dTmp - dFg0Stlinje.                           */
/*             iStAarPerlinnr = (iStLinjeAar - 1) * 1000 + iStPerlinnr.    */
        END.
/* OUTPUT TO c:\tmp\bud.txt APPEND.     */
/* PUT UNFORMATTED iStAarPerlinnr SKIP. */
/* OUTPUT CLOSE.                        */
        FIND StLinje WHERE StLinje.Butik    = iStatButNr AND 
                           StLinje.StTypeId = "BUTSTAT" AND 
                           StLinje.PerId    = "DAG" AND
                           StLinje.AarPerLinnr = iStAarPerlinnr NO-LOCK NO-ERROR.
        IF AVAIL StLinje THEN DO:
            ASSIGN 
                iAarMnd    = INT(STRING(iTilAar) + STRING(MONTH(d1Budget),"99")) 
                iAarMndDag = INT(STRING(iTilAar) + STRING(MONTH(d1Budget),"99") + STRING(DAY(d1Budget),"99")) 
                .      
