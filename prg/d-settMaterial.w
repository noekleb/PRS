...
&Scoped-Define ENABLED-OBJECTS rsAlle fiMaterialNr B-SokMaterialNr Btn_OK ~
Btn_Cancel fi-cMessage fi-cNumTotal fi-cNumSelected 
&Scoped-Define DISPLAYED-OBJECTS rsAlle fiMaterialNr fi-cMessage ~
...
DEFINE VARIABLE fiMaterialNr AS INTEGER FORMAT "z9" INITIAL ? 
     LABEL "Material" 
     VIEW-AS FILL-IN 
     SIZE 12.6 BY 1.
...
DEFINE FRAME Dialog-Frame
     ...
     fiMaterialNr AT ROW 3.86 COL 67 COLON-ALIGNED HELP
          "Produsent"
     B-SokMaterialNr AT ROW 3.86 COL 82
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Endre material */
DO:
...
&Scoped-define SELF-NAME B-SokMaterialNr
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-SokMaterialNr Dialog-Frame
ON CHOOSE OF B-SokMaterialNr IN FRAME Dialog-Frame /* ... */
OR "MOUSE-SELECT-DBLCLICK":U OF B-SokMaterialNr
OR "F10" OF B-SokMaterialNr
DO:
  DEF VAR cLookupValue AS CHAR NO-UNDO.

  cLookupValue = "MatKod".

  RUN JBoxDLookup.w ("Material;MatBeskr;MatKod", "where true", INPUT-OUTPUT cLookupValue).

  IF cLookupValue NE "" THEN DO: 
    fiMaterialNr:SCREEN-VALUE = cLookupValue.
    APPLY "return" TO fiMaterialNr.
  END.
END.
...
DO:
  ASSIGN fiMaterialNr rsAlle.
  ASSIGN cValues = (IF fiMaterialNr <> ?
                      THEN STRING(fiMaterialNr)
                      ELSE "?")
         oiReturn = rsAlle
         .
END.
...
------------------------------------------------------------------------------*/
  DISPLAY rsAlle fiMaterialNr fi-cMessage fi-cNumTotal fi-cNumSelected 
      WITH FRAME Dialog-Frame.
  ENABLE rsAlle fiMaterialNr B-SokMaterialNr Btn_OK Btn_Cancel fi-cMessage 
         fi-cNumTotal fi-cNumSelected 
      WITH FRAME Dialog-Frame.
