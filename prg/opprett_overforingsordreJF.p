...
DEFINE INPUT PARAMETER bOppdater      AS LOG NO-UNDO.
...
/* Om cRowId <> "" betyder det att vi önskar att göra ny överföring på specifik rad */
/* cKOrdre_Id_Lst sätts till Kordre_id och vid skapande av överföring så hoppas det över alla andra rader än rKOLinjeRowId */
IF cRowId <> "" THEN DO:
    rKOLinjeRowId = TO-ROWID(cRowId).
    FIND KOrdrelinje WHERE ROWID(KOrdrelinje) = rKOLinjeRowId NO-LOCK NO-ERROR.
    IF NOT AVAIL KOrdrelinje THEN
        RETURN.
    cKOrdre_Id_Lst = STRING(KOrdreLinje.KOrdre_id).
    RELEASE KOrdreLinje.
END.
    ...
    IF LAST-OF(ttt_OvBuffer.ButikkNrFra) THEN 
      ...
      DO:
        ...
        RUN LagraOvBuffer.p (INPUT-OUTPUT iBuntNr,
                           0,
                           'J' + CHR(1) + "Ordrenr. nettbutikk: " + KOrdreHode.EkstOrdreNr,
                           wEDB-System,
                           wTabell,
                           8).
...
/*------------------------------------------------------------------------------
  ...
  FOR EACH KOrdreLinje OF KOrdreHode NO-LOCK WHERE
    ...
    IF cRowId <> "" AND ROWID(KOrdrelinje) <> rKOLinjeRowId THEN
        NEXT.
