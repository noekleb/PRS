...
------------------------------------------------------------------------------*/
  ...
  ASSIGN 
      ...
      cItemIdTxt    = IF cItemIdTxt = "" AND CAN-DO("70,71,72,73,74,79,78",STRING(piHuvGr)) THEN "ITEMID=" + STRING(TT_ItemLine.ItemId) ELSE cItemIdTxt
  ...
  DO:
      ...
      IF NOT AVAIL StrekKode THEN DO:
          IF LENGTH(cStrekKode) < 13 THEN
              FIND StrekKode WHERE StrekKode.Kode = FILL("0",13 - LENGTH(StrekKode.kode)) + cStrekKode NO-LOCK NO-ERROR.
  ...
  DO:
      ...
      DO:
          ...
          ASSIGN
              ...
              Strekkode.Kode       = STRING(TT_ItemLine.BarCode)
              Strekkode.StrKode    = 1
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH tmpBongLinje WHERE tmpBongLinje.b_id = tmpBongHode.b_id AND 
                                CAN-DO("1,3,10",STRING(tmpBongLinje.ttId)) AND 
                                tmpBongLinje.KampId <> 0 AND tmpbonglinje.makulert = FALSE:
        FIND tt_tilbud WHERE tt_tilbud.kampid     = tmpBonglinje.kampid AND
        ...
        IF NOT lOK THEN
            NEXT.
        FOR EACH tt_tilbud WHERE tt_tilbud.kampid = tmpbonglinje.kampid AND
            ...
            IF CAN-FIND(tt_tilbudart WHERE tt_tilbudart.prodfamid = tt_tilbud.prodfamid AND
                                           tt_tilbudart.artikkelnr = DECI(tmpbonglinje.artikkelnr)) THEN DO:
    ...
    FOR EACH tt_tilbud BREAK BY tt_tilbud.kampid BY tt_tilbud.kamptilbId:
        ...
        END.
        IF LAST-OF(tt_tilbud.kamptilbid) AND tt_tilbud.Solgtantall > 0 THEN DO:
