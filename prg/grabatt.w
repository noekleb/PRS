...
DEFINE VARIABLE h_brabatt AS HANDLE NO-UNDO.
DEFINE VARIABLE h_drabatt AS HANDLE NO-UNDO.
...
DEFINE FRAME gDialog
     ...
     Btn_Cancel AT ROW 15.52 COL 52
...
ON GO OF FRAME gDialog /* Søkeliste rabattkoder */
DO:
        ASSIGN    
          cReturn-Value = "OK"    
          .
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL gDialog gDialog
ON WINDOW-CLOSE OF FRAME gDialog /* Søkeliste rabattkoder */
DO:  
...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             INPUT  'sdo/drabatt.wDB-AWARE':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'AppServiceASUsePromptASInfoForeignFieldsRowsToBatch200CheckCurrentChangedyesRebuildOnReposnoServerOperatingModeNONEDestroyStatelessnoDisconnectAppServernoObjectNamedrabattUpdateFromSourceno':U ,
             OUTPUT h_drabatt ).
       RUN repositionObject IN h_drabatt ( 14.57 , 16.00 ) NO-ERROR.
       /* Size in AB:  ( 1.86 , 10.80 ) */

       RUN constructObject (
             INPUT  'prg/brabatt.w':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'ScrollRemotenoDataSourceNamesUpdateTargetNamesHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_brabatt ).
       RUN repositionObject IN h_brabatt ( 2.91 , 1.00 ) NO-ERROR.
       RUN resizeObject IN h_brabatt ( 12.38 , 66.00 ) NO-ERROR.
       ...
       /* Links to SmartDataBrowser h_brabatt. */
       RUN addLink ( h_drabatt , 'Data':U , h_brabatt ).
       RUN addLink ( h_brabatt , 'Update':U , h_drabatt ).

       /* Links to SmartObject h_sortsok. */
       RUN addLink ( h_brabatt , 'Sortera':U , h_sortsok ).

       /* Adjust the tab order of the smart objects. */
       RUN adjustTabOrder ( h_brabatt ,
...
------------------------------------------------------------------------------*/
  ...
  IF cReturn-Value <> "AVBRYT" THEN  
   ASSIGN
       cColValues = DYNAMIC-FUNCTION('colValues':U IN h_drabatt,
                      INPUT "RabKod,RabProc" /* CHARACTER */).  
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  PUBLISH "Sortera" FROM h_brabatt.

  DYNAMIC-FUNCTION('findRowWhere':U IN h_drabatt,
     INPUT cFelt    + (IF cFelt = "" THEN "" ELSE ",") + "RabKod",
     INPUT cVerdier + (IF cVerdier = "" THEN "" ELSE CHR(1)) + cStart,
     INPUT "EQ,EQ" /* CHARACTER */).
  /* Signalerer til de andre objektene. */
  RUN dataAvailable IN h_drabatt
