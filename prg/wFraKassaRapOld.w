...
DEFINE VARIABLE cSEfolder AS CHARACTER INIT
 "Avdelning|Huvudgrupp|Varugrupp|Leverantör|Säljare|Timfsg|Varufsg|Butik"     
                 NO-UNDO.
...
TT_Selger TT_akt_rapp TT_VareSalg TT_Vg
...
&Scoped-Define ENABLED-OBJECTS Btn_Help B-Excel BUTTON-1 RECT-3 RECT-1 ~
B-Oppdater RECT-2 B-Chart BUTTON-Ok RS-Type CB-Butik B-Artikkelkort 
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             ...
             INPUT  'FolderLabels':U + 'Avdeling|Hovedgrupper|Varegrupper|Leverandør|Selgere|Timesalg|Varesalg|Butikker' + 'FolderTabWidth0FolderFont-1HideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_folder ).
...
------------------------------------------------------------------------------*/
  ...
  ENABLE Btn_Help B-Excel BUTTON-1 RECT-3 RECT-1 B-Oppdater RECT-2 B-Chart 
         BUTTON-Ok RS-Type CB-Butik B-Artikkelkort 
      WITH FRAME fMain IN WINDOW wWin.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN B-Chart:SENSITIVE IN FRAME {&FRAME-NAME} = CAN-DO("1,2,3,4,5,6,7,8",STRING(piPageNum)).
  ...
  CASE iFlipNum:
      ...
      WHEN 5 THEN DO:
          ...
          ASSIGN hQry   = QUERY QSelger:HANDLE
              cLabels    = "Selgere,Beskrivelse,Solgt,Salgssum brutto,Salgssum netto,Rabatter,DB kr,DB%,Mva,Varekost,Returer,Returer kr,Kvitteringer"
              cFelter    = "selgernr,navn,AntSolgt,salgbrutto,VerdiSolgt,VerdiRabatt,DBkr,DB%,mvaverdi,vvarekost,GjenkjopAnt,GjenkjopVerdi,AntKunder"
