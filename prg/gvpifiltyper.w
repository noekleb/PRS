...
DEFINE FRAME gDialog
     ...
     Btn_Cancel AT ROW 11.95 COL 16.2
     Btn_Help AT ROW 11.95 COL 44
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste VPIFiltyper"
...
ON GO OF FRAME gDialog /* Søkeliste VPIFiltyper */
DO:
  ASSIGN    
      cReturn-Value = "OK".
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL gDialog gDialog
ON WINDOW-CLOSE OF FRAME gDialog /* Søkeliste VPIFiltyper */
DO:  
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             ...
             INPUT  'AppServiceASUsePromptASInfoForeignFieldsRowsToBatch200CheckCurrentChangedyesRebuildOnReposnoServerOperatingModeNONEDestroyStatelessnoDisconnectAppServernoObjectNamedvpifiltypeUpdateFromSourceno':U ,
             OUTPUT h_dvpifiltype ).
       RUN repositionObject IN h_dvpifiltype ( 1.00 , 30.00 ) NO-ERROR.
       ...
       RUN repositionObject IN h_bvpifiltype ( 2.67 , 1.00 ) NO-ERROR.
       RUN resizeObject IN h_bvpifiltype ( 9.05 , 58.00 ) NO-ERROR.
       ...
       RUN addLink ( h_bvpifiltype , 'Sortera':U , h_sortsok ).

       /* Adjust the tab order of the smart objects. */
       RUN adjustTabOrder ( h_sortsok ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjustTabOrder ( h_bvpifiltype ,
             h_sortsok , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  IF cFelt <> "" THEN
  DO:
    DYNAMIC-FUNCTION('assignQuerySelection':U IN h_dvpifiltype,
...
------------------------------------------------------------------------------*/
  ...
  IF cReturn-Value <> "AVBRYT" THEN  
    ASSIGN
      cColValues = DYNAMIC-FUNCTION('colValues':U IN h_dvpifiltype,
                     INPUT "VPIFilTypeNr,VPIFiltypeKNavn" /* CHARACTER */).  
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  PUBLISH "Sortera" FROM h_bvpifiltype.

  DYNAMIC-FUNCTION('findRowWhere':U IN h_dvpifiltype,
     INPUT cFelt    + (IF cFelt = "" THEN "" ELSE ",") + "VPIFilTypeNr",
     INPUT cVerdier + (IF cVerdier = "" THEN "" ELSE CHR(1)) + cStart,
     INPUT "EQ,EQ" /* CHARACTER */).
  /* Signalerer til de andre objektene. */
  RUN dataAvailable IN h_dvpifiltype
