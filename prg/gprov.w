...
DEFINE VARIABLE h_bprov AS HANDLE NO-UNDO.
DEFINE VARIABLE h_dprov AS HANDLE NO-UNDO.
...
DEFINE FRAME gDialog
     ...
     Btn_Cancel AT ROW 16.24 COL 37
...
ON GO OF FRAME gDialog /* Søkeliste provisjonskoder */
DO:
        ASSIGN    
          cReturn-Value = "OK"    
          .
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL gDialog gDialog
ON WINDOW-CLOSE OF FRAME gDialog /* Søkeliste provisjonskoder */
DO:  
...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             INPUT  'sdo/dprov.wDB-AWARE':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'AppServiceASUsePromptASInfoForeignFieldsRowsToBatch200CheckCurrentChangedyesRebuildOnReposnoServerOperatingModeNONEDestroyStatelessnoDisconnectAppServernoObjectNamedprovUpdateFromSourceno':U ,
             OUTPUT h_dprov ).
       RUN repositionObject IN h_dprov ( 14.57 , 21.00 ) NO-ERROR.
       ...
       RUN constructObject (
             ...
             OUTPUT h_bprov ).
       RUN repositionObject IN h_bprov ( 2.91 , 1.00 ) NO-ERROR.
       RUN resizeObject IN h_bprov ( 13.10 , 51.00 ) NO-ERROR.
       ...
       /* Links to SmartDataBrowser h_bprov. */
       RUN addLink ( h_dprov , 'Data':U , h_bprov ).
       RUN addLink ( h_bprov , 'Update':U , h_dprov ).

       /* Links to SmartObject h_sortsok. */
       RUN addLink ( h_bprov , 'Sortera':U , h_sortsok ).

       /* Adjust the tab order of the smart objects. */
       RUN adjustTabOrder ( h_bprov ,
...
------------------------------------------------------------------------------*/
  ...
  IF cReturn-Value <> "AVBRYT" THEN  
   ASSIGN
       cColValues = DYNAMIC-FUNCTION('colValues':U IN h_dprov,
                      INPUT "ProvKod,ProvProc" /* CHARACTER */).  
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  PUBLISH "Sortera" FROM h_bprov.

  DYNAMIC-FUNCTION('findRowWhere':U IN h_dprov,
     INPUT cFelt    + (IF cFelt = "" THEN "" ELSE ",") + "ProvKod",
     INPUT cVerdier + (IF cVerdier = "" THEN "" ELSE CHR(1)) + cStart,
     INPUT "EQ,EQ" /* CHARACTER */).
  /* Signalerer til de andre objektene. */
  RUN dataAvailable IN h_dprov
