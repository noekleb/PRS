...
DEFINE VARIABLE h_bkunde AS HANDLE NO-UNDO.
DEFINE VARIABLE h_dkunde AS HANDLE NO-UNDO.
...
ON WINDOW-CLOSE OF FRAME gDialog /* Søkeliste kunderegister */
DO:  
...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             INPUT  'sdo/dkunde.wDB-AWARE':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'AppServiceASUsePromptASInfoForeignFieldsRowsToBatch200CheckCurrentChangedyesRebuildOnReposnoServerOperatingModeNONEDestroyStatelessnoDisconnectAppServernoObjectNamedkundeUpdateFromSourceno':U ,
             OUTPUT h_dkunde ).
       RUN repositionObject IN h_dkunde ( 15.29 , 34.00 ) NO-ERROR.
       /* Size in AB:  ( 1.86 , 10.80 ) */

       RUN constructObject (
             INPUT  'prg/bkunde.w':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'ScrollRemotenoDataSourceNamesUpdateTargetNamesHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_bkunde ).
       RUN repositionObject IN h_bkunde ( 2.91 , 1.00 ) NO-ERROR.
       RUN resizeObject IN h_bkunde ( 13.10 , 77.00 ) NO-ERROR.
       ...
       /* Links to SmartDataBrowser h_bkunde. */
       RUN addLink ( h_dkunde , 'Data':U , h_bkunde ).
       RUN addLink ( h_bkunde , 'Update':U , h_dkunde ).

       /* Links to SmartObject h_sortsok. */
       RUN addLink ( h_bkunde , 'Sortera':U , h_sortsok ).

       /* Adjust the tab order of the smart objects. */
       RUN adjustTabOrder ( h_bkunde ,
...
------------------------------------------------------------------------------*/
  ...
  IF cReturn-Value <> "AVBRYT" THEN  
    ASSIGN
      cColValues = DYNAMIC-FUNCTION('colValues':U IN h_dkunde,
                     INPUT "KundeNr,Navn,Adresse1,PostNr,Telefon,MobilTlf,KontE-Post,fuPostSted" /* CHARACTER */).  
...
------------------------------------------------------------------------------*/
  ...
  PUBLISH "Sortera" FROM h_bkunde.
  DYNAMIC-FUNCTION('findRowWhere':U IN h_dkunde,
     INPUT cFelt     + (IF cFelt     = "" THEN "" ELSE ",")    + "RowObject.KundeNr",
