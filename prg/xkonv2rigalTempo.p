...
DEF VAR piLevNr       AS INT INIT 500 NO-UNDO.
...
DEFINE TEMP-TABLE TT_Input NO-UNDO
    ...
    FIELD Momskod      AS CHAR
    FIELD Moms%Txt     AS CHAR
    FIELD EANnamn      AS CHAR
...
/*         VIEW-AS ALERT-BOX INFO BUTTONS OK. */
    ...
    FOR EACH TT_Input:
        ASSIGN cRigalstr = cRigalEntries
               ...
               ENTRY(6,cRigalstr)   = TT_Input.Lev
               ENTRY(7,cRigalstr)   = TT_Input.Artnr
               ENTRY(10,cRigalstr)  = '"' + TRIM(SUBSTR(TT_Input.Benamning,1,30)) + '"'
               ENTRY(11,cRigalstr)  = '"' + TRIM(SUBSTR(TT_Input.EANnamn,1,20)) + '"'
               ENTRY(12,cRigalstr)  = '"' + TRIM(SUBSTR(TT_Input.EANnamn,1,30)) + '"'
...
/*   IMPORT STREAM InnFil UNFORMATTED cLinje. /* Läs bort första raden! Rigal Header, testet om Rigal i TellOppLinjer */ */
  ...
  REPEAT:
    ...
    DO:
      ...
      NEXT LESERLINJER.
    END.
    ASSIGN tmpTT_Input.Moms%  = TRIM(STRING(DECI(tmpTT_Input.Moms%Txt),">>>>9.99"))
           tmpTT_Input.Inpris = TRIM(tmpTT_Input.AAFpris).
           tmpTT_Input.Inpris = TRIM(tmpTT_Input.Inpris).
