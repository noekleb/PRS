...
DEFINE VARIABLE h_bmedlem AS HANDLE NO-UNDO.
DEFINE VARIABLE h_dmedlem AS HANDLE NO-UNDO.
...
DEFINE FRAME gDialog
     ...
     Btn_Help AT ROW 19.33 COL 88
     SPACE(0.00) SKIP(0.04)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste medlemsregisteret"
...
ON WINDOW-CLOSE OF FRAME gDialog /* Søkeliste medlemsregisteret */
DO:  
...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             INPUT  'sdo/dmedlem.wDB-AWARE':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'AppServiceASUsePromptASInfoForeignFieldsRowsToBatch200CheckCurrentChangedyesRebuildOnReposnoServerOperatingModeNONEDestroyStatelessnoDisconnectAppServernoObjectNamedmedlemUpdateFromSourceno':U ,
             OUTPUT h_dmedlem ).
       RUN repositionObject IN h_dmedlem ( 18.62 , 49.00 ) NO-ERROR.
       /* Size in AB:  ( 1.86 , 10.80 ) */

       RUN constructObject (
             INPUT  'prg/bmedlem.w':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'ScrollRemotenoDataSourceNamesUpdateTargetNamesHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_bmedlem ).
       RUN repositionObject IN h_bmedlem ( 1.00 , 1.00 ) NO-ERROR.
       RUN resizeObject IN h_bmedlem ( 18.10 , 102.00 ) NO-ERROR.

       /* Links to SmartDataBrowser h_bmedlem. */
       RUN addLink ( h_dmedlem , 'Data':U , h_bmedlem ).
       RUN addLink ( h_bmedlem , 'Update':U , h_dmedlem ).

       /* Adjust the tab order of the smart objects. */
       RUN adjustTabOrder ( h_bmedlem ,
...
------------------------------------------------------------------------------*/
  ...
  IF cReturn-Value <> "AVBRYT" THEN  
    ASSIGN
      cColValues = DYNAMIC-FUNCTION('colValues':U IN h_dmedlem,
                     INPUT "MedlemsNr,ForNavn,EtterNavn" /* CHARACTER */).  
...
------------------------------------------------------------------------------*/
  ...
  DYNAMIC-FUNCTION('findRowWhere':U IN h_dmedlem,
     INPUT cFelt     + (IF cFelt     = "" THEN "" ELSE ",")    + "RowObject.MedlemsNr",
  ...
  RUN dataAvailable IN h_dmedlem
