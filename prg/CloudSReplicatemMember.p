...
DEFINE VARIABLE cMKlubbId AS CHARACTER   NO-UNDO.
   ...
   {syspara.i 200 20 1 cConnectionString}
   {syspara.i 200 20 2 cCompanyid}
   {syspara.i 200 20 3 cUserId} 
   {syspara.i 200 20 4 cPassword} 
   {syspara.i 200 20 5 RemoteSystemId} 
...
FUNCTION getNextidNumber RETURNS INT ():
...
IF cUserId     = "" THEN cUserId     = "PRS". 
IF cPassword   = "" THEN cPassword   = "PRSTest". 
IF cCompanyid  = "" THEN cCompanyid  = "200".
RemoteTableId  = "mMember" .
IF remoteSystemid = "" THEN RemoteSystemId = "PRS".
...
DO:
  ...
  DEFINE VARIABLE iMKlubbIdDefault  AS INTEGER INIT 1 NO-UNDO. /* 1 - GantExclusive */
  ...
  FOR EACH tt_medlem NO-LOCK BY tt_Medlem.eDato BY tt_Medlem.eTid : 
      ...
      FIND FIRST medlem WHERE medlem.medlemsnr = tt_medlem.medlemsnr NO-LOCK NO-ERROR.
      IF NOT AVAIL medlem THEN
          lCreateNewMedlem = TRUE. 
      ...
      DO:
          ...
          LOG-MANAGER:WRITE-MESSAGE("Update:" + STRING( medlem.MedlemsNr),"UPDATE").
      ...
      ELSE  IF lCreateNewMedlem THEN 
      DO:

            CREATE Medlem.
            Medlem.MedlemsNr       = getNextidNumber(). 
            Medlem.Registrertdato  = tt_Medlem.edato. 
            Medlem.RegistrertTid   = tt_medlem.etid.  
            Medlem.Registrertav    = "CloudS". 
            Medlem.MKlubbId        = iMKlubbIdDefault.
            RUN setExternalIdField.p ON hServer (cSessionId, cCompanyId,RemoteTableId,RemoteSystemId,
                 INPUT STRING(medlem.Medlemsnr),
                 INPUT STRING(tt_medlem.regkode),
                 OUTPUT lok). 
            ...
            ASSIGN 
                ...
                Medlem.Telefon           = tt_Medlem.Telefon  
                Medlem.FodselsDato       = tt_Medlem.FodselsDato. 
                Medlem.Bonus_Berettiget  = FALSE.  /* mottatt gave = false */   
                Medlem.Kilde             = 'CloudS'.
                Medlem.Edato             = tt_Medlem.edato. 
                Medlem.eTid              = tt_medlem.etid. 
