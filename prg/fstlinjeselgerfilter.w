...
/*        cLabels = "Selger,Beskrivelse,Periode,Solgt,Verdi solgt,Solgt%,Mva verdi,DbKr,Db%,Rabatter,Rabatt kr,VVarekost,Kunderekl,Kunderekl kr,Levrekl,Levrekl kr,Svinn,Svinn kr,Gjenkjøp,Gjenkjøp kr,Tilbud,Tilbud kr,Brekkasje,Brekkasje kr"                         */
...
&Scoped-Define ENABLED-OBJECTS Tg-VisPeriode B-Avdeling B-SelgerBlank ~
RS-SumLevel B-Aktiver B-AvdelingBlank B-HgBlank B-VgBlank Tg-VisPerBut ~
B-HuvGr B-VarGr B-Selger 
&Scoped-Define DISPLAYED-OBJECTS Tg-VisPeriode FI-Selger RS-SumLevel ~
FI-Avdeling FI-HuvGr FI-VarGr Tg-VisPerBut 
...
DEFINE BUTTON B-Selger  NO-FOCUS
     LABEL "..." 
     SIZE 4.4 BY 1.

DEFINE BUTTON B-SelgerBlank 
     LABEL "Blank" 
     SIZE 8 BY 1.
...
DEFINE VARIABLE FI-Selger AS CHARACTER FORMAT "X(10)":U 
     LABEL "Selger" 
     VIEW-AS FILL-IN 
     SIZE 14 BY 1 NO-UNDO.
...
DEFINE FRAME fMain
     ...
     FI-Selger AT ROW 1.19 COL 34.8 COLON-ALIGNED
     B-SelgerBlank AT ROW 1.19 COL 56.8
     ...
     B-Selger AT ROW 1.19 COL 51.8 NO-TAB-STOP 
...
/* SETTINGS FOR FILL-IN FI-HuvGr IN FRAME fMain
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN FI-Selger IN FRAME fMain
   NO-ENABLE                                                            */
...
DO:
  ...
  IF NOT DYNAMIC-FUNCTION('getKriterier':U IN h_fstperiode,
     OUTPUT cKriterier /* CHARACTER */) THEN
      RETURN.
  IF FI-Selger <> "*" THEN
      ASSIGN pcFeltListe = "SelgerNr"
             pcVerdier   = FI-Selger.
  /* Om vi har avd,hg,vg filter och vi inte avgrænsat på Selger lægger vi in lev før enklare hantering i SDO */
  IF FI-Avdeling <> "*" OR FI-HuvGr <> "*" OR FI-VarGr <> "*" THEN
    ASSIGN pcFeltListe = pcFeltListe + (IF pcFeltListe <> "" THEN "," ELSE "SelgerNr,") + IF FI-Avdeling <> "*" THEN "AvdelingNr" ELSE IF FI-HuvGr <> "*" THEN "Hg" ELSE "Vg"
  ...
  cTilleggsFelter = "Butik,Butnamn,SelgerNr,SelgerNavn,AvdelingNr,AvdelingNavn,Hg,HgBeskr,Vg,VgBeskr".
  ...
  IF RS-SumLevel:SCREEN-VALUE <> "" THEN DO:
    ASSIGN iEntry          = 4
           ...
           ENTRY(1,cFieldDefs) = "SelgerNr;Selgernr;;1,SelgerNavn;Selger;;".
    CASE RS-SumLevel:SCREEN-VALUE:
        ...
        WHEN "Artikkel" THEN
            ASSIGN ENTRY(3,cFieldDefs) = "Artikkelnr;Artikkelnr;;1,ArtBeskr;Beskrivelse;;".
    END CASE.
  ...
  PUBLISH "LoadGrid" (cFileName,INT(getSumFelter("PerLinTxt")) - IF Tg-VisPerBut:CHECKED THEN 1 ELSE 0).  /* 3 = antall frozen cols  */
...
&Scoped-define SELF-NAME B-Selger
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-Selger fFrameWin
ON CHOOSE OF B-Selger IN FRAME fMain /* ... */
DO:
    DEFINE VARIABLE cRowIdList AS CHARACTER  NO-UNDO.
    DEFINE VARIABLE cIdList    AS CHARACTER  NO-UNDO.
    DEFINE VARIABLE bOK               AS LOGICAL    NO-UNDO.
    ...
    IF bOK THEN DO:
        assign
          FI-Selger:SCREEN-VALUE = if cIdList = ""
                            then cAlle
                          else "( " + STRING(NUM-ENTRIES(cIdList,"|")) + " )"
          FI-Selger     = if cIdList = ""
                            then "*"
                            else REPLACE(cIdList,"|",",")
          FI-Selger:TOOLTIP = IF FI-Selger = "*" THEN "" ELSE FI-Selger.
        IF FI-Selger <> "*" THEN
            ASSIGN SELF:PRIVATE-DATA = cRowIdList + CHR(1) + cIdList
               FI-Selger:BGCOLOR      = 11.
        ELSE
            ASSIGN SELF:PRIVATE-DATA = ""
               FI-Selger:BGCOLOR      = ?.
     END.
...
&Scoped-define SELF-NAME B-SelgerBlank
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-SelgerBlank fFrameWin
ON CHOOSE OF B-SelgerBlank IN FRAME fMain /* Blank */
DO:
    IF FI-Selger:SCREEN-VALUE <> cAlle THEN DO:
        ASSIGN FI-Selger:SCREEN-VALUE = cAlle
               FI-Selger              = "*"
               FI-Selger:TOOLTIP      = ""
               FI-Selger:BGCOLOR      = ?
               B-Selger:PRIVATE-DATA  = "".
    END.
    RETURN NO-APPLY.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY Tg-VisPeriode FI-Selger RS-SumLevel FI-Avdeling FI-HuvGr FI-VarGr 
          Tg-VisPerBut 
      WITH FRAME fMain.
  ENABLE Tg-VisPeriode B-Avdeling B-SelgerBlank RS-SumLevel B-Aktiver 
         B-AvdelingBlank B-HgBlank B-VgBlank Tg-VisPerBut B-HuvGr B-VarGr 
         B-Selger 
      WITH FRAME fMain.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ASSIGN cStTypeId = "SELGER"
             cFilename = SESSION:TEMP-DIRECTORY + "gridstlinje.txt"
             FI-Selger  = "*"
             FI-Selger:SCREEN-VALUE = cAlle
             FI-Avdeling  = "*"
...
------------------------------------------------------------------------------*/
  ...
  CASE ENTRY(1,ipKriterier):
      ...
      END.
  END CASE.
  ASSIGN cStTypeId = IF RS-SumLevel:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "" THEN "SELGER" ELSE 
                     IF RS-SumLevel:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "Artikkel" THEN "SELGER-ART" ELSE "SELGER-VG"
      cStTypeId = IF cStTypeId = "SELGER" AND (FI-Avdeling <> "*" OR FI-HuvGr <> "*" OR FI-VarGr <> "*") THEN "SELGER-VG" ELSE cStTypeId
      cQryString = 
