...
DEFINE VARIABLE h_blevbas AS HANDLE NO-UNDO.
DEFINE VARIABLE h_dlevbas AS HANDLE NO-UNDO.
...
DEFINE FRAME gDialog
     ...
     Btn_Cancel AT ROW 17.91 COL 46
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søk leverandør"
...
ON GO OF FRAME gDialog /* Søk leverandør */
DO:
    ASSIGN
      cReturn-Value = "OK"
      .
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL gDialog gDialog
ON WINDOW-CLOSE OF FRAME gDialog /* Søk leverandør */
DO:  
...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             INPUT  'sdo/dlevbas.wDB-AWARE':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'AppServiceASUsePromptASInfoForeignFieldsRowsToBatch200CheckCurrentChangedyesRebuildOnReposnoServerOperatingModeNONEDestroyStatelessnoDisconnectAppServernoObjectNamedlevbasUpdateFromSourcenoToggleDataTargetsyesOpenOnInityesPromptOnDeleteyesPromptColumns(NONE)':U ,
             OUTPUT h_dlevbas ).
       RUN repositionObject IN h_dlevbas ( 16.95 , 22.00 ) NO-ERROR.
       /* Size in AB:  ( 1.86 , 10.80 ) */

       RUN constructObject (
             INPUT  'prg/blevbas.w':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'ScrollRemotenoNumDown0CalcWidthnoMaxWidth80FetchOnReposToEndyesDataSourceNamesUpdateTargetNamesLogicalObjectNameHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_blevbas ).
       RUN repositionObject IN h_blevbas ( 2.91 , 1.00 ) NO-ERROR.
       RUN resizeObject IN h_blevbas ( 14.76 , 60.00 ) NO-ERROR.
       ...
       /* Links to SmartDataBrowser h_blevbas. */
       RUN addLink ( h_dlevbas , 'Data':U , h_blevbas ).

       /* Links to SmartObject h_sortsok. */
       RUN addLink ( h_blevbas , 'Sortera':U , h_sortsok ).

       /* Adjust the tab order of the smart objects. */
       RUN adjustTabOrder ( h_blevbas ,
...
------------------------------------------------------------------------------*/
  ...
  IF cReturn-Value <> "AVBRYT" THEN  
    ASSIGN
      cColValues = DYNAMIC-FUNCTION('colValues':U IN h_dlevbas,
                     INPUT "Levnr,Levnamn" /* CHARACTER */).  
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  PUBLISH "Sortera" FROM h_blevbas.
  
  DYNAMIC-FUNCTION('findRowWhere':U IN h_dlevbas,
     INPUT cFelt    + (IF cFelt = "" THEN "" ELSE ",") + "Levnr",
     INPUT cVerdier + (IF cVerdier = "" THEN "" ELSE CHR(1)) + cStart,
     INPUT "EQ,EQ" /* CHARACTER */).
  /* Signalerer til de andre objektene. */
  RUN dataAvailable IN h_dlevbas
