...
DEFINE VARIABLE h_bhandtering AS HANDLE NO-UNDO.
DEFINE VARIABLE h_dhandtering AS HANDLE NO-UNDO.
...
DEFINE FRAME gDialog
     Btn_OK AT ROW 12.19 COL 1
     Btn_Cancel AT ROW 12.19 COL 51
     SPACE(0.00) SKIP(0.09)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste håndteringskoder"
...
ON GO OF FRAME gDialog /* Søkeliste håndteringskoder */
DO:
      ASSIGN    
          cReturn-Value = "OK"    
          .
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL gDialog gDialog
ON WINDOW-CLOSE OF FRAME gDialog /* Søkeliste håndteringskoder */
DO:  
...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             INPUT  'sdo/dhandtering.wDB-AWARE':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'AppServiceASUsePromptASInfoForeignFieldsRowsToBatch200CheckCurrentChangedyesRebuildOnReposnoServerOperatingModeNONEDestroyStatelessnoDisconnectAppServernoObjectNamedhandteringUpdateFromSourceno':U ,
             OUTPUT h_dhandtering ).
       RUN repositionObject IN h_dhandtering ( 11.48 , 17.00 ) NO-ERROR.
       /* Size in AB:  ( 1.86 , 10.80 ) */

       RUN constructObject (
             INPUT  'prg/bhandtering.w':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'ScrollRemotenoDataSourceNamesUpdateTargetNamesHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_bhandtering ).
       RUN repositionObject IN h_bhandtering ( 2.91 , 1.00 ) NO-ERROR.
       RUN resizeObject IN h_bhandtering ( 9.05 , 65.00 ) NO-ERROR.
       ...
       /* Links to SmartDataBrowser h_bhandtering. */
       RUN addLink ( h_dhandtering , 'Data':U , h_bhandtering ).
       RUN addLink ( h_bhandtering , 'Update':U , h_dhandtering ).

       /* Links to SmartObject h_sortsok. */
       RUN addLink ( h_bhandtering , 'Sortera':U , h_sortsok ).

       /* Adjust the tab order of the smart objects. */
       RUN adjustTabOrder ( h_sortsok ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjustTabOrder ( h_bhandtering ,
             h_sortsok , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  SUBSCRIBE "MouseDblClick" IN h_bhandtering.

  /* Setter opp Where sats i Query.  */
  /* Legges inn der hvor det skal gjøres avgrensning I datasett for søk. */
  IF cFelt <> "" THEN
  DO:
    DYNAMIC-FUNCTION('assignQuerySelection':U IN h_dhandtering,
...
------------------------------------------------------------------------------*/
  ...
  IF cReturn-Value <> "AVBRYT" THEN  
    ASSIGN
      cColValues = DYNAMIC-FUNCTION('colValues':U IN h_dhandtering,
                     INPUT "HandKode,Beskrivelse" /* CHARACTER */).  
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  PUBLISH "Sortera" FROM h_bhandtering.

  DYNAMIC-FUNCTION('findRowWhere':U IN h_dhandtering,
     INPUT cFelt    + (IF cFelt = "" THEN "" ELSE ",") + "HandKode",
     INPUT cVerdier + (IF cVerdier = "" THEN "" ELSE CHR(1)) + cStart,
     INPUT "EQ,EQ" /* CHARACTER */).
  /* Signalerer til de andre objektene. */
  RUN dataAvailable IN h_dhandtering
