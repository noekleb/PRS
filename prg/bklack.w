...
&Scoped-define FIELDS-IN-QUERY-br_table beskrivning klack-id 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table beskrivning klack-id 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table bTableWin _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      beskrivning COLUMN-LABEL "Beskrivelse" FORMAT "x(30)":U WIDTH 35.6
      klack-id FORMAT ">>9":U WIDTH 5.2
  ENABLE
      beskrivning
      klack-id HELP "?"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN NO-AUTO-VALIDATE NO-ROW-MARKERS SEPARATORS SIZE 46 BY 6.67 FIT-LAST-COLUMN.
...
ASSIGN 
       rowObject.beskrivning:COLUMN-READ-ONLY IN BROWSE br_table = TRUE
       rowObject.klack-id:COLUMN-READ-ONLY IN BROWSE br_table = TRUE.
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[1]   > _<SDO>.rowObject.beskrivning
"beskrivning" "Beskrivelse" ? "character" ? ? ? ? ? ? yes ? no no "35.6" yes no yes "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > _<SDO>.rowObject.klack-id
"klack-id" ? ? "integer" ? ? ? ? ? ? yes "?" no no "5.2" yes no yes "U" "" "" "" "" "" "" 0 no 0 no no
