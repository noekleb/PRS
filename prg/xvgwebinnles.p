...
------------------------------------------------------------------------------*/
  ...
  REPEAT TRANSACTION:
    ...
    FIND Strekkode NO-LOCK WHERE
      Strekkode.Kode = EANNo NO-ERROR.
    IF NOT AVAILABLE Strekkode THEN 
      NEXT LESERLINJER.
    ELSE DO:
      ASSIGN
        TT_Lagerstatus.lArtikkelNr  = Strekkode.ArtikkelNr
        TT_Lagerstatus.iStrKode     = Strekkode.StrKode
        .
    END.
    ...
    IF TT_Lagerstatus.lArtikkelNr < 8500000 THEN
    DO:
       DELETE TT_Lagerstatus.
       NEXT LESERLINJER.
    END.
    ...
    /* Denne størrelseskoden skal ikke ha lager. */
    IF TT_Lagerstatus.iStrKode = 0 THEN
    DO:
       DELETE TT_Lagerstatus.
       NEXT LESERLINJER.
    END.
  ...
  pcBkuFil   = VPIFilHode.Katalog + "~\bku" + "\" + 
               VPIFilHode.FilNavn
  cButikkFil = cEkspKatalog + "~\" + 'HKWEBV' + 
               replace(STRING(TODAY,'99/99/99'),'/','') + 
               replace(STRING(TIME,'HH:MM:SS'),':','') + 
               '.csv'
  cButikkFil2 = cButikkFil
  .

  /* Legger ut redusert fil til butikkene. */
  OUTPUT TO VALUE(cButikkFil) NO-ECHO.
  FOR EACH TT_Lagerstatus WHERE
      TT_Lagerstatus.lArtikkelNr > 0:
      PUT UNFORMATTED 
          TT_Lagerstatus.ArticleNo ';' 
          TT_Lagerstatus.EANNo ';'
          TT_Lagerstatus.ArtName ';' 
          TT_Lagerstatus.AktivWeb ';'
          TT_Lagerstatus.SendtTilWeb ';'
          TT_Lagerstatus.lArtikkelNr ';' 
          TT_Lagerstatus.iStrKode SKIP. 
  END.
  OUTPUT CLOSE.

  /* Sender filen til alle CL butikker */
  IF bHK THEN
  FOR EACH Butiker NO-LOCK WHERE
      Butiker.Butik = Butiker.CL AND
      Butiker.ApningsDato <> ? AND
      Butiker.NedlagtDato = ?:

      ENTRY(NUM-ENTRIES(cButikkFil2,'.'),cButikkFil2,'.') = STRING(Butiker.Butik). 
      OS-COPY value(cButikkFil) 
              value(cButikkFil2).
  END.
  ...
  END.
  IF SEARCH(cButikkFil) <> ? THEN
      OS-DELETE VALUE(cButikkFil).
