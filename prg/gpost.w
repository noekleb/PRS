...
DEFINE VARIABLE h_bpost AS HANDLE NO-UNDO.
DEFINE VARIABLE h_dpost AS HANDLE NO-UNDO.
...
DEFINE FRAME gDialog
     Btn_OK AT ROW 18.62 COL 1
     Btn_Cancel AT ROW 18.62 COL 56
     SPACE(0.19) SKIP(0.04)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Søkeliste postnummer"
...
ON GO OF FRAME gDialog /* Søkeliste postnummer */
DO:
      ASSIGN    
          cReturn-Value = "OK"    
          .
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL gDialog gDialog
ON WINDOW-CLOSE OF FRAME gDialog /* Søkeliste postnummer */
DO:  
...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             INPUT  'sdo/dpost.wDB-AWARE':U ,
             INPUT  FRAME gDialog:HANDLE ,
             INPUT  'AppServiceASUsePromptASInfoForeignFieldsRowsToBatch200CheckCurrentChangedyesRebuildOnReposnoServerOperatingModeNONEDestroyStatelessnoDisconnectAppServernoObjectNamedpostUpdateFromSourceno':U ,
             OUTPUT h_dpost ).
       RUN repositionObject IN h_dpost ( 17.67 , 21.00 ) NO-ERROR.
       ...
       RUN constructObject (
             ...
             OUTPUT h_bpost ).
       RUN repositionObject IN h_bpost ( 2.91 , 1.00 ) NO-ERROR.
       RUN resizeObject IN h_bpost ( 15.48 , 70.00 ) NO-ERROR.
       ...
       /* Links to SmartDataBrowser h_bpost. */
       RUN addLink ( h_dpost , 'Data':U , h_bpost ).
       RUN addLink ( h_bpost , 'Update':U , h_dpost ).

       /* Links to SmartObject h_sortsok. */
       RUN addLink ( h_bpost , 'Sortera':U , h_sortsok ).

       /* Adjust the tab order of the smart objects. */
       RUN adjustTabOrder ( h_sortsok ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjustTabOrder ( h_bpost ,
             h_sortsok , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  IF cReturn-Value <> "AVBRYT" THEN  
   ASSIGN
       cColValues = DYNAMIC-FUNCTION('colValues':U IN h_dpost,
                      INPUT "PostNR,Beskrivelse" /* CHARACTER */).  
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  PUBLISH "Sortera" FROM h_bpost.

  DYNAMIC-FUNCTION('findRowWhere':U IN h_dpost,
     INPUT cFelt    + (IF cFelt = "" THEN "" ELSE ",") + "PostNr",
     INPUT cVerdier + (IF cVerdier = "" THEN "" ELSE CHR(1)) + cStart,
     INPUT "EQ,EQ" /* CHARACTER */).
  /* Signalerer til de andre objektene. */
  RUN dataAvailable IN h_dpost
