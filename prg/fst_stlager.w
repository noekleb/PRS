...
ASSIGN cArtFieldDefs = 
  /* xx */ "T_db%;T.db%;2;1," +
  /* xx */ "Pris;Pris;2;1," +
  /* xx */ "T_Lagerverdi;T.Lagerverdi;;1," +
...
B-VgBlank B-FargBlank Tg-VisButikker B-LevNr B-HuvGr B-VarGr B-Farg ~
B-Sesong BUTTON-SokBut FI-LagerListeTxT 
...
RS-DataUrval Tg-VisButikker FI-LagerListeTxT 
...
/* SETTINGS FOR BUTTON B-Lagerliste IN FRAME fMain
   NO-ENABLE                                                            */
/* SETTINGS FOR COMBO-BOX CB-Antall IN FRAME fMain
   NO-ENABLE                                                            */
...
/* SETTINGS FOR RADIO-SET RS-DataUrval IN FRAME fMain
   NO-ENABLE                                                            */
/* SETTINGS FOR TOGGLE-BOX TG-Alarm IN FRAME fMain
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
       TG-Alarm:HIDDEN IN FRAME fMain           = TRUE.
  ...
  DO WITH FRAME {&FRAME-NAME}:
      IF NOT DYNAMIC-FUNCTION('getKriterier':U,
         OUTPUT cKriterier /* CHARACTER */) THEN
          RETURN.
      ASSIGN B-Lagerliste:SENSITIVE = NOT RS-LagerType:SCREEN-VALUE = "BUTSTAT" AND
...
DO:
    DO WITH FRAME {&FRAME-NAME}:
        ASSIGN B-Sesong:SENSITIVE = SELF:SCREEN-VALUE = "ARTIKKEL"
               B-SesongBlank:SENSITIVE = SELF:SCREEN-VALUE = "ARTIKKEL"
               B-Farg:SENSITIVE = SELF:SCREEN-VALUE = "ARTIKKEL"
               B-FargBlank:SENSITIVE = SELF:SCREEN-VALUE = "ARTIKKEL"
/*                CB-Antall:SENSITIVE = TRUE /* SELF:SCREEN-VALUE = "ARTIKKEL" */ */
               B-LagerAntBlank:SENSITIVE = SELF:SCREEN-VALUE = "ARTIKKEL"
               TG-AvFilter:SENSITIVE     = SELF:SCREEN-VALUE = "ARTIKKEL"
...
/*                        Tg-VisButikker:CHECKED = FALSE. */
            END.
        END CASE.
        ASSIGN B-Lagerliste:SENSITIVE   = FALSE
...
ON VALUE-CHANGED OF TG-Alarm IN FRAME fMain /* Alarmliste neg lagerteller HIDDEN */
DO:
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-1 B-Avdeling B-AvdelingBlank B-LevNrBlank RS-Lagertype 
         ...
         Tg-VisButikker B-LevNr B-HuvGr B-VarGr B-Farg B-Sesong BUTTON-SokBut 
         FI-LagerListeTxT 
      WITH FRAME fMain.
...
/*     ASSIGN CB-Antall:LIST-ITEMS   = cAlle + ", < 0, <= 0, = 0, >= 0, > 0, <> 0" */
    ASSIGN CB-Antall:LIST-ITEMS   = cAlle
           CB-Antall              = cAlle
      ...
      DO:
          ...
          ASSIGN cSumCols = getSumFelter(cSummerFelter)
              /* entry 1=antal decimaler,2=col där resultatet skall stå,3= det som står över vid div, 4 = det som står under */
                 ...
                 cSumString = getSumFelter("CharButik") + ",SUM" .
...
/* DEFINE        VARIABLE  cTilPerLinNr    AS CHARACTER  NO-UNDO. */
    DO WITH FRAME {&FRAME-NAME}:
        ...
        END.
        IF RS-Lagertype:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "ARTIKKEL" THEN
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    ELSE DO:
        ...
        ASSIGN cQryString = SUBSTITUTE("FOR EACH StLager WHERE AarPerLinNr = 0 AND StTypeId = '&1'",RS-Lagertype:SCREEN-VALUE)
...
/*                       VIEW-AS ALERT-BOX INFO BUTTONS OK.                                   */
            ...
            DO iCount = 1 TO NUM-ENTRIES(cButiker):
                ...
                END.
                ELSE DO:
                    IF CB-Antall:SCREEN-VALUE <> cAlle AND NOT lage.lagant > 0 THEN
                        NEXT.
                END.
/*                 ASSIGN lOK = TRUE.                                                 */
