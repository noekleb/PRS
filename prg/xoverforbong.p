...
/*------------------------------------------------------------------------
    File        :  xoverforbong.p
    Purpose     :  Overf�ring av bonger fra kasse.
...
/* Logger bonger for overf�ring til Nets - MainCard */
...
IF NOT AVAILABLE Filer THEN
    RETURN " ** Ukjent filkobling p� datasett (" + STRING(lDataSettId) + ").".
...
DO:
  ...
  IF DataSett.Butik = 0 OR NOT AVAILABLE Butiker THEN
      RETURN " ** Ingen oppdatering gj�res p� butikk 0 (" + STRING(lDataSettId) + ").".
  /* Markerer Datasettet som under overf�ring/delhvis overf�rt */
  IF Butiker.StatistikkOppdatering = FALSE THEN
  DO TRANSACTION:
      FIND CURRENT DataSett EXCLUSIVE-LOCK.
      ASSIGN
          DataSett.Behandlet = 5. /* Overf�rt */
      FIND CURRENT DataSett NO-LOCK.
      RETURN "".
  END.
END. /* SJEKKSTAT */
   
/* T�mmer buffer for lagerkontroll */
FOR EACH tt_Lager:
  DELETE tt_Lager.
END.
...
/* Avgj�r om dagsrapporten skal posteres inklusive eller eksklusive mva */
idags_moms = 0.
...
/* Overf�ringsordre basert p� varesalg */
...
/* Faktor for beregning av DB p� grunnlag av oms inkl. mva, hvis ikke kalkyle finnes p� artikkelen. */
plDbFaktorBrutto = 0.
{syspara.i 6 4 3 plDbFaktorBrutto DEC}
IF plDbFaktorBrutto = 0 THEN
    plDbFaktorBrutto = 0.24.

/* Faktor for beregning av DB p� grunnlag av oms ekskl. mva, hvis ikke kalkyle finnes p� artikkelen. */
plDbFaktorNetto = 0.
...
PROCEDURE EODLogg :
/*------------------------------------------------------------------------------
  Purpose:     Utskrift av Bokf�ringsbilag og finansrapport.
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
...
FOR EACH BongLinje EXCLUSIVE-LOCK WHERE
    ...
    FOR EACH Kasse NO-LOCK WHERE 
        ...
        IF NOT CAN-FIND(EODKasse WHERE
            ...
            THEN bSettEOD = FALSE.
    END.
    /* Hvis alle kasser har levert EOD, skal bokf�ringsbilaget flagges med EOD mottatt. */
    IF bSettEOD THEN 
    DO:
    ...
    /* Skriver ut rapporten hvis det er noe � skrive ut. */
    ...
    DO:
        ...
        */
    END.
    ELSE DO: /* �pningsskjemahantering */
      /* vi markerar butiken som st�ngd i mottakskontrollen */
...
/*    RUN w-rkassarapportx.w PERSISTENT SET hRapport2.                   */
/*    RUN AutoInit IN hRapport2 (iRappType,BongHode.Butik,BongHode.Dato).*/
    RUN bibl_logg.p ('xOverforBong-EOD', 'Bokf�ringsbilag Start Butikk: ' + STRING(BongHode.Butik) + 
                                         ' Dato: ' + STRING(BongHode.Dato)).                          
    RUN dagsrapp_utskrift.p ("2", BongHode.Butik, BongHode.Dato, BongHode.Dato, TRUE, OUTPUT cFilnavn).
    RUN bibl_logg.p ('xOverforBong-EOD', 'Bokf�ringsbilag Slutt Butikk: ' + STRING(BongHode.Butik) + 
                                         ' Dato: ' + STRING(BongHode.Dato) + 
                                         ' Filnavn: ' + cFilnavn).                          
  END.
...
/* Er det en bong med varesalg p� etikett artikkel p�, skal bongen makuleres. */
ETIKETTSJEKK:
FOR EACH BongLinje EXCLUSIVE-LOCK WHERE
    BongLinje.B_Id = BongHode.B_Id AND
    BongLinje.Makulert = FALSE AND
...
END. /*ETIKETTSJEKK */
/* Makulerer bongen.                                                                       */
/* Dette er n�dvendig da selgerne ofte registrerer feil og f�rst legger inn varesalg feil. */
IF bEtiBong THEN
    MAKULER:
    DO:
      BongHode.Makulert = 2.
...
/* Legger alle finansrapporter for en dag inn p� samme ZNr. */
ASSIGN
    iZNr   = 1
    piTTId = 0.
...
IF CAN-FIND(FIRST BongLinje NO-LOCK WHERE
  ...
  /* Flagger betalingsmiddel som er benyttet p� bongen */
  DO:
...
/* Sjekker om det er kreditsalg p� bongen */
IF CAN-FIND(FIRST BongLinje NO-LOCK WHERE
    BongLinje.B_Id = BongHode.B_Id AND
...
FOR EACH BongLinje NO-LOCK WHERE 
    ...
    DO:
        ...
        DO:
            ...
            /* Er bel�pet negativt, skal det legges som tilgode ut. */
...
/*         END.                                                                             */
        ...
        DO:
            ...
            ELSE DO: /* Utbetaling */
                ...
                FOR EACH ttBongLinje:
                    ...
                    /* M� trekkes ned her, for � motvikre postering av betalingsmiddel. */
        ...
        /* Reservel�sning Bank som betalingsmiddel. */
        ...
        DO:
            /* Adderer en for ikke � f� 0 i indeks */
            ASSIGN
...
/* Det er sjekket p� forh�nd at det er kundesalg p� denne bongen.   */
/* Dvs bongen skal posteres.                                        */ 
/* Det forutsettes her at det er kontrollert at kunden    finnes og */
/* Her posteres n� salg og betalinger.                              */
...
DO:
    RUN NyFilLogg IN h_Logg (INPUT Filer.FilId, STRING(TODAY) + " " + 
                    ...
                    " - Ukjent kundenummer p� bong (KortNr/BongNr/KundeNr: " + 
                    BongHode.KundeKort + "/" + 
...
FOR EACH BongLinje NO-LOCK WHERE
    ...
    /* Sjekker om det er kreditsalg eller rekvisisjon p� bongen. */
    IF CAN-DO("055,065",STRING(BongLinje.TTId,"999")) THEN
        ASSIGN
          pbKredit    = TRUE
          bMotPostert = FALSE 
          .
    /* Sjekker om det er varesalg p� bongen. */
    IF CAN-DO("001",STRING(BongLinje.TTId,"999")) THEN
        ASSIGN
          pbVareSalg = TRUE 
          .
    /* Sjekker om det er innbetaling p� bongen */
    IF CAN-DO("089",STRING(BongLinje.TTId,"999")) THEN
        ASSIGN
          pbInnBet = TRUE 
          .
    /* Flagger at varesalgslinjene skal markeres som motpostert.           */
    /* Hvis det er b�de varesalg og innbetaling p� bongen, vil varelinjene */
    /* p� bongen alltid v�re betalt. Rekvisisjon og kredit kommer aldri    */
      ...
      */
      ...
      DO WHILE TRUE:
        FIND bKundeSaldo EXCLUSIVE-LOCK WHERE
            ...
            NO-ERROR NO-WAIT.
        /* Posten holdes av en annen, vi fors�ker igjen. */
...
FOR EACH BongLinje NO-LOCK WHERE
    ...
    /* Kun betalingstranser og utg�ende gavekort skal med. */
    IF (BongLinje.TTId >= 50 AND BongLinje.TTId <  90) OR 
        can-do("134",STRING(BongLinje.TTId)) THEN. /* Gj�r ingenting. */
    ELSE 
        NEXT BETALINGER.
    ...
    DO TRANSACTION:
      ...
      ASSIGN
        ...
        .
      /* Er det salg av gavekort (134), skal dette h�ndteres spesielt.          */
      ...
      /* Disse skal ikke p�virke saldo. */
      IF CAN-DO("059,065,070,079",STRING(bKundeBetTrans.TTID,"999")) THEN
          bKundeBetTrans.Motpostert = TRUE.

      /* Disse skal alltid p�virke saldo p� en bong med kreditsalg. */
...
DEFINE VARIABLE cTransactionId AS CHARACTER FORMAT "x(20)" NO-UNDO.
  ...
  /* Spesifikasjoner for kuponginnl�sens bonger. */
...
/* Det er sjekket p� forh�nd at det er medlemssalg p� denne bongen. */
/* Dvs bongen skal posteres.                                        */ 
...
DO:
    RUN NyFilLogg IN h_Logg (INPUT Filer.FilId, STRING(TODAY) + " " + 
                    STRING(TIME,"HH:MM:SS") + " " + USERID("skotex") + 
                    " - Ukjent medlemsnummer p� bong (KortNr/BongNr/MedlemsNr: " + 
...
FOR EACH BongLinje EXCLUSIVE-LOCK WHERE
    ...
    DO:
        ...
        /* Her skal vi ikke finne noe kort fra f�r. */
        ...
        DO ON ERROR UNDO, RETRY:
            ...
            DO:
                ...
                ASSIGN
                    ...
                    Gavekort.IdentType  = 1 /* Alltid 1 n�r det opprettes fra kassen */
                    NO-ERROR.
                IF ERROR-STATUS:ERROR THEN
    ...
    DO:
        ...
        IF lDec = 0 THEN 
            pcIdentNr = STRING(BongLinje.TransDato) + '-' + STRING(BongLinje.KasseNr) + '-' + STRING(BongLinje.BongNr) + '-' + STRING(BongLinje.LinjeNr).
        /* Vi m�ste f�rst hitta mha piButikkNr som �r den som st�llt ut vid v�rt gavekortsnr */
        IF LENGTH(Bonglinje.strekkode) = 22 THEN DO:
            ...
            IF NOT AVAIL Gavekort THEN
                FIND FIRST GaveKort EXCLUSIVE-LOCK WHERE
                    Gavekort.IdentNr = pcIdentNr NO-ERROR.
        END.
        /* om vi inte hittat skal vi finne et kort fra f�r. */
        IF NOT AVAIL Gavekort THEN
            FIND GaveKort EXCLUSIVE-LOCK WHERE
        ...
        OPPRETT:
        DO ON ERROR UNDO, RETRY:
            /* Finner vi kortet og det er brukt fra f�r, m� nytt kort opprettes. */
...
/* TANSNR og SEQNR p�f�rs bonglinjene her.                     */
BONGLINJE:
FOR EACH BongLinje EXCLUSIVE-LOCK WHERE
    BongLinje.B_Id = BongHode.B_Id AND
    BongLinje.Makulert = FALSE AND
...
/*------------------------------------------------------------------------------
  ...
  /* Sendes det inn et kortnr, skal det benyttes. Hvis ikke benyttes det som st�r i bonghode. */
  IF cKort = '' THEN
  ...
  DO:
    ...
    DO:
      ...
      ASSIGN
        ...
        .
      /* Tar bort kundekort som opprettes i trigger da vi har f�tt inn kundekort fra kassen. */
      IF cKort <> '' THEN
    ...
    RUN genkundeMedlem.p (BongHode.ButikkNr,
                          ...
                          OUTPUT bOk,
                          OUTPUT cMsgs).
    /* Kundekortene legges opp p� samme kunde, men unike medlemskort */
    /* legges p� separate medlemmer.                              */
...
FOR EACH BongLinje EXCLUSIVE-LOCK WHERE
    ...
    DO:
        ...
        DO ON ERROR UNDO, RETRY:
            ...
            /* f�r bongen hvor det er utstedt.                             */
            pdecLoop = 0.
            ...
            DO:
                ...
                ASSIGN
                    ...
                    NO-ERROR.
                /* Oppst�r det feil, tar vi en sving til. */
                IF ERROR-STATUS:ERROR THEN
                DO:
    ...
    DO:
        ...
        IF NOT lExtern THEN DO:
            ...
            /* Sjekker om vi finne et kort fra f�r. */
            IF LENGTH(pcIdentNr) > 6 
            THEN FIND FIRST Tilgode NO-LOCK WHERE
                Tilgode.IdentNr = pcIdentNr NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  /* Leser alle bongene p� datasettet som ikke er overf�rt fra f�r. */
  ...
  FOR EACH BongHode OF DataSett EXCLUSIVE-LOCK WHERE
      ...
      /* Oppretter bokf�ringsbilag og setter nummer.                    */
      /* Det skal finnes et bilag for butikk og dato. Gj�r det ikke det */
      ...
      /* Flagger den som under overf�ring. */
      ASSIGN
          BongHode.BongStatus = 6 /* Delhvis overf�rt. */
      ...
      /* Spesifiserer returer p� kassarapporten. */
      RUN SpesAvReturer.
      ...
      /* Kundeordre registrert p� kasse */
      RUN Kundeordre.
      
      /* Poster kreditsalg som faktura.                                                             */
      /* plFaktura_Id settes her. Det benyttes til � koble faktura og pakkseddel i Overforingslogg. */
      ...
      /* Postering av overf�ringer i overf�ringsordre */
      RUN Overforingslogg.

      /* Oppdaterer TransLogg.                                            */
      /* NB: Denne m� kj�res f�r logging av medlems og kundesalg kanskje. */
      /*     TransNr og SeqNr p�f�res bonglinjene her.                    */
      /* NB: Her flagges ogs� om bongen er ferdig motpostert.             */
      RUN TransLogg.
      ...
      /* Poster medlemskj�p - Grunnlag for bonusberegning. */
      IF BongHode.MedlemsNr <> 0 OR BongHode.Medlemskort <> "" THEN
      ...
      /* Skriver finans og bokf�ringsbilag */
      RUN EODLogg /*SkrivFinans*/.

      /* Skriver ut reservasjonsbilag.                                 */
      /* Dette skal skrives ut n�r nettbutikk overf�rer varer til seg. */
      RUN reservasjonsBilag.

      /* Logger bonger i Nets. */
      RUN loggNets.
      
      /* Logger bonger fra Kuponginnl�sen */
      RUN loggKuponginnlosen.
      
      /* Logger bonger til MayFlower */
      RUN loggMayFlower.
      
      /* Flagger den ferdig overf�rt. */
      ASSIGN
  ...
  /* Markerer Datasettet som overf�rt */
  DO TRANSACTION:
...
/*       DYNAMIC-FUNCTION("runproc","beregn_kunde_saldo.p","idlist|" + STRING(ttKundeBut.KundeNr),?). */
      ...
      /* Gammel saldoberegning - skal tas bort n�r alle kunder har f�tt ny fakturarutine. */
      RUN beregnkundesaldo.p (ttKundeBut.KundeNr, ttKundeBut.ButikkNr).
  END.

  /* Ajourf�rer MedlemsSaldi */
  FOR EACH ttMedlemBut:
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       Det er f�rst opprettet en faktura. plFaktura_Id peker p� den faktura 
               som er utstedt som f�lge av overf�ringen. 
------------------------------------------------------------------------------*/
  DEF VAR pcTTId           AS CHAR NO-UNDO.
  DEF VAR piLinjeNr        AS INT  NO-UNDO.
  ...
  ASSIGN
      lPkSdlId  = 0
      piLinjeNr = 1.
  /* Logger overf�ringer fra kassen. */
  TRANSRAD:
  FOR EACH BongLinje NO-LOCK WHERE
      BongLinje.B_Id = BongHode.B_Id AND
      BongLinje.Makulert = FALSE AND
      ...
      /* TN 28/10-01 Valutapris ikke omregnet legges inn her. Ref. G�ran hos JF. */
      ...
      /* Henter butikk det overf�res til. */
      ...
      /* Varekost ved overf�ring, skal hentes fra gjeldende kalkyle.             */
      /* Er mottagende butikk sentrallageret, skal varekost f�r rabatt benyttes. */
      ...
      ASSIGN TT_OvBuffer.BuntNr      = 999 /* dummy, kan vara vad som helst */
          ...
          TT_OvBuffer.Varekost    = (IF bInnkjopsPris THEN lKalkVarekost ELSE vVareKost) /* �ndrat 17-09-10 fr�n raderna nedan */
...
/*              TT_OvBuffer.Varekost    = (IF TT_OvBuffer.Varekost = 0 THEN lKalkVarekost ELSE TT_OvBuffer.Varekost) /* Kostpris fra kalkyle. */ */
      ...
      IF plMinusbutikk = FALSE THEN
          ASSIGN
            ...
            .
      /* Overf�ring fra kassen som markerer for lite mottatte varer.       */
      /* Her snur vi retningen p� overf�ringen. Butikken skal motta varer. */
      ELSE DO:
          ASSIGN
  ...
  DO:
      /* Henter record for � f� p� plass riktig butikknr ogs� ved +/- butikk overf�ringer. */
      ...
      DO:
          ...
          DO:
              ...
              /* Overf�res det til sentrallageret (But 20), oversk.lager nettbutikk (but 50) eller +/- butikker, skal pakkseddelen innleveres umiddelbart. */
          ...
          /* Kommer bongen fra bakrom ved oppdatering av overf�ringsordre, skal det ikke opprettes ny ordre. */
          ...
          DO:
              ...
              RUN LagraOvBuffer.p (INPUT-OUTPUT iBuntNr,
                                   0,
                                   "N" + CHR(1) + "Overf�ring kasse " + STRING(TODAY) + STRING(TIME,"HH:MM") + CHR(1) + "N",
                                   wEDB-System,
                                   wTabell,
                                   7).
          END. /* OPPRETTOVBUNT */
          ...
          /* Overf�res det til sentrallager (But 20), og det finnes en outlet, skal det legges opp en pakksedel. */
          /* Dette skal gj�res uavhengig av bBrukTBId2 parameteren.                                              */
          ...
          /* Overf�res det til overskuddslager (But 50) fra nettbutikkens lager, skal det legges opp en pakksedel p� nettbutikkens lager. */
          /* Dette skal gj�res uavhengig av bBrukTBId2 parameteren.                                                                       */
...
/* Henter kunden som er satt inn p� bongen. */
FIND Kunde NO-LOCK WHERE
    Kunde.KundeNr = BongHode.KundeNr NO-ERROR.
...
/* NY PRS POS Flagger at det er en overf�ring og setter kundenr. for mottagende butikk. */
IF CAN-FIND(FIRST BongLinje WHERE
                  BongLinje.B_Id = BongHode.B_id AND
...
DO:
    ...
    /* Overf�ringer mellom nettbutikk og nettbutikks ventelager, skal ikke generere faktura. */
    ...
    IF AVAILABLE bufButiker AND BongHode.KundeNr = 0 THEN 
      DO TRANSACTION:
        ...
        /* Denne er normalt satt inn i bonghode, men det gj�res hvis dette ikke er gjort. */  
        FIND Kunde NO-LOCK WHERE
    ...
    /* Ved overf�ring m� internfakturering v�re satt p� begge butikker for at faktura skal opprettes. */
    IF Butiker.IntFaktOverforing = FALSE OR 
       bufButiker.IntFaktOverforing = FALSE THEN 
        RETURN.
                
    /* Ved overf�ring m� ogs� kunde p� mottagende butikk v�re opprettet og koblet til butikken. */ 
    IF NOT AVAILABLE Kunde THEN 
        RETURN.
END.
...
POSTER-FAKTURA:
DO:
    /* Er det en overf�ring og kunden representerer en Outlet, skal ikke faktura utstedes her.  */
    /* Faktura utstedes da f�rst ved varemottaket, samtidig med at overskuddslager trekkes ned. */
    ...
    /* Er det ikke kredittsalg eller overf�ringer p� bongen skal den ikke behandles her. */
    IF NOT CAN-FIND(FIRST BongLinje WHERE
                    BongLinje.B_Id = BongHode.B_id AND
                    CAN-DO("006,065,087",STRING(BongLinje.TTId,"999"))) THEN
        LEAVE POSTER-FAKTURA.
    /* Flagger at det er en varetransaksjon. Skal ha tilsvarende behandling som overf�ring med hensyn til betalingstransaksjonen. */
    IF CAN-FIND(FIRST BongLinje WHERE
                      BongLinje.B_Id = BongHode.B_id AND
...
/*        MESSAGE                                                              */
/*        PROGRAM-NAME(1)  SKIP                                                */
/*        "FEIL - Hentet Id p� ukjent faktura: " RETURN-VALUE SKIP plFaktura_Id*/
/*        VIEW-AS ALERT-BOX INFO BUTTONS OK.                                   */
    ...
    DO:
        ...
        DO TRANSACTION:
            ...
            /* Vi m� her overstyre bilagstype, som er satt tilbake til 1 i overst�ende INIT */
    ...
    /* Posterer bonglinjene p� faktura. */
    KREDITSALG:
    FOR EACH BongLinje NO-LOCK WHERE
        BongLinje.B_Id = BongHode.B_Id AND
        BongLinje.Makulert = FALSE:
        ...
        /* Er det overf�ring, skal ikke betalingstransaksjonen med */
            ...
            DO:
                ...
                ASSIGN
                    ...
                    FakturaLinje.Varetekst     = IF FakturaLinje.TTId = 134
                                                   THEN "Gavekort p�lydende kr: " + STRING(BongLinje.LinjeSum / BongLinje.Antall) 
    ...
    FAKTURASUM:
    DO:
        /* Legger p� fakturagebyr p� vanlige faktura. Ikke p� kreditnota. */
        ...
        /* P�f�rer og beregner rabatt. Rabatt pr. linje tildeles automatisk n�r totalrabatt <> 0 blir satt. */
    ...
    DO:
        ...
        DO:
            ...
            DO TRANSACTION:
                ...
                DO:         
                    /* Dette er tilfelle ved innlevering av pakkseddel p� outlet hvor outlet faktureres fra sentrallager. */  
...
/*------------------------------------------------------------------------------
  ...
  Parameters:  <none>
  Notes:       Kas_Rap og BongLinje er tilgjengelige n�r rutine kalles.
------------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------------
                        Purpose: Postering av medlemskj�p. Danner grunnlag for beregning
                                 av medlemsbonus.                                                                                                                                         
                        Notes:   Rutinene MedlemsSalg og Kundesalg som posterer i 
                                 MedTrans og KundeTrans er overfl�dige. De er erstattet
                                 av indeks p� kunde og medlem som er lagt inn p� Translogg.                                                                                                                               
        ------------------------------------------------------------------------------*/
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH BongLinje NO-LOCK WHERE
      ...
      ASSIGN
          ...
          .
      /* Vanlig salg skal bare logges n�r det er registrert reklamasjonskode. */
...
/*           IF BongLinje.FeilKode = 0 THEN */
      ...
      DO TRANSACTION:
          ...
          /* Beregner Mva av rabattbel�pet for reklamasjonsrabatt */
...
/* Overf�ringstransaksjon fra nettbutikk, med neg. antall, er reservasjon. */
IF CAN-FIND(FIRST BongLinje WHERE
    BongLinje.B_Id = BongHode.B_Id AND
    BongLinje.Makulert = FALSE AND
...
DO:
    ...
    DO:
        /* Er det nettbutikk, skal reservasjon sendes p� eMail. */
...
------------------------------------------------------------------------------*/
  ...
  /* Henter varekost i butikken det overf�res fra. */      
  ...
  IF pdVVareKost = 0 THEN /* or wBrutto% *** Skal ogs� utf�res for brutto% artikkler */
    DO:
      IF VALID-HANDLE(h_PrisKo) THEN
...
/*------------------------------------------------------------------------------
  ...
  DO:
      /* TN Her opprettes kunder som ikke finnes fra f�r. */
      IF NOT CAN-FIND(FIRST KundeKort WHERE
                      KundeKort.KortNr = BongHode.KundeKort)
        /* Er medlemskortet lagt p� plass, legges det inn her. Hvis ikke p�f�res det senere i SjekkMedlem. */ 
  ...
  DO:
      /* Sjekker om medlemskortet finnes og om det er det samme medlemmet som er lagt opp p� bongen fra f�r. */
      /* Medlemsnr. er blitt p�f�rt i transtype 62 hvis kunden ble opprettet.                                */
      FIND FIRST Medlemskort NO-LOCK WHERE
        MedlemsKort.KortNr = BongHode.MedlemsKort NO-ERROR.
      IF AVAILABLE MedlemsKort THEN 
        DO:
          /* Finnes medlemmet og medlemsnr er forskjellig fra det som er lagt p� ved behandling av transtype 62, skal */
          /* medlemsnummeret byttes ut. Er det ikke lagt inn medlemsnr, skal det p�f�res.                             */
      ...
      /* Ukjent medlemskort.                                                  */                                                   
      ELSE DO:
        /* Er det lagt opp en kunde, skal det legges opp nytt medlem p� kunden. */
...
/* Disse varene skal inn i kundeordre, og f�rst trekkes fra lager n�r    */
/* de faktureres.                                                        */
IF CAN-FIND(FIRST BongLinje WHERE 
                BongLinje.B_Id = BongHode.B_Id AND
                BongLinje.Makulert = FALSE AND
...
ASSIGN
    flVaremottak = FALSE
    .
/* Er det en bong med etiketter p�, skal allt p� bongen som ikke  */
/* er etiketter ignoreres.                                        */
/* Dette gj�res hvis etikett er registrert som vanlig vareslag p� */
/* samme bongen.                                                  */
ETIKETTSJEKK:
FOR EACH BongLinje EXCLUSIVE-LOCK WHERE
    BongLinje.B_Id = BongHode.B_Id AND
...
/* Legger opp transaksjonene i translogg. Kun salgstransaksjoner. */
/* TRANSNR og SEQNR p�f�rs bonglinjene her.                       */
SKAPELSEN:
FOR EACH BongLinje EXCLUSIVE-LOCK WHERE
    BongLinje.B_Id = BongHode.B_Id AND
    BongLinje.Makulert = FALSE AND
    ...
    IF AVAILABLE ArtBas THEN
      DO:
        /* Sjekker om artikkelen har v�rt nullstilt. */
    ...
    /* Sjekker om det er overf�ring til minusbutikk.         */
    /* Overf. fra kasse til minusbutikk skal g� andre veien. */
    ...
    /* Er pakkseddelen ikke mottatt fra f�r, innleveres den. Mens det */
    /* alltid skrives ut etiketter p� hele pakkseddelen.              */
    ...
    DO:
        ...
        DO WHILE TRUE ON ERROR UNDO, RETRY:
            ASSIGN TransLogg.Butik        = piButikkNr
                   ...
                   TransLogg.SeqNr        = 1
                   /* Setter inn pekere p� transaksjonene */
        ...
        ASSIGN
               ...
               .
        /* Overstyrer for 002-Brekkasje, 005-Varekj�p, 006-Overf�ring og 011-Internt forbruk. */
        /* Disse transaksjonene skal h�ndteres til varekost.                  */
...
/*------------------------------------------------------------------------------
                        Purpose:                                                                                                                                          
                        Notes:          Her h�ndteres varemottak i kasse. Bongen posteres som en 
                                  pakkseddel. I kassen er det regsitrert ordre og pakkseddelnr, 
                                  samt at alle linjene er skannet inn som varelinjer p� bongen.
                                  
                                  Denne rutinen skal ikke forveksles med rutinen som 
                                  tar h�ndt om transkode 25 i subrutine Translogg. Der er det bare
...
FOR EACH tt2BongLinje:
    DELETE tt2BongLinje.
END.
/* Legger opp ttBongLinje for overf�ring til varemottaksrutine. */
FOR EACH BongLinje WHERE
         BongLinje.B_Id = BongHode.B_Id AND
         BongLinje.Makulert = FALSE:
    CREATE tt2BongLinje.
...
/* Behandler varemottak. Oppretter pakkseddel, posterer varemottak, skriver ut etiketter og gj�r innleveranse. */
