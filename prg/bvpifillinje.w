...
rowObject.Behandlet rowObject.StorTekst rowObject.FilId 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table rowObject.LinjeNr ~
rowObject.Behandlet 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table bTableWin _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      LinjeNr FORMAT "->,>>>,>>9":U
      Behandlet COLUMN-LABEL "Beh" FORMAT "yes/no":U
      StorTekst FORMAT "X(160)":U
      FilId FORMAT ">>>>>>>>>>>>9":U
  ENABLE
      LinjeNr
      Behandlet
...
ASSIGN 
       rowObject.LinjeNr:COLUMN-READ-ONLY IN BROWSE br_table = TRUE
       rowObject.Behandlet:COLUMN-READ-ONLY IN BROWSE br_table = TRUE.
...
"LinjeNr" ? ? "integer" ? ? ? ? ? ? yes ? no no ? yes no yes "U" "" ""
     _FldNameList[2]   > _<SDO>.rowObject.Behandlet
"Behandlet" "Beh" ? "logical" ? ? ? ? ? ? yes ? no no ? yes no yes "U" "" ""
     _FldNameList[3]   = _<SDO>.rowObject.StorTekst
