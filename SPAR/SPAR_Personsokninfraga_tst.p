...
/*     {syspara.i 14 1 8 cXsdfil} */
    IF cXsdfil = '' THEN 
        cXsdfil = "c:\appdir\se\SPAR\PS_SPARpersonsokningfraga_test.xsd".

/*     {syspar2.i 14 1 8 cEndPoint} */
    IF cEndPoint = '' THEN 
        cEndPoint = "https://kt-ext-ws.statenspersonadressregister.se/spar-webservice/SPARPersonsokningService/".
...
/*                     cTempFileName.    */
/*     OUTPUT CLOSE.                     */
/*     message "2 res" cTempFileResult view-as alert-box. */
    cTempFileResult = "c:\tmp\" + cTempFileResult.
...
/* cTempFileResult = "c:\appdir\se\" + cTempFileResult. */
/*     message "2 res" cTempFileResult view-as alert-box. */
...
/*     message SEARCH(cFileName) view-as alert-box. */
...
------------------------------------------------------------------------------*/	
     ...
     RETURN "SPAR_Restult_" + STRING(YEAR(TODAY),"9999") +  STRING(MONTH(TODAY),"99") + STRING(DAY(TODAY),"99") + STRING(TIME) + ".xml".
...
------------------------------------------------------------------------------*/    
     ...
     RETURN "SPAR_Request_" + STRING(YEAR(TODAY),"9999") +  STRING(MONTH(TODAY),"99") + STRING(DAY(TODAY),"99") + STRING(TIME) + ".xml".
